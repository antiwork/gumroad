...
for (const v of views) {
  ...
  for (const leg of legs) {
    ...
    const finalSlug = finalUrl.split("/l/")[1]?.split(/[?#]/)[0] ?? "(none)";
    const bodyText = await page.evaluate(() => document.body.innerText.slice(0, 300));
    const is404 = /Page not found|Something broke|404/i.test(bodyText);
    ...
    await page.evaluate(({ req, st, fin }) => {
...
const ctx2 = await browser.newContext({ viewport: { width: 1440, height: 1000 }, deviceScaleFactor: 2, ignoreHTTPSErrors: true });
...
console.log(l2); log.push(l2);
if (r2.status() !== 200) throw new Error("ABORT bare domain: " + l2);
await p2.evaluate(({ req, st, fin }) => {
  const d = document.createElement("div");
  d.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:2147483647;background:#111;color:#0f0;font:13px/1.5 ui-monospace,Menlo,monospace;padding:8px 10px;white-space:pre-wrap;border-bottom:2px solid #0f0";
