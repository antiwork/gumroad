...
async function stripeText(page) {
  ...
  for (const f of page.frames()) {
    ...
    try { t += await f.evaluate(() => (document.body ? document.body.innerText : "")); } catch { /* detached */ }
  }
  return t;
}
    ...
    try {
      off = await f.evaluate((src) => {
        ...
        const n = Array.from(document.querySelectorAll("*"))
          .find((e) => e.children.length === 0 && rx.test((e.textContent || "").trim()));
...
for (const leg of legs) {
  for (const v of views) {
    ...
    say(`LEG ${leg.desc}/${v.name} total=${JSON.stringify(total)} upi_in_element=${upiVisible} element_inr=${inrElement} total_inr=${inrTotal} MISMATCH=${inrElement && !inrTotal}`);
