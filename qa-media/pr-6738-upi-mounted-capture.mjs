...
const propsOf = (page) => page.evaluate(() => {
...
async function stripeText(page) {
  ...
  for (const f of page.frames()) {
    ...
    try { t += await f.evaluate(() => (document.body ? document.body.innerText : "")); } catch {}
  }
  return t;
}

const legs = [
  { permalink: "qaupi",    desc: "membership-upi-mounted", label: "THIS PR's shape: INR Autopay membership, flag on, Indian buyer" },
...
for (const v of views) {
  for (const leg of legs) {
    ...
    say(`${v.name.padEnd(8)} ${leg.desc.padEnd(24)} UPI=${upi} recurring_upi_registration=${props.recurring_upi_registration} element=${props.currency}/${props.presentment_amount_cents} summaryTotal=${total} MISMATCH=${mismatch}`);

    if (!upi) throw new Error(`ABORT ${key}: UPI not mounted`);

    await page.evaluate(({ label, types, cur, amt, upi, total, subtotal, rur, mismatch }) => {
      const d = document.createElement("div");
      d.style.cssText = "position:fixed;top:0;left:0;right:0;z-index:2147483647;background:#111;color:#0f0;font:12px/1.45 ui-monospace,Menlo,monospace;padding:8px 10px;white-space:pre-wrap;border-bottom:2px solid " + (mismatch ? "#f33" : "#0f0");
      d.textContent =
        `${label}\n` +
    ...
    }, { label: leg.label, types: JSON.stringify(props.types), cur: props.currency, amt: props.presentment_amount_cents, upi, total, subtotal, rur: props.recurring_upi_registration, mismatch });
...
say(results["desktop/onetime-control"].mismatch === false ? "CONTROL consistent (₹) => the membership mismatch is shape-specific, not a fixture artifact"
