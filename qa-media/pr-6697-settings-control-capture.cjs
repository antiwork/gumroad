...
(async () => {
  ...
  console.log('MARK control before=' + before);
  ...
  console.log('MARK control alert=' + JSON.stringify(alert));
  if (/won't accept payouts/.test(alert)) throw new Error('ABORT a DIFFERENT address was refused — guard is not address-keyed');
  ...
  console.log('MARK control persisted=' + persisted);
  ...
  console.log('MARK shot control ' + Math.round(fs.statSync(f).size / 1024) + 'KB');
