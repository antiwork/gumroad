...
async function readSummary(page) {
  return await page.evaluate(() => {
    ...
    return {
      ...
      discounts: (txt.match(/US\$-([0-9.,]+)/) || [])[1] || null,
      total: grab('Total'),
      hasFixedChip: /QA6750FIXED/i.test(txt),
      lines: [...new Set((txt.match(/QA6750 Widget [AB]/g) || []))],
    };
  });
}
...
(async () => {
  ...
  for (const dev of [
    ...
    log(`${dev.tag} leg=${LEG} subtotal=${s.subtotal} discounts=-${s.discounts} total=${s.total} chip=${s.hasFixedChip} lines=${s.lines.length}`);

    if (s.subtotal !== '20') throw new Error(`${dev.tag}/${LEG}: subtotal ${s.subtotal} != 20`);
    if (s.discounts !== EXPECT.discount) throw new Error(`${dev.tag}/${LEG}: discount -${s.discounts} != -${EXPECT.discount}`);
