...
const LEGS = [
  ...
  { tag: 'inr-onetime-control', slug: 'qaupione', label: 'INR one-time (pre-existing lane, control)', expectUpi: true },
];

async function serverProps(page) {
  return page.evaluate(() => {
    const raw = document.documentElement.innerHTML.replace(/&quot;/g, '"');
    const i = raw.indexOf('"checkout_payment"');
    if (i < 0) return null;
    const seg = raw.slice(i, i + 900);
    const pick = (k) => { const m = seg.match(new RegExp('"' + k + '":(\\[[^\\]]*\\]|"[^"]*"|true|false|null|\\d+)')); return m ? m[1] : null; };
    return {
      integration: JSON.parse(pick('integration')), fallback_reason: JSON.parse(pick('fallback_reason')),
...
async function paymentMenuText(page) {
  ...
  for (const f of page.frames()) {
    ...
    try { frameText.push(await f.evaluate(() => (document.body ? document.body.innerText : ''))); } catch (e) { /* detached */ }
  }
...
(async () => {
  ...
  for (const leg of LEGS) {
    for (const [vp, dims] of [['desktop', { width: 1440, height: 1200 }], ['mobile', { width: 375, height: 900 }]]) {
      const ctx = await browser.newContext({
        ignoreHTTPSErrors: true, viewport: dims, deviceScaleFactor: 2,
        isMobile: vp === 'mobile', hasTouch: vp === 'mobile',
      });
      const page = await ctx.newPage();
      let errs = 0;
      page.on('pageerror', () => { errs++; });
      await page.goto(`${ROOT}/checkout?product=${leg.slug}&quantity=1`, { waitUntil: 'domcontentloaded', timeout: 120000 });
      await page.waitForTimeout(18000);

      const props = await serverProps(page);
      if (!props) throw new Error(`ABORT ${leg.tag} ${vp}: no checkout_payment props`);
      if (props.payment_method_types === null) throw new Error(`ABORT ${leg.tag} ${vp}: server did not choose client-confirm`);
      ...
      } catch (e) { P(`     (scroll: ${e.message.slice(0, 50)})`); }
      await page.waitForTimeout(4000);

      const { menu, page: pageText } = await paymentMenuText(page);
      const upiVisible = /\bUPI\b/i.test(menu) || /\bUPI\b/.test(pageText);
      const paypalVisible = /PayPal/i.test(pageText);
      ...
      rows.push({ leg: leg.label, vp, ...props, upiVisible, paypalVisible, total, file: file.split('/').pop() });
      await ctx.close();
    }
  }
  await browser.close();
  ...
  P('ALL-LEGS-CAPTURED');
})().catch((e) => { console.log('RUN-FAILED', e.message); process.exit(1); });
