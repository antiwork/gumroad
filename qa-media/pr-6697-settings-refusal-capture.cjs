const { BASE, launch, login, finish } = require('./pr-6697-preview-qa-lib.cjs');
...
async function shot(page, name) {
  ...
  console.log(`MARK shot ${name} ${kb}KB`);
  return f;
}
...
async function leg({ width, tag }) {
  ...
  page.on('pageerror', (e) => console.log('PAGEERROR', tag, String(e)));
  ...
  console.log(`MARK ${tag} props=${JSON.stringify(props)}`);

  // The PayPal method must already be the selected payout method (seeded payment_address).
  const emailId = await page.evaluate(() => {
    const i = Array.from(document.querySelectorAll('input[type="email"]')).find((e) => /paypal-email/.test(e.id));
    return i ? i.id : null;
  });
  ...
  console.log(`MARK ${tag} typed=${typed}`);
  if (typed !== REFUSED) throw new Error('ABORT typed value did not stick');

  let saveStatus = null;
  page.on('response', (r) => {
    if (/\/settings\/payments/.test(r.url()) && r.request().method() !== 'GET') saveStatus = r.status();
  });
  ...
  console.log(`MARK ${tag} alert=${JSON.stringify(alert)} http=${saveStatus}`);
...
(async () => {
  ...
  console.log('MARK done desktop_alert_matches=' + (a === b));
