...
describe Checkout::StripePaymentPresenter do
  ...
  describe "Payment Element confirm integration" do
    ...
    it "selects the scoped CardElement restoration for a recurring membership even with both flags" do
      ...
      # (The UPI Autopay registration shape is the one exemption; the INR membership examples
      # below pin that it stays on the client-confirm lane.)
  ...
  describe "method-forced test-mode QA surface (iDEAL/Bancontact)" do
    ...
    it "mounts the scoped CardElement restoration for a recurring EUR-priced presentment candidate instead of crashing" do
      ...
      # 1856) before either the method-forced or presentment checks run (the UPI Autopay
      # registration shape is the one exemption, and this EUR cart is not it), so the crash
      # this example originally pinned (the method-forced shape check scanning a resolver's
      # nil method list without consulting its eligibility verdict first) is no longer
      # reachable from this cart shape — but the regression coverage that this recurring,
      # EUR-priced, presentment-candidate cart resolves without raising is still worth keeping.
      seller = create(:user, disable_buyer_local_currency: false)
