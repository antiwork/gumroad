...
describe AffiliatedProductsPresenter do
  ...
  describe "pending invitations" do
    ...
    context "with pending invitations" do
      let!(:pending_affiliate) { create(:direct_affiliate, :pending, affiliate_user: user, seller: seller, products: [product]) }
      let(:presenter) { described_class.new(user) }

      it "includes pending invitations in the response" do
        props = presenter.affiliated_products_page_props

        expect(props[:pending_invitations]).to be_present
        expect(props[:pending_invitations].length).to eq(1)

        invitation = props[:pending_invitations].first
    ...
    context "with denied affiliates" do
      let!(:denied_affiliate) { create(:direct_affiliate, :denied, affiliate_user: user, seller: seller, products: [product]) }
      let(:presenter) { described_class.new(user) }

      it "excludes denied affiliates from both lists" do
        props = presenter.affiliated_products_page_props
