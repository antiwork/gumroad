...
describe SellerMobileAnalyticsService do
  ...
  describe "#process" do
    it "serializes historical currencies in all-time sales without converting the displayed amount" do
      ...
      purchase.update_columns(displayed_price_currency_type: "huf", displayed_price_cents: 1250)
      ...
      expect(result[:purchases].sole[:price]).to eq "12.50 Ft"
      expect(purchase.reload.formatted_display_price).to eq "12.50 Ft"
      expect(purchase.as_json[:currency_symbol]).to eq "Ft"
