...
describe Purchase::CreateService, :vcr do
  ...
  describe "transparent subscription restart" do
    # Note: Active subscription check is now in Order::CreateService
    # See spec/services/order/create_service_spec.rb for those tests
    let(:membership_product) { create(:subscription_product, user:, price_cents: price) }
    let(:membership_params) do
      base_params[:purchase][:email] = buyer.email
      base_params[:purchase][:perceived_price_cents] = price
      base_params[:purchase].merge!(
        card_data_handling_mode: "stripejs.0",
        credit_card_zipcode: zip_code,
        chargeable: successful_card_chargeable
    ...
    context "when buyer has a cancelled subscription" do
      ...
      it "transparently restarts the subscription instead of creating a new one" do
        ...
        expect(existing_subscription.deactivated_at).to be_nil
      end
    end

    context "when buyer has no existing subscription" do
      it "creates a new subscription purchase normally" do
        expect(Subscription::RestartAtCheckoutService).not_to receive(:new)
    ...
    context "when the purchase is a gift" do
      ...
      let!(:giftee_subscription) do
        existing_purchase = create(:membership_purchase, link: membership_product, email: "giftee@example.com")
        subscription = existing_purchase.subscription
        subscription.update!(
          cancelled_at: 1.day.ago,
          cancelled_by_buyer: true,
      ...
      it "does not check for restartable subscriptions (gifts create new subscriptions)" do
        expect(Subscription).not_to receive(:restartable_for_user_and_product)
        expect(Subscription).not_to receive(:active_for_user_and_product)

        # The gift creation will fail validation but that's expected
        # The important thing is we don't try to restart subscriptions for gifts
        Purchase::CreateService.new(
          product: membership_product,
          params: gift_params,
          buyer: buyer
    ...
    context "when matching by email (logged out user)" do
      let!(:existing_subscription) do
        existing_purchase = create(:membership_purchase, link: membership_product, email: "returning@example.com")
        subscription = existing_purchase.subscription
        subscription.update!(
          cancelled_at: 1.day.ago,
          cancelled_by_buyer: true,
      ...
      it "restarts subscription when email matches" do
