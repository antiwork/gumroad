...
describe Purchase::CreateService, :vcr do
  ...
  context "with offer code" do
    ...
    describe "default discount code tracking" do
      ...
      it "sets used_default_discount_code to true when the default offer code is used" do
        params[:purchase].merge!(discount_code: default_offer_code.code)
        service = Purchase::CreateService.new(product:, params:)
        purchase = service.send(:build_purchase, params[:purchase])
      ...
      it "sets used_default_discount_code to false when a different offer code is used" do
        ...
        params[:purchase].merge!(discount_code: other_offer_code.code)
        service = Purchase::CreateService.new(product:, params:)
        purchase = service.send(:build_purchase, params[:purchase])

        expect(purchase.offer_code).to eq(other_offer_code)
        expect(purchase.used_default_discount_code).to be false
      end

      it "defaults used_default_discount_code to false when no discount code is used" do
        service = Purchase::CreateService.new(product:, params:)
        purchase = service.send(:build_purchase, params[:purchase])
