...
describe Purchase::HandleFailedRefundService, "concurrency" do
  ...
  # Combined-charge version of the lock-cycle scenario. Charge#refund_and_save! commits
  # each purchase separately for durability, but still locks this purchase and every
  # later one in id order before Stripe or balance work. That keeps a sibling
  # failed-refund reversal queued behind purchase₂ instead of holding it while the
  # charge refund waits for the shared balance (purchase₁ → balance × purchase₂ → balance).
  it "does not create a lock cycle between a combined-charge refund and a sibling reversal" do
    ...
    note_charge_purchase_lock = lambda do |purchase_id|
      locked_by_charge_refund << purchase_id
      # Fires once the charge refund holds BOTH purchase rows (remaining-id pre-pass);
      # with payout holding the balance it then blocks before balance work.
      if (locked_by_charge_refund.uniq - [@purchase.id, purchase_two.id]).empty? && locked_by_charge_refund.uniq.size == 2
        charge_refund_locked_purchases << true
      end
    end
    # Charge pre-pass uses reload(lock: true); refund_purchase! / reversal still use lock!.
    allow_any_instance_of(Purchase).to receive(:reload).and_wrap_original do |method, *args, **kwargs|
      options = args.first.is_a?(Hash) ? args.first : kwargs
      result = method.call(*args, **kwargs)
      if options && options[:lock] && Thread.current[:refund_concurrency_role] == :charge_refund
        note_charge_purchase_lock.call(method.receiver.id)
      end
      result
    end
    allow_any_instance_of(Purchase).to receive(:lock!).and_wrap_original do |method, *args|
      ...
      when :charge_refund
        note_charge_purchase_lock.call(purchase_id)
      when :reversal
    ...
    begin
      ...
      # interlocking on the shared balance.
