...
describe Pages::ProductPrices do
  ...
  describe ".build" do
    ...
    # However high the cap, the check reads redemptions actually made, so a spent code is
    # spent at any cap size. A cap large enough to look like "unlimited" is not a licence to
    # quote a price checkout rejects.
    it "leaves an exhausted default offer code on the shelf however large its cap" do
      offer_code = seller.offer_codes.create!(code: "big", amount_percentage: 50, products: [product],
                                              max_purchase_count: 10_001)
      product.update!(default_offer_code: offer_code)
      create(:purchase, link: product, offer_code:, seller:, quantity: 10_001)
    ...
    it "keeps the discount on a high-capped code with uses remaining" do
      offer_code = seller.offer_codes.create!(code: "big", amount_percentage: 50, products: [product],
                                              max_purchase_count: 10_001)
      product.update!(default_offer_code: offer_code)
