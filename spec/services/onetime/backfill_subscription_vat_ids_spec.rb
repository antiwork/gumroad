...
describe Onetime::BackfillSubscriptionVatIds do
  describe ".process" do
    ...
    it "backfills VAT ID from original purchase's VAT refund" do
      subscription = create(:subscription, link: product, business_vat_id: nil)
      original_purchase = create(:free_purchase, is_original_subscription_purchase: true, link: product,
      ...
      expect { described_class.process }.to change { subscription.reload.business_vat_id }.from(nil).to("REFUND_VAT")
    end
    ...
    it "returns stats with count of backfilled subscriptions" do
      ...
      subscription3 = create(:subscription, link: product, business_vat_id: nil)
      ...
      # subscription3 has no VAT ID source
      create(:free_purchase, is_original_subscription_purchase: true, link: product, subscription: subscription3)
    ...
    it "continues processing if one subscription fails" do
      ...
      stats = nil
      expect { stats = described_class.process }.not_to raise_error
    ...
    it "supports dry run mode" do
      ...
      stats = described_class.process(dry_run: true)
