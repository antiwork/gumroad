...
describe SitemapService do
  ...
  describe "#generate_wishlists" do
    ...
    it "deletes the S3 sitemap object with the dedicated sitemap-uploader credentials when no wishlist remains indexable" do
      ...
      # The upload path in sitemap_config also constructs an AwsSdkAdapter that reads S3
      # credentials via GlobalConfig, but never touches Aws::S3::Client directly, so the
      # expectation above binds only to the deletion call this example is pinning.
