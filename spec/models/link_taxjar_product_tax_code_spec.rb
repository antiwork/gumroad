...
describe Link, "#taxjar_product_tax_code" do
  # The spec database is already seeded with the whole Discover taxonomy tree, so look the
  # existing nodes up instead of creating them — `create(:taxonomy, slug: ...)` raises
  # `Validation failed: Slug has already been taken`. Exercising the seeded tree is also what
