...
describe SendChargeReceiptJob do
  ...
  let(:purchase_one) { create(:test_purchase, link: product_one, seller: seller) }
  let(:product_two) { create(:product, user: seller, name: "Product Two") }
  let(:purchase_two) { create(:test_purchase, link: product_two, seller: seller) }
  ...
  context "with all purchases ready" do
    it "delivers the email and updates the charge" do
  ...
  context "when a purchase requires stamping" do
    ...
    it "still delivers the email without stamping" do
      described_class.new.perform(charge.id)
