...
describe HandleNewBankAccountWorker do
  describe "perform" do
    ...
    it "passes a name-only sync through to the manager" do
      expect(StripeMerchantAccountManager).to receive(:handle_new_bank_account).with(bank_account, holder_name_only: true)
      described_class.new.perform(bank_account.id, true)
    end

    it "calls StripeMerchantAccountManager.handle_new_bank_account with the bank account object" do
      expect(StripeMerchantAccountManager).to receive(:handle_new_bank_account).with(bank_account, holder_name_only: false)
    ...
    it "raises (triggering Sidekiq retry) when the manager returns :stripe_unknown_error" do
      allow(StripeMerchantAccountManager).to receive(:handle_new_bank_account).with(bank_account, holder_name_only: false).and_return(:stripe_unknown_error)
    ...
    it "does not raise when the manager returns a classified outcome" do
      %i[synced noop_metadata_match invalid_account_holder_name invalid_bank_account stripe_invalid_request].each do |outcome|
        allow(StripeMerchantAccountManager).to receive(:handle_new_bank_account).with(bank_account, holder_name_only: false).and_return(outcome)
