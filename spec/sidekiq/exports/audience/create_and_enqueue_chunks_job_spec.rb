...
describe Exports::Audience::CreateAndEnqueueChunksJob do
  ...
  let(:export) { create(:audience_export, seller: seller, recipient: recipient, audience_options: { "followers" => true }) }
  ...
  describe "#perform" do
    context "with audience members" do
      ...
      it "creates chunks and enqueues ProcessChunkJob for each" do
        ...
        expect(Exports::Audience::ProcessChunkJob).to have_enqueued_sidekiq_job(export.chunks.first.id)
        expect(Exports::Audience::ProcessChunkJob).to have_enqueued_sidekiq_job(export.chunks.second.id)
      end

      it "deletes stale chunks on retry" do
        create(:audience_export_chunk, export: export, member_ids: [999])
        ...
        expect(export.chunks.count).to eq(2)
        expect(export.chunks.pluck(:member_ids).flatten).not_to include(999)
      end
    end
    ...
    context "with different audience types" do
      ...
      it "only includes followers when followers option is set" do
        ...
        expect(export.chunks.count).to eq(1)
        expect(export.chunks.first.member_ids.size).to eq(1)
      end

      it "includes both when both options are set" do
        export.update!(audience_options: { "followers" => true, "customers" => true })
        described_class.new.perform(export.id)
        export.reload
