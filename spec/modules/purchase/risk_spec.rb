...
describe Purchase::Risk do
  ...
  describe "#check_for_fraud" do
    before do
      ...
      PlatformBlock.add!(object_type: BLOCKED_OBJECT_TYPES[:ip_address], object_value: "192.378.12.1", expires_in: 1.hour)
    end
    ...
    it "returns errors if the buyer browser_guid has been blocked" do
      browser_guid = "abc123"
      PlatformBlock.add!(object_type: BLOCKED_OBJECT_TYPES[:browser_guid], object_value: browser_guid, expires_in: 1.hour)
    ...
    it "returns errors if the seller ip_address has been blocked" do
      PlatformBlock.add!(object_type: BLOCKED_OBJECT_TYPES[:ip_address], object_value: "123.121.11.1", expires_in: 1.hour)
    ...
    describe "ip_address check" do
      ...
      before do
        PlatformBlock.add!(object_type: BLOCKED_OBJECT_TYPES[:ip_address], object_value: blocked_ip_address, expires_in: 1.hour)
      end
    ...
    describe "#check_for_past_blocked_email_domains" do
      ...
      context "when it is a paid product" do
        ...
        it "returns error if the purchaser's email domain has been blocked" do
          PlatformBlock.add!(object_type: BLOCKED_OBJECT_TYPES[:email_domain], object_value: Mail::Address.new(purchaser.email).domain)
        ...
        context "when it is a gift purchase" do
          ...
          it "returns error if the gift recipient's email domain has been blocked" do
            PlatformBlock.add!(object_type: BLOCKED_OBJECT_TYPES[:email_domain], object_value: "giftee.com")
          ...
          it "returns error if the gift sender's email domain has been blocked" do
            PlatformBlock.add!(object_type: BLOCKED_OBJECT_TYPES[:email_domain], object_value: "gifter.com")
