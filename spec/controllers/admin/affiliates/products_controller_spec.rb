...
describe Admin::Affiliates::ProductsController, inertia: true do
  ...
  describe "GET index" do
    before do
      get :index, params: { affiliate_id: affiliate_user.external_id }
    end

    it "returns successful response with Inertia page data" do
      expect(response).to be_successful
      expect(inertia.component).to eq("Admin/Affiliates/Products/Index")
      expect(inertia.props[:products].map { _1[:id] }).to contain_exactly(published_product.external_id, unpublished_product.external_id)
