...
describe Subscriptions::MagicLinksController, inertia: true do
  ...
  describe "GET new (magic_link)" do
    it "renders the magic link page" do
      ...
      expect(inertia.props[:subscription_id]).to eq(@subscription.external_id)
      expect(inertia.props[:product_name]).to eq(@product.name)
      expect(inertia.props[:is_installment_plan]).to eq(@subscription.is_installment_plan)
    end
  end

  describe "POST create (send_magic_link)" do
