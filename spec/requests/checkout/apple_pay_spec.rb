...
describe "Checkout with Apple Pay", :js, type: :system do
  ...
  context "when Apple Pay is available" do
    it "shows Apple Pay option with brand icon" do
    ...
    it "hides card input fields when Apple Pay is selected" do
      visit product.long_url
      add_to_cart(product)

      expect(page).to have_field("Card number", wait: 10)
    ...
    it "shows Pay button when Apple Pay is selected" do
      visit product.long_url
      add_to_cart(product)
  ...
  context "when Apple Pay is not available" do
    before do
      clear_payment_request_mocks
      mock_payment_request_availability(apple_pay: false, google_pay: false)
    end

    it "does not show Apple Pay option" do
      visit product.long_url
      add_to_cart(product)
  ...
  context "with physical products" do
    ...
    it "shows Apple Pay option for physical products" do
    ...
    it "collects shipping address when using Apple Pay" do
      visit physical_product.long_url
      add_to_cart(physical_product)
  ...
  context "with recurring products" do
    ...
    it "shows Apple Pay option for subscriptions" do
