...
describe "Checkout cart", :js, type: :system do
  ...
  describe "email typo validation" do
    before do
    ...
    context "when user enters an email with a typo" do
      it "shows a popup and disables checkout until acknowledged" do
        # Enter email with typo (gmail.co instead of gmail.com)
        fill_in "Email address", with: "test@gmail.co"

        # Trigger blur event by clicking outside
        find("body").click
        ...
        # Click "No" to keep the typo
        click_on "No"

        # Popup should disappear
        expect(page).to_not have_text("Did you mean test@gmail.com?")

        # Checkout button should be enabled again
        expect(page).to have_button("Pay", disabled: false)
      end

      it "updates email when user accepts the suggestion" do
        # Enter email with typo
        fill_in "Email address", with: "test@gmai.com"
        ...
        # Click "Yes" to accept suggestion
        click_on "Yes"

        # Email should be updated
        expect(find_field("Email address").value).to eq("test@gmail.com")
      ...
      it "does not show popup again for acknowledged emails" do
        # Enter email with typo
        fill_in "Email address", with: "test@hotmai.com"

        # Trigger blur event
        find("body").click

        # Popup should appear
        expect(page).to have_text("Did you mean test@hotmail.com?")
    ...
    context "when user is signed in" do
      it "does not show typo popup for account email" do
        user = create(:user, email: "typo@gmai.com")
        login_as(user)
        ...
        expect(find_field("Email address", disabled: true).value).to eq("typo@gmai.com")

        # No popup should appear even though email looks like a typo
        expect(page).to_not have_text("Did you mean")
      ...
      it "shows typo popup when signed-in user enters different email" do
        user = create(:user, email: "correct@gmail.com")
        login_as(user)

        visit @product.long_url
        add_to_cart(@product)

        # Clear pre-filled email and enter different one with typo
        fill_in "Email address", with: "other@gmai.com"

        # Trigger blur event
        find("body").click

        # Popup should appear for the different email
        expect(page).to have_text("Did you mean other@gmail.com?")
    ...
    context "when email changes after acknowledgement" do
      it "clears the typo suggestion when email is edited" do
        # Enter email with typo
        fill_in "Email address", with: "test@yahooo.com"

        # Trigger blur event
        find("body").click

        # Popup should appear
        expect(page).to have_text("Did you mean test@yahoo.com?")

        # Start typing a different email
        fill_in "Email address", with: "different@example.com"

        # Previous popup should disappear
        expect(page).to_not have_text("Did you mean test@yahoo.com?")

        # Checkout button should be enabled (no typo in new email)
        expect(page).to have_button("Pay", disabled: false)
      end
    end
  end
