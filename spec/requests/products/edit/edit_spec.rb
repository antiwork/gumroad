...
describe("Product Edit Scenario", type: :system, js: true) do
  ...
  it "does not allow publishing when creator's email is empty" do
    ...
    click_on "Publish and continue"
    expect(page).to have_alert(text: "To publish a product, we need you to have an email. Set an email to continue.", wait: 2)
    expect(page).to have_link("Set an email", href: settings_main_url(host: UrlService.domain_with_protocol), wait: 2)
