...
describe "Admin::UsersController Scenario", type: :system, js: true do
  ...
  describe "blocked user indicator" do
    ...
    it "shows blocked user indicator with appropriate tooltips for email and domain blocks" do
      ...
      # Block by email
      PlatformBlock.create!(object_type: BLOCKED_OBJECT_TYPES[:email], object_value: user.form_email, blocked_at: Time.current, blocked_by: admin.id)
      page.refresh
      ...
      # Add domain block
      PlatformBlock.create!(object_type: BLOCKED_OBJECT_TYPES[:email_domain], object_value: user.form_email_domain, blocked_at: Time.current, blocked_by: admin.id)
      page.refresh
