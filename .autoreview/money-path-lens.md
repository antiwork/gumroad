Round-8 panel of gumroad#7229 head a78ba680d7428f956f531e18873dc0e33f7190d2
("Move the checkout currency picker above the summary line items.").

This is a MONEY PATH. Displayed currency vs charged currency must be identical.
Prior panel at 38c3ce1fa89f6181db5f1764d9187a5575c964c9 (r7) is STALE by the file-list gate:
git diff 38c3ce1fa..a78ba680d7 --name-only -- app lib config db is NON-EMPTY
(app/javascript/components/Checkout/index.tsx — picker moved above summary line items).
Do not rubber-stamp because the delta is a layout move. Re-verify every r7 finding against THIS tree.
...
R7 FINDINGS (checklist — mark RESOLVED / STILL-OPEN / REGRESSED with file:line on THIS tree):
...
NEW delta at this head (must also review):
9. CurrencyPicker now sits above Subtotal/line items instead of next to Total. Does the new wrapper (`div.border-b`) create a shown-vs-charged mismatch, a double-divider, or a visible empty cell when the picker returns null?
10. The new test only asserts document order. Does it still hide on willSaveCard / wallet / nil-quote? A layout-only example is not coverage of the money identity.
...
- two independent booleans (willSaveCard × wallet × quote-present) are a PRODUCT; ask for the truth table
