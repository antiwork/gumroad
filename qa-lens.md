...
Exact executed artifacts available for QA (test data only):

At the second commit's code design, 37 focused examples passed (RSPEC_EXIT=0), including 1,001 real factory-created MySQL products, real Redis scheduler state and real Elasticsearch document updates. Probe output:
...
Earlier expanded run: 222 examples, 0 failures (full OfferCode model, job/service and search result specs). Later review fixes require renewed verification; these numbers must not be advertised as current-head full-suite results.

Mutation baseline at edfa1cc36033e91047fc19859edd6af881c391d9: 3 examples, 0 failures. Four mutants each reddened the correct example: cooldown removal -> catchup bound; generation acknowledgement unconditionally -> concurrent final edit; exclusions ignored -> value parity; limit removed -> batch bound. Harness failed to restore its last mutant; explicitly restored both target files via git restore --source=HEAD --worktree, verified git diff --exit-code = 0. Application worktree was separate and untouched.

Development reviewer identified actual upstream Suo absent-key acquisition race and overbroad targeted-change scans; second commit replaced with atomic SET NX/token release and targeted hash. Separate QA round identified destroy rollback snapshot reuse, targeted starvation and post-error cooldown gap; fix round adds tests for all three.

Backend surface: only offer_codes search projection strings and scheduling change. No templates/UI, migrations, flags, config or secrets. Hosted visual evidence not applicable to unchanged rendering. Real search result regression verifies product detachment no longer matches the discount. No production latency claim or deployment claim. Money capable means human merge, not autonomous.

Followup fixes targeted regressions: 19 examples, 0 failures. Review complete current diff again; do not inherit earlier clean assertions.
