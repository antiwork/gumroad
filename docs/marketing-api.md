...
All endpoints require the `edit_emails` OAuth scope. The broad legacy `account` scope is not accepted. The seller must pass the same `Marketing::Eligibility` gate the product page uses — `auto_marketing` enabled and not in the holdout; otherwise every endpoint returns HTTP 404. Products and actions are scoped to the token's seller. No scheduler or marketing state lives in the client.
