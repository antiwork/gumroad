...
Apple Pay cannot be tested against `http://localhost:3000` — Stripe requires HTTPS for Apple Pay domain registration, so the Apple Pay button will not render in local development. To test it, expose the app over HTTPS (e.g. via an ngrok tunnel or a local reverse proxy like Caddy with mkcert) and register that hostname in the Stripe dashboard.
