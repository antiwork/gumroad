...
Try running this and then run the test again:

```bash
RAILS_ENV=test bin/rails js:export
```

The app navigates to a different domain for checkout, which is set in an autogenerated JS file. The `bin/dev` script automatically sets the development domain, so without this command the tests will try to navigate to that instead of the test domain.
