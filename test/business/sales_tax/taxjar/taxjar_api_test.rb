...
class TaxjarApiTest < ActiveSupport::TestCase
  self.described_class = TaxjarApi
  self.rspec_metadata = { vcr: true }



  context_ TaxjarApi, :vcr do
    let(:origin) do
      {
        country: "US",
        state: "CA",
  ...
  context_ "#calculate_tax_for_order" do
  test "calculates the tax" do
  ...
  test "calculates the tax for quantity greater than 1" do
        expect(described_class.new.calculate_tax_for_order(origin:,
                                                           destination:,
                                                           nexus_address:,
                                                           quantity: 3,
  ...
  test "calculates the tax for product tax code 31000" do
        expect(described_class.new.calculate_tax_for_order(origin:,
                                                           destination:,
                                                           nexus_address:,
                                                           quantity: 1,
  ...
  test "caches the response and returns a cache hit on the next request" do
        expect(described_class.new.calculate_tax_for_order(origin:,
                                                           destination:,
                                                           nexus_address:,
                                                           quantity: 1,
        ...
        expect_any_instance_of(Taxjar::Client).not_to receive(:tax_for_order)
  ...
  test "propagates a TaxJar BadRequest error without notifying error tracker" do
  ...
  test "notifies error tracker and propagates other TaxJar client errors" do
  ...
  test "propagates a TaxJar server error" do
  ...
  context_ "#create_order_transaction" do
  test "creates an order transaction in TaxJar" do
