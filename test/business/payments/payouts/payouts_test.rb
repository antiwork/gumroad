...
class PayoutsTest < ActiveSupport::TestCase
  self.described_class = Payouts
  ...
  test "returns false for creators with paused payouts" do
  ...
  test "returns false for creators suspended for TOS violation" do
  ...
  test "returns false for creators suspended for fraud" do
          expect_processors_not_called
  ...
  test "returns true for creators that are compliant" do
  ...
  test "returns true for compliant creators who have a PayPal account connected", :vcr do
  ...
  test "returns false for accounts not reviewed" do
          expect_processors_not_called
  ...
  test "returns false for accounts on probation" do
          expect_processors_not_called
  ...
  test "returns false for accounts flagged for terms violation" do
          expect_processors_not_called
  ...
  test "returns false for accounts flagged for fraud" do
          expect_processors_not_called
  ...
  context_ "non-compliant user from admin" do
  ...
  context_ "enough money in balance to meet minimum" do
          before { u1b1 && u1b2 }

  test "considers the user payable" do
            expect(described_class.is_user_payable(u1, payout_date)).to eq(true)
          end
        end

  context_ "not enough money in balance to meet minimum" do
          before { u1b1 }

  context_ "no other paid balances for the same payout date" do
  test "considers the user NOT payable" do
              expect(described_class.is_user_payable(u1, payout_date)).to eq(false)
            end
          end

  context_ "paid balances for the same payout date" do
  ...
  context_ "returned balances for the same payout date" do
  ...
  test "returns true when instant payouts are supported and the user has an eligible balance" do
  ...
  test "returns false when instant payouts are not supported" do
  ...
  test "calls the stripe payout processor with only the instantly payable balance amount" do
          allow_any_instance_of(User).to receive(:instant_payouts_supported?).and_return(true)
  ...
  context_ "instant payouts with settling funds" do
  ...
  test "returns false and adds a settling funds note when add_comment is true" do
  ...
  test "returns false without adding a note when add_comment is false" do
          expect do
  ...
  context_ "payout processor logic" do
  ...
  context_ "no payout processor type specified" do
  test "asks all payout processors" do
  ...
  context_ "all processors say no" do
            before do
  ...
  context_ "one processor says yes, rest say no" do
            before do
              allow(PaypalPayoutProcessor).to receive(:is_user_payable).with(u1, 100_00, add_comment: false, from_admin: false, payout_type: anything).and_return(false)
  ...
  context_ "all processors say yes" do
            before do
  ...
  context_ "a payout processor type specified" do
          let(:payout_processor_type) { PayoutProcessorType::STRIPE }

  test "asks only that payout processors" do
            expect(PaypalPayoutProcessor).not_to receive(:is_user_payable).with(u1, 100_00, add_comment: anything, payout_type: anything)
            expect(StripePayoutProcessor).to receive(:is_user_payable).with(u1, 100_00, add_comment: false, from_admin: false, payout_type: anything)
            described_class.is_user_payable(u1, payout_date, processor_type: payout_processor_type)
          end

  context_ "processor says no" do
            before do
              allow(StripePayoutProcessor).to receive(:is_user_payable).with(u1, 100_00, add_comment: false, from_admin: false, payout_type: anything).and_return(false)
            end
  ...
  context_ "processor says yes" do
            before do
              allow(StripePayoutProcessor).to receive(:is_user_payable).with(u1, 100_00, add_comment: false, from_admin: false, payout_type: anything).and_return(true)
            end
  ...
  context_ "create_payments_for_balances_up_to_date" do
      let(:payout_date) { Date.yesterday }
      let(:payout_processor_type) { PayoutProcessorType::PAYPAL }

  test "calls on create_payments_for_balances_up_to_date_for_users with all users holding balance" do
  ...
  test "calls create_payments_for_balances_up_to_date_for_users with all users holding balance who have an active Stripe Connect account" do
  ...
  context_ "create_instant_payouts_for_balances_up_to_date" do
      let(:payout_date) { Date.yesterday }

  test "calls create_instant_payouts_for_balances_up_to_date_for_users with all users holding balance with a payout frequency of daily" do
  ...
  context_ "create_instant_payouts_for_balances_up_to_date_for_users" do
      let(:payout_date) { Date.yesterday }

  context_ "when the seller does not support instant payouts" do
  test "does not create payments" do
  ...
  context_ ".create_payments_for_balances_up_to_date_for_bank_account_types" do
      let(:payout_date) { Date.today - 1 }
      let(:payout_processor_type) { PayoutProcessorType::STRIPE }
  ...
  test "calls create_payments_for_balances_up_to_date_for_users for users holding balance once for every bank account type" do
  ...
  context_ "create_payments_for_balances_up_to_date_for_users" do
  context_ "when payouts are paused for the seller" do
  test "does not create payments" do
  ...
  context_ "attempting to payout for today" do
  test "raises an argument error" do
          expect do
  ...
  context_ "attempting to payout for a future date" do
  test "raises an argument error" do
          expect do
  ...
  test "does not create payments if next_payout_date does not match payout date" do
  ...
  context_ "payout skipped notes" do
  test "adds a comment if payout is skipped due to low balance", :vcr do
  ...
  test "adds a comment if payout is skipped because the account is under review" do
  ...
  test "adds a comment if payout is skipped because the account is suspended" do
  ...
  test "adds a comment if payout is skipped because payouts are paused by admin" do
          seller = create(:compliant_user, payment_address: "seller@gr.co")
          create(:user_compliance_info, user: seller)
          create(:balance, user: seller, date: Date.today - 3, amount_cents: 1000)
  ...
  test "adds a comment if payout is skipped because payouts are paused by stripe" do
  ...
  test "adds a comment if payout is skipped because payouts are paused by the system" do
          seller = create(:compliant_user)
          create(:ach_account, user: seller)
          create(:user_compliance_info, user: seller)
          create(:balance, user: seller, date: Date.today - 3, amount_cents: 1000)
  ...
  test "adds a comment if payout is skipped because payouts are paused by the seller" do
          seller = create(:compliant_user)
          create(:ach_account, user: seller)
          create(:user_compliance_info, user: seller)
          create(:balance, user: seller, date: Date.today - 3, amount_cents: 1000)
  ...
  context_ "slack notification" do
        before do
          @seller = create(:compliant_user, payment_address: "seller@gr.co")
  ...
  test "sends a started scheduling payouts message when scheduling payouts" do
  ...
  test "sends a retrying message when retrying failed payouts" do
          allow(Payouts).to receive(:is_user_payable).twice.and_return(true)
  ...
  test "includes the country info if payouts are for creators of a specific country" do
  ...
  test "includes the bank or debit card info if payouts are for creators from US", :vcr do
          seller = create(:user, unpaid_balance_cents: 100_00)
          create(:ach_account, user: seller)
  ...
  test "includes the Stripe Connect info for Stripe payouts without a bank account type" do
          seller = create(:user, unpaid_balance_cents: 100_00)
  ...
  context_ "a user is payable but balances are changed (e.g. by a chargeback) and will make for a negative payment" do
        let(:payout_date) { Date.today - 1 }
        let(:payout_processor_type) { PayoutProcessorType::PAYPAL }
  ...
  test "remarks the balances as unpaid" do
  ...
  test "does not alter the user's balance" do
          create_payments_for_balances_up_to_date_for_users
          expect(u1.reload.unpaid_balance_cents).to eq(-14_00)
        end

  test "does not create a payment" do
          expect { create_payments_for_balances_up_to_date_for_users }.not_to change { Payment.count }
        end
      end
    end
  end
