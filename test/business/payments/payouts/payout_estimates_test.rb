...
class PayoutEstimatesTest < ActiveSupport::TestCase
  self.described_class = PayoutEstimates



  context_ PayoutEstimates do
  context_ "estimate_held_amount_cents" do
  ...
  test "returns the aggregate of the amount being held at each holder of funds" do
  ...
  context_ "estimate_payments_for_balances_up_to_date_for_users" do
  context_ "common payment cases (ACH via Stripe)" do
        let(:payout_date) { Date.today - 1 }
        let(:payout_processor_type) { PayoutProcessorType::STRIPE }
  ...
  test "does not mark the balances that will be paid as processing" do
  ...
  test "does not mark the balances not being paid as processing" do
          estimate_payments_for_balances_up_to_date_for_users
          expect(u1b3.reload.state).to eq("unpaid")
          expect(u3b1.reload.state).to eq("unpaid")
        end

  test "does not deduct the balance from the user" do
          estimate_payments_for_balances_up_to_date_for_users
  ...
  test "does not create payments for the payable users, up to the date" do
          expect { estimate_payments_for_balances_up_to_date_for_users }.not_to change { [u1.payments, u2.payments, u3.payments] }
        end

  test "generates payment estimates containing info about where funds are held" do
