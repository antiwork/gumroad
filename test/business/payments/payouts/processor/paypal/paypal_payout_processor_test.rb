...
class PaypalPayoutProcessorTest < ActiveSupport::TestCase
  self.described_class = PaypalPayoutProcessor



  context_ PaypalPayoutProcessor do
  context_ "is_user_payable" do
      let(:user) { create(:singaporean_user_with_compliance_info, payment_address: "seller@gr.co") }

  context_ "creator has not set a payment address" do
        before do
  ...
  test "returns true if creator has a paypal account connected", :vcr do
  ...
  context_ "creator has set a payment address" do
  ...
  context_ "payment address contains non-ascii characters" do
          before do
  ...
  context_ "creator has an active bank account" do
          before do
  ...
  test "does not add a payout skipped note" do
            expect do
              described_class.is_user_payable(user, 10_01)
            end.not_to change { user.comments.with_type_payout_note.count }
  ...
  context_ "creator is payable via Stripe" do
          before do
  ...
  test "returns true if the user has valid payout info" do
        expect(user.has_valid_payout_info?).to eq true
      end

  test "returns false if the user is missing a payment address" do
        user.payment_address = ""
        expect(user.has_valid_payout_info?).to eq false
      end

  test "returns false if the user has an invalid payment address" do
  ...
  test "returns false if the user hasn't provided their compliance info" do
        user.alive_user_compliance_info.destroy!
        expect(user.has_valid_payout_info?).to eq false
      end

  test "returns true if the user has a PayPal account connected", :vcr do
        user.update!(payment_address: "")
  ...
  test "calls payout methods with the correct payments" do
  ...
  test "calls the correct methods for every payment even if exceptions are raised" do
        allow(described_class).to receive(:perform_payments).with(anything)
  ...
  context_ "when user has compliance info but legal entity name is blank" do
        let!(:compliance_info) { create(:user_compliance_info, user:, first_name: "", last_name: "") }

  test "returns the note with the user's name" do
          expected_note = "Test User, selling digital products / memberships"
          expect(described_class.note_for_paypal_payment(payment)).to eq(expected_note)
        end
      end

  context_ "when user does not have compliance info" do
  test "returns the note with the user's name" do
  ...
  context_ "pay via paypal and handle IPNs", :vcr do
      before do
  ...
  test "creates the correct payment objects and update them once the IPN comes in" do
  ...
  test "decreases the user's balance by the amount of the payment and not down to 0" do
        Payouts.create_payments_for_balances_up_to_date_for_users(Date.today - 1, PayoutProcessorType::PAYPAL, User.holding_balance)
  ...
  test "creates the proper payments and mark the balances and make the association between those" do
          WebMock.stub_request(:post, PAYPAL_ENDPOINT)
            .to_return(body: "TIMESTAMP=2012%2d10%2d26T20%3a29%3a14Z&CORRELATIONID=c51c5e0cecbce&ACK=Success&VERSION=90%2e0&BUILD=4072860")
          Payouts.create_payments_for_balances_up_to_date_for_users(Date.today - 1, PayoutProcessorType::PAYPAL, User.holding_balance)
  ...
  test "marks the balances as unpaid if the paypal call fails" do
          WebMock.stub_request(:post, PAYPAL_ENDPOINT)
  ...
  test "handles unclaimed payments properly" do
          WebMock.stub_request(:post, PAYPAL_ENDPOINT)
            .to_return(body: "TIMESTAMP=2012%2d10%2d26T20%3a29%3a14Z&CORRELATIONID=c51c5e0cecbce&ACK=Success&VERSION=90%2e0&BUILD=4072860")
          Payouts.create_payments_for_balances_up_to_date_for_users(Date.today - 1, PayoutProcessorType::PAYPAL, User.holding_balance)
  ...
  test "gets correct latest status from paypal for payments with pending status" do
  ...
  test "enqueues a job to set the status from PayPal for payments with pending status" do
  ...
  context_ "creator is not payable through paypal" do
        before do
  ...
  test "does not create a Payment object" do
          expect do
  ...
  context_ "perform_payment_in_split_mode" do
      before do
  ...
  context_ "handle_paypal_event_for_split_payment" do
      before do
        @creator = create(:singaporean_user_with_compliance_info, payment_address: "amir_1351103838_biz@gumroad.com", should_paypal_payout_be_split: true)
  ...
  test "transitions the split payment from pending to completed on correct IPN from paypal" do
        paypal_response_stub = { "TIMESTAMP" => "2018-08-16T08:07:09Z", "CORRELATIONID" => "357a5b454bd3d", "ACK" => "Success", "VERSION" => "90.0", "BUILD" => "46457558" }
        WebMock.stub_request(:post, PAYPAL_ENDPOINT).to_return(body: paypal_response_stub.to_query)
        Payouts.create_payments_for_balances_up_to_date_for_users(Date.yesterday, PayoutProcessorType::PAYPAL, [@creator])
        payment = @creator.payments.last
  ...
  context_ ".handle_paypal_event_for_non_split_payment" do
      before do
        @creator = create(:singaporean_user_with_compliance_info, payment_address: "amir_1351103838_biz@gumroad.com", should_paypal_payout_be_split: true)
  ...
  test "does not do anything if the payment is in a terminal state" do
        Payouts.create_payments_for_balances_up_to_date_for_users(Date.today - 1, PayoutProcessorType::PAYPAL, [@creator])
  ...
  context_ ".split_payment_by_cents" do
  test "returns the value from the database if is present and less than the maximum" do
        user = create(:user, split_payment_by_cents: 5000_00)

        expect(described_class.split_payment_by_cents(user)).to eq(5000_00)
      end

  test "returns the maximum value if the database value is greater than the maximum" do
  ...
  test "returns the maximum value if no value is present in the database" do
        user = create(:user)
  ...
  context_ "#search_payment_on_paypal", :vcr do
  test "searches transaction on PayPal using transaction id if it present" do
        amount_cents = 89771
  ...
  test "searches transaction on PayPal using amount, payment address, and period if transaction id is not present" do
        amount_cents = 89771
  ...
  test "raises error if more than one transaction found in the same period with same payment address and amount" do
        amount_cents = 89771
        payment_address = "sb-dmsfs3088295@business.example.com"
