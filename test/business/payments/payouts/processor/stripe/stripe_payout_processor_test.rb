...
class StripePayoutProcessorTest < ActiveSupport::TestCase
  self.described_class = StripePayoutProcessor
  self.rspec_metadata = { vcr: true }



  context_ StripePayoutProcessor, :vcr do
    include CurrencyHelper
    include StripeChargesHelper

  context_ "is_user_payable" do
      before do
  ...
  context_ "creator no longer has an ach account" do
        before do
          @b1.mark_deleted!
        end
  ...
  test "adds a payout skipped note if the flag is set" do
          expect do
  ...
  context_ "creator has a ach account without a corresponding stripe id" do
        before do
  ...
  context_ "creator does not have a merchant account" do
        before do
          @m1.mark_deleted!
          @u1.reload
        end
  ...
  test "returns true when the user is marked as compliant" do
        expect(described_class.is_user_payable(@u1, 10_01)).to eq(true)
      end

  context_ "instant payouts" do
  test "returns true when the user has an eligible balance" do
          expect(described_class.is_user_payable(@u1, 100_01, payout_type: Payouts::PAYOUT_TYPE_INSTANT)).to eq(true)
        end

  test "returns false when the user has a balance above the maximum instant payout amount" do
          expect(described_class.is_user_payable(@u3, 10_000_01, payout_type: Payouts::PAYOUT_TYPE_INSTANT)).to eq(false)
        end

  test "returns false when the user has a balance below the minimum instant payout amount" do
          expect(described_class.is_user_payable(@u1, 99_99, payout_type: Payouts::PAYOUT_TYPE_INSTANT)).to eq(false)
        end
      end

  context_ "when the user has a previous payout in processing state" do
        before do
  ...
  context_ "creator has a Stripe Connect account" do
        before do
          @m1.mark_deleted!
          @b1.mark_deleted!
  ...
  test "returns false if Stripe Connect account is from Brazil" do
  ...
  test "returns true if the user otherwise has valid payout info" do
        expect(user.has_valid_payout_info?).to eq true
      end

  test "returns false if the user does not have an active bank account" do
        user.active_bank_account.destroy!
        expect(user.has_valid_payout_info?).to eq false
      end

  test "returns false if the user's bank account is not linked to Stripe" do
        user.active_bank_account.update!(stripe_bank_account_id: "")
        expect(user.has_valid_payout_info?).to eq false
      end

  test "returns false if the user does not have a Stripe account" do
        user.stripe_account.destroy!
        expect(user.has_valid_payout_info?).to eq false
      end

  test "returns true if the user has a connected Stripe account regardless of other checks" do
  ...
  context_ "is_balance_payable" do
  context_ "balance is associated with a Gumroad merchant account" do
  ...
  context_ "balance is associated with a Creators' merchant account" do
  ...
  context_ "balance is associated with a Creators' merchant account but in the wrong currency for some reason" do
  ...
  context_ "prepare_payment_and_set_amount" do
  ...
  test "sets the currency" do
        expect(payment.currency).to eq(Currency::CAD)
      end

  test "sets the amount as the sum of the balances" do
        expect(payment.amount_cents).to eq(300_00)
      end
    end

  context_ "destination_balance_drift_error" do
      let(:user) { create(:user) }
  ...
  context_ "when Stripe's available + pending balance is less than Gumroad's recorded held-at-Stripe balance" do
        before do
          allow(Stripe::Balance).to receive(:retrieve).and_return(
            Stripe::Balance.construct_from(object: "balance",
  ...
  test "marks the payment as failed with INSUFFICIENT_FUNDS before any internal transfer" do
  ...
  test "surfaces the drift message through payment.errors so admin endpoints get an informative response body" do
  ...
  context_ "when Stripe's pending balance covers the gap (funds settling, no true drift)" do
        before do
          allow(Stripe::Balance).to receive(:retrieve).and_return(
            Stripe::Balance.construct_from(object: "balance",
                                            available: [{ amount: 1_096_45, currency: "eur" }],
  ...
  test "does not flag drift because settling pending funds will land before next cycle" do
          errors = described_class.prepare_payment_and_set_amount(payment, [eur_balance])
  ...
  context_ "when Stripe reports negative pending (reversals/disputes/refunds in flight) but available covers the payout" do
        before do
          allow(Stripe::Balance).to receive(:retrieve).and_return(
            Stripe::Balance.construct_from(object: "balance",
  ...
  test "does not flag drift because negative pending is clamped at zero" do
          errors = described_class.prepare_payment_and_set_amount(payment, [eur_balance])
  ...
  context_ "when Stripe's available balance matches or exceeds Gumroad's recorded held-at-Stripe balance" do
        before do
          allow(Stripe::Balance).to receive(:retrieve).and_return(
            Stripe::Balance.construct_from(object: "balance",
  ...
  test "proceeds with the payout preparation" do
          errors = described_class.prepare_payment_and_set_amount(payment, [eur_balance])
  ...
  context_ "when Stripe has no balance entry in the destination currency" do
        before do
          allow(Stripe::Balance).to receive(:retrieve).and_return(
            Stripe::Balance.construct_from(object: "balance",
  ...
  test "treats both available and pending as zero for the missing currency and fails with the full gap" do
          errors = described_class.prepare_payment_and_set_amount(payment, [eur_balance])
  ...
  context_ "when the destination is a user-connected Stripe Standard account" do
  ...
  test "skips the drift check because Gumroad does not manage the destination balance" do
  ...
  context_ "when there are no balances held at Stripe" do
        before do
          allow(described_class).to receive(:get_payout_details)
            .and_return([eur_merchant_account, [], []])
        end

  test "skips the drift check" do
          expect(Stripe::Balance).not_to receive(:retrieve)
  ...
  context_ "when the destination merchant account is KRW" do
  ...
  test "skips the drift check because KRW subunit conventions differ between Gumroad and Stripe" do
          expect(Stripe::Balance).not_to receive(:retrieve)
  ...
  context_ "when Stripe::Balance.retrieve raises Stripe::APIConnectionError" do
        before do
          allow(Stripe::Balance).to receive(:retrieve)
            .and_raise(Stripe::APIConnectionError.new("connection failed"))
        end

  test "lets the error propagate to the existing rescue and marks the payment failed" do
          expect do
            described_class.prepare_payment_and_set_amount(payment, [eur_balance])
  ...
  context_ "when Stripe::Balance.retrieve raises Stripe::InvalidRequestError (e.g. account_invalid)" do
        before do
          allow(Stripe::Balance).to receive(:retrieve)
  ...
  test "lets the error propagate to the existing rescue, returns the error message, and marks the payment failed" do
          errors = described_class.prepare_payment_and_set_amount(payment, [eur_balance])
  ...
  context_ "prepare_payment_and_set_amount when merchant_account is nil" do
      let(:user) { create(:user) }
      let(:payment) { create(:payment, user:, currency: nil, amount_cents: nil) }
  ...
  test "returns an error and marks the payment as failed" do
  ...
  context_ "prepare_payment_and_set_amount when balance_transaction is nil" do
      let(:user) { create(:user) }
  ...
  test "raises an error after retries and marks the payment as failed" do
  ...
  test "succeeds when balance_transaction becomes available on retry" do
  ...
  context_ "prepare_payment_and_set_amount with currency-mismatched balances" do
      let(:user) { create(:user) }
      let(:payment) { create(:payment, user:, currency: nil, amount_cents: nil) }
      let(:gumroad_merchant_account) do
        MerchantAccount.gumroad(StripeChargeProcessor.charge_processor_id) ||
  ...
  context_ "when a Gumroad-held balance has holding_currency != usd" do
  ...
  test "fails the payment with an explanatory error rather than summing across currencies" do
  ...
  test "does not silently produce a $9.45 wire amount from $126.72 of seller balance" do
  ...
  context_ "when a Stripe-held balance has holding_currency != merchant_account.currency" do
  ...
  test "fails the payment with an explanatory error" do
  ...
  context_ "filter_aggregate_payable_balances" do
      let(:user) { create(:user) }
  ...
  context_ "when total Gumroad-held USD is below the per-currency floor" do
  ...
  test "excludes the Gumroad-held USD balances so they roll forward" do
  ...
  context_ "when total Gumroad-held USD clears the floor" do
  ...
  test "keeps every balance so the internal transfer goes through" do
  ...
  context_ "when the merchant account currency is USD" do
  ...
  test "keeps the balance because no FX conversion is needed" do
  ...
  context_ "when no Gumroad-held balances are present" do
        before do
          allow(described_class).to receive(:get_payout_details)
  ...
  context_ "when merchant_account is nil" do
  ...
  test "returns the input unchanged so existing nil-merchant handling runs downstream" do
  ...
  context_ "when balances is empty" do
  test "returns the empty array without calling get_payout_details" do
  ...
  context_ "prepare_payment_and_set_amount for Korean bank account" do
      let(:user) { create(:user) }
  ...
  test "sets the amount as the sum of the balances, converted to match the database for KRW" do
        expect(payment.amount_cents).to eq(39640900)
      end
    end

  context_ ".enqueue_payments" do
      let!(:yesterday) { Date.yesterday.to_s }
      let!(:user_ids) { [1, 2, 3, 4] }

  test "enqueues PayoutUsersWorker jobs for the supplied payments" do
  ...
  test "creates a transfer at stripe" do
  ...
  test "marks the payment as processing" do
        described_class.prepare_payment_and_set_amount(payment, balances)
        described_class.perform_payment(payment)
        expect(payment.state).to eq("processing")
      end

  test "stores the stripe account identifier of the account the transfer was created on, on the payment" do
        described_class.prepare_payment_and_set_amount(payment, balances)
        errors = described_class.perform_payment(payment)
        expect(errors).to be_empty
  ...
  test "stores the stripe transfer's identifier on the payment" do
        described_class.prepare_payment_and_set_amount(payment, balances)
        errors = described_class.perform_payment(payment)
        expect(errors).to be_empty
  ...
  test "does not store an internal stripe transfer's identifier on the payment" do
        described_class.prepare_payment_and_set_amount(payment, balances)
        errors = described_class.perform_payment(payment)
        expect(errors).to be_empty
  ...
  context_ "the payment includes funds not held by stripe, which don't sum to a positive amount" do
  ...
  test "creates a normal transfer" do
          expect(Stripe::Payout).to receive(:create).with(
            {
              amount: payment_amount_cents,
              currency: "usd",
  ...
  test "stores the internal stripe transfer's identifier on the payment" do
          described_class.prepare_payment_and_set_amount(payment, payment.balances.to_a)
          errors = described_class.perform_payment(payment)
          expect(errors).to be_empty
          expect(payment.stripe_internal_transfer_id).to eq(nil)
  ...
  context_ "the external transfer fails" do
          before do
  ...
  test "returns the errors" do
            described_class.prepare_payment_and_set_amount(payment, payment.balances.to_a)
            errors = described_class.perform_payment(payment)
            expect(errors).to be_present
          end

  test "marks the payment as failed" do
            described_class.prepare_payment_and_set_amount(payment, payment.balances.to_a)
            described_class.perform_payment(payment)
            payment.reload
            expect(payment.state).to eq("failed")
  ...
  context_ "the external transfer fails because the account cannot be paid" do
          before do
  ...
  test "marks the payment with a failure reason of cannot pay" do
            described_class.prepare_payment_and_set_amount(payment, payment.balances.to_a)
            described_class.perform_payment(payment)
            payment.reload
  ...
  test "adds a payout note to the user" do
            described_class.prepare_payment_and_set_amount(payment, payment.balances.to_a)
  ...
  context_ "the external transfer fails because payouts cannot be created" do
          before do
  ...
  context_ "the external transfer fails because of an unsupported reason" do
          before do
  ...
  context_ "the payment includes funds not held by stripe, which sum to a positive amount" do
        let(:balances_held_by_gumroad) do
          [
  ...
  test "creates an internal transfer and a normal transfer" do
  ...
  context_ "the internal transfer fails" do
          before do
  ...
  test "creates a reversal for the internal transfer" do
  ...
  context_ "transfer fails due to an invalid request (amount over balance of creator)" do
  ...
  context_ "perform_payment for a US account with instant payout method type" do
      let(:user) { create(:user) }
      let(:tos_agreement) { create(:tos_agreement, user:) }
      let(:user_compliance_info) { create(:user_compliance_info, user:) }
      let(:bank_account) { create(:ach_account_stripe_succeed, user:) }
  ...
  context_ "perform_payment with a Canadian payout" do
      let(:user) { create(:user) }
      let(:tos_agreement) { create(:tos_agreement, user:) }
  ...
  test "stores the stripe payout's arrival date on the payment" do
        described_class.prepare_payment_and_set_amount(payment, balances)
        errors = described_class.perform_payment(payment)
        expect(errors).to be_empty
  ...
  test "creates a credit if necessary" do
              expect(described_class).to receive(:create_credit_for_difference_from_reversed_internal_transfer)
              described_class.prepare_payment_and_set_amount(payment, payment.balances.to_a)
              described_class.perform_payment(payment)
            end
  ...
  context_ "the reverse amount was the same as the original internal transfer" do
  test "does not create a credit for the difference" do
                described_class.perform_payment(payment)
                expect(Credit.last).to eq(nil)
              end
            end

  context_ "the reverse amount was different for the managed account" do
              # Very hard to test
            end
          end
        end
  ...
  context_ "perform_payment for a German merchant account" do
      let(:user) { create(:user) }
      let(:tos_agreement) { create(:tos_agreement, user:) }
  ...
  context_ "the reverse amount was different for the managed account" do
  test "creates a credit for the difference" do
                described_class.perform_payment(payment)
  ...
  context_ "perform_payment for a Singaporean merchant account" do
      let(:user) { create(:user) }
      let(:tos_agreement) { create(:tos_agreement, user:) }
  ...
  context_ "perform_payment for a Korean merchant account" do
      let(:user) { create(:user) }
      let(:tos_agreement) { create(:tos_agreement, user:) }
  ...
  context_ "an event we do nothing with, like payout.created" do
          let(:stripe_event_type) { "payout.created" }

  test "does not error or do anything interesting" do
            expect(Stripe::Payout).not_to receive(:retrieve)
            described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
          end
        end

  context_ "an event about a manual payout on stripe standard connect account" do
  ...
  test "ignores the event and does not raise an error" do
            expect do
              described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
  ...
  context_ "an event about an automatic payout to bank account" do
          let(:stripe_event_type) { "payout.paid" }
  ...
  context_ "an event about an automatic bank debit made by stripe", :sidekiq_inline do
  context_ "payout.paid" do
            let!(:stripe_event) do
              {
  ...
  context_ "when payout is successful" do
  test "adds credit equal to debited amount to creator gumroad balance" do
  ...
  context_ "when event is about a payout we issued to a creator" do
          let!(:merchant_account) { create(:merchant_account, charge_processor_merchant_id: stripe_connect_account_id) }
  context_ "payout.paid" do
            let(:stripe_event_type) { "payout.paid" }
  ...
  context_ "payout doesn't match a payment" do
  ...
  context_ "payout partially matches a payment" do
              let(:payment) do
  ...
  context_ "payout does match a payment" do
              let(:payment) do
                create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
                                 stripe_connect_account_id:, stripe_transfer_id:, processor_fee_cents: 0)
              end
  ...
  test "marks the respective payment as complete" do
                described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                payment.reload
                expect(payment.state).to eq("completed")
              end

  context_ "payment was already marked as failed" do
                before do
                  payment.mark_failed!
                end
  ...
  context_ "payout matches a payment" do
              let!(:payment) do
  ...
  test "marks the respective payment as cancelled" do
                described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                payment.reload
                expect(payment.state).to eq("cancelled")
              end

  test "marks the respective balances as unpaid" do
  ...
  context_ "when payment not in processing state" do
                before do
  ...
  context_ "when payment had an internal transfer" do
                let(:user) { create(:user) }
  ...
  test "reverses the internal transfer" do
                  expect(stripe_internal_transfer).to receive_message_chain(:reversals, :create)
                  described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                end

  context_ "when the reverse amount was the same as the original internal transfer" do
  test "does not create a credit for the difference" do
                    described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                    expect(Credit.last).to eq(nil)
                  end
  ...
  context_ "when the reverse amount was different for the managed account" do
  ...
  test "changes the balance by the difference" do
                    described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
  ...
  test "marks the respective payment as failed" do
                described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                payment.reload
                expect(payment.state).to eq("failed")
              end

  test "saves the failure reason and notifies the user" do
                expect do
                  described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                  payment.reload
  ...
  context_ "payment was already marked as completed" do
                before do
                  payment.mark_completed!
                end

  test "sets the state to returned" do
                  described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                  payment.reload
  ...
  context_ "had an internal transfer" do
                let(:user) { create(:user) }
                let(:stripe_account_id) { "acct_1234" }
                let(:merchant_account) do
                  create(
  ...
  test "has changed the balance by the difference" do
                    described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                    user = payment.user.reload
                    # In this test the balance was zero, so we'll check if the balance is now the difference.
                    expect(user.balances.unpaid.sum(:holding_amount_cents)).to eq(-5_00)
  ...
  context_ "when event is about a reversal of a payout we issued to a creator", :sidekiq_inline do
          let!(:merchant_account) { create(:merchant_account, charge_processor_merchant_id: stripe_connect_account_id) }
          let(:stripe_event_object) do
            {
              object: "payout",
  ...
  context_ "when payout doesn't match a payment" do
  test "raises an error" do
                expect do
                  described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                end.to raise_error(RuntimeError)
  ...
  context_ "when payout metadata doesn't match payment's ID" do
              let!(:payment) do
  ...
  context_ "when payout matches a payment" do
              let(:payment) do
  ...
  context_ "when payment is in processing state" do
  test "sets payment's state to failed" do
                  described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                  payment.reload
                  expect(payment.state).to eq("failed")
                end
  ...
  context_ "when payment is in completed state" do
  test "sets payment's state to returned" do
  ...
  test "marks payment's balances as unpaid" do
                expect(payment.balances.first.state).to eq("processing")
                described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                payment.reload
                expect(payment.balances.first.state).to eq("unpaid")
  ...
  context_ "when payout had an internal transfer" do
                let(:stripe_account_id) { "acct_1234" }
                let(:stripe_internal_transfer_id) { "tr_5678" }
                let(:stripe_destination_payment_id) { "py_1234" }
                let(:stripe_refund_balance_transaction_id) { "txn_1Ects" }
  ...
  test "ignores the event - nothing to do, a manual reversal was canceled" do
                expect do
                  described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                end.not_to raise_error
              end
  ...
  context_ "when payment has not been marked as reversed before" do
  test "ignores the event - nothing to do, a manual reversal did not succeed" do
                  expect do
                    described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
                  end.not_to raise_error
                end
  ...
  context_ "when payment has been marked as reversed before" do
                before do
                  payment.processor_reversing_payout_id = "reversal_payout_id"
                  payment.save!
                end

  test "notifies error tracker that a previously successful reversal has changed state to failed" do
                  expect do
                    described_class.handle_stripe_event(stripe_event, stripe_connect_account_id:)
  ...
  context_ ".instantly_payable_amount_cents_on_stripe" do
      let(:user) { create(:user) }
  ...
  context_ "when user has no active bank account" do
        before do
          user.bank_accounts = []
        end
  ...
  context_ "when not eligible for instant payouts" do
        before do
          allow(Stripe::Balance).to receive(:retrieve).and_return(
  ...
  context_ "when eligible for instant payouts" do
        before do
          allow(Stripe::Balance).to receive(:retrieve).and_return(
            Stripe::Balance.construct_from(
              object: "balance",
  ...
  test "returns the instant available amount" do
          expect(described_class.instantly_payable_amount_cents_on_stripe(user)).to eq(123456)
        end
      end

  context_ "when eligible but bank account doesn't match" do
        before do
          allow(Stripe::Balance).to receive(:retrieve).and_return(
            Stripe::Balance.construct_from(
              object: "balance",
  ...
  context_ ".perform_payment Stripe error handling" do
      let(:user) { create(:user) }
  ...
  context_ "when Stripe rejects the payout because the destination bank account has been deleted" do
  ...
  test "marks the payment with failure_reason BANK_ACCOUNT_NOT_FOUND_AT_STRIPE" do
          described_class.perform_payment(payment)
          expect(payment.reload.failure_reason).to eq(Payment::FailureReason::BANK_ACCOUNT_NOT_FOUND_AT_STRIPE)
        end

  test "marks the bank account deleted so subsequent runs skip until the seller re-adds it" do
          expect { described_class.perform_payment(payment) }
            .to change { bank_account.reload.deleted_at }.from(nil)
        end

  test "stores the Stripe error message on the payment" do
          described_class.perform_payment(payment)
          expect(payment.reload.error_message).to eq(stripe_error_message)
        end

  test "does not notify the error tracker" do
          expect(ErrorNotifier).not_to receive(:notify)
          described_class.perform_payment(payment)
        end

  test "adds a payout note directing the seller to re-add their bank account" do
          expect { described_class.perform_payment(payment) }
  ...
  context_ "and the internal transfer reversal raises a transient Stripe error afterwards" do
          before do
  ...
  test "still marks the bank account deleted before the reversal runs" do
  ...
  context_ "when Stripe raises an unmatched Stripe::InvalidRequestError" do
        before do
          allow(Stripe::Payout).to receive(:create).and_raise(Stripe::InvalidRequestError.new("Something unexpected.", "param"))
        end

  test "captures the error message on the payment" do
          described_class.perform_payment(payment)
          expect(payment.reload.error_message).to eq("Something unexpected.")
        end

  test "leaves failure_reason nil so the existing flow is unchanged" do
          described_class.perform_payment(payment)
          expect(payment.reload.failure_reason).to be_nil
        end

  test "still notifies the error tracker for unmatched errors" do
          expect(ErrorNotifier).to receive(:notify)
          described_class.perform_payment(payment)
        end
      end

  context_ "when Stripe raises a generic Stripe::StripeError" do
        before do
  ...
  context_ "when Stripe raises Stripe::APIConnectionError" do
        before do
          allow(Stripe::Payout).to receive(:create).and_raise(Stripe::APIConnectionError.new("connection refused"))
        end

  test "re-raises the error and stores the class and message on the payment" do
          expect { described_class.perform_payment(payment) }.to raise_error(Stripe::APIConnectionError)
