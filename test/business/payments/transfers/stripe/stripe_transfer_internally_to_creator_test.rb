...
class StripeTransferInternallyToCreatorTest < ActiveSupport::TestCase
  self.described_class = StripeTransferInternallyToCreator
  self.rspec_metadata = { vcr: true }


  context_ StripeTransferInternallyToCreator, :vcr do
    include StripeMerchantAccountHelper
    include StripeChargesHelper

  context_ "transfer_funds_to_account" do
      before do
  ...
  context_ "transfer_funds_to_account" do
  context_ "when no related charge given" do
  test "creates a transfer at stripe destined for the connected account" do
  ...
  test "returns a transfer that has balance transactions for itself and for the application fee" do
  ...
  context_ "when a related charge is given" do
  test "creates a transfer at stripe destined for the connected account" do
            expect(Stripe::Transfer).to receive(:create).with(hash_including(
                                                                destination: managed_account_id,
                                                                currency: "usd",
  ...
  context_ "when metadata is given" do
