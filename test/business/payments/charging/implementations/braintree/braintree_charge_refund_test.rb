...
class BraintreeChargeRefundTest < ActiveSupport::TestCase
  self.described_class = BraintreeChargeRefund
  self.rspec_metadata = { vcr: true }



  context_ BraintreeChargeRefund, :vcr do
  ...
  context_ "with a braintree refund" do
  test "has a charge_processor_id set to 'braintree'" do
          expect(subject.charge_processor_id).to eq "braintree"
        end

  test "has the #id from the braintree refund" do
          expect(subject.id).to eq(braintree_refund.id)
        end

  test "has the #charge_id from the braintree refund" do
          expect(subject.charge_id).to eq(braintree_refund.refunded_transaction_id)
        end

  test "has the #charge_id from the original braintree charge" do
