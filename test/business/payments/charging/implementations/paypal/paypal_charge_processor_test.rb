...
class PaypalChargeProcessorTest < ActiveSupport::TestCase
  self.described_class = PaypalChargeProcessor
  ...
  context_ PaypalChargeProcessor, :vcr do
  ...
  test "returns 'paypal'" do
        expect(described_class.charge_processor_id).to eq "paypal"
      end
    end

  context_ "PayPal payment events handler" do
  context_ "non payment event determined by the invoice field existing" do
        before do
          raw_payload =
  ...
  test "does not process it and raises an error" do
          expect do
  ...
  context_ "reversal and reversal cancelled events" do
  context_ "reversal event" do
            before do
              raw_payload =
                "payment_type=echeck&payment_date=Sun%20May%2024%202015%2014%3A32%3A31%20GMT-0700%20%28PDT%29&payment_status=Reversed&" \
                "payer_status=verified&first_name=John&last_name=Smith&payer_email=buyer%40paypalsandbox.com&payer_id=TESTBUYERID01&address_name=John%20Smith&address_country=United%20States&" \
  ...
  test "handles a chargeback message form PayPal" do
              expect(@purchase.chargeback_date).to be(nil)

              described_class.handle_paypal_event(@payload)

              @purchase.reload
              expect(@purchase.chargeback_date).not_to be(nil)
  ...
  context_ "reversal cancelled event" do
  ...
  context_ "reversal cancelled event" do
  context_ "no transaction found for parent transaction ID" do
  test "raises a NoMethodError exception" do
  ...
  context_ "the dispute was resolved in our favor" do
                before(:each) do
  ...
  test "handles a chargeback reversed message form PayPal" do
                  expect(@purchase.chargeback_date).not_to be(nil)
  ...
  test "tells the charge processor that a dispute was won" do
                  original_handle_event = ChargeProcessor.method(:handle_event)
                  expect(ChargeProcessor).to(receive(:handle_event) do |charge_event|
                                               expect(charge_event).to be_a(ChargeEvent)
  ...
  context_ "the dispute was resolved in the buyers favor" do
                before(:each) do
                  @mock_paypal_charge.paypal_payment_status = PaypalApiPaymentStatus::REVERSED
                  expect_any_instance_of(described_class).to receive(:get_charge).and_return(@mock_paypal_charge)
                end

  test "does not send any events and do nothing" do
  ...
  context_ "payment completion event" do
          before do
            raw_payload =
  ...
  test "creates a informational charge event with the fee information" do
  ...
  context_ "other non-completion payment event" do
          before do
            raw_payload =
  ...
  context_ "on combined charges" do
  test "creates a informational charge event with the fee information" do
  ...
  context_ ".handle_order_events" do
  context_ "when event type is CUSTOMER.DISPUTE.CREATED" do
  test "sets chargeback details on purchase" do
  ...
  test "raises `ChargeProcessorError` on error" do
  ...
  context_ "when event type is CUSTOMER.DISPUTE.RESOLVED" do
  test "marks dispute as WON when seller wins the dispute" do
  ...
  test "marks dispute as LOST when seller loses the dispute" do
  ...
  context_ "when event type is PAYMENT.CAPTURE.COMPLETED" do
  test "updates the processor fee for the purchase" do
  ...
  test "does nothing if seller_receivable_breakdown is absent" do
          purchase = create(:purchase, stripe_transaction_id: "5B223658W54364539",
                                       processor_fee_cents: nil, processor_fee_cents_currency: nil)
  ...
  context_ "when event is for payment capture failure" do
        before do
  ...
  test "refunds the purchase and reverts associated affiliate balance when event type is PAYMENT.CAPTURE.DENIED" do
  ...
  test "refunds the purchase and reverts associated affiliate balance when event type is PAYMENT.CAPTURE.REVERSED" do
  ...
  test "sets the processor_refund_id and status on the refund record" do
  ...
  test "does not do anything if there is already a refund with same processor_refund_id" do
          @purchase.update!(stripe_transaction_id: "0JF852973C016714D")
  ...
  test "creates refund if there is already a refund associated with the purchase but with different " \
  ...
  test "refunds the purchase and reverts associated affiliate balance when event type is PAYMENT.CAPTURE.REFUNDED" do
          @purchase.update!(stripe_transaction_id: "0JF852973C016714D")
  ...
  test "does nothing if purchase is already fully refunded" do
          @purchase.update!(stripe_transaction_id: "0JF852973C016714D")
  ...
  test "records the refund but does not debit the seller's balance again when the purchase is already chargedback" do
  ...
  test "refunds remaining amount if purchase is partially refunded" do
          @purchase.update!(stripe_transaction_id: "0JF852973C016714D")
  ...
  test "refunds the correct partial amount for partial refunds" do
          @purchase.update!(stripe_transaction_id: "0JF852973C016714D")
  ...
  test "refunds the correct amount for single unit currency refunds" do
          @purchase.update!(stripe_transaction_id: "0JF852973C016714D")
  ...
  context_ "when billing agreement id is passed in arguments" do
        let(:params) { { billing_agreement_id: "B-38D505255T217912K" } }
  test "returns object of PaypalChargeable with billing agreement id set as fingerptint" do
  ...
  context_ "when paypal order id is passed in arguments" do
        let(:params) { { paypal_order_id: "B-38D505255T217912K" } }
  test "returns object of PaypalApprovedOrderChargeable with paypal order id set as fingerptint" do
          chargeable = PaypalChargeProcessor.new.get_chargeable_for_params(params, nil)
  ...
  context_ "when neither billing agreement id nor paypal order id are passed in arguments" do
  test "returns nil" do
  ...
  context_ "when purchase is created from order API" do
  ...
  test "fetches order and returns paypal charge" do
  ...
  context_ "when purchase is not created from order API" do
  context_ "successful transaction" do
  test "retrieves the charge successfully" do
            paypal_charge = subject.get_charge("0MK73950Y39198240")
            expect(paypal_charge).not_to be(nil)
  ...
  context_ "refunded transaction" do
  test "retrieves the charge successfully" do
  ...
  context_ "invalid / non-existant transaction" do
  ...
  context_ "when billing_agreement id is present in chargeable" do
        let(:valid_paypal_chargeable) do
          PaypalChargeable.new("B-38D505255T217912K", "paypal-gr-integspecs@gumroad.com", "US")
        end

  test "creates new paypal order and charges it and returns PaypalChargeIntent" do
  ...
  test "does not raise error and returns PaypalCharge when transaction is complete" do
          paypal_auth_token =
  ...
  test "does not raise error and returns PaypalCharge when transaction is pending review" do
          paypal_auth_token =
            "Bearer A21AAIW-mWqtYFHC_juLm6Ded4UvrZmFdAprGbqIsnyf9Tjay2qdeZqLzZnjGJfNAcQvdNS4Rx0Hgbyu2ukMDARwWHEEBdh1Q"
          allow_any_instance_of(PaypalPartnerRestCredentials).to receive(:auth_token).and_return(paypal_auth_token)
  ...
  test "refunds the transaction on paypal and raises error when it is an echeck payment" do
  ...
  context_ "when paypal order id is present in chargeable" do
  test "charges already approved order without creating a new order and returns PaypalCharge" do
  ...
  context_ "when billing agreement id is not present in chargeable" do
  ...
  context_ "when one combined charge is created for multiple purchases" do
        before do
          seller = create(:user)
  ...
  context_ "when refund is for order api" do
  context_ "when it is a partial refund" do
  ...
  test "refunds the specified amount" do
  ...
  context_ "when invalid capture id is passed" do
  test "raises ChargeProcessorInvalidRequestError error" do
              expect do
  ...
  context_ "when purchase unit is already refunded" do
  test "raises ChargeProcessorInvalidRequestError error" do
  ...
  context_ "when refund amount is greater than the remaining amount" do
  test "raises ChargeProcessorInvalidRequestError error" do
              error_pattern =
  ...
  context_ "when it is a full refund" do
  test "refunds entire amount" do
  ...
  test "refunds entire amount if currency is not usd" do
  ...
  context_ "when purchase unit is already refunded" do
  test "raises ChargeProcessorAlreadyRefundedError error" do
              expect do
  ...
  test "raises ChargeProcessorUnavailableError when refund API is not working" do
  ...
  context_ "when valid inputs are passed" do
        before do
          @response = PaypalChargeProcessor.create_order(PaypalChargeProcessor.paypal_order_info(purchase))
        end

  test "returns order_id" do
          expect(@response).not_to be(nil)
        end
      end

  context_ "when invalid inputs are passed" do
  test "raises proper error" do
          purchase.quantity = 0
          expect do
  ...
  context_ "when `purchase` is empty" do
  test "notifies error tracker and raises the exception" do
            expect(ErrorNotifier).to receive(:notify).exactly(:once)
  ...
  test "raises ChargeProcessorPayerCancelledBillingAgreementError error if paypal returns billing agreement cancelled" \
         " error" do
  ...
  test "raises ChargeProcessorUnavailableError error if paypal returns internal error" do
        error_response = JSON.parse({ result: {
          name: "INTERNAL_ERROR",
  ...
  test "raises ChargeProcessorPaymentDeclinedByPayerAccountError error if paypal returns transaction declined error" do
        error_response = JSON.parse({ result: {
          name: "UNPROCESSABLE_ENTITY",
  ...
  test "raises ChargeProcessorPayeeAccountRestrictedError error if paypal returns problem with payee account" do
        error_response = JSON.parse({ result: {
          name: "UNPROCESSABLE_ENTITY",
  ...
  test "raises ChargeProcessorPaymentDeclinedByPayerAccountError error if paypal returns a problem stating the payer " \
         "cannot pay" do
        error_response = JSON.parse({ result: {
          name: "UNPROCESSABLE_ENTITY",
  ...
  test "returns order information" do
  ...
  context_ ".log_paypal_api_response" do
  test "logs the response in a format we desire" do
  ...
  test "returns the info required to create paypal order for a given purchase" do
        paypal_order_info = {
  ...
  test "sets the item name as product's custom_permalink if product's name becomes empty on sanitization" do
        @product.name = "🚧 🚧 🍖 💥 💥"
  ...
  test "sets the item name as product's unique_permalink if the product's name becomes empty on sanitization and " \
  ...
  context_ ".create_order_from_purchase" do
  test "creates a new paypal order for the purchase" do
        creator = create(:user)
  ...
  context_ ".create_order_from_product_info" do
  test "creates a new paypal order with the given product info" do
        creator = create(:user)
        product = create(:product, user: creator)
        paypal_merchant_account = create(:merchant_account_paypal, charge_processor_merchant_id: "CJS32DZ7NDN5L",
                                                                   user: creator, country: "GB", currency: "gbp")
  ...
  test "creates a new paypal order if product currency is usd but merchant account currency is not" do
        creator = create(:user)
  ...
  test "creates a new paypal order if merchant account currency is usd but product currency is not" do
        creator = create(:user)
  ...
  test "creates a new paypal order if merchant account currency and product currency are different and none is usd" do
        creator = create(:user)
        product = create(:product, user: creator, price_currency_type: "aud")
        paypal_merchant_account = create(:merchant_account_paypal, charge_processor_merchant_id: "CJS32DZ7NDN5L",
                                                                   user: creator, country: "GB", currency: "gbp")
  ...
  test "creates a new paypal order with item name as product's custom_permalink if product's name becomes empty on " \
         "sanitization" do
        creator = create(:user)
  ...
  test "creates a new paypal order with item name as product's unique_permalink if the product's name becomes empty " \
  ...
  context_ ".update_order_from_product_info" do
  test "updates the paypal order with the given product info and returns true" do
        creator = create(:user)
        product = create(:product, user: creator)
  ...
  test "raises error if paypal order id is not present" do
        updated_purchase_unit_info = {
  ...
  test "raises error if product info is not present" do
        expect do
  ...
  context_ "#update_invoice_id" do
  test "updates the invoice id for paypal order" do
        creator = create(:user)
        product = create(:product, user: creator)
        paypal_merchant_account = create(:merchant_account_paypal, charge_processor_merchant_id: "CJS32DZ7NDN5L",
                                                                   user: creator, country: "GB", currency: "gbp")
  ...
  test "raises ChargeProcessorUnavailableError error if paypal api call fails with INTERNAL_ERROR" do
  ...
  test "raises ChargeProcessorInvalidRequestError error if paypal api call fails with any other error than " \
         "INTERNAL_ERROR" do
  ...
  context_ "#get_charge_for_order_api" do
      let(:paypal_order_id) { "35R447743K110460F" }

  test "returns a PayPal charge object with details of the given paypal capture id and order id" do
  ...
  test "returns a PayPal charge object with details of the paypal capture even if only the order id is provided" do
  ...
  test "returns a PayPal charge object with details of the paypal order attached to the given purchase" do
  ...
  test "returns nil if no paypal order is found for the given purchase" do
        expect(subject.search_charge(purchase: create(:purchase))).to be(nil)
      end
    end

  context_ ".formatted_amount_for_paypal" do
  test "returns decimal values for currencies other than TWD, HUF, and JPY" do
  ...
  test "returns integer values for TWD, HUF, and JPY currencies" do
