...
class PaypalRestApiTest < ActiveSupport::TestCase
  self.described_class = PaypalRestApi
  self.rspec_metadata = { vcr: true }


  context_ PaypalRestApi, :vcr do
  ...
  test "creates new order as requested and returns the order details in response" do
  ...
  test "limits the product (item) names to 127 characters" do
  ...
  test "Formats the amounts to not have a thousands separator and have 2 decimal places" do
  ...
  test "does not have decimal values for amounts in TWD" do
  ...
  test "calculates the amounts in HUF correctly" do
  ...
  test "uses items details if passed to create new order" do
        seller = create(:user)
  ...
  test "updates the paypal order with the product info" do
        allow_any_instance_of(Purchase).to receive(:external_id).and_return("G_-mnBf9b1j9A7a4ub4nFQ==")
  ...
  context_ "when invalid order id is passed" do
  ...
  context_ "when valid order id is passed" do
        let(:create_order_response) do
          api_object.create_order(purchase_unit_info: PaypalChargeProcessor.paypal_order_info(purchase))
        end
  ...
  test "returns success with valid order having completed status" do
  ...
  context_ "#refund" do
  context_ "when it a full refund" do
  ...
  context_ "when it is partial refund" do
  context_ "when invalid capture id is passed" do
  ...
  test "refunds the passed amount" do
            expect(refund_response.status_code).to eq(201)
  ...
  context_ "when merchant account details are not available" do
  test "gets the paypal account details from original order and refunds successfully" do
