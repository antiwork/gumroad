...
class PaypalChargeTest < ActiveSupport::TestCase
  self.described_class = PaypalCharge


  context_ PaypalCharge do
  context_ "when order api is used" do
  context_ "when paypal transaction is present" do
  ...
  test "sets all the properties of the order" do
  ...
  test "does not throw error if paypal_fee value is absent" do
  ...
  context_ "when paypal transaction is not present" do
        subject do
          PaypalCharge.new(paypal_transaction_id: nil, order_api_used: true)
        end

  test "doesn't set order API property" do
          is_expected.to have_attributes(charge_processor_id: PaypalChargeProcessor.charge_processor_id,
                                         id: nil,
                                         fee: nil,
  ...
  context_ "when express checkout api is used" do
  context_ "with payment and payer info passed in" do
  ...
  test "populates the required payment info and optional payer info" do
  ...
  test "populates the required payment and does not set payer fields if payload is not passed in" do
          paypal_charge = PaypalCharge.new(paypal_transaction_id: "5SP884803B810025T",
                                           order_api_used: false,
                                           payment_details: {
                                             paypal_payment_info:
  ...
  test "sets the refund states based on the PayPal PaymentInfo PaymentStatus" do
  ...
  test "does not have a flow of funds" do
          paypal_charge = PaypalCharge.new(paypal_transaction_id: "5SP884803B810025T",
                                           order_api_used: false,
                                           payment_details: {
                                             paypal_payment_info:
