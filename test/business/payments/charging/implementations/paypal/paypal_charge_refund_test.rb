...
class PaypalChargeRefundTest < ActiveSupport::TestCase
  self.described_class = PaypalChargeRefund
  self.rspec_metadata = { vcr: true }



  context_ PaypalChargeRefund, :vcr do
  ...
  context_ "with a paypal refund response" do
  test "has a charge_processor_id set to 'paypal' and id and charge_id but no flow_of_funds" do
