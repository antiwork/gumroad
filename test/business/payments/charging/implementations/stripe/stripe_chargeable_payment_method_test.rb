...
class StripeChargeablePaymentMethodTest < ActiveSupport::TestCase
  self.described_class = StripeChargeablePaymentMethod
  self.rspec_metadata = { vcr: true }



  context_ StripeChargeablePaymentMethod, :vcr do
  ...
  context_ "#prepare!" do
  test "retrieves token details from stripe" do
        expect(Stripe::PaymentMethod).to receive(:retrieve).with(stripe_payment_method_id).and_call_original
        chargeable.prepare!
      end

  test "does not prepare for direct charge if merchant account is not a stripe connect account" do
  ...
  test "prepares for direct charge if merchant account is a stripe connect account" do
  ...
  context_ "#reusable_token!" do
  test "uses stripe to get a reusable token" do
  ...
  context_ "#visual" do
  test "calls ChargeableVisual to build a visual" do
  ...
  context_ "#stripe_charge_params" do
  test "returns the original customer and payment method details if merchant account is not a stripe connect account" do
        allow_any_instance_of(StripeChargeablePaymentMethod).to receive(:get_merchant_account).and_return(create(:merchant_account))
        expect(Stripe::PaymentMethod).to receive(:retrieve).with(stripe_payment_method_id).and_call_original
        expect_any_instance_of(StripeChargeablePaymentMethod).not_to receive(:prepare_for_direct_charge)
  ...
  context_ "when customer_id is provided from a prior SetupIntent authentication" do
  ...
  test "includes the authenticated customer in charge params so Stripe can skip SCA" do
          allow_any_instance_of(StripeChargeablePaymentMethod).to receive(:get_merchant_account).and_return(create(:merchant_account))
  ...
  test "uses the authenticated customer when cloning the payment method for Connect direct charges" do
          allow_any_instance_of(StripeChargeablePaymentMethod).to receive(:get_merchant_account).and_return(create(:merchant_account_stripe_connect))
  ...
  test "returns the cloned payment method details if merchant account is a stripe connect account" do
        allow_any_instance_of(StripeChargeablePaymentMethod).to receive(:get_merchant_account).and_return(create(:merchant_account_stripe_connect))
        expect(Stripe::PaymentMethod).to receive(:retrieve).with(stripe_payment_method_id).and_call_original
