...
class StripeChargeRadarProcessorTest < ActiveSupport::TestCase
  self.described_class = StripeChargeRadarProcessor
  self.rspec_metadata = { vcr: true }



  context_ StripeChargeRadarProcessor, :vcr do
  context_ "#handle_event" do
      let(:purchase) { create(:purchase) }

      shared_examples_for "purchase doesn't exist without Stripe Connect" do
  context_ "when not in production environment" do
  ...
  context_ "when in production environment" do
          before do
  ...
  context_ "radar.early_fraud_warning.created" do
  context_ "without Stripe Connect" do
  ...
  context_ "with Stripe Connect" do
          let(:stripe_params) do
            {
  ...
  context_ "radar.early_fraud_warning.updated" do
  context_ "without Stripe Connect" do
          let!(:stripe_params) do
            {
  ...
  test "handles the event without creating a new EFW record" do
                expect_any_instance_of(EarlyFraudWarning).to receive(:update_from_stripe!).and_call_original
                expect do
                  StripeChargeRadarProcessor.handle_event(stripe_params)
                end.not_to change(EarlyFraudWarning, :count)
