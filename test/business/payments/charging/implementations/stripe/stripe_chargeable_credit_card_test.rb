...
class StripeChargeableCreditCardTest < ActiveSupport::TestCase
  self.described_class = StripeChargeableCreditCard
  self.rspec_metadata = { vcr: true }



  context_ StripeChargeableCreditCard, :vcr do
  ...
  test "returns persistable token" do
        chargeable.prepare!
  ...
  test "returns customer and payment method" do
        chargeable.prepare!
  ...
  context_ "#charge!" do
  context_ "when merchant account is not a stripe connect account" do
  ...
  test "charges using the credit card payment method id" do
  ...
  test "retrieves payment method id if it is not present" do
          expect_any_instance_of(StripeChargeableCreditCard).not_to receive(:prepare_for_direct_charge)
  ...
  test "charges the payment method on gumroad stripe account" do
  ...
  context_ "when merchant account is a stripe connect account" do
  ...
  test "charges using the cloned payment method" do
  ...
  test "charges the payment method cloned on the connected stripe account" do
          chargeable.prepare!
  ...
  context_ "when saved credit card only has a customer ID and no payment method ID" do
          let(:merchant_account) { create(:merchant_account_stripe_connect) }
          let(:chargeable) do
            StripeChargeableCreditCard.new(
              merchant_account,
  ...
  test "retrieves the payment method associated with the customer and clones it" do
            expect_any_instance_of(StripeChargeableCreditCard).to receive(:prepare_for_direct_charge).and_call_original
