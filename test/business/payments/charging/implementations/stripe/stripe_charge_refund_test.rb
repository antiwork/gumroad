...
class StripeChargeRefundTest < ActiveSupport::TestCase
  self.described_class = StripeChargeRefund
  self.rspec_metadata = { vcr: true }



  context_ StripeChargeRefund, :vcr do
  ...
  context_ "with a stripe refund" do
  test "has a charge_processor_id set to 'stripe'" do
          expect(subject.charge_processor_id).to eq "stripe"
        end

  test "has the #id from the stripe refund" do
          expect(subject.id).to eq(stripe_refund[:id])
        end

  test "has the #charge_id from the stripe refund" do
          expect(subject.charge_id).to eq(stripe_refund[:charge])
        end

  test "has the #charge_id from the original stripe charge" do
          expect(subject.charge_id).to eq(stripe_charge[:id])
        end
      end

  context_ "with a stripe refund for a charge that was destined for a managed account" do
  ...
  context_ "that involves the destination" do
          let(:stripe_refund_additional_params) do
            {
  ...
  context_ "#merchant_account_net_amount" do
  ...
  test "matches the amount of the destination payment refund minus Gumroad's application fee refund amount" do
  ...
  context_ "that doesn't involve the destination" do
  ...
  context_ "some sanity checks on what we expect from stripe" do
  test "Stripe should not have refunded the destination payment" do
  ...
  test "matches the currency of that the transaction was made in as fallback (cad)" do
                expect(settled_amount.currency).to eq(stripe_refund_bt.currency)
              end
  ...
  test "matches the currency of that the transaction was made in (usd)" do
                expect(gumroad_amount.currency).to eq(stripe_refund.currency)
              end

  test "matches the amount taken from the funds that gumroad holds (settled amount as fallback)" do
