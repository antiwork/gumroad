...
class StripeChargeProcessorTest < ActiveSupport::TestCase
  self.described_class = StripeChargeProcessor
  self.rspec_metadata = { vcr: true }



  context_ StripeChargeProcessor, :vcr do
  ...
  context_ "with Stripe token" do
  context_ "with only token" do
          let(:token) { CardParamsSpecHelper.success.to_stripejs_token }

  test "returns a chargeable token" do
  ...
  context_ "with token and zip code" do
          let(:token) { CardParamsSpecHelper.success.to_stripejs_token }
  ...
  context_ "with token and zip code and zip code required" do
  ...
  context_ "with Stripe payment method" do
  context_ "with only a payment method" do
          let(:payment_method_id) { StripePaymentMethodHelper.success.to_stripejs_payment_method_id }

  test "returns a chargeable payment method" do
  ...
  context_ "with a payment method" do
          let(:payment_method_id) { StripePaymentMethodHelper.success.to_stripejs_payment_method_id }
  ...
  context_ "with a payment method and a zip code and zip code not required" do
          let(:payment_method_id) { StripePaymentMethodHelper.success.to_stripejs_payment_method_id }
  ...
  context_ "with a payment method and a zip code and zip code required" do
  ...
  context_ "with data" do
  test "returns a chargeable" do
  ...
  context_ "with data including zip code" do
  test "returns a chargeable with zip code" do
          chargeable = subject.get_chargeable_for_data(
            "customer-id",
            "payment_method_id",
            "fingerprint",
  ...
  test "returns a Stripe::Charge object with details of the charge attached to the given purchase" do
  ...
  test "returns a Stripe::Charge object for the given Stripe Connect purchase" do
  ...
  test "returns nil if no stripe charge is found for the given purchase" do
        allow_any_instance_of(Purchase).to receive(:id).and_return(1234567890)
  ...
  test "retrieves charge from stripe" do
  ...
  test "calls update_dispute on the stripe charge with the evidence as a parameter" do
        stripe_charge.refresh
  ...
  test "includes the receipt image's Stripe file id in the dispute evidence" do
  ...
  test "includes the customer communication file's Stripe file id in the dispute evidence" do
        dispute_evidence = disputed_purchase.dispute.dispute_evidence
  ...
  context_ "when the associated product is not a membership" do
  test "includes the refund_policy's Stripe file id in the dispute evidence" do
  ...
  context_ "when the associated product is a membership" do
        let(:disputed_purchase) do
          create(
            :membership_purchase,
            price_cents: 100,
  ...
  test "includes the cancellation_policy's Stripe file id in the dispute evidence" do
  ...
  context_ "#create_dispute_evidence_stripe_file" do
  ...
  test "returns the Stripe::File id when the attachment downloads successfully" do
  ...
  test "returns nil when no file is attached" do
        expect(Stripe::File).not_to receive(:create)
        expect(subject.send(:create_dispute_evidence_stripe_file, unattached)).to be_nil
      end

  test "returns nil and notifies when the underlying file is missing from storage" do
  ...
  test "creates a setup intent" do
  ...
  context_ "for a managed Stripe account" do
  ...
  test "creates a setup intent on behalf of a managed account" do
          expect(Stripe::SetupIntent).to receive(:create).with(hash_including(payment_method: payment_method_id, customer: customer_id)).and_call_original
          subject.setup_future_charges!(merchant_account, chargeable)
        end
      end

  context_ "for a card with SCA support" do
        let(:stripe_card) { StripePaymentMethodHelper.success_with_sca }

  test "creates a setup intent that requires action" do
          expect(Stripe::SetupIntent).to receive(:create).with(hash_including(payment_method: payment_method_id, customer: customer_id)).and_call_original
          setup_intent = subject.setup_future_charges!(merchant_account, chargeable)
  ...
  test "creates a mandate if future off-session charges are required" do
            mandate_options = {
  ...
  context_ "for a non-Indian card" do
  context_ "with SCA support" do
            let(:stripe_card) { StripePaymentMethodHelper.success_with_sca }

  test "follows the default Radar rules and does not request 3DS manually" do
  ...
  context_ "without SCA support" do
            let(:stripe_card) { StripePaymentMethodHelper.success }
  ...
  test "passes on the reference" do
  ...
  test "passes on the description" do
        expect(Stripe::PaymentIntent).to receive(:create).with(hash_including(description: "test description")).and_call_original
        subject.create_payment_intent_or_charge!(merchant_account, chargeable, 1_00, 30, "reference", "test description")
      end

  context_ "for a card without SCA support" do
        let(:payment_method_id) { StripePaymentMethodHelper.success.to_stripejs_payment_method_id }

  test "returns a successful StripeChargeIntent with a charge" do
  ...
  context_ "when on-session" do
  test "returns a StripeChargeIntent that requires action" do
  ...
  context_ "when off-session" do
  context_ "usage was prepared for card" do
            let(:payment_method_id) { StripePaymentMethodHelper.success_future_usage_set_up.to_stripejs_payment_method_id }

  test "returns a successful StripeChargeIntent" do
  ...
  context_ "usage was not prepared for card" do
  test "fails with ChargeProcessorCardError" do
              expect do
  ...
  context_ "for a non-Indian card with SCA support" do
            let(:payment_method_id) { StripePaymentMethodHelper.success_future_usage_set_up.to_stripejs_payment_method_id }
  ...
  context_ "when on-session" do
  context_ "when NOT setting up future usage" do
  context_ "for an Indian card with SCA support" do
              let(:payment_method_id) { StripePaymentMethodHelper.build(token: "tok_in").to_stripejs_payment_method_id } # https://stripe.com/docs/testing#international-cards
  ...
  context_ "when setting up future usage" do
  context_ "for an Indian card with SCA support" do
              let(:payment_method_id) { StripePaymentMethodHelper.build(token: "tok_in").to_stripejs_payment_method_id } # https://stripe.com/docs/testing#international-cards
  ...
  context_ "if a reusable token has been requested already" do
        let(:reusable_token) { chargeable.reusable_token!(nil) }

  test "charges the persistable token (stripe customer)" do
  ...
  test "does not set a destination since it's on our stripe account" do
  ...
  context_ "on the merchant account mirroring a Stripe managed account" do
        let(:user) { create(:user) }
        let(:stripe_account) { create_verified_stripe_account(country: "US") }
  ...
  test "sets a stripe_account for the managed account" do
  ...
  test "sets the application fee to gumroad's portion of the transaction" do
          expect(Stripe::PaymentIntent).to receive(:create).with(hash_including(transfer_data: { destination: stripe_account.id, amount: 70 })).and_call_original
          subject.create_payment_intent_or_charge!(merchant_account, chargeable, 100, 30, "reference", "test description")
        end

  context_ "if the charge_processor_merchant_id isn't set for some reason" do
  context_ "nil" do
            before do
  ...
  test "raises an error containing card_declined error code" do
            subject.create_payment_intent_or_charge!(merchant_account, chargeable, 1_00, 30, "reference", "test description")
          rescue ChargeProcessorCardError => e
            expect(e.error_code).to eq("card_declined_generic_decline")
          end

  test "raises an error containing the charge id" do
            subject.create_payment_intent_or_charge!(merchant_account, chargeable, 1_00, 30, "reference", "test description")
          rescue ChargeProcessorCardError => e
  ...
  test "raises an error containing incorrect_cvc error code" do
            subject.create_payment_intent_or_charge!(merchant_account, chargeable, 1_00, 30, "reference", "test description")
          rescue ChargeProcessorCardError => e
  ...
  context_ "card declined due to Stripe expecting it's fraudulent" do
  ...
  context_ "statement description provided" do
  test "is sent if the statement description is provided" do
  ...
  test "allows dot and slashes but limit to 22 chars" do
  ...
  test "sanitizes the statement description" do
  ...
  test "is not sent if the statement description is blank" do
  ...
  test "is not sent if the statement description is nil" do
          expect(Stripe::PaymentIntent).to receive(:create).with(hash_not_including(:statement_descriptor_suffix)).and_call_original
  ...
  test "is not sent if the statement description is not provided" do
          expect(Stripe::PaymentIntent).to receive(:create).with(hash_not_including(:statement_descriptor_suffix)).and_call_original
          subject.create_payment_intent_or_charge!(merchant_account, chargeable, 1_00, 30, "reference", "the description")
        end

  test "is not sent if the statement description only has invalid characters" do
          expect(Stripe::PaymentIntent).to receive(:create).with(hash_not_including(:statement_descriptor_suffix)).and_call_original
  ...
  test "is not sent if the statement description only has invalid characters with whitespace" do
          expect(Stripe::PaymentIntent).to receive(:create).with(hash_not_including(:statement_descriptor_suffix)).and_call_original
  ...
  test "returns a ChargeIntent object" do
  ...
  context_ "when a blank charge intent ID is passed" do
  test "raises an error" do
          expect do
  ...
  context_ "when non-existing charge intent ID is passed" do
  test "raises an error" do
          expect do
  ...
  test "returns a SetupIntent object" do
  ...
  context_ "when a blank setup intent ID is passed" do
  test "raises an error" do
          expect do
  ...
  context_ "when non-existing setup intent ID is passed" do
  test "raises an error" do
          expect do
  ...
  context_ "when SCA was not performed" do
        let(:payment_method_id) { StripePaymentMethodHelper.success_with_sca.to_stripejs_payment_method_id }
  ...
  context_ "when SCA was performed" do
  ...
  test "confirms the payment intent" do
  ...
  context_ "when intent is pending SCA" do
        let(:payment_method_id) { StripePaymentMethodHelper.success_with_sca.to_stripejs_payment_method_id }

  test "cancels intent" do
  ...
  context_ "when intent has already succeeded" do
        let(:payment_method_id) { StripePaymentMethodHelper.success.to_stripejs_payment_method_id }

  test "fails with ChargeProcessorError" do
  ...
  test "calls refund on the stripe charge without an amount" do
          expect(Stripe::Refund).to receive(:create).with({ charge: charge_id }).and_call_original
          subject.refund!(charge_id)
        end

  test "calls refund with reason if refund is for fraud" do
  ...
  context_ "return value" do
          let(:charge_refund) { subject.refund!(charge_id) }

  test "returns a StripeChargeRefund" do
            expect(charge_refund).to be_a(StripeChargeRefund)
          end

  test "returns a StripeChargeRefund with a simple flow_of_funds" do
  ...
  test "calls refund on the stripe charge with reverse transaction and refund fees" do
  ...
  test "has refunded the application fee" do
            subject.refund!(charge_id)
  ...
  test "has reversed the transfer" do
            subject.refund!(charge_id)
  ...
  test "returns a StripeChargeRefund with a flow_of_funds" do
  ...
  context_ "without application fee" do
            let(:destination_currency) { Currency::CAD }
  ...
  test "does not have an application fee" do
              subject.refund!(charge_id)
              stripe_charge = Stripe::Charge.retrieve(id: charge_id, expand: %w[application_fee])
              expect(stripe_charge.application_fee).to be(nil)
            end

  test "reverses the transfer" do
              subject.refund!(charge_id)
              stripe_charge = Stripe::Charge.retrieve(id: charge_id, expand: %w[transfer])
              expect(stripe_charge.transfer.reversed).to eq(true)
            end
  ...
  context_ "for a charge with affiliate and without on_behalf_of & application_fee_amount parameters" do
            let(:stripe_charge) do
  ...
  test "returns a StripeChargeRefund with a flow_of_funds containing correct merchant and gumroad amounts" do
  ...
  context_ "with standard stripe connect account" do
  ...
  test "calls refund on the stripe charge with refund_application_fee flag set to false" do
  ...
  test "calls refund on the stripe charge with an amount" do
  ...
  test "calls refund on the stripe charge with the amount" do
  ...
  test "has not refunded the application fee" do
            subject.refund!(charge_id, amount_cents: refund_amount_cents)
            stripe_charge = Stripe::Charge.retrieve(id: charge_id, expand: %w[application_fee])
            expect(stripe_charge.application_fee.refunded).to eq(false)
          end

  test "has not reversed the transfer" do
            subject.refund!(charge_id, amount_cents: refund_amount_cents)
            stripe_charge = Stripe::Charge.retrieve(id: charge_id, expand: %w[transfer])
  ...
  context_ "already fully refunded" do
          before do
  ...
  context_ "refunding more than original amount" do
  test "raises invalid request error" do
            expect { subject.refund!(charge_id, amount_cents: 11_00) }.to raise_error(ChargeProcessorInvalidRequestError)
          end
        end

  context_ "multiple refunds" do
          before do
            subject.refund!(charge_id, amount_cents: 5_00)
          end

  test "allows a second partial refund" do
            expect { subject.refund!(charge_id, amount_cents: 5_00) }.not_to raise_error
          end

  context_ "when total partials would be more than original" do
  test "raises invalid request error" do
  ...
  context_ "refund another portion" do
  context_ "return value" do
  ...
  context_ "refund another portion that happens to be all remaining" do
  context_ "return value" do
  ...
  context_ "refund all remaining" do
  context_ "return value" do
                let(:charge_refund) { subject.refund!(charge_id) }
  ...
  context_ "merchant account is a stripe managed account" do
  ...
  context_ "merchant account is not a stripe managed account" do
  ...
  context_ "event charge succeeded" do
  ...
  test "works as only informational event if corresponding purchase is not on an Indian card" do
            expect(ChargeProcessor).to(receive(:handle_event)).with(an_instance_of(ChargeEvent)).and_call_original
  ...
  test "works as only informational event if corresponding purchase is not an off-session purchase" do
            expect(ChargeProcessor).to(receive(:handle_event)).with(an_instance_of(ChargeEvent)).and_call_original
  ...
  test "marks the corresponding purchase successful if it's in progress for an off-session charge on an Indian card" do
            expect(ChargeProcessor).to(receive(:handle_event)).with(an_instance_of(ChargeEvent)).and_call_original
  ...
  context_ "event charge refund updated" do
  ...
  test "refunds the purchase corresponding to the Stripe charge" do
  ...
  test "refunds the purchases corresponding to the Stripe charge in case of combined charge" do
            expect(ChargeProcessor).to(receive(:handle_event)).with(an_instance_of(ChargeEvent)).and_call_original
  ...
  test "updates the corresponding refund statuses if purchases are already refunded" do
  ...
  test "does not refund the purchases if refund status is not succeeded" do
  ...
  context_ "event payment failed" do
  ...
  test "marks the corresponding purchase failed if it's in progress for an off-session charge on an Indian card" do
            expect(ChargeProcessor).to(receive(:handle_event)).with(an_instance_of(ChargeEvent)).and_call_original
  ...
  context_ "event object: dispute" do
  ...
  context_ "event dispute updated" do
          let(:stripe_event_type) { "charge.dispute.updated" }

  context_ "for a charge on Gumroads account" do
  test "tells the charge processor about the informational event" do
              expect(ChargeProcessor).to receive(:handle_event).with(an_instance_of(ChargeEvent)).and_call_original
              StripeChargeProcessor.handle_stripe_event(stripe_event)
            end
          end

  context_ "for a charge on a managed account" do
  ...
  context_ "event dispute created" do
          let(:stripe_event_type) { "charge.dispute.created" }

  context_ "for a charge on Gumroads account" do
  test "tells the charge processor that a dispute was created" do
  ...
  context_ "event dispute funds withdrawn" do
  ...
  test "reverses the transfer and refunds the application fee" do
  ...
  context_ "replaying the event" do
              before do
                StripeChargeProcessor.handle_stripe_event(stripe_event)
              end

  test "does not reverse the transfer or refund the application fee" do
                original_stripe_charge_retrieve = Stripe::Charge.method(:retrieve)
                allow(Stripe::Charge).to receive(:retrieve).and_call_original
                expect(Stripe::Charge).to receive(:retrieve).with(hash_including(id: stripe_charge_id)) do |*args|
                  stripe_charge_retrieved = original_stripe_charge_retrieve.call(*args)
  ...
  context_ "for a charge on managed account without on_behalf_of and application_fee_amount parameters" do
            let(:stripe_managed_account) { create_verified_stripe_account(country: "CA", default_currency: Currency::CAD) }
            let(:stripe_charge_destination) { stripe_managed_account.id }
  ...
  context_ "event dispute funds reinstated" do
  ...
  test "creates a transfer for the funds" do
  ...
  test "tells the charge processor that the dispute was won" do
              stripe_transfer = nil
  ...
  context_ "for a charge on a managed account without on_behalf_of and application_fee_amount parameters" do
            let(:stripe_charge) do
  ...
  context_ "event dispute closed" do
          let(:stripe_event_type) { "charge.dispute.closed" }

  context_ "in our favor" do
  context_ "status 'won'" do
              before do
                Stripe::Dispute.update(stripe_dispute.id, { evidence: { uncategorized_text: "winning_evidence" } })
                stripe_dispute.refresh
                while stripe_dispute.status != "won"
  ...
  context_ "status 'warning_closed'" do
              before do
                Stripe::Dispute.update(stripe_dispute.id, { evidence: { uncategorized_text: "winning_evidence" } })
                stripe_dispute.refresh
                while stripe_dispute.status != "won"
  ...
  context_ "for a charge on Gumroads account" do
  test "tells the charge processor that hte dispute was won" do
                  original_handle_event = ChargeProcessor.method(:handle_event)
                  expect(ChargeProcessor).to(receive(:handle_event)) do |charge_event|
                    expect(charge_event).to be_a(ChargeEvent)
                    expect(charge_event.type).to eq(ChargeEvent::TYPE_DISPUTE_WON)
  ...
  context_ "NOT in our favor" do
  context_ "status 'lost'" do
              before do
  ...
  test "tells the charge processor the dispute was lost" do
                  original_handle_event = ChargeProcessor.method(:handle_event)
                  expect(ChargeProcessor).to(receive(:handle_event)) do |charge_event|
                    expect(charge_event).to be_a(ChargeEvent)
  ...
  context_ "event charge without charge id" do
  ...
  context_ "event dispute without charge id" do
          let(:stripe_event_type) { "charge.dispute.created" }
  ...
  context_ "event distpute without created at" do
          let(:stripe_event) do
            {
              "id" => stripe_event_id,
              "type" => "charge.created",
  ...
  context_ "event object: radar.early_fraud_warning.created" do
        let(:stripe_event) do
          {
  ...
  test "adds a negative credit if not already done for this paydown" do
  ...
  test "does not add another negative credit if one is already added for this paydown" do
            create(:credit,
                   user: creator,
  ...
  test "does not add a loan deduction if it is not deducted from Stripe payments as per the description" do
  ...
  context_ ".debit_stripe_account_for_refund_fee" do
      before do
        @merchant_account = create(:merchant_account, charge_processor_merchant_id: "acct_1MdawPS4gcql7bLm", country: "AE")
      end

  test "reverses an internal transfer made to the stripe connect account if present" do
  ...
  test "reverses the earliest sale transfer if no internal transfer is present" do
  ...
  context_ ".debit_stripe_account_for_australia_backtaxes" do
  test "does not attempt a transfer if there are already backtax collection transfers for this agreement" do
        ...
        end.not_to change { BacktaxCollection.count }
      end

  context_ "when merchant account is PayPal" do
        before do
          @merchant_account = create(:merchant_account_paypal, country: "VN")
        end

  test "creates a backtax collection with no stripe transfer id for the amount of the negative credit" do
  ...
  context_ "when merchant account is a creator's Gumroad-controlled Stripe account" do
  context_ "when the merchant account is US" do
          before do
  ...
  test "creates a debit transfer for the taxes owed amount" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
  ...
  context_ "when the merchant account is non-US" do
          before do
  ...
  test "creates a reversal from a single past internal transfer" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
            create(:payment_completed, user: @merchant_account.user,
                                       stripe_connect_account_id: @merchant_account.charge_processor_merchant_id,
                                       stripe_internal_transfer_id: nil)
  ...
  test "creates reversals for multiple past internal transfers" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
            create(:payment_completed, user: @merchant_account.user,
                                       stripe_connect_account_id: @merchant_account.charge_processor_merchant_id,
                                       stripe_internal_transfer_id: nil)
  ...
  test "creates reversals for multiple transfers associated with old purchases" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
            create(:payment_completed, user: @merchant_account.user,
                                       stripe_connect_account_id: @merchant_account.charge_processor_merchant_id,
                                       stripe_internal_transfer_id: nil)
  ...
  test "creates reversals for both past internal transfers and transfers associated with old purchases" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
            create(:payment_completed, user: @merchant_account.user,
                                       stripe_connect_account_id: @merchant_account.charge_processor_merchant_id,
                                       stripe_internal_transfer_id: nil)
  ...
  test "does not create reversals if the accumlated sum of transfer amounts is not enough to cover the taxes owed" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
            create(:payment_completed, user: @merchant_account.user,
                                       stripe_connect_account_id: @merchant_account.charge_processor_merchant_id,
                                       stripe_internal_transfer_id: nil)
  ...
  test "creates reversals in usd and skips reversals in cad when usd transfers cover the amount owed" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
            create(:payment_completed, user: @merchant_account.user,
                                       stripe_connect_account_id: @merchant_account.charge_processor_merchant_id,
                                       stripe_internal_transfer_id: nil)
  ...
  test "creates reversals in cad and skips reversals in usd when cad transfers cover the amount owed" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
            create(:payment_completed, user: @merchant_account.user,
                                       stripe_connect_account_id: @merchant_account.charge_processor_merchant_id,
                                       stripe_internal_transfer_id: nil)
  ...
  test "creates reversals covering the rest of the amount owed when only a partial amount has been transferred" do
  ...
  test "creates reversals when it is necessary to list Stripe transfers more than once" do
            credit = create(:credit, user: @merchant_account.user, amount_cents: -1000, merchant_account_id: @merchant_account.id, backtax_agreement: create(:backtax_agreement))
            create(:payment_completed, user: @merchant_account.user,
                                       stripe_connect_account_id: @merchant_account.charge_processor_merchant_id,
                                       stripe_internal_transfer_id: nil)
