...
class PaypalMerchantAccountManagerTest < ActiveSupport::TestCase
  ...
  context_ "handle_paypal_event" do
  context_ "when event type is #{PaypalEventType::MERCHANT_PARTNER_CONSENT_REVOKED}" do
  ...
  context_ "when merchant account is not present" do
  ...
  context_ "when merchant account is present" do
          before do
            @merchant_account = create(:merchant_account_paypal, charge_processor_merchant_id: paypal_event["resource"]["merchant_id"])
          end

  test "marks the merchant account as deleted" do
            described_class.new.handle_paypal_event(paypal_event)
            @merchant_account.reload
            expect(@merchant_account.alive?).to be(false)
          end

  test "marks all the merchant accounts as deleted if there are more than one connected to the same paypal account" do
  ...
  test "does nothing if there are no alive merchant accounts for the paypal account" do
  ...
  context_ "when user is merchant migration enabled" do
            before do
  ...
  test "marks the merchant account as deleted and disables sales" do
  ...
  context_ "when event type is #{PaypalEventType::MERCHANT_ONBOARDING_COMPLETED}" do
  context_ "when tracking_id is absent in the webhook payload" do
  ...
  context_ "when merchant account record is not present" do
          let!(:user) { create(:user) }
  ...
  test "does not create a new merchant account record" do
            expect do
              described_class.new.handle_paypal_event(paypal_event)
  ...
  context_ "when event type is #{PaypalEventType::MERCHANT_CAPABILITY_UPDATED}" do
  ...
  context_ "when merchant account record is present" do
          before do
  ...
  test "does not re-enable if merchant account is deleted" do
            @merchant_account.mark_deleted!
            expect(@merchant_account.reload.alive?).to be(false)
  ...
  test "updates the merchant account if it is not deleted" do
  ...
  context_ "when event type is #{PaypalEventType::MERCHANT_SUBSCRIPTION_UPDATED}" do
  context_ "when merchant account record is not present" do
  ...
  context_ "when event type is #{PaypalEventType::MERCHANT_EMAIL_CONFIRMED}" do
  context_ "when merchant account record is not present" do
  ...
  context_ "when event type is #{PaypalEventType::MERCHANT_ONBOARDING_SELLER_GRANTED_CONSENT}" do
  context_ "when merchant account record is not present" do
  ...
  context_ "#update_merchant_account" do
  test "sends a confirmation email when the paypal connect account is updated" do
        creator = create(:user)
  ...
  test "does not send a confirmation email when the paypal connect account info is not changed" do
        creator = create(:user)
  ...
  context_ "when oauth_integrations is missing from the PayPal response" do
  ...
  test "treats the account as incomplete instead of raising NoMethodError" do
  ...
  test "marks all other paypal merchant accounts of the creator as deleted" do
        creator = create(:user)
        creator.mark_compliant!(author_name: "ContentModeration")
        allow_any_instance_of(User).to receive(:sales_cents_total).and_return(100_00)
        create(:payment_completed, user: creator)
