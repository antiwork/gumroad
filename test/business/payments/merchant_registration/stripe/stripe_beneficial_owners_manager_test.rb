...
class StripeBeneficialOwnersManagerTest < ActiveSupport::TestCase
  self.described_class = StripeBeneficialOwnersManager



  context_ StripeBeneficialOwnersManager do
  ...
  context_ ".eligible?" do
  test "returns true for a business user with a Gumroad-managed Stripe merchant account" do
        expect(described_class.eligible?(user)).to be true
      end

  test "returns false when the user has no Gumroad-managed Stripe account" do
        allow(user).to receive(:stripe_account).and_return(nil)
        expect(described_class.eligible?(user)).to be false
      end

  test "returns false when the seller is on a Stripe Connect OAuth account" do
  ...
  test "returns false for an individual account" do
  ...
  context_ ".list" do
  test "returns every person on the account, including the representative, with relationship flags intact" do
  ...
  test "returns the lone representative when no other owners exist" do
  ...
  test "exposes nationality so the React form can pre-fill it when re-editing an owner" do
  ...
  test "does not expose the raw id_number or ssn_last_4" do
  ...
  test "raises NotEligibleError for ineligible users" do
        allow(user).to receive(:stripe_account).and_return(nil)
  ...
  test "creates a Stripe person with mapped relationship flags and no representative bit" do
  ...
  test "lets a Stripe ownership-percent error propagate to the caller" do
  ...
  test "raises MissingRequiredFieldError when email or phone is missing" do
  ...
  test "raises MissingRequiredFieldError when address sub-fields are missing" do
  ...
  test "does not require postal_code when the address country is Botswana (BW has no postal codes)" do
  ...
  test "requires nationality when seller's compliance country is one of AE/SG/BD/PK" do
  ...
  test "sends kanji/kana name + address_kanji + address_kana for JP business sellers" do
        user.alive_user_compliance_info.mark_deleted!
  ...
  test "sends kanji/kana names with a Latin address when a JP seller adds a non-JP-resident BO" do
        user.alive_user_compliance_info.mark_deleted!
        create(:user_compliance_info, user: user, country: "Japan", is_business: true,
                                      first_name_kanji: "太郎", last_name_kanji: "山田",
                                      first_name_kana: "タロウ", last_name_kana: "ヤマダ")
  ...
  test "sends full_name_aliases for SGP sellers (Singapore MAS rule — required on every Person)" do
        user.alive_user_compliance_info.mark_deleted!
  ...
  test "raises MissingRequiredFieldError when id_number is missing on create" do
  ...
  test "raises MissingRequiredFieldError when dob is missing entirely (API client bypassing the UI)" do
  ...
  test "raises MissingRequiredFieldError when one of day/month/year is blank" do
  ...
  test "accepts a blank address.state for countries outside the curated state-list set (Singapore, mirroring the rep flow)" do
        user.alive_user_compliance_info.mark_deleted!
        sg_compliance = create(:user_compliance_info_singapore, user: user)
        sg_compliance.dup_and_save! { |c| c.is_business = true }
        sg_params = params.deep_dup
  ...
  test "raises MissingRequiredFieldError when owner=true but percent_ownership is blank" do
  ...
  test "allows owner=false with blank percent_ownership (director-only BO)" do
  ...
  test "updates the person and never sets representative: true" do
  ...
  test "allows editing the representative's percent_ownership, title, and role flags only" do
  ...
  test "omits id_number when the form value is blank, preserving Stripe's stored value" do
  ...
  test "raises MissingRequiredFieldError when email or phone is missing on a non-rep update" do
        allow(Stripe::Account).to receive(:retrieve_person).with(stripe_account_id, "person_phil").and_return(other_owner_person)
  ...
  test "does not re-assert full_name_aliases on SGP non-rep updates so externally-set aliases are preserved" do
        user.alive_user_compliance_info.mark_deleted!
        sg_compliance = create(:user_compliance_info_singapore, user: user)
        sg_compliance.dup_and_save! { |c| c.is_business = true }
        allow(Stripe::Account).to receive(:retrieve_person).with(stripe_account_id, "person_phil").and_return(other_owner_person)
  ...
  context_ ".destroy" do
  test "deletes the person and returns a confirmation hash" do
        allow(Stripe::Account).to receive(:retrieve_person).with(stripe_account_id, "person_phil").and_return(other_owner_person)
  ...
  test "refuses to delete the representative" do
        allow(Stripe::Account).to receive(:retrieve_person).with(stripe_account_id, "person_rep").and_return(representative_person)
