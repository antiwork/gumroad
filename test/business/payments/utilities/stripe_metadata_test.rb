...
class StripeMetadataTest < ActiveSupport::TestCase
  self.described_class = StripeMetadata



  context_ StripeMetadata do
  context_ "build_metadata_large_list" do
      let(:key) { "myKey" }
      let(:items) { %w[asdfasdf asdfasdfasdfasdf] }

  test "returns a key-value pair hash of items" do
        expect(subject.build_metadata_large_list(items, key:)).to eq(
          "myKey{0}" => "asdfasdf,asdfasdfasdfasdf"
        )
      end

  context_ "key is symbol" do
        let(:key) { :my_key }

  test "results in a string key too" do
          expect(subject.build_metadata_large_list(items, key:)).to eq("my_key{0}" => "asdfasdf,asdfasdfasdfasdf")
        end
      end

  context_ "big list" do
        let(:items) { (1..99).map { |n| format("asdfasdf%.2i", n) } }

  test "returns a multiple key-value pairs" do
          expect(subject.build_metadata_large_list(items, key:)).to eq(
  ...
  test "returns a multiple key-value pairs with the big items in each one" do
          expect(subject.build_metadata_large_list(items, key:)).to eq(
            "myKey{0}" =>
    "asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01" \
    "asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01asdfasdf01" \
