...
class PaypalEventHandlerTest < ActiveSupport::TestCase
  self.described_class = PaypalEventHandler


  context_ PaypalEventHandler do
  context_ "#schedule_paypal_event_processing" do
  context_ "when event is from paypal orders API" do
  ...
  context_ "when event is from Integrated signup API" do
  ...
  context_ "when event is from paypal legacy API" do
  ...
  context_ "when event is from paypal legacy API" do
  context_ "and IPN verification succeeds" do
          before do
            WebMock.stub_request(:post, "https://ipnpb.sandbox.paypal.com/cgi-bin/webscr").to_return(body: "VERIFIED")
          end

  context_ "paypal events mocked from production" do
  context_ "pay via paypal and handle IPNs" do
  context_ "reversal and cancelled reversal IPN messages" do
  ...
  test "handles a chargeback message from PayPal" do
  ...
  test "handles a chargeback reversed message from PayPal" do
                    expect(PaypalPayoutProcessor).not_to receive(:handle_paypal_event)
  ...
  context_ "purchase event with an invoice field", :vcr do
              before do
                raw_payload =
  ...
  test "is handled by the PayPal charge processor" do
                expect(PaypalChargeProcessor).to receive(:handle_paypal_event).and_call_original
                expect(PaypalPayoutProcessor).not_to receive(:handle_paypal_event)
  ...
  context_ "payouts IPN messages" do
              before do
                raw_payload =
  ...
  test "assigns messages with the masspay transaction type to the payout processor" do
  ...
  context_ "ignored IPN messages" do
  context_ "cart checkout IPN messages" do
              before do
                raw_payload =
  ...
  test "does nothing with express checkout messages for Order creation and does not error out" do
                expect(Payment).not_to receive(:find)
  ...
  context_ "express checkout IPN messages", :vcr do
              before do
                raw_payload =
  ...
  context_ "express checkout IPN message with invalid IPN", :vcr do
              before do
                raw_payload =
                    "payment_date=17:36:04 Jul 22, 2015 PDT&txn_type=express_checkout&last_name=buyer&residence_country=US&pending_reason=order&item_name=Gumroad Purchase& " \
              "payment_gross=127.56&mc_currency=USD&verify_sign=AUkau1FwogE3kL3qo1vGTARqlijQAi30ARWjcqXEBUjAiWyIw2fh8BbU&payer_status=verified&test_ipn=1&tax=0.00&" \
  ...
  test "does nothing with invalid transaction ipn" do
                expect(Payment).not_to receive(:find)
                expect(Purchase).not_to receive(:find_by_external_id)
                expect(ErrorNotifier).not_to receive(:notify)
  ...
  context_ "Billing agreement creation checkout messages" do
              before do
                raw_payload =
  ...
  test "does nothing with express checkout messages for new billing agreeement and does not error out" do
                expect(Payment).not_to receive(:find)
                expect(Purchase).not_to receive(:find_by_external_id)
                expect(ErrorNotifier).not_to receive(:notify)
