...
class PaypalWebhookVerifierTest < ActiveSupport::TestCase
  self.described_class = PaypalWebhookVerifier



  context_ PaypalWebhookVerifier do
    let(:headers) do
      {
  ...
  test "returns false and logs error when network error occurs" do
