...
class CustomerPresenterTest < ActiveSupport::TestCase
  self.described_class = CustomerPresenter


  context_ CustomerPresenter do
    let(:seller) { create(:named_seller) }

  context_ "#missed_posts" do
  ...
  test "returns the correct props for each customer" do
  ...
  context_ "purchase has a call" do
        let(:call) { create(:call) }

  test "includes the call" do
  ...
  context_ "purchase has a commission", :vcr do
        let(:commission) { create(:commission) }
        let(:commission_file) { fixture_file_upload("test/support/fixtures/test.pdf") }
  ...
  test "includes the commission and custom fields" do
  ...
  context_ "purchase has an installment plan" do
  ...
  test "includes the installment plan" do
  ...
  context_ "purchase has a utm link" do
  ...
  context_ "when purchase has no full_name but purchaser has a name" do
        let(:purchaser) { create(:named_user) }
        let(:purchase) { create(:purchase, full_name: nil, purchaser:) }

  test "falls back to the purchaser name" do
          props = described_class.new(purchase:).customer(pundit_user:)
          expect(props[:name]).to eq(purchaser.name)
        end
      end

  context_ "when purchase has a whitespace-only full_name but purchaser has a name" do
        let(:purchaser) { create(:named_user) }
  ...
  context_ "when purchase has no full_name and no purchaser" do
  ...
  context_ "purchase has review videos" do
  ...
  test "includes the review videos" do
  ...
  context_ "purchase has downloads" do
        let(:purchase) { create(:purchase) }
  ...
  test "returns nil for download count" do
          presenter = described_class.new(purchase: purchase.reload)
          expect(presenter.download_count).to be_nil
        end
      end

  context_ "purchase for bundle" do
