...
class WorkflowPresenterTest < ActiveSupport::TestCase
  self.described_class = WorkflowPresenter



  context_ WorkflowPresenter do
    let(:seller) { create(:named_seller) }

  context_ "#new_page_react_props" do
  ...
  context_ "#edit_page_react_props" do
      it_behaves_like "with workflow form 'context' in response" do
        let(:user) { seller }
  ...
  test "returns the 'workflow' in response" do
  ...
  context_ "#workflow_props" do
      let(:seller) { create(:named_seller) }
      let(:product) { create(:product, user: seller) }
      let(:workflow) { create(:workflow, seller:, link: product) }

  test "includes the necessary workflow details" do
        ...
        expect(props.keys).not_to include(:abandoned_cart_products, :seller_has_products)
      end

  test "includes 'abandoned_cart_products' for an abandoned cart workflow" do
  ...
  test "includes 'seller_has_products' for an abandoned cart workflow" do
        workflow.update!(workflow_type: Workflow::ABANDONED_CART_TYPE)
  ...
  context_ "when the workflow is published" do
        before do
  ...
  context_ "when the workflow has installments" do
  ...
  test "returns installments in order of which ones will be delivered first" do
          props = described_class.new(seller:, workflow:).workflow_props
