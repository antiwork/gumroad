...
class CheckoutPresenterTest < ActiveSupport::TestCase
  self.described_class = CheckoutPresenter


  context_ CheckoutPresenter do
    include ManageSubscriptionHelpers
    include Rails.application.routes.url_helpers

  context_ "#checkout_props" do
      before do
        vcr_turned_on do
  ...
  test "returns basic props for the checkout page" do
  ...
  test "does not show paused upsells" do
  ...
  test "does not show paused upsell variants" do
        product = create(:product_with_digital_versions, user: create(:named_user))
        upsell = create(:upsell, seller: product.user, product:)
  ...
  test "does not show paused cross-sells" do
  ...
  test "does not accept paused upsells as accepted offers" do
        product = create(:product_with_digital_versions, user: create(:named_user))
        upsell = create(:upsell, seller: product.user, product:)
  ...
  test "allows adding a product" do
  ...
  test "allows adding products from a wishlist" do
  ...
  test "does not add deleted wishlist products" do
        wishlist = create(:wishlist)
  ...
  test "keeps per-product query growth bounded when rendering a large wishlist" do
        wishlist = create(:wishlist)
  ...
  context_ "when gifting a wishlist product" do
  ...
  test "clears the cart and gifts the product" do
          expect(@instance.checkout_props(params:, browser_guid:)).to include(
            clear_cart: true,
            gift: {
              type: "anonymous",
  ...
  test "falls back to the username when the user has not set a name" do
  ...
  test "does not add unavailable wishlist products" do
        wishlist = create(:wishlist)
  ...
  test "respects single-unit currencies in exchange_rate" do
  ...
  context_ "when all PayPal sales are disabled" do
        let!(:product) { create(:product) }

  test "returns nil for supports_paypal when the creator does not have their PayPal account connected" do
  ...
  test "returns nil for supports_paypal when the creator has their PayPal account connected" do
  ...
  context_ "when PayPal Connect sales are disabled" do
        before do
          Feature.activate(:disable_paypal_connect_sales)
          Feature.deactivate(:disable_braintree_sales)
        end

  context_ "when the product is a recurring subscription" do
          let (:product) { create(:subscription_product) }

  test "returns nil for supports_paypal" do
            expect(@instance.checkout_props(params: { product: product.unique_permalink }, browser_guid:)[:add_products].first[:product][:supports_paypal]).to be_nil
          end
        end

  context_ "when the product is not a recurring subscription" do
          let (:product) { create(:product) }

  test "returns braintree for supports_paypal" do
            expect(@instance.checkout_props(params: { product: product.unique_permalink }, browser_guid:)[:add_products].first[:product][:supports_paypal]).to eq "braintree"
          end

  test "returns nil for supports_paypal if Braintree sales are also disabled" do
  ...
  context_ "when PayPal Connect sales are disabled for NSFW products" do
        before do
          Feature.activate(:disable_nsfw_paypal_connect_sales)
        end

  context_ "when the product is NSFW" do
  ...
  context_ "when Braintree sales are disabled" do
        before do
          Feature.activate(:disable_braintree_sales)
        end

  test "returns nil for supports_paypal if product doesn't support native PayPal" do
          product = create(:product)
          expect(@instance.checkout_props(params: { product: product.unique_permalink }, browser_guid:)[:add_products].first[:product][:supports_paypal]).to be_nil
        end

  test "returns native for supports_paypal if product supports native PayPal" do
          seller = create(:user)
  ...
  test "returns nil for supports_paypal if native PayPal is also disabled" do
          Feature.activate(:disable_paypal_connect_sales)
  ...
  context_ "when the product is a bundle product" do
  ...
  test "includes the bundle products" do
  ...
  context_ "#checkout_product" do
  test "eager loads purchases to avoid N+1 queries" do
  ...
  context_ "#subscription_manager_props", :vcr do
  context_ "tiered membership product" do
        before :each do
  ...
  test "returns subscription data object for the subscription manage page" do
  ...
  test "does not return a deleted original offer code discount",
  ...
  test "does not return an ineligible tiered existing-customer discount",
           vcr: { cassette_name: "CheckoutPresenter/_subscription_manager_props/tiered_membership_product/returns_subscription_data_object_for_the_subscription_manage_page" } do
          buyer = create(:user)
  ...
  test "uses the logged-in viewer for renewal discount eligibility",
           vcr: { cassette_name: "CheckoutPresenter/_subscription_manager_props/tiered_membership_product/returns_subscription_data_object_for_the_subscription_manage_page" } do
          buyer = create(:user)
          ownership_product = create(:product, user: @product.user)
          create(:purchase,
  ...
  context_ "membership missing variants" do
          before :each do
            @purchase.variant_attributes = []
          end

  test "returns the default tier variant ID" do
            result = described_class.new(logged_in_user: nil, ip: "127.0.0.1").subscription_manager_props(subscription: @subscription)

            expect(result[:subscription][:option_id]).to eq @default_tier.external_id
          end

  context_ "when price has changed" do
  test "uses the new price for the default tier price" do
  ...
  context_ "membership for PWYW tier" do
          before do
  ...
  test "returns the correct current subscription price and tier displayed price" do
            result = described_class.new(logged_in_user: nil, ip: "127.0.0.1").subscription_manager_props(subscription: @subscription)
  ...
  test "returns the tier price when the tier price is lower than the current plan price" do
  ...
  test "returns the tier price when tier price is greater than the current plan price" do
            new_price = @pwyw_price_cents + 100
            @tier_price.update!(price_cents: new_price)
            result = described_class.new(logged_in_user: nil, ip: "127.0.0.1").subscription_manager_props(subscription: @subscription)
            displayed_tier_price = result[:product][:options][0][:recurrence_price_values]["monthly"][:price_cents]
  ...
  test "returns an auto-renewal discount after the original discount duration is exhausted" do
  ...
  context_ "non-tiered membership product" do
  context_ "subscription missing variants" do
  test "returns a nil option_id" do
  ...
  context_ "gifted membership product" do
  ...
  test "returns gift information" do
          result = described_class.new(logged_in_user: nil, ip: "127.0.0.1").subscription_manager_props(subscription:)
  ...
  context_ ".saved_card" do
  test "returns nil when no card is given" do
        expect(CheckoutPresenter.saved_card(nil)).to eq nil
      end

  test "returns nil when a paypal card is given" do
        expect(CheckoutPresenter.saved_card(CreditCard.new(card_type: "paypal", visual: "buyer@example.com"))).to eq nil
      end

  test "returns a serialized card when a credit card is given" do
