...
class ProfileSectionsPresenterTest < ActiveSupport::TestCase
  self.described_class = ProfileSectionsPresenter


  context_ ProfileSectionsPresenter do
  ...
  context_ "#cached_sections" do
  test "returns the cached array of sections for the given query" do
  ...
  test "returns the correct props for the seller" do
  ...
  test "returns the correct props for another user" do
        sections = common_sections_props
  ...
  context_ "sold-out product filtering" do
  ...
  test "excludes sold-out products with hide_sold_out_variants enabled from non-owner view" do
  ...
  test "includes sold-out products for the owner view" do
  ...
  test "does not exclude products with hide_sold_out_variants enabled that still have stock" do
        result = subject.props(request:, pundit_user:, seller_custom_domain_url: nil)
        product_section = result[:sections].find { _1[:type] == "SellerProfileProductsSection" }
        product_names = product_section[:search_results][:products].map { _1[:name] }
  ...
  test "preserves correct total count when sold-out products are filtered across paginated results" do
  ...
  test "does not fire per-product N+1 queries for the sold-out filter" do
  ...
  context_ "compute_description parameter" do
  test "passes compute_description: false to ProductPresenter.card_for_web for search results" do
