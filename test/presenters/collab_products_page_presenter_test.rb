...
class CollabProductsPagePresenterTest < ActiveSupport::TestCase
  self.described_class = CollabProductsPagePresenter
  self.rspec_metadata = { vcr: true }



  context_ CollabProductsPagePresenter, :vcr do
  ...
  context_ "#initial_page_props", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  test "returns stats and collab products" do
  ...
  test "returns collaborators_disabled_reason if using Brazilian Stripe Connect account" do
  ...
  test "caches product data", :sidekiq_inline do
        expect do
  ...
  context_ "#products_table_props", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  context_ "#memberships_table_props", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  test "returns memberships details" do
  ...
  test "caches memberships data", :sidekiq_inline do
        expect do
