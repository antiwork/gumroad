...
class AuthPresenterTest < ActiveSupport::TestCase
  self.described_class = AuthPresenter



  context_ AuthPresenter do
    let(:params) { {} }
    let(:application) { nil }
  ...
  context_ "#login_props" do
  context_ "with no params" do
  ...
  context_ "with an oauth application" do
  ...
  context_ "#signup_props" do
  context_ "with no options and data" do
        before do
  ...
  context_ "with options passed" do
  ...
  context_ "with a team invitation" do
  ...
  test "extracts the email to prefill" do
