...
class SettingsTeamPresenterTest < ActiveSupport::TestCase
  self.described_class = Settings::TeamPresenter



  context_ Settings::TeamPresenter do
  ...
  context_ "#member_infos" do
  context_ "without records" do
  test "returns owner member info only" do
  ...
  test "returns seller_one member infos" do
          member_infos = described_class.new(pundit_user:).member_infos
