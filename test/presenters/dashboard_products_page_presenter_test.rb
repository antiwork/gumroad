...
class DashboardProductsPagePresenterTest < ActiveSupport::TestCase
  self.described_class = DashboardProductsPagePresenter



  context_ DashboardProductsPagePresenter do
  ...
  context_ "#page_props" do
      let!(:archived_product) { create(:product, user: seller, archived: true) }

  test "returns has_products false when seller has no visible non-archived products" do
  ...
  context_ "when seller has visible non-archived products" do
        let!(:product) { create(:product, user: seller, name: "Active product") }

  test "returns has_products true" do
          presenter = described_class.new(pundit_user:)
          expect(presenter.page_props[:has_products].call).to eq(true)
        end

  test "keeps has_products true even when query has no matches" do
  ...
  context_ "#products_table_props" do
  ...
  test "returns only the seller's visible non-archived products" do
        presenter = described_class.new(pundit_user:)
  ...
  test "returns products with correct properties" do
        presenter = described_class.new(pundit_user:)
  ...
  test "returns empty results for a query that matches no products" do
  ...
  context_ "when the user has read-only access" do
        let(:pundit_user) { SellerContext.new(user: support_for_seller, seller:) }

  test "returns correct policy props" do
          presenter = described_class.new(pundit_user:)
          returned_product = presenter.products_table_props[:products].first
  ...
  context_ "#memberships_table_props" do
      include Rails.application.routes.url_helpers
  ...
  test "returns only the seller's visible non-archived memberships" do
        presenter = described_class.new(pundit_user:)
  ...
  test "returns memberships with correct properties" do
        presenter = described_class.new(pundit_user:)
  ...
  test "filters memberships by name" do
          presenter = described_class.new(pundit_user:, query: "another")
          membership_names = presenter.memberships_table_props[:memberships].map { |m| m["name"] }
  ...
  test "returns empty results for a query that matches no memberships" do
          presenter = described_class.new(pundit_user:, query: "nonexistent_xyz")
  ...
  context_ "products pagination" do
  ...
  test "returns paginated products" do
  ...
  test "returns correct page of products" do
  ...
  test "returns last page on page overflow" do
  ...
  context_ "when some products are deleted" do
        before do
          products.first(2).each { |p| p.update!(deleted_at: Time.current) }
        end

  test "paginates only visible products" do
          props = described_class.new(pundit_user:, products_page: 1).products_table_props
  ...
  context_ "memberships pagination" do
  ...
  test "returns paginated memberships" do
  ...
  test "returns correct page of memberships" do
  ...
  context_ "when some memberships are deleted" do
        before do
          memberships.first(2).each { |m| m.update!(deleted_at: Time.current) }
        end

  test "paginates only visible memberships" do
          props = described_class.new(pundit_user:, memberships_page: 1).memberships_table_props
  ...
  context_ "sorting + pagination", :elasticsearch_wait_for_refresh do
  ...
  test "paginates sorted results correctly" do
  ...
  test "sorts by successful_sales_count ascending" do
  ...
  test "sorts by successful_sales_count descending" do
  ...
  test "sorts by revenue ascending" do
  ...
  test "sorts by revenue descending" do
  ...
  test "sorts by display_price_cents ascending" do
  ...
  test "sorts by display_price_cents descending" do
  ...
  test "sorts by status ascending (unpublished first)" do
  ...
  test "sorts by status descending (published first)" do
  ...
  context_ "caching", :sidekiq_inline do
      let!(:product) { create(:product, user: seller) }
      let!(:membership) { create(:membership_product, user: seller) }

  test "caches dashboard data" do
        presenter = described_class.new(pundit_user:)
  ...
  context_ "with archived: true" do
  context_ "#empty?" do
  context_ "when there are no archived products or memberships" do
          before do
  ...
  context_ "when there are archived products" do
          before do
  ...
  context_ "when there are archived memberships" do
          before do
  ...
  context_ "when there are only deleted archived products" do
          before do
  ...
  context_ "#page_props" do
  test "returns only can_create_product (no archived_products_count)" do
          presenter = described_class.new(pundit_user:, archived: true)
          props = presenter.page_props
          expect(props[:has_products].call).to eq(false)
          expect(props[:can_create_product].call).to eq(true)
  ...
  test "returns only the seller's archived products" do
          presenter = described_class.new(pundit_user:, archived: true)
          product_names = presenter.products_table_props[:products].map { |p| p["name"] }
  ...
  test "returns empty results for a query that matches no archived products" do
  ...
  test "returns only the seller's archived memberships" do
          presenter = described_class.new(pundit_user:, archived: true)
          membership_names = presenter.memberships_table_props[:memberships].map { |m| m["name"] }
  ...
  test "returns empty results for a query that matches no archived memberships" do
            presenter = described_class.new(pundit_user:, archived: true, query: "nonexistent_xyz")
            expect(presenter.memberships_table_props[:memberships]).to be_empty
          end
        end
  ...
  context_ "sorting", :elasticsearch_wait_for_refresh do
        include_context "with products and memberships", archived: true

  context_ "archived products" do
  test "sorts by name ascending" do
  ...
  context_ "archived memberships" do
  test "sorts by name ascending" do
  ...
  context_ "#empty?" do
  context_ "when archived: false (default)" do
  test "returns nil" do
          presenter = described_class.new(pundit_user:)
