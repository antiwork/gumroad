...
class ProductPresenterProductPropsTest < ActiveSupport::TestCase
  self.described_class = ProductPresenter::ProductProps



  context_ ProductPresenter::ProductProps do
    include Rails.application.routes.url_helpers

    let(:presenter) { described_class.new(product:) }

  context_ "#props", :vcr do
  ...
  context_ "when requested from gumroad domain" do
  ...
  test "returns properties for the product page" do
  ...
  test "returns the product-level refund policy" do
  ...
  context_ "when the fine_print is empty" do
              before do
                product_refund_policy.update!(fine_print: "")
              end

  test "returns the refund policy" do
                expect(presenter.props(seller_custom_domain_url: nil, request:, pundit_user:)[:product][:refund_policy]).to eq(
                  {
                    title: product_refund_policy.title,
                    fine_print: nil,
  ...
  context_ "with invalid offer code" do
  test "returns an error if the offer code is sold out" do
  ...
  test "returns an error if the offer code doesn't exist" do
  ...
  context_ "digital versioned product" do
  ...
  test "handles users without a username set" do
  ...
  test "uses the default offer code when no discount code is provided" do
  ...
  test "uses a better offer code when provided and overrides the default" do
  ...
  test "uses the default offer code when an inferior offer code is provided" do
  ...
  test "uses the default when an inferior percentage offer code is provided" do
  ...
  test "sets bundle_products correctly" do
  ...
  context_ "apply_to_all_products is true" do
  context_ "collaborator dont_show_as_co_creator is false" do
  test "includes the collaborating user" do
  ...
  context_ "product affiliate dont_show_as_co_creator is true" do
  ...
  test "caches sales_count and tracks cache hits/misses", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  test "includes free downloads in the sales_count for products with paid variants", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  context_ "with current seller" do
        let(:product) { create(:product) }
        let(:pundit_user) { SellerContext.new(user: buyer, seller: buyer) }

  test "includes wishlists" do
  ...
  context_ "when custom domain is specified" do
        let(:product) { create(:product) }

  test "uses the custom domain for the seller profile url" do
          expect(
