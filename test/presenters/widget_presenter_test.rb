...
class WidgetPresenterTest < ActiveSupport::TestCase
  self.described_class = WidgetPresenter



  context_ WidgetPresenter do
    include Rails.application.routes.url_helpers

    let!(:demo_product) { create(:product, unique_permalink: "demo") }

  context_ "#widget_props" do
  context_ "when user is signed in" do
        let(:user) { create(:user) }

        subject { described_class.new(seller: user) }

  context_ "when user doesn't have own products" do
  test "returns a demo product" do
  ...
  context_ "when user has own products" do
          let!(:product) { create(:product, user:) }

  test "returns user's own products" do
            expect(subject.widget_props).to eq(
              {
                display_product_select: true,
                default_product: {
  ...
  context_ "when user has affiliated products" do
  ...
  test "returns demo product and affiliated products" do
            expect(subject.widget_props).to eq(
              {
                display_product_select: true,
                default_product: {
  ...
  context_ "when user has multiple products" do
  ...
  context_ "when product argument is provided" do
            subject { described_class.new(seller: user, product: old_product) }

  test "returns the default_product as the provided product" do
              expect(subject.widget_props).to eq(
                {
  ...
  context_ "when product argument is not provided" do
            subject { described_class.new(seller: user) }

  test "returns the default_product as the newest user product" do
              expect(subject.widget_props).to eq(
                {
                  display_product_select: true,
                  default_product: {
