...
class CreatorHomePresenterTest < ActiveSupport::TestCase
  self.described_class = CreatorHomePresenter


  context_ CreatorHomePresenter do
  ...
  context_ "#creator_home_props" do
  test "deduces creator name from payout info" do
  ...
  test "when creator has made a sale" do
  ...
  test "returns only first_product in getting started stats when checklist is dismissed" do
  ...
  test "includes initial user getting started stats" do
  ...
  test "doesn't consider workflow installments for first_email" do
        create(:installment, seller:, workflow_id: 1)
        expect(presenter.creator_home_props[:getting_started_stats]["first_email"]).to eq(false)
      end

  test "doesn't consider bundle product purchases for first_sale" do
  ...
  test "includes updated user getting started stats" do
  ...
  test "includes product data", :sidekiq_inline, :elasticsearch_wait_for_refresh  do
  ...
  test "excludes soft-deleted thumbnails from product data" do
  ...
  test "excludes deleted products from the Best Selling section", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  test "shows the 3 most sold products in past 30 days", :sidekiq_inline, :elasticsearch_wait_for_refresh do
        product1 = create(:product, user: seller)
        product2 = create(:product, user: seller)
  ...
  test "includes sorted interlaced activity items", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  test "includes the verification error message from Stripe if there's an active Stripe account" do
  ...
  test "includes formatted balance information" do
  ...
  context_ "when seller should be shown currencies always" do
          before do
            allow(seller).to receive(:should_be_shown_currencies_always?).and_return(true)
          end

  test "includes currency in formatted balance information" do
            balances = presenter.creator_home_props[:balances]
  ...
  test "includes tax forms since the seller's account was created" do
  ...
  test "only includes years where the seller is eligible for a 1099" do
  ...
  test "only includes years that have downloadable tax forms" do
  ...
  test "returns empty tax forms when tax_center feature flag is enabled for US user" do
          create(:user_compliance_info, user: seller)
  ...
  test "limits tax form years to avoid timeouts for long-lived accounts" do
  ...
  test "disables tax center for non-US user even with feature flag enabled" do
