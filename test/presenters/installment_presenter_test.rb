...
class InstallmentPresenterTest < ActiveSupport::TestCase
  self.described_class = InstallmentPresenter


  context_ InstallmentPresenter do
  ...
  test "returns true 'has_scheduled_emails' when seller has scheduled emails" do
  ...
  test "returns necessary props with `installment` attribute when `copy_from` is present" do
  ...
  test "includes the necessary installment details" do
        ...
        expect(props.keys).not_to include(:published_once_already, :member_cancellation, :new_customers_only, :delayed_delivery_time_duration, :delayed_delivery_time_period, :displayed_delayed_delivery_time_period)
        expect(props.keys).not_to include(:recipient_description, :to_be_published_at)
      end

  context_ "when installment is of type `product`" do
  ...
  test "includes necessary attributes" do
          props = described_class.new(seller:, installment:).props
          expect(props).to match(a_hash_including(
            external_id: installment.external_id,
  ...
  context_ "when installment is of type `variant`" do
  ...
  context_ "when installment is of type `seller`" do
        let(:variant) { create(:variant, variant_category: create(:variant_category, link: product)) }
  ...
  context_ "when installment is of type `follower`" do
  ...
  context_ "when installment is of type `audience`" do
  ...
  context_ "when installment is of type `affiliate`" do
  ...
  context_ "when installment is not published" do
        before do
          installment.update!(published_at: nil)
        end

  test "includes appropriate 'recipient_description' for a seller type installment" do
  ...
  test "includes appropriate 'recipient_description' for a product type installment" do
  ...
  test "includes appropriate 'recipient_description' for a variant type installment" do
  ...
  test "includes appropriate 'recipient_description' for a follower type installment" do
  ...
  test "includes appropriate 'recipient_description' for an audience type installment" do
  ...
  test "includes appropriate 'recipient_description' for an affiliate type installment" do
  ...
  test "includes appropriate 'recipient_description' for an affiliate product post" do
  ...
  context_ "when installment is scheduled" do
  test "includes 'to_be_published_at'" do
  ...
  context_ "when installment is in draft" do
  test "does not include 'to_be_published_at'" do
            props = described_class.new(seller:, installment:).props

            expect(props).not_to have_key(:to_be_published_at)
          end
        end
      end

  context_ "for a workflow installment" do
  ...
  test "includes additional workflow-related details" do
          props = described_class.new(seller:, installment:).props
          ...
          expect(props.keys).not_to include(:clicked_urls, :view_count, :full_url, :recipient_description, :to_be_published_at)
        end
      end
    end
  end
