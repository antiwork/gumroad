...
class AdminPaymentPresenterTest < ActiveSupport::TestCase
  self.described_class = Admin::PaymentPresenter



  context_ Admin::PaymentPresenter do
  context_ "#props" do
  ...
  context_ "when payment has a user" do
  ...
  context_ "when payment has no user" do
  ...
  test "identifies non-terminal state correctly" do
            expect(props[:non_terminal_state]).to be true
          end
        end

  context_ "when payment is failed" do
  ...
  test "identifies terminal state correctly" do
            expect(props[:non_terminal_state]).to be false
          end
        end

  context_ "when payment is cancelled" do
  ...
  context_ "when payment is returned" do
  ...
  context_ "when payment is unclaimed" do
  ...
  context_ "processor types" do
  context_ "when payment uses PayPal processor" do
          let(:payment) do
  ...
  test "includes PayPal-specific information" do
  ...
  test "identifies PayPal processor correctly" do
  ...
  context_ "when payment uses Stripe processor" do
          let(:payment) do
            create(:payment,
  ...
  test "includes Stripe-specific information" do
  ...
  test "identifies Stripe processor correctly" do
  ...
  context_ "bank account information" do
  context_ "when payment has no bank account" do
          let(:payment) { create(:payment, bank_account: nil) }

  test "returns nil for bank_account" do
            expect(props[:bank_account]).to be_nil
          end
        end

  context_ "when payment has a bank account" do
          let(:bank_account) { create(:uk_bank_account) }
          let(:payment) { create(:payment, bank_account:) }

  test "returns bank account information" do
  ...
  context_ "when bank account has a credit card" do
  ...
  test "includes the credit card visual" do
              expect(props[:bank_account][:credit_card][:visual]).to eq("Visa ending in 1234")
            end
          end

  context_ "when bank account has no credit card" do
  test "includes nil for credit card visual" do
