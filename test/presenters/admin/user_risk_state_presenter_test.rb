...
class AdminUserRiskStatePresenterTest < ActiveSupport::TestCase
  self.described_class = Admin::UserRiskStatePresenter



  context_ Admin::UserRiskStatePresenter do
  context_ "#props" do
  test "returns compliant state for a compliant user" do
  ...
  test "returns suspended state for a suspended fraud user" do
  ...
  test "returns flagged state for a flagged fraud user" do
  ...
  test "returns the most recent risk-state comment timestamp" do
        user = create(:compliant_user)
