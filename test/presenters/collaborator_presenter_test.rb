...
class CollaboratorPresenterTest < ActiveSupport::TestCase
  self.described_class = CollaboratorPresenter



  context_ CollaboratorPresenter do
  context_ "#new_collaborator_props" do
  ...
  test "returns the seller's visible and not archived products" do
        ...
        expect(props[:form_data]).not_to include(:id)
  ...
  context_ "#collaborator_props" do
      let(:seller) { create(:user) }
      let(:visible_product) { create(:product, user: seller) }
      let!(:archived_product) { create(:product, user: seller, archived: true) }
      let!(:deleted_product) { create(:product, user: seller, deleted_at: 1.day.ago) }
  ...
  test "returns the collaborator and its products" do
  ...
  context_ "#edit_collaborator_props" do
      let(:seller) { create(:user) }
      let!(:visible_product) { create(:product, user: seller) }
      let!(:archived_product) { create(:product, user: seller, archived: true) }
      let!(:deleted_product) { create(:product, user: seller, deleted_at: 1.day.ago) }
  ...
  test "returns the collaborator and eligible products" do
  ...
  context_ "#incoming_collaborator_props" do
      let(:seller) { create(:user) }
  ...
  test "returns the collaborator with seller info and products" do
  ...
  context_ "#inertia_shared_props" do
      let(:seller) { create(:user) }

  test "returns nil when seller does not have a Brazilian Stripe Connect account" do
        props = described_class.new(seller:).inertia_shared_props

        expect(props[:collaborators_disabled_reason]).to be_nil
      end

  test "returns collaborators supported as false if using a Brazilian Stripe Connect account" do
