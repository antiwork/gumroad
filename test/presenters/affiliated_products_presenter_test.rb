...
class AffiliatedProductsPresenterTest < ActiveSupport::TestCase
  self.described_class = AffiliatedProductsPresenter



  context_ AffiliatedProductsPresenter do
    include CurrencyHelper

  context_ "#affiliated_products_page_props", :vcr do
      # Users
  ...
  test "returns affiliated products details, stats, and global affiliates data" do
  ...
  test "returns affiliates_disabled_reason if using Brazilian Stripe Connect account" do
  ...
  context_ "when there is a search query" do
  context_ "when the query exactly matches an affiliated product" do
          let(:query) { "Creator 1 Product 1" }

  test "returns only the matching product" do
  ...
  context_ "when the query partially matches an affiliated product" do
          let(:query) { "Creator 1" }

  test "returns all matching products" do
            props = described_class.new(affiliate_user, query:).affiliated_products_page_props
            products_details = [
              { fee_percentage: 15,
                humanized_revenue: "$2.36",
  ...
  context_ "when the query does not match any affiliated products" do
  ...
  context_ "supports pagination" do
        before { stub_const("AffiliatedProductsPresenter::PER_PAGE", 5) }

  test "returns page 1 by default" do
          props = described_class.new(affiliate_user).affiliated_products_page_props
  ...
  test "returns the specified page if in range" do
  ...
  test "returns the last page if out of range" do
  ...
  context_ "when sorting" do
        before { stub_const("AffiliatedProductsPresenter::PER_PAGE", 1) }

  test "returns the products sorted by created timestamp by default" do
          props = described_class.new(affiliate_user).affiliated_products_page_props
  ...
  test "returns the products sorted by revenue when specified" do
  ...
  test "returns the products sorted by sales when specified" do
  ...
  test "returns the products sorted by name when specified" do
  ...
  test "returns the products sorted by commission when specified" do
  ...
  test "returns the products sorted by created timestamp when the sort field is invalid" do
  ...
  test "returns the products in ascending order when the sort direction is invalid" do
  ...
  context_ "#affiliated_products_page_props when the global affiliate has been deleted" do
  test "returns nil for global affiliate fields without raising" do
  ...
  context_ "#archived_tab_visible" do
      let(:seller) { create(:user, username: "seller1") }
      let!(:product) { create(:product, archived: true, user: seller) }

  test "returns archived products present and feature active" do
