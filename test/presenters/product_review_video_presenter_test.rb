...
class ProductReviewVideoPresenterTest < ActiveSupport::TestCase
  self.described_class = ProductReviewVideoPresenter



  context_ ProductReviewVideoPresenter do
  ...
  context_ "can_approve" do
  context_ "when the user has approval permission" do
          let(:pundit_user) { SellerContext.new(user: seller, seller:) }
  ...
  context_ "when the user does not have approval permission" do
  ...
  context_ "can_reject" do
  context_ "when the user has rejection permission" do
          let(:pundit_user) { SellerContext.new(user: seller, seller:) }
  ...
  context_ "when the user does not have rejection permission" do
          let(:pundit_user) { SellerContext.new(user: another_seller, seller: another_seller) }
