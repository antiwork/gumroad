...
class CommunitiesPresenterTest < ActiveSupport::TestCase
  self.described_class = CommunitiesPresenter



  context_ CommunitiesPresenter do
  ...
  context_ "when notification settings are missing" do
          before { notification_setting.destroy! }

  test "returns default notification settings" do
  ...
  test "includes unread counts and last read timestamps" do
  ...
  context_ "when there are no last read messages" do
          let!(:message1) { create(:community_chat_message, community:, user: seller, created_at: 3.minutes.ago) }
          let!(:message2) { create(:community_chat_message, community:, user: seller, created_at: 2.minutes.ago) }
          let!(:message3) { create(:community_chat_message, community:, user: seller, created_at: 1.minute.ago) }

  test "returns all messages as unread" do
            community_props = props[:communities].sole
  ...
  context_ "when user has multiple communities" do
