...
class PaginatedInstallmentsPresenterTest < ActiveSupport::TestCase
  self.described_class = PaginatedInstallmentsPresenter


  context_ PaginatedInstallmentsPresenter do
  context_ "#props" do
  ...
  test "returns paginated installments for the first page" do
  ...
  test "returns paginated installments for the specified page" do
          result = presenter.props
  ...
  context_ "when 'type' is 'scheduled'" do
        let(:type) { "scheduled" }
  ...
  test "returns scheduled installments ordered by 'to_be_published_at' earliest first" do
          result = presenter.props
  ...
  context_ "when the specified 'type' option is invalid" do
        let(:type) { "invalid" }
  ...
  context_ "when seller has no installments" do
  ...
  test "returns has_posts as false" do
          result = presenter.props
  ...
  context_ "when a non-nil 'query' option is specified" do
  ...
  test "returns paginated installments for the specified query for the first page" do
  ...
  test "returns paginated installments for the specified query for the specified page" do
            expect(InstallmentSearchService).to receive(:search).with({
                                                                        exclude_deleted: true,
                                                                        type:,
                                                                        exclude_workflow_installments: true,
