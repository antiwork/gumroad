...
class PaginatedUtmLinksPresenterTest < ActiveSupport::TestCase
  self.described_class = PaginatedUtmLinksPresenter



  context_ PaginatedUtmLinksPresenter do
  ...
  context_ "#props" do
  test "returns the paginated UTM links props" do
  ...
  test "sorts by date by default" do
        props = described_class.new(seller:).props
  ...
  context_ "sorting" do
  test "sorts by different columns" do
  ...
  context_ "when sorted by sales_count, revenue_cents, or conversion_rate columns" do
  test "returns values for stats" do
  ...
  context_ "when sorted by any other column" do
  test "returns nil values for stats" do
