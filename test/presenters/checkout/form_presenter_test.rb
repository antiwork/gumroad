...
class CheckoutFormPresenterTest < ActiveSupport::TestCase
  self.described_class = Checkout::FormPresenter


  context_ Checkout::FormPresenter do
  context_ "#form_props" do
  ...
  context_ "when tipping is enabled for the user" do
        before do
          seller.update!(tipping_enabled: true)
        end

  test "returns true for tipping_enabled" do
          expect(presenter.form_props[:user][:tipping_enabled]).to eq(true)
        end
      end

  context_ "when the seller has the offer code field enabled" do
        before do
  ...
  context_ "when the seller has an alive product" do
        let!(:product) { create(:product, user: seller) }

  test "includes it as a cart item, card product, and in the list of products" do
  ...
  context_ "when the seller has custom fields" do
  test "returns the correct props" do
