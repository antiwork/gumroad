...
class ProductPresenterTest < ActiveSupport::TestCase
  self.described_class = ProductPresenter



  context_ ProductPresenter do
    include Rails.application.routes.url_helpers
    include PreorderHelper
    include ProductsHelper

  context_ ".new_page_props" do
  ...
  test "returns well-formed props with show_orientation_text true for new users with no products" do
  ...
  test "returns well-formed props with show_orientation_text false for existing users with products" do
  ...
  test "returns properties from the page presenter" do
  ...
  context_ "#product_page_props" do
  ...
  test "returns the properties for the product page" do
  ...
  context_ "layout-specific props methods" do
      let(:request) { ActionDispatch::TestRequest.create }
      let(:pundit_user) { SellerContext.new(user: @user, seller: @user) }
      let(:seller) { create(:user) }
  ...
  test "returns product_page_props with sections for default layout" do
  ...
  test "returns product_props without sections for iframe layout" do
        props = presenter.iframe_product_props(**base_kwargs)
        expect(props[:product]).to be_present
        expect(props).not_to have_key(:sections)
      end

  test "merges creator_profile for profile layout" do
  ...
  test "merges discover_props for discover layout" do
  ...
  test "returns the properties for the product edit page", :freeze_time do
  ...
  context_ "when the price_checker feature flag is enabled for the seller" do
        before { Flipper.enable(:price_checker, product.user) }

  test "exposes price_checker_enabled: true in edit_props" do
          expect(presenter.edit_props[:price_checker_enabled]).to eq(true)
        end
      end

  context_ "with default offer code" do
  ...
  test "includes default offer code data in edit_props" do
  ...
  test "includes existing-customer-only default offer code data in edit_props" do
  ...
  test "returns properties for the product edit page" do
  ...
  test "returns availabilities" do
  ...
  context_ "with community chat enabled" do
        before do
  ...
  context_ ".card_for_web" do
      let(:request) { instance_double(ActionDispatch::Request, host: "test.gumroad.com", host_with_port: "test.gumroad.com:1234", protocol: "http") }
      let(:product) { create(:product) }

  test "returns properties from the card presenter" do
  ...
  test "passes compute_description parameter to the card presenter" do
  ...
  test "defaults compute_description to true when not provided" do
        expect(ProductPresenter::Card).to receive(:new).with(product:).and_call_original
  ...
  test "returns product data object for the admin page" do
  ...
  context_ "empty custom attributes" do
        before do
  ...
  test "excludes fully empty custom attributes" do
  ...
  context_ "a membership product" do
        before do
  ...
  test "returns existing files" do
