...
class UrlRedirectPresenterTest < ActiveSupport::TestCase
  self.described_class = UrlRedirectPresenter



  context_ UrlRedirectPresenter do
    include Rails.application.routes.url_helpers

  context_ "#download_attributes" do
  test "returns all necessary attributes for the download page" do
  ...
  test "includes thumbnail_url for product files that have thumbnails" do
        product = create(:product)
        file = create(:streamable_video, link: product)
  ...
  context_ "#download_page_with_content_props" do
      before do
  ...
  test "returns all props for DownloadPageWithContent component" do
  ...
  test "includes 'custom_receipt' in props" do
  ...
  test "includes 'last_content_page_id' in props" do
        instance = described_class.new(url_redirect: @url_redirect, logged_in_user: @user)
  ...
  test "includes 'discord' in props" do
  ...
  test "includes 'download_all_button' in props for installments with files" do
        freeze_time do
  ...
  test "returns nil for `download_all_button` when the URL redirect is not for an installment" do
        file1 = create(:readable_document)
        file2 = create(:streamable_video)
  ...
  test "includes 'creator' in props" do
  ...
  test "includes 'membership' in props" do
  ...
  test "includes rich content in props" do
  ...
  context_ "'posts' in props" do
  context_ "when rich content is present" do
  context_ "when rich content contains a 'posts' embed" do
            before do
              create(:rich_content, entity: @product, title: "Page title", description: [{ "type" => "posts" }])
            end

  test "includes 'posts' in props" do
              instance = described_class.new(url_redirect: @url_redirect, logged_in_user: @user)
  ...
  test "includes 'posts' in props for an installment URL redirect" do
  ...
  test "includes license in props" do
  ...
  context_ "when the rich content contains video embeds" do
  ...
  context_ "when logged in user is not the seller" do
  ...
  context_ "when all videos are transcoded" do
            before do
  ...
  context_ "when all videos are not transcoded" do
            before do
              create(:transcoded_video, streamable: video1, is_hls: true)
            end

  context_ "when videos are set to transcode on first sale" do
              before do
                @product.update!(transcode_videos_on_purchase: true)
              end

  test "returns the video transcoding info" do
                expect(presenter.download_page_with_content_props[:content][:video_transcoding_info]).to eq(transcode_on_first_sale: true)
              end
            end

  context_ "when videos are not set to transcode on first sale" do
  test "returns the video transcoding info" do
  ...
  context_ "community_chat_url prop" do
  ...
  test "returns nil when purchase is not present" do
  ...
  test "returns nil when communities feature is not active for seller" do
          expect(presenter.download_page_with_content_props[:content][:community_chat_url]).to be_nil
        end

  context_ "when communities feature is active for seller" do
          before do
            Feature.activate_user(:communities, product.user)
          end

  test "returns nil when community chat is not enabled for product" do
            expect(presenter.download_page_with_content_props[:content][:community_chat_url]).to be_nil
          end

  context_ "when community chat is enabled for product" do
            before do
              product.update!(community_chat_enabled: true)
            end

  test "returns nil when product has no active community" do
              expect(presenter.download_page_with_content_props[:content][:community_chat_url]).to be_nil
            end

  test "returns nil when purchase has no purchaser_id" do
              purchase.update!(purchaser_id: nil)
              expect(presenter.download_page_with_content_props[:content][:community_chat_url]).to be_nil
            end

  test "returns community path when product has an active community" do
  ...
  test "returns login path with next parameter when user is not logged in" do
                community = create(:community, seller: product.user, resource: product)
  ...
  context_ "when purchase has no purchaser_id" do
              before do
                purchase.update!(purchaser_id: nil)
              end

  test "returns signup path with next parameter" do
                community = create(:community, seller: product.user, resource: product)
  ...
  context_ "#download_page_without_content_props" do
      before do
  ...
  context_ "when the purchase is a call purchase" do
        let!(:call) { create(:call) }

        before { call.purchase.create_url_redirect! }

  test "includes the call in the props" do
  ...
  context_ "when the call has no url" do
          before { call.update!(call_url: nil) }

  test "does not include the url in the props" do
  ...
  test "does not include purchase email when the email confirmation is required" do
  ...
  test "includes 'installment' and correct 'creator' in props" do
  ...
  context_ "with completed installment plan" do
  test "marks installment plan as completed when all installments are completed" do
  ...
  test "does not mark installment plan as completed when installments are incomplete" do
          purchase = create(:installment_plan_purchase)
          subscription = purchase.subscription
          product = subscription.link
  ...
  test "returns props for the stream page" do
  ...
  test "returns nil for purchase_id when purchase is not present" do
  ...
  context_ "when logged in user is the seller" do
        let(:seller) { product.user }

  test "shows transcoding notice when videos are not transcoded" do
  ...
  test "does not show transcoding notice when videos are transcoded" do
  ...
  test "does not show transcoding notice" do
  ...
  context_ "transcode_on_first_sale" do
  test "returns true when product has transcode_videos_on_purchase enabled" do
          product.update!(transcode_videos_on_purchase: true)
          presenter = described_class.new(url_redirect:, logged_in_user: nil)
          props = presenter.stream_page_props(product_file: video_file)
  ...
  test "returns false when product does not have transcode_videos_on_purchase enabled" do
          presenter = described_class.new(url_redirect:, logged_in_user: nil)
          props = presenter.stream_page_props(product_file: video_file)
  ...
  context_ "with multiple video files" do
  ...
  test "returns index_to_play for the requested product file" do
          presenter = described_class.new(url_redirect:, logged_in_user: nil)
          props = presenter.stream_page_props(product_file: video_file)
  ...
  test "includes all video files in the playlist" do
          presenter = described_class.new(url_redirect:, logged_in_user: nil)
          props = presenter.stream_page_props(product_file: video_file)
  ...
  context_ "#read_page_props" do
  test "returns all necessary props for the PDF reader page" do
  ...
  test "includes latest_media_location when available" do
        product = create(:product_with_pdf_file)
        purchase = create(:purchase, link: product, purchaser: create(:user))
        url_redirect = create(:url_redirect, purchase:)
        product_file = product.product_files.first
  ...
  test "returns nil for purchase_id when there is no purchase" do
        creator = create(:user)
