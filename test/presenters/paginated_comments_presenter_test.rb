...
class PaginatedCommentsPresenterTest < ActiveSupport::TestCase
  self.described_class = PaginatedCommentsPresenter


  context_ PaginatedCommentsPresenter do
  context_ "#result" do
  ...
  context_ "when 'page' option is not specified" do
  test "returns paginated comments for the first page along with pagination metadata" do
  ...
  context_ "when 'page' option is specified" do
        let(:options) { { page: 2 } }

  test "returns paginated comments for the specified page along with pagination metadata" do
          result = presenter.result
  ...
  context_ "when there exists comments with nested replies" do
  ...
  test "always returns count of all root comments and their descendants irrespective of specified 'page' option" do
          result = presenter.result
  ...
  context_ "when 'page' option is specified with value of 1" do
          let(:options) { { page: 1 } }

  test "returns paginated roots comments for the first page along with their descendants" do
            result = presenter.result
  ...
  context_ "when 'page' option is specified with value of 2" do
          let(:options) { { page: 2 } }

  test "returns paginated roots comments for the second page along with their descendants" do
            result = presenter.result
