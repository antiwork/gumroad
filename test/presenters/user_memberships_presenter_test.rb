...
class UserMembershipsPresenterTest < ActiveSupport::TestCase
  self.described_class = UserMembershipsPresenter



  context_ UserMembershipsPresenter do
  ...
  context_ "when role is marketing" do
      before do
  ...
  context_ "with other memberships present" do
  ...
  context_ "without other memberships" do
        before do
          user.user_memberships.delete_all
        end

  test "doesn't notify" do
