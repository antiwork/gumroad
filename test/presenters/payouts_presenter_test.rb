...
class PayoutsPresenterTest < ActiveSupport::TestCase
  self.described_class = PayoutsPresenter


  context_ PayoutsPresenter do
    include PayoutsHelper
    include CurrencyHelper

  context_ "#next_payout_period_data" do
  test "returns not_payable status when user balance is below minimum" do
  ...
  test "returns has_stripe_connect set to false if the user is not stripe connect" do
        user = create(:user, user_risk_state: "compliant")
        instance = described_class.new(seller: user)
  ...
  test "returns has_stripe_connect set to true if the user is stripe connect" do
        user = create(:user, user_risk_state: "compliant")
  ...
  test "returns empty array when no processing payouts exist" do
        user = create(:user, user_risk_state: "compliant")
        instance = described_class.new(seller: user)
  ...
  test "adds has_stripe_connect to each processing payout period" do
        user = create(:user, user_risk_state: "compliant")
        MerchantAccount.create!(
          user: user,
          charge_processor_id: StripeChargeProcessor.charge_processor_id,
  ...
  context_ "#instant_payout_data" do
  test "returns nil when instant payouts are not supported" do
        user = create(:user, user_risk_state: "compliant")
        instance = described_class.new(seller: user)
  ...
  test "returns instant payout details when supported" do
        user = create(:user, user_risk_state: "compliant")
        instance = described_class.new(seller: user)
  ...
  context_ "#past_payout_period_data" do
  test "returns empty array when no past payouts exist" do
        user = create(:user, user_risk_state: "compliant")
        instance = described_class.new(seller: user)
  ...
  test "transforms past payouts using payout_period_data helper" do
        user = create(:user, user_risk_state: "compliant")
  ...
  context_ "#pagination_data" do
  test "returns pagination props" do
        user = create(:user, user_risk_state: "compliant")
