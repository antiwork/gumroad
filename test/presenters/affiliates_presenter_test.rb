...
class AffiliatesPresenterTest < ActiveSupport::TestCase
  self.described_class = AffiliatesPresenter


  context_ AffiliatesPresenter do
  ...
  test "returns the necessary props to render the affiliates page" do
  ...
  context_ "when search query is present" do
  context_ "when partially matching an affiliate user" do
          let(:query) { "affil" }
  ...
  context_ "and value does not match any affiliate users" do
          let(:query) { "gumbot" }
  ...
  context_ "and value is empty" do
          let(:query) { "" }
  ...
  context_ "#onboarding_form_props" do
      let(:seller) { create(:named_user) }
      let(:admin_for_seller) { create(:user, username: "adminforseller") }
      let(:support_for_seller) { create(:user, username: "supportforseller") }
      let(:pundit_user) { SellerContext.new(user: admin_for_seller, seller:) }
  ...
  test "returns the necessary props to render the affiliate onboarding page" do
  ...
  context_ "#new_affiliate_props" do
      let(:seller) { create(:named_user) }
      let(:admin_for_seller) { create(:user, username: "adminforseller") }
      let(:pundit_user) { SellerContext.new(user: admin_for_seller, seller:) }
  ...
  test "returns products with enabled: false and default values" do
  ...
  context_ "when seller has Brazilian Stripe account" do
        before do
          allow(seller).to receive(:has_brazilian_stripe_connect_account?).and_return(true)
        end

  test "returns the disabled reason" do
          props = described_class.new(pundit_user).new_affiliate_props
  ...
  context_ "#edit_affiliate_props" do
      let(:seller) { create(:named_user) }
      let(:admin_for_seller) { create(:user, username: "adminforseller") }
      let(:pundit_user) { SellerContext.new(user: admin_for_seller, seller:) }
  ...
  test "returns affiliate data with products" do
