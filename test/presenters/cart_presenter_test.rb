...
class CartPresenterTest < ActiveSupport::TestCase
  self.described_class = CartPresenter



  context_ CartPresenter do
  context_ "#cart_props" do
  ...
  test "returns props for the alive guest cart matching the `browser_guid`" do
  ...
  test "returns nil if no matching guest cart is found" do
          expect(presenter.cart_props).to be_nil
        end
      end

  context_ "when the user has no cart" do
  test "returns nil" do
          expect(presenter.cart_props).to be_nil
        end
      end

  context_ "when the user has a cart with items" do
  ...
  context_ "when the cart contains archived products" do
  ...
  test "excludes archived products from cart items" do
  ...
  context_ "when all cart products are archived" do
          before do
            active_product.update!(archived: true)
          end

  test "returns an empty items list" do
            props = presenter.cart_props
  ...
  context_ "with many cart items" do
        let(:cart) { create(:cart, user:) }
  ...
  test "keeps per-product query growth bounded" do
  ...
  context_ "with discount codes and offers" do
  context_ "when the user has accepeted an upsell" do
  ...
  context_ "when the user has accepted a cross-sell" do
          let(:user) { nil }
  ...
  context_ "when the accepted cross-sell has an existing-customer discount" do
  ...
  test "does not expose the discount to anonymous users" do
              expect(presenter.cart_props).to match(a_hash_including(
                items: include(
                  a_hash_including(
                    product: a_hash_including(permalink: offered_product.unique_permalink),
