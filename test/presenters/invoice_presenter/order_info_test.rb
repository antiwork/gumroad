...
class InvoicePresenterOrderInfoTest < ActiveSupport::TestCase
  self.described_class = InvoicePresenter::OrderInfo


  context_ InvoicePresenter::OrderInfo do
  ...
  context_ "#pdf_attributes" do
        before do
          purchase.update!(was_purchase_taxable: true, tax_cents: 100)
        end

  test "returns an Array of attributes" do
  ...
  test "returns correct business VAT ID label" do
  ...
  context_ "when broader country support keeps the submitted business id without refunding tax" do
  ...
  test "uses the submitted country label and omits the reverse charge note" do
            expect(presenter.pdf_attributes).to include(
              {
                label: "VAT ID",
                value: "DE123456789",
  ...
  context_ "with business_vat_id already provided" do
          before do
            purchase.update!(gumroad_tax_cents: 100, was_purchase_taxable: true)
            purchase_sales_tax_info.update!(business_vat_id:)
          end

  test "includes VAT ID attributes" do
  ...
  context_ "when the charge has a second purchase" do
  ...
  test "includes second purchase product" do
          expect(presenter.pdf_attributes).to include(
            {
  ...
  context_ "with Gumroad tax", :vcr do
  ...
  context_ "when the tax was not refunded" do
  test "includes the tax" do
              expect(presenter.pdf_attributes).to include(
                {
                  label: "Sales tax (included)",
                  value: "$1.50",
  ...
  context_ "when the tax was refunded" do
            before do
  ...
  test "subtracts the refunded amounts" do
