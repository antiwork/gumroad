...
class ReviewsPresenterTest < ActiveSupport::TestCase
  self.described_class = ReviewsPresenter



  context_ ReviewsPresenter do
  context_ "#reviews_props" do
  ...
  test "returns props for the reviews page" do
  ...
  context_ "when the reviewed product has been deleted" do
  ...
  test "marks the product unavailable so the edit UI can be hidden" do
  ...
  context_ "when a purchase awaiting review is for a deleted product" do
        let!(:deleted_product) { create(:product, deleted_at: 1.day.ago) }
        let!(:purchase_on_deleted_product) { create(:purchase, purchaser: user, link: deleted_product) }

  test "omits it from the purchases-awaiting-review list" do
  ...
  context_ "when the reviewed product has #{attribute} set but is not deleted" do
  ...
  test "still marks the product available — only deleted_at blocks the edit UI" do
  ...
  context_ "when a purchase awaiting review has #{attribute} set on its product but is not deleted" do
          let!(:product_with_attr) { create(:product, attribute => 1.day.ago) }
          let!(:purchase_on_product) { create(:purchase, purchaser: user, link: product_with_attr) }

  test "keeps it in the purchases-awaiting-review list" do
