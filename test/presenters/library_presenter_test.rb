...
class LibraryPresenterTest < ActiveSupport::TestCase
  self.described_class = LibraryPresenter



  context_ LibraryPresenter do
  ...
  test "returns all necessary properties for library page" do
  ...
  test "does not return the URL of a deleted thumbnail" do
  ...
  test "handles users without a name set" do
  ...
  context_ "when a user has purchased a subscription multiple times" do
        let!(:purchase_2) do
  ...
  test "returns results for all live subscriptions and excludes cancelled ones when access is blocked" do
          purchases, creators = described_class.new(buyer).library_cards
  ...
  context_ "when subscription is cancelled or ended" do
  context_ "when block_access_after_membership_cancellation is enabled (default)" do
          before do
            product.update!(block_access_after_membership_cancellation: true)
          end

  test "excludes cancelled subscriptions from library" do
  ...
  test "excludes ended subscriptions from library" do
  ...
  test "excludes failed subscriptions from library" do
  ...
  context_ "when block_access_after_membership_cancellation is disabled" do
          before do
            product.update!(block_access_after_membership_cancellation: false)
          end

  test "includes cancelled subscriptions in library" do
            purchase.subscription.update!(cancelled_at: 1.day.ago)
  ...
  test "includes ended subscriptions in library" do
            purchase.subscription.update!(ended_at: 1.day.ago)
  ...
  test "includes failed subscriptions in library" do
            purchase.subscription.update!(failed_at: 1.day.ago)
  ...
  context_ "when the user has a gifted membership purchase" do
  ...
  test "includes gifted membership purchases in the library" do
          purchases, _ = described_class.new(buyer).library_cards
  ...
  context_ "has_third_party_analytics" do
  test "detects product-level receipt analytics" do
  ...
  test "detects product-level analytics with 'all' location" do
  ...
  test "detects user-level universal receipt analytics" do
  ...
  test "ignores deleted analytics" do
  ...
  test "ignores analytics for non-receipt locations" do
  ...
  test "includes the bundle attributes" do
