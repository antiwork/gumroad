...
class TaxCenterPresenterTest < ActiveSupport::TestCase
  self.described_class = TaxCenterPresenter


  context_ TaxCenterPresenter do
  ...
  context_ "#props" do
  context_ "when seller has no tax forms" do
  test "returns empty documents with available years and selected year" do
  ...
  context_ "when seller was created in the current year" do
        let(:seller) { create(:user, created_at: Time.current) }
        let(:presenter) { described_class.new(seller:, year: Time.current.year) }

  test "returns empty available years and nil selected year" do
          result = presenter.props
  ...
  context_ "when seller has a tax form for the year" do
  ...
  test "returns document with necessary data" do
          result = presenter.props
  ...
  test "returns document with necessary data including filing date when present" do
  ...
  test "caches the document data" do
          presenter.props
