...
class ReceiptPresenterItemInfoTest < ActiveSupport::TestCase
  self.described_class = ReceiptPresenter::ItemInfo



  context_ ReceiptPresenter::ItemInfo do
  ...
  test "assigns instance variables" do
  ...
  context_ "notes" do
  context_ "when the purchase is a free trial" do
          let(:purchase) { create(:free_trial_membership_purchase) }

  test "returns note" do
  ...
  test "returns physical_product_note" do
  ...
  context_ "if is preorder authorization" do
  ...
  context_ "custom_receipt_note" do
  context_ "when the product does not have a custom receipt note" do
  test "returns nil" do
            expect(props[:custom_receipt_note]).to be_nil
          end
        end

  context_ "when the product has a custom receipt note" do
          before do
  ...
  context_ "when the purchase has a gift note" do
  ...
  context_ "show_download_button" do
  context_ "when the purchase has url_redirect" do
          before do
            purchase.create_url_redirect!
          end

  context_ "when the product has content for url redirects" do
  ...
  context_ "when is a preorder authorization" do
              include_context "when is a preorder authorization"
  ...
  context_ "when is a coffee purchase" do
  ...
  context_ "license_key" do
  context_ "when the purchase has a license" do
          include_context "when the purchase has a license"

  test "returns license key" do
            expect(props[:license_key]).to eq(purchase.license_key)
          end
        end

  context_ "when the purchase does not have a license" do
  test "returns nil" do
  ...
  context_ "product" do
  test "calls ProductPresenter#card_for_email" do
          expect(ProductPresenter).to receive(:card_for_email).with(product:).and_call_original
          expect(props[:product]).to be_present
        end
      end

  context_ "gift_attributes" do
  context_ "when is not gift sender purchase" do
  test "returns an empty array" do
  ...
  test "returns gift attributes" do
  ...
  context_ "when the gift note is empty" do
            before do
              gift.update!(gift_note: "  ")
            end

  test "doesn't return gift_message_attribute" do
              expect(props[:gift_attributes]).to eq(
                [
  ...
  context_ "general_attributes" do
  context_ "when the purchase is not for membership" do
  context_ "when the purchase doesn't have a variant or quantity" do
  test "returns product price" do
  ...
  context_ "when the purchase is in EUR" do
              before do
                purchase.update!(
  ...
  test "returns product price in EUR" do
                expect(props[:general_attributes]).to eq(
                  [
                    { label: "Product price", value: "€14.99" }
                  ]
  ...
  test "returns general attributes including product price and duration" do
  ...
  context_ "when the purchase has variants and quantity" do
  ...
  test "returns general attributes with variant, price, and quantity" do
              expect(props[:general_attributes]).to eq(
                [
  ...
  test "returns general attributes with variant and quantity and no product price" do
                expect(props[:general_attributes]).to eq(
                  [
                    { label: "Variant", value: "small, red" },
                    { label: "Quantity", value: 2 }
  ...
  context_ "when the purchase is a coffee purchase" do
              before { purchase.link.update!(native_type: Link::NATIVE_TYPE_COFFEE) }

  test "returns general attributes including donation and excluding variant and quantity" do
                expect(props[:general_attributes]).to eq(
  ...
  context_ "when the purchase is a multi-seat license" do
                before do
                  allow_any_instance_of(Purchase).to receive(:is_multiseat_license?).and_return(true)
                end

  test "returns quantity as seats" do
                  expect(props[:general_attributes]).to eq(
                    [
                      { label: "Variant", value: "small, red" },
                      { label: "Product price", value: "$7.49" },
  ...
  context_ "when the purchase contains only one quantity" do
                before do
                  purchase.update!(quantity: 1)
                end

  test "returns general attributes with variant and product price only" do
                  expect(props[:general_attributes]).to eq(
                    [
                      { label: "Variant", value: "small, red" },
                      { label: "Product price", value: "$14.99" },
  ...
  context_ "when the purchase is bundle" do
  ...
  test "returns bundle attribute" do
  ...
  context_ "when the purchase is for a membership" do
          let(:purchase) do
            create(
              :membership_purchase,
              link: product,
  ...
  test "returns general attributes with variant" do
          end

  context_ "when the purchase has quantity" do
            before do
              purchase.update!(quantity: 2)
            end
  ...
  context_ "without custom_fields" do
  test "returns product price only" do
            expect(props[:general_attributes]).to eq(
              [
                { label: "Product price", value: "$14.99" },
              ]
  ...
  test "includes correct custom fields" do
            expect(props[:general_attributes]).to eq(
              [
                {
                  label: "Product price",
  ...
  context_ "when the purchase has a tip" do
          before { purchase.create_tip!(value_cents: 500) }

  test "includes tip price attribute" do
  ...
  context_ "#refund_policy_attribute" do
  context_ "when the purchase doesn't have a refund policy" do
  test "returns product price only" do
            expect(props[:general_attributes]).to eq(
              [
                { label: "Product price", value: "$14.99" },
  ...
  test "includes refund policy attribute" do
            expect(props[:general_attributes]).to eq(
              [
                { label: "Product price", value: "$14.99" },
  ...
  context_ "when the purchase is a gift receiver purchase" do
  ...
  test "uses gifter_purchase's refund policy" do
  ...
  context_ "when the purchase is a bundle product purchase" do
          let(:bundle) { create(:product, user: seller, is_bundle: true, name: "Bundle product") }
  ...
  test "uses the bundle purchase's refund policy" do
            presenter = described_class.new(purchase.product_purchases.last)
            expect(presenter.props[:general_attributes]).to eq(
              [
                {
  ...
  context_ "when the purchase is a bundle gift receiver purchase" do
          let(:bundle) { create(:product, user: seller, is_bundle: true, name: "Bundle product") }
  ...
  test "uses the gifter bundle purchase's refund policy" do
            presenter = described_class.new(purchase.product_purchases.last)
            expect(presenter.props[:general_attributes]).to eq(
              [
                {
  ...
  context_ "when the purchase is not a subscription" do
  test "returns nil" do
  ...
  test "returns subscription note" do
  ...
  context_ "when not used in an email" do
            let(:for_email) { false }

  test "requires email confirmation in the subscription settings link" do
              url = Rails.application.routes.url_helpers.manage_subscription_url(
                purchase.subscription.external_id,
                host: UrlService.domain_with_protocol,
              )
  ...
  context_ "when the purchase is a gift sender purchase" do
  test "returns gift subscription note" do
  ...
  context_ "when the purchase is an installment plan" do
  test "returns the installment plan manage note with dates and link" do
  ...
  test "returns the correct product price" do
