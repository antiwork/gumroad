...
class ReceiptPresenterPaymentInfoTest < ActiveSupport::TestCase
  self.described_class = ReceiptPresenter::PaymentInfo



  context_ ReceiptPresenter::PaymentInfo do
  ...
  context_ "with a Purchase" do
  test "assigns the purchase to instance variables" do
          expect(payment_info.send(:chargeable)).to eq(purchase)
          expect(payment_info.send(:orderable)).to eq(purchase)
        end
      end

  context_ "with a Charge", :vcr do
  ...
  test "assigns the charge and order to instance variables" do
  ...
  context_ "#title" do
  context_ "when is not a recurring subscription charge" do
  test "returns correct title" do
          expect(payment_info.title).to eq("Payment info")
        end
      end

  context_ "when the purchase is recurring subscription" do
  ...
  context_ "payment attributes" do
      ...
      shared_examples "payment attributes for single purchase" do
  ...
  test "returns an empty arrays" do
  ...
  test "returns an empty array for upcoming payment" do
            expect(upcoming_payment_attributes).to eq([])
          end
        end

  context_ "when is a membership purchase" do
  ...
  context_ "with shipping and tax" do
            before do
              purchase.update!(
  ...
  test "returns pricing attributes" do
              expect(today_payment_attributes).to eq(
                [
  ...
  test "returns today's pricing attributes in USD" do
                expect(today_payment_attributes).to eq(
                  [
                    { label: "Today's payment", value: nil },
                    { label: "Membership product", value: "$17.44" },
  ...
  test "returns today's payment attributes" do
              expect(today_payment_attributes).to eq(
                [
                  { label: "Today's payment", value: nil },
  ...
  test "returns upcoming payment attributes" do
              expect(upcoming_payment_attributes).to eq(
                [
  ...
  test "return today's payment attributes with product price and tax as zeros" do
  ...
  test "does not includes today's payment header and upcoming payments" do
                expect(today_payment_attributes).to eq(
                  [
  ...
  test "returns empty upcoming payment attributes" do
                expect(upcoming_payment_attributes).to eq([])
              end
            end

  context_ "when the purchase is a test purchase with no more remaining charges" do
              let(:product) { create(:membership_product, name: "Membership product") }
              let(:purchase) do
                create(
                  :test_purchase,
  ...
  test "does not show upcoming payment for test purchase" do
                expect(upcoming_payment_attributes).to eq([])
              end

  test "shows correct charge progress for test purchase" do
  ...
  context_ "when is not a membership purchase" do
  context_ "when the purchase is free" do
            let(:purchase) { create(:free_purchase) }

  test "returns an empty arrays for payment attributes" do
              expect(today_payment_attributes).to eq([])
              expect(upcoming_payment_attributes).to eq([])
            end
          end

  context_ "when the purchase is for digital product" do
  context_ "when there is only 1 qty" do
  test "returns today's pricing attribute with product price" do
                expect(today_payment_attributes).to eq(
                  [
  ...
  context_ "when the purchase is taxable" do
                before do
                  purchase.update!(
                    was_purchase_taxable: true,
                    tax_cents: 254,
  ...
  test "returns today's pricing attributes with product name and sales tax" do
                  expect(today_payment_attributes).to eq(
                    [
  ...
  context_ "when the quantity is greater than 1" do
              before do
                purchase.update!(
                  quantity: 2,
  ...
  context_ "when there is no tax or shipping" do
  test "returns today's pricing attributes without amount paid" do
                  expect(today_payment_attributes).to eq(
                    [
  ...
  context_ "when the purchase has a license with quantity" do
                include_context "when the purchase has a license"

  test "returns today's pricing attributes with quantity" do
                  expect(today_payment_attributes).to eq(
                    [
                      { label: "Digital product × 2", value: "$29.98" },
                      { label: nil, value: link_to("Generate invoice", invoice_url) },
  ...
  test "returns today's pricing attributes with subtotal, quantity, sales tax" do
                  expect(today_payment_attributes).to eq(
                    [
  ...
  test "returns today's pricing attributes" do
              expect(today_payment_attributes).to eq(
                [
  ...
  context_ "when the purchase is a commission deposit purchase", :vcr do
          let(:purchase) { create(:commission_deposit_purchase, price_cents: 200) }
          let(:payment_info) { described_class.new(purchase) }

          before { purchase.create_artifacts_and_send_receipt! }

  test "returns correct today payment attributes" do
  ...
  test "returns correct upcoming payment attributes" do
  ...
  test "includes both purchases with amount paid" do
            expect(today_payment_attributes).to eq(
              [
                { label: "Digital product", value: "$14.99" },
  ...
  context_ "when purchases have shipping and tax" do
            before do
              purchase_two.update!(
                was_purchase_taxable: true,
  ...
  context_ "#recurring_subscription_notes" do
  context_ "when is not a recurring subscription charge" do
  test "returns nil" do
  ...
  context_ "when the purchase is an installment payment" do
  ...
  test "returns installment-specific note content" do
  ...
  context_ "#credit_card_note" do
      shared_examples "credit card note" do
  test "returns note content" do
  ...
  context_ "when the card_type is blank" do
  ...
  context_ "when the card_type is paypal" do
  ...
  context_ "#payment_method_attribute" do
      shared_examples "payment method attribute" do
  test "returns payment method attribute" do
  ...
  context_ "when the purchase is a free purchase" do
          let(:purchase) { create(:free_purchase) }
  ...
  context_ "#today_membership_paid_until_attribute" do
  context_ "when is not a recurring subscription charge" do
  test "returns nil" do
  ...
  test "return the paid until date" do
  ...
  context_ "with two free purchases" do
          let(:purchase) { create(:free_purchase) }
  ...
  context_ "when the second purchase is not free" do
