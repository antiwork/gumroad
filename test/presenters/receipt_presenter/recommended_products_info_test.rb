...
class ReceiptPresenterRecommendedProductsInfoTest < ActiveSupport::TestCase
  self.described_class = ReceiptPresenter::RecommendedProductsInfo



  context_ ReceiptPresenter::RecommendedProductsInfo do
  ...
  test "doesn't return products" do
  ...
  context_ "when the purchase doesn't have a purchaser" do
          it_behaves_like "doesn't return products"
        end

  context_ "when the purchase has a purchaser" do
          before do
            purchase.update!(purchaser:)
          end

  context_ "when the feature is active" do
  ...
  context_ "with a regular purchase" do
  test "calls RecommendedProductsService with correct args" do
