...
class ProfilePresenterTest < ActiveSupport::TestCase
  self.described_class = ProfilePresenter


  context_ ProfilePresenter do
  ...
  context_ "#creator_profile" do
  test "returns profile data object" do
  ...
  context_ "#profile_props" do
  test "returns the props for the profile products tab" do
  ...
  test "includes data for the edit view when logged in as the seller" do
  ...
  context_ "#profile_settings_props" do
  test "returns profile settings props object" do
        Link.import(force: true, refresh: true)
