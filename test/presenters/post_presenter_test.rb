...
class PostPresenterTest < ActiveSupport::TestCase
  self.described_class = PostPresenter



  context_ PostPresenter do
    before do
  ...
  test "sets the presented identifiers as instance variables" do
  ...
  context_ "#post_component_props" do
      before do
  ...
  test "returns props for the post component" do
        @creator.reload
        pundit_user = SellerContext.new(user: @user, seller: @user)
  ...
  context_ "when 'allow_comments' flag is disabled" do
        pundit_user = SellerContext.logged_out
  ...
  test "responds with 'paginated_comments' set to nil" do
  ...
  context_ ".snippet" do
  test "returns a sanitized version of the message limited to 150 characters" do
  ...
  test "snips post the sanitization and retains more information" do
  ...
  context_ "#social_image" do
  test "returns a Post::SocialImage with properly set url and caption" do
        post = build(
  ...
  context_ "when a customer without a relevant purchase is viewing the post" do
        let(:user) { create(:purchase, link: create(:product, user: @creator)).purchaser }
        let(:pundit_user) { SellerContext.new(user:, seller: user) }

  test "returns true for a product-type post" do
          presenter = PostPresenter.new(pundit_user:, post: @product_post, purchase_id_param: nil)
          expect(presenter.e404?).to be(true)
        end

  test "returns true for a seller-type shown-on-profile post" do
          presenter = PostPresenter.new(pundit_user:, post: seller_post, purchase_id_param: nil)
          expect(presenter.e404?).to be(true)
        end
      end

  context_ "when a non-customer is viewing the post" do
        let(:user) { create(:user) }
        let(:pundit_user) { SellerContext.new(user:, seller: user) }

  test "returns true for a shown_on_profile=false post" do
          presenter = PostPresenter.new(pundit_user:, post: hidden_post, purchase_id_param: nil)
          expect(presenter.e404?).to be(true)
        end
      end

  context_ "when an unauthenticated user is viewing the post" do
        let(:pundit_user) { SellerContext.logged_out }

  test "returns true with a refunded purchase's param" do
  ...
  test "returns true with a chargedback purchase's param" do
          purchase = create(:purchase, link: @product, chargeback_date: Date.today)
          presenter = PostPresenter.new(pundit_user:, post: @product_post, purchase_id_param: purchase.external_id)
          expect(presenter.e404?).to be(true)
        end

  test "returns true with a purchase param that should not have access to a product-type post" do
          purchase = create(:purchase, link: create(:product, user: @creator)) # another of the seller's products
          presenter = PostPresenter.new(pundit_user:, post: @product_post, purchase_id_param: purchase.external_id)
          expect(presenter.e404?).to be(true)
        end

  test "returns true with a purchase param that should not have access to a seller-type shown-on-profile post" do
          purchase = create(:purchase, link: create(:product, user: @creator)) # another of the seller's products
          presenter = PostPresenter.new(pundit_user:, post: seller_post, purchase_id_param: purchase.external_id)
          expect(presenter.e404?).to be(true)
        end

  test "returns false with a valid purchase's param" do
