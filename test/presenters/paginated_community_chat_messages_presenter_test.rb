...
class PaginatedCommunityChatMessagesPresenterTest < ActiveSupport::TestCase
  self.described_class = PaginatedCommunityChatMessagesPresenter



  context_ PaginatedCommunityChatMessagesPresenter do
  ...
  context_ "when fetching older messages" do
  ...
  context_ "when there are more than MESSAGES_PER_PAGE older messages" do
          before do
  ...
  test "returns MESSAGES_PER_PAGE messages and next_older_timestamp" do
  ...
  context_ "when fetching newer messages" do
  ...
  context_ "when there are more than MESSAGES_PER_PAGE newer messages" do
          before do
            CommunityChatMessage.delete_all
          end
  ...
  test "returns MESSAGES_PER_PAGE messages and next_newer_timestamp" do
            expect(props[:messages].length).to eq(100)
  ...
  context_ "when fetching messages around timestamp" do
  ...
  test "returns equal number of older and newer messages" do
          expect(props[:messages].length).to eq(100)
  ...
  context_ "when messages are deleted" do
        let(:fetch_type) { "older" }
