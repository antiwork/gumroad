...
class SettingsPresenterTest < ActiveSupport::TestCase
  self.described_class = SettingsPresenter



  context_ SettingsPresenter do
    let(:product) do
  ...
  context_ "with user as #{role} for owner" do
          let(:user) { create(:user) }
  ...
  context_ "when support emails exist" do
        before { product.update!(support_email: "support@example.com") }

  test "includes product_level_support_emails in main_props" do
  ...
  context_ "#advanced_props" do
      let!(:custom_domain) { create(:custom_domain, user: seller, domain: "example.com") }

  context_ "when custom domain is unverified" do
        before do
  ...
  context_ "when custom domain is verified" do
        before do
  ...
  context_ "#third_party_analytics_props" do
  ...
  context_ "when attributes are set" do
  ...
  test "returns correct values for props" do
          expect(presenter.third_party_analytics_props).to eq(
  ...
  context_ "when seller is registered using email" do
  test "returns the correct props" do
  ...
  context_ "#authorized_applications_props" do
  context_ "when some applications have no access grants" do
  ...
  test "returns props with only applications which have access grants" do
  ...
  context_ "when seller is not the owner of the application" do
  ...
  test "returns props with is_own_app set to false" do
          expect(presenter.authorized_applications_props).to eq({
                                                                  authorized_applications: [{
                                                                    name: oauth_application1.name,
                                                                    icon_url: oauth_application1.icon_url,
  ...
  test "returns authorized applications ordered by first_authorized_at" do
  ...
  test "returns correct props for a seller who has no compliance info or payout method" do
        expect(presenter.payments_props).to eq(@base_props)
      end

  test "shows the AU backtax prompt when the feature is on and the creator owes more than $100 and the creator has received an email" do
  ...
  test "does not show the AU backtax prompt when the creator owes less than $100" do
        Feature.activate_user(:au_backtaxes, seller)
  ...
  context_ "when seller is from the US" do
        before do
  ...
  test "returns correct props when seller does not have a payout method" do
          expect(presenter.payments_props).to eq(@base_us_props)
        end

  test "includes the suspension reason when the seller is suspended for a policy violation" do
  ...
  test "includes the suspension reason when the seller is suspended for fraud" do
  ...
  test "returns correct props when seller is eligible for PayPal Connect" do
          expect(presenter.payments_props).to eq(@base_us_props)
  ...
  test "returns correct props when seller has a bank account and a PayPal Connect account", :vcr do
  ...
  test "returns correct props when seller has a Stripe Connect account" do
  ...
  test "includes Stripe verification requests if applicable" do
  ...
  test "flags the Stripe account as rejected and hides the remediation link when Stripe has rejected it" do
  ...
  test "keeps the under review status alongside Stripe verification requirements" do
          create(:merchant_account, user: seller)
          create(:user_compliance_info_request, user: seller, field_needed: UserComplianceInfoFields::Individual::STRIPE_IDENTITY_DOCUMENT_ID)
  ...
  test "keeps the admin pause source alongside Stripe verification requirements" do
          create(:merchant_account, user: seller)
          create(:user_compliance_info_request, user: seller, field_needed: UserComplianceInfoFields::Individual::STRIPE_IDENTITY_DOCUMENT_ID)
  ...
  test "keeps both the under review status and admin pause source when Stripe verification is also required" do
          create(:merchant_account, user: seller)
          create(:user_compliance_info_request, user: seller, field_needed: UserComplianceInfoFields::Individual::STRIPE_IDENTITY_DOCUMENT_ID)
          seller.put_on_probation!(author_name: "test")
          seller.update!(payouts_paused_internally: true)
  ...
  test "sets needs_id_upload to true when document-type compliance requests exist" do
          create(:merchant_account, user: seller)
          create(:user_compliance_info_request, user: seller, field_needed: UserComplianceInfoFields::Individual::STRIPE_IDENTITY_DOCUMENT_ID)
  ...
  test "sets needs_id_upload to true for each document-type field" do
          create(:merchant_account, user: seller)
          [
  ...
  test "sets needs_id_upload to false when only non-document-type compliance requests exist" do
          create(:merchant_account, user: seller)
          create(:user_compliance_info_request, user: seller, field_needed: UserComplianceInfoFields::Individual::TAX_ID)
  ...
  test "sets needs_id_upload to false for additional document requests" do
          create(:merchant_account, user: seller)
  ...
  context_ "when the seller is a business with a different personal and business country" do
  test "returns payout_currency based on the business country, not the personal country" do
  ...
  context_ "when the seller has a non-US business_tax_id with trailing letters" do
  test "exposes the last four characters of the stored value, preserving letters" do
  ...
  context_ "when payouts are paused internally" do
        before do
          seller.update!(payouts_paused_internally: true)
        end

  test "returns true for payouts_paused_internally" do
          expect(presenter.payments_props[:payouts_paused_internally]).to eq(true)
        end
      end

  context_ "when payouts are paused by user" do
        before do
          seller.update!(payouts_paused_by_user: true)
        end

  test "returns true for payouts_paused_by_user" do
          expect(presenter.payments_props[:payouts_paused_by_user]).to eq(true)
        end
      end

  context_ "when seller has a payout threshold set" do
        before do
  ...
  context_ "when seller is in a cross-border payout country with a stored threshold below the country minimum" do
  ...
  test "returns the stored payout threshold, not the effective minimum" do
          expect(seller.minimum_payout_amount_cents).to be > 1000
          expect(presenter.payments_props[:payout_threshold_cents]).to eq(1000)
        end
      end

  context_ "when seller has a quarterly payout frequency" do
        before do
          seller.update!(payout_frequency: User::PayoutSchedule::QUARTERLY)
        end

  test "returns the quarterly payout frequency" do
          expect(presenter.payments_props[:payout_frequency]).to eq(User::PayoutSchedule::QUARTERLY)
        end
      end

  context_ "when seller can connect Stripe" do
        before do
          seller.update!(can_connect_stripe: true)
        end

  test "returns true for can_connect_stripe" do
          expect(presenter.payments_props[:user][:can_connect_stripe]).to eq(true)
        end
      end

  context_ "when the seller can receive PayPal payouts" do
  test "returns true for show_paypal if country does not support bank payouts" do
  ...
  test "returns true for show_paypal if seller already has a PayPal payment address setup" do
  ...
  test "returns true for show_paypal if user country is Egypt" do
  ...
  test "returns true for show_paypal if user country is Kazakhstan" do
  ...
  context_ "when seller's payouts are paused" do
  test "returns the payout pause source" do
          seller.update!(payouts_paused_internally: true)
  ...
  context_ "#payments_props :can_manage_beneficial_owners gating" do
      before do
  ...
  test "exposes true for the owner of a business with a Gumroad-managed Stripe account" do
        expect(presenter.payments_props[:can_manage_beneficial_owners]).to be(true)
      end

  context_ "when the logged-in user is an admin for the seller" do
        let(:user) { create(:user) }
  ...
  test "exposes false so the section is hidden from team admins who lack :update? on payments" do
