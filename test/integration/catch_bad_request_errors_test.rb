...
class RequestsCatchBadRequestErrorsTest < ActionDispatch::IntegrationTest



  context_ "CatchBadRequestErrors middleware" do
    let!(:user) { create(:user) }
    let!(:oauth_application) { create(:oauth_application, owner: user) }

  context_ "when a request contains invalid params" do
  test "returns 400 (Bad Request) response" do
  ...
  test "returns error JSON response with 400 (Bad Request) status for a JSON request" do
  ...
  context_ "when a request contains valid params" do
