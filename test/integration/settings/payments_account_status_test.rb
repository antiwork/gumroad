...
class RequestsSettingsPaymentsAccountStatusTest < ActionDispatch::IntegrationTest



  context_ "Settings::Payments account_status", type: :request do
  ...
  test "redirects to login after TOS suspension invalidates the session" do
  ...
  test "redirects to login after fraud suspension invalidates the session" do
  ...
  test "combines country-aware field labels into one sentence for pending requests without a verification error message" do
