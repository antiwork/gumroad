...
class RequestsMobileApiTest < ActionDispatch::IntegrationTest


  context_ "Mobile API Request Specs" do
    before do
      @product = create(:product)
    end

  context_ "product download urls" do
      before do
  ...
  test "redirects to download URL" do
        10.times do
  ...
  context_ "external link url" do
      before do
        @url_redirect = create(:url_redirect, link: @product, purchase: nil)
      end

  test "adds external_link_url for external link files" do
  ...
  test "does not add external_link_url for non external link files" do
  ...
  test "serves stream urls that are usable" do
  ...
  test "sets the url redirect attributes" do
