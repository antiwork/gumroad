...
class RequestsApiV2ProductRateLimitingTest < ActionDispatch::IntegrationTest



  context_ "Product API rate limiting", type: :request do
  ...
  context_ "POST /api/v2/products" do
  test "throttles product creation after 10 requests per minute" do
  ...
  context_ "PUT /api/v2/products/:id" do
      let(:product) { create(:product, user:) }

  test "throttles product updates after 30 requests per minute" do
        travel_to(Time.current) do
          30.times do
  ...
  context_ "POST /v2/products" do
  test "throttles requests on the api.gumroad.com mount" do
        travel_to(Time.current) do
          10.times do
            post "/v2/products",
                 params: { access_token: token.token, name: "Test", price: 100 },
  ...
  context_ "PATCH /api/v2/products/:id" do
      let(:product) { create(:product, user:) }

  test "throttles product updates via PATCH after 30 requests per minute" do
        travel_to(Time.current) do
          30.times do
  ...
  context_ "format suffix" do
  test "throttles POST /api/v2/products.json" do
        travel_to(Time.current) do
          10.times do
  ...
  test "throttles PUT /api/v2/products/:id.json" do
