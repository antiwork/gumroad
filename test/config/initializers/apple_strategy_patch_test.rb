...
class ConfigInitializersAppleStrategyPatchTest < ActiveSupport::TestCase



  context_ "Apple OmniAuth strategy cookie-based nonce patch" do
  ...
  context_ "#stored_nonce" do
  test "reads the nonce from the signed cookie" do
  ...
  test "returns nil when cookie is missing" do
  ...
  test "returns nil when cookie has been tampered with" do
        env = build_env(
          "/users/auth/apple/callback",
  ...
  context_ "#callback_phase" do
  test "restores omniauth.params from cookie data without the nonce" do
        nonce = SecureRandom.urlsafe_base64(32)
        cookie_value = sign_cookie({ "nonce" => nonce, "referer" => "/library" })
  ...
  test "parses JSON user param into a hash" do
  ...
  test "leaves non-JSON user param as-is" do
        cookie_value = sign_cookie({ "nonce" => "test" })
  ...
  test "sets a signed SameSite=None cookie with nonce and referer" do
  ...
  test "omits referer from cookie when not provided" do
  ...
  context_ "#authorize_params" do
  test "includes a nonce parameter" do
        env = build_env("/users/auth/apple", method: "GET")
        prepare_strategy(env)
  ...
  test "generates a unique nonce each time" do
        env = build_env("/users/auth/apple", method: "GET")
        prepare_strategy(env)
  ...
  test "returns hash user data as-is from form-encoded request params" do
        env = build_env(
          "/users/auth/apple/callback",
  ...
  test "parses a JSON string user param without callback_phase form_hash rewrite" do
        user_json = '{"name":{"firstName":"Jane","lastName":"Appleseed"},"email":"jane@example.com"}'
        env = build_env(
          "/users/auth/apple/callback",
          input: "user=#{Rack::Utils.escape(user_json)}"
  ...
  test "returns empty hash when no user data is present" do
        env = build_env("/users/auth/apple/callback")
        prepare_strategy(env)
