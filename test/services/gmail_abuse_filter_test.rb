...
class GmailAbuseFilterTest < ActiveSupport::TestCase
  ...
  context_ ".add!" do
  test "adds the normalized email to the Redis set" do
  ...
  test "ignores non-Gmail addresses" do
  ...
  test "removes the normalized email from the Redis set" do
  ...
  test "populates the set with normalized emails of abusive accounts" do
  ...
  test "excludes non-Gmail addresses" do
        described_class.rebuild!

        expect(described_class.exists?("nongmail@example.com")).to be(false)
      end

  test "replaces the previous set atomically" do
