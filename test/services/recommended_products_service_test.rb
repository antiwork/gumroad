...
class RecommendedProductsServiceTest < ActiveSupport::TestCase
  self.described_class = RecommendedProductsService



  context_ RecommendedProductsService do
  context_ ".fetch" do
  ...
  test "returns an ActiveRecord::Relation of products sorted by customer count and excludes non-alive products" do
  ...
  context_ "when `exclude_ids` is passed" do
  test "excludes products with the specified IDs" do
  ...
  context_ "when `user_ids` is passed" do
  test "only returns products that belong to the specified users" do
          expect(described_class.fetch(
            model: RecommendedProductsService::MODEL_SALES,
            ids: [sample_product.id],
            user_ids: [seller.id],
  ...
  context_ "when `number_of_results` is passed" do
  test "returns at most the specified number of products" do
          expect(described_class.fetch(
            model: RecommendedProductsService::MODEL_SALES,
            ids: [sample_product.id],
            number_of_results: 1,
