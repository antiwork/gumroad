...
class ProductDuplicatorServiceTest < ActiveSupport::TestCase
  self.described_class = ProductDuplicatorService



  context_ ProductDuplicatorService do
  ...
  test "duplicates the product and marks the duplicate product as draft" do
      ...
      expect(duplicate_product.unique_permalink).not_to be(nil)
      ...
      expect(duplicate_product.purchase_disabled_at).not_to be(nil)
  ...
  test "duplicates the product and marks is_duplicating as false" do
  ...
  test "maintains atomicity and rolls back the transaction if some error occurs in the middle" do
  ...
  test "duplicates released preorder product and sets the new release_at as 1 month from now" do
      preorder_link = create(:preorder_link, link: product, url: s3_url)
  ...
  test "does not put membership tiers into an invalid state" do
      product_params = { user: seller, price_cents: 5000, name: "test product",
                         description: "description for test product",
                         is_recurring_billing: true, subscription_duration: "monthly",
                         is_in_preorder_state: true,
  ...
  context_ "duplicating a collab product" do
      before { create(:collaborator, products: [product]) }

  test "does not mark a duplicated collab product as a collab" do
  ...
  context_ "duplicating a coffee product" do
  ...
  test "raises a validation error when an active coffee product already exists" do
        expect do
  ...
  test "stores the validation message so the dashboard can surface it" do
  ...
  test "allows duplicating an archived coffee when no active coffee exists" do
  ...
  test "duplicates a product whose variant has a membership price change effective date in the past" do
  ...
  context_ "prices" do
  test "handles products with rental prices" do
  ...
  test "handles asset preview attachments for images" do
  ...
  test "handles asset preview attachments for videos" do
        asset_preview = AssetPreview.new(link: create(:product))
  ...
  test "ignores deleted asset previews" do
        asset_preview = AssetPreview.new(link: create(:product))
        asset_preview.file.attach(fixture_file_upload("test-small.jpg", "image/jpeg"))
        asset_preview.save!
        asset_preview.file.analyze
  ...
  test "preserves the cover order when duplicating" do
  ...
  test "handles duplication for an image attachment" do
        ...
        expect(duplicate_product.thumbnail).not_to eq(nil)
  ...
  test "duplicates the variant-level rich content ensuring the integrity of the file embeds" do
  ...
  context_ "error handling" do
  test "stores the error message on failure and re-raises" do
  ...
  test "duplicates public files and updates their embed IDs in the description" do
        duplicate_product = ProductDuplicatorService.new(product.id).duplicate
  ...
  test "removes embeds for non-existent public files" do
