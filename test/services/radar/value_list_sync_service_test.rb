...
class RadarValueListSyncServiceTest < ActiveSupport::TestCase
  self.described_class = Radar::ValueListSyncService



  context_ Radar::ValueListSyncService do
    let(:service) { described_class.new }

    let(:value_list) { double("ValueList", id: "rsl_123") }

  context_ "#sync_blocked_emails" do
      before do
  ...
  test "pushes recently blocked emails to Stripe Radar" do
  ...
  test "skips emails blocked more than 25 hours ago" do
  ...
  test "removes recently unblocked emails from Stripe Radar" do
  ...
  test "removes expired blocked emails from Stripe Radar" do
  ...
  test "creates the value list if it does not exist" do
        allow(Stripe::Radar::ValueList).to receive(:retrieve)
          .with("gumroad_blocked_emails")
  ...
  test "picks up re-blocked emails by filtering on blocked_at" do
  ...
  context_ "#sync_blocked_cards" do
      before do
        allow(Stripe::Radar::ValueList).to receive(:retrieve)
  ...
  test "pushes recently blocked card fingerprints to Stripe Radar" do
  ...
  test "skips fingerprints blocked more than 25 hours ago" do
        travel_to 2.days.ago do
  ...
  test "removes recently unblocked card fingerprints from Stripe Radar" do
  ...
  test "removes expired blocked card fingerprints from Stripe Radar" do
