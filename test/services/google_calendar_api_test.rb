...
class GoogleCalendarApiTest < ActiveSupport::TestCase
  self.described_class = GoogleCalendarApi



  context_ GoogleCalendarApi do
  ...
  test "requests user info with the given token" do
  ...
  test "requests calendar_list with the given token" do
  ...
  test "requests disconnect with the given token" do
  ...
  context_ "POST refresh_token" do
  test "requests refresh_token with the given token" do
        WebMock.stub_request(:post, "#{GoogleCalendarApi::GOOGLE_CALENDAR_OAUTH_URL}/token").
          with(
            body: {
  ...
  context_ "POST insert_event" do
  test "inserts an event with the given parameters" do
        calendar_id = "primary"
        event = {
          summary: "Test Event",
