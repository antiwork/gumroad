...
class EarlyFraudWarningUpdateServiceTest < ActiveSupport::TestCase
  self.described_class = EarlyFraudWarning::UpdateService
  self.rspec_metadata = { vcr: true }



  context_ EarlyFraudWarning::UpdateService, :vcr do
  ...
  context_ "when the record is resolved" do
        let!(:record) do
          create(
            :early_fraud_warning,
            purchase:,
  ...
  context_ "when the purchase belongs to a Stripe Connect account" do
  ...
  context_ "when the record is not yet persisted" do
  ...
  context_ "when the record is persisted" do
          let(:record) do
            create(
              :early_fraud_warning,
  ...
  context_ "when the purchase does not belong to a Stripe Connect account" do
