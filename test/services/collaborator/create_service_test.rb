...
class CollaboratorCreateServiceTest < ActiveSupport::TestCase
  self.described_class = Collaborator::CreateService



  context_ Collaborator::CreateService do
  context_ "#process" do
  ...
  context_ "with 'apply_to_all_products' enabled" do
  test "creates a collaborator" do
          expect do
  ...
  context_ "with 'apply_to_all_products' disabled" do
  ...
  test "returns an error when commission is missing" do
          params[:products] = [
  ...
  test "creates a collaborator if the user is associated with a deleted collaborator" do
  ...
  test "returns an error if the user cannot be found" do
  ...
  test "returns an error if the user is already a collaborator, regardless of invitation status" do
        collaborator = create(
          :collaborator,
  ...
  test "returns an error if product cannot be found" do
  ...
  test "returns an error if the user requires approval of any collaborator requests" do
  ...
  test "returns an error if the seller is using a Brazilian Stripe Connect account" do
  ...
  test "returns an error if the collaborating user is using a Brazilian Stripe Connect account" do
  ...
  context_ "email notifications" do
  test "sends a collaborator invitation email when a collaborator is invited" do
          expect do
            result = described_class.new(seller:, params:).process
            expect(result).to eq({ success: true })
          end
  ...
  test "sends a collaborator creation email when a collaborator is added" do
  ...
  context_ "invitations" do
  context_ "when reciprocal collaboration exists" do
          let!(:reciprocal_collaboration) do
            create(:collaborator, seller: collaborating_user, affiliate_user: seller)
          end

  test "does not build a collaborator invitation" do
            result = described_class.new(seller:, params:).process
            expect(result).to eq({ success: true })
  ...
  test "builds a collaborator invitation when invitation not accepted" do
  ...
  test "builds a collaborator invitation when reciprocal collaboration is soft deleted" do
  ...
  test "builds a collaborator invitation when reciprocal collaboration does not exist" do
          result = described_class.new(seller:, params:).process
          expect(result).to eq({ success: true })
