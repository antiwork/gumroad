...
class UpdatePayoutMethodTest < ActiveSupport::TestCase
  self.described_class = UpdatePayoutMethod



  context_ UpdatePayoutMethod do
  context_ "#process" do
  context_ "updating only the account holder name" do
        let(:user) { create(:named_user) }

  context_ "when the seller is in a country that syncs holder name to Stripe" do
          let!(:bank_account) { create(:japan_bank_account, user:) }
          let!(:compliance_info) { create(:user_compliance_info, user:, country: "Japan") }

  test "saves the name and enqueues HandleNewBankAccountWorker" do
  ...
  test "normalizes ASCII spaces to full-width and enqueues HandleNewBankAccountWorker" do
            params = ActionController::Parameters.new(
  ...
  test "syncs a pre-validator invalid stored ASCII-space name when the seller re-saves it" do
  ...
  test "returns a validation error and does not enqueue HandleNewBankAccountWorker when the name mixes scripts" do
            params = ActionController::Parameters.new(
  ...
  test "does not enqueue HandleNewBankAccountWorker when the submitted name differs only by surrounding whitespace" do
            params = ActionController::Parameters.new(
  ...
  test "does not enqueue HandleNewBankAccountWorker for #{klass.name.gsub('BankAccount', '')} when the submitted name differs only by surrounding whitespace" do
  ...
  context_ "when the seller is in a country that does NOT sync holder name to Stripe" do
          let!(:bank_account) { create(:ach_account, user:, account_holder_full_name: "Old Name") }

  test "saves the name without enqueueing HandleNewBankAccountWorker" do
            params = ActionController::Parameters.new(
  ...
  context_ "replacing the active bank account" do
        let(:user) { create(:named_user) }
        let!(:existing_bank_account) { create(:ach_account, user:) }

  context_ "when the new bank account fails validation" do
  test "returns bank_account_error and keeps the existing bank account alive" do
            params = ActionController::Parameters.new(
              bank_account: {
  ...
  context_ "when the user already has multiple alive bank accounts" do
          let!(:orphaned_bank_account) { create(:ach_account, user:) }

  test "deletes all existing alive bank accounts and does not report an inconsistency" do
            params = ActionController::Parameters.new(
              bank_account: {
                type: AchAccount.name,
  ...
  context_ "when account number exceeds maximum length" do
        let(:user) { create(:named_user) }

  test "returns an error without attempting RSA encryption" do
  ...
  context_ "switching to card payouts" do
        let(:user) { create(:named_user) }
        let!(:existing_bank_account) { create(:ach_account, user:) }
  ...
  test "discards the prepared credit card when a concurrent payout-method change wins the race" do
          expect(prepared_credit_card).to receive(:destroy!)

          expect(service.process).to eq(error: :concurrent_payout_method_change)
        end

  test "discards the prepared credit card when an exception escapes after card preparation" do
