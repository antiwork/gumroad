...
class BundleSearchProductsServiceTest < ActiveSupport::TestCase
  self.described_class = BundleSearchProductsService



  context_ BundleSearchProductsService do
  ...
  context_ "#call" do
  test "excludes the bundle itself from results" do
  ...
  test "only returns products from the seller" do
  ...
  test "excludes bundles, subscriptions, and calls" do
  ...
  test "only returns alive products" do
  ...
  test "calculates has_more correctly" do
        create_list(:product, 15, user: seller)
        index_model_records(Link)
  ...
  test "returns all products when all: true" do
  ...
  test "returns empty results when seller has no products" do
        result = described_class.new(bundle:, seller:).call
  ...
  test "defaults page to 1 when page is invalid" do
