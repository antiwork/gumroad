...
class MergeCartsServiceTest < ActiveSupport::TestCase
  self.described_class = MergeCartsService



  context_ MergeCartsService do
  context_ "#process" do
      let!(:user) { create(:user) }
      let(:browser_guid) { SecureRandom.uuid }

  context_ "when source cart is nil" do
  test "updates the details of the target cart" do
  ...
  context_ "target cart is nil" do
  test "updates the details of the source cart" do
          ...
          end.not_to change { cart.browser_guid }
  ...
  test "deletes the source cart when both source and target carts do not have alive cart products" do
  ...
  test "merges the cart products, discount codes and other attributes from source cart to the target cart" do
