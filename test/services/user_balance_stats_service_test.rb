...
class UserBalanceStatsServiceTest < ActiveSupport::TestCase
  self.described_class = UserBalanceStatsService



  context_ UserBalanceStatsService do
  ...
  context_ "when value should be retrieved from the cache" do
        before do
  ...
  context_ "when cached value exists" do
  ...
  context_ "when cached value does not exist" do
  test "returns generated value" do
  ...
  context_ "when value should not be retrieved from the cache" do
        before do
  ...
  context_ "next_payout_period_data" do
  ...
  context_ "when there is no standard payout processing" do
  test "returns the next payout period data" do
            expect(generated.fetch(:next_payout_period_data)).not_to eq(nil)
          end
        end

  context_ "when an instant payout is processing" do
          before do
            create(
              :payment,
              user:,
  ...
  context_ "when a standard payout is processing" do
          before do
            create(
              :payment,
              user:,
  ...
  test "returns the next payout period data as nil" do
            expect(generated.fetch(:next_payout_period_data)).to eq(nil)
          end
        end
      end

  context_ "processing_payout_periods_data" do
        let(:user) { create(:compliant_user, unpaid_balance_cents: 10_01) }
  ...
  context_ "when there are no processing payouts" do
  test "returns an empty array" do
            expect(generated.fetch(:processing_payout_periods_data)).to eq([])
          end
        end

  context_ "when there are multiple processing payouts" do
          before do
  ...
  test "returns the processing payout period data" do
  ...
  context_ "#should_use_cache?" do
  context_ "when user is large seller" do
        before do
  ...
  context_ "with sales count below threshold" do
  ...
  context_ "with sales count above threshold" do
  ...
  context_ "when user is not a large seller" do
  test "returns false" do
          expect(instance.send(:should_use_cache?)).to eq(false)
        end
      end
  ...
  context_ "#write_cache" do
  test "writes generated values" do
  ...
  context_ "#read_cache" do
  test "reads cached values" do
        expect(instance.send(:read_cache)).to eq(nil)
        expect(instance).to receive(:generate).and_return(example_values)
        instance.write_cache
        expect(instance.send(:read_cache)).to eq(example_values)
  ...
  context_ ".cacheable_users" do
  test "returns correct list of users" do
