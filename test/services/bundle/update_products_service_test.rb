...
class BundleUpdateProductsServiceTest < ActiveSupport::TestCase
  self.described_class = Bundle::UpdateProductsService



  context_ Bundle::UpdateProductsService do
  context_ "#perform" do
  ...
  test "ignores deleted bundle products that have become invalid" do
  ...
  test "soft deletes alive bundle products without re-running variant validation" do
        stale_product = create(:product, user: seller)
        stale_bundle_product = create(:bundle_product, bundle:, product: stale_product)
  ...
  test "restores a previously deleted bundle product instead of creating a duplicate" do
