...
class PriceCheckerServiceTest < ActiveSupport::TestCase
  self.described_class = PriceCheckerService



  context_ PriceCheckerService do
  ...
  context_ ".call" do
  context_ "with at least 5 same-taxonomy matches" do
        before do
          12.times do |i|
            create(
              :product,
  ...
  test "returns ok with the with_taxonomy tier" do
  ...
  context_ "when taxonomy has fewer than 5 matches but broader pool has at least 5" do
        before do
          2.times do |i|
            create(
              :product,
  ...
  test "returns ok with the broadened tier" do
          result = described_class.call(product:)
  ...
  context_ "with fewer than 5 matches even broadened" do
        before do
          2.times do |i|
  ...
  test "returns insufficient_data" do
          result = described_class.call(product:)
  ...
  context_ "with override fields" do
        before do
          12.times do |i|
            create(
              :product,
  ...
  test "uses override taxonomy_id, name, and description for matching" do
  ...
  context_ "when relevance filter excludes unrelated products" do
        before do
          12.times do |i|
            create(
              :product,
  ...
  test "treats unrelated names as insufficient_data" do
          result = described_class.call(product:)
  ...
  context_ "when ES returns null percentiles even though there are enough matches" do
        before do
          12.times do |i|
  ...
  test "cascades to insufficient_data instead of dereferencing nil" do
  ...
  test "excludes own seller, the product itself, bundles, PWYW, different currency, different native_type" do
          result = described_class.call(product:)
  ...
  test "caches the result and bypasses the cache when force_refresh is true" do
  ...
  test "uses different cache keys for different descriptions so a description edit busts the cache" do
  ...
  test "uses different cache keys for different currencies so a currency edit busts the cache" do
  ...
  context_ "with a currency override" do
  test "returns the effective currency in the response, not the saved one" do
  ...
  context_ "when the product is in the 'other' taxonomy" do
  ...
  test "returns a nil taxonomy_label so the UI does not show a misleading badge" do
          result = described_class.call(product:)
  ...
  context_ "when an ES query times out" do
  test "raises TimeoutError when the response has timed_out: true" do
          results_double = double(total: 12)
  ...
  test "raises TimeoutError when the Ruby-level Timeout fires" do
