...
class SaveInstallmentServiceTest < ActiveSupport::TestCase
  self.described_class = SaveInstallmentService



  context_ SaveInstallmentService do
  ...
  test "updates profile posts sections based on 'shown_in_profile_sections' param" do
  ...
  context_ "when installment is nil" do
  test "creates a product-type installment" do
  ...
  test "creates a variant-type installment" do
  ...
  test "creates a seller-type installment" do
  ...
  test "creates an audience-type installment" do
  ...
  test "creates a follower-type installment" do
  ...
  test "creates a affiliate-type installment" do
  ...
  test "creates and publishes the installment" do
  ...
  test "publishes the installment but does not send emails" do
  ...
  test "creates and sends a preview email" do
  ...
  test "sends a preview email to the impersonated user" do
  ...
  test "creates and schedules the installment" do
        freeze_time do
  ...
  test "returns an error if the schedule date is in past" do
  ...
  test "returns an error if no channel is provided" do
  ...
  test "returns an error if paid more than is greater than paid less than" do
  ...
  test "returns an error if bought after date is after bought before date" do
  ...
  test "returns an error if the message is missing" do
  ...
  test "invokes SaveContentUpsellsService with correct arguments" do
  ...
  context_ "when installment is present" do
      let(:installment) { create(:installment, seller:, installment_type: "seller") }

  test "updates the installment" do
        service = described_class.new(seller:, installment:, params:, preview_email_recipient:)
        expect do
          service.process
  ...
  test "marks the old file as deleted" do
  ...
  test "removes the existing files" do
        installment.product_files.create!(url: "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachment/old-url.txt")
  ...
  test "updates and publishes the installment" do
  ...
  test "returns an error while publishing an installment if the seller is not eligible to send emails" do
        allow_any_instance_of(User).to receive(:sales_cents_total).and_return(Installment::MINIMUM_SALES_CENTS_VALUE - 1)

        service = described_class.new(seller:, installment:, params: params.deep_merge(publish: true), preview_email_recipient:)
        expect do
          service.process
        end.not_to change { installment.reload.published? }
  ...
  test "updates and publishes the installment but does not send emails" do
        allow_any_instance_of(User).to receive(:sales_cents_total).and_return(Installment::MINIMUM_SALES_CENTS_VALUE - 1)
  ...
  test "allows updating only certain attributes for a published installment" do
        freeze_time do
  ...
  test "does not allow updating 'send_emails' if it has been published and already blasted" do
        ...
        expect do
          service.process
        end.not_to change { installment.reload.send_emails }

        expect(service.error).to be_nil
      end

  test "updates the installment but do not change the publish date if it is unchanged for a published installment" do
        ...
        expect do
          service.process
        end.not_to change { installment.reload.published_at }
  ...
  test "does not allow a future publish date for an already published installment" do
        ...
        expect(installment.name).not_to eq("Hello")
      end

  test "updates the installment and sends a preview email" do
        allow(PostSendgridApi).to receive(:process).and_call_original
  ...
  test "returns an error while previewing an email if the logged-in user has uncofirmed email" do
        ...
        expect do
          service.process
        end.not_to change { installment.reload }

        expect(service.error).to eq("You have to confirm your email address before you can do that.")
      end

  test "updates the installment and schedules the installment" do
        freeze_time do
          service = described_class.new(seller:, installment:, params: params.merge(to_be_published_at: 1.day.from_now.to_s), preview_email_recipient:)
          expect do
            service.process
  ...
  test "returns an error while scheduling an installment if the seller is not eligible to send emails" do
        allow_any_instance_of(User).to receive(:sales_cents_total).and_return(Installment::MINIMUM_SALES_CENTS_VALUE - 1)
  ...
  test "updates the installment and changes the existing schedule time of an already scheduled installment" do
        freeze_time do
  ...
  test "returns an error while scheduling if the seller has not confirmed their email address" do
  ...
  context_ "#save_with_unique_slug!" do
  test "retries with a unique suffix on ActiveRecord::RecordNotUnique for slug index" do
  ...
  test "re-raises RecordNotUnique for non-slug index violations" do
