...
class HelperClientTest < ActiveSupport::TestCase
  self.described_class = Helper::Client



  context_ Helper::Client do
    let(:helper) { Helper::Client.new }
    let(:conversation_id) { "123456" }
    let(:timestamp) { DateTime.current.to_i }

  context_ "#create_hmac_digest" do
  ...
  context_ "when payload is query params" do
  test "creates a digest from url-encoded payload" do
  ...
  context_ "when payload is JSON" do
  test "creates a digest from JSON string" do
  ...
  context_ "when both params and json are provided" do
  ...
  context_ "when neither params nor json are provided" do
  test "raises an error" do
  ...
  context_ "#close_conversation" do
  test "sends a PATCH request to close the conversation" do
  ...
  context_ "when the request fails" do
  ...
  context_ "#send_reply" do
      let(:message) { "Test reply message" }

  test "sends a POST request to send a reply" do
  ...
  test "sends a POST request to send a draft" do
        stub_request(:post, "https://api.helper.ai/api/v1/mailboxes/gumroad/conversations/#{conversation_id}/emails/")
  ...
  test "handles optional response_to" do
  ...
  context_ "#add_note" do
      let(:message) { "Test note message" }

  test "sends a POST request to add a note" do
