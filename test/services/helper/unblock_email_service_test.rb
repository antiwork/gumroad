...
class HelperUnblockEmailServiceTest < ActiveSupport::TestCase
  self.described_class = Helper::UnblockEmailService



  context_ Helper::UnblockEmailService do
  ...
  context_ "when email is blocked by gumroad" do
        let(:reply) do
          <<~REPLY
          Hey there,
  ...
  context_ "when recent blocked purchase is present" do
  ...
  context_ "when auto_reply_for_blocked_emails_in_helper feature is active" do
            before do
              Feature.activate(:auto_reply_for_blocked_emails_in_helper)
            end

  test "sends reply to the customer and closes the conversation" do
  ...
  context_ "when auto_reply_for_blocked_emails_in_helper feature is not active" do
  test "drafts the reply" do
  ...
  context_ "when recent blocked purchase is not present" do
          before do
            Feature.activate(:auto_reply_for_blocked_emails_in_helper)
          end

  test "unblocks the email" do
            expect do
              expect_any_instance_of(Helper::Client).to receive(:send_reply).with(conversation_id:, message: simple_format(reply))
              expect_any_instance_of(Helper::Client).to receive(:close_conversation).with(conversation_id:)
  ...
  context_ "when email is suppressed by SendGrid" do
        let(:reply) do
          <<~REPLY
          Hey,
  ...
  context_ "when reasons for suppressions are present" do
          before do
  ...
  test "adds as a note to the conversation" do
  ...
  test "unblocks email and sends reply to the customer and closes the conversation" do
  ...
  test "unblocks email and drafts the reply" do
            expect_any_instance_of(EmailSuppressionManager).to receive(:unblock_email)
            expect_any_instance_of(Helper::Client).to receive(:send_reply).with(conversation_id:, message: simple_format(reply), draft: true, response_to: email_id)
  ...
  context_ "when email is not found in SendGrid suppression lists" do
          before do
            allow_any_instance_of(EmailSuppressionManager).to receive(:unblock_email).and_return(false)
          end

  test "doesn't send a reply" do
  ...
  context_ "when a reply is already sent" do
          before do
            allow_any_instance_of(Helper::UnblockEmailService).to receive(:replied?).and_return(true)
          end

  test "unblocks email and doesn't send a reply again" do
            expect_any_instance_of(EmailSuppressionManager).to receive(:unblock_email)
            expect_any_instance_of(Helper::Client).not_to receive(:send_reply)
  ...
  context_ "when email is blocked by creator" do
        let(:reply) do
          <<~REPLY
          Hey there,
