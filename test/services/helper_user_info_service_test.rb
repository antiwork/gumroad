...
class HelperUserInfoServiceTest < ActiveSupport::TestCase
  self.described_class = HelperUserInfoService



  context_ HelperUserInfoService do
    include Rails.application.routes.url_helpers

    let(:user) { create(:user, email: "user@example.com") }

  context_ "#customer_info" do
  ...
  context_ "value calculation" do
        let(:product) { create(:product, user:, price_cents: 100_00) }

  test "returns the higher value between lifetime sales and last-90-day purchases" do
  ...
  test "returns empty user details and metadata" do
          result = service.customer_info
  ...
  context_ "with recent purchase" do
        let(:service) { HelperUserInfoService.new(email: user.email) }

  test "includes recent purchase info" do
  ...
  test "includes the stripe_connect_account_id in actions" do
  ...
  context_ "when there's a failed purchase" do
  test "includes failed purchase info" do
          product = create(:product)
  ...
  context_ "when purchase has a refund policy" do
  test "includes refund policy info" do
          product = create(:product)
  ...
  context_ "when purchase has a license key" do
  test "includes license key info" do
  ...
  context_ "when user has country" do
  test "includes country in the metadata" do
  ...
  context_ "when user has no country" do
  test "does not include country in the metadata" do
  ...
  context_ "structured comments" do
        let(:service) { described_class.new(email: user.email) }

  test "returns structured comment objects with external_id" do
  ...
  test "returns comments in descending order" do
  ...
  test "caps at STRUCTURED_COMMENTS_LIMIT" do
  ...
  test "returns all comment types" do
  ...
  context_ "can_add_comment" do
  test "is true when user exists by primary email" do
          result = described_class.new(email: user.email).customer_info
          expect(result[:can_add_comment]).to be true
        end

  test "is false when no user exists" do
          result = described_class.new(email: "nobody@example.com").customer_info
          expect(result[:can_add_comment]).to be false
        end

  test "is false when user is found via support_email only" do
          user.update!(support_email: "support@example.com")
          result = described_class.new(email: "support@example.com").customer_info
          expect(result[:can_add_comment]).to be false
        end

  test "is false for soft-deleted user" do
          user.mark_deleted!
          result = described_class.new(email: user.email).customer_info
          expect(result[:can_add_comment]).to be false
        end
