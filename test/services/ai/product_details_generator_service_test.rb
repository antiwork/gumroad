...
class AiProductDetailsGeneratorServiceTest < ActiveSupport::TestCase
  self.described_class = Ai::ProductDetailsGeneratorService
  self.rspec_metadata = { vcr: true }



  context_ Ai::ProductDetailsGeneratorService, :vcr do
    let(:current_seller) { create(:user) }
    let(:service) { described_class.new(current_seller:) }

  context_ "#generate_product_details" do
      let(:prompt) { "Create a coding tutorial ebook about Ruby on Rails for $29.99" }

  context_ "with valid prompt" do
  test "generates product details successfully" do
  ...
  test "includes price frequency for membership products" do
  ...
  context_ "with different currency code" do
  test "returns price in seller's currency" do
  ...
  context_ "when OpenAI returns invalid JSON" do
        before do
  ...
  test "raises an error and retries" do
          expect { service.generate_product_details(prompt:) }
  ...
  context_ "#generate_cover_image" do
      let(:product_name) { "Joy of Programming in Ruby" }

  context_ "with valid product name" do
  test "generates cover image successfully" do
  ...
  context_ "when OpenAI image generation fails" do
        before do
  ...
  context_ "#generate_rich_content_pages" do
      let(:product_info) do
        {
  ...
  context_ "with valid product info" do
  test "generates rich content pages successfully" do
  ...
  context_ "when OpenAI returns malformed JSON with type colon syntax" do
        before do
          malformed_response = {
            "pages" => [
              {
  ...
  test "cleans up the JSON and parses successfully" do
          result = service.generate_rich_content_pages(product_info)
