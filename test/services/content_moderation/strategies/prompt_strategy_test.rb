...
class ContentModerationStrategiesPromptStrategyTest < ActiveSupport::TestCase
  self.described_class = ContentModeration::Strategies::PromptStrategy
  self.rspec_metadata = { vcr: true }



  context_ ContentModeration::Strategies::PromptStrategy, :vcr do
  ...
  context_ "when OpenAI rejects the request with a 400" do
  ...
  test "treats both presets as compliant and reports each rejection to Sentry" do
  ...
  context_ "when image URLs have unsupported formats" do
  ...
  context_ "when OpenAI times out" do
  ...
  test "returns compliant when OpenAI returns a 500 server error" do
  ...
  test "skips the flagged result when the uncertainty check gets a 500 server error" do
        call_count = 0
        allow(client).to receive(:chat) do |_kwargs|
          call_count += 1
          case call_count
  ...
  context_ "SPAM_RULES (affiliate recruitment carveout)" do
  test "tells the model that affiliate recruitment emails are legitimate" do
