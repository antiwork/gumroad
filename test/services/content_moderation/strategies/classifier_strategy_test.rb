...
class ContentModerationStrategiesClassifierStrategyTest < ActiveSupport::TestCase
  self.described_class = ContentModeration::Strategies::ClassifierStrategy
  self.rspec_metadata = { vcr: true }



  context_ ContentModeration::Strategies::ClassifierStrategy, :vcr do
  ...
  test "skips image URLs that OpenAI rejects as bad requests and continues with remaining images" do
      image_urls = [
  ...
  test "returns flagged with a retry reason and notifies Sentry when every image URL fails and there is no text" do
      image_urls = [
        "blob:https://gumroad.com/bad-1",
  ...
  test "returns compliant and notifies Sentry when every image fails but text was moderated successfully" do
      image_urls = [
  ...
  test "still flags text-flagged categories when image moderation fails alongside successful text moderation" do
  ...
  test "does not flag unavailability when text exists and image_urls is empty" do
      allow(client).to receive(:moderations).and_return(
        "results" => [{ "category_scores" => {} }]
      )
  ...
  test "returns flagged with unavailable reason after MAX_MODERATION_ATTEMPTS timeouts" do
  ...
  test "returns flagged with unavailable reason after MAX_MODERATION_ATTEMPTS parsing errors" do
  ...
  test "returns flagged with unavailable reason after MAX_MODERATION_ATTEMPTS connection failures" do
  ...
  test "returns flagged with unavailable reason after MAX_MODERATION_ATTEMPTS server errors" do
