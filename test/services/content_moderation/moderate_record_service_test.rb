...
class ContentModerationModerateRecordServiceTest < ActiveSupport::TestCase
  self.described_class = ContentModeration::ModerateRecordService
  self.rspec_metadata = { vcr: true }



  context_ ContentModeration::ModerateRecordService, :vcr do
  ...
  context_ ".check" do
  test "returns passed when the feature flag is off" do
  ...
  test "skips moderation for verified sellers" do
  ...
  test "skips moderation for products with content_moderation_disabled set" do
  ...
  test "returns passed when content is empty" do
  ...
  context_ "when blocklist flags the content" do
        before do
          allow(ContentModeration::Strategies::BlocklistStrategy).to receive(:new).and_return(
  ...
  test "returns passed: false with reasons" do
          result = described_class.check(product, :product)
  ...
  test "short-circuits without running AI strategies" do
  ...
  test "leaves a note on the user for Gumclaw review" do
          expect do
            described_class.check(product, :product)
  ...
  test "does not create a duplicate note on rapid retries with identical content" do
          described_class.check(product, :product)
  ...
  test "creates a fresh note once the dedup window has elapsed" do
          described_class.check(product, :product)
  ...
  context_ "when an AI strategy flags the content" do
        before do
          allow(ContentModeration::Strategies::ClassifierStrategy).to receive(:new).and_return(
  ...
  test "returns passed: false with AI reasons" do
          result = described_class.check(product, :product)
  ...
  test "leaves a note on the user" do
          expect do
            described_class.check(product, :product)
          end.to change { seller.reload.comments.count }.by(1)
  ...
  context_ "when all strategies return compliant" do
  test "returns passed: true without creating a comment" do
          result = nil
          expect do
            result = described_class.check(product, :product)
          end.not_to change { seller.reload.comments.count }
  ...
  test "propagates errors raised by AI strategies" do
