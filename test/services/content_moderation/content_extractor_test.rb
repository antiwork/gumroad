...
class ContentModerationContentExtractorTest < ActiveSupport::TestCase
  self.described_class = ContentModeration::ContentExtractor



  context_ ContentModeration::ContentExtractor do
  context_ "#extract_from_product" do
  ...
  test "extracts product text and image URLs" do
  ...
  test "handles nil URLs from cover image previews without raising" do
  ...
  context_ "when a product file's S3 object is missing" do
        before do
  ...
  test "skips the missing file and collects remaining valid image URLs" do
          result = extractor.extract_from_product(product)
  ...
  context_ "#extract_from_product with missing S3 objects" do
      let(:extractor) { described_class.new }
  ...
  test "skips files with missing S3 objects without raising" do
        result = extractor.extract_from_product(product)
  ...
  test "parses the post HTML once and extracts text and images" do
  ...
  test "ignores images without a src attribute" do
  ...
  test "ignores images with an empty src attribute" do
