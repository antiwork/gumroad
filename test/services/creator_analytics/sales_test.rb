...
class CreatorAnalyticsSalesTest < ActiveSupport::TestCase
  self.described_class = CreatorAnalytics::Sales



  context_ CreatorAnalytics::Sales do
  ...
  context_ CreatorAnalytics::Sales, "timezone with DST gap at midnight" do
  context_ "when user timezone has a DST transition at midnight (e.g. Tehran)" do
  test "filters and buckets sales consistently on March 22, 2026" do
  ...
  context_ CreatorAnalytics::Sales, "DST handling" do
  context_ "when sale occurs near midnight during DST" do
  test "correctly attributes sale to the right day" do
  ...
  test "correctly buckets sales across DST boundary" do
        user = create(:user, timezone: "Pacific Time (US & Canada)")
        product = create(:product, user: user)
