...
class CreatorAnalyticsChurnTest < ActiveSupport::TestCase
  self.described_class = CreatorAnalytics::Churn



  context_ CreatorAnalytics::Churn do
  ...
  test "returns structure with metadata and data for current period" do
  ...
  test "includes current period metadata with date range" do
        result = service.generate_data(start_date:, end_date:)
  ...
  test "includes product info in metadata" do
        result = service.generate_data(start_date:, end_date:)
  ...
  test "includes current period data with daily, monthly, and summary" do
        result = service.generate_data(start_date:, end_date:)
  ...
  context_ "when previous period can be built" do
        let(:start_date) { Date.new(2020, 2, 15) }
        let(:end_date) { Date.new(2020, 2, 20) }

  test "includes previous period metadata and data" do
          result = service.generate_data(start_date:, end_date:)
  ...
  test "calculates previous period with same length as current period" do
          result = service.generate_data(start_date:, end_date:)
  ...
  test "sets previous period end date to one day before current period start" do
          result = service.generate_data(start_date:, end_date:)
  ...
  context_ "when previous period cannot be built" do
        let(:start_date) { Date.new(2020, 1, 1) }
        let(:end_date) { Date.new(2020, 1, 5) }

  test "returns nil for previous period" do
          result = service.generate_data(start_date:, end_date:)
  ...
  test "reads from cache when cacheable range exists" do
          cached_payload = {
  ...
  test "writes to cache after computing cacheable range" do
          result = cached_service.generate_data(start_date: cached_start_date, end_date: cached_end_date)
  ...
  test "does not cache recent dates (within 2 days)" do
  ...
  context_ "without caching" do
  test "does not use cache for regular sellers" do
          result = service.generate_data(start_date:, end_date:)
  ...
  test "does not write to cache for regular sellers" do
  ...
  context_ "with actual churn data" do
        let(:churn_events) do
          {
  ...
  test "includes churn data in current period" do
          result = service.generate_data(start_date:, end_date:)
  ...
  context_ "with invalid date range" do
