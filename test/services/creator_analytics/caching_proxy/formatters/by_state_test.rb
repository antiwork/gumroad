...
class CreatorAnalyticsCachingProxyFormattersByStateTest < ActiveSupport::TestCase
  self.described_class = CreatorAnalytics::CachingProxy::Formatters::ByState



  context_ CreatorAnalytics::CachingProxy::Formatters::ByState do
    before do
      @service = CreatorAnalytics::CachingProxy.new(build(:user))
    end

  context_ "#merge_data_by_state" do
  test "returns data merged by state" do
        day_one = {
          by_state: {
