...
class CreatorAnalyticsFollowingTest < ActiveSupport::TestCase
  self.described_class = CreatorAnalytics::Following



  context_ CreatorAnalytics::Following do
    before do
  ...
  test "returns expected data when user has no followers" do
  ...
  test "returns net total of followers" do
        add_event("added", Time.utc(2021, 1, 1))
        add_event("added", Time.utc(2021, 1, 1))
        add_event("removed", Time.utc(2021, 1, 1))
        add_event("added", Time.utc(2021, 1, 2))
  ...
  context_ "up to a specific date" do
  test "returns net total of followers before that date" do
          add_event("added", Time.utc(2021, 1, 1))
          add_event("added", Time.utc(2021, 1, 2))
  ...
  context_ "#first_follower_date" do
  test "returns nil if there are no followers" do
        expect(@service.first_follower_date).to eq(nil)
      end

  test "returns the date of the first follower" do
        add_event("added", Time.utc(2021, 1, 1))
  ...
  context_ "#counts" do
  test "returns hash of followers added, removed and running net total by day" do
  ...
  context_ "when event occurs near midnight during DST" do
  test "correctly attributes event to the right day" do
  ...
  test "correctly buckets events across DST boundary" do
          user = create(:user, timezone: "Pacific Time (US & Canada)")
          service = described_class.new(user)
