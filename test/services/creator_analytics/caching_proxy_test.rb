...
class CreatorAnalyticsCachingProxyTest < ActiveSupport::TestCase
  self.described_class = CreatorAnalytics::CachingProxy



  context_ CreatorAnalytics::CachingProxy do
  context_ "#data_for_dates" do
      before do
  ...
  test "returns merged mix of cached and generated data" do
  ...
  test "can generate grouped data, by day and month" do
  ...
  test "calls original method if cache shouldn't be used" do
  ...
  context_ "#generate_cache" do
  test "generates the cached data for all non-existent days of activity of users" do
  ...
  test "does not update data for yesterday, today, or any day after" do
        allow(@service).to receive(:use_cache?).and_return(true)
  ...
  test "does not update data for days before analytics were generated" do
        allow(@service).to receive(:use_cache?).and_return(true)
        expect(ComputedSalesAnalyticsDay).not_to receive(:upsert_data_from_key)
        @service.overwrite_cache(Date.new(2012, 9, 1))
      end

  test "does not update data when cache should not be used" do
        allow(@service).to receive(:use_cache?).and_return(false)
        expect(ComputedSalesAnalyticsDay).not_to receive(:upsert_data_from_key)
        @service.overwrite_cache(Date.yesterday)
      end

  test "regenerates and stores analytics for the date" do
        allow(@service).to receive(:use_cache?).and_return(true)
  ...
  context_ "#use_cache?" do
  test "returns true if user is a large seller" do
        user = create(:user)
        service = described_class.new(user)
  ...
  context_ "#cache_key_for_data" do
  test "returns cache key for data by date" do
  ...
  context_ "#user_cache_key" do
  test "returns cache key for the user and its timezone" do
  ...
  context_ "#analytics_data" do
  test "proxies method call to user" do
        user = create(:user, timezone: "London")
  ...
  test "returns cached data if it exists, generates the data if not" do
  ...
  test "does not cache the data if the date is yesterday, today, or any day after" do
  ...
  context_ "#uncached_dates" do
  test "returns all dates missing from cache" do
        user = create(:user)
        service = described_class.new(user)
  ...
  test "constrains start and end dates to a maximum of Today" do
        expect(@service.requested_dates(Date.today + 2, Date.today + 5)).to eq([Date.today])
      end

  test "constrains start date to a minimum of the user creation date" do
  ...
  test "returns the correct array when the dates are valid" do
  ...
  test "handles nonsensical requested dates" do
  ...
  test "returns at least a date whatever dates were requested" do
  ...
  context_ "#fetch_data_for_dates" do
      before do
        @user = create(:user, timezone: "London")
        @service = described_class.new(@user)
      end

  test "returns a hash with the stored values" do
  ...
  context_ "#compile_data_for_dates_and_fill_missing" do
      before do
        @user = create(:user)
        @service = described_class.new(@user)
      end

  test "returns array of data for all days, including missing" do
  ...
  context_ "#find_missing_date_ranges" do
      before do
        @service = described_class.new(build(:user))
      end

  test "contiguous missing dates as ranges" do
