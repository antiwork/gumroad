...
class CreatorAnalyticsProductPageViewsTest < ActiveSupport::TestCase
  self.described_class = CreatorAnalytics::ProductPageViews



  context_ CreatorAnalytics::ProductPageViews do
  ...
  context_ "#by_product_and_date" do
      before do
  ...
  context_ "when user time zone is Pacific Time" do
  ...
  context_ "when user time zone is Central European Time" do
  ...
  context_ "#by_product_and_country_and_state" do
      before do
  ...
  context_ "#by_product_and_referrer_and_date" do
      before do
  ...
  context_ "when the product_page_views Elasticsearch index does not exist" do
      before do
        allow(ProductPageView).to receive(:search).and_raise(Elasticsearch::Transport::Transport::Errors::NotFound)
      end

  test "by_product_and_date returns an empty hash" do
        expect(@service.by_product_and_date).to eq({})
      end

  test "by_product_and_country_and_state returns an empty hash" do
        expect(@service.by_product_and_country_and_state).to eq({})
      end

  test "by_product_and_referrer_and_date returns an empty hash" do
        expect(@service.by_product_and_referrer_and_date).to eq({})
      end
    end

  context_ "DST handling" do
  context_ "when page view occurs near midnight during DST" do
        let(:user_timezone) { "Pacific Time (US & Canada)" }

  test "correctly attributes page view to the right day" do
          user = create(:user, timezone: user_timezone)
  ...
  context_ "when query spans DST transition" do
        let(:user_timezone) { "Pacific Time (US & Canada)" }

  test "correctly buckets page views across DST boundary" do
          user = create(:user, timezone: user_timezone)
          product = create(:product, user: user)
  ...
  context_ "when the boundary date skips midnight" do
        let(:user_timezone) { "Tehran" }

  test "filters and buckets page views consistently on March 22, 2026" do
          user = create(:user, timezone: user_timezone)
          product = create(:product, user: user)
