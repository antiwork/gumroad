...
class CreatorAnalyticsChurnDatasetBuilderTest < ActiveSupport::TestCase
  self.described_class = CreatorAnalytics::Churn::DatasetBuilder



  context_ CreatorAnalytics::Churn::DatasetBuilder do
  ...
  context_ "#build" do
  context_ "with empty data" do
  test "returns structure with empty daily, monthly, and summary data" do
  ...
  test "includes metadata with date range and products" do
          result = service.build
  ...
  test "returns zero stats for all daily entries" do
          result = service.build
  ...
  context_ "with churn events and new subscriptions" do
        let(:churn_events) do
          {
  ...
  test "sums monthly counts before calculating rate (not averaging daily percentages)" do
  ...
  context_ "with zero denominator" do
        let(:churn_events) do
          {
  ...
  context_ "with multiple months" do
        let(:start_date) { Date.new(2020, 1, 15) }
