...
class UpdateUserCountryTest < ActiveSupport::TestCase
  self.described_class = UpdateUserCountry



  context_ UpdateUserCountry do
    before do
  ...
  context_ "#process" do
  test "deletes the old compliance info and creates a new one" do
  ...
  test "deletes the old stripe account" do
  ...
  test "marks all pending compliance info requests as provided" do
  ...
  test "deletes the old bank account" do
  ...
  test "adds country changed comment" do
        UpdateUserCountry.new(new_country_code: "GB", user: @user).process
  ...
  context_ "when old and new country are not Stripe-supported countries" do
  test "retains PayPal payment address" do
  ...
  context_ "when changing from Japan with invalid kana data" do
        before do
  ...
  test "soft-deletes old compliance info without raising validation errors" do
          old_compliance_info = @user.alive_user_compliance_info
  ...
  context_ "when user has balance" do
        before do
  ...
  test "marks balances as forfeited" do
          UpdateUserCountry.new(new_country_code: "GB", user: @user).process
  ...
  test "adds comment on the user" do
          UpdateUserCountry.new(new_country_code: "GB", user: @user).process
