...
class CircleApiTest < ActiveSupport::TestCase
  self.described_class = CircleApi
  self.rspec_metadata = { vcr: true }



  context_ CircleApi, :vcr, :without_circle_rate_limit do
  ...
  test "fails if invalid api_key is passed" do
  ...
  context_ "POST community_members" do
  ...
  test "is successful if member already exists" do
  ...
  test "fails if invalid member email is passed" do
  ...
  context_ "DELETE community_members" do
      before do
        @circle_api_handle.add_member(test_community_id, test_space_group_id, test_member_email)
      end

  test "deletes an existing member" do
  ...
  test "fails if member does not exist" do
