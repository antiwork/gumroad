...
class SslCertificatesGenerateTest < ActiveSupport::TestCase
  self.described_class = SslCertificates::Generate



  context_ SslCertificates::Generate do
    before do
  ...
  context_ "#hourly_rate_limit_reached?" do
      before do
  ...
  context_ "when hourly limit is not reached" do
  test "returns false" do
          expect(@obj.send(:hourly_rate_limit_reached?)).to eq false
        end
      end

  context_ "when hourly limit is reached" do
  context_ "when there are no deleted domains" do
          before do
  ...
  context_ "when there are deleted domains" do
          before do
            create(:custom_domain, domain: "www.example-2.com").set_ssl_certificate_issued_at!
  ...
  context_ "#can_order_certificates?" do
  context_ "with a valid certificate" do
  test "returns false when the domain already has a valid certificate" do
          @custom_domain.set_ssl_certificate_issued_at!
          expect(@obj.send(:can_order_certificates?)).to eq [false, "Has valid certificate"]
        end
      end

  context_ "when the domain is invalid" do
        before do
          @custom_domain.domain = "test_store.example.com"
          @custom_domain.save(validate: false)
        end

  test "returns false with an error message" do
          expect(@obj.send(:can_order_certificates?)).to eq [false, "Invalid domain"]
        end
      end

  context_ "when the hourly limit is reached" do
  test "returns false when the hourly rate limit is reached" do
  ...
  test "returns false when no domains are pointed to Gumroad" do
          expect(@obj.send(:can_order_certificates?)).to eq [false, "No domains pointed to Gumroad"]
        end
      end
    end

  context_ "#domain_check_cache_key" do
  test "returns domin check cache key" do
        expect(@obj.send(:domain_check_cache_key)).to eq "domain_check_www.example.com"
      end
    end

  context_ "#generate_certificate" do
      before do
  ...
  test "invokes process method of LetsEncrypt service" do
  ...
  context_ "#process" do
  context_ "when `can_order_certificates?` returns false" do
        before do
  ...
  context_ "when the certificate is successfully created" do
        before do
  ...
  test "set ssl_certificate_issued_at and logs a message" do
          @domains_pointed_to_gumroad.each do |domain|
  ...
  context_ "when the certificate generation fails" do
        before do
  ...
  test "writes the information to cache and resets custom_domain's ssl_certificate_issued_at attribute" do
