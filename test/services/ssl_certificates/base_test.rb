...
class SslCertificatesBaseTest < ActiveSupport::TestCase
  self.described_class = SslCertificates::Base



  context_ SslCertificates::Base do
    before do
  ...
  context_ "self.supported_environment?" do
  ...
  test "sets the required config variables as methods" do
  ...
  context_ "#certificate_authority" do
  test "returns the certificate authority" do
        expect(@obj.send(:certificate_authority)).to eq SslCertificates::LetsEncrypt
      end
    end

  context_ "#ssl_file_path" do
  test "returns the S3 SSL file path" do
  ...
  test "deletes the file from S3" do
  ...
  context_ "#write_to_s3" do
  test "writes the content to S3" do
        test_key = "test_key"
