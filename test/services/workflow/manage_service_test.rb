...
class WorkflowManageServiceTest < ActiveSupport::TestCase
  self.described_class = Workflow::ManageService



  context_ Workflow::ManageService do
    before do
  ...
  context_ "when workflow does not exist" do
  ...
  test "creates a product workflow" do
  ...
  test "creates a variant workflow" do
  ...
  test "creates a seller workflow" do
  ...
  test "creates an audience workflow" do
  ...
  test "creates a follower workflow" do
  ...
  test "creates an affiliate workflow" do
  ...
  test "creates a workflow with 'member_cancellation' trigger" do
  ...
  test "creates a workflow with the new customer trigger when 'workflow_type' is 'audience'" do
          params[:workflow_type] = Workflow::AUDIENCE_TYPE
          params[:workflow_trigger] = Workflow::MEMBER_CANCELLATION_WORKFLOW_TRIGGER
  ...
  test "creates an abandoned cart workflow with a ready-made installment" do
          create(:payment_completed, user: seller)
          params.merge!(
  ...
  test "returns error if seller is not eligible for abandoned cart workflows" do
  ...
  test "creates a workflow with filters" do
            expect do
              expect(described_class.new(seller:, params:, product:, workflow: nil).process).to eq([true, nil])
            end.to change { Workflow.count }.from(0).to(1)
  ...
  test "returns false if the paid filters are invalid" do
  ...
  test "returns false if the date filters are invalid" do
  ...
  context_ "when workflow already exists" do
  ...
  test "updates the workflow and its installments" do
  ...
  test "updates the workflow name but ignores all other params if the workflow was previously published" do
  ...
  context_ "when changing the workflow type to 'abandoned_cart'" do
  test "updates the workflow, deletes all existing installments and creates a ready-made abandoned cart installment" do
            params[:workflow_type] = Workflow::ABANDONED_CART_TYPE
  ...
  context_ "when changing the workflow type from 'abandoned_cart' to another type" do
  test "updates the workflow and deletes the abandoned cart installment" do
  ...
  test "updates the workflow filters" do
  ...
  test "updates the workflow and publishes it if the save action is 'save_and_publish'" do
  ...
  test "updates the workflow and unpublishes it if the save action is 'save_and_unpublish'" do
  ...
  test "does not save changes while publishing the workflow if the seller's email is not confirmed" do
          stripe_connect_account = create(:merchant_account_stripe_connect, user: seller)
          create(:purchase, seller:, link: product, merchant_account: stripe_connect_account)
  ...
  test "returns an error if the paid filters are invalid" do
          params[:workflow_type] = Workflow::PRODUCT_TYPE
          params[:paid_more_than] = "10"
          params[:paid_less_than] = "5"
  ...
  test "returns an error if the date filters are invalid" do
          params[:workflow_type] = Workflow::PRODUCT_TYPE
          params[:created_after] = "2020-12-31"
          params[:created_before] = "2019-01-01"
