...
class WorkflowSaveInstallmentsServiceTest < ActiveSupport::TestCase
  self.described_class = Workflow::SaveInstallmentsService



  context_ Workflow::SaveInstallmentsService do
    before do
  ...
  context_ "for an abandoned cart workflow" do
        let!(:workflow) { create(:workflow, seller:, link: product, workflow_type: Workflow::ABANDONED_CART_TYPE) }

  test "returns an error when the number of saved installments are zero" do
          params[:installments] = []
  ...
  test "returns an error while saving more than one installment" do
  ...
  test "saves only one installment" do
  ...
  test "appends the <product-list-placeholder /> tag to the message if it's missing" do
  ...
  test "creates installments and publishes them if save_action_name is 'save_and_publish'" do
  ...
  test "updates installments" do
        expect_any_instance_of(Workflow).not_to receive(:schedule_installment)

        process_and_perform_assertions_for_updated_installments
      end

  test "updates installments and publishes them if save_action_name is 'save_and_publish'" do
        params[:save_action_name] = Workflow::SAVE_AND_PUBLISH_ACTION
  ...
  test "updates installments and unpublishes them if save_action_name is 'save_and_unpublish'" do
        workflow.publish!
  ...
  context_ "when delayed_delivery_time changes" do
  ...
  test "does not reschedule that installment if save_action_name is other than 'save'" do
  ...
  context_ "when installment has not been published" do
  test "does not reschedule that installment" do
            expect(SendWorkflowPostEmailsJob).not_to receive(:perform_async)
  ...
  test "reschedules a newly added installment if the workflow is already published" do
        workflow.publish!
  ...
  test "does not reschedule an installment if the delayed_delivery_time does not change" do
        expect(SendWorkflowPostEmailsJob).not_to receive(:perform_async)
  ...
  test "deletes installments that are missing from the params" do
        installment = create(:installment, workflow:)
  ...
  test "updates the 'send_to_past_customers' flag only if the workflow is yet to be published" do
  ...
  test "maintains a mapping of old and new installment ids" do
  ...
  test "does not save installments if there are errors" do
  ...
  test "does not save installments while publishing if the seller's email is not confirmed" do
  ...
  context_ "email preview" do
  context_ "for an abandoned cart workflow" do
          let(:workflow) { create(:abandoned_cart_workflow, seller:) }

  test "sends preview email" do
  ...
  context_ "for a non-abandoned cart workflow" do
  test "sends preview email" do
  ...
  test "does not send preview email if the reciepient hasn't confirmed their email address yet" do
          ...
          expect { service.process }.not_to change { installment.reload.name }
          expect(service.errors.full_messages.first).to eq("You have to confirm your email address before you can do that.")
        end

  test "does not send preview email if send_preview_email is false" do
          expect(PostSendgridApi).not_to receive(:process)
  ...
  test "processes upsell cards in the installment message" do
