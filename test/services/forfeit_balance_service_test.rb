...
class ForfeitBalanceServiceTest < ActiveSupport::TestCase
  self.described_class = ForfeitBalanceService



  context_ ForfeitBalanceService do
  ...
  context_ "#process" do
  context_ "when the user doesn't have an unpaid balance" do
  test "returns nil" do
  ...
  context_ "when the user has an unpaid balance" do
          before do
            @balance = create(:balance, merchant_account:, user:, amount_cents: 1050)
          end

  test "marks the balances as forfeited" do
            @service.process
  ...
  test "adds a comment on the user" do
            @service.process
  ...
  test "does not add a negative credit" do
            @service.process
  ...
  context_ "#balance_amount_cents_to_forfeit" do
  test "returns the correctly formatted balance" do
          create(:balance, user:, merchant_account:, amount_cents: 765)

          expect(@service.balance_amount_cents_to_forfeit).to eq(765)
        end

  test "excludes balances held by Gumroad" do
  ...
  context_ "#balance_amount_formatted" do
  test "returns the correctly formatted balance" do
  ...
  context_ "when the user has a positive unpaid balance" do
          before do
  ...
  context_ "when the user has a negative unpaid balance" do
          before do
            @balance = create(:balance, user:, merchant_account:, amount_cents: -765)
          end

  test "doesn't forfeit the balance" do
            expect(@service.process).to eq(nil)
  ...
  context_ "#balance_amount_cents_to_forfeit" do
  test "returns the correct amount" do
