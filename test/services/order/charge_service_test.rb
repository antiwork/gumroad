...
class OrderChargeServiceTest < ActiveSupport::TestCase
  self.described_class = Order::ChargeService
  self.rspec_metadata = { vcr: true }

  # frozen_string_literal: false

  context_ Order::ChargeService, :vcr do
  ...
  test "charges all purchases in the order with the payment method provided in params" do
  ...
  test "charges all purchases in the order when seller has a Stripe merchant account" do
  ...
  test "charges 2.9% + 30c of processor fee when seller has a Stripe merchant account and existing credit card is used for payment" do
        seller_stripe_account = create(:merchant_account, user: seller_1, charge_processor_merchant_id: create_verified_stripe_account(country: "US").id)
  ...
  test "does not charge Gumroad fee and taxes when seller has a Brazilian Stripe Connect account" do
  ...
  test "charges the correct custom fee when seller has custom Gumroad fee set" do
  ...
  test "returns error responses for all purchases if corresponding charge fails" do
  ...
  test "returns error responses with USD formatted price even when product display currency is EUR" do
  ...
  test "returns SCA response if the payment method provided in params requires SCA" do
  ...
  test "creates multiple charges in case of purchases from different sellers" do
  ...
  test "creates a charge with no amount if all the items from a seller are free" do
  ...
  test "creates a charge with no amount for a free trial membership product" do
        line_items_params = {
          line_items: [
            {
              uid: "unique-id-0",
  ...
  test "creates charges with no amounts for sellers whose items don't require an immediate payment" do
        line_items_params = {
          line_items: [
            {
              uid: "unique-id-0",
  ...
  test "includes free purchases in charges along with the paid purchases" do
  ...
  test "skips purchases that already have a processor payment intent" do
        create(:merchant_account, user: seller_1, charge_processor_merchant_id: create_verified_stripe_account(country: "US").id)
  ...
  context_ "when payment method requires mandate" do
  ...
  test "creates a mandate for a single membership purchase" do
  ...
  test "creates a mandate for multiple membership purchases" do
  ...
  context_ "#ensure_all_purchases_processed" do
  test "does not raise when purchases is nil" do
        order = create(:order)
  ...
  test "does not raise NoMethodError when an error occurs before non_free_seller_purchases is assigned" do
        seller = create(:user)
  ...
  test "falls back to seller_purchases for cleanup when non_free_seller_purchases is nil" do
        seller = create(:user)
        product = create(:product, user: seller, price_cents: 10_00)
        line_items = {
          line_items: [
  ...
  test "marks free purchases as successful in the fallback path when an exception occurs before non_free_seller_purchases is assigned" do
        seller = create(:user)
  ...
  test "does not schedule FailAbandonedPurchaseWorker due to stale charge_intent from a prior seller when an exception occurs" do
  ...
  test "retries marking as successful when charge_intent succeeded but post-charge processing failed" do
        seller = create(:user)
  ...
  test "marks successful without recreating balance transactions when charge data was already saved" do
        seller = create(:user)
        merchant_account = create(:merchant_account, user: nil)
        product = create(:product, user: seller, price_cents: 10_00)
        order = create(:order)
  ...
  test "applies an orphan seller balance transaction before marking successful" do
        seller = create(:user)
        merchant_account = create(:merchant_account, user: nil)
        product = create(:product, user: seller, price_cents: 10_00)
        order = create(:order)
  ...
  test "keeps lock timeouts while applying orphan seller balance transactions from escaping" do
        seller = create(:user)
        merchant_account = create(:merchant_account, user: nil)
        product = create(:product, user: seller, price_cents: 10_00)
        order = create(:order)
  ...
  test "keeps record validation errors while applying orphan seller balance transactions from escaping" do
        seller = create(:user)
        merchant_account = create(:merchant_account, user: nil)
        product = create(:product, user: seller, price_cents: 10_00)
        order = create(:order)
  ...
  test "keeps recommended purchase failures from turning successful charged retries into errors" do
        seller = create(:user)
        merchant_account = create(:merchant_account, user: nil)
        product = create(:product, user: seller, price_cents: 10_00)
        order = create(:order)
  ...
  test "keeps post-success finalization failures from turning successful charged retries into errors" do
        seller = create(:user)
        merchant_account = create(:merchant_account, user: nil)
        product = create(:product, user: seller, price_cents: 10_00)
        order = create(:order)
  ...
  test "creates affiliate credit from an applied affiliate balance transaction without duplicating it" do
        seller = create(:user)
        merchant_account = create(:merchant_account, user: nil)
        product = create(:product, user: seller, price_cents: 10_00)
  ...
  test "does not retry marking as successful for errored purchases without charge data" do
        seller = create(:user)
        product = create(:product, user: seller, price_cents: 10_00)
        order = create(:order)
  ...
  test "returns mandate options of the purchase in case of single purchase" do
  ...
  test "returns mandate options with sporadic interval and amount as maximum of the price of included purchases" do
        order = create(:order)
        purchase = create(:purchase_in_progress, link: membership_product, is_original_subscription_purchase: true,
  ...
  context_ "#perform rejecting a cart that overruns an offer code limit" do
  test "fails the offending line items, skips Stripe, and returns an error response per line item" do
        seller = create(:user)
