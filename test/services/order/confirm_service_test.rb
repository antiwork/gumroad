...
class OrderConfirmServiceTest < ActiveSupport::TestCase
  self.described_class = Order::ConfirmService
  self.rspec_metadata = { vcr: true }

  # frozen_string_literal: false

  context_ Order::ConfirmService, :vcr do
  context_ "#perform" do
  ...
  test "calls Purchase::ConfirmService#perform for all purchases in the order" do
  ...
  test "returns error responses for all purchases in case of SCA failure" do
        expect(Purchase::ConfirmService).to receive(:new).exactly(2).times.and_call_original
  ...
  test "returns purchase error responses and offer codes in case of SCA failure with offer codes applied" do
