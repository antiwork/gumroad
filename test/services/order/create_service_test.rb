...
class OrderCreateServiceTest < ActiveSupport::TestCase
  self.described_class = Order::CreateService
  self.rspec_metadata = { vcr: true }

  # frozen_string_literal: false


  context_ Order::CreateService, :vcr do
  ...
  context_ "#perform" do
  test "creates an order along with the associated purchases in progress" do
        expect do
          expect do
            expect do
  ...
  test "calls Purchase::CreateService for all line items in params with is_part_of_combined_charge set to true" do
  ...
  test "sets all the common fields on all purchases correctly" do
        order, _ = Order::CreateService.new(params:).perform
  ...
  test "saves the referrer info correctly" do
  ...
  test "returns failure responses with correct errors for purchases that fail" do
  ...
  test "does not delete the cart when all line items fail" do
        failed_params = {
          line_items: [
  ...
  test "creates an order along with the associated purchases in progress when merchant account is a Brazilian Stripe Connect account" do
  ...
  context_ "when a line item has a restartable subscription" do
  ...
  test "treats submitted checkout payment data as a new card during subscription restart in a multi-seller cart" do
          multi_seller_params = {
            line_items: [
              {
                uid: "unique-id-0",
  ...
  test "does not add the restarted subscription's original purchase to the order" do
          updater_service = instance_double(Subscription::UpdaterService)
  ...
  test "includes the regular purchase in the order" do
          updater_service = instance_double(Subscription::UpdaterService)
          allow(Subscription::UpdaterService).to receive(:new).and_return(updater_service)
          allow(updater_service).to receive(:perform).and_return({ success: true, success_message: "Membership restarted" })
  ...
  test "passes through SCA data when the restart requires card action" do
  ...
  test "SCA response can be confirmed via Purchase::ConfirmService" do
  ...
  test "cleans up the cart when all line items are subscription restarts" do
          restart_only_params = {
            line_items: [
              {
                uid: "unique-id-0",
  ...
  context_ "when a line item has an active subscription" do
        let(:membership_product) { create(:membership_product, user: seller_1, price_cents: price_1) }
        let(:buyer) { create(:user, email: "buyer@gumroad.com") }
        let!(:subscription) do
          sub = create(:subscription, link: membership_product, user: buyer)
  ...
  test "returns an error for the membership line item" do
