...
class ExportsTaxSummaryPayableTest < ActiveSupport::TestCase
  self.described_class = Exports::TaxSummary::Payable
  self.rspec_metadata = { vcr: true }



  context_ Exports::TaxSummary::Payable, :vcr do
  ...
  test "generates total transactions count" do
  ...
  test "adds compliance and other user related fields" do
        csv = Exports::TaxSummary::Payable.new(user: @user, year:).perform
        parsed_csv = CSV.parse(csv)
        row = parsed_csv[1]
  ...
  context_ "when user is a business" do
        before do
          @user.user_compliance_infos.delete_all
          @compliance_info = create(:user_compliance_info_business, user: @user)
        end

  test "adds individual tax id and business tax id if user is a business" do
          csv = Exports::TaxSummary::Payable.new(user: @user, year:).perform
          parsed_csv = CSV.parse(csv)
          row = parsed_csv[1]
          expect(row[15]).to eq(@compliance_info.individual_tax_id.decrypt(GlobalConfig.get("STRONGBOX_GENERAL_PASSWORD")))
  ...
  test "returns no data if no payments exist" do
