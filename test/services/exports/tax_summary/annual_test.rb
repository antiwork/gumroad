...
class ExportsTaxSummaryAnnualTest < ActiveSupport::TestCase
  self.described_class = Exports::TaxSummary::Annual
  self.rspec_metadata = { vcr: true }



  context_ Exports::TaxSummary::Annual, :vcr do
  ...
  test "does not export for non compliant users" do
  ...
  test "does not export for non-us users" do
  ...
  test "does not export for invalid compliance country users" do
        UserComplianceInfo.delete_all
        @compliance_info = create(:user_compliance_info, user: @user)
  ...
  test "does not export users without min sales amount" do
  ...
  test "generates total transactions amount" do
        csv_url = described_class.new(year:).perform
        URI.open(csv_url) do |f|
  ...
  test "creates monthly breakdown with transaction amount" do
        csv_url = described_class.new(year:).perform
  ...
  test "returns compliance and other user related fields" do
        csv_url = described_class.new(year:).perform
        URI.open(csv_url) do |f|
          parsed_csv = CSV.parse f.read
          row = parsed_csv[1]
  ...
  test "adds tax id if user is an individual" do
        csv_url = described_class.new(year:).perform
        URI.open(csv_url) do |f|
          parsed_csv = CSV.parse f.read
          row = parsed_csv[1]
  ...
  test "adds business tax id if user is a business" do
