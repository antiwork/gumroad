...
class ExportsAudienceExportServiceTest < ActiveSupport::TestCase
  self.described_class = Exports::AudienceExportService



  context_ Exports::AudienceExportService do
  context_ "#perform" do
  ...
  context_ "when options has followers" do
        let(:options) { { followers: true } }

  test "generates csv with followers" do
  ...
  context_ "when options has customers" do
        let(:options) { { customers: true } }

  test "generates csv with customers" do
          rows = CSV.parse(subject.perform.tempfile.read)
  ...
  context_ "when options has affiliates" do
  ...
  context_ "when options has all audience types" do
        let(:options) { { followers: true, customers: true, affiliates: true } }

  test "generates csv with all audience types" do
          rows = CSV.parse(subject.perform.tempfile.read)
  ...
  context_ "when user is both a follower and a customer" do
  ...
  test "generates csv with unique entries with minimum created_at" do
          rows = CSV.parse(subject.perform.tempfile.read)
