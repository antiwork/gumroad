...
class ExportsPayoutsAnnualTest < ActiveSupport::TestCase
  self.described_class = Exports::Payouts::Annual
  self.rspec_metadata = { vcr: true }



  context_ Exports::Payouts::Annual, :vcr do
  ...
  test "shows all activity related to the yearly payout" do
        date_for_year = Date.new(year)
  ...
  test "returns total_amount from the yearly payout" do
        date_for_year = Date.new(year)
        data = Exports::Payouts::Annual.new(user: @user, year:).perform
  ...
  test "returns no data on failed payments" do
        date_for_year = Date.new(year)
  ...
  test "does not return sales falling on days not in given year" do
