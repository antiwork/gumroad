...
class ExportsPayoutsCsvTest < ActiveSupport::TestCase
  self.described_class = Exports::Payouts::Csv
  self.rspec_metadata = { vcr: true }



  context_ Exports::Payouts::Csv, :vcr do
  context_ "perform" do
  ...
  test "shows all activity related to the payout" do
  ...
  context_ "Technical Adjustment entries" do
  test "does not add entry for non-usd payment currency" do
  ...
  test "add entry for usd payment currency" do
          @payout.amount_cents = @payout.amount_cents + 100
          @payout.save!
          csv = Exports::Payouts::Csv.new(payment: @payout).perform
          parsed_csv = CSV.parse(csv)
  ...
  context_ "affiliate fees from Stripe Connect sales" do
  test "correctly adds the affiliate fee entries from Stripe Connect sales" do
        seller = create :user
  ...
  context_ "affiliate credits involving several balances" do
      let!(:now) { Time.current }
      let!(:payout_date) { 1.week.ago }
      let(:payout_processor_type) { PayoutProcessorType::PAYPAL }
  ...
  test "takes refunded credit into account, even if actual credit happened in a different payout" do
        csv = Exports::Payouts::Csv.new(payment: @payout).perform
        parsed_csv = CSV.parse(csv)
