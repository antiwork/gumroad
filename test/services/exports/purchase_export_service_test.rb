...
class ExportsPurchaseExportServiceTest < ActiveSupport::TestCase
  self.described_class = Exports::PurchaseExportService



  context_ Exports::PurchaseExportService do
  context_ "#perform" do
      before do
        @seller = create(:user)
  ...
  test "includes the variant price cents", :vcr do
  ...
  test "shows the custom sku", :vcr do
        @product = create(:product, price_range: "$1", skus_enabled: true, user: @seller)
        @category1 = create(:variant_category, title: "Size", link: @product)
        @variant1 = create(:variant, variant_category: @category1, name: "Small")
        @category2 = create(:variant_category, title: "Color", link: @product)
  ...
  test "includes the preorder authorization date-time", :vcr do
  ...
  context_ "when country is #{country.inspect}, rate is #{rate.inspect}, and excluded is #{excluded.inspect}" do
              before do
                @purchase.update!(
  ...
  test "includes Stripe fields only for Stripe Connect sales", :vcr do
  ...
  test "raises error if a value is not JSON safe (type other than String, Number, Array, Hash, Boolean, NilClass)" do
  ...
  test "includes the tip amount in USD" do
          expect(field_value(last_data_row, "Tip ($)")).to eq("4.5")
          expect(field_value(totals_row, "Tip ($)")).to eq("5.5")
        end
      end

  context_ "when the purchase has no tip" do
  test "shows 0 for the tip amount" do
          expect(field_value(last_data_row, "Tip ($)")).to eq("0.0")
          expect(field_value(totals_row, "Tip ($)")).to eq("0.0")
        end
      end

  context_ "UTM parameters" do
  context_ "when the purchase was not driven by a UTM link" do
  test "includes blank values for UTM parameters" do
  ...
  context_ "when the purchase was driven by a UTM link" do
  test "includes the UTM parameters" do
