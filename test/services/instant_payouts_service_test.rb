...
class InstantPayoutsServiceTest < ActiveSupport::TestCase
  self.described_class = InstantPayoutsService
  self.rspec_metadata = { vcr: true }



  context_ InstantPayoutsService, :vcr do
  ...
  context_ "#perform" do
  context_ "with stripe account" do
        before do
  ...
  context_ "when seller has a balance within the instant payout range" do
          before do
  ...
  test "creates and processes an instant payout" do
            expect do
  ...
  context_ "when the seller has balances that exceed the maximum instant payout amount" do
  ...
  test "creates and processes multiple instant payouts" do
            expect do
              result = described_class.new(seller).perform
              expect(result[:success]).to be true
  ...
  context_ "when seller has a balance less than $100" do
          before do
  ...
  context_ "when seller has a balance greater than $10,000" do
          before do
  ...
  context_ "when seller is not eligible for instant payouts" do
        before do
  ...
  context_ "when seller has no stripe account" do
  ...
  context_ "when seller is not compliant" do
        before do
  ...
  context_ "when the payout fails" do
