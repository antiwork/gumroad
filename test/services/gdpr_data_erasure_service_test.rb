...
class GdprDataErasureServiceTest < ActiveSupport::TestCase
  self.described_class = GdprDataErasureService



  context_ GdprDataErasureService do
  ...
  test "anonymizes buyer purchases" do
        purchase = create(
          :free_purchase,
          purchaser: user,
          email: user.email,
  ...
  test "anonymizes guest purchases using the original email address" do
        purchase = create(
          :free_purchase,
          purchaser: nil,
          email: user.email,
  ...
  test "anonymizes all of the user's carts and credit card records" do
  ...
  test "deletes the user's device records" do
  ...
  test "invokes the private subscription cancellation helper during erasure" do
        expect(user).to receive(:cancel_active_subscriptions!)

        described_class.new(user, performed_by: admin).perform!
      end

  test "deactivates the account and deletes products" do
  ...
  test "reports only alive products in the erasure summary" do
  ...
  test "logs the erasure as a comment" do
        described_class.new(user, performed_by: admin).perform!
  ...
  test "returns external cleanup instructions" do
        result = described_class.new(user, performed_by: admin).perform!
  ...
  test "skips profile asset removal when transactional erasure work fails" do
