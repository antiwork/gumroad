...
class DefaultAbandonedCartWorkflowGeneratorServiceTest < ActiveSupport::TestCase
  self.described_class = DefaultAbandonedCartWorkflowGeneratorService



  context_ DefaultAbandonedCartWorkflowGeneratorService do
  ...
  context_ "when seller does not have an abandoned cart workflow" do
  test "creates a new abandoned cart workflow and publishes it" do
          expect do
            subject.generate
  ...
  context_ "when seller already has an abandoned cart workflow" do
        before do
          create(:workflow, seller:, workflow_type: Workflow::ABANDONED_CART_TYPE, deleted_at: 1.hour.ago)
        end

  test "does not create a new abandoned cart workflow" do
          expect do
            expect do
              expect do
                subject.generate
              end.not_to change { seller.workflows.abandoned_cart_type.count }
