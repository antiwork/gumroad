...
class ApiInternalAdminCursorPaginationTest < ActiveSupport::TestCase
  self.described_class = Api::Internal::Admin::CursorPagination



  context_ Api::Internal::Admin::CursorPagination do
    let(:base_time) { Time.zone.local(2026, 1, 15, 12, 0, 0) }
    let(:default_order) { [[:created_at, :desc], [:id, :desc]] }

  context_ ".encode and .decode" do
