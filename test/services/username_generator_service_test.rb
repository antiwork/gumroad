...
class UsernameGeneratorServiceTest < ActiveSupport::TestCase
  self.described_class = UsernameGeneratorService



  context_ UsernameGeneratorService do
  context_ "#username" do
      let(:user) { build(:user, email: "johnsmith@gmail.com") }
      let(:username_generator) { described_class.new(user) }

  test "returns a username generated by OpenAI", :vcr do
  ...
  test "uses the user's name when they don't have an email address", :vcr do
        user = build(:user, email: nil, name: "Alexander Nathaniel Harrison")
        expect(described_class.new(user).username).to eq "alxnatharrison"
      end

  test "doesn't try to generate a username if the user doesn't have a name or email" do
        user = build(:user, email: nil, name: nil)
        expect(described_class.new(user).username).to be_nil
      end

  context_ "email contains a \"tricky\" email with common email domain", :vcr do
  ...
  test "uses the desired username for #{email}" do
  ...
  context_ "OpenAI returns a username that is invalid" do
  test "generates a valid username when given a username with only numbers" do
  ...
  test "generates a valid username when given a blank username" do
  ...
  test "generates a valid username when given a username with capital letters and invalid characters" do
  ...
  test "generates a valid username when given a username that is too short" do
  ...
  test "generates a valid username when given a username that is too long" do
  ...
  test "generates a valid username when given a username that is a word from the DENYLIST" do
