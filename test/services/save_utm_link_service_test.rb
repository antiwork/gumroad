...
class SaveUtmLinkServiceTest < ActiveSupport::TestCase
  self.described_class = SaveUtmLinkService



  context_ SaveUtmLinkService do
    let(:seller) { create(:user) }
    let(:product) { create(:product, user: seller) }
    let(:post) { create(:audience_post, :published, shown_on_profile: true, seller:) }

  context_ "#perform" do
  context_ "when 'utm_link' is not provided" do
  ...
  test "raises an error if the UTM link fails to save" do
          expect do
            described_class.new(
              seller:,
              params: {
  ...
  context_ "when 'utm_link' is provided" do
  ...
  test "updates only the permitted attributes" do
