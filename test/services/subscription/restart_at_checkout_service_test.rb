...
class SubscriptionRestartAtCheckoutServiceTest < ActiveSupport::TestCase
  self.described_class = Subscription::RestartAtCheckoutService


  context_ Subscription::RestartAtCheckoutService do
  ...
  context_ "#perform" do
  context_ "delegation to UpdaterService" do
        let!(:subscription) do
  ...
  test "delegates to Subscription::UpdaterService with transformed params" do
  ...
  test "transforms checkout params to UpdaterService format" do
  ...
  test "uses the buyer identity when defaulting the perceived restart price" do
  ...
  test "treats submitted checkout payment data as a new card" do
  ...
  test "uses default variants when not provided in params" do
  ...
  test "adapts successful result with restarted_subscription flag" do
          updater_service = instance_double(Subscription::UpdaterService)
  ...
  test "includes requires_card_action when present" do
          updater_service = instance_double(Subscription::UpdaterService)
          allow(Subscription::UpdaterService).to receive(:new).and_return(updater_service)
          allow(updater_service).to receive(:perform).and_return({
                                                                   success: true,
  ...
  context_ "recurrence change (issue #117)" do
        let!(:subscription) do
          create_subscription_for_product(
            product: product,
            purchaser: buyer,
  ...
  test "passes the new price_id to UpdaterService when changing recurrence" do
  ...
  context_ "quantity passthrough" do
  ...
  test "uses params[:quantity] when provided" do
  ...
  test "falls back to original purchase quantity when params[:quantity] is not provided" do
          subscription.original_purchase.update!(quantity: 3)
  ...
  test "passes quantity of 1 for single-quantity subscriptions" do
          service = described_class.new(
            subscription: subscription,
            product: expensive_product,
            params: base_params.merge(price_id: expensive_product.prices.alive.first.external_id),
  ...
  context_ "offer code resolution" do
        let(:expensive_product) { create(:membership_product, user: seller, price_cents: 10_00) }
  ...
  context_ "when no discount code is entered" do
  test "does not include offer_code in params" do
            service = described_class.new(
              subscription: subscription,
              product: expensive_product,
  ...
  test "sets clear_discount to true when the original purchase has a discount" do
  ...
  test "sets clear_discount to false when the original purchase has no discount" do
            service = described_class.new(
              subscription: subscription,
              product: expensive_product,
              params: offer_code_params,
  ...
  test "sets clear_discount to false when an offer code is entered" do
            params_with_discount = offer_code_params.deep_merge(
              purchase: { discount_code: offer_code.code }
            )
  ...
  context_ "when an invalid discount code is entered" do
  test "does not include offer_code in params" do
            params_with_discount = offer_code_params.deep_merge(
  ...
  context_ "PWYW subscription restart with discount change" do
  ...
  test "passes price_range so PWYW price is preserved when discount changes" do
  ...
  context_ "integration behavior" do
