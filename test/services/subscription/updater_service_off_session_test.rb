...
class SubscriptionUpdaterServiceTest < ActiveSupport::TestCase
  ...
  context_ Subscription::UpdaterService, "#charge_user! off_session behavior" do
  ...
  context_ "when stripe_setup_intent_id is present (SetupIntent already authenticated SCA)" do
  test "passes off_session: true to subscription.charge!" do
  ...
  test "does not merge setup_future_charges into purchase params" do
        mandate_card = instance_double(CreditCard, requires_mandate?: true)
        allow(subscription).to receive(:credit_card_to_charge).and_return(mandate_card)
        allow(subscription).to receive(:charge!).and_return(successful_purchase)
  ...
  context_ "when stripe_setup_intent_id is absent and card requires mandate" do
  test "passes off_session: false to subscription.charge!" do
        mandate_card = instance_double(CreditCard, requires_mandate?: true)
        allow(subscription).to receive(:credit_card_to_charge).and_return(mandate_card)
        allow(subscription).to receive(:charge!).and_return(successful_purchase)
  ...
  test "merges setup_future_charges into purchase params" do
        mandate_card = instance_double(CreditCard, requires_mandate?: true)
        allow(subscription).to receive(:credit_card_to_charge).and_return(mandate_card)
        allow(subscription).to receive(:charge!).and_return(successful_purchase)
  ...
  context_ "when stripe_setup_intent_id is absent and card does not require mandate" do
  test "passes off_session: true to subscription.charge!" do
