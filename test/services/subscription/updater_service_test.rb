...
class SubscriptionUpdaterServiceTest < ActiveSupport::TestCase
  self.described_class = Subscription::UpdaterService
  self.rspec_metadata = { vcr: true }



  context_ Subscription::UpdaterService, :vcr do
    include ManageSubscriptionHelpers
    include CurrencyHelper

  context_ "#perform" do
  context_ "tiered membership subscription" do
        let(:gift) { nil }
        before :each do
  ...
  context_ "updating the credit card" do
  test "updates the subscription but not the user's card" do
            ...
            expect do
              service.perform
            end.not_to have_enqueued_mail(CustomerLowPriorityMailer, :subscription_giftee_added_card)
  ...
  context_ "when the new card requires e-mandate" do
  test "updates the subscription but not the user's card" do
  ...
  context_ "restarting a membership" do
          before :each do
  ...
  context_ "using the existing card" do
  test "reactivates the subscription" do
  ...
  test "charges the existing card" do
              expect(@subscription).to receive(:send_restart_notifications!)
  ...
  test "does not update the user's credit card" do
              expect(@subscription).to receive(:send_restart_notifications!)
  ...
  test "raises error if both subscription and user payment methods are not supported by the creator" do
  ...
  test "does not send email when charge user fails" do
  ...
  test "does not raise error if the user payment method is not supported by the creator but subscription one is" do
              expect(@subscription).to receive(:send_restart_notifications!)
              paypal_credit_card = create(:credit_card, chargeable: build(:native_paypal_chargeable), user: @user)
              @user.update!(credit_card: paypal_credit_card)
  ...
  test "updates the subscription's card" do
              expect(@subscription).to receive(:send_restart_notifications!)
  ...
  test "charges the new card and returns proper SCA response" do
                expect(@subscription).not_to receive(:send_restart_notifications!)
                old_card = @original_purchase.credit_card
  ...
  context_ "when the price has changed" do
  test "charges the current price" do
  ...
  context_ "changing plans" do
  test "allows downgrading recurrence immediately" do
              expect do
  ...
  test "allows upgrading recurrence immediately" do
  ...
  test "allows downgrading tier immediately" do
              expect do
  ...
  test "allows upgrading tier immediately" do
  ...
  test "allows upgrading tier immediately when card on record requires an e-mandate" do
  ...
  context_ "changing quantity" do
            before do
              setup_subscription(quantity: 2)
            end

  context_ "when increasing quantity" do
  test "immediately updates the purchase and charges the user" do
  ...
  context_ "when decreasing quantity" do
  test "creates a plan change and does not charge the user" do
                travel_to(@originally_subscribed_at + 1.day)
                result = Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  context_ "when the membership was cancelled by the creator" do
  test "does not allow restarting the membership" do
  ...
  context_ "updating card after charge failure but before cancellation" do
          before do
            travel_to(@originally_subscribed_at + @subscription.period + 1.minute)
          end

  test "updates the card and charges the user" do
            old_card = @original_purchase.credit_card
            old_subscription_card = @subscription.credit_card
            old_user_card = @user.credit_card
  ...
  test "applies any plan changes immediately" do
            expect do
  ...
  test "updates the card and charges the user correctly if seller has a connected Stripe account" do
            old_card = @original_purchase.credit_card
            old_subscription_card = @subscription.credit_card
            old_user_card = @user.credit_card
  ...
  test "updates and charges the new card and returns proper SCA response" do
              old_card = @original_purchase.credit_card
              PostToPingEndpointsWorker.jobs.clear
  ...
  context_ "changing the price on a PWYW tier" do
          before :each do
            travel_back
  ...
  context_ "to a price that is higher" do
  test "charges the user the difference and creates a new 'original' purchase with the new price" do
              params = @params.merge(
                price_range: 7_99,
  ...
  context_ "when the card requires e-mandate" do
              before do
                indian_cc = create(:credit_card, user: @user, chargeable: create(:chargeable, card: StripePaymentMethodHelper.success_indian_card_mandate))
                @subscription.credit_card = indian_cc
                @subscription.save!
  ...
  test "charges the difference and returns proper SCA response" do
                params = @params.merge(
                  price_range: 7_99,
                  perceived_price_cents: 7_99,
                  perceived_upgrade_price_cents: 3_38,
  ...
  context_ "to a price that is lower" do
  test "records that the plan should be changed and does not charge the user" do
              params = @params.merge(
                price_range: 5_99,
  ...
  context_ "switching to a PWYW tier" do
          before :each do
            @new_tier.update!(customizable_price: true)
          end

  context_ "when the price is above the suggested price" do
            let(:params) do
              {
                price_id: @yearly_product_price.external_id,
                variants: [@new_tier.external_id],
  ...
  test "creates a new 'original' purchase with the new variant and price, including perceived_price_cents" do
              result = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params:,
  ...
  test "charges the pro-rated rate for the new variant for the remainder of the period" do
              expect do
                Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  context_ "when the price is below the suggested price" do
            let(:params) do
              {
                price_id: @monthly_product_price.external_id,
                variants: [@new_tier.external_id],
  ...
  test "does not change the variant or price immediately" do
              result = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params:,
  ...
  test "records that the plan should be changed at the end of the next billing period" do
              expect do
                Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  context_ "updating during a free trial" do
          let(:free_trial) { true }
  ...
  context_ "upgrading" do
  test "upgrades the user immediately and does not charge them" do
  ...
  test "sends a subscription_updated notification" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: upgrade_tier_params.merge(perceived_upgrade_price_cents: 0),
  ...
  context_ "downgrade" do
  test "downgrades the user immediately and does not charge them" do
              expect do
                Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  test "sends a subscription_udpated notification with the correct effective time" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: downgrade_tier_params.merge(perceived_upgrade_price_cents: 0),
  ...
  context_ "setting contact info" do
  test "sets the contact info on the original purchase" do
            result = Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
  ...
  context_ "also updating plan" do
  test "sets the contact info on the new 'original' purchase as well" do
              result = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
  ...
  context_ "updating pending plan changes" do
          let!(:plan_change) do
            create(:subscription_plan_change, subscription: @subscription)
          end

  context_ "when the plan has not changed" do
  test "does not delete pending plan changes" do
              params = {
                price_id: @quarterly_product_price.external_id,
                variants: [@original_tier.external_id],
                use_existing_card: true,
  ...
  context_ "when upgrading" do
  test "deletes pending plan changes" do
              create(:subscription_plan_change, subscription: @subscription)
  ...
  test "creates a new plan change" do
              expect do
                Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  test "deletes existing plan changes" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params:,
  ...
  context_ "purchase with a license" do
          let!(:license) { create(:license, purchase: @original_purchase) }

  context_ "when upgrading" do
  test "associates the license with the new subscription if updating succeeds" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
  ...
  context_ "when downgrading" do
  test "does not modify the license" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
  ...
  context_ "when not changing plan" do
  test "does not modify the license" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
  ...
  context_ "purchase with sent emails" do
          before do
  ...
  context_ "when upgrading" do
  test "associates the email infos with the new subscription if updating succeeds" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: upgrade_tier_params,
  ...
  context_ "when downgrading" do
  test "does not modify the email infos" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: downgrade_recurrence_params,
  ...
  test "restores the comments with the original_purchase" do
  ...
  context_ "membership has files" do
  test "creates a URL redirect for the new original purchase if upgrading" do
            travel_back
  ...
  context_ "updating when price has increased since subscribing" do
          before :each do
  ...
  context_ "upgrading" do
  test "uses the preexisting subscription price to determine amount owed" do
              result = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: upgrade_tier_params,
  ...
  context_ "and subscription has no tier associated" do
            before :each do
  ...
  context_ "updating when price has decreased since subscribing" do
          before :each do
            @original_price = @original_tier_quarterly_price.price_cents
  ...
  context_ "not changing plan" do
  test "does not charge the user or record a plan change" do
              expect do
                result = Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  context_ "updating a test subscription" do
  context_ "upgrading" do
  test "marks both the upgrade purchase and new original purchase as 'test_successful'" do
  ...
  context_ "updating a subscription with fixed duration" do
          before do
            @subscription.update!(charge_occurrence_count: 4)
          end

  test "allows updating credit card" do
            result = Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
              params: update_card_params,
  ...
  test "allows updating contact info" do
            result = Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
              params: update_contact_info_params,
  ...
  test "does not allow changing recurrence" do
            expect do
              result = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
  ...
  test "does not allow changing tier" do
            expect do
              result = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
  ...
  context_ "when upgrading tiers" do
  test "schedules workflow(s) associated with the new tier" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: upgrade_tier_params,
  ...
  context_ "when downgrading tiers" do
  test "does not schedule workflow(s) associated with the new tier" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
  ...
  context_ "when not changing tiers" do
  test "does not schedule any workflows" do
              Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: same_plan_params,
  ...
  context_ "error cases" do
  test "returns an error if missing or invalid variant" do
            invalid_params = [
              {
                price_id: @quarterly_product_price.external_id,
                variants: [],
  ...
  test "returns an error if missing or invalid price_id" do
            invalid_params = [
              {
                price_id: nil,
                variants: [@original_tier.external_id],
  ...
  test "returns an error if missing or invalid perceived_price_cents if user is changing plan" do
            invalid_params = [
              {
                price_id: @monthly_product_price.external_id,
                variants: [@original_tier.external_id],
  ...
  test "returns an error if missing or invalid perceived_upgrade_price_cents if user is changing plan" do
            invalid_params = [
              {
                price_id: @monthly_product_price.external_id,
                variants: [@original_tier.external_id],
  ...
  context_ "credit card errors" do
  context_ "when the user should be charged" do
  test "returns an error when using existing card that is invalid" do
  ...
  test "returns an error when using a new card with invalid parameters" do
                params = {
                  price_id: @yearly_product_price.external_id,
                  variants: [@new_tier.external_id],
                  perceived_price_cents: @new_tier_yearly_price.price_cents,
  ...
  context_ "coming from a card declined email" do
  test "does not enqueue declined card tasks" do
  ...
  context_ "errors saving records" do
            before :each do
  ...
  test "rolls back the transaction" do
              result = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params:,
  ...
  context_ "when old plan price has changed" do
  test "does not apply the new price, but fully rolls back the transaction" do
  ...
  context_ "PWYW errors" do
  test "returns an error if price is too low" do
              @new_tier.update!(customizable_price: true)
  ...
  context_ "contact info errors" do
  test "returns an error if email is missing" do
              [nil, ""].each do |email|
                params = {
                  price_id: @quarterly_product_price.external_id,
                  variants: [@original_tier.external_id],
  ...
  context_ "perceived prices don't match" do
  context_ "when new subscription price doesn't match" do
  context_ "when upgrading" do
  ...
  context_ "when upgrade purchase price doesn't match" do
  context_ "when upgrading" do
  test "returns an error" do
                  params = {
                    price_id: @yearly_product_price.external_id,
  ...
  context_ "notifying buyer and creator on upgrade" do
          let(:params) do
            {
              price_id: @yearly_product_price.external_id,
              variants: [@new_tier.external_id],
  ...
  test "emails an upgrade receipt to the buyer" do
            Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
              params:,
  ...
  context_ "notifying creator on downgrade" do
  test "notifies the creator" do
            mail_double = double
            allow(mail_double).to receive(:deliver_later)
  ...
  context_ "API notification" do
          before do
            # don't enqueue sale notification for the upgrade purchase to facilitate testing
            allow_any_instance_of(Purchase).to receive(:send_notification_webhook)
          end

  test "sends a subscription_updated notification when upgrading" do
            Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
              params: upgrade_tier_params,
  ...
  test "sends a subscription_updated notification when downgrading" do
            Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
              params: downgrade_tier_params,
  ...
  test "sends a subscription_updated notification when changing PWYW price" do
            travel_back
            setup_subscription(pwyw: true) # @original_purchase price is now $6.99
            travel_to(@originally_subscribed_at + 1.month)
  ...
  test "sends a subscription_updated notification when downgrading to a free plan" do
  ...
  test "does not send a subscription_updated notification when not changing plan" do
            Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
  ...
  context_ "enqueueing update integrations worker" do
  test "enqueues worker if new tier is different" do
            result = Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
              params: upgrade_tier_params,
  ...
  test "does not enqueue worker if tier did not change" do
            result = Subscription::UpdaterService.new(
              subscription: @subscription,
              gumroad_guid: @gumroad_guid,
              params: upgrade_recurrence_params,
  ...
  context_ "gifted subscription" do
  ...
  context_ "when upgrading" do
  test "create new original purchase while keeping as a gift" do
              service = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: upgrade_tier_params,
              ...
              expect(@subscription.original_purchase).not_to eq @original_purchase
  ...
  context_ "when giftee is adding a payment method" do
            before do
              @subscription.update!(credit_card: nil)
            end

  test "add payment method to the subscription and send a email notification" do
              service = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params: update_card_params,
  ...
  context_ "non-tiered membership subscription" do
        before :each do
  ...
  context_ "updating payment method" do
          before :each do
  ...
  context_ "to another card" do
  test "updates the card on file" do
  ...
  context_ "to a card that requires an e-mandate" do
  test "updates the card on file" do
  ...
  context_ "to PayPal via Braintree" do
  test "updates the card on file" do
  ...
  context_ "when subscription is pending cancellation (within the last billed period)" do
            before :each do
  ...
  test "restarts the membership and does not charge the user" do
              expect do
                result = Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  context_ "when subscription has been cancelled" do
            before :each do
              @subscription.update!(cancelled_at: @originally_subscribed_at + 2.weeks, cancelled_by_buyer: true)
              travel_to(@originally_subscribed_at + 5.weeks)
            end

  test "restarts the membership and charges the user" do
              expect do
                result = Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  context_ "changing plans" do
  context_ "upgrading recurrence" do
            let(:params) do
              {
                id: @subscription.external_id,
  ...
  test "makes the change and charges the user" do
              expect do
                result = Subscription::UpdaterService.new(
                  subscription: @subscription,
                  gumroad_guid: @gumroad_guid,
  ...
  test "does not send a subscription_updated notification" do
              result = Subscription::UpdaterService.new(
                subscription: @subscription,
                gumroad_guid: @gumroad_guid,
                params:,
  ...
  context_ "buyer-aware fallback pricing" do
  ...
  test "uses the logged in user when validating the unchanged plan price" do
  ...
  test "uses the logged in user when checking whether the subscription price changed" do
  ...
  test "passes the logged in user when charging an immediate update" do
  ...
  context_ "when restarting with offer code changes" do
        let(:free_trial) { false }
  ...
  test "clears the discount when restarting without an offer code" do
          original_purchase = @subscription.original_purchase
  ...
  test "updates the discount when the seller changed the offer code percentage" do
  ...
  test "applies a different offer code when provided" do
  ...
  test "does not update the plan when the same unchanged offer code is provided" do
  ...
  context_ "when restarting a completed installment plan" do
  test "returns an error and does not restart the subscription" do
  ...
  context_ "inventory counter cache" do
        before :each do
          setup_subscription
          @gumroad_guid = "abc123"
          @remote_ip = "11.22.33.44"
  ...
  test "does not drift link or variant cache on a non-immediate downgrade" do
  ...
  test "credits the new variant on an immediate tier upgrade and decrements the previous tier" do
          product = @product.reload
          original_tier = @original_tier.reload
  ...
  test "does not double-count when resubscribing with a tier change" do
          travel_to(@originally_subscribed_at + 4.months)
          @subscription.update!(cancelled_at: 1.day.ago, cancelled_by_buyer: true)
