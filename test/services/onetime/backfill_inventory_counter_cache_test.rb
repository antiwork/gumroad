...
class OnetimeBackfillInventoryCounterCacheTest < ActiveSupport::TestCase
  self.described_class = Onetime::BackfillInventoryCounterCache



  context_ Onetime::BackfillInventoryCounterCache do
  context_ ".process" do
  ...
  test "sums quantities of qualifying purchases into both link and variant caches" do
  ...
  test "writes 0 when there are no qualifying purchases, overwriting any stale value" do
        variant
  ...
  test "overwrites stale cache values with the correct sum" do
  ...
  context_ "purchase state filtering" do
  test "excludes purchases not in COUNTS_TOWARDS_INVENTORY_STATES" do
  ...
  test "includes preorder_authorization_successful, in_progress, and not_charged states" do
  ...
  context_ "flag-based filtering" do
  ...
  context_ "subscription handling" do
  ...
  test "counts a gift receiver purchase on a subscription" do
          sub = create(:subscription, link: membership)
  ...
  test "does not count recurring (non-original, non-gift) subscription charges" do
          sub = create(:subscription, link: membership)
  ...
  test "does not count subscription purchases when the subscription is deactivated" do
  ...
  test "respects start_link_id, leaving earlier links untouched" do
  ...
  test "respects end_link_id, leaving later links untouched" do
          create_purchase_for(product_a)
          create_purchase_for(product_b)
          create_purchase_for(product_c)
  ...
  test "respects start_base_variant_id and end_base_variant_id" do
  ...
  test "supports running disjoint ranges in parallel without overlap" do
          create_purchase_for(product_a)
          create_purchase_for(product_b)
          create_purchase_for(product_c)
          reset_link_caches
  ...
  context_ "aggregation across multiple purchases" do
  test "groups by base_variant_id correctly when multiple variants share an id range" do
  ...
  context_ "batch_size parameter" do
  test "produces the same result with a small batch size as a large one" do
          create_historical_purchase(variant_attributes: [variant], quantity: 4)
