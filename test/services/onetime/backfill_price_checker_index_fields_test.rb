...
class OnetimeBackfillPriceCheckerIndexFieldsTest < ActiveSupport::TestCase
  self.described_class = Onetime::BackfillPriceCheckerIndexFields



  context_ Onetime::BackfillPriceCheckerIndexFields do
  context_ ".process" do
  ...
  test "scrolls Link.index_name and enqueues SendToElasticsearchWorker for each hit" do
  ...
  test "clears the scroll context when done" do
        described_class.process
