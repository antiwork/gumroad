...
class OfferCodeDiscountComputingServiceTest < ActiveSupport::TestCase
  self.described_class = OfferCodeDiscountComputingService



  context_ OfferCodeDiscountComputingService do
  ...
  test "returns invalid error_code in result when offer code is invalid" do
  ...
  test "does not return an invalid error_code in result when offer code amount is 0 cents" do
      result = OfferCodeDiscountComputingService.new(zero_cents_discount_code.code, products_data).process

      expect(result[:error_code]).to be_nil
    end

  test "does not return an invalid error_code in result when offer code amount is 0%" do
      result = OfferCodeDiscountComputingService.new(zero_percent_discount_code.code, products_data).process

      expect(result[:error_code]).to be_nil
    end

  test "returns sold_out error_code in result when offer code is sold out" do
  ...
  test "applies offer code on multiple products when offer code is universal" do
      result = OfferCodeDiscountComputingService.new(universal_offer_code.code, products_data).process
  ...
  test "rejects product with quantity greater than the offer code limit when offer code is universal" do
  ...
  test "applies offer code on single product in bundle when offer code is not universal" do
  ...
  test "includes the expiration date in the result" do
  ...
  test "includes the minimum quantity in the result" do
  ...
  test "includes the duration in the result" do
  ...
  test "rejects product with quantity greater than the offer code limit when offer code is not universal" do
  ...
  context_ "when offer code is not yet valid" do
      before do
        offer_code.update!(valid_at: 1.years.from_now)
      end

  test "returns inactive error code" do
        result = OfferCodeDiscountComputingService.new(offer_code.code, products_data).process
  ...
  context_ "when the user has multiple offer codes with the same name" do
  ...
  test "applies the correct offer code to each product based on product applicability" do
  ...
  context_ "when the code exists but doesn't apply to this product" do
      let!(:offer_code_for_product2) do
        create(
          :offer_code,
          user: seller,
  ...
  context_ "when offer code is expired" do
      before do
  ...
  context_ "when an offer code's minimum quantity is unmet" do
      before do
        offer_code.update!(minimum_quantity: 5)
      end

  test "returns insufficient quantity error code" do
        result = OfferCodeDiscountComputingService.new(offer_code.code, products_data).process
  ...
  context_ "when product has cross-sells" do
  ...
  context_ "universal offer code" do
  ...
  test "applies discount to main product and all applicable cross-sells" do
  ...
  context_ "product-specific offer code" do
  ...
  test "applies corresponding offer code to applicable products including cross-sells" do
          result = OfferCodeDiscountComputingService.new(shared_code_name, products_data).process
  ...
  context_ "existing-customer-only cross-sell offer code" do
  ...
  test "uses the buyer-resolved tier discount for applicable cross-sells" do
  ...
  test "skips cross-sell discounts when the buyer does not qualify" do
          create(
            :offer_code,
            user: seller,
            code: shared_code_name,
  ...
  context_ "existing-customer-only offer codes" do
  ...
  test "rejects redemption with :not_existing_customer when the buyer has no qualifying purchase" do
  ...
  test "rejects redemption with :not_existing_customer when buyer is nil" do
        result = OfferCodeDiscountComputingService.new(offer_code.code, products_data, buyer: nil).process
        expect(result[:error_code]).to eq(:not_existing_customer)
      end

  test "applies the discount when the buyer owns a required product" do
  ...
  test "resolves to the matching tier percentage when the code is tiered" do
        offer_code.update!(
          amount_percentage: 0,
          ownership_duration_tiers: [
