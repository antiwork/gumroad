...
class ProductBulkUpdateSupportEmailServiceTest < ActiveSupport::TestCase
  self.described_class = Product::BulkUpdateSupportEmailService



  context_ Product::BulkUpdateSupportEmailService do
  ...
  context_ "when user does not have product_level_support_emails enabled" do
        before do
          Feature.deactivate_user(:product_level_support_emails, user)
        end

  test "does not update any product support emails" do
