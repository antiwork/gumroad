...
class ProductSaveIntegrationsServiceTest < ActiveSupport::TestCase
  self.described_class = Product::SaveIntegrationsService



  context_ Product::SaveIntegrationsService do
  ...
  test "calls disconnect if integration is removed" do
          product.active_integrations << create("#{integration_name}_integration".to_sym)
  ...
  test "does not call disconnect if integration is removed but the same integration is present on another product by same user" do
          ...
          if integration_1.same_connection?(integration_2)
            expect_any_instance_of(get_integration_class(integration_name)).not_to receive(:disconnect!)
          end
          expect do
            described_class.perform(product, {})
          end.to change { product.active_integrations.count }.by(-1)
  ...
  test "removes bot from server if server id is valid" do
