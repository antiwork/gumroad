...
class ProductSavePostPurchaseCustomFieldsServiceTest < ActiveSupport::TestCase
  self.described_class = Product::SavePostPurchaseCustomFieldsService



  context_ Product::SavePostPurchaseCustomFieldsService do
  context_ "#perform" do
  ...
  test "syncs post-purchase custom fields with rich content nodes" do
        expect do
  ...
  context_ "product has different content for each variant" do
  ...
  test "syncs post-purchase custom fields with rich content nodes for all variants" do
          expect do
            described_class.new(product.reload).perform
          end.to change { CustomField.exists?(short_answer.id) }.from(true).to(false)
