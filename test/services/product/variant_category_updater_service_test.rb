...
class ProductVariantCategoryUpdaterServiceTest < ActiveSupport::TestCase
  self.described_class = Product::VariantCategoryUpdaterService



  context_ Product::VariantCategoryUpdaterService do
  context_ ".perform" do
  context_ "when trying to update a variant from another product" do
  ...
  test "raises an error and does not update the variant" do
  ...
  context_ "notifying subscribers of price changes" do
        let(:product) { create(:product) }
  ...
  context_ "when apply_price_changes_to_existing_memberships is enabled and the effective date changed" do
          let(:apply_price_changes_to_existing_memberships) { true }
          let(:subscription_price_change_effective_date) { 7.days.from_now.to_date }

  test "schedules ScheduleMembershipPriceUpdatesJob" do
  ...
  context_ "when disabling apply_price_changes_to_existing_memberships" do
  ...
  test "does not schedule ScheduleMembershipPriceUpdatesJob" do
            expect(ScheduleMembershipPriceUpdatesJob).not_to receive(:perform_async).with(variant.id)
          end

  context_ "when the flags previously changed" do
  test "does not notify error tracker" do
  ...
  context_ "when the effective date previously changed" do
            let(:was_applying_price_changes_to_existing_memberships) { false }
            let(:apply_price_changes_to_existing_memberships) { false }
  ...
  context_ "when apply_price_changes_to_existing_memberships is enabled but effective date did not change" do
          let(:was_applying_price_changes_to_existing_memberships) { false }
          let(:apply_price_changes_to_existing_memberships) { true }
          let(:subscription_price_change_effective_date) { was_subscription_price_change_effective_date }
  ...
  context_ "when changing the effective date and apply_price_changes_to_existing_memberships is already enabled" do
          let(:was_applying_price_changes_to_existing_memberships) { true }
          let(:apply_price_changes_to_existing_memberships) { true }
          let(:subscription_price_change_effective_date) { 7.days.from_now.to_date }
  ...
  context_ "when associating files" do
        before do
  ...
  test "saves new files on versions" do
  ...
  context_ "when versions are removed" do
  test "marks them as deleted and queues a DeleteProductRichContentWorker" do
            freeze_time do
              expect do
                Product::VariantCategoryUpdaterService.new(
                  product: @product,
  ...
  context_ "with recurrence pricing" do
          before :each do
  ...
  test "saves variants with valid recurrence prices" do
            Product::VariantCategoryUpdaterService.new(
              product: @product,
              category_params: @variant_category_params
            ).perform
  ...
  test "saves suggested prices" do
              Product::VariantCategoryUpdaterService.new(
                product: @product,
                category_params: @variant_category_params
              ).perform
  ...
  context_ "without suggested prices" do
  test "succeeds" do
  ...
  context_ "missing all prices values" do
  ...
  context_ "with a suggested price that is below price_cents" do
  test "raises an error" do
  ...
  context_ "with invalid recurrences" do
  context_ "such as missing a price for the default recurrence" do
  test "raises an error" do
  ...
  context_ "such as specifying enabled: false for the default recurrence" do
  test "raises an error" do
  ...
  context_ "such as different recurrences for different variants" do
  test "raises an error" do
                category_params = @variant_category_params
  ...
  context_ "such as missing price for a recurrence" do
  test "raises an error" do
                category_params = @variant_category_params
  ...
  context_ "such as a price that is too high" do
  test "raises an error" do
                category_params = @variant_category_params
  ...
  context_ "such as a price that is too low" do
  test "raises an error" do
                category_params = @variant_category_params
  ...
  context_ "such as an invalid recurrence option" do
  test "raises an error" do
                category_params = @variant_category_params
  ...
  context_ "when tiers are removed" do
  test "marks them as deleted and queues a DeleteProductRichContentWorker" do
  ...
  test "processes content upsells for variant rich content" do
          new_description = [
            {
              "type" => "paragraph",
              "content" => [
