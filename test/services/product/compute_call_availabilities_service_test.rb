...
class ProductComputeCallAvailabilitiesServiceTest < ActiveSupport::TestCase
  self.described_class = Product::ComputeCallAvailabilitiesService
  self.rspec_metadata = { freeze_time: true }



  context_ Product::ComputeCallAvailabilitiesService, :freeze_time do
  ...
  context_ "when product is not a call product" do
  ...
  test "excludes availabilities from the past" do
  ...
  test "excludes availabilities that are within the minimum notice period" do
  ...
  test "excludes availabiltiies on days that exceed the maximum call limit for the seller's timezone" do
  ...
  test "excludes availabilities that are sold" do
  ...
  test "includes availabilities that are sold but no longer take up availabilities" do
      create_call_availability(start_time: 10.hours.from_now, end_time: 16.hours.from_now)
