...
class UpdateUserComplianceInfoTest < ActiveSupport::TestCase
  self.described_class = UpdateUserComplianceInfo



  context_ UpdateUserComplianceInfo do
  context_ "#process" do
      let(:user) { create(:user) }

  context_ "when individual_tax_id exceeds maximum length" do
  ...
  context_ "when business_tax_id exceeds maximum length" do
  test "returns an error without attempting RSA encryption" do
          oversized_tax_id = "1" * 201
  ...
  context_ "when ssn_last_four exceeds maximum length" do
  test "returns an error without attempting RSA encryption" do
          oversized_ssn = "1" * 201
  ...
  context_ "when individual_tax_id is valid but ssn_last_four exceeds maximum length" do
  test "returns an error before assigning either value" do
  ...
  context_ "when submitted compliance values match the current compliance info" do
        let!(:compliance_info) { create(:user_compliance_info, user:) }

  test "returns success without creating a new compliance info row or submitting it to Stripe" do
  ...
  context_ "when submitted compliance values change the current compliance info" do
        let!(:compliance_info) { create(:user_compliance_info, user:) }

  test "creates a new compliance info row and submits it to Stripe" do
  ...
  context_ "with a US business that already has a 9-digit business_tax_id saved" do
        let(:us_business_user) do
          create(:user).tap { |u| create(:user_compliance_info_business, user: u) }
        end

  test "accepts a non-tax-id field update without re-submitting business_tax_id" do
          params = ActionController::Parameters.new(
            is_business: true,
  ...
  context_ "with a non-US business" do
