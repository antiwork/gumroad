...
class PayoutUsersServiceTest < ActiveSupport::TestCase
  self.described_class = PayoutUsersService
  self.rspec_metadata = { vcr: true }



  context_ PayoutUsersService, :vcr do
  ...
  test "marks the balances as processing, alters the users' balances, and creates payments" do
  ...
  test "works even if the supplied `user_ids` argument is not an array" do
        service_object = described_class.new(date_string: payout_date.to_s, processor_type: payout_processor_type,
                                             user_ids: user1.id)
        expect do
          result = service_object.process
  ...
  test "processes payments for all users even if there are exceptions in the process" do
  ...
  context_ "PayoutUsersService#create_payments" do
      before do
  ...
  test "returns array of payments and cross-border payments" do
  ...
  context_ "when the processor_type is 'STRIPE'" do
  ...
  test "does not process cross-border payouts immediately and schedules for 25 hours later", :freeze_time do
  ...
  test "marks the balances as processing, alters the users' balances, and creates payments when payout method is instant" do
  ...
  test "marks the payments as failed if the instant payout amount is more than the limit" do
        allow(StripePayoutProcessor).to receive(:instantly_payable_amount_cents_on_stripe).with(anything).and_return(99_999_00)
  ...
  test "does not process the payments if the amount is less than the instantly payable unpaid balances amount" do
  ...
  context_ "when the processor_type is 'PAYPAL'" do
  ...
  test "processes cross-border payouts immediately" do
