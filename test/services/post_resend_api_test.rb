...
class PostResendApiTest < ActiveSupport::TestCase
  self.described_class = PostResendApi
  self.rspec_metadata = { freeze_time: true }



  context_ PostResendApi, :freeze_time do
  ...
  context_ "preheader text" do
      before { send_default_email }

  test "is included at the start of the email" do
        expect(body_plaintext(sent_email_content)).to start_with("post body")
      end
    end

  context_ "call to action button" do
      before do
        @post.update!(call_to_action_url: "https://cta.example/", call_to_action_text: "Click here")
        send_default_email
      end

  test "is included when CTA url and CTA text are set" do
  ...
  test "is not included when there are no attachments" do
        send_default_email
        expect(sent_email_content).not_to include("View content")
      end

  test "is included when there are attachments" do
  ...
  test "raises an error when there are too many recipients" do
  ...
  test "raises an error when a recipient has no email" do
        expect do
          send_emails(recipients: [{ email: "" }])
        end.to raise_error(/must have an email/i)
      end

  test "raises an error when a post has files but no url_redirect" do
        @post.product_files << create(:product_file)
        expect do
          send_default_email
  ...
  test "raises an error when recipients have inconsistent records" do
        expect do
  ...
  context_ "'Reply with a comment' button" do
  test "is not included when allow_comments=false" do
        send_default_email
        expect(sent_email_content).not_to include("Reply with a comment")
      end

  test "is not included when allow_comments=true and shown_on_profile=false" do
        @post.update!(allow_comments: true, shown_on_profile: false)
        send_default_email
        expect(sent_email_content).not_to include("Reply with a comment")
      end

  test "is included when allow_comments=true and shown_on_profile=true" do
  ...
  context_ "Update reason" do
  test "is generic when installment_type='seller'" do
  ...
  context_ "when installment_type='product' or 'variant'" do
        before do
          @product = create(:product, user: @seller)
          @post.update!(installment_type: Installment::PRODUCT_TYPE, link: @product)
        end

  test "mentions when it's due to the subscription being cancelled" do
  ...
  test "mentions when it's due to it being an active subscription" do
          @product.update!(is_recurring_billing: true)
          send_default_email
  ...
  test "mentions when it's due to it being a purchase" do
          send_default_email
  ...
  context_ "Unsubscribe link" do
  test "links to purchase unsubscribe page when coming with a purchase" do
  ...
  test "links to unfollow page when coming with a follower" do
  ...
  test "links to affiliate unsubscribe page when coming with an affiliate" do
  ...
  test "sends the correct text / subject / reply-to" do
  ...
  test "creates EmailInfo record" do
      send_emails(recipients: [
  ...
  test "records the email events" do
  ...
  test "sends push notifications" do
  ...
  test "updates delivery statistics" do
  ...
  context_ "when preview email" do
      before do
  ...
  test "uses Resend::Emails.send for a single recipient" do
  ...
  test "uses Resend::Batch.send for multiple recipients" do
        response = double(success?: true)
  ...
  test "includes a link to Gumroad" do
      send_default_email
  ...
  context_ "Cache" do
  test "prevents the template from being rendered several times for the same post, across multiple calls" do
        cache = {}
