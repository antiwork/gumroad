...
class AdminSearchServiceTest < ActiveSupport::TestCase
  self.described_class = AdminSearchService



  context_ AdminSearchService do
  context_ "#search_purchases" do
  test "returns no results when no search criteria are provided" do
        create(:purchase)
        purchases = AdminSearchService.new.search_purchases
        expect(purchases).to be_empty
      end

  test "applies a default limit of MAX_RESULTS" do
  ...
  test "returns no Purchases if query is invalid" do
        purchases = AdminSearchService.new.search_purchases(query: "invalidquery")

        expect(purchases.size).to eq(0)
      end

  test "returns purchases matching email, directly and through gifts" do
  ...
  test "returns purchase when searching by external_id" do
  ...
  test "returns purchase when searching by internal id" do
        purchase = create(:purchase)
        other_purchase = create(:purchase)
  ...
  context_ "email parameter" do
  test "filters purchases by email when provided" do
  ...
  test "can use email and query together to find specific purchase" do
          purchase = create(:free_purchase, email: "customer@example.com")
  ...
  test "returns empty when query matches but email doesn't" do
          purchase = create(:free_purchase, email: "customer@example.com")
  ...
  test "returns purchases when searching by email with empty card parameters" do
        email = "user@example.com"
  ...
  test "returns purchases for products created by a seller" do
  ...
  test "returns no purchases when creator email is not found" do
        create(:purchase)
        purchases = AdminSearchService.new.search_purchases(creator_email: "nonexistent@example.com")
        expect(purchases.size).to eq(0)
      end

  test "returns purchase associated with a license key" do
        purchase = create(:purchase)
  ...
  test "returns no purchases when license key is not found" do
        create(:purchase)
        purchases = AdminSearchService.new.search_purchases(license_key: "nonexistent-key")
        expect(purchases.size).to eq(0)
      end

  context_ "searching by card" do
  ...
  context_ "when transaction_date value is not a date" do
  ...
  test "supports filtering by combination of params" do
  ...
  context_ "searching by ip_address" do
        let(:ip_v4) { "203.0.113.42" }
        let(:ip_v6) { "2001:db8::1" }
        let(:other_ip) { "198.51.100.10" }

  test "returns purchases matching ip_address" do
  ...
  context_ "product_title_query" do
  ...
  context_ "when query is set" do
  test "filters by product title" do
            purchases = AdminSearchService.new.search_purchases(query:, product_title_query:)
            expect(purchases).to eq [purchase]
          end
        end

  context_ "when query is not set" do
          let(:query) { nil }

  test "returns no results without search criteria" do
            purchases = AdminSearchService.new.search_purchases(query:, product_title_query:)
            expect(purchases).to be_empty
          end
        end
  ...
  context_ "when query is set" do
  test "filters by successful status" do
  ...
  test "filters by not_charged status" do
  ...
  test "filters by chargeback status (excluding reversed)" do
  ...
  test "filters by refunded status" do
