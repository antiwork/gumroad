...
class InstallmentSearchServiceTest < ActiveSupport::TestCase
  self.described_class = InstallmentSearchService



  context_ InstallmentSearchService do
  context_ "#process" do
  ...
  test "can exclude deleted posts" do
  ...
  test "filters by type to only include the specified posts" do
        installment_1 = create(:installment)
  ...
  test "can exclude workflow posts" do
  ...
  test "supports fulltext/autocomplete search" do
