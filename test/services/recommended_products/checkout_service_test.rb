...
class RecommendedProductsCheckoutServiceTest < ActiveSupport::TestCase
  self.described_class = RecommendedProducts::CheckoutService



  context_ RecommendedProducts::CheckoutService do
  ...
  context_ "#result" do
  test "passes user IDs for seller with recommendations turned on whose products are in the cart" do
  ...
  test "calls search_products with the correct arguments" do
  ...
  context_ "affiliate recommendations" do
  ...
  context_ "when at least one of the creators has direct affiliate recommendations turned on" do
          before do
            seller1.update!(recommendation_type: User::RecommendationType::DIRECTLY_AFFILIATED_PRODUCTS)
          end

  test "returns the user's products and products that are directly affiliated and recommendable" do
            expect(RecommendedProductsService).to receive(:fetch).with(
              model: RecommendedProductsService::MODEL_SALES,
              ids: [product1.id, product3.id, product2.id],
              exclude_ids: [product1.id, product3.id, product2.id],
  ...
  context_ "when at least one of the creators has Gumroad affiliate recommendations turned on" do
          before do
            seller1.update!(recommendation_type: User::RecommendationType::GUMROAD_AFFILIATES_PRODUCTS)
          end

  test "returns the user's products and products that are recommendable" do
            expect(RecommendedProductsService).to receive(:fetch).with(
              model: RecommendedProductsService::MODEL_SALES,
              ids: [product1.id, product3.id, product2.id],
              exclude_ids: [product1.id, product3.id, product2.id],
  ...
  context_ "when there are NSFW products in the cart" do
          let(:cart_product) do
            create(
              :product,
  ...
  test "includes that product in the results" do
            product_infos = described_class.fetch_for_cart(
              purchaser:,
  ...
  context_ "when there aren't NSFW products in the cart" do
          let(:cart_product) do
            create(
              :product,
              user: create(:user, recommendation_type: User::RecommendationType::GUMROAD_AFFILIATES_PRODUCTS)
  ...
  context_ "when providing affiliate recommendations" do
  test "excludes the product from the results" do
              product_infos = described_class.fetch_for_cart(
                purchaser:,
                cart_product_ids: [cart_product.id],
                recommender_model_name:,
  ...
  context_ "when not providing affiliate recommendations" do
  ...
  context_ "when the product is not visible" do
  ...
  test "doesn't include that product in the results" do
  ...
  context_ "when the recommendation model returns fewer than the limit" do
  ...
  test "fills out the results with the users' products" do
              expect(RecommendedProductsService).to receive(:fetch).with(
                {
                  model: RecommendedProductsService::MODEL_SALES,
  ...
  context_ "when a bundle is one of the associated products" do
            let!(:bundle) { create(:product, :bundle) }

  test "excludes the bundle's products from the results" do
              expect(RecommendedProductsService).to receive(:fetch).with(
