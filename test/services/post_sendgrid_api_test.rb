...
class PostSendgridApiTest < ActiveSupport::TestCase
  self.described_class = PostSendgridApi
  self.rspec_metadata = { freeze_time: true }



  context_ PostSendgridApi, :freeze_time do
  ...
  context_ "Update reason" do # little text in the footer indicating why the recipient got this email
  ...
  test "mentions when it's due to the subscription being cancelled" do # only workflows
  ...
  test "sets the correct custom_args" do
      purchase = create(:purchase, :from_seller, seller: @seller)
      follower = create(:follower, user: @seller)
      affiliate = create(:direct_affiliate, seller: @seller)
