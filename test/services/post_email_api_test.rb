...
class PostEmailApiTest < ActiveSupport::TestCase
  self.described_class = PostEmailApi



  context_ PostEmailApi do
  ...
  context_ ".process" do
  context_ "when the feature flag is active" do
        before do
  ...
  test "splits recipients between Resend and SendGrid" do
  ...
  test "routes non-ASCII emails through SendGrid" do
  ...
  test "routes emails with local parts exceeding 64 characters through SendGrid" do
          # Set to resend to test the fallback
          allow(MailerInfo::Router).to receive(:determine_email_provider).and_return(MailerInfo::EMAIL_PROVIDER_RESEND)
  ...
  test "routes emails with special characters through SendGrid" do
          # Set to resend to test the fallback
          allow(MailerInfo::Router).to receive(:determine_email_provider).and_return(MailerInfo::EMAIL_PROVIDER_RESEND)
  ...
  test "routes emails with formatting issues through SendGrid" do
          # Set to resend to test the fallback
          allow(MailerInfo::Router).to receive(:determine_email_provider).and_return(MailerInfo::EMAIL_PROVIDER_RESEND)
  ...
  test "routes emails from excluded domains through SendGrid" do
          # Set to resend to test the fallback
          allow(MailerInfo::Router).to receive(:determine_email_provider).and_return(MailerInfo::EMAIL_PROVIDER_RESEND)
  ...
  test "routes emails exceeding maximum length through SendGrid" do
          # Set to resend to test the fallback
          allow(MailerInfo::Router).to receive(:determine_email_provider).and_return(MailerInfo::EMAIL_PROVIDER_RESEND)
  ...
  test "routes valid emails through Resend when determined by the router" do
  ...
  test "routes valid emails through SendGrid when determined by the router" do
          # Valid email with only permitted special characters
  ...
  context_ ".max_recipients" do
  context_ "when the feature flag is active" do
  test "returns the Resend max recipients" do
  ...
  context_ "when the feature flag is inactive" do
  test "returns the SendGrid max recipients" do
