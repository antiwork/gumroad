...
class AdminScheduledPayoutEnrichmentServiceTest < ActiveSupport::TestCase
  self.described_class = Admin::ScheduledPayoutEnrichmentService



  context_ Admin::ScheduledPayoutEnrichmentService do
  context_ "#call" do
      let(:merchant_account) { create(:merchant_account, user: nil) }

  test "returns an empty hash for an empty input" do
        expect(described_class.new([]).call).to eq({})
      end

  test "counts alive products only" do
  ...
  test "counts alive direct affiliates and collaborators granted by the seller" do
        seller = create(:compliant_user)
  ...
  test "sums unpaid balances only and formats the total" do
        seller = create(:compliant_user)
  ...
  test "formats zero unpaid balance" do
        seller = create(:compliant_user)
        scheduled_payout = create(:scheduled_payout, user: seller)
  ...
  test "returns the top three categories by product count with a stable tie-breaker" do
        seller = create(:compliant_user)
  ...
  test "excludes products without taxonomies from top categories" do
        seller = create(:compliant_user)
  ...
  test "returns risk state from the shared presenter" do
        users = [
  ...
  test "uses a bounded number of queries for distinct users" do
