...
class HandleEmailEventInfoForInstallmentEmailTest < ActiveSupport::TestCase
  self.described_class = HandleEmailEventInfo::ForInstallmentEmail


  context_ HandleEmailEventInfo::ForInstallmentEmail do
    before do
  ...
  context_ ".perform" do
  test "creates a new CreatorEmailOpenEvent object" do
        now = Time.current
  ...
  test "sets cache for open event" do
        params = { "_json" => [{ "event" => "open", "type" => "CreatorContactingCustomersMailer.purchase_installment",
                                 "identifier" => @identifier, "installment_id" => @installment.id }] }
  ...
  test "creates a new CreatorEmailOpenEvent object and then update it if there are 2 identical open events" do
        now = Time.current
        params = { "_json" => [{ "event" => "open", "type" => "CreatorContactingCustomersMailer.purchase_installment",
                                 "identifier" => @identifier, "installment_id" => @installment.id }] }
        travel_to(now) do
  ...
  test "creates a new CreatorEmailClickSummary object and a new CreatorEmailClickEvent object" do
        now = Time.current
  ...
  test "sets cache on click event" do
        params = { "_json" => [{ "event" => "click", "type" => "CreatorContactingCustomersMailer.purchase_installment",
                                 "identifier" => @identifier, "installment_id" => @installment.id, "url" => "https://www&#46;gumroad&#46;com" }] }
  ...
  test "creates 1 new CreatorEmailClickSummary and 2 CreatorEmailClickEvent objects for different URLs, \
  ...
  test "does not modify CreatorEmailClickSummary if it sees two duplicate events, \
  ...
  test "registers two unique clicks for two different users clicking the same url for the same installment" do
        now = Time.current
        params = { "_json" => [{ "event" => "click", "type" => "CreatorContactingCustomersMailer.purchase_installment",
                                 "identifier" => @identifier, "installment_id" => @installment.id, "url" => "https://www&#46;gumroad&#46;com" }] }
        travel_to(now) do
  ...
  test "handles the second event in the params array even if the first one is malformed" do
        now = Time.current
  ...
  test "creates a corresponding open event if a click event is logged but \
  ...
  test "does not create a corresponding open event if a click event is logged if an open event already exists" do
        now = Time.current
        params = { "_json" => [{ "event" => "open", "type" => "CreatorContactingCustomersMailer.purchase_installment",
                                 "identifier" => @identifier, "installment_id" => @installment.id },
                               { "event" => "click", "type" => "CreatorContactingCustomersMailer.purchase_installment",
  ...
  test "replaces the url for an attachment link with 'Attached Files'" do
        now = Time.current
        params = { "_json" => [{
          "event" => "click",
  ...
  test "does not create an event for unsubscribes" do
        now = Time.current
        params = { "_json" => [{ "event" => "click", "type" => "CreatorContactingCustomersMailer.purchase_installment",
                                 "identifier" => @identifier, "installment_id" => @installment.id,
  ...
  context_ "cancel follower" do
        before do
          @non_existent_purchase_id = 999_999_999
        end

  test "cancels the follower on bounce event" do
  ...
  test "cancels the follower on spamreport event" do
          follower = create(:active_follower, email: "test@example.com", followed_id: @installment.seller_id)
  ...
  context_ "email info" do
  context_ "purchase installment" do
  context_ "existing email info" do
            before do
              @email_info = create(:creator_contacting_customers_email_info, installment: @installment, purchase: @purchase)
            end

  test "marks it as bounced and cancel the follower" do
  ...
  test "unsubscribes the buyer of the purchase and cancels the follower when the event type is 'spamreport'" do
              follower = create(:active_follower, email: @purchase.email, followed_id: @purchase.seller_id)
  ...
  test "does not unsubscribe the buyer when the event type is 'spamreport' from Resend" do
              follower = create(:active_follower, email: @purchase.email, followed_id: @purchase.seller_id)
              another_product = create(:product, user: @purchase.seller)
              another_purchase = create(:purchase, link: another_product, email: @purchase.email)
  ...
  context_ "creating new email info" do
  test "creates a new email info and mark it as bounced" do
  ...
  test "creates a new email info and mark it as delivered" do
              expect(CreatorContactingCustomersEmailInfo.count).to eq 0
              params = { "_json" => [{ "event" => "delivered", "type" => "CreatorContactingCustomersMailer.purchase_installment",
                                       "identifier" => @identifier, "installment_id" => @installment.id, "timestamp" => 1.day.ago.to_i }] }
              travel_to(Time.current) do
  ...
  test "creates a new email info and mark it as opened" do
              expect(CreatorContactingCustomersEmailInfo.count).to eq 0
              params = { "_json" => [{ "event" => "open", "type" => "CreatorContactingCustomersMailer.purchase_installment",
                                       "identifier" => @identifier, "installment_id" => @installment.id, "timestamp" => 1.hour.ago.to_i }] }
              travel_to(Time.current) do
  ...
  test "unsubscribes the buyer of the purchase when the event type is 'spamreport'" do
              another_product = create(:product, user: @purchase.seller)
              another_purchase = create(:purchase, link: another_product, email: @purchase.email)
  ...
  context_ "subscription installment" do
          before do
  ...
  context_ "follower installment" do
  test "does not create a new email info" do
            expect(CreatorContactingCustomersEmailInfo.count).to eq 0
