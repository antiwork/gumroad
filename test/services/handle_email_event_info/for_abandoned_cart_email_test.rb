...
class HandleEmailEventInfoForAbandonedCartEmailTest < ActiveSupport::TestCase
  self.described_class = HandleEmailEventInfo::ForAbandonedCartEmail


  context_ HandleEmailEventInfo::ForAbandonedCartEmail do
  ...
  context_ ".perform" do
      shared_examples "tracks a delivered event" do |email_provider|
  test "tracks a delivered event" do
          expect do
      ...
      shared_examples "handles opened events" do |email_provider|
  test "tracks an open event" do
          now = Time.current
  ...
  test "tracks an open event and update it if there are 2 identical open events" do
          now = Time.current
          travel_to(now) do
            handler_class_for(email_provider).new.perform(params)
          end
      ...
      shared_examples "handles click events" do |email_provider|
  test "tracks a click event with email click summary" do
          now = Time.current
  ...
  test "tracks multiple click events and only one email click summary record for different URLs for an installment" do
          now = Time.current
          travel_to(now) do
            handler_class_for(email_provider).new.perform(params1)
          end
  ...
  test "records a single email click summary for duplicate click events and updates the timestamps for the tracked click event" do
          now = Time.current
          travel_to(now) do
            handler_class_for(email_provider).new.perform(params1)
          end
      ...
      shared_examples "records an open event while tracking a click event when a corresponding open event does not exist yet" do |email_provider|
  test "records an open event while tracking a click event when a corresponding open event does not exist yet" do
          now = Time.current
          travel_to(now) do
            handler_class_for(email_provider).new.perform(params)
          end
  ...
  context_ "with delivered event" do
  ...
  context_ "with clicked event" do
  ...
  context_ "with mailer_args with single workflow_id" do
            before do
  ...
  test "does not record an open event while recording a click event when a corresponding open event already exists" do
          now = Time.current
          params = {
            "_json" => [
