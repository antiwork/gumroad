...
class HandleEmailEventInfoForReceiptEmailTest < ActiveSupport::TestCase
  self.described_class = HandleEmailEventInfo::ForReceiptEmail


  context_ HandleEmailEventInfo::ForReceiptEmail do
  ...
  context_ ".perform" do
      shared_examples "handles bounced event type" do |email_provider|
  context_ "when CustomerEmailInfo doesn't exist" do
  ...
  context_ "when CustomerEmailInfo exists" do
          let!(:email_info) { create(:customer_email_info, email_name: mailer_method, purchase:) }

  test "marks it as bounced and deletes the follower" do
            travel_to(Time.current) do
              handler_class_for(email_provider).new.perform(params)
            end
      ...
      shared_examples "handles delivered event type" do |email_provider|
  context_ "when CustomerEmailInfo doesn't exist" do
  ...
  test "marks it as delivered" do
      ...
      shared_examples "handles open event type" do |email_provider|
  context_ "when CustomerEmailInfo doesn't exist" do
  ...
  test "marks it as opened" do
      ...
      shared_examples "handles spamreport event type" do |email_provider|
  context_ "when CustomerEmailInfo doesn't exist" do
  test "unsubscribes the buyer of the purchase" do
  ...
  test "unsubscribes the buyer of the purchase and cancels the follower" do
            follower&.destroy
  ...
  context_ "#{mailer_method} method" do
  ...
  context_ "with type and identifier unique args" do
              let(:params) do
                {
                  "_json" => [{
                    "event" => event_type,
  ...
  context_ "with spamreport event" do
  ...
  context_ "with records as individual unique args" do
              let(:params) do
                {
                  "_json" => [{
                    "event" => event_type,
  ...
  context_ "receipt - for a Charge", :vcr do
        let(:purchase) { create(:purchase) }
  ...
  context_ "when CustomerEmailInfo doesn't exist" do
  test "creates a new email info and marks it as bounced" do
              travel_to(Time.current) do
