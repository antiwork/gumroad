...
class PdfStampingServiceStampTest < ActiveSupport::TestCase
  self.described_class = PdfStampingService::Stamp



  context_ PdfStampingService::Stamp do
  context_ ".can_stamp_file?" do
  context_ "with readable PDF" do
  ...
  test "stamps the PDF without errors" do
  ...
  test "stamps only the first page of the PDF" do
  ...
  context_ "when applying the watermark fails" do
  context_ "when the PDF is encrypted" do
          let(:pdf_url) { "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/specs/encrypted_pdf.pdf" }

  test "logs and raises PdfStampingService::Stamp::Error" do
            expect(Rails.logger).to receive(:error).with(
              /\[PdfStampingService::Stamp.apply_watermark!\] Failed to execute command: pdftk/
            )
            expect(Rails.logger).to receive(:error).with(/\[PdfStampingService::Stamp.apply_watermark!\] STDOUT: /)
  ...
  context_ "when pdftk command fails" do
          before do
