...
class GdprBuyerErasureServiceTest < ActiveSupport::TestCase
  self.described_class = GdprBuyerErasureService



  context_ GdprBuyerErasureService do
  ...
  context_ "with guest buyer data" do
        let!(:purchase1) do
  ...
  test "anonymizes all PII columns on matching events rows" do
          event = Event.new(
            email: buyer_email,
            ip_address: "9.9.9.9",
            ip_country: "US",
  ...
  context_ "charges with shared purchases across buyers" do
  ...
  context_ "second erasure after a fresh purchase under the same email" do
