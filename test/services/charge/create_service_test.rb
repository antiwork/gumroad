...
class ChargeCreateServiceTest < ActiveSupport::TestCase
  self.described_class = Charge::CreateService
  self.rspec_metadata = { vcr: true }

  # frozen_string_literal: false

  context_ Charge::CreateService, :vcr do
  ...
  context_ "#perform" do
  test "creates a charge and associates the purchases with it" do
  ...
  test "handles charge processor error and adds corresponding error on each purchase" do
        order, _ = Order::CreateService.new(params:).perform
        merchant_account = create(:merchant_account_stripe, user: seller_1)
