...
class PushNotificationServiceAndroidTest < ActiveSupport::TestCase
  self.described_class = PushNotificationService::Android



  context_ PushNotificationService::Android do
    before do
      Feature.activate(:send_notifications_to_android_devices)
    end

  context_ "consumer app" do
  context_ "when notification sound is passed" do
  test "creates a FCM notification with sound" do
          app = double("fcm_app")
  ...
  context_ "when notification sound is not passed" do
  test "creates a FCM notification without sound" do
          app = double("fcm_app")
          allow(PushNotificationService::Android).to receive(:consumer_app).and_return(app)
