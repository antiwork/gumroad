...
class CustomDomainVerificationServiceTest < ActiveSupport::TestCase
  self.described_class = CustomDomainVerificationService



  context_ CustomDomainVerificationService do
    let(:domain) { "example.com" }
    subject(:service) { described_class.new(domain:) }

  context_ "#process" do
  context_ "domain CNAME configuration" do
        let(:domain) { "store.example.com" }

  context_ "when there exists a CNAME record pointing to the correct domain" do
          before do
  ...
  context_ "when there doesn't exist a CNAME record pointing to the correct domain" do
          before do
            allow_any_instance_of(Resolv::DNS)
              .to receive(:getresources)
              .with(domain, Resolv::DNS::Resource::IN::CNAME)
  ...
  context_ "domain ALIAS configuration" do
            before do
              allow_any_instance_of(Resolv::DNS)
                .to receive(:getresources)
  ...
  context_ "when the domain is pointed to CUSTOM_DOMAIN_CNAME using ALIAS records" do
              before do
                allow_any_instance_of(Resolv::DNS)
                  .to receive(:getresources)
  ...
  context_ "when the domain is pointed to CUSTOM_DOMAIN_STATIC_IP_HOST using ALIAS records" do
              before do
                allow_any_instance_of(Resolv::DNS)
                  .to receive(:getresources)
                  .with(domain, Resolv::DNS::Resource::IN::A)
  ...
  context_ "when the domain is not pointed to either CUSTOM_DOMAIN_CNAME or CUSTOM_DOMAIN_STATIC_IP_HOST using ALIAS records" do
              before do
                allow_any_instance_of(Resolv::DNS)
                  .to receive(:getresources)
                  .with(domain, Resolv::DNS::Resource::IN::A)
  ...
  context_ "#domains_pointed_to_gumroad" do
      before(:each) do
        allow_any_instance_of(Resolv::DNS)
          .to receive(:getresources)
  ...
  context_ "when it is a root domain" do
  test "returns the root domain and the one with www prefix" do
          expect(service.domains_pointed_to_gumroad).to eq ["example.com", "www.example.com"]
        end
      end

  context_ "when it's a domain with subdomain'" do
  ...
  context_ "#has_valid_ssl_certificates?" do
      before(:each) do
        allow_any_instance_of(Resolv::DNS)
          .to receive(:getresources)
          .with(anything, Resolv::DNS::Resource::IN::CNAME)
  ...
  test "returns true when valid SSL certificates are found for all associated domains" do
