...
class FollowerCreateServiceTest < ActiveSupport::TestCase
  self.described_class = Follower::CreateService

  # frozen_string_literal: false

  context_ Follower::CreateService do
  ...
  context_ "when follower is present" do
  test "updates the source of the follow and follower_user_id" do
  ...
  context_ "when follower is cancelled" do
  test "uncancels the follower" do
          Follower::CreateService.perform(
            followed_user: user,
            follower_email: deleted_follower.email
          )
  ...
  context_ "when follower exists in master DB but not in replica" do
  test "reactivates the existing follower" do
          deleted_follower
  ...
  context_ "when follower not confirmed nor cancelled" do
  test "sends the confirmation email again" do
            expect do
              Follower::CreateService.perform(
                followed_user: user,
  ...
  context_ "when imported from CSV" do
  test "auto-confirms and does not send the confirmation email" do
            expect do
  ...
  context_ "when follower is not logged in" do
  test "does not auto-confirm and sends the confirmation email" do
            expect do
              follower = Follower::CreateService.perform(
                followed_user: user,
                follower_email: deleted_follower.email
  ...
  test "sends the confirmation email even if follower is confirmed" do
            expect do
              Follower::CreateService.perform(
                followed_user: user,
  ...
  test "does not send the confirmation email if the new email is invalid" do
            expect do
              follower = Follower::CreateService.perform(
                followed_user: user,
                follower_email: active_follower.email,
  ...
  context_ "when follower is logged in" do
  test "auto-confirms if logged-in user has confirmed email" do
  ...
  test "sends the confirmation email if-logged in user has unconfirmed email" do
  ...
  context_ "when follower is not present" do
      it do
        expect do
          Follower::CreateService.perform(
            followed_user: user,
  ...
  test "does not send the confirmation email if the email is invalid" do
        expect do
          follower = Follower::CreateService.perform(
            followed_user: user,
