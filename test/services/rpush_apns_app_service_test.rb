...
class RpushApnsAppServiceTest < ActiveSupport::TestCase
  self.described_class = RpushApnsAppService



  context_ RpushApnsAppService do
  ...
  test "returns the record" do
          ...
          end.not_to change { Rpush::Apns2::App.where(name: app_name).size }
        end
      end
  ...
  test "creates an app with environment production when the Rails environment is staging and returns it" do
  ...
  test "creates an app with environment production when the Rails environment is production and returns it" do
  ...
  context_ "#creator_app?" do
  test "returns correct app type" do
