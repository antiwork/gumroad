...
class DisputeEvidenceGenerateUncategorizedTextServiceTest < ActiveSupport::TestCase
  self.described_class = DisputeEvidence::GenerateUncategorizedTextService
  self.rspec_metadata = { vcr: true }



  context_ DisputeEvidence::GenerateUncategorizedTextService, :vcr do
    let(:product) do
      create(
        :physical_product,
  ...
  context_ ".perform" do
  test "returns customer location, billing postal code, and previous purchases information" do
  ...
  context_ "when the other purchase has a different fingerprint" do
        before do
          other_undisputed_purchase.update!(stripe_fingerprint: "other_fintgerprint")
        end

  test "does not include previous purchases information" do
          expected_uncategorized_text = <<~TEXT.strip_heredoc.rstrip
            Device location: California, United States
            Billing postal code: 12345
          TEXT
