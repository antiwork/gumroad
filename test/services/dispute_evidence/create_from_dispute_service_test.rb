...
class DisputeEvidenceCreateFromDisputeServiceTest < ActiveSupport::TestCase
  self.described_class = DisputeEvidence::CreateFromDisputeService
  self.rspec_metadata = { vcr: true }



  context_ DisputeEvidence::CreateFromDisputeService, :vcr, :versioning do
    let(:product) do
  ...
  context_ "when dispute is on a combined charge" do
      let(:purchase) do
        create(
          :purchase,
  ...
  test "attaches refund policy image" do
        dispute_evidence = DisputeEvidence.create_from_dispute!(disputed_purchase.dispute)

        expect(dispute_evidence.refund_policy_image).to be_attached
      end

  context_ "when the refund policy image is too big" do
        before do
  ...
  test "doesn't attach refund policy image" do
          dispute_evidence = DisputeEvidence.create_from_dispute!(disputed_purchase.dispute)
  ...
  context_ "when the refund policy image generation raises an unexpected error" do
        let(:js_error) do
          StandardError.new("javascript error: Cannot read properties of null (reading 'parentElement')")
        end
  ...
  test "still creates the dispute evidence without the refund policy image" do
  ...
  context_ "when there is a view event before the purchase" do
        let!(:event) do
  ...
  test "generates refund policy disclosure" do
          dispute_evidence = DisputeEvidence.create_from_dispute!(disputed_purchase.dispute)
  ...
  context_ "#find_refund_policy_fine_print_view_events" do
  ...
  context_ "when there are no events" do
  test "returns empty array" do
          expect(events.count).to eq(0)
        end
      end

  context_ "when there is one eligible event" do
        let!(:event) do
          disputed_purchase.events.create!(
            event_name: Event::NAME_PRODUCT_REFUND_POLICY_FINE_PRINT_VIEW,
            link_id: disputed_purchase.link_id,
  ...
  test "returns an array with the event" do
          expect(events).to eq([event])
        end
      end

  context_ "when the event belongs to a different product" do
  ...
  test "doesn't include the event" do
          expect(events.count).to eq(0)
        end
      end

  context_ "when the event has a different browser guid" do
        let!(:event) do
          disputed_purchase.events.create!(
            event_name: Event::NAME_PRODUCT_REFUND_POLICY_FINE_PRINT_VIEW,
            link_id: disputed_purchase.link_id,
  ...
  context_ "when the view event is done after the purchase" do
        let!(:event) do
          disputed_purchase.events.create!(
            event_name: Event::NAME_PRODUCT_REFUND_POLICY_FINE_PRINT_VIEW,
            link_id: disputed_purchase.link_id,
  ...
  context_ "#mobile_purchase?" do
      let(:service_instance) { described_class.new(disputed_purchase.dispute) }

  context_ "when purchase.is_mobile is false" do
  ...
  context_ "when purchase.is_mobile is empty" do
  ...
  context_ "when purchase.is_mobile is 1" do
