...
class RobotsServiceTest < ActiveSupport::TestCase
  self.described_class = RobotsService



  context_ RobotsService do
    before do
  ...
  test "generates sitemap configs" do
  ...
  test "doesn't generate sitemaps configs when cache exists" do
  ...
  context_ "#user_agent_rules" do
  test "returns the user agent rules" do
        expect(described_class.new.user_agent_rules).to eq @user_agent_rules
      end
    end

  context_ "#expire_sitemap_configs_cache" do
      before do
        @redis_namespace.set("sitemap_configs", @sitemap_config)
      end

  test "expires sitemap_configs cache" do
