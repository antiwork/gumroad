...
class IntegrationsCircleIntegrationServiceTest < ActiveSupport::TestCase
  self.described_class = Integrations::CircleIntegrationService



  context_ Integrations::CircleIntegrationService, :without_circle_rate_limit do
  ...
  test "adds member to the community specified in the integration" do
  ...
  test "does nothing if integration does not exist" do
          expect_any_instance_of(CircleApi).not_to receive(:add_member)
          Integrations::CircleIntegrationService.new.activate(@purchase_without_integration)
        end
      end

  context_ "#deactivate" do
  test "removes member from the community specified in the integration" do
  ...
  test "does nothing if integration does not exist" do
          expect_any_instance_of(CircleApi).not_to receive(:remove_member)
  ...
  context_ "#activate" do
  test "adds member to the community if variant has integration" do
          expect_any_instance_of(CircleApi).to receive(:add_member).with(community_id, space_group_id, email)
          Integrations::CircleIntegrationService.new.activate(@purchase_with_integration)
        end

  test "adds member to the community if product has integration and purchase does not have a variant specified" do
          expect_any_instance_of(CircleApi).to receive(:add_member).with(community_id, space_group_id, email)
          Integrations::CircleIntegrationService.new.activate(@purchase_without_variant)
        end

  test "does nothing if variant does not have integration" do
          expect_any_instance_of(CircleApi).not_to receive(:add_member)
          Integrations::CircleIntegrationService.new.activate(@purchase_without_integration)
        end
      end
  ...
  context_ "#deactivate" do
  test "removes member from the community if variant has integration" do
          expect_any_instance_of(CircleApi).to receive(:remove_member).with(community_id, email)
  ...
  test "removes member from the community if product has integration and purchase does not have a variant specified" do
          expect_any_instance_of(CircleApi).to receive(:remove_member).with(community_id, email)
  ...
  context_ "#activate" do
  test "adds member to the community if tier has integration" do
          expect_any_instance_of(CircleApi).to receive(:add_member).with(community_id, space_group_id, email)
          Integrations::CircleIntegrationService.new.activate(@purchase_with_integration)
        end

  test "does nothing if tier does not have integration" do
          expect_any_instance_of(CircleApi).not_to receive(:add_member)
          Integrations::CircleIntegrationService.new.activate(@purchase_without_integration)
        end
      end
  ...
  context_ "#deactivate" do
  test "removes member from the community if tier has integration" do
          expect_any_instance_of(CircleApi).to receive(:remove_member).with(community_id, email)
          Integrations::CircleIntegrationService.new.deactivate(@purchase_with_integration)
        end
  ...
  test "activates integration if new tier has integration and old tier did not" do
