...
class StripeTaxFormsApiTest < ActiveSupport::TestCase
  self.described_class = StripeTaxFormsApi



  context_ StripeTaxFormsApi, vcr: { cassette_name: "StripeTaxFormsApi/tax_forms" } do
  ...
  context_ "#tax_forms_by_year" do
  test "returns tax forms grouped by year" do
  ...
  test "raises error for invalid form type" do
  ...
  test "returns empty hash on Stripe API error" do
  ...
  context_ "#download_tax_form" do
  test "downloads the tax form PDF for the specified year" do
  ...
  test "returns nil when tax form not found for the year" do
