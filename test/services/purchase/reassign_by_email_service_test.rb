...
class PurchaseReassignByEmailServiceTest < ActiveSupport::TestCase
  self.described_class = Purchase::ReassignByEmailService



  context_ Purchase::ReassignByEmailService do
  ...
  context_ "#perform" do
  context_ "when both emails are missing or blank" do
  test "returns reason :missing_params when from_email is blank" do
  ...
  test "returns reason :missing_params when to_email is blank" do
  ...
  context_ "when no purchases match from_email" do
  test "returns reason :not_found" do
  ...
  context_ "when from_email and to_email match" do
  test "returns reason :no_changes for an exact match" do
  ...
  test "rejects same email case-insensitively" do
          result = described_class.new(from_email: "User@Example.com", to_email: "user@example.com").perform
  ...
  test "does not query Purchase or enqueue a receipt when same emails are submitted" do
  ...
  context_ "when target user exists" do
  ...
  test "reassigns email and purchaser_id to the target user" do
          result = described_class.new(from_email:, to_email:).perform
  ...
  test "transfers subscription ownership to the target user for original subscription purchases" do
  ...
  test "does not modify subscription.user when the original-subscription purchase save fails" do
          subscription = create(:subscription, user: buyer)
          sub_purchase = create(:purchase, email: from_email, purchaser: buyer, is_original_subscription_purchase: true, subscription:, merchant_account:)
  ...
  test "transfers full ownership of an original_purchase that is not in the matched set" do
          subscription = create(:subscription, user: buyer)
  ...
  test "does not modify subscription.user when the original_purchase update fails" do
          subscription = create(:subscription, user: buyer)
          original_purchase = create(:purchase, email: "old_original@example.com", purchaser: buyer, is_original_subscription_purchase: true, subscription:, merchant_account:)
  ...
  test "enqueues a grouped receipt for all reassigned purchases" do
  ...
  context_ "when no purchases save successfully" do
        let!(:target_user) { create(:user, email: to_email) }
        let!(:purchase) { create(:purchase, email: from_email, purchaser: buyer, merchant_account:) }

  test "returns reason :no_changes and does not enqueue a grouped receipt" do
          allow_any_instance_of(Purchase).to receive(:save).and_return(false)
          expect(CustomerMailer).not_to receive(:grouped_receipt)
  ...
  context_ "when target user does not exist" do
        let!(:purchase) { create(:purchase, email: from_email, purchaser: buyer, merchant_account:) }

  test "still reassigns the email but sets purchaser_id to nil" do
  ...
  test "clears the subscription user for original subscription purchases" do
          subscription = create(:subscription, user: buyer)
          sub_purchase = create(:purchase, email: from_email, purchaser: buyer, is_original_subscription_purchase: true, subscription:, merchant_account:)
  ...
  context_ "when the to_email belongs only to a soft-deleted user" do
        let!(:deleted_user) { create(:user, email: to_email).tap(&:deactivate!) }
        let!(:purchase) { create(:purchase, email: from_email, purchaser: buyer, merchant_account:) }

  test "treats the email as having no target user and reassigns with purchaser_id nil" do
          result = described_class.new(from_email:, to_email:).perform
