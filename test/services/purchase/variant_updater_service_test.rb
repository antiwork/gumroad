...
class PurchaseVariantUpdaterServiceTest < ActiveSupport::TestCase
  self.described_class = Purchase::VariantUpdaterService



  context_ Purchase::VariantUpdaterService do
  context_ ".perform" do
  ...
  context_ "and the purchase has a variant for the category" do
  ...
  context_ "and the purchase doesn't have a variant for the category" do
  ...
  context_ "and there is insufficient inventory" do
  context_ "when switching variants" do
  ...
  context_ "when not switching variants" do
  test "returns an error" do
              purchase = create(
                :purchase,
                link: @product,
  ...
  context_ "gift sender purchase" do
  ...
  test "invokes the service on the gift receiver purchase" do
  ...
  context_ "when the product has SKUs" do
        before :each do
  ...
  context_ "and the purchase has a SKU" do
  ...
  context_ "and the purchase doesn't have a SKU" do
  test "adds the SKU" do
            purchase = create(
              :physical_purchase,
              link: @product
            )
  ...
  context_ "inventory counter caches" do
        before { @product = create(:product) }

  test "moves the cached sales count from the old variant to the new one when only the variant changes" do
  ...
  test "reconciles caches when the variant and quantity both change" do
          category = create(:variant_category, link: @product, title: "Color")
          blue = create(:variant, variant_category: category, name: "Blue")
          green = create(:variant, variant_category: category, name: "Green")
  ...
  context_ "with invalid arguments" do
  context_ "such as an invalid variant_id" do
  test "returns an error" do
  ...
  context_ "such as a variant that doesn't belong to the right product" do
  test "returns an error" do
            purchase = create(:purchase)
