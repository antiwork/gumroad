...
class PurchaseAssociateBundleProductLevelGiftServiceTest < ActiveSupport::TestCase
  self.described_class = Purchase::AssociateBundleProductLevelGiftService

  # frozen_string_literal: false

  context_ Purchase::AssociateBundleProductLevelGiftService do
  context_ "#perform" do
  ...
  context_ "when both product purchases exist" do
        before do
          sender_product_purchase
          receiver_product_purchase
        end

  test "creates a full Gift record" do
          expect do
  ...
  test "is indifferent to which bundle purchase instance is passed (sender or receiver)" do
  ...
  context_ "when bundle_product does not belong to the bundle" do
  ...
  context_ "when only one side's product purchase exists" do
  test "creates a Gift with the available side associated" do
