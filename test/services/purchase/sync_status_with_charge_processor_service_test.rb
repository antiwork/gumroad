...
class PurchaseSyncStatusWithChargeProcessorServiceTest < ActiveSupport::TestCase
  self.described_class = Purchase::SyncStatusWithChargeProcessorService
  self.rspec_metadata = { vcr: true }

  # frozen_string_literal: false

  context_ Purchase::SyncStatusWithChargeProcessorService, :vcr do
    before do
  ...
  test "marks a free purchase as successful and returns true" do
  ...
  test "marks a free gift purchase as successful and marks the associated giftee purchase as successful too in case of a successful gift purchase and returns true" do
      gift = create(:gift)
  ...
  test "marks a free purchase for a subscription as succcessful and creates the subscription and returns true" do
  ...
  test "marks a free purchase for a subscription as successful and does not create a subscription if one is already present and returns true" do
      product = create(:product, :is_subscription, user: @seller)
      offer_code = create(:offer_code, products: [product], amount_cents: 100)
      purchase = create(:free_purchase, link: product, purchase_state: "in_progress", offer_code:, price: product.default_price)
      purchase.process!
  ...
  test "marks the purchase as successful and returns true if purchase's charge was successful" do
  ...
  test "marks the purchase that is part of a combined charge as successful and returns true" do
  ...
  test "returns false and leaves the purchase in_progress when a combined charge has nil flow_of_funds (transient unsettled state)" do
  ...
  test "marks the associated gift and giftee purchase as successful too in case of a successful gift purchase" do
      gift = create(:gift)
  ...
  test "creates a subscription in case of a successful subscription purchase" do
      product = create(:product, :is_subscription, user: @seller)
  ...
  test "does not try to create a new subscription if one is already present" do
      product = create(:product, :is_subscription, user: @seller)
      purchase = create(:purchase, link: product, purchase_state: "in_progress", chargeable: create(:chargeable), price: product.default_price)
      purchase.process!
      subscription = create(:subscription, link: product)
  ...
  test "does not increment seller's balance again if it is already done once for this purchase" do
      purchase = create(:purchase, link: @product, purchase_state: "in_progress", chargeable: create(:chargeable))
      purchase.process!
  ...
  test "marks the purchase as failed and returns false if purchase's charge was not successful" do
  ...
  test "does not raise any error and returns false if purchase's merchant account is nil" do
      purchase = create(:purchase, link: @product, purchase_state: "in_progress", chargeable: create(:chargeable_success_charge_decline))
      purchase.process!
  ...
  test "does not mark purchase as failed if mark_as_failed flag is not set" do
      purchase = create(:purchase, link: @product, purchase_state: "in_progress", chargeable: create(:chargeable_success_charge_decline))
      purchase.process!
      purchase.merchant_account_id = nil
      purchase.save!
  ...
  test "marks a free preorder authorization purchase as preorder_authorization_successful and returns true if mark_as_failed flag is set" do
      offer_code = create(:offer_code, products: [@product], amount_cents: 100)
  ...
  context_ "for a paypal connect purchase" do
  test "marks the purchase as successful and returns true if purchase's charge was successful" do
  ...
  test "marks the purchase as failed and returns false if purchase's charge has been refunded" do
        merchant_account = create(:merchant_account_paypal, user: @product.user,
                                                            charge_processor_merchant_id: "CJS32DZ7NDN5L", currency: "gbp")
  ...
  context_ "for a Stripe Connect purchase" do
  test "marks the purchase as successful and returns true if purchase's charge was successful" do
