...
class PurchaseConfirmServiceTest < ActiveSupport::TestCase
  self.described_class = Purchase::ConfirmService
  self.rspec_metadata = { vcr: true }

  # frozen_string_literal: false

  context_ Purchase::ConfirmService, :vcr do
    include ManageSubscriptionHelpers

    let(:user) { create(:user) }
    let(:chargeable) { build(:chargeable, card: StripePaymentMethodHelper.success_sca_not_required) }

  context_ "when purchase has been marked as failed" do
  ...
  context_ "for a classic product" do
  ...
  test "marks purchase as failed and returns an error message" do
  ...
  test "marks pre-order as authorization_failed and returns an error message" do
          params = {
            stripe_error: {
              code: "invalid_request_error",
              message: "We are unable to authenticate your payment method."
  ...
  context_ "for a membership upgrade purchase" do
        before do
          setup_subscription
  ...
  test "reverts the subscription to old tier and returns an error message" do
  ...
  context_ "for a membership restart purchase" do
        before do
          setup_subscription
  ...
  test "marks the purchase as failed and unsubscribes the membership" do
  ...
  test "marks purchase as successful" do
  ...
  test "does not return an error if purchase is already successful" do
  ...
  test "enqueues activate integrations worker" do
          allow_any_instance_of(Stripe::PaymentIntent).to receive(:confirm)
  ...
  context_ "when charge processor is unavailable" do
          before do
  ...
  test "marks pre-order successful" do
          error_message = Purchase::ConfirmService.new(purchase: authorization_purchase, params: {}).perform
  ...
  test "marks the purchase as successful and updates integrations" do
          expect(@membership_upgrade_purchase.purchase_state).to eq("in_progress")
          expect(@subscription.original_purchase.variant_attributes).to eq [@new_tier]
  ...
  test "marks the purchase as successful and sends membership restart notifications" do
          expect(@membership_restart_purchase.purchase_state).to eq("in_progress")
          expect(@subscription.reload.is_resubscription_pending_confirmation?).to be true
          expect(@subscription.alive?).to be(true)
          allow_any_instance_of(Purchase).to receive(:confirm_charge_intent!).and_return true
