...
class HandlePayoutReversedWorkerTest < ActiveSupport::TestCase
  self.described_class = HandlePayoutReversedWorker


  context_ HandlePayoutReversedWorker do
  context_ "#perform" do
  ...
  context_ "reversing payout succeeds" do
        let(:reversing_payout_status) { "paid" }
        let(:balance_transaction_status) { "available" }

  test "calls the StripePayoutProcessor#handle_stripe_event_payout_reversed" do
  ...
  context_ "reversing payout fails" do
  ...
  context_ "reversing payout isn't finalized" do
        let(:reversing_payout_status) { "paid" }
