...
class SendYearInReviewEmailJobTest < ActiveSupport::TestCase
  self.described_class = SendYearInReviewEmailJob


  context_ SendYearInReviewEmailJob do
    include PaymentsHelper, ProductPageViewHelpers

  context_ ".perform" do
  context_ "when no payouts exist for the selected year" do
  ...
  context_ "when payouts exist for the selected year", :vcr, :elasticsearch_wait_for_refresh do
  ...
  context_ "when seller made only affiliate sales" do
          before do
  ...
  context_ "when seller sold only one product" do
          before do
            ...
            allow_any_instance_of(Exports::Payouts::Annual).to receive(:perform) {
              {
                csv_file: Rack::Test::UploadedFile.new("#{Rails.root}/test/support/fixtures/followers_import.csv"),
  ...
  test "shows stats only for one product" do
            expect do
              described_class.new.perform(seller.id, date.year)
            ...
            expect(mail.body.sanitized).not_to include("Elsewhere")
  ...
  test "renders the AI text list section" do
            expect do
              described_class.new.perform(seller.id, date.year)
            end.to change { ActionMailer::Base.deliveries.count }.by(1)
  ...
  context_ "when seller is from US" do
          before do
  ...
  context_ "when seller is eligible for 1099" do
            before { allow_any_instance_of(User).to receive(:eligible_for_1099?).and_return(true) }

  test "sends an email with 1099 eligibility confirmation" do
              expect do
                described_class.new.perform(seller.id, date.year)
              end.to change { ActionMailer::Base.deliveries.count }.by(1)
              ...
              expect(mail.body.sanitized).to include("You sold products in 1 country")
              expect(mail.body.sanitized).not_to include("United States")
  ...
  context_ "when recipient is passed as param" do
  test "sends an email to recipient instead of seller" do
                expect do
  ...
  context_ "when seller is not eligible for 1099" do
  test "sends an email with 1099 non eligibility confirmation" do
              expect do
                described_class.new.perform(seller.id, date.year)
              end.to change { ActionMailer::Base.deliveries.count }.by(1)
  ...
  context_ "when seller is not from US" do
          let(:seller) do
            create(
  ...
  test "sends an email without 1099 section" do
            expect do
              described_class.new.perform(seller.id, date.year)
            end.to change { ActionMailer::Base.deliveries.count }.by(1)
            ...
            expect(mail.body.sanitized).to include(seller.financial_annual_report_url_for(year: date.year))
            expect(mail.body.sanitized).not_to include("You do not qualify for a 1099 this year.")
            expect(mail.body.sanitized).not_to include("Your 1099 form is available for download")
          end
        end
      end
    end
