...
class TriggerCommunityChatRecapRunJobTest < ActiveSupport::TestCase
  self.described_class = TriggerCommunityChatRecapRunJob



  context_ TriggerCommunityChatRecapRunJob do
    subject(:job) { described_class.new }

  context_ "#perform" do
  context_ "when recap_frequency is invalid" do
  test "raises ArgumentError" do
          expect { job.perform("invalid") }.to raise_error(ArgumentError, "Recap frequency must be daily or weekly")
        end
      end

  context_ "when recap_frequency is daily" do
        let(:from_date) { Date.yesterday.beginning_of_day }
        let(:to_date) { from_date.end_of_day }

  context_ "when no messages exist" do
  test "creates a daily recap run and marks it as finished" do
            expect do
              expect do
                job.perform("daily")
  ...
  context_ "when a recap run already exists" do
          let!(:existing_run) { create(:community_chat_recap_run, from_date:, to_date:) }

  test "does not create a new recap run" do
            expect do
              expect do
                job.perform("daily")
  ...
  context_ "when from_date is provided" do
          let(:custom_date) { 2.days.ago.to_date.to_s }

  test "uses the provided date" do
            expect do
  ...
  context_ "when recap_frequency is weekly" do
        let(:from_date) { (Date.yesterday - 6.days).beginning_of_day }
        let(:to_date) { (from_date + 6.days).end_of_day }

  context_ "when no messages exist" do
  test "creates a weekly recap run and marks it as finished" do
            expect do
              expect do
                job.perform("weekly")
              end.to change(CommunityChatRecapRun, :count).by(1)
  ...
  test "creates a pending weekly recap run" do
            expect do
              job.perform("weekly")
            end.to change(CommunityChatRecapRun, :count).by(1)
              .and change(CommunityChatRecap, :count).by(1)
