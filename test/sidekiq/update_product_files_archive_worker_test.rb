...
class UpdateProductFilesArchiveWorkerTest < ActiveSupport::TestCase
  self.described_class = UpdateProductFilesArchiveWorker
  self.rspec_metadata = { vcr: true }



  context_ UpdateProductFilesArchiveWorker, :vcr do
  context_ "#perform" do
      before do
  ...
  context_ "when rich content provider is not present" do
        before do
  ...
  test "creates a zip archive of product files while skipping external link and stream only files" do
  ...
  context_ "when product files archive is marked as deleted" do
          before do
            @product_files_archive.mark_deleted!
          end

  test "doesn't update the archive" do
            expect do
  ...
  context_ "when rich content provider is present" do
        before do
  ...
  test "creates a zip archive of embedded ungrouped files and grouped files while skipping external link and stream only files" do
          UpdateProductFilesArchiveWorker.new.perform(@product_files_archive.id)
          @product_files_archive.reload
          expect(@product_files_archive.url).to end_with("zip")
  ...
  test "creates a zip archive of folder files" do
