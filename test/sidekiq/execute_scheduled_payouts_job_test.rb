...
class ExecuteScheduledPayoutsJobTest < ActiveSupport::TestCase
  self.described_class = ExecuteScheduledPayoutsJob



  context_ ExecuteScheduledPayoutsJob do
  context_ "#perform" do
      let(:user) { create(:user) }

  test "executes due scheduled payouts" do
  ...
  test "does not execute future scheduled payouts" do
  ...
  test "does not execute already executed scheduled payouts" do
        create(:scheduled_payout, user: user, status: "executed", scheduled_at: 1.day.ago)

        expect { described_class.new.perform }.not_to raise_error
      end

  test "flags payouts with active chargebacks and sends email" do
  ...
  test "continues processing other payouts when one fails" do
