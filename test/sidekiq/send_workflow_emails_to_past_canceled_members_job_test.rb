...
class SendWorkflowEmailsToPastCanceledMembersJobTest < ActiveSupport::TestCase
  self.described_class = SendWorkflowEmailsToPastCanceledMembersJob
  self.rspec_metadata = { freeze_time: true }



  context_ SendWorkflowEmailsToPastCanceledMembersJob, :freeze_time do
    before do
      @seller = create(:user)
  ...
  test "schedules a worker immediately for past cancellations whose deactivated_at + delay is in the past" do
  ...
  test "schedules a worker at deactivated_at + delay when that time is in the future" do
  ...
  test "does nothing when the workflow has been deleted" do
      @workflow.mark_deleted!
      described_class.new.perform(@installment.id)
      expect(SendWorkflowInstallmentWorker.jobs).to be_empty
    end

  test "does nothing when the installment has been deleted" do
      @installment.mark_deleted!
      described_class.new.perform(@installment.id)
      expect(SendWorkflowInstallmentWorker.jobs).to be_empty
    end

  test "does nothing when the installment is unpublished" do
      @installment.update!(published_at: nil)
      described_class.new.perform(@installment.id)
      expect(SendWorkflowInstallmentWorker.jobs).to be_empty
    end

  test "does nothing when send_to_past_customers is false" do
      @workflow.update!(send_to_past_customers: false)
      described_class.new.perform(@installment.id)
      expect(SendWorkflowInstallmentWorker.jobs).to be_empty
    end

  test "does nothing when workflow trigger is not member_cancellation" do
      @workflow.update!(workflow_trigger: nil)
      described_class.new.perform(@installment.id)
      expect(SendWorkflowInstallmentWorker.jobs).to be_empty
    end

  test "does nothing when workflow type is not seller, product, or variant" do
  ...
  test "does nothing when the installment has no rule" do
      @rule.destroy!
      described_class.new.perform(@installment.id)
      expect(SendWorkflowInstallmentWorker.jobs).to be_empty
    end

  test "skips alive subscriptions" do
  ...
  test "skips pending-cancellation subscriptions (not yet deactivated)" do
  ...
  context_ "for a seller-type workflow" do
      before do
  ...
  test "schedules workers for canceled subscriptions across all seller's products" do
