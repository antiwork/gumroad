...
class UpdateSellerRefundEligibilityJobTest < ActiveSupport::TestCase
  self.described_class = UpdateSellerRefundEligibilityJob



  context_ UpdateSellerRefundEligibilityJob do
  ...
  context_ "when refund is currently disabled" do
      before { user.disable_refunds! }

  test "enables refunds when balance goes above $0" do
  ...
  context_ "when refund is currently enabled" do
      before { user.enable_refunds! }

  test "disables refunds when balance dips below -$100" do
        # Current balance is 0.01
        create(:balance, user: user, amount_cents: 1)
        expect { perform }.not_to change { user.reload.refunds_disabled? }
  ...
  context_ "when user is on LowBalanceFraudCheck probation and balance recovers", versioning: true do
      before do
        user.send(:disable_refunds_and_put_on_probation!)
        user.enable_refunds!
      end

  test "automatically removes probation once balance reaches $100" do
