...
class SendPreorderSellerSummaryWorkerTest < ActiveSupport::TestCase
  self.described_class = SendPreorderSellerSummaryWorker
  self.rspec_metadata = { vcr: true }



  context_ SendPreorderSellerSummaryWorker, :vcr do
    before do
  ...
  test "sends the summary email if all preorders are charged" do
  ...
  test "does not send the summary email if there are un-charged preorders" do
      authorization_purchase = build(:purchase, link: @product, chargeable: @card_will_succeed, purchase_state: "in_progress", is_preorder_authorization: true)
      preorder = @preorder_product.build_preorder(authorization_purchase)
      preorder.authorize!
      preorder.mark_authorization_successful
  ...
  test "sends the summary email if the preorder was charged and failed" do
  ...
  context_ "when preorders take more than 24h to charge" do
  test "stops waiting and notifies error tracker", :sidekiq_inline do
        authorization_purchase = build(:purchase, link: @product, chargeable: @card_will_decline, purchase_state: "in_progress",
                                                  is_preorder_authorization: true)
        preorder = @preorder_product.build_preorder(authorization_purchase)
        preorder.authorize!
