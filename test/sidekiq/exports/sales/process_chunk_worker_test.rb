...
class ExportsSalesProcessChunkWorkerTest < ActiveSupport::TestCase
  self.described_class = Exports::Sales::ProcessChunkWorker



  context_ Exports::Sales::ProcessChunkWorker do
    before do
  ...
  context_ "when there are still chunks to process" do
      before do
  ...
  context_ "when chunks were processed with another revision" do
      before do
  ...
  test "requeues chunks that were processed with another revision" do
        @worker.perform(@chunk_2.id)
