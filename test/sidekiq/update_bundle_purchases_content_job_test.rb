...
class UpdateBundlePurchasesContentJobTest < ActiveSupport::TestCase
  self.described_class = UpdateBundlePurchasesContentJob



  context_ UpdateBundlePurchasesContentJob do
  ...
  context_ "#perform" do
  test "updates bundle purchase content for outdated purchases" do
        expect(Purchase::UpdateBundlePurchaseContentService).to receive(:new).with(outdated_purchase).and_call_original
        expect(Purchase::UpdateBundlePurchaseContentService).not_to receive(:new).with(purchase).and_call_original
