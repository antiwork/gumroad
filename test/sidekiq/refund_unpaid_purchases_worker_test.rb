...
class RefundUnpaidPurchasesWorkerTest < ActiveSupport::TestCase
  self.described_class = RefundUnpaidPurchasesWorker
  self.rspec_metadata = { vcr: true }



  context_ RefundUnpaidPurchasesWorker, :vcr do
  context_ "#perform" do
      before do
  ...
  test "does not refund purchases if the user is not suspended" do
  ...
  test "queues the refund of unpaid purchases" do
