...
class RecurringChargeReminderWorkerTest < ActiveSupport::TestCase
  self.described_class = RecurringChargeReminderWorker
  self.rspec_metadata = { vcr: true }



  context_ RecurringChargeReminderWorker, :vcr do
  ...
  test "sends a reminder email about an upcoming charge" do
      expect do
  ...
  test "does not send a reminder if the subscription is no longer alive" do
  ...
  test "does not send a reminder if the subscription is pending cancellation" do
  ...
  test "does not send a reminder for a fixed-length subscription that has had its last charge" do
  ...
  test "does not send a reminder if `send_renewal_reminders?` is false" do
  ...
  test "does not send a reminder for a subscription still in a free trial" do
