...
class CancelPreorderWorkerTest < ActiveSupport::TestCase
  self.described_class = CancelPreorderWorker
  self.rspec_metadata = { vcr: true }



  context_ CancelPreorderWorker, :vcr do
  context_ "perform" do
  test "cancels the preorder if it is in authorization_successful state but does not send notification emails" do
        ...
        expect do
          ...
          end.not_to have_enqueued_mail(CustomerLowPriorityMailer, :preorder_cancelled).with(preorder.id)
        end.not_to have_enqueued_mail(ContactingCreatorMailer, :preorder_cancelled).with(preorder.id)
