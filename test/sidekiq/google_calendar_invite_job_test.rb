...
class GoogleCalendarInviteJobTest < ActiveSupport::TestCase
  self.described_class = GoogleCalendarInviteJob



  context_ GoogleCalendarInviteJob do
  context_ "#perform" do
  ...
  context_ "when the call already has a Google Calendar event ID" do
        before do
  ...
  context_ "when the call does not have a Google Calendar event ID" do
        before do
          allow(call).to receive(:google_calendar_event_id).and_return(nil)
        end

  context_ "when the Google Calendar integration is not present" do
          before do
  ...
  context_ "when the Google Calendar integration is present" do
          let(:event) do
            {
  ...
  context_ "when the API call is successful" do
  ...
  test "creates a Google Calendar event and updates the call" do
  ...
  context_ "when the API call fails with a 401 error" do
  ...
  test "refreshes the token and retries inserting the event" do
  ...
  context_ "when the API call fails with a non-401 error" do
