...
class FindSubscriptionsWithMissingChargeWorkerTest < ActiveSupport::TestCase
  self.described_class = FindSubscriptionsWithMissingChargeWorker



  context_ FindSubscriptionsWithMissingChargeWorker do
  context_ "#perform" do
      before do
  ...
  context_ "without a batch_number" do
  test "queues a job for each of the 10 batches" do
          freeze_time
  ...
  context_ "with a batch_number" do
        before do
  ...
  context_ "not matching a subscription id" do
          before do
            @batch_number = 1
          end

  test "does not queue overdue subscriptions with the wrong id" do
  ...
  context_ "matching a subscription id" do
          before do
            @batch_number = 0
          end

  test "queues subscriptions according to last charge's date" do
            travel_to @six_months_after_purchase do
              described_class.new.perform(@batch_number)
  ...
  test "does not queue subscriptions that are not overdue for a charge" do
  ...
  test "does not queue subscriptions that are less than 75 minutes overdue for a charge" do
  ...
  test "does not queue free subscriptions" do
            travel_to @six_months_after_purchase do
  ...
  test "queues subscriptions that were discounted to free with an elapsed discount" do
  ...
  test "does not queue subscriptions to products from suspended users" do
            travel_to @six_months_after_purchase do
  ...
  test "does not queue subscriptions that already have a charge in progress" do
            travel_to @six_months_after_purchase do
