...
class DeleteProductFilesWorkerTest < ActiveSupport::TestCase
  self.described_class = DeleteProductFilesWorker



  context_ DeleteProductFilesWorker do
    before do
  ...
  test "does not delete the product files" do
          freeze_time do
            described_class.new.perform(@product.id)
          end
  ...
  test "does not delete files if the product is not deleted" do
