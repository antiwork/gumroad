...
class RecurringChargeWorkerTest < ActiveSupport::TestCase
  self.described_class = RecurringChargeWorker
  self.rspec_metadata = { vcr: true }



  context_ RecurringChargeWorker, :vcr do
  ...
  test "doesn't call charge on test_subscriptions" do
  ...
  test "doesn't call charge on free purchases" do
  ...
  test "doesn't call charge if there was a purchase made the period for a monthly subscription" do
  ...
  test "doesn't call charge if there was a purchase made the period for a yearly subscription" do
  ...
  test "doesn't call `charge` when invoked one day before the subscription period end date" do
  ...
  test "calls `charge` when invoked at the end of the subscription period" do
  ...
  test "doesn't call `charge` when invoked early, after refunded purchase" do
  ...
  test "doesn't call `charge` when invoked one day after the subscription period end date but there's already a purchase in progress" do
      create(:purchase, link: @product, price_cents: @product.price_cents, is_original_subscription_purchase: true, subscription: @subscription)
  ...
  test "calls `charge` when invoked one day after the subscription period end date" do
      create(:purchase, link: @product, price_cents: @product.price_cents, is_original_subscription_purchase: true, subscription: @subscription)
      travel_to(@subscription.period.from_now + 1.day) do
        expect_any_instance_of(Subscription).to receive(:charge!)
        described_class.new.perform(@subscription.id)
  ...
  test "calls charge when invoked one year after the subscription period end date" do
      create(:purchase, link: @product, price_cents: @product.price_cents, is_original_subscription_purchase: true, subscription: @subscription)
  ...
  test "calls `charge` for subscriptions purchased on 30th January when invoked at the end of the subscription period" do
  ...
  context_ "with ignore_consecutive_failures = true" do
  context_ "when last purchase failed" do
        before { create(:membership_purchase, subscription: @subscription, link: @product, purchase_state: "failed") }

  test "does not call `charge`" do
  ...
  test "calls `unsubscribe_and_fail!` when the subscription is at least 5 days overdue for a charge" do
          allow_any_instance_of(Subscription).to receive(:seconds_overdue_for_charge).and_return(5.days + 1.minute)
  ...
  context_ "subscription has a pending plan change" do
      before do
        setup_subscription
  ...
  test "updates the variants and prices before charging" do
        described_class.new.perform(@subscription.id)
  ...
  test "marks the plan change deleted and applied, and marks older plan changes deleted" do
        described_class.new.perform(@subscription.id)
  ...
  context_ "for a PWYW tier" do
  test "sets the original purchase price to the perceived_price_cents" do
  ...
  test "relies on the price at the time of the downgrade" do
  ...
  context_ "when the recurrence option has been deleted" do
  test "still uses that recurrence" do
  ...
  context_ "when the tier has been deleted" do
  test "still uses that tier" do
  ...
  context_ "when the plan change is not currently applicable" do
  test "does not apply the plan change" do
  ...
  test "schedules tier workflows if tier has changed" do
          described_class.new.perform(@subscription.id)
  ...
  test "does not schedule workflows if tier has not changed" do
  ...
  test "enqueues integrations update if tier has changed" do
          described_class.new.perform(@subscription.id)

          expect(UpdateIntegrationsOnTierChangeWorker).to have_enqueued_sidekiq_job(@subscription.id)
        end

  test "does not enqueue integrations update if tier has not changed" do
          @plan_change.update!(tier: @original_tier)
  ...
  context_ "non-tiered subscription has a pending plan change" do
      before do
  ...
  test "updates the price before charging" do
        described_class.new.perform(@subscription.id)
  ...
  context_ "subscription_id provided" do
  test "does not call charge on a subscription" do
          expect_any_instance_of(Subscription).not_to receive(:charge!)
          described_class.new.perform(@subscription.id)
        end
      end
  ...
  context_ "subscriber removes his credit card" do
  test "calls `charge` on subscriptions" do
  ...
  context_ "subscription has free trial" do
      before do
  ...
  context_ "free trial has ended" do
  ...
  context_ "free trial has not yet ended" do
  ...
  context_ "subscription has a fixed-duration offer code that makes the product free for the first billing period" do
      before do
  ...
  test "calls charge when the offer code has elapsed" do
        travel_to(@subscription.period.from_now) do
          expect_any_instance_of(Subscription).to receive(:charge!)
          described_class.new.perform(@subscription.id)
        end
