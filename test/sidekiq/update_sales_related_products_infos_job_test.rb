...
class UpdateSalesRelatedProductsInfosJobTest < ActiveSupport::TestCase
  self.described_class = UpdateSalesRelatedProductsInfosJob



  context_ UpdateSalesRelatedProductsInfosJob do
  context_ "#perform" do
      before do
  ...
  context_ "when a SalesRelatedProductsInfo record exists" do
  ...
  context_ "when increment is false" do
  test "decrements the sales_count" do
            described_class.new.perform(new_purchase.id, false)
            expect(sales_related_products_info.reload.sales_count).to eq(1)
          end
        end

  context_ "when increment is true" do
  test "increments the sales_count" do
            described_class.new.perform(new_purchase.id)
            expect(sales_related_products_info.reload.sales_count).to eq(3)
          end
        end

  test "enqueues UpdateCachedSalesRelatedProductsInfosJob for the product and related products" do
          described_class.new.perform(new_purchase.id)
  ...
  context_ "when a SalesRelatedProductsInfo record doesn't exist" do
  test "creates a SalesRelatedProductsInfo record with sales_count set to 1" do
          expect do
            described_class.new.perform(new_purchase.id)
