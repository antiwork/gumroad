...
class UpdateLargeSellersSalesCountJobTest < ActiveSupport::TestCase
  self.described_class = UpdateLargeSellersSalesCountJob



  context_ UpdateLargeSellersSalesCountJob do
  context_ "#perform" do
  ...
  test "updates sales_count for large sellers when count has changed" do
  ...
  test "does not update sales_count when it hasn't changed" do
  ...
  test "updates sales_count even when below threshold" do
        user3 = create(:user)
