...
class SendMembershipsPriceUpdateEmailsJobTest < ActiveSupport::TestCase
  self.described_class = SendMembershipsPriceUpdateEmailsJob


  context_ SendMembershipsPriceUpdateEmailsJob do
  context_ "#perform" do
  ...
  context_ "when there are applicable subscription plan changes" do
  ...
  test "sends notification emails for applicable changes", :freeze_time do
          expect do
            subject.perform
  ...
  test "does not send notification emails for subscriptions pending cancellation" do
  ...
  test "does not send notification emails for subscriptions that are fully cancelled" do
  ...
  test "does not send notification emails for subscriptions that have ended" do
  ...
  test "does not send notification emails for subscriptions that have failed" do
  ...
  context_ "when there are non-applicable subscription plan changes" do
        before do
  ...
  test "does not send any emails" do
          subject.perform
          expect(CustomerLowPriorityMailer).not_to have_received(:subscription_price_change_notification)
        end
      end

  context_ "when there are no subscription plan changes" do
  test "does not send any emails" do
          subject.perform
          expect(CustomerLowPriorityMailer).not_to have_received(:subscription_price_change_notification)
        end
