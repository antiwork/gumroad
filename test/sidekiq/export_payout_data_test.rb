...
class ExportPayoutDataTest < ActiveSupport::TestCase
  self.described_class = ExportPayoutData


  context_ ExportPayoutData do
  context_ "#perform" do
  ...
  test "generates CSV files for each payment" do
  ...
  test "sends an email with a zip file when multiple payments are provided" do
  ...
  test "sends an email with a single CSV file when only one payment is provided" do
        mail_double = instance_double(ActionMailer::MessageDelivery)
        expect(mail_double).to receive(:deliver_now)
  ...
  test "handles a single ID passed as a non-array" do
        mail_double = instance_double(ActionMailer::MessageDelivery)
        expect(mail_double).to receive(:deliver_now)
  ...
  test "does nothing if no payments are found" do
        expect(ContactingCreatorMailer).not_to receive(:payout_data)

        ExportPayoutData.new.perform([999999], seller.id)
      end

  test "does nothing if payments are from different sellers" do
  ...
  context_ "with duplicate filenames" do
        let(:payment1) { create(:payment, user: seller, created_at: Time.zone.now) }
        let(:payment2) { create(:payment, user: seller, created_at: Time.zone.now) }

  test "creates unique filenames in the zip archive" do
          mail_double = instance_double(ActionMailer::MessageDelivery)
          expect(mail_double).to receive(:deliver_now)
