...
class HandleStripeAutodebitForNegativeBalanceTest < ActiveSupport::TestCase
  self.described_class = HandleStripeAutodebitForNegativeBalance


  context_ HandleStripeAutodebitForNegativeBalance do
  context_ "#perform" do
  ...
  context_ "payout succeeds" do
        let(:stripe_payout_status) { "paid" }
        let(:balance_transaction_status) { "available" }

  test "calls the StripePayoutProcessor#handle_stripe_negative_balance_debit_event" do
  ...
  context_ "payout fails" do
        let(:stripe_payout_status) { "failed" }
        let(:balance_transaction_status) { "available" }

  test "does nothing and does not raise an error" do
  ...
  context_ "payout isn't finalized" do
        let(:stripe_payout_status) { "paid" }
        let(:balance_transaction_status) { "pending" }

  test "raises an error", :vcr, :sidekiq_inline do
          expect do
            HandleStripeAutodebitForNegativeBalance.new.perform(stripe_event_id, stripe_connect_account_id, stripe_payout_id)
