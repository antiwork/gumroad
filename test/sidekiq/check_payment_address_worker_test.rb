...
class CheckPaymentAddressWorkerTest < ActiveSupport::TestCase
  self.described_class = CheckPaymentAddressWorker


  context_ CheckPaymentAddressWorker do
  context_ "payment address checks" do
      before do
  ...
  test "flags the user for fraud if a blocked email object exists for their payment address" do
  ...
  context_ "stripe fingerprint checks" do
  test "does not flag the user for fraud if there are no suspended users with the same stripe fingerprint" do
        user = create(:user)
  ...
  test "flags the user for fraud if a suspended user has the same stripe fingerprint" do
  ...
  test "flags the user for fraud if a suspended_for_tos_violation user has the same stripe fingerprint" do
  ...
  test "flags the user for fraud if a blocked fingerprint object exists" do
        user = create(:user)
  ...
  test "flags the user even if the suspended user's bank account is deleted (fraud history is preserved)" do
        suspended_user = create(:user, user_risk_state: "suspended_for_fraud")
  ...
  test "does not flag if the new user's bank account is deleted" do
        suspended_user = create(:user, user_risk_state: "suspended_for_fraud")
  ...
  test "does not flag if stripe fingerprint is blank" do
        user = create(:user)
  ...
  test "checks all fingerprints when new user has multiple bank accounts" do
        suspended_user = create(:user, user_risk_state: "suspended_for_fraud")
  ...
  test "flags if any of the user's fingerprints is blocked" do
        user = create(:user)
        create(:ach_account, user:, stripe_fingerprint: "clean_fingerprint")
  ...
  test "does not flag the user if they cannot be flagged for fraud" do
        suspended_user = create(:user, user_risk_state: "suspended_for_fraud")
  ...
  test "does not raise error if user is not found" do
