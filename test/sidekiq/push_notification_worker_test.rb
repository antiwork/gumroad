...
class PushNotificationWorkerTest < ActiveSupport::TestCase
  self.described_class = PushNotificationWorker



  context_ PushNotificationWorker do
    before do
      @user = create(:user)
  ...
  test "sends the notification to each of the user's devices" do
      ios_a = double("ios_a")
      ios_c = double("ios_c")
  ...
  test "sends the notification only to given app_type" do
      ios_a = double("ios_a")
      ios_d = double("ios_d")
      ...
      expect(ios_a).not_to receive(:process)
