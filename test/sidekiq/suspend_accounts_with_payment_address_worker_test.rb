...
class SuspendAccountsWithPaymentAddressWorkerTest < ActiveSupport::TestCase
  self.described_class = SuspendAccountsWithPaymentAddressWorker


  context_ SuspendAccountsWithPaymentAddressWorker do
  context_ "#perform" do
  context_ "with payment address" do
        before do
  ...
  test "does not suspend already suspended users with same payment address" do
  ...
  context_ "with stripe fingerprint" do
        before do
          @user = create(:user)
          @user_2 = create(:user)
          @user_3 = create(:user)
  ...
  test "suspends other accounts with the same stripe fingerprint" do
          described_class.new.perform(@user.id)
  ...
  test "creates a suspended comment with fingerprint details" do
          described_class.new.perform(@user.id)
  ...
  test "does not suspend if fingerprint is blank" do
  ...
  test "does not suspend users whose bank accounts are deleted" do
  ...
  test "does not suspend already suspended users" do
          @user_2.suspend_for_fraud!(author_name: "test")
          initial_comment_count = @user_2.comments.count
  ...
  test "still suspends related accounts even if suspended user's bank account is deleted" do
  ...
  test "checks all fingerprints when suspended user has multiple bank accounts" do
  ...
  test "uses the matching fingerprint in the comment when user has multiple fingerprints" do
          create(:ach_account, user: @user, stripe_fingerprint: "another_fingerprint")
          user_with_another_fingerprint = create(:user)
          create(:ach_account, user: user_with_another_fingerprint, stripe_fingerprint: "another_fingerprint")
  ...
  context_ "with both payment address and stripe fingerprint" do
        before do
          @user = create(:user, payment_address: "sameuser@paypal.com")
  ...
  test "suspends accounts matching either payment address or stripe fingerprint" do
          described_class.new.perform(@user.id)
