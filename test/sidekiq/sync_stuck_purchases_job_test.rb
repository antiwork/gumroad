...
class SyncStuckPurchasesJobTest < ActiveSupport::TestCase
  self.described_class = SyncStuckPurchasesJob
  self.rspec_metadata = { vcr: true }



  context_ SyncStuckPurchasesJob, :vcr do
  context_ "#perform" do
  ...
  test "does not sync any purchases if there are none in progress" do
  ...
  test "does not sync any in progress purchases if they are outside of the query time range" do
  ...
  test "does not sync an in progress purchases if it is an off session charge on an Indian card created less than 26 hours ago" do
  ...
  test "syncs the in progress free purchase and does nothing if the new purchase state is successful and there are no subsequent successful purchases" do
  ...
  test "syncs the in progress purchase and does nothing if the new purchase state is successful and there are no subsequent successful purchases" do
  ...
  test "syncs the in progress which is an off session charge on an Indian card created more than 26 hours ago and does nothing if the new purchase state is successful and there are no subsequent successful purchases" do
  ...
  context_ "when there is a subsequent successful purchase" do
        let!(:successful_purchase) { create(:purchase, link: product) }

  test "syncs the in progress purchase and does nothing if the new purchase state is failed" do
          ...
          expect_any_instance_of(Purchase).not_to receive(:refund_and_save!)
  ...
  test "syncs the in progress purchase and then refunds it if the new purchase state is successful" do
  ...
  test "syncs the in progress free purchase and then does not refund it because it is free if the new purchase state is successful" do
          stuck_in_progress_free_purchase_that_will_succeed_when_synced = travel_to(Time.current - 12.hours) do
            offer_code = create(:offer_code, products: [product], amount_cents: 100)
  ...
  context_ "when there is a subsequent successful purchase for a variant of the product" do
  ...
  test "syncs the in progress purchase and then refunds it if the new purchase state is successful for the same variant of the product" do
          allow_any_instance_of(User).to receive(:unpaid_balance_cents).and_return(100_00)
  ...
  test "syncs the in progress purchase and then does NOT refund it if the new purchase state is successful for a different variant of the product" do
          stuck_in_progress_purchase_that_will_succeed_when_synced = travel_to(Time.current - 12.hours) do
