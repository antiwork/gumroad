...
class SyncStuckPayoutsJobTest < ActiveSupport::TestCase
  self.described_class = SyncStuckPayoutsJob


  context_ SyncStuckPayoutsJob do
  context_ "#perform" do
  context_ "when processor type is PayPal" do
        before do
  ...
  test "syncs all stuck PayPal payouts" do
  ...
  test "does not sync those payments that are not either in 'creating', 'processing', or 'unclaimed' state" do
  ...
  test "does not try to sync Stripe payouts" do
  ...
  test "processes all stuck payouts even if any of them raises an error" do
          create(:payment, processor: PayoutProcessorType::PAYPAL, state: "creating")
          create(:payment, processor: PayoutProcessorType::PAYPAL, state: "creating")
  ...
  context_ "when processor type is Stripe" do
  test "syncs stuck Stripe payouts past the stored arrival date" do
  ...
  test "syncs stuck Stripe payouts without arrival_date using created_at fallback" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  test "fails Stripe payouts stuck in creating for over 24 hours" do
  ...
  test "syncs Stripe payouts stuck in creating with Stripe IDs to their actual status" do
  ...
  test "does not sync Stripe payouts still in creating under 24 hours" do
          create(:payment, processor: PayoutProcessorType::STRIPE, state: "creating",
                           stripe_transfer_id: nil, stripe_connect_account_id: nil,
  ...
  test "does not sync recent Stripe payouts still legitimately in transit" do
          create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  test "does not sync Stripe payouts whose arrival date is still in the future" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  test "handles failed Stripe payouts" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  test "handles cancelled Stripe payouts" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  test "does not sync payments in terminal states" do
  ...
  test "does not include unclaimed Stripe payments in the scope" do
          create(:payment, processor: PayoutProcessorType::STRIPE, state: "unclaimed",
  ...
  test "does not sync Stripe payouts whose arrival date is today" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  test "does not try to sync PayPal payouts" do
          create(:payment, processor: PayoutProcessorType::PAYPAL, state: "processing")
