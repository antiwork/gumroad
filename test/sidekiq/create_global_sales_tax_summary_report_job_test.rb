...
class CreateGlobalSalesTaxSummaryReportJobTest < ActiveSupport::TestCase
  self.described_class = CreateGlobalSalesTaxSummaryReportJob



  context_ CreateGlobalSalesTaxSummaryReportJob do
    let(:month) { 1 }
    let(:year) { 2024 }

  test "raises an argument error if the year is out of bounds" do
      expect { described_class.new.perform(month, 2013) }.to raise_error(ArgumentError)
    end

  test "raises an argument error if the month is out of bounds" do
      expect { described_class.new.perform(13, year) }.to raise_error(ArgumentError)
    end

  context_ "report generation" do
      before :context do
  ...
  context_ "with purchases across multiple countries" do
        before do
  ...
  test "creates a summary CSV with correct headers, country/state breakdown, and sends email" do
  ...
  context_ "country normalization" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "normalizes country names using common_name" do
          described_class.new.perform(month, year)
  ...
  context_ "null country fallback" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "falls back to ip_country when country is nil" do
          described_class.new.perform(month, year)
  ...
  test "groups purchases with no country as Unknown" do
          described_class.new.perform(month, year)
  ...
  context_ "refund handling", :vcr do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "uses net-of-refunds amounts for partially refunded purchases" do
          described_class.new.perform(month, year)
  ...
  context_ "excluded purchases" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "excludes fully refunded and zero-tax purchases" do
          described_class.new.perform(month, year)
  ...
  context_ "US state resolution via IP fallback" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "falls back to GeoIp lookup when zip code is missing" do
  ...
  context_ "Canada province via ip_state fallback" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "falls back to ip_state for Canada province" do
          described_class.new.perform(month, year)
  ...
  context_ "India state with invalid ip_state" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "uses empty state when ip_state is invalid for India" do
          described_class.new.perform(month, year)
  ...
  context_ "GMV and tax formatting" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
            create_taxed_purchase(product, country: "Germany", gumroad_tax_cents: 1900)
  ...
  test "formats monetary values with cents" do
          described_class.new.perform(month, year)
  ...
  context_ "US state resolution via GeoIp for invalid zip code" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "falls back to GeoIp lookup when zip code is invalid" do
  ...
  context_ "country alias normalization" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "normalizes country aliases to canonical names" do
          described_class.new.perform(month, year)
  ...
  context_ "no double-counting between aggregation and GeoIp fallback" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "counts each purchase exactly once" do
          allow(GeoIp).to receive(:lookup).and_return(double(region_name: "TX"))
  ...
  context_ "chunked month processing" do
        before do
          product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "aggregates purchases from different date chunks into one report row" do
          described_class.new.perform(month, year)
  ...
  context_ "binary-safe GROUP BY prevents collation merges" do
        before do
          travel_to(Time.find_zone("UTC").local(2024, 1, 15)) do
            product = create(:product, price_cents: 100_00, native_type: "digital")
  ...
  test "does not merge case-different country values in GROUP BY and aggregates correctly in Ruby" do
          described_class.new.perform(month, year)
