...
class UpdatePayoutStatusWorkerTest < ActiveSupport::TestCase
  self.described_class = UpdatePayoutStatusWorker


  context_ UpdatePayoutStatusWorker do
  context_ "#perform" do
  ...
  test "fetches and sets the new payment status from PayPal" do
  ...
  test "does not attempt to fetch or update the status for a payment not in the 'processing' state" do
  ...
  test "raises an exception if the status fetched is 'pending'" do
          expect(PaypalPayoutProcessor).to(
            receive(:get_latest_payment_state_from_paypal).with(50,
                                                                "4LR",
                                                                payment.created_at.beginning_of_day - 1.day,
