...
class ElasticsearchIndexerWorkerTest < ActiveSupport::TestCase
  self.described_class = ElasticsearchIndexerWorker



  context_ ElasticsearchIndexerWorker, :elasticsearch_wait_for_refresh do
  context_ "#perform without ActiveRecord objects" do
      before do
        class TravelEvent
  ...
  context_ "when indexing" do
  test "creates a document with the specified body" do
  ...
  test "creates a document in an index which name is specified dynamically" do
          id = SecureRandom.uuid
          described_class.new.perform(
            "index",
  ...
  context_ "#perform with ActiveRecord objects" do
      before do
        @client = EsClient.dup
  ...
  context_ "when updating" do
        before do
          @record = @model.create!(name: "Drawing", country: "France")
        end

  context_ "an existing document" do
          before do
            @client.index(
  ...
  context_ "a non-existing document" do
          subject(:perform) do
            described_class.new.perform(
              "update",
              "class_name" => @model.name,
  ...
  context_ "while ignoring 404 errors from index" do
            before do
              $redis.sadd(RedisKey.elasticsearch_indexer_worker_ignore_404_errors_on_indices, @model.index_name)
            end

  test "does not raise an error and ignores the request" do
  ...
  context_ "when updating by query" do
        before do
          @model.mapping do
            indexes :country, type: :keyword
  ...
  test "updates values matching the query" do
  ...
  test "scrolls through results if the first query fails" do
  ...
  test "handles conflicts when updating scrolled results" do
          stub_const("#{described_class}::UPDATE_BY_QUERY_SCROLL_SIZE", 1)
  ...
  test "does not support nested fields", elasticsearch_wait_for_refresh: false do
          expect do
            described_class.new.perform(
              "update_by_query",
              "class_name" => @model.name,
  ...
  context_ ".columns_to_fields" do
  test "returns matching fields" do
