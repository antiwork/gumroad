...
class SendPaypalTopupNotificationJobTest < ActiveSupport::TestCase
  self.described_class = SendPaypalTopupNotificationJob


  context_ SendPaypalTopupNotificationJob do
  context_ "#perform" do
      before do
  ...
  test "sends a notification to slack with the required topup amount and sets redis key to true" do
  ...
  test "includes details of payout amount in transit in the slack notification" do
  ...
  test "sends no more topup required green notification and sets redis key to false if there's sufficient amount in PayPal" do
  ...
  context_ "when notify_only_if_topup_needed is true" do
  test "sends notification when topup is needed" do
          allow(PaypalPayoutProcessor).to receive(:topup_amount_in_transit).and_return(0)
  ...
  test "does not send notification when topup is not needed and sets redis key to false" do
          allow(PaypalPayoutProcessor).to receive(:topup_amount_in_transit).and_return(300_000)
