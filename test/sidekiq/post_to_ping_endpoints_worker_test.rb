...
class PostToPingEndpointsWorkerTest < ActiveSupport::TestCase
  self.described_class = PostToPingEndpointsWorker
  self.rspec_metadata = { vcr: true }



  context_ PostToPingEndpointsWorker, :vcr do
    before do
  ...
  test "enqueues job PostToIndividualPingEndpointWorker with the correct parameters" do
  ...
  test "includes a license key if the purchase has one" do
  ...
  test "includes gift related information if the purchase is a gift" do
  ...
  test "includes a purchaser id if there is one for the purchase" do
  ...
  test "includes the sku id in the post if there's a sku" do
  ...
  test "includes the custom sku in the post if there's a sku" do
      @product.skus_enabled = true
      @product.is_physical = true
      @product.require_shipping = true
      @product.shipping_destinations << ShippingDestination.new(country_code: Product::Shipping::ELSEWHERE, one_item_rate_cents: 0, multiple_items_rate_cents: 0)
  ...
  test "includes the sku id in the post if the product is sku-enabled and doesn't have a sku" do
      @product.skus_enabled = true
      @product.is_physical = true
      @product.require_shipping = true
      @product.shipping_destinations << ShippingDestination.new(country_code: Product::Shipping::ELSEWHERE, one_item_rate_cents: 0, multiple_items_rate_cents: 0)
  ...
  context_ "shipping information" do
  test "makes a post with the correct body" do
  ...
  test "makes the request to the endpoint with the correct paramters" do
        purchase = create(
          :purchase,
          link: @product,
          price_cents: 500,
  ...
  context_ "offer_code" do
  test "makes request to the endpoint with the correct parameters" do
        purchase = create(:purchase, link: @product, price_cents: 500)
  ...
  test "makes request to endpoint with the correct parameters" do
        purchase = create(:purchase, link: @product, price_cents: 500)
  ...
  context_ "refund resource" do
  test "does not send a ping to the user's notification_endpoint about sale refund", :sidekiq_inline do
  ...
  context_ "cancellation resource" do
  test "does not send a ping to the user's notification_endpoint about subscription cancellation", :sidekiq_inline do
  ...
  context_ "resource subscription" do
      before do
  ...
  test "posts to the app's post url" do
        purchaser = create(:user)
  ...
  test "posts affiliate_credit information to the app's post url" do
  ...
  test "posts to all apps' post urls and to the user's notification endpoint" do
  ...
  test "does not post to the app's post url if the token is revoked" do
        @token.revoke
        purchase = create(:purchase, link: @product, price_cents: 500, email: "ibuy@gumroad.com")
  ...
  test "does not post to the app's post url if the post url is invalid" do
        @resource_subscription.update!(post_url: "http://localhost/path")
        purchase = create(:purchase, link: @product)

        PostToPingEndpointsWorker.new.perform(purchase.id, nil)

        expect(PostToIndividualPingEndpointWorker).not_to have_enqueued_sidekiq_job(@resource_subscription.post_url, anything, @resource_subscription.content_type, anything)
      end

  test "does not post to the app's post url if the user hasn't given view_sales permissions to the app" do
        another_app = create(:oauth_application, owner: create(:user), name: "another app")
  ...
  test "posts to the app's post url if the user has given multiple permissions to the app, including view_sales" do
        @token.revoke
  ...
  test "posts to the app's post url if the token has account scope" do
        @token.revoke
  ...
  test "posts to the app's post url even if the token is expired" do
  ...
  test "posts sale refunded ping to the 'refunded' resource's post_url", :sidekiq_inline do
        purchase = create(:purchase, link: @product, price_cents: 500, stripe_refunded: true)
        params = purchase.payload_for_ping_notification(resource_name: ResourceSubscription::REFUNDED_RESOURCE_NAME)
  ...
  test "posts subscription cancelled ping to the 'cancellation' resource's post_url", :sidekiq_inline do
        purchase = create(:membership_purchase, link: @product, price_cents: 500)
        subscription = create(:subscription, link: @product, cancelled_at: Time.current, user_requested_cancellation_at: Time.current)
        subscription.purchases << purchase
        params = subscription.payload_for_ping_notification(resource_name: ResourceSubscription::CANCELLED_RESOURCE_NAME)
  ...
  test "posts subscription ended ping to the 'subscription_ended' resource's post_url", :sidekiq_inline do
  ...
  test "does not post subscription ended ping to the 'subscription_ended' resource's post_url if the subscription has not ended", :sidekiq_inline do
  ...
  test "posts subscription restarted ping ot the 'subscription_restarted' resource's post_url", :sidekiq_inline do
        subscription = create(:subscription, link: @product)
        create(:membership_purchase, subscription:, link: @product)
  ...
  test "does not post subscription restarted ping to the 'subscription_restarted' resource's post_url if the subscription has a termination date", :sidekiq_inline do
  ...
  test "posts subscription updated ping to the 'subscription_updated' resource's post_url", :sidekiq_inline do
        subscription = create(:subscription, link: @product)
        create(:membership_purchase, subscription:, link: @product)
  ...
  test "posts sale disputed ping to the 'dispute' resource's post_url", :sidekiq_inline do
  ...
  test "posts sale dispute won ping to the 'dispute_won' resource's post_url", :sidekiq_inline do
