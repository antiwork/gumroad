...
class SuspendUsersWorkerTest < ActiveSupport::TestCase
  self.described_class = SuspendUsersWorker


  context_ SuspendUsersWorker do
  context_ "#perform" do
  ...
  test "suspends the users appropriately with IDs" do
  ...
  test "suspends the users appropriately with external IDs" do
  ...
  test "suspends the users appropriately with mixed internal and external IDs" do
  ...
  context_ "with scheduled payout params" do
  ...
  test "creates a scheduled payout for newly-suspended users" do
  ...
  test "does not create a scheduled payout for users who were already suspended" do
          described_class.new.perform(admin_user.id, user_ids_to_suspend, reason, additional_notes, scheduled_payout)

          expect(already_suspended_user.reload.scheduled_payouts.count).to eq(0)
        end

  test "defaults delay_days to 21 when not provided" do
  ...
  test "does nothing when scheduled_payout is nil" do
  ...
  test "does nothing when scheduled_payout action is blank" do
          described_class.new.perform(admin_user.id, [not_reviewed_user.id], reason, additional_notes, { "action" => "", "delay_days" => "14" })

          expect(not_reviewed_user.reload.scheduled_payouts.count).to eq(0)
        end

  test "does not create a scheduled payout when the user has no unpaid balance" do
