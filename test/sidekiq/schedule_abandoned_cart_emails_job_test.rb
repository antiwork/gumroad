...
class ScheduleAbandonedCartEmailsJobTest < ActiveSupport::TestCase
  self.described_class = ScheduleAbandonedCartEmailsJob



  context_ ScheduleAbandonedCartEmailsJob do
  context_ "#perform" do
  ...
  context_ "when there are no abandoned carts" do
  test "does not schedule any emails" do
          create(:cart)
          expect { described_class.new.perform }.not_to have_enqueued_mail(CustomerMailer, :abandoned_cart)
        end
      end

  context_ "when there are abandoned carts" do
  context_ "when there are no matching abandoned cart workflows" do
  test "does not schedule any emails" do
  ...
  context_ "when there are matching abandoned cart workflows" do
  ...
  test "schedules emails for the matching abandoned carts belonging to both logged-in users and guest carts" do
            expect do
  ...
  context_ "when there are multiple matching abandoned cart workflows for a cart" do
  ...
  test "schedules only one email for each of the corresponding carts" do
            expect do
              described_class.new.perform
  ...
  context_ "when seller is not eligible for abandoned cart workflows" do
        let(:cart) { create(:cart) }
  ...
  test "does not schedule any abandoned cart emails" do
          expect do
            described_class.new.perform
