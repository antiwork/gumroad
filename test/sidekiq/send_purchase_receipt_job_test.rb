...
class SendPurchaseReceiptJobTest < ActiveSupport::TestCase
  self.described_class = SendPurchaseReceiptJob



  context_ SendPurchaseReceiptJob do
  ...
  context_ "when the purchase is for a product with stampable PDFs" do
      before do
  ...
  test "stamps the PDFs and delivers the email" do
  ...
  context_ "when stamping the PDF fails" do
        before do
          allow(PdfStampingService).to receive(:stamp_for_purchase!).and_raise(PdfStampingService::Error)
        end

  test "doesn't deliver the email and raises an error" do
  ...
  context_ "when the purchase is for a product without stampable PDFs" do
      before do
        allow(PdfStampingService).to receive(:stamp_for_purchase!)
        allow_any_instance_of(Link).to receive(:has_stampable_pdfs?).and_return(false)
      end

  test "delivers the email and doesn't stamp PDFs" do
        expect(CustomerMailer).to receive(:receipt).with(purchase.id).and_return(mail_double)
        described_class.new.perform(purchase.id)
  ...
  context_ "when the purchase is a bundle product purchae" do
      before do
        allow_any_instance_of(Purchase).to receive(:is_bundle_product_purchase?).and_return(true)
      end

  test "doens't deliver email" do
        expect(CustomerMailer).not_to receive(:receipt).with(purchase.id)
        described_class.new.perform(purchase.id)
      end
    end
