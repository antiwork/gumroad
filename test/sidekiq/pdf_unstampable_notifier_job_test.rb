...
class PdfUnstampableNotifierJobTest < ActiveSupport::TestCase
  self.described_class = PdfUnstampableNotifierJob



  context_ PdfUnstampableNotifierJob do
  context_ "#perform" do
      let!(:product) { create(:product_with_pdf_file) }

  test "does not notify seller if a pdf is stampable" do
  ...
  test "does nothing if no files have pdf stamping enabled" do
  ...
  test "notifies seller if a pdf can't be stamped" do
        product.product_files.alive.pdf.each { |product_file| product_file.update!(pdf_stamp_enabled: true) }
  ...
  test "does not try to stamp a pdf that has already been marked as stampable or non-stampable" do
  ...
  context_ "with eligible sale" do
  ...
  test "enqueues job to generate stamped pdfs for existing sales" do
          expect(PdfStampingService).to receive(:can_stamp_file?).and_return(true)
          expect do
            described_class.new.perform(product.id)
          end.not_to have_enqueued_mail(ContactingCreatorMailer, :unstampable_pdf_notification)
