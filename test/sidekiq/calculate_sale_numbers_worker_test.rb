...
class CalculateSaleNumbersWorkerTest < ActiveSupport::TestCase
  self.described_class = CalculateSaleNumbersWorker
  self.rspec_metadata = { vcr: true }



  context_ CalculateSaleNumbersWorker, :vcr do
  context_ "#perform" do
  test "sets the correct values for `total_made` and `number_of_creators` in Redis" do
