...
class RemoveDeletedFilesFromS3JobTest < ActiveSupport::TestCase
  self.described_class = RemoveDeletedFilesFromS3Job



  context_ RemoveDeletedFilesFromS3Job do
    before do
  ...
  test "does not attempt to delete files already marked as removed from S3" do
  ...
  test "does not remove files with existing alive duplicate files from S3" do
