...
class PostToIndividualPingEndpointWorkerTest < ActiveSupport::TestCase
  self.described_class = PostToIndividualPingEndpointWorker



  context_ PostToIndividualPingEndpointWorker do
    before do
      @http_double = double
  ...
  context_ "post to individual endpoint" do
  context_ "when the content_type is application/x-www-form-urlencoded" do
  test "posts to the right endpoint using the right params" do
  ...
  test "posts to the right endpoint and encodes brackets" do
  ...
  context_ "when the content_type is application/json" do
  test "posts to the right endpoint using the right params" do
  ...
  test "does not raise when it encounters an internet error" do
  ...
  test "re-raises a non-internet error" do
  ...
  test "retries 50x status codes the right number of times and does not raise", :sidekiq_inline do
  ...
  test "does not retry other status codes", :sidekiq_inline do
      allow(@http_double).to receive(:success?).and_return(false)
  ...
  context_ "logging" do
  test "logs url, response code and params" do
  ...
  context_ "ping failure notifications" do
  ...
  context_ "when retries are exhausted" do
  test "sends email notification when user_id is provided", :sidekiq_inline do
  ...
  test "does not send email notification when user_id is nil", :sidekiq_inline do
  ...
  test "does not send email notification when user_id is not provided", :sidekiq_inline do
          expect(ContactingCreatorMailer).not_to receive(:ping_endpoint_failure)

          PostToIndividualPingEndpointWorker.new.perform(ping_url, params, Mime[:url_encoded_form].to_s)
        end

  test "updates user's last_ping_failure_notification_at timestamp", :sidekiq_inline do
  ...
  context_ "when notification throttling is active" do
        before do
          user.last_ping_failure_notification_at = (PostToIndividualPingEndpointWorker::NOTIFICATION_THROTTLE_PERIOD.ago + 1.day).to_s
          user.save!
        end

  test "does not send email notification when last notification was within throttle period", :sidekiq_inline do
          expect(ContactingCreatorMailer).not_to receive(:ping_endpoint_failure)

          PostToIndividualPingEndpointWorker.new.perform(ping_url, params, Mime[:url_encoded_form].to_s, user.id)
        end

  test "does not update last_ping_failure_notification_at when throttled", :sidekiq_inline do
          expect do
            PostToIndividualPingEndpointWorker.new.perform(ping_url, params, Mime[:url_encoded_form].to_s, user.id)
  ...
  context_ "when notification throttling has expired" do
        before do
          user.last_ping_failure_notification_at = (PostToIndividualPingEndpointWorker::NOTIFICATION_THROTTLE_PERIOD.ago - 1.day).to_s
          user.save!
        end

  test "sends email notification when throttle period has passed", :sidekiq_inline do
          expect(ContactingCreatorMailer).to receive(:ping_endpoint_failure).with(user.id, ping_url, 500).and_call_original
          expect_any_instance_of(ActionMailer::MessageDelivery).to receive(:deliver_later).with(queue: "critical")
  ...
  test "updates last_ping_failure_notification_at when throttle has expired", :sidekiq_inline do
          allow(ContactingCreatorMailer).to receive(:ping_endpoint_failure).and_return(double(deliver_later: true))
  ...
  context_ "when request succeeds" do
        before do
          allow(@http_double).to receive(:success?).and_return(true)
          allow(@http_double).to receive(:code).and_return(200)
        end

  test "does not send email notification on successful request", :sidekiq_inline do
          expect(ContactingCreatorMailer).not_to receive(:ping_endpoint_failure)
  ...
  context_ "when request fails with non-retryable status code" do
        before do
          allow(@http_double).to receive(:success?).and_return(false)
          allow(@http_double).to receive(:code).and_return(404)
        end

  test "sends email notification immediately for non-retryable errors", :sidekiq_inline do
  ...
  context_ "when ping URL is not seller's notification_endpoint" do
  ...
  test "does not send email notification for resource subscription URL failures", :sidekiq_inline do
          expect(ContactingCreatorMailer).not_to receive(:ping_endpoint_failure)

          PostToIndividualPingEndpointWorker.new.perform(resource_subscription_url, params, Mime[:url_encoded_form].to_s, user.id)
        end

  test "does not update last_ping_failure_notification_at for resource subscription failures", :sidekiq_inline do
          expect do
            PostToIndividualPingEndpointWorker.new.perform(resource_subscription_url, params, Mime[:url_encoded_form].to_s, user.id)
          end.not_to change { user.reload.last_ping_failure_notification_at }
        end
