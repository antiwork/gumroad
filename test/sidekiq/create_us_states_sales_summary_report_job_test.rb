...
class CreateUsStatesSalesSummaryReportJobTest < ActiveSupport::TestCase
  self.described_class = CreateUsStatesSalesSummaryReportJob



  context_ CreateUsStatesSalesSummaryReportJob do
    let(:subdivision_codes) { ["WA", "WI"] }
    let(:month) { 8 }
    let(:year) { 2022 }

  test "is configured with retry: 3" do
      expect(described_class.sidekiq_options["retry"]).to eq(3)
    end

  context_ "sidekiq_retries_exhausted" do
  test "emails payments notification with the failure context" do
  ...
  test "raises an argument error if any subdivision code is not valid" do
  ...
  test "creates a summary CSV file with correct totals for each state and submits transactions to TaxJar" do
  ...
  test "creates a summary CSV file with correct totals for each state without submitting transactions to TaxJar when push_to_taxjar is false" do
        expect(s3_bucket_double).to receive(:object).ordered.and_return(@s3_object)
