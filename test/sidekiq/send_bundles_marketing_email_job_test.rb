...
class SendBundlesMarketingEmailJobTest < ActiveSupport::TestCase
  self.described_class = SendBundlesMarketingEmailJob



  context_ SendBundlesMarketingEmailJob do
  context_ "#perform" do
  ...
  test "enqueues the mail with the correct arguments" do
        expect do
  ...
  context_ "when a bundle doesn't have at least two products" do
        before do
          products[..2].each { _1.update!(created_at: Time.new(2022, 1, 1)) }
        end

  test "excludes that bundle" do
          expect do
            described_class.new.perform
          end.to have_enqueued_mail(CreatorMailer, :bundles_marketing).with(
            seller_id: seller.id,
  ...
  context_ "no bundles" do
        before do
          products.each { _1.destroy! }
          index_model_records(Link)
        end

  test "doesn't enqueue any mail" do
          expect do
            described_class.new.perform
          end.not_to have_enqueued_mail(CreatorMailer, :bundles_marketing)
        end
      end

  context_ "seller is suspended" do
