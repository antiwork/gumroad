...
class CreateIndiaSalesReportJobTest < ActiveSupport::TestCase
  self.described_class = CreateIndiaSalesReportJob



  context_ CreateIndiaSalesReportJob do
  context_ "#perform" do
  test "raises an ArgumentError if the year is less than 2014 or greater than 3200" do
        expect do
  ...
  test "raises an ArgumentError if the month is not within 1 and 12 inclusive" do
        expect do
  ...
  test "defaults to previous month when no parameters provided" do
  ...
  test "generates CSV report for India sales" do
  ...
  test "excludes purchases with business VAT ID" do
        expect(s3_bucket_double).to receive(:object).and_return(@s3_object)
  ...
  test "handles invalid Indian states" do
