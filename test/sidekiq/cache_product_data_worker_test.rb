...
class CacheProductDataWorkerTest < ActiveSupport::TestCase
  self.described_class = CacheProductDataWorker


  context_ CacheProductDataWorker do
  context_ "#perform" do
      before do
        @product = create(:product)
      end

  test "creates new product cache data" do
        expect do
  ...
  context_ "when data is already cached" do
        before do
          @product_cached_value = @product.product_cached_values.create!
        end

  test "expires the current cache and creates a new cached record" do
          expect do
            described_class.new.perform(@product.id)
