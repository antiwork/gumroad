...
class HandleGrmcCallbackJobTest < ActiveSupport::TestCase
  self.described_class = HandleGrmcCallbackJob


  context_ HandleGrmcCallbackJob do
  ...
  context_ "#perform" do
  context_ "when notification status is 'success'" do
        let(:notification) { { "job_id" => transcoded_video.job_id, "status" => "success" } }

  test "updates the product file and transcoded video, and marks it as completed" do
  ...
  test "updates all processing transcoded videos, and marks them as completed" do
  ...
  context_ "when notification status is not 'success'" do
        let(:notification) { { "job_id" => transcoded_video.job_id, "status" => "failure" } }

  test "enqueues TranscodeVideoForStreamingWorker" do
          described_class.new.perform(notification)
  ...
  test "marks transcoded video, marks it as errored" do
          described_class.new.perform(notification)
          transcoded_video.reload
