...
class GenerateCommunityChatRecapJobTest < ActiveSupport::TestCase
  self.described_class = GenerateCommunityChatRecapJob



  context_ GenerateCommunityChatRecapJob do
  ...
  test "marks the corresponding recap run as finished when all associated recaps are finished" do
  ...
  test "does not mark the corresponding recap run as finished when not all associated recaps are finished" do
  ...
  test "marks the corresponding recap run as finished when all associated recaps are either finished or failed" do
  ...
  context_ "sidekiq_retries_exhausted", :sidekiq_inline do
      let(:recap_run) { create(:community_chat_recap_run) }
      let(:community) { create(:community) }
      let(:recap) { create(:community_chat_recap, community:, community_chat_recap_run: recap_run) }
  ...
  test "updates the failing recap status to failed" do
        expect_any_instance_of(CommunityChatRecapRun).to receive(:check_if_finished!).and_call_original
