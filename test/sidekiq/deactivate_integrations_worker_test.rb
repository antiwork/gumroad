...
class DeactivateIntegrationsWorkerTest < ActiveSupport::TestCase
  self.described_class = DeactivateIntegrationsWorker



  context_ DeactivateIntegrationsWorker do
  ...
  test "errors out if purchase is not found" do
      ...
      [Integrations::CircleIntegrationService, Integrations::DiscordIntegrationService].each do |integration_service|
        expect_any_instance_of(integration_service).not_to receive(:deactivate)
      end
    end

  test "deactivates integrations for other purchases on the same subscription" do
