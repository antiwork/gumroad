...
class DeleteExpiredProductCachedValuesWorkerTest < ActiveSupport::TestCase
  self.described_class = DeleteExpiredProductCachedValuesWorker



  context_ DeleteExpiredProductCachedValuesWorker do
  context_ "#perform" do
  test "deletes expired rows, except the latest one per product" do
