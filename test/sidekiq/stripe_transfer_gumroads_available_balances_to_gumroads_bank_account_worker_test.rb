...
class StripeTransferGumroadsAvailableBalancesToGumroadsBankAccountWorkerTest < ActiveSupport::TestCase
  self.described_class = StripeTransferGumroadsAvailableBalancesToGumroadsBankAccountWorker
  self.rspec_metadata = { vcr: true }



  context_ StripeTransferGumroadsAvailableBalancesToGumroadsBankAccountWorker, :vcr do
  ...
  test "aborts the process if a particular feature flag is set" do
        Feature.activate(:skip_transfer_from_stripe_to_bank)
        expect(StripeTransferExternallyToGumroad).not_to receive(:transfer_all_available_balances)
  ...
  test "transfers all available balances with a buffer of 500k+ the expected Stripe connect payouts Gumroad is holding" do
  ...
  test "leaves 530k in the Stripe balance" do
        described_class.new.perform
