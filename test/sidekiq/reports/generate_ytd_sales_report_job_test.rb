...
class ReportsGenerateYtdSalesReportJobTest < ActiveSupport::TestCase
  self.described_class = Reports::GenerateYtdSalesReportJob



  context_ Reports::GenerateYtdSalesReportJob do
  ...
  context_ "#perform" do
  context_ "when processing actual purchase records", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  test "generates correct CSV data including refunds" do
  ...
  test "enqueues emails to recipients" do
          travel_to Time.zone.local(2023, 8, 15) do
            job.perform
          end
