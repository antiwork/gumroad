...
class FailAbandonedPurchaseWorkerTest < ActiveSupport::TestCase
  self.described_class = FailAbandonedPurchaseWorker
  self.rspec_metadata = { vcr: true }



  context_ FailAbandonedPurchaseWorker, :vcr do
    include ManageSubscriptionHelpers

  context_ "#perform" do
      let(:chargeable) { build(:chargeable, card: StripePaymentMethodHelper.success_with_sca) }

  context_ "when purchase has succeeded" do
  ...
  context_ "when purchase has failed" do
  ...
  context_ "when purchase is in_progress" do
  context_ "preorder purchase" do
  ...
  context_ "when purchase was abandoned and SCA never completed" do
  test "cancels the setup intent and marks purchase as failed" do
  ...
  context_ "when setup intent has been canceled in parallel" do
            before do
              ChargeProcessor.cancel_setup_intent!(purchase.merchant_account, purchase.processor_setup_intent_id)
            end

  test "handles the error gracefully and does not change the purchase state" do
              described_class.new.perform(purchase.id)
              expect(purchase.reload.purchase_state).to eq("in_progress")
            end
          end

  context_ "when setup intent has succeeded in parallel" do
            before do
  ...
  context_ "when unexpected charge processor error occurs" do
            before do
  ...
  context_ "when purchase was abandoned and SCA never completed" do
  test "cancels the charge intent and marks purchase as failed" do
  ...
  context_ "when payment intent has been canceled in parallel" do
            before do
  ...
  context_ "when payment intent has succeeded in parallel" do
            before do
  ...
  context_ "membership upgrade purchase" do
  ...
  context_ "when purchase was abandoned and SCA never completed" do
  test "cancels the charge intent, marks purchase as failed, and cancels membership upgrade" do
              expect(ChargeProcessor).to receive(:cancel_payment_intent!).and_call_original
  ...
  context_ "membership restart purchase" do
          let(:user) { create(:user) }
  ...
  context_ "when the job is called before time to complete SCA has expired" do
          let(:purchase) { create(:purchase_in_progress, chargeable:) }
  ...
  test "does not cancel payment intent and reschedules the job instead" do
            expect(ChargeProcessor).not_to receive(:cancel_charge_intent!)
  ...
  context_ "when purchase has no processor_payment_intent_id or processor_setup_intent_id" do
