...
class ScheduleMembershipPriceUpdatesJobTest < ActiveSupport::TestCase
  self.described_class = ScheduleMembershipPriceUpdatesJob


  context_ ScheduleMembershipPriceUpdatesJob do
  context_ "perform" do
  context_ "for a non-tiered membership variant" do
  ...
  context_ "for a membership tier" do
  ...
  context_ "for a tier with apply_price_changes_to_existing_memberships disabled" do
  test "does nothing" do
  ...
  context_ "that has a pending plan change to an enabled tier" do
  test "records a plan change" do
  ...
  context_ "for a tier with apply_price_changes_to_existing_memberships enabled" do
  test "records a plan change for each live or pending cancellation subscription on the next charge after the effective date" do
  ...
  test "applies the offer code when calculating the new price" do
  ...
  context_ "when the subscription has a pending plan change" do
  context_ "that will switch away from the given tier" do
  test "does nothing" do
  ...
  context_ "that will switch to the given tier" do
  test "records a subscription plan change if the price is different from the agreed on price" do
  ...
  test "does nothing but notify error tracker if the price is the same as the agreed on price" do
  ...
  context_ "that switches billing periods" do
  test "records a subscription plan change if the new price is different from the agreed on price" do
  ...
  test "does nothing but notify error tracker if the new price is the same as the agreed on price" do
  ...
  test "deletes existing plan changes for product price changes, but not user-initiated plan changes" do
  ...
  context_ "but updating the subscription raises an error" do
  test "rolls back the transaction and does not retry" do
  ...
  context_ "for a fixed length subscription that has completed its charges" do
  test "does nothing" do
  ...
  context_ "for a subscription that is inactive" do
  test "does nothing" do
  ...
  context_ "when the price has not changed" do
  test "does nothing but notify error tracker" do
              expect(ErrorNotifier).to receive(:notify).with("Not adding a plan change for membership price change - subscription_id: #{enabled_subscription.id} - reason: price has not changed")
  ...
  context_ "when the price for a recurrence is deleted" do
  test "does nothing but notify error tracker" do
