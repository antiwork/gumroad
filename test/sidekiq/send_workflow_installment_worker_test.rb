...
class SendWorkflowInstallmentWorkerTest < ActiveSupport::TestCase
  self.described_class = SendWorkflowInstallmentWorker


  context_ SendWorkflowInstallmentWorker do
    before do
      @product = create(:product)
    end

  context_ "purchase_installment" do
      before do
  ...
  test "calls purchase mailer if same version" do
  ...
  test "does not call mailer if different version" do
  ...
  test "does not call mailer if deleted installment" do
        @installment.update_attribute(:deleted_at, Time.current)
        expect(PostSendgridApi).not_to receive(:process)
        SendWorkflowInstallmentWorker.new.perform(@installment.id, @installment_rule.version, @purchase.id, nil, nil)
      end

  test "does not call mailer if workflow is deleted" do
        @workflow.update_attribute(:deleted_at, Time.current)
        expect(PostSendgridApi).not_to receive(:process)
        SendWorkflowInstallmentWorker.new.perform(@installment.id, @installment_rule.version, @purchase.id, nil, nil)
      end

  test "does not call mailer if installment is not published" do
        @installment.update_attribute(:published_at, nil)
        expect(PostSendgridApi).not_to receive(:process)
        SendWorkflowInstallmentWorker.new.perform(@installment.id, @installment_rule.version, @purchase.id, nil, nil)
      end

  test "does not call mailer if installment is not found" do
        expect(PostSendgridApi).not_to receive(:process)
        SendWorkflowInstallmentWorker.new.perform("non-existing-installment-id", @installment_rule.version, @purchase.id, nil, nil)
      end

  test "does not call mailer if seller is suspended" do
  ...
  test "calls follower mailer if same version" do
  ...
  context_ "member_cancellation_installment" do
      before do
        @creator = create(:user)
  ...
  test "calls cancellation mailer if given subscription id" do
        expect(PostSendgridApi).to receive(:process).with(
          post: @installment,
  ...
  test "caches template rendering" do
      @workflow = create(:workflow, seller: @product.user, link: @product, created_at: Time.current)
      @installment = create(:installment, link: @product, workflow: @workflow, published_at: Time.current)
      @installment_rule = create(:installment_rule, installment: @installment, delayed_delivery_time: 1.day)
