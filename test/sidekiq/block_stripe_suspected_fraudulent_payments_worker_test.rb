...
class BlockStripeSuspectedFraudulentPaymentsWorkerTest < ActiveSupport::TestCase
  self.described_class = BlockStripeSuspectedFraudulentPaymentsWorker



  context_ BlockStripeSuspectedFraudulentPaymentsWorker do
  context_ "#perform" do
      before do
  ...
  context_ "when email is not from Stripe" do
  test "does not trigger any processing" do
  ...
  context_ "when email body does not contain any transaction IDs" do
  test "does not trigger any processing" do
          expect_any_instance_of(Helper::Client).not_to receive(:add_note)
          expect_any_instance_of(Helper::Client).not_to receive(:close_conversation)
  ...
  context_ "when there is an error" do
