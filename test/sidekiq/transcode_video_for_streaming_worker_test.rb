...
class TranscodeVideoForStreamingWorkerTest < ActiveSupport::TestCase
  self.described_class = TranscodeVideoForStreamingWorker


  context_ TranscodeVideoForStreamingWorker do
  context_ "#perform" do
      let(:product) { create(:product_with_video_file) }

  context_ "when the product file is not transcodable" do
        let(:product_file) { product.product_files.first }

  test "notifies the creator and does not create a transcoding job" do
  ...
  test "does nothing if `#attempt_to_transcode?` returns `false`" do
          ...
          expect do
            described_class.new.perform(product_file.id)
          end.not_to change { TranscodedVideo.count }
        end
      end

  context_ "when the product file is transcodable" do
        let(:product_file) do
  ...
  test "does nothing when the product file is deleted" do
          product_file.delete!
  ...
  test "creates a transcoded_video record and marks it as completed if there's already another duplicate completed one" do
  ...
  test "creates a transcoded_video but doesn't process it if there's already another duplicate processing one" do
  ...
  test "transcodes the video and does not send the transcoding error email", :vcr do
          expect(ContactingCreatorMailer).not_to receive(:video_transcode_failed)
  ...
  test "marks the existing transcoded video still processing as failed, when `allowed_when_processing` is true" do
  ...
  test "does not mark the existing transcoded video still processing as failed, when `allowed_when_processing` is false" do
          transcoded_video = create(:transcoded_video, streamable: product_file, original_video_key: product_file.s3_key, state: "processing")
  ...
  test "transcodes the video when input video key is more than 255 bytes long", :vcr do
  ...
  context_ "transcode using MediaConvert" do
          before do
  ...
  test "creates MediaConvert job and a TranscodedVideo object" do
  ...
  context_ "with GRMC", :freeze_time do
            before do
              allow(GlobalConfig).to receive(:get).with("GRMC_API_KEY").and_return("test_api_key")
            end

  context_ "when request to GRMC is successful" do
              before do
  ...
  test "creates a GRMC job, a TranscodedVideo object, and schedules a retry with AWS MediaConvert" do
                expect_any_instance_of(Aws::MediaConvert::Client).not_to receive(:create_job)
  ...
  context_ "when request to GRMC is not successful" do
              before do
                stub_request(:post, described_class::GRMC_ENDPOINT)
  ...
  context_ "when transcoder param is set to ets" do
            before do
  ...
  test "transcodes the video using ETS" do
  ...
  context_ "when transcoder param is set to MediaConvert" do
  test "transcodes the video using MediaConvert" do
              expect_any_instance_of(Aws::MediaConvert::Client).to receive(:create_job)
  ...
  context_ "transcode using ETS" do
        let(:product_file) { product.product_files.first }

  context_ "output_key_prefix" do
          before do
            ets_client_double = double("ets_client")
            allow(ets_client_double).to receive(:data).and_return(OpenStruct.new({ job: { id: 1 } }))
            allow_any_instance_of(Aws::ElasticTranscoder::Client).to receive(:create_job).and_return(ets_client_double)
  ...
  context_ "when repeated occurrences of extension is present in the filename" do
  test "generates the correct output_key_prefix" do
  ...
  test "generates the correct output_key_prefix when filename contains multiple lines" do
  ...
  context_ "when repeated occurrences of extension is not present in the filename" do
  test "generates the correct output_key_prefix" do
