...
class GenerateSalesReportJobTest < ActiveSupport::TestCase
  self.described_class = GenerateSalesReportJob



  context_ GenerateSalesReportJob do
    let (:country_code) { "GB" }
    let(:start_date) { Date.new(2015, 1, 1) }
    let (:end_date) { Date.new(2015, 3, 31) }

  test "raises an argument error if the country code is not valid" do
      expect { described_class.new.perform("AUS", start_date, end_date, GenerateSalesReportJob::ALL_SALES) }.to raise_error(ArgumentError)
    end

  test "raises an argument error if the sales_type is neither all_sales nor discover_sales" do
  ...
  test "creates a CSV file for sales into the United Kingdom" do
  ...
  test "creates a CSV file for sales into Australia" do
        expect(ExpiringS3FileService).to receive(:new) do |args|
  ...
  test "creates a CSV file for sales into Singapore" do
        expect(ExpiringS3FileService).to receive(:new) do |args|
  ...
  test "includes Customer Tax ID column in CSV", vcr: { cassette_name: "GenerateSalesReportJob/happy_case/creates_a_CSV_file_for_sales_into_the_United_Kingdom" } do
  ...
  test "creates a CSV file for sales into the United Kingdom and does not send slack notification when send_notification is false",
  ...
  test "creates a CSV file for sales into the United Kingdom and sends slack notification when send_notification is true",
         vcr: { cassette_name: "GenerateSalesReportJob/happy_case/creates_a_CSV_file_for_sales_into_the_United_Kingdom" } do
        expect(ExpiringS3FileService).to receive(:new).and_return(@mock_service)
  ...
  test "creates a CSV file for sales into the United Kingdom and sends slack notification when send_slack_notification is not provided (default behavior)",
         vcr: { cassette_name: "GenerateSalesReportJob/happy_case/creates_a_CSV_file_for_sales_into_the_United_Kingdom" } do
        expect(ExpiringS3FileService).to receive(:new).and_return(@mock_service)
  ...
  test "creates a CSV file for discover sales into the United Kingdom sales_type is set as discover_sales",
  ...
  context_ "s3_prefix functionality", :vcr do
      before do
        @mock_service = double("ExpiringS3FileService")
        allow(ExpiringS3FileService).to receive(:new).and_return(@mock_service)
        allow(@mock_service).to receive(:perform).and_return("#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/test-url")
  ...
  test "uses custom s3_prefix when provided" do
  ...
  test "handles s3_prefix with trailing slash" do
        custom_prefix = "custom/reports/"
        expect(ExpiringS3FileService).to receive(:new) do |args|
  ...
  test "uses default path when s3_prefix is nil" do
        expect(ExpiringS3FileService).to receive(:new) do |args|
          expect(args[:path]).to eq("sales-tax/gb-sales-quarterly")
          expect(args[:filename]).to include("united-kingdom-all-sales-report-2015-01-01-to-2015-03-31")
          expect(args[:bucket]).to eq(REPORTING_S3_BUCKET)
  ...
  test "uses default path when s3_prefix is empty string" do
        expect(ExpiringS3FileService).to receive(:new) do |args|
          expect(args[:path]).to eq("sales-tax/gb-sales-quarterly")
          expect(args[:filename]).to include("united-kingdom-all-sales-report-2015-01-01-to-2015-03-31")
          expect(args[:bucket]).to eq(REPORTING_S3_BUCKET)
