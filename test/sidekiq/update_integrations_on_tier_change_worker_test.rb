...
class UpdateIntegrationsOnTierChangeWorkerTest < ActiveSupport::TestCase
  self.described_class = UpdateIntegrationsOnTierChangeWorker



  context_ UpdateIntegrationsOnTierChangeWorker do
  ...
  test "errors out if subscription is not found" do
      ...
      [Integrations::CircleIntegrationService, Integrations::DiscordIntegrationService].each do |integration_service|
        expect_any_instance_of(integration_service).not_to receive(:update_on_tier_change)
      end
    end
  end
end
