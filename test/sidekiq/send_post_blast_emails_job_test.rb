...
class SendPostBlastEmailsJobTest < ActiveSupport::TestCase
  self.described_class = SendPostBlastEmailsJob
  self.rspec_metadata = { freeze_time: true }



  context_ SendPostBlastEmailsJob, :freeze_time do
  ...
  test "ignores unpublished posts" do
  ...
  test "ignores posts where send_emails is false" do
        basic_post_with_audience.update!(published_at: nil)
        blast = create(:blast, :just_requested, post: basic_post_with_audience)
        described_class.new.perform(blast.id)
  ...
  test "ignores completed blasts" do
  ...
  test "records when blast started processing" do
        blast = create(:blast, :just_requested, post: basic_post_with_audience)
        described_class.new.perform(blast.id)
  ...
  test "does not email the same recipients twice, when the post has been published twice" do
        blast = create(:blast, :just_requested, post: basic_post_with_audience)
        described_class.new.perform(blast.id)
  ...
  context_ "for different post types" do
        before do
          @products = []
  ...
  test "when product_type? is true, it sends the expected emails" do
  ...
  test "when product_type? is true and not_bought_products filter is present it sends the expected emails" do
  ...
  test "when variant_type? is true, it sends the expected emails" do
  ...
  test "when seller_type? is true, it sends the expected emails" do
  ...
  test "when follower_type? is true, it sends the expected emails" do
  ...
  test "when affiliate_type? is true, it sends the expected emails" do
  ...
  test "when audience_type? is true, it sends the expected emails" do
          post = create(:audience_post, :published, seller: @seller)
          blast = create(:blast, :just_requested, post:)
          described_class.new.perform(blast.id)
  ...
  context_ "Attachments and UrlRedirect" do
        before do
  ...
  test "creates the UrlRedirect records and adds a download button to the email" do
  ...
  test "does not create the UrlRedirect records if the post has no attachments" do
          expect do
            described_class.new.perform(@blast.id)
  ...
  context_ "recipients slice size" do
        before do
  ...
  test "is equal to PostSendgridApi::MAX_RECIPIENTS by default" do
  ...
  test "can be controlled by redis key" do
  ...
  test "deletes sent_post_emails records if PostEmailApi.process raises an error" do
