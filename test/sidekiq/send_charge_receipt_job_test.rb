...
class SendChargeReceiptJobTest < ActiveSupport::TestCase
  self.described_class = SendChargeReceiptJob



  context_ SendChargeReceiptJob do
  ...
  context_ "with all purchases ready" do
  test "delivers the email and updates the charge without stamping" do
  ...
  context_ "when the charge receipt has already been sent" do
      before do
  ...
  context_ "when a purchase requires stamping" do
      before do
  ...
  context_ "when stamping fails" do
        before do
