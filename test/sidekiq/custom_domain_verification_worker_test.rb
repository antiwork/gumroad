...
class CustomDomainVerificationWorkerTest < ActiveSupport::TestCase
  self.described_class = CustomDomainVerificationWorker



  context_ CustomDomainVerificationWorker do
  ...
  test "marks an invalid custom domain as unverified" do
      expect do
        expect do
          described_class.new.perform(invalid_custom_domain.id)
        end.not_to change { invalid_custom_domain.reload.verified? }
  ...
  test "ignores verification of a deleted custom domain" do
      ...
      end.not_to change { deleted_custom_domain.reload }
    end

  test "ignores verification of custom domain with invalid domain names" do
      ...
      end.not_to change { invalid_domain.reload }
    end
  end
end
