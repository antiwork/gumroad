...
class ChargePreorderWorkerTest < ActiveSupport::TestCase
  self.described_class = ChargePreorderWorker
  self.rspec_metadata = { vcr: true }



  context_ ChargePreorderWorker, :vcr do
  context_ "#perform" do
      before do
  ...
  test "charges the preorder and mark it as successful" do
        @preorder.authorize!
  ...
  test "raises an error if preorder product is not in chargeable state" do
        @preorder.authorize!
        @preorder.mark_authorization_successful!
  ...
  test "retries the charge if the purchase fails with 'processing_error'",
  ...
  test "retries the charge three times if the purchase fails with 'processing_error'",
        ...
        expect(described_class).not_to have_enqueued_sidekiq_job(@preorder.id, 5)
      end

  test "sends an email to the buyer if the purchase fails with 'card_declined'" do
        @preorder.authorize!
        @preorder.mark_authorization_successful!
  ...
  test "schedules CancelPreorderWorker job to cancel the preorder after 2 weeks if purchase fails with 'card_declined' error" do
        @preorder.authorize!
        @preorder.mark_authorization_successful!
        @preorder.credit_card = CreditCard.create(@good_card_but_cant_charge)
