...
class CollectUnclaimedBalancesOfInactiveStripeAccountsJobTest < ActiveSupport::TestCase
  self.described_class = CollectUnclaimedBalancesOfInactiveStripeAccountsJob


  context_ CollectUnclaimedBalancesOfInactiveStripeAccountsJob do
  context_ "#perform", :vcr do
      before do
        stub_const("CollectUnclaimedBalancesOfInactiveStripeAccountsJob::STRIPE_ACCOUNT_INACTIVE_AFTER_DURATION", 3.weeks)
      end

  test "collects the balance amount from inactive Stripe US merchant accounts" do
  ...
  test "does not attempt to collect balance from non-US Stripe merchant accounts" do
  ...
  test "does not attempt to collect balance from any PayPal merchant accounts" do
  ...
  test "does not attempt to collect balance from merchant accounts that are considered active based on creation date" do
  ...
  test "does not attempt to collect balance from merchant accounts that are considered active based on user's last payout" do
        us_stripe_account_1 = create(:merchant_account, country: "US", currency: "usd", charge_processor_merchant_id: "acct_1SO1bwI533JwXS4r", created_at: 2.months.ago)
        create(:balance, user: us_stripe_account_1.user, merchant_account: us_stripe_account_1, amount_cents: 100_00)
        us_stripe_account_2 = create(:merchant_account, country: "US", currency: "usd", charge_processor_merchant_id: "acct_1SOZihINUYgu4sRU", created_at: 1.month.ago)
        create(:balance, user: us_stripe_account_2.user, merchant_account: us_stripe_account_2, amount_cents: 200_00)
  ...
  test "does not attempt to collect balance from merchant accounts that are considered active based on users' last sale" do
        us_stripe_account_1 = create(:merchant_account, country: "US", currency: "usd", charge_processor_merchant_id: "acct_1SO1bwI533JwXS4r", created_at: 2.months.ago)
        create(:balance, user: us_stripe_account_1.user, merchant_account: us_stripe_account_1, amount_cents: 100_00)
        us_stripe_account_2 = create(:merchant_account, country: "US", currency: "usd", charge_processor_merchant_id: "acct_1SOZihINUYgu4sRU", created_at: 1.month.ago)
        create(:balance, user: us_stripe_account_2.user, merchant_account: us_stripe_account_2, amount_cents: 200_00)
  ...
  test "does not attempt to collect balance from merchant accounts that have already been processed" do
        us_stripe_account_1 = create(:merchant_account, country: "US", currency: "usd", charge_processor_merchant_id: "acct_1SO1bwI533JwXS4r", created_at: 2.months.ago)
        create(:balance, user: us_stripe_account_1.user, merchant_account: us_stripe_account_1, amount_cents: 100_00)
        us_stripe_account_2 = create(:merchant_account, country: "US", currency: "usd", charge_processor_merchant_id: "acct_1SOZihINUYgu4sRU", created_at: 1.month.ago)
        create(:balance, user: us_stripe_account_2.user, merchant_account: us_stripe_account_2, amount_cents: 200_00)
  ...
  test "does not attempt to collect balance if the Stripe account is considered active based on creation date" do
  ...
  test "does not attempt to collect balance if the Stripe account is considered active based on last payout date" do
        # Freeze time to keep VCR cassette timestamps (recorded Feb 3, 2026) within the stubbed 3-week
        # STRIPE_ACCOUNT_INACTIVE_AFTER_DURATION window. Without this, the test breaks as real time drifts
        # past the cassette dates.
        travel_to(Date.new(2026, 2, 11)) do
  ...
  test "does not attempt to collect balance if the Stripe account is considered active based on last charge date" do
        # Freeze time to keep VCR cassette timestamps (recorded Feb 3, 2026) within the stubbed 3-week
        # STRIPE_ACCOUNT_INACTIVE_AFTER_DURATION window. Without this, the test breaks as real time drifts
        # past the cassette dates.
        travel_to(Date.new(2026, 2, 11)) do
  ...
  test "does not attempt to collect balance if the Stripe account is a standard Stripe account" do
