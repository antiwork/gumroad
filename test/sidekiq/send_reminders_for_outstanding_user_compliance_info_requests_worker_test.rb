...
class SendRemindersForOutstandingUserComplianceInfoRequestsWorkerTest < ActiveSupport::TestCase
  self.described_class = SendRemindersForOutstandingUserComplianceInfoRequestsWorker



  context_ SendRemindersForOutstandingUserComplianceInfoRequestsWorker do
  context_ "#perform" do
  ...
  test "reminds a user of all the outstanding requests, if they have never been reminded" do
  ...
  test "does not remind a user who was just reminded" do
        expect(user_2_request_1.emails_sent_at).to eq([Time.current])
      end

  test "does not remind a user who was reminded less than 2 days ago" do
        expect(user_3_request_1.emails_sent_at).to eq([1.day.ago])
      end

  test "does not remind a user who was reminded 2 days ago" do
        expect(user_4_request_1.emails_sent_at).to eq([2.days.ago])
      end

  test "reminds a user who was reminded more than 2 days ago" do
  ...
  test "does not remind a user who was reminded twice already" do
        expect(user_7_request_1.emails_sent_at).to eq([10.days.ago, 8.days.ago])
      end

  test "does not remind a deleted user" do
        expect(user_8_request_1.emails_sent_at).to be_empty
      end

  test "does not remind a suspended user" do
        expect(user_9_request_1.emails_sent_at).to be_empty
      end
    end

  context_ "singapore identity verification requests", :vcr do
      before do
        @user = create(:user)
  ...
  test "reminds a user of the outstanding request if they have never been reminded" do
        freeze_time do
          described_class.new.perform
  ...
  test "does not remind a user who was reminded less than 7 days ago" do
        freeze_time do
  ...
  test "reminds a user who was reminded more than 7 days ago" do
        freeze_time do
  ...
  test "does not remind a user if their account was created more than 120 days ago" do
        user = create(:user)
  ...
  test "does not remind if user's current stripe account is not a singapore account" do
        user = create(:user)
        sg_verification_request = create(:user_compliance_info_request, field_needed: UserComplianceInfoFields::Individual::STRIPE_ENHANCED_IDENTITY_VERIFICATION, user:)
