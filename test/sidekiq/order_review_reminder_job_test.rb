...
class OrderReviewReminderJobTest < ActiveSupport::TestCase
  self.described_class = OrderReviewReminderJob



  context_ OrderReviewReminderJob do
  ...
  context_ "when there are no eligible purchases" do
      before do
        allow(order).to receive(:purchases).and_return([ineligible_purchase])
      end

  test "does not enqueue any emails" do
        expect do
  ...
  context_ "when there is one eligible purchase" do
      before do
  ...
  context_ "when there are multiple eligible purchases" do
  ...
  test "enqueues an order review reminder once" do
        expect do
          described_class.new.perform(order.id)
          described_class.new.perform(order.id)
