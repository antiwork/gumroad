...
class ProductsHelperTest < ActionView::TestCase
  ...
  context_ ProductsHelper, "url_for_product_page with offer codes" do
  ...
  context_ "when offer_code is BLACKFRIDAY2025" do
  context_ "when on user's domain" do
  ...
  context_ "when not on user's domain" do
  ...
  test "includes other parameters except query" do
          url = url_for_product_page(
            product,
            request:,
            offer_code: "BLACKFRIDAY2025",
  ...
  context_ "when offer_code is nil" do
      let(:request) { instance_double(ActionDispatch::Request, host: "test.gumroad.com", host_with_port: "test.gumroad.com:1234", protocol: "http") }

  test "does not add code parameter to URL" do
  ...
  context_ "when offer_code is empty string" do
      let(:request) { instance_double(ActionDispatch::Request, host: "test.gumroad.com", host_with_port: "test.gumroad.com:1234", protocol: "http") }
