...
class ProductsHelperTest < ActionView::TestCase
  self.described_class = ProductsHelper
  tests ProductsHelper



  context_ ProductsHelper do
  context_ "#view_content_button_text" do
  test "shows the custom button text when available" do
  ...
  test "shows the correct default button text" do
        product = create(:product)
  ...
  test "returns correct cloudfront url for gumroad-specs bucket" do
  ...
  test "returns correct cloudfront url for gumroad-staging bucket" do
  ...
  test "returns unchanged s3 url for other bucket" do
  ...
  test "properly sorts and paginates products" do
  ...
  context_ "#url_for_product_page" do
      let(:product) { create(:product) }

      shared_examples "long url" do
  test "returns the long_url" do
  ...
  context_ "when offer_code is present" do
  test "returns the long_url with the offer_code" do
  ...
  context_ "when recommended_by is present" do
  test "returns product link request's host and port, and includes recommended_by param" do
  ...
  context_ "when recommended_by is not present" do
  test "returns product link with request's host and port" do
  ...
  context_ "when no request exists" do
        before do
  ...
  context_ "when host is DOMAIN" do
        before do
  ...
  context_ "when host is VALID_DISCOVER_REQUEST_HOST" do
        before do
  ...
  context_ "when on the user's subdomain" do
        before do
  ...
  context_ "when on the user's custom domain" do
        before do
  ...
  context_ "when on a different user's subdomain" do
  ...
  context_ "when on a different user's custom domain" do
        before do
          @request.host = "example.com"
  ...
  context_ "#variant_names_displayable" do
  context_ "when no names are provided" do
  test "returns nil" do
          expect(helper.variant_names_displayable([])).to eq nil
        end
      end

  context_ "when only Untitled is provided" do
  test "returns nil" do
          expect(helper.variant_names_displayable([])).to eq nil
        end
      end

  context_ "when provided wit names" do
  test "returns the names joined" do
