...
class PayoutsHelperTest < ActionView::TestCase
  ...
  test "returns the proper sales amount for the current payout period given sales that span 2 payout periods" do
        @user = create(:singaporean_user_with_compliance_info, payment_address: "balance@gumroad.com")
  ...
  test "returns the proper data for the current payout period given sales that span 2 payout periods and one old payment" do
  ...
  test "returns the proper date for the current payout period given sales that span 2 payout periods \
  ...
  test "returns the proper date for an old payment" do
        @user = create(:singaporean_user_with_compliance_info, payment_address: "balance@gumroad.com")
        link = create(:product, user: @user, price_cents: 120_00)
  ...
  context_ "displayable_payout_period_range" do
  ...
  test "renders the correct displayable_payout_period_range when 2 payouts have the same date" do
  ...
  context_ "refund for affiliate credits" do
  test "makes sure that balances are correct for seller and affiliate user when an affiliate credit \
  ...
  context_ "payout period data" do
  test "shows minimum payout volume for user without enough balance" do
        user = create(:user)
  ...
  test "shows payout data without payout given and no previous payouts" do
        travel_to(Time.find_zone("UTC").local(2015, 3, 1)) do
          user = create(:user)
  ...
  test "shows payout data without payout given and previous payouts" do
        travel_to(Time.find_zone("UTC").local(2015, 3, 1)) do
          user = create(:user)
          create(:ach_account, user:)
  ...
  test "shows user's payout data with payout given and previous payouts" do
        travel_to(Time.find_zone("UTC").local(2015, 3, 1)) do
          user = create(:user)
  ...
  test "shows user's affiliate credits and fees separately", :vcr do
  ...
  test "includes the payout fee for instant payout under the fees head" do
        travel_to(Time.find_zone("UTC").local(2015, 3, 1)) do
          user = create(:user)
  ...
  test "shows the payout fee for instant payout under the direct fees head even if there is no other fee" do
        travel_to(Time.find_zone("UTC").local(2015, 3, 1)) do
          user = create(:user)
          payment = create(:payment,
                           user:,
