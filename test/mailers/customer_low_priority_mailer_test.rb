...
class CustomerLowPriorityMailerTest < ActionMailer::TestCase
  self.described_class = CustomerLowPriorityMailer
  tests CustomerLowPriorityMailer



  context_ CustomerLowPriorityMailer do
  ...
  test "notifies user of subscription cancellation" do
  ...
  test "sets the correct SendGrid account with creator's mailer_level" do
          stub_const(
            "EMAIL_CREDENTIALS",
            {
  ...
  test "notifies user of installment plan pause" do
          mail = CustomerLowPriorityMailer.subscription_autocancelled(subscription.id)
  ...
  test "confirms with user that they meant to cancel their subscription" do
        subscription = create(:subscription, link: @product)
  ...
  context_ "subscription_cancelled_by_seller" do
  context_ "memberships" do
        before do
          @product = create(:membership_product, name: "fan club", subscription_duration: "monthly")
          @subscription = create(:subscription, link: @product)
  ...
  test "notifies customer that seller cancelled their subscription" do
  ...
  test "notifies customer that seller cancelled their installment plan" do
          mail = CustomerLowPriorityMailer.subscription_cancelled_by_seller(subscription.id)
  ...
  context_ "subscription_card_declined" do
  test "lets user know their card was declined" do
  ...
  context_ "subscription_card_declined_warning" do
  test "reminds user their card was declined" do
        subscription = create(:subscription, link: create(:product))
        create(:purchase, is_original_subscription_purchase: true, link: subscription.link, subscription:)
  ...
  context_ "subscription_renewal_reminder" do
  context_ "memberships" do
  test "reminds the user they will be charged for their subscription soon", :vcr do
  ...
  test "shows the upcoming renewal price instead of the original signup price" do
          purchase = create(:membership_purchase)
          subscription = purchase.subscription
  ...
  test "reminds the user they will be charged for their installment payment soon" do
          mail = CustomerLowPriorityMailer.subscription_renewal_reminder(subscription.id)
  ...
  context_ "subscription_price_change_notification" do
  ...
  test "notifies the subscriber the price will be changing soon" do
  ...
  test "includes tax details if applicable" do
  ...
  test "includes custom message if set" do
  ...
  test "uses the correct tense if effective date is in the past" do
  ...
  context_ "chargeback_notice_to_customer" do
  ...
  context_ "sample_subscription_price_change_notification" do
      let(:effective_date) { 1.month.from_now.to_date }
      let(:product) { create(:membership_product) }
      let(:tier) { product.default_tier }

  test "sends the user a sample price change notification" do
  ...
  test "includes a custom message, if present" do
  ...
  test "includes the charge count for a fixed-length membership" do
  ...
  context_ "subscription_charge_failed" do
  context_ "memberships" do
  test "notifies the customer their charge failed" do
          subscription = create(:subscription, link: create(:product))
          create(:purchase, is_original_subscription_purchase: true, link: subscription.link, subscription:)
  ...
  test "notifies the customer their installment payment failed" do
          mail = CustomerLowPriorityMailer.subscription_charge_failed(subscription.id)
  ...
  test "has the correct text for a membership ending" do
  ...
  test "notifies the customer their installment plan has been paid in full" do
          mail = CustomerLowPriorityMailer.subscription_ended(subscription.id)
  ...
  context_ "#subscription_early_fraud_warning_notification" do
      let(:purchase) do
        create(
          :membership_purchase,
  ...
  context_ "#subscription_giftee_added_card", vcr: true do
  ...
  context_ "without tracking" do
        before do
          @shipment = create(:shipment, purchase: @purchase, ship_state: :shipped)
        end

  test "mails to and reply-to the correct email" do
  ...
  test "has the link name in the body" do
          mail = CustomerLowPriorityMailer.order_shipped(@shipment.id)
          expect(mail.body.encoded).to include @product.name
        end
      end

  context_ "with tracking" do
        before do
  ...
  test "has the correct link name in the body" do
          mail = CustomerLowPriorityMailer.order_shipped(@shipment.id)
          expect(mail.body.encoded).to include @product.name
        end

  test "has the tracking url in the body" do
          mail = CustomerLowPriorityMailer.order_shipped(@shipment.id)
  ...
  context_ "free_trial_expiring_soon" do
  test "notifies the customer that their free trial is expiring soon" do
  ...
  context_ "expiring credit card membership", :vcr do
  test "notifies the customer that their credit card is expiring" do
  ...
  context_ "preorder_card_declined" do
  test "notifies the customer that the preorder charge was declined" do
  ...
  context_ "#bundle_content_updated" do
      let(:purchase) { create(:purchase) }

      before { purchase.seller.update!(name: "Seller") }

  test "notifies the customer that the bundle content has been updated" do
  ...
  context_ "#purchase_review_reminder" do
      let(:purchase) { create(:purchase, full_name: "Buyer") }

  context_ "purchase does not have review" do
  ...
  context_ "purchase has review" do
  ...
  context_ "#order_review_reminder" do
      let(:purchase) { create(:purchase, full_name: "Buyer") }
  ...
  context_ "when the follower has just unsubscribed" do
  ...
  test "notifies the customer their subscription has been canceled" do
  ...
  test "notifies the customer their installment plan has been canceled" do
          mail = CustomerLowPriorityMailer.subscription_product_deleted(subscription.id)
          expect(mail.subject).to eq "Your installment plan has been canceled."
