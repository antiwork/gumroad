...
class AffiliateMailerTest < ActionMailer::TestCase
  self.described_class = AffiliateMailer
  tests AffiliateMailer



  context_ AffiliateMailer do
  context_ "#notify_affiliate_of_sale" do
  ...
  test "sends email to affiliate" do
  ...
  test "tells them they will receive a commission for recurring charges" do
  ...
  context_ "for a free trial purchase" do
  test "clarifies when they will receive their affiliate credits" do
  ...
  test "shows the correct affiliate commission for the purchased product" do
  ...
  context_ "for a global affiliate" do
  ...
  test "notifies collaborator of a sale" do
          mail = AffiliateMailer.notify_affiliate_of_sale(purchase.id)
  ...
  test "includes variant information if the purchase is for a variant" do
  ...
  context_ "#notify_direct_affiliate_of_updated_products" do
  test "sends email to affiliate" do
        ...
        expect(mail_plaintext).not_to include("The Road of Gum")
  ...
  context_ "#direct_affiliate_removal" do
  test "sends email to affiliate" do
        seller = create(:named_user)
  ...
  context_ "#collaboration_ended_by_affiliate_user" do
  test "sends email to seller" do
  ...
  context_ "#direct_affiliate_invitation" do
      let(:seller) { create(:named_seller) }
      let(:product) { create(:product, user: seller) }

      before { create(:product, name: "Unaffiliated product that we ignore", user: seller) }

  context_ "when affiliate has just a single product" do
  context_ "when affiliate destination URL is set" do
  test "sends email to affiliate with the affiliated product's URL and destination URL" do
  ...
  context_ "when affiliate destination URL is not set" do
  test "sends email to affiliate with the affiliated product's URL" do
            direct_affiliate = create(:direct_affiliate, seller:)
            create(:product_affiliate, product:, affiliate: direct_affiliate, destination_url: "https://example2.com")
            ...
            expect(mail.body.encoded).not_to include("— after they click, we'll redirect them to")
          end
        end

  context_ "when affiliate has a destination URL but product affiliate does not" do
  test "uses the model fallback URL instead of nil" do
            direct_affiliate = create(:direct_affiliate, seller:, destination_url: "https://example.com")
  ...
  context_ "when affiliate has multiple products" do
  test "sends email to affiliate with the affiliated products' URLs" do
  ...
  context_ "when prevent_sending_invitation_email_to_seller param is true" do
  test "doesn't send the email to seller" do
  ...
  context_ "#notify_direct_affiliate_of_new_product" do
  test "sends email to affiliate" do
        seller = create(:named_user)
        direct_affiliate = create(:direct_affiliate, seller:)
  ...
  context_ "#collaborator_creation" do
      let(:seller) { create(:named_user) }
      let(:collaborator) { create(:collaborator, seller:) }

  test "sends email to collaborator" do
  ...
  context_ "when the collaborator has more than 5 products" do
  ...
  test "sends email to collaborator and truncates the product list" do
          mail = AffiliateMailer.collaborator_creation(collaborator.id)
          expect(mail.to).to eq([collaborator.affiliate_user.form_email])
          subject = "#{seller.name} has added you as a collaborator on Gumroad"
          expect(mail.subject).to eq(subject)
  ...
  context_ "#collaborator_update" do
      let(:seller) { create(:named_user) }
      let(:collaborator) { create(:collaborator, seller:) }
  ...
  test "sends email to collaborator and truncates the product list" do
          ...
          expect(mail.body.encoded).not_to have_text("What is a collaborator?")
  ...
  context_ "#collaboration_ended_by_seller" do
      let(:seller) { create(:named_user) }
      let(:collaborator) { create(:collaborator, seller:) }
  ...
  context_ "#collaborator_invited" do
      let(:seller) { create(:named_user) }
      let(:collaborator) { create(:collaborator, seller:) }
  ...
  context_ "#collaborator_invitation_accepted" do
      let(:collaborator) { create(:collaborator) }

  test "sends email to the seller" do
  ...
  context_ "#collaborator_invitation_declined" do
      let(:collaborator) { create(:collaborator) }
