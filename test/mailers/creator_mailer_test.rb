...
class CreatorMailerTest < ActionMailer::TestCase
  ...
  context_ "#gumroad_day_fee_saved" do
  test "includes details of fee saved on Gumroad day" do
  ...
  test "includes the correct text and product links" do
  ...
  context_ "when the seller is earning in USD" do
  test "includes the correct text and product links" do
  ...
  context_ "when the seller is earning in GBP" do
        before do
          seller.update!(currency_type: "gbp")
        end

  test "converts the total amount to GBP" do
          mail = CreatorMailer.year_in_review(seller:, year:, analytics_data:)
  ...
  context_ "#scheduled_payout_chargeback_hold" do
      let(:user) { create(:user) }
      let(:scheduled_payout) { create(:scheduled_payout, user: user, action: "payout") }

  test "sends chargeback hold notification email" do
  ...
  test "does not send if scheduled payout does not exist" do
