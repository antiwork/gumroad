...
class OneOffMailerTest < ActionMailer::TestCase
  self.described_class = OneOffMailer
  tests OneOffMailer



  context_ OneOffMailer do
  ...
  test "doesn't send attempt to email if both email and id are not provided" do
  ...
  test "doesn't send email if email address provided is invalid" do
  ...
  test "allows safe html tags and strips unsafe ones" do
  ...
  test "sets reply_to header if provided" do
  ...
  test "uses default from email when from parameter is not provided" do
        mail = described_class.email(email:, subject:, body:)
        expect(mail.from).to eq(["hi@#{CUSTOMERS_MAIL_DOMAIN}"])
      end

  test "uses custom from email when from parameter is provided" do
  ...
  test "uses default sender_domain (customers) when sender_domain parameter is not provided" do
  ...
  test "uses custom sender_domain when sender_domain parameter is provided" do
  ...
  context_ "#email_using_installment" do
  ...
  test "does not show the unsubscribe link" do
        mail = described_class.email_using_installment(email:, installment_external_id:)
