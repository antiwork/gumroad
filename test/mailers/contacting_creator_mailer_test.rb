...
class ContactingCreatorMailerTest < ActionMailer::TestCase
  self.described_class = ContactingCreatorMailer
  tests ContactingCreatorMailer



  context_ ContactingCreatorMailer do
  ...
  test "uses SUPPORT_EMAIL_WITH_NAME as default from address" do
      expect(described_class.default[:from]).to eq(ApplicationMailer::SUPPORT_EMAIL_WITH_NAME)
    end

  context_ "cannot pay" do
      before { @payment = create(:payment) }

  test "sends notice to the payment user" do
  ...
  context_ "purchase refunded" do
  test "sends notification to the seller about refunded purchase" do
  ...
  context_ "purchase refunded for fraud" do
  test "sends notification to the seller about purchase refunded for fraud" do
        purchase = create(:purchase, link: create(:product, name: "Digital Membership"), email: "test@example.com", price_cents: 10_00)
  ...
  test "sends chargeback notice correctly" do
  ...
  context_ "when the seller is contacted to submit evidence" do
          let!(:dispute_evidence) do
            create(:dispute_evidence, dispute:)
          end

  test "includes copy to submit evidence" do
            mail = ContactingCreatorMailer.chargeback_notice(dispute.id)
  ...
  context_ "when the purchase was done via a PayPal Connect account" do
          before do
            purchase.update!(charge_processor_id: PaypalChargeProcessor.charge_processor_id)
          end

  test "includes copy about PayPal's dispute process" do
            mail = ContactingCreatorMailer.chargeback_notice(dispute.id)
  ...
  context_ "for a dispute on Charge" do
        let(:charge) do
  ...
  context_ "preorder_release_reminder" do
  ...
  test "sends the email with the the correct text" do
          ...
          expect(email.body.encoded).not_to include "You will need to upload a file before its release, or we won't be able to release the product and charge your customers"
        end
      end

  context_ "for a non-physical product" do
  context_ "when the product does not have delivery content saved" do
          before do
  ...
  context_ "when the product has delivery content saved" do
          before do
  ...
  test "sends an update to the seller" do
  ...
  test "does not show any new subscriptions" do
          ...
          expect(mail.body).to include @product.name
          expect(mail.body).to include "1 existing subscription"
          expect(mail.body).not_to include "new subscription"
        end
      end
  ...
  context_ "credit_notification" do
      before do
        @user = create(:user)
      end

  test "notifies user about credit to their account" do
  ...
  context_ "gumroad_day_credit_notification" do
      before do
        @user = create(:user)
      end
  ...
  test "uses SUPPORT_EMAIL as from address" do
  ...
  test "displays the resolved tiered offer code discount" do
  ...
  test "works without a purchaser" do
  ...
  test "does not work without a buyer email" do
  ...
  test "includes discover notice and sets the referrer to Gumroad Discover" do
        seller = create(:user, email: "bob@gumroad.com")
        buyer = create(:user, email: "bob2@gumroad.com")
  ...
  test "sets the referrer to Direct when the referrer URL equals 'direct'" do
        seller = create(:user, email: "bob@gumroad.com")
  ...
  test "sets the referrer to Profile when the referrer URL is from the seller's profile" do
        seller = create(:user, email: "bob@gumroad.com")
        product = create(:product, user: seller)
  ...
  test "sets the referrer to Twitter when the referrer URL is from twitter" do
        seller = create(:user, email: "bob@gumroad.com")
        product = create(:product, user: seller)
  ...
  test "includes quantity if greater than 1" do
        seller = create(:user, email: "bob@gumroad.com")
        buyer = create(:user, email: "bob2@gumroad.com")
        link = create(:physical_product, user: seller)
  ...
  test "includes product price, shipping cost, and total transaction" do
        seller = create(:user, email: "bob@gumroad.com")
        buyer = create(:user, email: "bob2@gumroad.com")
        link = create(:physical_product, user: seller)
  ...
  test "includes product price, nonzero tax, and total transaction" do
        seller = create(:user, email: "bob@gumroad.com")
        buyer = create(:user, email: "bob2@gumroad.com")
        link = create(:product, user: seller)
  ...
  test "includes product price, nonzero VAT, and total transaction" do
        seller = create(:user, email: "bob@gumroad.com")
        buyer = create(:user, email: "bob2@gumroad.com")
        link = create(:product, user: seller)
        purchase = create(:purchase,
  ...
  test "includes product price, nonzero VAT inclusive, and total transaction" do
        seller = create(:user, email: "bob@gumroad.com")
        buyer = create(:user, email: "bob2@gumroad.com")
        link = create(:product, user: seller)
        purchase = create(:purchase,
  ...
  test "does not include tax if it is 0" do
        seller = create(:user, email: "bob@gumroad.com")
        buyer = create(:user, email: "bob2@gumroad.com")
        link = create(:product, user: seller)
        purchase = create(:purchase,
        ...
        expect(mail.body.encoded).to include "Product price"
        expect(mail.body.encoded).to include "$1"
        expect(mail.body.encoded).not_to include "Sales tax"
        expect(mail.body.encoded).to include "Order total"
        expect(mail.body.encoded).to include "$1"
  ...
  test "includes product price, shipping cost, and total transaction for jpy" do
        seller = create(:user, email: "bob@gumroad.com")
        buyer = create(:user, email: "bob2@gumroad.com")
  ...
  test "includes discount information" do
        seller = create(:user, email: "bob@gumroad.com")
  ...
  test "includes upsell information without offer code" do
        seller = create(:user, email: "bob@gumroad.com")
  ...
  test "includes upsell information with offer code" do
        seller = create(:user, email: "bob@gumroad.com")
  ...
  test "includes variant information" do
        seller = create(:user, email: "bob@gumroad.com")
        product = create(:product_with_digital_versions, user: seller)
  ...
  test "includes tier information" do
          seller = create(:user, email: "bob@gumroad.com")
  ...
  test "includes the affiliate commission information when 'apply_to_all_products' is true" do
        seller = create(:user, email: "bob@gumroad.com")
  ...
  test "includes the product commission information when 'apply_to_all_products' is false" do
        seller = create(:user, email: "bob@gumroad.com")
        product = create(:product, user: seller, price_cents: 10_00)
        affiliate_user = create(:affiliate_user)
  ...
  test "does not include affiliate information when price is $0" do
        seller = create(:user, email: "bob@gumroad.com")
        product = create(:product, user: seller, price_cents: 0, customizable_price: true)
        affiliate_user = create(:affiliate_user)
        ...
        purchase.update_balance_and_mark_successful!
        mail = ContactingCreatorMailer.notify(purchase.id)
        expect(mail.body.encoded).not_to include "Affiliate commission"

        expect_push_alert(seller.id, mail.subject)
      end

  context_ "for a collab product" do
  ...
  test "includes the collaborator information" do
          mail = ContactingCreatorMailer.notify(purchase.id)
  ...
  context_ "when send_creator_notifications_to_consumer_app feature flag is enabled" do
        before do
          Feature.activate(:send_sales_notifications_to_consumer_app)
        end

  test "sends sale notification to both creator and consumer apps" do
          mail = ContactingCreatorMailer.notify(purchase.id)
  ...
  test "adds a row displaying the giftee's email" do
          seller = create(:user, email: "bob@gumroad.com")
          product = create(:product, user: seller)
  ...
  context_ "when the purchase is via staff picks" do
        before do
          purchase.update!(recommended_by: RecommendationType::GUMROAD_STAFF_PICKS_RECOMMENDATION)
        end

  test "includes subheading about staff picks" do
          mail = ContactingCreatorMailer.notify(purchase.id)
  ...
  context_ "when the purchase is via more like this" do
        before do
  ...
  test "includes subheading about more like this" do
          mail = ContactingCreatorMailer.notify(purchase.id)
          expect(mail.body.encoded).to include "via More like this recommendations"
        end

  test "includes more like this as the referrer" do
          mail = ContactingCreatorMailer.notify(purchase.id)
          expect(mail.body.encoded).to include "Referrer"
  ...
  context_ "for original purchase" do
  test "renders the correct subject" do
  ...
  context_ "for recurring purchase" do
  test "renders the correct subject" do
  ...
  context_ "for upgrade purchase" do
  test "renders the correct subject" do
            purchase = create(:recurring_membership_purchase, is_original_subscription_purchase: false)
  ...
  test "works without a purchase" do
          seller = create(:user)
          link = create(:product, user: seller)
  ...
  test "works without a purchase with variants" do
          seller = create(:user)
          link = create(:product, user: seller)
  ...
  test "works without a purchase with shipping info" do
          seller = create(:user)
  ...
  test "works with a price range" do
          seller = create(:user)
          link = create(:product, user: seller)
  ...
  context_ "payment notification settings" do
          before do
  ...
  context_ "with default notification settings" do
  test "sends email and push notification" do
              mail = ContactingCreatorMailer.notify(@purchase.id)
              expect(mail.subject).to eq "New sale of #{@link.name} for #{@purchase.formatted_total_price}"
  ...
  context_ "with email notifications disabled" do
            before do
              @user.enable_payment_email = false
              @user.save!
            end

  test "does not sends email but sends push notification" do
              mail = ContactingCreatorMailer.notify(@purchase.id)
  ...
  context_ "with push notifications disabled" do
              before do
                @user.enable_payment_push_notification = false
                @user.save!
              end

  test "does not sends email or push notifications" do
                mail = ContactingCreatorMailer.notify(@purchase.id)
                expect(mail.message).to be_a(ActionMailer::Base::NullMail)
  ...
  context_ "pre-orders" do
  test "delivers notification with pre order subject" do
          creator = create(:user)
  ...
  test "excludes quantity and variant from email" do
  ...
  context_ "call products" do
        let(:variant_category) { call.link.variant_categories.first }

  context_ "call that starts and ends on the same day" do
  ...
  test "includes call information in the email" do
  ...
  context_ "call that spans multiple days" do
  ...
  test "includes call information for multi-day calls" do
            mail = ContactingCreatorMailer.notify(call.purchase.id)
  ...
  context_ "purchase with tip", :vcr do
        before { purchase.create_tip(value_cents: 100, value_usd_cents: 100) }

  test "includes the tip details" do
          mail = ContactingCreatorMailer.notify(purchase.id)
  ...
  test "includes the commission details" do
  ...
  test "includes the UTM link driven sale details" do
  ...
  context_ "subscription_cancelled" do
  ...
  context_ "subscription_autocancelled" do
  context_ "memberships" do
        before do
  ...
  test "includes the purchase failure if available" do
  ...
  test "includes non-paypal purchase failure if available" do
  ...
  context_ "subscription_downgraded" do
  test "has the correct text" do
  ...
  context_ "subscription_restarted" do
  context_ "memberships" do
  test "has the correct text" do
          product = create(:membership_product_with_preset_tiered_pricing)
          purchase = create(:membership_purchase, link: product, variant_attributes: [product.default_tier])
  ...
  context_ "subscription_product_deleted" do
  context_ "memberships" do
  ...
  context_ "subscription_ended" do
  context_ "memberships" do
  ...
  context_ "unremovable_discord_member" do
      let(:seller) { create(:user) }
      let(:product) { create(:product, user: seller) }
      let(:purchase) { create(:purchase, seller:, link: product) }

  test "has the correct subject and content" do
  ...
  context_ "unstampable_pdf_notification" do
      let(:product) { create(:product) }
  ...
  context_ "chargeback_lost_no_refund_policy" do
      let(:seller) { create(:user) }
  ...
  context_ "physical preorders" do
        before do
  ...
  test "sends the correct summary email", :vcr do
          ...
          expect(mail.body.encoded).not_to include "Unfortunately"
        end

  test "includes the copy for the failed charges in the email", :vcr do
  ...
  test "does not send email if the preorder had no sales", :vcr do
          mail = ContactingCreatorMailer.preorder_summary(@preorder_product.id)
          expect(mail.subject).to eq nil
        end
      end

  context_ "digital preorders" do
  test "sends the correct summary email", :vcr do
  ...
  context_ "user_sales_data" do
  test "contains the correct text, attachment, and attributes" do
        user = create(:user)
  ...
  context_ "payout_data" do
      let(:recipient) { create(:user) }
      let(:attachment_name) { "payout_data_#{SecureRandom.hex}.csv" }
      let(:extension) { "csv" }
      let(:tempfile) { Tempfile.new }

  context_ "when file can be attached directly" do
        before do
  ...
  test "contains the correct attachment and attributes" do
  ...
  context_ "when file is too large and a URL is provided instead" do
  ...
  test "contains the download URL in the email body" do
          mail = ContactingCreatorMailer.payout_data(attachment_name, extension, tempfile, recipient.id)
  ...
  context_ "when attachment size is above threshold" do
        before do
          stub_const("MailerAttachmentOrLinkService::MAX_FILE_SIZE", 1.byte)
        end

  test "contains a link instead of an attachment" do
          expect(mail.to).to eq([@recipient.email])
          expect(mail.subject).to include("Here is your affiliates data")
  ...
  context_ "video_transcode_failed" do
      before do
  ...
  test "has the correct text when product file has display_name set" do
  ...
  test "has the correct text when product file does not have display_name set" do
  ...
  test "returns early without error when the associated link has been deleted" do
  ...
  context_ "tax_form_1099k" do
  context_ "filed form" do
  test "has the correct subject and body with form download url included" do
          creator = create(:user)
  ...
  context_ "informational not-filed form" do
  test "has the correct subject and body with form download url included" do
          creator = create(:user)
          year = Date.current.year.pred
  ...
  context_ "#singapore_identity_verification_reminder" do
  test "has the correct subject and body with payments settings page url included" do
        creator = create(:user)
  ...
  context_ "#stripe_document_verification_failed" do
  test "has the correct subject and body with payments settings page url included" do
        creator = create(:user)
  ...
  context_ "#stripe_identity_verification_failed" do
  test "has the correct subject and body with payments settings page url included" do
        creator = create(:user)
  ...
  context_ "#review_submitted" do
      let(:review) { create(:product_review) }

  test "has the correct subject and body" do
  ...
  context_ "no message" do
        before { review.update!(message: nil) }
  test "omits the quotation marks" do
          mail = ContactingCreatorMailer.review_submitted(review.id)
          expect(mail.body.encoded).not_to have_text('""')
        end
      end

  context_ "when the review has a pending video" do
  ...
  test "includes the video thumbnail" do
          mail = ContactingCreatorMailer.review_submitted(review.id)
  ...
  context_ "#upcoming_call_reminder" do
  context_ "email content", :freeze_time do
  ...
  test "includes the correct information" do
          ...
          expect(mail.body).not_to include("File upload")
  ...
  context_ "when the call is not eligible for reminder" do
  ...
  context_ "#stripe_remediation", :vcr do
      let!(:seller) { create(:named_seller) }
  ...
  context_ "#suspended_due_to_stripe_risk" do
      let(:seller) { create(:named_seller) }
  ...
  context_ "product_level_refund_policies_reverted" do
      let(:seller) { create(:user) }

  test "sends the email correctly" do
  ...
  context_ "ping_endpoint_failure" do
      let(:seller) { create(:user, email: "seller@example.com") }
      let(:ping_url) { "https://example.com/webhook" }
      let(:response_code) { 500 }

  test "sends notification to the seller about failed ping endpoint" do
  ...
  test "includes seller information in the email" do
        mail = ContactingCreatorMailer.ping_endpoint_failure(seller.id, ping_url, response_code)

        expect(mail.body.encoded).to include seller.name_or_username
      end

  test "handles different response codes correctly" do
  ...
  test "handles different ping URLs correctly with redaction" do
        test_cases = [
  ...
  test "redacts URL path while preserving protocol and domain" do
  ...
  context_ "when the seller has a pending scheduled payout" do
  test "includes scheduled payout amount and chargeback disclaimer" do
  ...
  context_ "when the seller has a pending scheduled refund" do
  test "includes refund amount" do
  ...
  context_ "when the seller has a pending scheduled hold" do
  test "includes hold amount" do
  ...
  context_ "when the seller has no scheduled payout" do
  test "does not include payout information or chargeback disclaimer" do
          mail = ContactingCreatorMailer.account_suspended(seller.id)
  ...
  context_ "#flagged_for_explicit_nsfw_tos_violation" do
      let(:seller) { create(:named_seller) }
