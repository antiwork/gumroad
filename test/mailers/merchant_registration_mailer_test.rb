...
class MerchantRegistrationMailerTest < ActionMailer::TestCase
  self.described_class = MerchantRegistrationMailer
  tests MerchantRegistrationMailer



  context_ MerchantRegistrationMailer do
  context_ "#account_deauthorized_to_user" do
  ...
  test "shows message for unavailable products" do
  ...
  context_ "#account_needs_registration_to_user" do
      let(:charge_processor_id) { StripeChargeProcessor.charge_processor_id }

  context_ "for an affiliate" do
  ...
  test "advises the affiliate to connect their account" do
  ...
  test "advises the collaborator to connect their account" do
  ...
  context_ "stripe_charges_disabled" do
  test "alerts the user that payments have been disabled" do
        user = create(:user)
  ...
  context_ "stripe_payouts_disabled" do
  test "notifies the user that their payouts have been paused" do
        user = create(:user)
