...
class AffiliateRequestMailerTest < ActionMailer::TestCase
  self.described_class = AffiliateRequestMailer
  tests AffiliateRequestMailer



  context_ AffiliateRequestMailer do
  ...
  context_ "when requester already has an account" do
        let!(:requester) { create(:user, email: requester_email) }

  test "does not ask to create an account" do
          expect(mail.body.encoded).not_to include("In the meantime, <a href=\"#{signup_url(email: requester_email)}\">create your Gumroad account</a> using email #{requester_email} and confirm it. You'll receive your affiliate links once your Gumroad account is active.")
        end
      end
    end

  context_ "notify_requester_of_request_approval" do
      let(:affiliate_request) { create(:affiliate_request, email: requester_email, seller: creator) }
      let!(:requester) { create(:user, email: requester_email) }
  ...
  context_ "notify_requester_of_ignored_request" do
  ...
  context_ "notify_unregistered_requester_of_request_approval" do
      let(:affiliate_request) { create(:affiliate_request, email: requester_email, seller: creator) }
