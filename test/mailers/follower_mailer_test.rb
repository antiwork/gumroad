...
class FollowerMailerTest < ActionMailer::TestCase
  self.described_class = FollowerMailer
  tests FollowerMailer



  context_ FollowerMailer do
  context_ "#confirm_follower" do
      let(:followee) { create(:user) }
      let(:unconfirmed_follower) { create(:follower, user: followee) }

  test "sends email to follower to confirm the follow" do
  ...
  test "sets the correct SendGrid account" do
        stub_const(
          "EMAIL_CREDENTIALS",
          {
