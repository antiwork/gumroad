...
class CustomerMailerTest < ActionMailer::TestCase
  self.described_class = CustomerMailer
  tests CustomerMailer



  context_ CustomerMailer do
  context_ "receipt" do
      subject(:mail) do
  ...
  test "renders the headers for a receipt" do
  ...
  context_ "when support email exists" do
        subject(:mail) do
          user = create(:user, email: "bob@gumroad.com", name: "bob walsh")
  ...
  test "uses correct support email" do
          expect(mail[:reply_to].value).to eq("support@example.com")
        end
      end

  test "renders the headers with UrlRedirect" do
  ...
  context_ "when user name contains special characters" do
        before do
          @user = create(:user)
  ...
  context_ "with Spanish letters" do
  test "sets the FROM name to the creator's name" do
  ...
  context_ "and symbols" do
  test "sets the FROM name to 'Gumroad'" do
  ...
  context_ "with Romanian letters" do
  test "sets the FROM name to the creator's name" do
  ...
  context_ "with Asian letters" do
  test "sets the FROM name to the creator's name" do
  ...
  context_ "with ASCII symbols" do
  test "wraps creator name in quotes" do
  ...
  context_ "with new line characters and leading blank spaces" do
  test "deletes the new lines and strips whitespaces from the creator name" do
            @user.update!(name: "   \nGumbot\n Generalist @ Gumroad   ")
            mail = CustomerMailer.receipt(@purchase.id)
  ...
  test "has no-reply when creator uses messaging" do
  ...
  test "has the right subject for a purchase" do
  ...
  test "has the right subject" do
          expect(CustomerMailer.receipt(purchase.id).subject).to match(/^You got /)
        end

  test "does not include generate invoice section" do
          mail = CustomerMailer.receipt(purchase.id)
          expect(mail.body.encoded).not_to have_link("Generate invoice")
        end
      end

  test "has the right subject for a rental purchase" do
  ...
  test "has the right subject for an original subscription purchase" do
  ...
  context_ "when the purchase is for a recurring subscription" do
  ...
  test "has the right subject for a recurring subscription purchase" do
          expect(mail.subject).to match(/^Recurring charge for /)
        end

  test "has the right subject for a subscription upgrade purchase" do
  ...
  test "includes recurring subscription specific content" do
          mail = CustomerMailer.receipt(purchase.id)
  ...
  test "does not have the download link in the receipt if link has no content" do
        user = create(:user)
        link = create(:product, user:)
        purchase = create(:purchase, link:, seller: link.user)
        ...
        mail = CustomerMailer.receipt(purchase.id)
        expect(mail.body.sanitized).not_to match("Download")
      end

  context_ "Recommended products section" do
  ...
  context_ "when there are no recommended products" do
  test "doesn't include recommended products section" do
            mail = CustomerMailer.receipt(purchase.id)
            expect(mail.body.sanitized).not_to have_text("Customers who bought this item also bought")
          end
        end

  context_ "when there are recommended products" do
  ...
  test "includes recommended products section" do
  ...
  context_ "with membership purchase" do
  ...
  test "has the view content link in the receipt for a membership product" do
          mail = CustomerMailer.receipt(purchase.id)
          expect(mail.body.sanitized).to match("View content")
        end

  context_ "when is original purchase" do
          before do
            purchase.update!(is_original_subscription_purchase: true)
          end

  test "has the view content button and details" do
            mail = CustomerMailer.receipt(purchase.id)
            expect(mail.body.sanitized).to match("View content")
          end
        end

  test "renders subscription receipt" do
          mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "with shipping charge" do
            before do
              purchase.update!(
                shipping_cents: 499,
  ...
  test "renders shipping amount" do
              mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "when purchase is in EUR" do
              before do
  ...
  test "renders correct amounts" do
                mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "with free trial membership purchase" do
          let(:purchase) do
            create(
  ...
  test "renders payment details" do
  ...
  context_ "when the purchase has fixed length" do
  context_ "when there is at least one more remaining charge" do
            before do
              purchase.subscription.update!(charge_occurrence_count: 2)
            end

  test "renders upcoming payment information" do
              mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "when there are no more remaining charges" do
            before do
              purchase.subscription.update!(charge_occurrence_count: 1)
            end

  test "does not render upcoming payment information" do
              mail = CustomerMailer.receipt(purchase.id)
  ...
  test "includes license details" do
            mail = CustomerMailer.receipt(purchase.id)
            expect(mail.body.sanitized).to include license.serial
          end

  context_ "when is not the original purchase" do
  ...
  test "does not include license details" do
  ...
  context_ "when is a multi-seat license" do
            before do
              purchase.update!(
  ...
  test "includes number of seats" do
              mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "when the purchase has custom fields" do
          before do
  ...
  test "includes custom fields details" do
            mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "when the product requires shipping" do
  ...
  test "includes shipping details" do
            mail = CustomerMailer.receipt(purchase.id)
  ...
  test "shows the custom view content text in the receipt" do
  ...
  test "shows the view content link in the receipt for a product having rich content" do
  ...
  test "discloses sales tax when it was taxed" do
        user = create(:user)
        link = create(:product, user:)
  ...
  test "does not render tax or total fields if purchas was not taxed" do
        user = create(:user)
        link = create(:product, user:)
        purchase = create(:purchase, link:, seller: link.user)
        ...
        mail = CustomerMailer.receipt(purchase.id)
        expect(mail.body.sanitized).not_to match("Sales tax")
        expect(mail.body.sanitized).not_to match("Order total")
      end

  test "correctly renders variants" do
        user = create(:user)
        link = create(:product, user:)
        purchase = create(:purchase, link:, seller: link.user)
  ...
  context_ "when is a test purchase" do
        let(:seller) { create(:named_seller) }
  ...
  context_ "shipping display" do
  context_ "product with shipping" do
  ...
  test "shows the shipping line item in the receipt" do
            mail = CustomerMailer.receipt(purchase.id)
            expect(mail.body.sanitized).to match("Shipping")
          end
        end

  context_ "product without shipping" do
          let(:product) { create(:product) }
          let(:purchase) { create(:purchase, link: product) }

  test "does not show the shipping line item in the receipt" do
            mail = CustomerMailer.receipt(purchase.id)
            expect(mail.body.sanitized).not_to match("Shipping")
          end
        end
      end

  context_ "when the purchase has a refund policy" do
  ...
  test "includes the refund policy" do
          mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "subscription with limited duration discount" do
        let(:purchase) do
          create(
            :membership_purchase,
            price_cents: 500,
  ...
  test "includes the correct payment amounts" do
            mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "when the purchase includes tax" do
            before do
  ...
  context_ "commission completion purchase", :vcr do
  ...
  test "includes the correct subject and view content link" do
  ...
  test "includes the correct attributes" do
          ...
          expect(mail.body.sanitized).not_to have_text("Variant")
        end
      end
  ...
  test "includes information for all of the bundle products" do
          mail = CustomerMailer.receipt(purchase.id)
  ...
  context_ "when the purchase includes shipping" do
          let(:product) { create(:physical_product) }
  ...
  test "includes the shipping info section" do
            expect(mail.body.sanitized).to have_text("Shipping info")
          end
        end

  context_ "when the purchase is a membership" do
          let(:product) { create(:membership_product) }
  ...
  context_ "when the recipient email is hidden" do
          before { gift.update!(is_recipient_hidden: true) }

  test "shows the name instead" do
  ...
  context_ "when it is a membership purchase" do
          let(:product) { create(:membership_product) }
  ...
  test "does not includes the manage membership link, only a notice regarding renewal" do
            expect(mail.body.sanitized).not_to have_text("If you would like to manage your membership you can visit subscription settings.")
  ...
  context_ "receipt for multi-items" do
      let(:seller) { create(:named_seller) }
  ...
  test "processes the mailer and generates the correct headers" do
  ...
  context_ "products have the same support emails" do
  ...
  test "use the product support email" do
          expect(mailer[:reply_to].value).to eq(product_support_email)
        end
      end

  context_ "when products have different support emails" do
  ...
  test "renders the abandoned cart email" do
        ...
        expect(sanitized_body).not_to have_text("Product 4")
        expect(sanitized_body).not_to have_text("Another seller product")
  ...
  context_ "when the cart is missing user_id" do
  ...
  test "sends an email" do
          expect do
            ...
            expect(mail.body.sanitized).not_to have_text("S1 Product 2")
  ...
  test "does not send an email if the email is blank" do
  ...
  context_ "when the cart is not abandoned" do
  ...
  context_ "when the cart is abandoned" do
        let(:cart) { create(:cart) }
        let(:stubbed_cart_id) { "stubbed_secure_cart_id" }
  ...
  context_ "when the provided workflow is not published" do
          before do
  ...
  context_ "when the provided workflow is not of 'abaonded_cart' type" do
          before do
  ...
  context_ "when the provided workflow does not have matching abandoned products" do
          before do
  ...
  context_ "when the provided workflow is published and has matching abandoned products" do
  test "sends an email" do
            expect do
              ...
              expect(mail.body.sanitized).to have_text("S1 Product 1")
              expect(mail.body.sanitized).not_to have_text("S1 Product 2")
              expect(mail.body.sanitized).not_to have_text("S2 Product 1")
              expect(mail.body.sanitized).to have_text("Thanks!")
              expect(mail.body).to have_link("Complete checkout", href: checkout_url(host: UrlService.domain_with_protocol, cart_id: stubbed_cart_id))
            end.to change { SentAbandonedCartEmail.count }.by(1)
  ...
  context_ "when more than three products are matched" do
  test "renders the first three products and a link to the rest" do
              ...
              expect(mail.body.sanitized).not_to have_text("S1 Product 4")
            end
          end

  context_ "when 'is_preview' is true" do
  test "does not create SentAbandonedCartEmail records" do
              expect do
  ...
  context_ "when multiple workflows are provided" do
  test "sends a combined email" do
            expect do
              seller1_workflow_installment.update!(name: "Uh oh, you left something in your cart!")
              ...
              expect(mail.body.sanitized).to have_text("S1 Product 1")
              expect(mail.body.sanitized).to have_text("S1 Product 2")
              expect(mail.body.sanitized).not_to have_text("S1 Product 3")
              expect(mail.body.sanitized).not_to have_text("S1 Product 4")
              ...
              expect(mail.body.sanitized).not_to have_text("S2 Product 4")
  ...
  test "renders the review response email" do
  ...
  test "includes details about the upcoming call" do
  ...
  context_ "#subscription_magic_link" do
      let(:purchase) { create(:membership_purchase) }
  ...
  context_ "#paypal_purchase_failed" do
      let(:purchase) { create(:membership_purchase) }
  ...
  context_ "#grouped_receipt" do
      let(:seller) { create(:named_seller) }
      let(:product) { create(:product, user: seller) }
      let(:purchases) { create_list(:purchase, 2, link: product, seller:) }

  test "sends a grouped receipt email" do
  ...
  test "eager loads charges and orders to avoid N+1 queries" do
  ...
  context_ "#partial_refund" do
      let(:seller) { create(:named_seller) }
      let(:product) { create(:product, user: seller) }
      let(:purchase) { create(:purchase, link: product, seller:) }

  test "sends a partial refund email" do
  ...
  context_ "#files_ready_for_download" do
      let(:seller) { create(:named_seller) }
      let(:product) { create(:product, user: seller) }
  ...
  test "sends the email with correct headers and content" do
