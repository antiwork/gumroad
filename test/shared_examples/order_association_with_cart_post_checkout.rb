...
shared_examples_for "order association with cart post checkout" do
...
test "associates the order with the user cart and deletes it" do
      expect do
        call_action
...
test "associates the order with the guest cart and deletes it" do
      expect do
        call_action
      end.to change { Order.count }.from(0).to(1)
