...
shared_context "when is a preorder authorization" do
...
shared_context "when is a gift sender purchase" do
...
shared_context "when the purchase has a license" do
...
shared_context "when the purchase is for a physical product" do
  let(:product) { create(:product, :is_physical) }
  let(:purchase) { create(:physical_purchase, link: product) }
end

shared_context "when the purchase is recurring subscription" do
