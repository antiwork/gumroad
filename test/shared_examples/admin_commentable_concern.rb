...
shared_examples_for "Admin::Commentable" do
...
context_ "when there are comments" do
      let!(:comment1) do
        create(:comment,
...
test "returns all comments in descending order" do
...
test "paginates comments correctly" do
...
test "returns empty array when there are no comments" do
      get :index, params: route_params, format: :json
...
test "creates a new comment for the commentable" do
      expect do
...
test "associates the comment with the current admin user" do
      post :create, params: route_params.merge(comment: valid_comment_params), format: :json
...
test "creates a comment with 'note' type when comment_type is not provided" do
...
test "returns an error when content is too long" do
