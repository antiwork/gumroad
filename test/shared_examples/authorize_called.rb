...
#
shared_examples_for "authorize called for controller" do |policy_klass|
...
#
shared_examples_for "authorize called for action" do |verb, action|
test "calls authorize with correct arguments on #{verb}: #{action}" do
...
# * seller (required)
#
shared_context "with user signed in with given role for seller" do |role|
...
TeamMembership::ROLES.excluding(TeamMembership::ROLE_OWNER).each do |role|
  ...
  #
  shared_context "with user signed in as #{role} for seller" do
...
# * seller (required)
#
shared_context "with switching account to user with given role for seller" do |options|
  let(:user_with_role_for_seller) do
...
TeamMembership::ROLES.excluding(TeamMembership::ROLE_OWNER).each do |role|
  shared_context "with switching account to user as #{role} for seller" do |options = {}|
    options.merge!(role:)
