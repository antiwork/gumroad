...
context_ "when #{audience_type} #{described_class.to_s.downcase}" do
context_ "when no param filters are set" do
      let(:params) do
        {
          bought_products: [],
          bought_variants: [],
...
context_ "when valid param filters are set" do
      let(:params) do
        {
          bought_products: "B",
          bought_variants: ["123"],
...
test "sets the filter attributes" do
...
context_ "when paid_more_than_cents and paid_less_than_cents are given" do
        let(:params) do
          {
...
context_ "when paid more is greater than paid less" do
      let(:params) do
        {
...
test "returns false and adds a base error to the object" do
...
context_ "when created_after is greater than created_before" do
      let(:params) do
        {
          affiliate_products: [],
          bought_products: [],
...
test "returns true and does not add a base error to the object" do
        is_expected.to eq(true)
