...
shared_examples_for "a paginated API" do
test "contains pagination meta in the response body" do
...
test "can paginate and customize the number of items per page" do
    get @action, params: @params
    expect(response.status).to eq(200)
    expect(response.parsed_body[@response_key_name].size).to eq(@records.size)
...
test "returns 400 error if page param is incorrect" do
