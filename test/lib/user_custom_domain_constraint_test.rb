...
class UserCustomDomainConstraintTest < ActiveSupport::TestCase
  self.described_class = UserCustomDomainConstraint



  context_ UserCustomDomainConstraint do
  context_ ".matches?" do
  context_ "when request doesn't come from subdomain or custom domain" do
        before do
  ...
  context_ "when request comes from subdomain" do
        before do
  ...
  context_ "when request comes from custom domain" do
        before do
  ...
  context_ "when request comes from a host that's configured to redirect" do
        before do
