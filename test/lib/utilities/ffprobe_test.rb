...
class FfprobeTest < ActiveSupport::TestCase
  self.described_class = Ffprobe



  context_ Ffprobe do
  context_ "#parse" do
  context_ "when a valid movie file is supplied" do
  ...
  test "has the correct value for #{property}" do
            expect(ffprobe_parsed.public_send(property)).to eq value
          end
        end
      end

  context_ "when a video file with multiple audio streams encoded before the video stream is supplied" do
        #
  ...
  context_ "when an invalid movie file is supplied" do
  test "raises a NoMethodError" do
          expect { Ffprobe.new(fixture_file("sample.epub")).parse }.to raise_error(NoMethodError)
        end
      end

  context_ "when a non-existent file is supplied" do
