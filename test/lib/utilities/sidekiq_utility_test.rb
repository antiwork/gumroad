...
class SidekiqUtilityTest < ActiveSupport::TestCase
  self.described_class = SidekiqUtility



  context_ SidekiqUtility do
    before do
  ...
  test "returns SidekiqUtility object with process_set and timeout_at variables" do
  ...
  context_ "#instance_id" do
  test "returns the instance_id" do
        expect(@sidekiq_utility.send(:instance_id)).to eq "sample_instance_id"
      end
    end

  context_ "#lifecycle_params" do
  test "returns lifecycle_params hash" do
        lifecycle_params = {
  ...
  context_ "#hostname" do
  test "returns hostname of the server" do
        expect(@sidekiq_utility.send(:hostname)).to eq Socket.gethostname
      end
    end

  context_ "#asg_client" do
  test "returns AWS Auto Scaling Group instance" do
  ...
  test "returns the sidekiq process" do
        expect(@sidekiq_utility.send(:sidekiq_process)["hostname"]).to eq "test1"
      end
    end

  context_ "proceed_with_instance_termination" do
  test "completes lifecycle ation" do
  ...
  context_ "#timeout_exceeded?" do
  test "returns true if timeout is exceeded" do
  ...
  context_ "when timeout is not exceeded" do
        before do
  ...
  test "records the lifecycle heartbeat until the timeout exceeds" do
  ...
  context_ "when all jobs in the worker belong to ignored classes" do
        before do
          workers = [
  ...
  test "logs the stuck jobs and breaks the loop" do
  ...
  context_ "when not all jobs in the worker belong to ignored classes" do
        before do
          workers = [
            ["test_identity", "worker1", { "payload" => { "class" => "HandleSendgridEventJob" }.to_json }],
  ...
  test "continues the loop and records the lifecycle heartbeat" do
