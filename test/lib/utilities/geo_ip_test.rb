...
class GeoIpTest < ActiveSupport::TestCase
  ...
  context_ "an IP to location match is not possible" do
        let(:ip) { "127.0.0.1" }

  test "returns a nil result" do
          expect(result).to eq(nil)
        end
      end

  context_ "an IP to location match is possible" do
  ...
  context_ "an IPv6 to location match is possible" do
  ...
  context_ "an IP to location match is possible but the underlying GEOIP has invalid UTF-8 in fields" do
        let(:ip) { "104.193.168.19" }
