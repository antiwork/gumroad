...
class DevToolsTest < ActiveSupport::TestCase
  self.described_class = DevTools



  context_ DevTools do
    # Very basic tests just to guard for basic runtime errors (method not found, etc).
  context_ ".reindex_all_for_user" do
  test "succeeds after indexing at least one record" do
  ...
  test "does not delete the indices" do
        product = create(:product)
        Link.__elasticsearch__.create_index!(force: true)
  ...
  context_ ".delete_all_indices_and_reindex_all" do
  test "succeeds after indexing at least one record" do
        product = create(:product)
  ...
  test "does not execute in production" do
  ...
  context_ ".reimport_follower_events_for_user!" do
      let(:index_class) { ConfirmedFollowerEvent }
      let(:index_name) { index_class.index_name }

  test "delete all events and creates events for every confirmed follower" do
        user = create(:user)
