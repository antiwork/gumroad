...
class PaymentStatsTest < ActiveSupport::TestCase
  self.described_class = Payment::Stats
  self.rspec_metadata = { vcr: true }



  context_ Payment::Stats, :vcr do
  context_ "revenue_by_link" do
      before do
        @user = create(:singaporean_user_with_compliance_info, user_risk_state: "compliant", payment_address: "bob@cat.com")
        @json = JSON.parse(File.open("#{Rails.root}/test/support/fixtures/stripe.json").read)
  ...
  test "calculates the revenue per link based on all purchases during the payout period, even with affiliate_credits" do
  ...
  context_ "chargeback", :vcr do
  test "deducts the chargeback and refund fees from the link's revenue if we didn't waive our fee" do
          travel_to(Date.today - 10) do
  ...
  test "deducts the full purchase amount if the chargeback or refund happened in this period but the original purchase did not (fees waived)" do
  ...
  test "deducts the full purchase amount if the chargeback or refund happened in this period but the original purchase did not (fees not waived)" do
          refunded_purchase = nil
          chargedback_purchase = nil
          travel_to(Date.today - 20) do
            refunded_purchase = create(:purchase_in_progress, link: @product_1, chargeable: create(:chargeable), affiliate: @affiliate)
  ...
  test "deducts the refund amount if the refund happened in this period but the original purchase did not (fees not waived)" do
          refunded_purchase = nil
          travel_to(Date.today - 20) do
  ...
  test "deducts the refund amount - fee amount if the refund happened in this period but the original purchase did not (fees waived)" do
          refunded_purchase = nil
          travel_to(Date.today - 20) do
            refunded_purchase = create(:purchase_in_progress, link: @product_3, chargeable: create(:chargeable), affiliate: @affiliate)
            refunded_purchase.process!
  ...
  test "deducts the refund amount - refunded fee amount - refunded affiliate amount if the partial refunds and the original purchase both happened in this period (fees waived)" do
