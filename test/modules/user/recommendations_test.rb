...
class ProductRecommendationsTest < ActiveSupport::TestCase
  ...
  context_ "when user is recommendable" do
      let(:user) { create(:recommendable_user) }

  test "is recommendable" do
  ...
  context_ "payout info" do
      let(:user) { create(:compliant_user) }

  test "is false if user doesn't have payout info" do
  ...
  test "is true if user has payment_address" do
        expect(user.recommendable_reasons[:payout_filled]).to be(true)
        expect(user.recommendable?).to be(true)
      end

  test "is true if user has active bank_account" do
        user.update!(payment_address: nil)
  ...
  test "is true if user has a paypal account connected" do
        user.update!(payment_address: nil)
