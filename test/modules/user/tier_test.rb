...
class UserTierTest < ActiveSupport::TestCase
  self.described_class = User::Tier



  context_ User::Tier do
  context_ "tier state machine" do
  test "upgrades to next tier" do
        user = create(:user)
  ...
  test "upgrades with tier override" do
        user = create(:user)
        expect(user.tier).to eq(User::TIER_0)
  ...
  context_ "#tier" do
  test "returns 0 if creator has not received any payments" do
        creator = create(:user)
        expect(creator.tier).to eq(0)
      end

  test "returns 0 if creator has negative sales" do
        creator = create(:user)
        sales_cents = -1000
        expect(creator.tier(sales_cents)).to eq(0)
      end

  test "returns value from tier_state column" do
  ...
  test "returns correct tier based on revenue" do
        creator = create(:user)
