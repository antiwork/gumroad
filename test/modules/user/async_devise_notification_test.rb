...
class UserAsyncDeviseNotificationTest < ActiveSupport::TestCase
  self.described_class = User::AsyncDeviseNotification



  context_ User::AsyncDeviseNotification do
  ...
  context_ "##{devise_email_method}" do
  test "queues the #{devise_email_name} email in the background" do
          expect do
            user.public_send(devise_email_method)
          end.to(have_enqueued_mail(UserSignupMailer, devise_email_name))
        end

  test "actually sends the email" do
