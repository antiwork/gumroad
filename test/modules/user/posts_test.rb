...
class UserPostsTest < ActiveSupport::TestCase
  self.described_class = User::Posts
  self.rspec_metadata = { freeze_time: true }



  context_ User::Posts, :freeze_time do
  context_ "visible_posts_for", :vcr do
      before do
  ...
  context_ "posts with shown_on_profile true" do
  test "returns only audience posts that are shown on profile if logged_in_user is not present or is not a customer or follower" do
  ...
  test "returns audience posts and posts from purchased products that are shown on profile when logged_in_user is a customer" do
  ...
  test "returns audience posts and follower posts that are shown on profile when logged_in_user is a follower" do
  ...
  test "returns only audience posts that are shown on profile when logged_in_user is an affiliate" do
  ...
  test "returns audience posts, seller posts, follower posts, and product/variant posts that are shown on profile from purchased products when logged_in_user is a follower and a customer" do
  ...
  test "returns audience posts, seller posts, follower posts, and product/variant posts that are shown on profile from valid purchases when logged_in_user is a customer" do
  ...
  context_ "all posts irrespective of shown_on_profile" do
  test "returns only audience posts that are shown on profile if logged_in_user is not present or is not a customer or follower" do
          pundit_user = SellerContext.logged_out
  ...
  test "returns all audience posts and posts from purchased products when logged_in_user is a customer" do
          pundit_user = SellerContext.new(user: @dude, seller: @dude)
          visible_posts = @creator.visible_posts_for(pundit_user:, shown_on_profile: false)
          expect(visible_posts).to contain_exactly(@audience_post,
  ...
  test "returns audience posts and follower posts when logged_in_user is a follower" do
          pundit_user = SellerContext.new(user: @follower, seller: @follower)
          visible_posts = @creator.visible_posts_for(pundit_user:, shown_on_profile: false)
          expect(visible_posts).to contain_exactly(@audience_post,
                                                   @audience_post_not_on_profile,
  ...
  test "returns audience posts and affiliate posts when logged_in_user is an affiliate" do
          affiliate_user = @direct_affiliate.affiliate_user
          pundit_user = SellerContext.new(user: affiliate_user, seller: affiliate_user)
          visible_posts = @creator.visible_posts_for(pundit_user:, shown_on_profile: false)
          expect(visible_posts).to contain_exactly(@audience_post,
  ...
  test "returns audience posts, seller posts, follower posts, and product/variant posts from purchased products when logged_in_user is a follower and a customer" do
          create(:follower, user: @creator, email: @dude.email, confirmed_at: Time.current)
          pundit_user = SellerContext.new(user: @dude, seller: @dude)
          visible_posts = @creator.visible_posts_for(pundit_user:, shown_on_profile: false)
          expect(visible_posts).to contain_exactly(@audience_post,
  ...
  context_ "user has stopped and restarted subscription" do
  test "does not return posts published while the subscription was stopped" do
  ...
  context_ "#last_5_created_posts" do
  ...
  test "includes deleted and not published posts" do
