...
class UserSocialGoogleTest < ActiveSupport::TestCase
  self.described_class = User::SocialGoogle



  context_ User::SocialGoogle do
    before(:all) do
      @data = JSON.parse(File.open("#{Rails.root}/test/support/fixtures/google_omniauth.json").read)
    end

  context_ ".find_or_create_for_google_oauth2" do
      before do
  ...
  test "creates a new user if one does not exist with the corresponding google uid or email" do
        User.find_or_create_for_google_oauth2(@data)

        expect(User.find_by(email: @data["info"]["email"])).not_to eq(nil)
        expect(User.find_by(google_uid: @data["uid"])).not_to eq(nil)
      end

  test "finds a user using google's uid payload" do
  ...
  test "finds a user using email when google's uid is missing and fills in uid" do
  ...
  test "creates user with sanitized name when name contains colons" do
  ...
  test "retries after a deadlock and returns the created user" do
  ...
  test "returns nil and notifies after exhausting deadlock retries" do
        deadlock_data = @data.deep_dup
  ...
  test "retries after a lock wait timeout and returns the created user" do
  ...
  test "returns nil and notifies after exhausting lock wait timeout retries" do
        lock_timeout_data = @data.deep_dup
  ...
  context_ ".google_picture_url", :vcr do
      before do
        @user = create(:user, google_uid: @data["uid"])
      end

  test "stores the user's profile picture from Google to S3 and returns the URL for the saved file" do
  ...
  test "returns nil for unsupported avatar content types" do
  ...
  test "returns nil when avatar file size exceeds maximum" do
  ...
  test "passes open_timeout and read_timeout to URI.open" do
  ...
  test "returns nil when the remote server times out" do
  ...
  test "returns nil when reading from the remote server times out" do
  ...
  context_ ".query_google" do
  context_ "email change" do
  test "sets email if the email coming from google is different" do
  ...
  context_ "when the email already exists in a different case" do
          before do
            @user = create(:user, email: @data["info"]["email"].upcase)
          end

  test "doesn't update email" do
            expect { User.query_google(@user, @data) }.not_to change { @user.reload.email }
          end

  test "doesn't raise error" do
  ...
  test "does not set a name if one already exists" do
          @user = create(:user, name: "Spongebob")
          expect { User.query_google(@user, @data) }.not_to change { @user.reload.name }
        end
      end
  ...
  test "sets the google uid if one does not exist upon creation" do
          expect { User.query_google(@user, @data) }.to change { @user.reload.google_uid }.from(nil).to(@data["uid"])
        end

  test "sets the name if one does not exist upon creation" do
