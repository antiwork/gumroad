...
class UserFeatureStatusTest < ActiveSupport::TestCase
  self.described_class = User::FeatureStatus



  context_ User::FeatureStatus do
  context_ "#merchant_migration_enabled?" do
  test "returns true if either feature flag is enabled of else false" do
        creator = create(:user)
  ...
  test "returns false if user country is not supported by Stripe Connect" do
        creator = create(:user)
  ...
  context_ "#charge_paypal_payout_fee?" do
      let!(:seller) { create(:user) }
      before do
        create(:user_compliance_info, user: seller)
      end

  test "returns true if feature flag is set and user is not from Brazil or India and user should be charged fee" do
        expect(seller.charge_paypal_payout_fee?).to be true
      end

  test "returns false if paypal_payout_fee feature flag is disabled" do
        expect(seller.charge_paypal_payout_fee?).to be true
  ...
  test "returns false if paypal_payout_fee_waived flag is set of the user" do
  ...
  test "returns false if user is from Brazil or India" do
        expect(seller.charge_paypal_payout_fee?).to be true
  ...
  context_ "#has_stripe_account_connected?" do
  test "returns true if there is a connected Stripe account and merchant migration flag is enabled" do
        creator = create(:user)
        create(:user_compliance_info, user: creator)
  ...
  test "returns false if there is a connected Stripe account but merchant migration flag is not enabled" do
        creator = create(:user)
        create(:user_compliance_info, user: creator)
  ...
  test "returns false if there is no connected Stripe account" do
        creator = create(:user)
        create(:user_compliance_info, user: creator)
  ...
  context_ "#has_paypal_account_connected?" do
  test "returns true if there is a connected PayPal account otherwise returns false" do
        creator = create(:user)
        create(:user_compliance_info, user: creator)
  ...
  context_ "#can_publish_products?" do
  ...
  test "returns false if no payout method is setup" do
        expect(seller.can_publish_products?).to be false
      end

  test "returns true if a bank account is setup", :vcr do
        create(:merchant_account_stripe, user: seller)

        expect(seller.can_publish_products?).to be true
      end

  test "returns true if a PayPal account is connected" do
        create(:merchant_account_paypal, user: seller)

        expect(seller.can_publish_products?).to be true
      end

  test "returns true if a Stripe account is connected" do
        create(:merchant_account_stripe_connect, user: seller)

        expect(seller.can_publish_products?).to be true
      end

  test "returns true if a PayPal payment address is present" do
        seller.update!(payment_address: "payme@example.com")

        expect(seller.can_publish_products?).to be true
      end

  test "returns true if user is admin team member without any payment methods" do
  ...
  context_ "can_setup_bank_payouts?" do
  test "returns true if country is in Stripe supported list (except India without an active bank account)" do
  ...
  test "returns true if user has an active bank account" do
        seller = create(:user)
        create(:ach_account_stripe_succeed, user: seller)
        expect(seller.can_setup_bank_payouts?).to be true
      end

  test "returns false if country is not in Stripe supported list and there's no active bank account" do
        seller = create(:user)
        create(:user_compliance_info, user: seller, country: "Brazil")
        expect(seller.can_setup_bank_payouts?).to be false
      end

  test "returns true if user is from UAE" do
        seller = create(:user)
        create(:user_compliance_info, user: seller, country: "United Arab Emirates")
        expect(seller.can_setup_bank_payouts?).to be true
      end

  test "returns false for new India users without an active bank account" do
        seller = create(:user)
        create(:user_compliance_info, user: seller, country: "India")
        expect(seller.can_setup_bank_payouts?).to be false
      end

  test "returns true for India users with an active bank account" do
        seller = create(:user)
        create(:user_compliance_info, user: seller, country: "India")
  ...
  context_ "can_setup_paypal_payouts?" do
  test "returns true if user already has a payment address set" do
  ...
  test "returns true if country is not in Stripe supported list" do
  ...
  test "returns true if user is from Egypt" do
        seller = create(:user, payment_address: nil)
        create(:user_compliance_info, user: seller, country: "Egypt")
        expect(seller.can_setup_paypal_payouts?).to be true
      end

  test "returns true if user is from Kazakhstan" do
        seller = create(:user, payment_address: nil)
        create(:user_compliance_info, user: seller, country: "Kazakhstan")
        expect(seller.can_setup_paypal_payouts?).to be true
      end

  test "returns true if user is from India" do
        seller = create(:user, payment_address: nil)
        create(:user_compliance_info, user: seller, country: "India")
        expect(seller.can_setup_paypal_payouts?).to be true
      end

  test "returns false for all Stripe supported countries except UAE, Kazakhstan, Egypt, and India" do
  ...
  context_ "#paypal_connect_allowed?" do
      let!(:seller) { create(:user) }
  ...
  test "returns true if seller is compliant and has more than $100 in sales and a successful payout" do
        expect(seller.paypal_connect_allowed?).to eq true
      end

  test "returns false if seller is not compliant" do
        seller.update!(user_risk_state: "not_reviewed")
        expect(seller.reload.paypal_connect_allowed?).to eq false
      end

  test "returns false if seller does not have $100 in sales" do
        allow_any_instance_of(User).to receive(:sales_cents_total).and_return(99_00)
        expect(seller.reload.paypal_connect_allowed?).to eq false
      end

  test "returns false if seller does not have a successful payout" do
        seller.payments.last.update!(state: "failed")
        expect(seller.reload.paypal_connect_allowed?).to eq false
      end

  test "returns false if seller does not meet any eligibility requirement" do
        seller.update!(user_risk_state: "not_reviewed")
        allow_any_instance_of(User).to receive(:sales_cents_total).and_return(99_00)
        seller.payments.last.update!(state: "failed")
  ...
  context_ "#stripe_disconnect_allowed?" do
  test "returns true if there is no connected Stripe account" do
        creator = create(:user)
        create(:user_compliance_info, user: creator)
  ...
  test "returns true if there is a connected Stripe account but no active subscriptions using it" do
        creator = create(:user)
        create(:user_compliance_info, user: creator)
  ...
  test "returns false if there is a connected Stripe account and active subscriptions use it" do
        creator = create(:user)
        create(:user_compliance_info, user: creator)
  ...
  context_ "#product_level_support_emails_enabled?" do
      let(:user) { create(:user) }

  test "returns true if product_level_support_emails feature flag is enabled for user" do
  ...
  context_ "#waive_gumroad_fee_on_new_sales?" do
  test "returns true if waive_gumroad_fee_on_new_sales feature flag is set for seller" do
        seller = create(:user)
  ...
  test "returns true if today is Gumroad day in seller's timezone" do
        seller = create(:user)
  ...
  test "returns false if today is not Gumroad day and feature flag is not set for seller" do
        seller = create(:user)
  ...
  test "returns false if today is Gumroad day in some other timezone but not in seller's timezone" do
  ...
  test "uses seller's gumroad_day_timezone when present to check if it is Gumroad Day" do
        $redis.set(RedisKey.gumroad_day_date, "2024-4-4")
        gumroad_day = Date.new(2024, 4, 4)
        gumroad_day_in_act = gumroad_day.in_time_zone("Melbourne")
        gumroad_day_in_pst = gumroad_day.in_time_zone("Pacific Time (US & Canada)")
