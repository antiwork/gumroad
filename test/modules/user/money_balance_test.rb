...
class UserMoneyBalanceTest < ActiveSupport::TestCase
  self.described_class = User::MoneyBalance



  context_ User::MoneyBalance do
    before :each do
      @user = create(:user)
    end

  context_ "#unpaid_balance_cents" do
  context_ "via SQL" do
  ...
  test "still returns the correct balance if Elasticsearch call failed and sends error notification" do
          create(:balance, user: @user, amount_cents: 100, state: "unpaid", date: 1.day.ago)
          create(:balance, user: @user, amount_cents: 200, state: "unpaid", date: 3.days.ago)
  ...
  context_ "#unpaid_balance_cents_up_to_date" do
  test "returns sum of unpaid balance in cents with date up to a given date" do
  ...
  test "ignores unpaid balance with date after a given date" do
  ...
  context_ "#unpaid_balances_up_to_date" do
  test "returns all unpaid balances with date up to a given date" do
  ...
  test "ignores paid balances" do
        create(:balance, user: @user, amount_cents: 100, state: "paid", date: 3.days.ago)
        expect(@user.unpaid_balances_up_to_date(1.day.ago)).to be_empty
      end

  test "ignores balances with date after a given date" do
        create(:balance, user: @user, amount_cents: 100, state: "unpaid", date: 1.day.ago)
        expect(@user.unpaid_balances_up_to_date(3.days.ago)).to be_empty
      end
    end

  context_ "#unpaid_balance_cents_up_to_date_held_by_gumroad" do
  test "returns unpaid cents that are held by gumroad with date up to the given date" do
  ...
  test "does not include balance held in stripe connect account" do
  ...
  test "does not include paid balance" do
        create(:balance, user: @user, merchant_account: MerchantAccount.gumroad(StripeChargeProcessor.charge_processor_id),
                         amount_cents: 100, state: "paid", date: 3.days.ago)
        expect(@user.unpaid_balance_cents_up_to_date_held_by_gumroad(1.day.ago)).to eq(0)
      end

  test "does not include balance with date after the given date" do
        create(:balance, user: @user, merchant_account: MerchantAccount.gumroad(BraintreeChargeProcessor.charge_processor_id),
  ...
  context_ "#unpaid_balance_cents_up_to_date_held_by_stripe" do
      let(:stripe_connect_merchant_account) { create(:merchant_account, user: @user, currency: "aud") }

  test "returns unpaid amount cents held in stripe connect account with date up to the given date" do
  ...
  test "does not include balance held by gumroad" do
        create(:balance, user: @user, merchant_account: MerchantAccount.gumroad(StripeChargeProcessor.charge_processor_id),
                         amount_cents: 100, state: "unpaid", date: 3.days.ago)
  ...
  test "returns 0 when user has no stripe account" do
        allow(@user).to receive(:stripe_account).and_return(nil)
        expect(@user.unpaid_balance_cents_up_to_date_held_by_stripe(1.day.ago)).to eq(0)
      end
    end

  context_ "#unpaid_balance_holding_cents_up_to_date_held_by_stripe" do
      let(:stripe_connect_merchant_account) { create(:merchant_account, user: @user, currency: "aud") }

  test "returns unpaid holding cents that are held in stripe connect account with date up to the given date" do
        create(:balance, user: @user, merchant_account: stripe_connect_merchant_account,
                         holding_currency: stripe_connect_merchant_account.currency,
                         amount_cents: 100, holding_amount_cents: 129, state: "unpaid", date: 3.days.ago)
        create(:balance, user: @user, merchant_account: stripe_connect_merchant_account,
  ...
  context_ "#paid_payments_cents_for_date" do
  ...
  test "returns the sum of balances that haven't failed or been returned/reversed, and only the ones on the same day" do
        expect(@user.paid_payments_cents_for_date(payout_date)).to eq(111_00)
      end
    end

  context_ "#formatted_balance_to_forfeit" do
  context_ "country_change" do
        let(:merchant_account) { create(:merchant_account, user: @user) }

  test "returns formatted balance if there's a positive balance" do
  ...
  test "returns nil if there's a zero or negative balance to forfeit" do
  ...
  context_ "account_closure" do
  test "returns nil when the delete_account_forfeit_balance feature is inactive" do
          create(:balance, user: @user, state: :unpaid, amount_cents: 942)
          expect(@user.formatted_balance_to_forfeit(:account_closure)).to eq(nil)
        end

  context_ "when the delete_account_forfeit_balance feature is active" do
          before do
  ...
  context_ "#forfeit_unpaid_balance!" do
      before do
  ...
  test "does nothing if there is no balance to forfeit" do
  ...
  test "marks balances as forfeited and adds a comment" do
  ...
  context_ "account_closure" do
  context_ "when the delete_account_forfeit_balance feature is not active" do
  ...
  context_ "#validate_account_closure_balances!" do
  context_ "when the delete_account_forfeit_balance feature is not active" do
  test "raises UnpaidBalanceError" do
          create(:balance, user: @user, state: :unpaid, amount_cents: 942)
  ...
  test "returns nil if there's a zero balance" do
          expect(@user.validate_account_closure_balances!).to eq(nil)
        end

  test "raises User::UnpaidBalanceError if there's non-zero balance to forfeit" do
