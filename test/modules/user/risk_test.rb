...
class UserRiskTest < ActiveSupport::TestCase
  self.described_class = User::Risk



  context_ User::Risk do
  ...
  context_ "suspension state machine callback" do
  ...
  test "skips the generic suspension email when called with skip_generic_suspension_email" do
        user = create(:user)
        user.flag_for_tos_violation!(author_name: "admin", bulk: true)
  ...
  test "does not send the generic suspension email when the feature flag is inactive" do
  ...
  context_ "#suspend_due_to_stripe_risk" do
      let(:user) { create(:user) }

      before { Feature.activate(:account_suspended_email) }

  test "sends the Stripe-risk-specific email and not the generic suspension email" do
        expect do
  ...
  context_ "#suspend_sellers_other_accounts" do
      let(:transition) { double("transition", args: []) }

  context_ "when user has PayPal as payout processor" do
  test "calls SuspendAccountsWithPaymentAddressWorker only once for all related accounts" do
  ...
  context_ "#unblock_seller_ip!" do
      let(:ip) { "203.0.113.42" }
      let(:user) { create(:user, last_sign_in_ip: ip) }

  test "does nothing when last_sign_in_ip is blank" do
        user.update_column(:last_sign_in_ip, nil)
        expect { user.unblock_seller_ip! }.not_to raise_error
      end

  test "only unblocks rows scoped to the ip_address type" do
