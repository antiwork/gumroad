...
class PurchaseReviewsTest < ActiveSupport::TestCase
  self.described_class = Purchase::Reviews



  context_ Purchase::Reviews do
  context_ "#post_review" do
  context_ "non-recurring purchase" do
        let!(:purchase) { create(:purchase) }

  context_ "when there is an existing product_review" do
          let!(:existing_product_review) { create(:product_review, purchase:, rating: 1, message: "Original message") }

  test "updates the existing product_review with the provided rating" do
  ...
  test "does not update the rating if the purchase does not allow reviews to be counted" do
  ...
  context_ "when there is no existing product_review" do
  test "creates a new product review for a purchase that allows reviews to be counted" do
  ...
  test "creates a product review and enqueues an email notification" do
            expect(purchase.product_review).to be(nil)
  ...
  test "does not enqueue an email when review notifications are disabled" do
  ...
  test "does not add review when the purchase does not allow reviews to be counted" do
  ...
  test "saves the link_id associated with the purchase on the product review" do
  ...
  context_ "recurring purchase" do
  ...
  test "updates the product review of the original purchase in case of recurring purchase of a subscription" do
  ...
  context_ "when there is no existing product_review" do
  test "adds product review to the original purchase in case of recurring purchase of a subscription" do
  ...
  context_ "#update_product_review_stat" do
  test "does not update the product_review_stat after update if no product review exists" do
  ...
  test "does not update the product_review_stat after update if no changes have been saved" do
        purchase = create(:purchase)
  ...
  test "updates the product_review_stat after update" do
        purchase = create(:purchase)
  ...
  context_ "#refund_purchase! effect on reviews" do
  test "updates product_review_stat" do
        purchase = create(:purchase)
  ...
  test "removes reviews from all products in a bundle when bundle is refunded" do
  ...
  test "does not remove review from a standalone product when partially refunded" do
        purchase = create(:purchase)
        product = purchase.link
  ...
  test "removes reviews from all products in a bundle when bundle is partially refunded" do
        bundle_purchase = create(:purchase, link: create(:product, :bundle))
        bundle_purchase.create_artifacts_and_send_receipt!
  ...
  context_ "revoking/unrevoking access" do
  test "updates the product review" do
  ...
  context_ "#allows_review_to_be_counted? & .allowing_reviews_to_be_counted & #allows_review?", :vcr do
      before do
  ...
  test "is only valid for matching successful or free trial purchases" do
  ...
  test "returns true for recurring subscription purchases" do
        @matching_purchases << @recurring_purchase
        expect(@matching_purchases.all?(&:allows_review?)).to eq(true)
      end
    end

  context_ "#original_product_review" do
  context_ "when purchase is for a subscription" do
  ...
  test "returns the product review of the original purchase for all purchases in the subscription" do
  ...
  test "returns giftee's product review for either purchases" do
