...
class PurchaseTargetingTest < ActiveSupport::TestCase
  self.described_class = Purchase::Targeting



  context_ Purchase::Targeting do
    before do
      @seller = create(:user)
  ...
  test "does nothing if no variant_id is passed" do
        expect(Purchase.by_variant(nil).count).to eq 8
      end

  test "filters by variant if variant_id is passed" do
  ...
  test "does nothing if no bought products are passed" do
        expect(Purchase.for_products(nil).count).to eq 8
      end

  test "filters by product if only one bought product is passed" do
        expect(Purchase.for_products([@product.id]).count).to eq 2
        expect(Purchase.for_products([@product2.id]).count).to eq 3
      end

  test "filters by multiple products if more than one bought product is passed" do
  ...
  context_ "#email_not" do
  test "excludes the purchases made with specific emails" do
        link = create(:product)
  ...
  context_ "#paid_more_than" do
  test "does nothing if no paid_more_than is passed" do
          expect(Purchase.paid_more_than(nil).count).to eq 8
        end

  test "filters by paid if paid_more_than is passed" do
  ...
  context_ "#paid_less_than" do
  test "does nothing if no paid_less_than is passed" do
          expect(Purchase.paid_less_than(nil).count).to eq 8
        end

  test "filters by paid if paid_less_than is passed" do
  ...
  context_ "paid between" do
  test "does nothing if no paid_more_than or paid_less_than are passed" do
          expect(Purchase.paid_less_than(nil).paid_more_than(nil).count).to eq 8
        end

  test "filters by paid between if both scopes are applied" do
  ...
  context_ "created" do
  context_ "#created_after" do
  test "does nothing if no date passed" do
          expect(Purchase.created_after(nil).count).to eq 8
        end

  test "filters by created at if date is passed" do
  ...
  context_ "#created_before" do
  test "does nothing if no date is passed" do
  ...
  context_ "created between" do
  test "does nothing if no after date or before date are passed" do
  ...
  context_ "#filter_by_country_bought_from" do
      before do
  ...
  test "does nothing if no bought_from is passed" do
        expect(Purchase.country_bought_from(nil).count).to eq 8
      end

  test "filters by country or ip country if bought_from is passed" do
  ...
  test "filters by country for a country whose name is different for `countries` gem and `iso_country_codes` gem" do
  ...
  context_ "#by_external_variant_ids_or_products" do
      before do
  ...
  test "returns purchases for both products and variants" do
  ...
  test "returns purchases for variants if product_ids are blank" do
        result = Purchase.by_external_variant_ids_or_products(@variant2.external_id, nil)
        expect(result).to include(@purchase2)
        expect(result.length).to eq 1
      end

  test "returns purchase for products if external_variant_ids are nil" do
