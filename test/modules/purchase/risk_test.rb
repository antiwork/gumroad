...
class PurchaseRiskTest < ActiveSupport::TestCase
  self.described_class = Purchase::Risk



  context_ Purchase::Risk do
    before do
      Feature.activate(:purchase_check_for_fraudulent_ips)
    end

  context_ "check purchase for previous chargebacks" do
  test "returns errors if the email has charged-back" do
  ...
  test "returns errors if the guid has charged-back" do
        product = create(:product)
        product_2 = create(:product)
  ...
  test "returns errors if the email is associated with suspended for fraud creator" do
        product = create(:product)
  ...
  test "doesn't return error when buyer user doesn't exist" do
        product = create(:product)
  ...
  test "doesn't return error if the chargeback has been won" do
        product = create(:product)
        product_2 = create(:product)
  ...
  context_ "#check_for_past_chargebacks" do
      let(:product) { create(:product) }
      let(:new_purchase) { build(:purchase, link: product, email: "test@example.com", browser_guid: "test-guid-123") }

  context_ "when there are no past chargebacks" do
  test "does not add errors" do
  ...
  context_ "when there is a past chargeback by email" do
        before do
          create(:purchase, link: product, email: "test@example.com", chargeback_date: Time.current)
        end

  test "adds chargeback error" do
  ...
  test "sets the correct error message" do
  ...
  context_ "when there is a past chargeback by browser_guid" do
        before do
  ...
  context_ "when there are chargebacks by both email and browser_guid" do
        before do
          create(:purchase, link: product, email: "test@example.com", chargeback_date: Time.current)
          create(:purchase, link: product, browser_guid: "test-guid-123", chargeback_date: Time.current)
        end
  ...
  context_ "when chargeback has been reversed" do
        before do
  ...
  context_ "when purchase has neither email nor browser_guid" do
  ...
  test "handles timeout and returns nil" do
  ...
  test "returns errors if the buyer ip_address has been blocked" do
  ...
  test "returns errors if the buyer browser_guid has been blocked" do
        browser_guid = "abc123"
  ...
  test "returns errors if the seller ip_address has been blocked" do
  ...
  test "returns error if the purchaser's ip_address has been blocked" do
  ...
  context_ "when it is an original subscription purchase" do
  test "returns error if the ip_address is blocked" do
  ...
  context_ "when it is a subscription recurring charge" do
  test "doesn't block the purchase when ip_address is blocked" do
  ...
  context_ "when it's a free purchase" do
  test "doesn't block the purchase when ip_address is blocked" do
  ...
  context_ "when the creator's ip_address has been blocked but the seller is compliant" do
  ...
  context_ "#check_for_past_blocked_email_domains" do
        let!(:purchaser) { create(:user) }
        let!(:purchase) { build(:purchase, purchaser:, email: "john@example.com") }

  context_ "when it is a paid product" do
          vague_purchase_error_notice = "Your card was not charged."

  test "returns error if the specified email domain has been blocked" do
  ...
  test "returns error if the purchaser's email domain has been blocked" do
  ...
  context_ "when it is a gift purchase" do
  ...
  test "returns error if the gift recipient's email domain has been blocked" do
  ...
  test "returns error if the gift sender's email domain has been blocked" do
  ...
  test "returns error without raising when gifter_purchase is not yet persisted on the gift" do
  ...
  context_ "when it is a free product" do
  ...
  test "doesn't return an error if neither the purchaser email's domain nor the specified email's domain has been blocked" do
          expect do
            purchase.check_for_fraud
          end.not_to change { purchase.error_code }

          expect(purchase.errors.any?).to be(false)
        end

  context_ "when purchaser has blank email" do
          let(:purchaser) { create(:user, email: "", provider: "twitter") }

  test "doesn't return an error" do
            expect do
              purchase.check_for_fraud
            end.not_to change { purchase.error_code }
