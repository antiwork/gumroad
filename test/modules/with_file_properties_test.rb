...
class WithFilePropertiesTest < ActiveSupport::TestCase
  self.described_class = WithFileProperties



  context_ WithFileProperties do
        ...
        before do
          ...
          allow(s3_double).to receive(:get) do |options|
            File.open(options[:response_target], "w+") do |f|
              f.write(File.open("#{Rails.root}/test/support/fixtures/#{properties[:filename]}").read)
            end
          end
  ...
  test "has the correct filegroup for #{file_type}" do
          expect(@product_file.filegroup).to eq properties[:filegroup]
        end

  test "has the correct properties" do
  ...
  test "sets the metadata and the analyze_completed flag" do
        expected_metadata = {
          bitrate: 27_506,
          duration: 4,
          framerate: 60,
  ...
  context_ "when auto-transcode is disabled for the product" do
  test "doesn't transcode the video and sets product.transcode_videos_on_purchase to true" do
          @video_file.assign_video_attributes(@file_path)
  ...
  context_ "when auto-transcode is enabled for the product" do
        before do
          allow(@video_file.link).to receive(:auto_transcode_videos?).and_return(true)
        end

  test "transcodes the video" do
          @video_file.assign_video_attributes(@file_path)
  ...
  test "sets the pagelength" do
        expect(@epub_file.pagelength).to eq 10
      end

  test "returns nil in pagelength for json" do
  ...
  test "sets the epub section information" do
  ...
  test "does not have framerate, resolution, duraration" do
  ...
  test "does not throw an exception due to long tempfile name" do
  ...
  context_ "with a incorrect s3_key" do
  test "corrects it and succeeds in analyzing the file" do
