...
class ModulesProductSearchableSearchTest < ActiveSupport::TestCase
  ...
  context_ "Product::Searchable - Search scenarios" do
    before do
      @product = create(:product_with_files)
    end

  context_ "Elasticsearch `search`" do
  context_ "on indexed products with price values" do
        before do
  ...
  test "filters by min and max price" do
  ...
  test "filters when min and max price are 0" do
  ...
  test "min and max filter include products with a decimal price" do
  ...
  test "clamps extremely large price filter values to prevent Elasticsearch long overflow" do
  ...
  test "clamps negative price filter values to zero" do
  ...
  context_ "is_alive_on_profile" do
  ...
  test "filters by is_alive_on_profile" do
  ...
  test "sorts by fee revenue and sales volume" do
  ...
  test "sorts by fee revenue and sales volume when no sort param is present" do
  ...
  test "filters by ID" do
          ...
          expect(records.map(&:id)).not_to include(id)
        end

  context_ "when sorting by curated" do
  test "boosts recommended products then sorts by price ascending" do
  ...
  context_ "on indexed products with filetypes" do
        before do
          @creator = create(:recommendable_user)
  ...
  test "filters by one filetype" do
  ...
  test "filters by multiple filetypes" do
  ...
  test "aggregates the filetypes for all params passed, except filetypes" do
  ...
  context_ "top creators aggregation" do
  ...
  test "aggregates the top 6 creators by sales volume from the last 3 months" do
  ...
  test "filters top creators by search options" do
  ...
  test "does not include top creators if include_top_creators param is not present" do
  ...
  context_ "on indexed products with tags" do
        before do
  ...
  test "filters to the union of two tags" do
  ...
  context_ "on indexed products with `created_at` value" do
  ...
  context_ "searching by section" do
        before do
          @creator = create(:user)
  ...
  test "returns only the products in the given section, in the correct order" do
  ...
  context_ "search_options with String ids param" do
  test "does not raise NoMethodError when ids is a String" do
          expect do
  ...
  context_ "on indexed products with reviews" do
        before do
          creator = create(:compliant_user, username: "username")
          product_ids = []
          time = Time.current
  ...
  test "sorts by most reviewed product" do
  ...
  test "sorts and filters by average rating" do
  ...
  context_ "on all products with reviews_count" do
        before do
  ...
  test "filters products with reviews_count greater than min_reviews_count" do
          ...
          expect(results_1).not_to include @product_3
          ...
          expect(results_2).not_to include @product_2
          expect(results_2).not_to include @product_3
        end
      end

  context_ "on indexed products with taxonomy set" do
        before do
  ...
  test "returns products with matching taxonomy" do
          search_options_1 = Link.search_options(taxonomy_id: @music.id)
          results_1 = Link.search(search_options_1).records
          expect(results_1).to include @product_1
          expect(results_1).not_to include @product_2

          search_options_2 = Link.search_options(taxonomy_id: @standup.id)
          results_2 = Link.search(search_options_2).records
          expect(results_2).to include @product_2
          expect(results_2).not_to include @product_1
        end

  test "does not return descendant products" do
  ...
  context_ "when include_taxonomy_descendants params is true" do
  test "returns products whose taxonomy is a child of the set taxonomy" do
            search_options = Link.search_options(
            ...
            expect(results).not_to include @product_1
          end

  test "returns products whose taxonomy is the grandchildren of the set taxonomy" do
            search_options = Link.search_options(
  ...
  context_ "on indexed products rated as adult" do
        before do
  ...
  test "excludes adult products when include_rated_as_adult is absent" do
          search_options = Link.search_options({})
          results = Link.search(search_options).records
          expect(results).to include @product_sfw
          expect(results).not_to include @product_nsfw
        end

  test "includes adult products when include_rated_as_adult is present" do
  ...
  context_ "on indexed products with staff_picked_at" do
  ...
  test "includes all products when staff_picked is not set" do
          search_options = Link.search_options({})
          results = Link.search(search_options).records
  ...
  context_ "with staff_picked filter" do
  test "includes only staff_picked products, sorted" do
  ...
  context_ "option 'from'" do
  test "is clamped from 0 to a valid max result window" do
          expect do
  ...
  context_ "searching by multiple user IDs" do
  ...
  test "returns products from both users" do
