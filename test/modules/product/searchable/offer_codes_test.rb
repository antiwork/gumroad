...
class ModulesProductSearchableOfferCodesTest < ActiveSupport::TestCase



  context_ "Product::Searchable - Offer codes filtering" do
  context_ "filtering by offer_codes", :elasticsearch_wait_for_refresh do
      before do
  ...
  test "returns only products with BLACKFRIDAY2025 offer code" do
  ...
  test "returns all products when offer_code param is not provided" do
  ...
  test "returns no products when searching with __no_match__ code" do
  ...
  test "returns products when offer code is deleted and reindexed" do
  ...
  context_ "offer codes indexing" do
      before do
        @creator = create(:recommendable_user)
  ...
  test "includes all alive offer codes in search property" do
  ...
  test "returns empty array when product has no offer codes" do
