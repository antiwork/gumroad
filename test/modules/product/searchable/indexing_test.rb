...
class ModulesProductSearchableIndexingTest < ActiveSupport::TestCase
  ...
  context_ "Product::Searchable - Indexing scenarios" do
    before do
      @product = create(:product_with_files)
    end

  context_ "#as_indexed_json" do
  test "includes all properties" do
  ...
  context_ "#build_search_update" do
  test "returns the attributes to update in Elasticsearch" do
  ...
  context_ "#build_search_property for offer_codes" do
  test "returns all codes when total is less than MAX_OFFER_CODES_IN_INDEX" do
  ...
  test "limits to MAX_OFFER_CODES_IN_INDEX most recent codes when total exceeds limit" do
        stub_const("Product::Searchable::MAX_OFFER_CODES_IN_INDEX", 5)
  ...
  context_ "#enqueue_search_index!" do
  test "indexes product via ProductIndexingService" do
  ...
  context_ "#enqueue_index_update_for" do
  test "updates product document via ProductIndexingService" do
  ...
  context_ "Indexing the changes through callbacks" do
  test "imports all search fields when a product is created" do
        product = build(:product)
        expect(ProductIndexingService).to receive(:perform).with(product:, action: "index", on_failure: :async).and_call_original
        product.save!
      end

  test "correctly indexes price_cents", :elasticsearch_wait_for_refresh do
        document = EsClient.get(index: Link.index_name, id: @product.id)
        expect(document["_source"]["price_cents"]).to eq(100)
      end

  context_ "on product update" do
  test "requests update of the name & rated_as_adult fields on name change" do
          expect_product_update %w[name rated_as_adult]

          @product.update!(name: "I have a great idea for a name.")
        end

  test "requests update of the description & rated_as_adult fields on description change" do
          expect_product_update %w[description rated_as_adult]

          @product.update!(description: "I have a great idea for a description.")
        end

  test "sends updated rated_as_adult field on `is_adult` change" do
  ...
  test "sends updated is_recommendable & display_product_reviews fields on `display_product_reviews` change" do
  ...
  test "sends updated is_recommendable and is_alive_on_profile and is_alive fields on `purchase_disabled_at` change" do
  ...
  test "sends updated is_recommendable and is_alive_on_profile and is_alive fields on `banned_at` change" do
          expect_product_update %w[is_recommendable is_alive_on_profile is_alive]

          @product.update!(banned_at: Time.current)
        end

  test "sends updated is_recommendable and is_alive_on_profile and is_alive fields on `deleted_at` change" do
          expect_product_update %w[is_recommendable is_alive_on_profile is_alive]

          @product.update!(deleted_at: Time.current)
        end

  test "sends updated taxonomy_id field on `taxonomy_id` change" do
          expect_product_update %w[taxonomy_id is_recommendable]

          @product.update!(taxonomy: create(:taxonomy))
        end

  test "sends updated is_recommendable & is_alive_on_profile field on `archived` change" do
          expect_product_update %w[is_recommendable is_alive_on_profile]

          @product.update!(archived: true)
        end

  test "sends updated price_cents & available_price_cents fields on price_range change" do
  ...
  test "sends updated price_cents & available_price_cents fields on price_cents change" do
          expect_product_update %w[price_cents available_price_cents]
  ...
  test "sends updated is_preorder on preorder state change" do
  ...
  test "sends updated is_recommendable field on max_purchase_count change" do
          expect_product_update %w[is_recommendable]

          @product.update!(max_purchase_count: 100)
        end

  test "sends updated is_call & native_type fields when native_type changes to call" do
  ...
  test "does not index changes when transaction is rolled back" do
  ...
  test "updates review search properties on review save" do
  ...
  test "updates the index for the associated product on product file save" do
  ...
  context_ "on user update" do
  test "sends updated rated_as_adult & creator_name on name change" do
          expect_product_update %w[rated_as_adult creator_name]
          user = @product.user
          user.update!(name: "New user name")
        end

  test "sends updated rated_as_adult & creator_name on username change" do
          expect_product_update %w[rated_as_adult creator_name]
          @product.user.update!(username: "newusername")
        end

  test "sends updated rated_as_adult field on bio change" do
          expect_product_update %w[rated_as_adult]
          user = @product.user
          user.update!(bio: "New user bio")
        end

  test "sends updated rated_as_adult field on all_adult_products change" do
          expect_product_update %w[rated_as_adult]
          user = @product.user
          user.update!(all_adult_products: true)
        end

  test "sends updated is_recommendable flag on payment_address update" do
          expect_product_update %w[is_recommendable]
  ...
  test "sends updated is_recommendable flag on creation of active bank_account" do
          expect_product_update %w[is_recommendable]
  ...
  test "does not index products if user already has an active bank_account" do
          create(:canadian_bank_account, user: @product.user)
          expect(ProductIndexingService).not_to receive(:perform)
          create(:canadian_bank_account, user: @product.user)
        end

  test "sends updated is_recommendable field when user is marked compliant" do
          expect_product_update %w[is_recommendable]
          @product.user.mark_compliant!(author_id: create(:user).id)
        end

  test "sends updated is_recommendable field when compliant user is suspended for fraud" do
          admin = create(:user)
  ...
  test "does nothing if no watched attributes change" do
          expect(ProductIndexingService).not_to receive(:perform)
          @product.user.update!(kindle_email: "someone@kindle.com")
        end
      end

  test "sends updated tags when a tag is created" do
        expect_product_update %w[tags]
        @product.tag!("new tag")
      end

  test "sends updated filetypes when a file is added" do
        expect_product_update %w[filetypes]
        @product.save_files!([{ external_id: SecureRandom.uuid, url: "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachment/pic.jpg" }])
      end

  test "enqueues the job when a purchase transitions to the successful state" do
        expect_product_update %w[is_recommendable]
  ...
  test "enqueues the job when a purchase transitions to the preorder_authorization_successful state" do
        expect_product_update %w[is_recommendable]
