...
class ModulesProductSearchableNameFieldSearchTest < ActiveSupport::TestCase



  context_ "Product::Searchable - Name fields search scenarios" do
    before do
      @product = create(:product_with_files)
    end

  context_ "on name fields for all products", :elasticsearch_wait_for_refresh do
      before do
  ...
  test "supports ES query syntax" do
  ...
  test "sorts products by sales_volume and created_at", :sidekiq_inline do
  ...
  test "excludes non-recommendable and recommendable-adult products" do
          args = Link.partial_search_options(query: "sampl")
  ...
  test "excludes non-recommendable products, but keeps adult recommendable ones when include_rated_as_adult is true" do
