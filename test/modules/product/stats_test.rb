...
class ProductStatsTest < ActiveSupport::TestCase
  self.described_class = Product::Stats



  context_ Product::Stats do
    include ManageSubscriptionHelpers

  context_ ".successful_sales_count", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  test "does not take into account refunded/disputed purchases" do
  ...
  test "does not double count subscriptions that have been upgraded" do
  ...
  test "supports multiple products" do
  ...
  context_ "#successful_sales_count" do
  test "returns value from the class method" do
  ...
  context_ "#total_usd_cents", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  test "returns net revenue" do
        product = create(:product)
  ...
  context_ "with a created_after option" do
  test "returns the sum of matching net revenue" do
          product = create(:product)
  ...
  test "does not take into account refunded or not charged purchases", :sidekiq_inline, :elasticsearch_wait_for_refresh do
        @product = create(:product, price_cents: 500)
        create(:purchase, link: @product)
        create(:purchase, link: @product, stripe_refunded: true)
  ...
  context_ "#total_fee_cents", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  test "does not take into account fully refunded, not charged, chargeback not reverted, giftee purchases" do
        product = create(:product, price_cents: 500)
        create(:purchase, link: product)
        create(:purchase, link: product, stripe_refunded: true)
  ...
  test "returns the correct pending balance" do
        expect(@product.pending_balance).to eq 1100
      end

  test "shoud return $0 if there are no pending balances" do
        @sub.update_attribute(:cancelled_at, Time.current)
        expect(@product.pending_balance).to eq 0
      end

  context_ "when a subscription has been updated" do
  test "uses the updated purchase price" do
  ...
  context_ "#revenue_pending" do
  context_ "when product has a duration set" do
        before do
          @product = create(:subscription_product, user: create(:user), duration_in_months: 12)
  ...
  test "returns the correct pending revenue" do
          expect(@product.revenue_pending).to eq 1100
        end
      end

  context_ "when product has no duration set" do
        before do
          @product = create(:product)
        end
  ...
  context_ ".monthly_recurring_revenue", :sidekiq_inline, :elasticsearch_wait_for_refresh do
      subject { Link.monthly_recurring_revenue(products: [@product]) }

  context_ "for a non-tiered subscription product" do
        before do
  ...
  test "returns the monthly recurring revenue" do
          is_expected.to eq 1833.3333129882812
        end

  test "discards inactive subscriptions" do
          @sub_yearly.update_attribute(:cancelled_at, Time.current)
          is_expected.to eq 1000
        end
      end

  context_ "for a tiered membership product", :vcr do
        before do
  ...
  context_ "with a subscription that has been upgraded" do
  test "uses the new 'original' purchase" do
  ...
  test "returns only the active subscriptions" do
            # One yearly $10 subscription
            is_expected.to eq 83.33333587646484
          end
        end

  context_ "with a subscription cancelled by buyer but still active" do
          before do
            @sub_monthly.update!(
  ...
  context_ "with a subscription cancelled by admin but still active" do
          before do
            @sub_monthly.update!(
  ...
  context_ "with a cancelled subscription" do
          before do
            @sub_monthly.update!(
              cancelled_by_buyer: true,
  ...
  context_ "when multiple tiered membership products exist" do
          before do
  ...
  test "does not count subscriptions from other products" do
            is_expected.to eq 383.33333587646484
          end
        end
      end
  ...
  test "returns monthly recurring revenue from the class method" do
        product = create(:product)
  ...
  context_ "#number_of_views" do
  test "returns the views total", :sidekiq_inline, :elasticsearch_wait_for_refresh do
        product = create(:product)
