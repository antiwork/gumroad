...
class ProductPreviewTest < ActiveSupport::TestCase
  self.described_class = Product::Preview
  ...
  context_ "Asset Previews", :vcr do
  context_ "with attached files" do
  test "has default attributes if no preview exists" do
          link = create(:product)
  ...
  test "saves existing preview image as its main asset preview" do
          link = build(:product)
  ...
  test "saves a new preview image with correct attributes" do
          link = create(:product)
          expect do
  ...
  test "saves a new preview video with correct attributes" do
          link = create(:product)
          expect do
  ...
  test "does not save an arbitrary filetype" do
          link = create(:product)
          expect do
            expect do
          ...
          end.not_to change { link.main_preview }
        end

  test "creates a new asset preview when preview is changed" do
  ...
  test "does not create a product preview when an existing product with no asset previews is saved" do
          link = create(:product, preview: nil, preview_url: nil)
          expect do
            link.name = "A link by any other name would preview as sweet"
            link.save!
          end.not_to change { link.asset_previews.alive.count }.from(0)
        end

  test "does nothing if a product has an asset preview and its preview is unchanged" do
          ...
          end.not_to change { link.main_preview.updated_at }
        end

  test "deletes an asset preview" do
          link = create(:product, preview: @png_file)
          expect { link.preview = nil }.to change { link.main_preview }.to(nil)
        end
      end

  context_ "with embeddable preview URLs" do
  test "creates with correct dimensions from a fully-qualified URL" do
  ...
  test "creates with a relative URL" do
          link = create(:product, preview_url: "https://www.youtube.com/watch?v=apiu3pTIwuY")
          expect(link.preview_oembed_url).to eq("https://www.youtube.com/embed/apiu3pTIwuY?feature=oembed&showinfo=0&controls=0&rel=0&enablejsapi=1")
        end

  test "deletes an asset preview URL" do
          link = create(:product, preview_url: "https://www.youtube.com/watch?v=apiu3pTIwuY")
          expect do
            link.preview = ""
            link.save!
