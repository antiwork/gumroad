...
class ProductRecommendationsTest < ActiveSupport::TestCase
  self.described_class = Product::Recommendations



  context_ Product::Recommendations, :elasticsearch_wait_for_refresh do
    before do
      @product = create(:product, user: create(:compliant_user, name: "Some creator person"), taxonomy: create(:taxonomy))
    end

  test "is true if has recent sale" do
      create(:purchase, :with_review, link: @product, created_at: 1.week.ago)
      expect(@product.recommendable?).to be(true)
    end

  test "is true if no recent sale" do
      create(:purchase, :with_review, link: @product, created_at: 4.months.ago)
      expect(@product.recommendable?).to be(true)
    end

  test "is true even if the product is rated to be adult" do
      create(:purchase, :with_review, link: @product, created_at: 1.week.ago)
  ...
  test "is true if it displays product reviews" do
      create(:purchase, :with_review, link: @product, created_at: 1.month.ago)
      expect(@product.recommendable?).to be(true)
    end

  test "is false if it does not display product reviews" do
      create(:purchase, :with_review, link: @product, created_at: 1.month.ago)
  ...
  test "is false if no sale made" do
  ...
  test "is false if there are no non-refunded sales" do
  ...
  context_ "when taxonomy is not set" do
      before do
  ...
  test "is true if it has a review" do
      create(:purchase, :with_review, link: @product, created_at: 1.week.ago)

      expect(@product.recommendable?).to be(true)
    end

  test "is true if does not have any review" do
      create(:purchase, link: @product, created_at: 1.week.ago)

      expect(@product.recommendable?).to be(true)
    end

  test "is false if item is out of stock" do
  ...
  test "is false if item is in stock" do
