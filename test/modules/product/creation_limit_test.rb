...
class ProductCreationLimitTest < ActiveSupport::TestCase
  self.described_class = Product::CreationLimit



  context_ Product::CreationLimit, :enforce_product_creation_limit do
    let(:non_compliant_user) { create(:user, user_risk_state: "not_reviewed") }
    let(:compliant_user) { create(:user, user_risk_state: "compliant") }

  context_ "for non-compliant users" do
  test "prevents creating more than 10 products in 24 hours" do
  ...
  context_ "for compliant users" do
  test "allows creating up to 100 products in 24 hours" do
  ...
  test "skips the daily product creation limit" do
  ...
  context_ "with custom daily_product_creation_limit" do
  test "uses the user's custom limit when set" do
  ...
  context_ ".bypass_product_creation_limit" do
  test "bypasses the limit within the block and restores it afterwards" do
