...
class ProductReviewStatTest < ActiveSupport::TestCase
  self.described_class = Product::ReviewStat



  context_ Product::ReviewStat do
    before do
      @product = create(:product)
    end

  context_ "when the product_review_stat record exists" do
      before do
  ...
  context_ "#update_review_stat_via_rating_change & #update_review_stat_via_purchase_changes" do
  test "computes reviews_count & average_rating for the product correctly" do
  ...
  test "computes aggregate fields correctly for a new rating added" do
  ...
  test "computes aggregate fields correctly for an existing rating being updated" do
  ...
  test "computes aggregate fields correctly for an existing rating becoming invalid due to a refund" do
  ...
  test "computes aggregate fields correctly for an existing rating becoming invalid due to purchase state" do
          purchase = create(:purchase, link: @product)
          create(:product_review, purchase:, rating: 1)
  ...
  test "computes aggregate fields correctly for an existing rating becoming invalid due to a charge back" do
          purchase = create(:purchase, link: @product)
          create(:product_review, purchase:, rating: 1)
  ...
  test "computes aggregate fields correctly for an existing rating for a free purchase becoming invalid due to a revoke" do
  ...
  test "computes aggregate fields correctly for an existing rating for a paid purchase NOT becoming invalid due to a revoke" do
          purchase = create(:purchase, link: @product)
          create(:product_review, purchase:, rating: 1)
  ...
  context_ "#average_rating" do
  test "returns the average rating based on all the reviews from successful purchases of the product" do
          expect(@product.average_rating).to eq(@expected_average_rating)
        end
      end

  context_ "#rating_counts" do
  test "returns a list of counts of product reviews by rating" do
          expected = { 1 => 2, 2 => 0, 3 => 1, 4 => 0, 5 => 0 }
          expect(@product.rating_counts).to eq(expected)
        end
      end

  context_ "#reviews_count" do
  test "returns the count of reviews from successful purchases" do
          expect(@product.reviews_count).to eq(3)
        end
      end

  context_ "#sync_review_stat & #generate_review_stat_attributes" do
  test "recomputes reviews_count & average_rating for the product correctly" do
  ...
  test "creates product_review_stat when there are reviews & no product_review_stat" do
  ...
  test "doesn't create product_review_stat when there are no reviews & no product_review_stat" do
          product = create(:product)
          product.sync_review_stat
          expect(product.product_review_stat).to be_nil
        end

  test "resets product_review_stat when there are no reviews" do
          product = create(:product)
  ...
  context_ "when the product_review_stat record does not exist" do
  context_ "#average_rating" do
  ...
  test "#update_review_stat_via_rating_change allows to uncount a rating" do
      product = create(:product)
      purchase = create(:purchase, link: product)
  ...
  test "does nothing if there are no purchase changes" do
        product = create(:product)
        purchase = create(:purchase, link: product)
  ...
  test "does nothing when updating a subsequent subscription purchase is updated" do
