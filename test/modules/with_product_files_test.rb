...
class WithProductFilesTest < ActiveSupport::TestCase
  self.described_class = WithProductFiles



  context_ WithProductFiles do
  context_ "associations" do
  context_ "has many `product_folders`" do
  test "does not return deleted folders" do
  ...
  context_ "#needs_updated_entity_archive?" do
  test "returns false when irrelevant changes are made to a product's rich content" do
  ...
  test "returns true when relevant changes have been made to an entity's rich content" do
  ...
  context_ "#map_rich_content_files_and_folders" do
  context_ "when there is no rich content provider" do
  ...
  context_ "when the rich content has no files" do
  test "returns an empty hash" do
          product = create(:product)
  ...
  context_ "when the rich content has files" do
  context_ "when the rich content has only one untitled default page" do
  test "does not include the fallback page title in the mapping" do
            product_file1 = create(:product_file, display_name: "First file")
  ...
  context_ "when the rich content has multiple pages" do
  test "includes the custom page titles as well as the incremented untitled page titles in the mapping" do
            product_file1 = create(:product_file, display_name: "First file")
            product_file2 = create(:product_file, display_name: "Second file")
  ...
  context_ "when the rich content has file groups" do
  test "includes the custom folder names as well as the incremented untitled folder names in the mapping" do
            product_file1 = create(:product_file, display_name: "First file")
            product_file2 = create(:product_file, display_name: "Second file")
            product_file3 = create(:product_file, display_name: "Third file")
  ...
  context_ "#has_stream_only_files?" do
  ...
  test "returns false if product only has non-streamable files" do
  ...
  test "returns false if product has streamable files that are not marked as stream-only" do
        product.product_files << create(:streamable_video)

        expect(product.has_stream_only_files?).to eq(false)
      end

  test "returns true if product has streamable files that are marked as stream-only" do
        product.product_files << create(:readable_document)
        product.product_files << create(:streamable_video)
  ...
  test "returns false if product has a file that is not stream-only" do
        product.product_files << create(:streamable_video, stream_only: true)
        product.product_files << create(:readable_document)
  ...
  test "returns true if product only has stream-only files" do
        product.product_files << create(:streamable_video, stream_only: true)
        product.product_files << create(:streamable_video, stream_only: true)
  ...
  context_ "#save_files!" do
  context_ "when called on a Link record" do
        let(:product) { create(:product_with_pdf_files_with_size) }

  test "enqueues a `PdfUnstampableNotifierJob` job when new stampable files are added" do
  ...
  test "does not enqueue a `PdfUnstampableNotifierJob` job when new non-stampable files are added" do
          product_files_params = product.product_files.each_with_object([]) { |file, params| params << { external_id: file.external_id, url: file.url } }
  ...
  test "enqueues a `PdfUnstampableNotifierJob` job when existing files are marked as stampable" do
  ...
  test "does not enqueue a `PdfUnstampableNotifierJob` job when existing files are marked as non-stampable" do
  ...
  test "does not enqueue a `PdfUnstampableNotifierJob` job when existing files are removed" do
          expect do
            product.save_files!([])
  ...
  test "does not enqueue a `PdfUnstampableNotifierJob` job when no changes are made to files" do
  ...
  test "does not enqueue a `PdfUnstampableNotifierJob` job for a non-product resource" do
  ...
  test "sets content_updated_at when new files are added" do
          product_files_params = product.product_files.each_with_object([]) { |file, acc| acc << { external_id: file.external_id, url: file.url } }
  ...
  test "does not set content_updated_at when existing files are removed" do
          expect do
  ...
  test "raises error on the product if url is invalid when creating an external link file" do
          product_files_params = product.product_files.each_with_object([]) { |file, acc| acc << { external_id: file.external_id, url: file.url } }
  ...
  test "preserves folder_id on file if folder is deleted" do
          product = create(:product)
  ...
  test "preserves folder_id on file if folder does not exist" do
          product = create(:product)
  ...
  test "sets folder_id to nil if file is moved out of folder" do
          product = create(:product)
  ...
  test "respects the `modified` flag if present on file" do
          product = create(:product)
  ...
  context_ "when called on an Installment record" do
        let(:installment) { create(:installment) }

  test "does not enqueue a `PdfUnstampableNotifierJob` job when a new file is added" do
          expect do
  ...
  test "does not enqueue a `PdfUnstampableNotifierJob` job when a file is removed" do
          installment.save_files!([{ external_id: SecureRandom.uuid, url: "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachment/pic.jpg" }])
  ...
  test "generates the product files archive" do
          expect do
            installment.save_files!([{ external_id: SecureRandom.uuid, url: "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachment/pic.jpg" }])
          end.to change { installment.product_files.alive.count }.by(1)
  ...
  context_ "#folder_to_files_mapping" do
  test "returns an empty hash when there aren't folders with files" do
        file1 = create(:product_file, display_name: "First file")
        product = create(:product)
  ...
  test "returns the proper mapping when there are folders with files" do
        file1 = create(:product_file, display_name: "First file")
        file2 = create(:product_file, display_name: "Second file")
  ...
  context_ "#generate_folder_archives!" do
  test "generates folder archives for valid folders in the rich content" do
        file1 = create(:product_file, display_name: "First file")
        file2 = create(:product_file, display_name: "Second file")
        file3 = create(:product_file, display_name: "Third file")
        file4 = create(:product_file, display_name: "Fourth file")
  ...
  test "regenerates file group archives containing the provided files" do
        ...
        expect { product.generate_folder_archives! }.not_to change { archive.reload.deleted? }
  ...
  context_ "#generate_entity_archive!" do
  test "generates an entity archive" do
        file1 = create(:product_file, display_name: "First file")
        file2 = create(:product_file, display_name: "Second file")
  ...
  test "deletes the previous archive" do
        file1 = create(:product_file, display_name: "First file")
        file2 = create(:product_file, display_name: "Second file")
        installment = create(:installment, product_files: [file1, file2])
        ...
        expect { installment.generate_entity_archive! }.not_to change { installment.product_files_archives.entity_archives.alive.count }
        expect(archive.reload.alive?).to eq(false)
        expect(installment.product_files_archives.entity_archives.alive.last.product_files).to eq([file1, file2])
      end
    end

  context_ "#has_files?" do
  test "returns false if product has no files" do
        product = create(:product)
        expect(product.has_files?).to eq(false)
      end

  test "returns true if product has alive files" do
  ...
  context_ "#has_been_transcoded?" do
      before do
        @product = create(:product)
  ...
  test "indicates that it has been transcoded if one of the transcoded videos are completed" do
  ...
  test "indicates that it hasn't been transcoded if none of the transcoded videos are completed" do
  ...
  context_ "#transcode_videos!" do
  test "adds delay if there are too many videos", :freeze_time do
        product = create(:product)
