...
class ImmutableTest < ActiveSupport::TestCase
  self.described_class = Immutable



  context_ Immutable do
  ...
  context_ "updating" do
      before do
        model.save!
      end

  context_ "no changes" do
  test "is able to update a record with no changes" do
          model.save!
        end
      end

  context_ "changes allowed" do
        before do
          model.deleted_at = Time.current
        end

  test "is able to update the record" do
          model.save!
        end
      end

  context_ "changes not allowed" do
        before do
          model.first_name = "Santa Clause"
        end

  test "isn't able to update the record" do
          expect { model.save! }.to raise_error(Immutable::RecordImmutable)
        end
      end
    end

  context_ "#dup_and_save" do
      let(:model) { create(:user_compliance_info, birthday: today - 30.years) }

  context_ "changes are valid" do
        before do
  ...
  test "returns a result of true" do
          expect(@result).to eq(true)
        end

  test "returns a duplicate of the model" do
  ...
  test "returns a model with the change made" do
          expect(@new_model.birthday).not_to eq(model.birthday)
          expect(@new_model.birthday).to eq(today - 20.years)
        end

  test "returns a duplicate of the model with an id since its been persisted" do
          expect(@new_model.id).to be_present
        end

  test "marks the original model as deleted" do
          expect(model).to be_deleted
        end
      end

  context_ "changes are valid, original values are invalid" do
        before do
  ...
  context_ "changes are invalid" do
        before do
          @result, @new_model = model.dup_and_save do |new_model|
  ...
  test "returns a duplicate of the model without an id since it hasn't been persisted" do
  ...
  test "does not mark the original model as deleted" do
  ...
  context_ "changes are invalid" do
  test "raises a validation error" do
          expect do
  ...
  context_ "after the error is raised" do
          before do
            @model_id = model.id
            expect do
              model.dup_and_save! do |new_model|
  ...
  test "does not have created a new record" do
