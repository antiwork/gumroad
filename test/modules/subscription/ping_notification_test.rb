...
class SubscriptionPingNotificationTest < ActiveSupport::TestCase
  self.described_class = Subscription::PingNotification



  context_ Subscription::PingNotification do
  context_ "#payload_for_ping_notification" do
      let(:purchase) { create(:membership_purchase) }
      let(:subscription) { purchase.subscription }

  test "contains subscription details" do
  ...
  test "contains custom fields if present in original purchase" do
  ...
  test "contains license_key if product has licensing enabled" do
  ...
  context_ "cancellation event" do
  test "contains the subscription cancellation details if the input resource_name is 'cancellation'" do
  ...
  test "contains the details of who cancelled the subscription - admin/buyer/seller/payment failures" do
  ...
  test "contains subscribing user's id and email when user is present, else contains subscribing email for logged out purchases" do
  ...
  context_ "ended event" do
  test "contains the subscription ending details if the subscription ended after a fixed subscription length" do
  ...
  test "contains the subscription ending details if the subscription was cancelled" do
          ended_at = 1.minute.ago
  ...
  test "contains the subscription ending details if the subscription was terminated due to failed payments" do
          ended_at = 1.minute.ago
  ...
  test "contains empty subscription ending details if the subscription has not ended" do
  ...
  context_ "passing additional parameters" do
  test "includes those parameters in the payload" do
          subscription = create(:subscription)
          create(:membership_purchase, subscription:)
