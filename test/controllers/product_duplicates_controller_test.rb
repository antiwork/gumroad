...
class ProductDuplicatesControllerTest < ActionController::TestCase
  self.described_class = ProductDuplicatesController
  tests ProductDuplicatesController



  context_ ProductDuplicatesController do
  ...
  test "returns 404 when the id parameter is missing" do
        expect do
          post :create
        end.to raise_error(ActionController::RoutingError)
      end

  test "returns 404 when the id parameter is not the correct identifier for a product" do
        expect do
          post :create, params: { id: "invalid" }
        end.to raise_error(ActionController::RoutingError)
      end

  test "returns success when the id parameter is the correct identifier for a product" do
  ...
  test "returns an error message when the product is already duplicating" do
  ...
  test "queues DuplicateProductWorker when the id parameter is the correct identifier for a product" do
        post :create, params: { id: product.unique_permalink }

        expect(DuplicateProductWorker).to have_enqueued_sidekiq_job(product.id)
      end

  test "returns error_message when the current user is not the creator of the product" do
  ...
  test "returns an error message when the product is still duplicating" do
        product.update!(is_duplicating: true)
  ...
  test "returns an error message when the product is not duplicating or recently duplicated" do
  ...
  test "returns the stored error message when duplication failed" do
  ...
  test "successfully returns a recently duplicated product" do
