...
class HealthcheckControllerTest < ActionController::TestCase
  self.described_class = HealthcheckController
  tests HealthcheckController



  context_ HealthcheckController do
  context_ "GET 'index'" do
  test "returns 'healthcheck' as text" do
        get :index
  ...
  test "returns HTTP success when the jobs count is under limit" do
  ...
  test "returns HTTP service_unavailable when the jobs count is over the limit" do
          allow(queue_double).to receive(:size).and_return(limit + 1)
  ...
  context_ "GET 'sidekiq'" do
  context_ "Sidekiq queues" do
  ...
  context_ "GET 'paypal_balance'" do
  context_ "when PayPal topup is not needed (Redis key is false)" do
        before do
  ...
  context_ "when Redis key is not set" do
        before do
          $redis.del(RedisKey.paypal_topup_needed)
        end

  test "returns HTTP service_unavailable" do
          get :paypal_balance
  ...
  context_ "when PayPal topup is needed (Redis key is true)" do
        before do
  ...
  context_ "when the successful purchases count meets the threshold" do
        before do
  ...
  context_ "when the successful purchases count is below the threshold" do
        before do
  ...
  context_ "when successful purchases are older than 10 minutes" do
        before do
          $redis.set(redis_key, 1)
          create(:purchase, purchase_state: "successful", created_at: 15.minutes.ago)
        end

  test "ignores them and returns HTTP service_unavailable" do
          get :purchases
  ...
  context_ "when the Redis threshold is not set" do
