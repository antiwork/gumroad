...
class TaxCenterControllerTest < ActionController::TestCase
  self.described_class = TaxCenterController
  tests TaxCenterController



  context_ TaxCenterController, type: :controller, inertia: true do
  ...
  test "uses the provided year parameter" do
  ...
  context_ "when tax_center feature is disabled" do
        before do
          Feature.deactivate_user(:tax_center, seller)
        end

  test "redirects to dashboard with alert" do
          get :index
  ...
  context_ "when seller is not from the US" do
        before do
  ...
  test "sends the tax form PDF file" do
  ...
  context_ "when tax form does not exist" do
  ...
  context_ "when tax form has stripe_account_id stored" do
  test "uses the stored stripe_account_id if it belongs to the seller" do
  ...
  context_ "when tax form was filed with a different Stripe account" do
  ...
  test "uses the old Stripe account stored in the tax form" do
          pdf_tempfile = Tempfile.new(["tax_form", ".pdf"])
          pdf_tempfile.write("PDF content")
          pdf_tempfile.rewind
  ...
  context_ "when stored stripe_account_id does not belong to seller" do
        before do
          tax_form = UserTaxForm.last
  ...
  context_ "when download fails from Stripe API" do
        before do
          allow_any_instance_of(StripeTaxFormsApi).to receive(:download_tax_form).and_return(nil)
        end

  test "redirects to the S3 download url if it is present" do
