...
class LoginsControllerTest < ActionController::TestCase
  self.described_class = LoginsController
  tests LoginsController



  context_ LoginsController, type: :controller, inertia: true do
    render_views
  ...
  context_ "when disable_login_recaptcha feature flag is active" do
        before { Feature.activate(:disable_login_recaptcha) }
        after { Feature.deactivate(:disable_login_recaptcha) }

  test "does not pass reCAPTCHA site key" do
          get :new
  ...
  context_ "with an email in the query parameters" do
  test "renders successfully" do
  ...
  context_ "with an email in the next parameter" do
  test "renders successfully" do
  ...
  test "redirects with the 'next' value from the referrer if not supplied in the params" do
  ...
  test "does not redirect with the 'next' value from the referrer if it equals to the root route" do
  ...
  test "responds with bad request if format is json" do
          get :new, params: { next: @next_url }, format: :json
          expect(response).to be_a_bad_request
        end

  test "sets the application" do
          get :new, params: { next: @next_url }

          expect(assigns[:application]).to eq @oauth_application
        end

  test "sets noindex header when next param starts with /oauth/authorize" do
          get :new, params: { next: "/oauth/authorize?client_id=123" }
          expect(response.headers["X-Robots-Tag"]).to eq "noindex"
        end

  test "does not set noindex header for regular login" do
          get :new
  ...
  test "logs in if user already exists" do
        post "create", params: { user: { login_identifier: @user.email, password: "password" } }
        expect(response).to redirect_to(dashboard_path)
      end

  test "shows proper error if password is incorrect" do
  ...
  test "shows proper error if email doesn't exist" do
  ...
  test "returns an error with no params" do
        post "create"
        expect(response).to redirect_to(login_path)
        expect(flash[:warning]).to eq("An account does not exist with that email.")
      end

  test "logs in if user already exists and redirects to next if present" do
        post "create", params: { user: { login_identifier: @user.email, password: "password" }, next: "/about" }
        expect(response).to redirect_to("/about")
      end

  test "does not redirect to absolute url" do
  ...
  test "redirects back to subdomain URL" do
  ...
  test "disallows logging in if the user has been deleted" do
  ...
  test "does not log in a user when reCAPTCHA is not completed" do
  ...
  test "logs in a user when reCAPTCHA is completed correctly" do
        post :create, params: { user: { login_identifier: @user.email, password: "password" } }
  ...
  test "skips reCAPTCHA validation and logs in the user" do
          allow(controller).to receive(:valid_recaptcha_response?).and_return(false)
  ...
  test "logs in a user when reCAPTCHA site key is not set in development environment" do
  ...
  test "does not log in a user when reCAPTCHA site key is not set in production environment" do
  ...
  test "sets the 'Remember Me' cookie" do
        post :create, params: { user: { login_identifier: @user.email, password: "password" } }

        expect(response.cookies["remember_user_token"]).to be_present
      end

  context_ "user suspended" do
        before do
          @bad_user = create(:user)
        end

  context_ "for ToS violation" do
          before do
  ...
  test "is allowed to login" do
            post :create, params: { user: { login_identifier: @user.email, password: "password" } }
            expect(response).to redirect_to(dashboard_path)
          end
        end

  test "allows a user suspended for fraud to log in" do
  ...
  context_ "referrer present" do
  context_ "referrer is root" do
          before do
            @request.env["HTTP_REFERER"] = root_path
          end
  ...
  context_ "referrer is not root" do
          before do
            @product = create(:product)
            @request.env["HTTP_REFERER"] = short_link_path(@product)
          end

  test "redirects back to the last location" do
            post :create, params: { user: { login_identifier: @user.email, password: "password" } }
  ...
  context_ "two factor authentication" do
        before do
  ...
  context_ "two factor authentication stale session" do
  test "does not redirect to two factor auth when the pending 2FA session belongs to another user" do
  ...
  test "clears cookies on sign out" do
  ...
  test "resets impersonated user" do
  ...
  test "signs out the user and redirects to root path with notice" do
        delete :destroy
