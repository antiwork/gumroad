...
class CustomerSurchargeControllerTest < ActionController::TestCase
  self.described_class = CustomerSurchargeController
  self.rspec_metadata = { vcr: true }
  tests CustomerSurchargeController



  context_ CustomerSurchargeController, :vcr do
  ...
  test "returns the correct tax value when buyer location is British Columbia Canada" do
  ...
  test "returns tax as 0 when buyer location is EU and a valid VAT ID is provided" do
  ...
  test "allows querying multiple products at once" do
  ...
  context_ "for a subscription", :vcr do
  context_ "when original purchase was charged VAT" do
        before :each do
          setup_subscription_with_vat
        end

  context_ "and the buyer is in the EU" do
  test "uses the original purchase's location info" do
  ...
  context_ "and the buyer is currently not in the EU" do
  test "still uses the original purchase's location info" do
  ...
  context_ "when original purchase was not charged VAT" do
        before :each do
          setup_subscription
        end
  ...
  context_ "and the buyer is currently in the EU" do
  test "still uses the original purchase's location info" do
            post "calculate_all", params: { products: [{ permalink: @product.unique_permalink, price: 500, quantity: 1, subscription_id: @subscription.external_id }], postal_code: 10115, country: "DE" }, as: :json
  ...
  context_ "when original purchase had a VAT ID" do
  test "uses the VAT ID" do
