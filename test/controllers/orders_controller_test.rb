...
class OrdersControllerTest < ActionController::TestCase
  self.described_class = OrdersController
  ...
  context_ OrdersController, :vcr do
    before do
  ...
  test "creates an order, a charge, and a purchase" do
          expect do
            expect do
              expect do
  ...
  test "responds with success: false for specific line item" do
            post :create, params: single_purchase_params
  ...
  test "creates an order and a failed purchase and a charge if purchase failed after charge attempt" do
            expect do
              expect do
                expect do
                  post :create, params: single_purchase_params
  ...
  test "creates a gift purchase" do
            expect do
              post :create, params: single_purchase_params
  ...
  context_ "when purchasing a call", :freeze_time do
  ...
  context_ "with all required values" do
  ...
  test "creates a purchase with the correct start and end time" do
              expect do
  ...
  test "returns an error for the line item" do
              post :create, params: call_purchase_params
  ...
  context_ "invalid call start time" do
            let(:call_purchase_params) do
              {
                line_items: [{
                  uid: "call-product-uid",
  ...
  context_ "selected time is no longer available" do
            let(:call_purchase_params) do
              {
                line_items: [{
                  uid: "call-product-uid",
  ...
  context_ "purchase attribution to UTM links", :sidekiq_inline do
  ...
  test "attributes the qualified purchase to the matching UTM link having a visit with the same browser guid" do
  ...
  test "does not attribute purchase when there is no matching UTM link visit" do
            utm_link = create(:utm_link, seller: product_1.user)
  ...
  test "does not attribute purchase when browser guid does not match" do
            utm_link = create(:utm_link, seller: product_1.user)
  ...
  test "does not attribute a failed purchase" do
            utm_link = create(:utm_link, seller: product_1.user)
  ...
  test "does not attribute a purchase if the :utm_links feature is not active" do
            utm_link = create(:utm_link, seller: product_1.user)
            create(:utm_link_visit, utm_link:, browser_guid:)
  ...
  context_ "multiple purchases" do
  ...
  test "creates an order, the associated purchases and a combined charge" do
          expect do
            expect do
              expect do
  ...
  test "creates an order, the associated purchases and a combined charge for each seller" do
  ...
  context_ "response format" do
  context_ "when all purchases succeed" do
  test "responds with success: true for every line item" do
              post :create, params: multiple_purchase_params
  ...
  context_ "when all purchases fail" do
            let(:payment_params) { StripePaymentMethodHelper.decline.to_stripejs_params }

  test "responds with success: false for every line item" do
              post :create, params: multiple_purchase_params
  ...
  test "creates an order with purchases associated" do
              expect do
                expect do
                  post :create, params: multiple_purchase_params
  ...
  context_ "when some purchases fail and some succeed" do
            before do
              product_2.update_attribute(:max_purchase_count, 0)
            end

  test "responds with proper 'success' value for each line item" do
              post :create, params: multiple_purchase_params
  ...
  context_ "when product is not found" do
  ...
  test "creates purchase events" do
  ...
  test "saves recommended information" do
  ...
  context_ "chargeable construction" do
  ...
  test "passes in the _gumroad_guid from cookies along with the purchase params" do
  ...
  test "preserves paypal_order_id for correct charging" do
            expect do
  ...
  context_ "strong params for reusable Stripe payments" do
  test "passes stripe_setup_intent_id through to order creation" do
  ...
  context_ "single item purchases that require SCA" do
          let(:price) { 10_00 }
  ...
  test "creates an in_progress purchase and preorder and renders a proper response" do
              expect do
                expect do
                  expect do
  ...
  test "creates an in_progress purchase and renders a proper response for stripe connect account" do
  ...
  context_ "allow user to test buying their own links" do
          before do
            @user = create(:user)
            sign_in(@user)
  ...
  test "creates a test purchase" do
            expect do
              expect do
                post :create, params: multiple_purchase_params
                expect(response.parsed_body["line_items"]["unique-uid-0"]["success"]).to be true
  ...
  test "sends the notification webhook if user requested" do
  ...
  test "sends the correct receipt" do
  ...
  test "doesn't allow it to go through if the guid is blank" do
          cookies[:_gumroad_guid] = ""
          expect do
            post :create, params: multiple_purchase_params
  ...
  context_ "reCAPTCHA skipping behavior" do
  test "does not attempt to verify reCAPTCHA if all purchases are free and don't require captcha" do
            ...
            expect_any_instance_of(OrdersController).not_to receive(:valid_recaptcha_response_and_hostname?)
  ...
  test "does not attempt to verify reCAPTCHA when line_items are submitted as hash-like params" do
            allow_any_instance_of(Link).to receive(:require_captcha?).and_return(false)
  ...
  test "verifies reCAPTCHA if any of the purchases require it" do
  ...
  context_ "when payment is made using a wallet" do
  ...
  test "does not attempt to verify reCAPTCHA if payment is made using a wallet" do
              expect_any_instance_of(OrdersController).not_to receive(:valid_recaptcha_response_and_hostname?)
  ...
  test "doesn't allow purchasing if reCAPTCHA verification fails" do
  ...
  test "allows purchasing if reCAPTCHA verification succeeds" do
  ...
  test "saves the user's locale in json_data" do
          expect do
  ...
  test "does not expire the product action cache" do
          Rails.cache.write("views/#{product_1.unique_permalink}_en.html", "<html>hello</html>")
          post :create, params: multiple_purchase_params
          expect(Rails.cache.read("views/#{product_1.unique_permalink}_en.html")).not_to be(nil)
        end

  context_ "purchase with variants" do
  ...
  context_ "when variant is available" do
  test "allows the purchase" do
              multiple_purchase_params[:line_items][1][:variants] = [variant.external_id]
  ...
  context_ "multi-quanity" do
  test "allows a multi-quantity purchase" do
  ...
  test "does not allow a multi-quantity purchase if the quantity exceeds the variant availability" do
              multiple_purchase_params[:line_items][1].merge!(
                variants: [variant.external_id],
  ...
  context_ "physical product with skus" do
            let(:product_2) { create(:physical_product, price_cents: price_2) }

  test "adds the default sku to the purchase and allow the purchase" do
              post :create, params: multiple_purchase_params
  ...
  test "does not add the default sku to the purchase if one exists" do
  ...
  context_ "variants with price_difference_cents" do
            let(:variant) { create(:variant, name: "small", price_difference_cents: 200, max_purchase_count: 2, variant_category: category) }

  context_ "fixed price" do
  context_ "when perceived_price_cents is incorrect" do
  test "rejects the purchase" do
                  multiple_purchase_params[:line_items][1][:variants] = [variant.external_id]
                  post :create, params: multiple_purchase_params
  ...
  context_ "perceived_price_cents is correct" do
  test "allows the purchase" do
                  multiple_purchase_params[:line_items][1].merge!(
                    variants: [variant.external_id],
  ...
  context_ "variable pricing" do
              before do
                product_2.update(price_range: "3+")
              end

  test "rejects purchase if range is not great enough" do
                multiple_purchase_params[:line_items][1].merge!(
                  variants: [variant.external_id],
                  price_range: 3,
  ...
  test "allows purchase if price_range sufficient" do
                multiple_purchase_params[:line_items][1].merge!(
                  variants: [variant.external_id],
                  price_range: 6,
  ...
  test "does not add the purchases made into the session anymore" do
          post :create, params: multiple_purchase_params
  ...
  test "fails the purchase if required custom fields are not filled in" do
  ...
  test "adds custom fields to the purchase object" do
  ...
  test "does not set custom fields that are not set on the product" do
  ...
  test "transforms field values to boolean for checkbox" do
  ...
  test "sets the bundle custom fields" do
  ...
  test "allows an affiliate purchase to go through and associates the purchase to the affiliate through AffiliateCredit" do
            expect do
              post :create, params: multiple_purchase_params
            end.to change(Purchase.successful, :count).by(2)
  ...
  context_ "when multiple affiliates have referred the buyer" do
  ...
  test "gives credit to the last non-suspended affiliate that referred the buyer" do
              post :create, params: multiple_purchase_params
  ...
  test "credits a direct affiliate if a cookie is set for a different product by that seller" do
  ...
  test "does not credit the affiliate if a cookie is set for a product by a different seller" do
  ...
  test "credits the direct affiliate over the global affiliate if both exist" do
  ...
  test "does not credit a global affiliate that does not have a cookie set, even if another cookie is set for that seller's products" do
              product = create(:product, :recommendable)
  ...
  test "does not credit a global affiliate that makes the purchase themself (to fraudently obtain a discount)" do
  ...
  test "does not set the (externally supplied) `affiliate_id` for an ineligible affiliate purchase" do
  ...
  context_ "recommended product" do
            before do
              multiple_purchase_params[:line_items][0].merge!(was_product_recommended: true, recommended_by: "discover")
  ...
  test "allows the purchase to go through as a recommended product purchase, not associated with an affiliate" do
              post :create, params: multiple_purchase_params
  ...
  context_ "when 'affiliate_id' is passed in as a request parameter" do
            before(:each) do
  ...
  context_ "when a matching affiliate cookie exists" do
  test "doesn't make use of 'affiliate_id' from params" do
  ...
  context_ "when a matching affiliate cookie doesn't exist" do
              before(:each) do
                cookies.delete "#{Affiliate::AFFILIATE_COOKIE_NAME_PREFIX}#{direct_affiliate.external_id}"
              end

  test "fetches affiliate using the 'affiliate_id' parameter" do
  ...
  context_ "when the 'affiliate_id' parameter doesn't match with the product's affiliates" do
                let(:another_direct_affiliate) { create(:direct_affiliate, affiliate_user:, affiliate_basis_points: 1500) }

  test "doesn't credit the affiliate commission" do
  ...
  test "creates a successful test purchase" do
            expect do
              expect do
                post :create, params: multiple_purchase_params
                expect(response.parsed_body["line_items"]["unique-id-1"]["success"]).to be(true)
  ...
  test "sends the ping webhook" do
  ...
  test "creates a successful purchase" do
  ...
  test "assigns the new credit card to the logged in user" do
                post :create, params: multiple_purchase_params
  ...
  test "uses the buyer's existing credit card" do
  ...
  test "does not allow sign-up when gifter already has an account set up" do
  ...
  test "passes through giftee_id when provided instead of email" do
  ...
  test "allows the purchase with no cc creds" do
            expect do
              post :create, params: multiple_purchase_params
              expect(response.parsed_body["can_buyer_sign_up"]).to eq(false)
            end.to change(Purchase.successful, :count).by(2)
  ...
  test "stores the ip address of the client" do
          post :create, params: multiple_purchase_params
          expect(Purchase.last.ip_address).not_to be(nil)
        end

  context_ "failed purchase" do
          let(:payment_params) { StripePaymentMethodHelper.decline.to_stripejs_params }

  test "does not error with empty card country string" do
  ...
  test "sets the proper payment attributes" do
            post :create, params: multiple_purchase_params
  ...
  test "sets to false if not mobile" do
  ...
  test "sets to true if mobile" do
  ...
  context_ "when url_parameters is empty and referrer contains product URL with query string" do
  context_ "when query string param contains one value" do
  test "sets url_parameters from referrer" do
  ...
  context_ "when query string param contains multiple values" do
  test "sets url_parameters from referrer with values in an array" do
  ...
  context_ "when product URL is a subdomain URL" do
  test "sets url_parameters from referrer" do
  ...
  context_ "when url_paramers is empty and referrer doesn't contain a valid product url" do
  test "doesn't set url_parameters from referrer" do
  ...
  context_ "when url_parameters is set and referrer contains product URL with query string" do
  test "sends the ping using the configured url_parameters" do
              multiple_purchase_params[:referrer] = "#{product_2.long_url}?test=1234"
  ...
  context_ "when it creates a url redirect" do
  test "returns the url_redirect location in the response" do
            post :create, params: multiple_purchase_params
  ...
  test "returns url_redirect for 0+ links" do
  ...
  context_ "remember who made the purchase" do
  test "tracks a valid user purchase" do
            sign_in create(:user)
  ...
  test "tracks a valid anonymous purchase" do
            post :create, params: multiple_purchase_params
  ...
  test "sets the purchaser if buyer is logged out user" do
            user = create(:user)
  ...
  test "does not set the purchaser if buyer is seller" do
  ...
  test "fails the purchase gracefully if the credit card is bad, even in production where tell_chat fully runs" do
  ...
  context_ "cookies" do
  context_ "when cookies are turned off" do
            before do
              cookies[:_gumroad_guid] = nil
            end

  context_ "when all purchases are free" do
              before do
                product_1.update!(price_range: "0+")
                product_2.update!(price_range: "0+")
  ...
  context_ "when at least one paid purchase" do
              before do
                product_1.update!(price_range: "0+")
  ...
  context_ "bots" do
  test "detects the bot and does not create a purchase" do
  ...
  test "attributes all successful purchases in the order" do
            utm_link = create(:utm_link, seller: product_1.user)
            utm_link_visit = create(:utm_link_visit, utm_link:, browser_guid:)
  ...
  test "only attributes successful purchases when some fail" do
  ...
  context_ "when purchaser's email is empty" do
  ...
  test "saves the purchase email as purchaser's unconfirmed email" do
  ...
  test "does not update purchaser's email if an account already exists with the email" do
  ...
  test "does not update purchaser's email if current email is not empty" do
          user = create(:user)
  ...
  test "returns 404 for plain external_id" do
        expect do
          post :confirm, params: { id: order.external_id }
        end.to raise_error(ActionController::RoutingError)
      end

  test "returns 404 for expired token" do
  ...
  context_ "when purchase was marked as failed" do
        before do
  ...
  context_ "when SCA fails" do
  test "marks purchase as failed and renders an error" do
          post :confirm, params: {
            id: secure_id,
            stripe_error: {
  ...
  context_ "when confirmation fails" do
        before do
  ...
  test "does not delete the bundle cookie" do
  ...
  context_ "when confirmation succeeds" do
        before do
          allow_any_instance_of(Stripe::PaymentIntent).to receive(:confirm)
        end

  test "confirms the purchases in the order" do
  ...
  context_ "when the purchase doesn't belong to a Charge" do
  test "does not send the charge receipt" do
            expect(purchase.reload.successful?).to eq(false)
            expect(Purchase::ConfirmService).to receive(:new).with(hash_including(purchase:)).and_call_original
  ...
  test "sends the charge receipt" do
            expect(purchase.reload.successful?).to eq(false)
            expect(Purchase::ConfirmService).to receive(:new).with(hash_including(purchase:)).and_call_original
