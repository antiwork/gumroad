...
class PurchasesControllerTest < ActionController::TestCase
  self.described_class = PurchasesController
  self.rspec_metadata = { vcr: true }
  tests PurchasesController
  ...
  context_ PurchasesController, :vcr do
  ...
  test "allows to update giftee_email address" do
  ...
  test "allows to update shipping information for the seller" do
  ...
  test "does not change shipping information for blank params" do
  ...
  test "does not allow to update if signed in user is not a seller" do
          sign_in create(:user)
          put :update, params: { id: obfuscated_id, street_address: "1640 3rd Street", city: "San Francisco", state: "CA", zip_code: "94107", country: "United States" }
  ...
  context_ "multiseat license" do
        before do
          @product = create(:membership_product, is_multiseat_license: true, user: seller)
        end

  test "derives product's `is_multiseat_license` for new purchases" do
  ...
  test "updates quantity for purchase with multiseat license enabled" do
          purchase = create(:purchase, link: @product)
  ...
  test "does not update quantity for purchase with multiseat license disabled" do
          @product.update(is_multiseat_license: false)
          purchase = create(:purchase, link: @product)
  ...
  test "404s for already refunded purchases" do
  ...
  test "returns 404 for PayPal purchases that are more than 6 months old" do
          put :refund, params: { id: @old_paypal_purchase.external_id, format: :json }
          expect(response.parsed_body).to eq "success" => false, "error" => "Not found"
          expect(response).to have_http_status(:not_found)
        end

  test "404s for non existent purchases" do
          put :refund, params: { id: 121_212_121, format: :json }
          expect(response.parsed_body).to eq "success" => false, "error" => "Not found"
          expect(response).to have_http_status(:not_found)
        end

  test "404s for free 0+ purchases" do
  ...
  test "404s for purchases not belong to this user" do
  ...
  test "sends refund email" do
          expect do
            put :refund, params: { id: @obfuscated_id, format: :json }
          end.to have_enqueued_mail(CustomerMailer, :refund)
        end

  test "returns status message on success" do
          put :refund, params: { id: @obfuscated_id, format: :json }
          expect(response.parsed_body["success"]).to be(true)
          expect(response.parsed_body["message"]).not_to be(nil)
        end

  test "returns error message on failure" do
  ...
  test "displays insufficient funds error if creator's paypal account does not funds to refund the purchase" do
  ...
  test "displays an error when refund amount contains commas" do
  ...
  test "issues a full refund when total amount is passed as param" do
          put :refund, params: { id: @obfuscated_id, amount: "1.00", format: :json }
  ...
  test "issues a partial refund when partial amount is passed as param" do
  ...
  context_ "when product is sold in a single unit currency type" do
          before do
  ...
  test "issues a full refund when amount param is missing" do
            put :refund, params: { id: @obfuscated_id, format: :json }
  ...
  context_ "when there's a record invalid exception" do
          before do
            allow_any_instance_of(Purchase).to receive(:refund!).and_raise(ActiveRecord::RecordInvalid)
          end

  test "notifies error tracker and responds with error message" do
  ...
  test "returns some proper json" do
          product = create(:product, user: seller)
  ...
  test "returns results sorted by score" do
          product = create(:product, user: seller)
          purchases = [
  ...
  test "does not return the recurring purchase" do
  ...
  test "does not return both gift purchases" do
          product = create(:product, user: seller)
  ...
  test "includes variant details" do
          product = create(:product, user: seller)
  ...
  context_ "for subscriptions that have been upgraded" do
          before do
            setup_subscription
          end

  test "includes the upgraded purchase" do
            seller = @product.user
  ...
  context_ "change_can_contact" do
        before do
          @product = create(:product, user: seller)
          @purchase = create(:purchase, link: @product)
        end
  ...
  test "changes can_contact and returns HTTP success" do
          expect do
  ...
  context_ "POST cancel_preorder_by_seller" do
  ...
  test "returns 404 json for invalid purchase" do
          expect do
            post :cancel_preorder_by_seller, params: { id: ObfuscateIds.encrypt(999_999) }
          end.to raise_error(ActionController::RoutingError, "Not Found")
        end

  context_ "with successful preorder" do
  ...
  context_ "when number of sales is larger than threshold" do
          before do
            stub_const("Exports::PurchaseExportService::SYNCHRONOUS_EXPORT_THRESHOLD", 1)
          end

  test "queues sidekiq job and redirects to customers path with flash message" do
            get :export
  ...
  context_ "when running sidekiq jobs" do
  test "results with the expected compiled CSV being emailed", :sidekiq_inline do
  ...
  context_ "when admin is signed in and impersonates seller" do
            before do
  ...
  test "queues sidekiq job for the admin" do
              get :export
  ...
  context_ "when sales data is smaller than threshold" do
  test "sends data as CSV" do
            get :export
            expect(response.header["Content-Type"]).to include "text/csv"
            expect_correct_csv(response.body.to_s)
          end

  test "sends data as CSV with purchases in the correct order", :elasticsearch_wait_for_refresh do
  ...
  test "can filter by products and variants" do
  ...
  test "filters by start_time and end_time" do
  ...
  test "filters out non-charged sales, and sales from other sellers" do
          create(:purchase)
  ...
  test "includes not_charged free trial purchases" do
  ...
  test "interprets the submitted start_time / end_time as UTC regardless of seller's time zone" do
  ...
  test "includes purchases at UTC midnight boundary regardless of seller's timezone" do
  ...
  context_ "GET export with mobile authentication" do
  ...
  test "uses the mobile token to export sales" do
          get :export, params: {
  ...
  context_ "when the token does not have the creator_api scope" do
          let(:access_token) do
            create("doorkeeper/access_token", application: oauth_app, resource_owner_id: seller.id, scopes: "mobile_api")
          end

  test "does not sign in the token owner" do
  ...
  context_ "when the browser has a stale seller switch cookie" do
  ...
  test "exports the mobile token owner's sales" do
            get :export, params: {
              access_token: access_token.token,
              mobile_token: Api::Mobile::BaseController::MOBILE_TOKEN
            }
  ...
  test "updates purchase and returns HTTP success" do
  ...
  context_ "PUT undo_revoke_access" do
        let(:product) { create(:product, user: seller) }
  ...
  test "resends the receipt and returns true" do
  ...
  test "resends the receipt to both gifter and giftee for gift purchases" do
  ...
  test "enqueues receipt jobs on default queue" do
              post :resend_receipt, params: { id: ObfuscateIds.encrypt(@gifter_purchase.id) }
              expect(response).to be_successful
  ...
  context_ "when pre-order purchase" do
  ...
  test "marks pre-order authorized" do
              expect(Purchase::ConfirmService).to receive(:new).with(hash_including(purchase:)).and_call_original
  ...
  test "creates recommended purchase info" do
            expect do
              post :confirm, params: { id: secure_confirm_id }
              purchase.reload
  ...
  context_ "PUT update_subscription" do
        before do
          setup_subscription
  ...
  test "updates the variant, price, and contact information associated with the subscription" do
          travel_to(@originally_subscribed_at + 1.month) do
  ...
  context_ "when params include custom field values" do
  test "does not change existing purchase custom fields" do
  ...
  context_ "when encrypted cookie is not present" do
          before do
  ...
  context_ "reCAPTCHA behavior" do
  test "does not attempt to verify the Google reCAPTCHA verification if using an existing card" do
  ...
  test "verifies the Google reCAPTCHA verification if using a new card" do
  ...
  test "does not attempt to verify the Google reCAPTCHA verification if using a new card but upgrade price is 0" do
            expect_any_instance_of(PurchasesController).not_to receive(:valid_recaptcha_response_and_hostname?)
  ...
  test "does not allow the purchase to proceed if reCAPTCHA verification fails" do
  ...
  context_ "subscribe" do
  test "404s on bad subscribe" do
          expect { get :subscribe, params: { id: "notreal" } }.to raise_error(ActionController::RoutingError)
        end

  test "only sets can_contact to true" do
  ...
  test "sets can_contact to true for all purchases" do
  ...
  test "404s on bad unsubscribe" do
          expect { get :unsubscribe, params: { id: "notreal" } }.to raise_error(ActionController::RoutingError)
        end

  test "404s when secure_external_id is invalid and confirmation_text is present" do
          expect do
            get :unsubscribe, params: { id: "invalid_token", confirmation_text: "bot@example.com" }
          end.to raise_error(ActionController::RoutingError)
        end

  context_ "with secure_external_id" do
  test "sets can_contact to false for all purchases from same seller and email" do
            purchase = create(:purchase, can_contact: true)
            purchase2 = create(:purchase, seller_id: purchase.seller.id, link: create(:product, user: purchase.seller), email: purchase.email, can_contact: true)
  ...
  context_ "with confirmation_text from secure_redirect_controller" do
  test "handles correct confirmation_text" do
  ...
  test "handles incorrect confirmation_text by checking charge ID" do
  ...
  test "redirects to root path when confirmation_text doesn't match any associated email" do
  ...
  test "redirects to root path when confirmation_text doesn't match and purchase has no charge" do
  ...
  context_ "with legacy external_id" do
  context_ "when purchase exists" do
  ...
  test "includes encrypted purchase email for confirmation" do
              purchase = create(:purchase, can_contact: true, email: "test@example.com")
              allow(SecureEncryptService).to receive(:encrypt).and_call_original
  ...
  context_ "when charge exists with multiple successful purchases" do
  test "collects all emails from charge's successful purchases" do
              seller = create(:user)
              product = create(:product, user: seller)
  ...
  test "handles case where charge external_id is used but purchase also exists" do
              seller = create(:user)
              product = create(:product, user: seller)
              charge = create(:charge, seller: seller)
  ...
  test "redirects to secure redirect when no purchase found but charge exists" do
              seller = create(:user)
              product = create(:product, user: seller)
              charge = create(:charge, seller: seller)
  ...
  context_ "when neither purchase nor charge exists" do
  ...
  context_ "GET confirm_receipt_email" do
        let(:purchase) { create(:purchase) }

  test "renders the confirm_receipt_email template" do
  ...
  context_ "GET receipt" do
        let(:purchase) { create(:purchase, email: "test@example.com") }

  context_ "when email is not verified" do
  test "redirects to confirm_receipt_email page" do
  ...
  test "shows receipt when correct email is provided" do
            get :receipt, params: { id: purchase.external_id, email: "test@example.com" }
            expect(response).to be_successful
            expect(response.body).to match("Generate invoice")
          end

  test "redirects back to confirm_receipt_email when incorrect email is provided" do
            get :receipt, params: { id: purchase.external_id, email: "wrong@example.com" }
            expect(response).to redirect_to(confirm_receipt_email_purchase_path(purchase.external_id))
            expect(flash[:alert]).to eq("Wrong email. Please try again.")
          end

  test "is case insensitive when comparing emails" do
            get :receipt, params: { id: purchase.external_id, email: "TEST@example.com" }
            expect(response).to be_successful
          end

  test "trims whitespace from email input" do
            get :receipt, params: { id: purchase.external_id, email: " test@example.com " }
            expect(response).to be_successful
          end
        end

  context_ "when user is logged in as purchaser" do
  ...
  test "shows the receipt for the purchase" do
            get :receipt, params: { id: purchase.external_id }
            expect(response).to be_successful
            expect(response.body).to match("Generate invoice")
          end

  test "404s for an invalid id" do
            expect do
  ...
  test "calls CustomerMailer with correct args" do
  ...
  context_ "when user is the purchaser" do
          let(:user) { create(:user) }
          let(:purchase) { create(:purchase, email: "test@example.com", purchaser: user) }
  ...
  test "renders the receipt without requiring email verification" do
            get :receipt, params: { id: purchase.external_id }
            expect(response).to be_successful
          end
        end
  ...
  context_ "View content button" do
          let(:purchase) { create(:purchase, email: "test@example.com") }
  ...
  test "renders View content button in English" do
