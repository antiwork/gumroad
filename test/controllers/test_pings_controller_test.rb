...
class TestPingsControllerTest < ActionController::TestCase
  self.described_class = TestPingsController
  tests TestPingsController



  context_ TestPingsController do
  ...
  test "posts a test ping containing latest sale details to the specified endpoint" do
  ...
  test "fails and displays error if no sale present for the user" do
  ...
  test "fails and displays error if invalid URL is passed" do
        expect(HTTParty).not_to receive(:post)
