...
class ProductReviewResponsesControllerTest < ActionController::TestCase
  self.described_class = ProductReviewResponsesController
  tests ProductReviewResponsesController



  context_ ProductReviewResponsesController do
  ...
  test "creates a new response if there is none" do
  ...
  test "updates an existing response if there is one, regardless of who the original author is" do
  ...
  test "allows responding to reviews that do not count towards review stats" do
  ...
  test "returns an error if the message is blank" do
  ...
  test "404s for non-existent purchase" do
        expect do
          put :update, params: { id: "non_existent_id", message: "Updated" }, as: :json
        end.to raise_error(ActionController::RoutingError, "Not Found")
      end

  test "404s when there is no product review" do
  ...
  test "401s when the product review is for another seller" do
        put :update, params: {
