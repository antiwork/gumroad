...
class LibraryControllerTest < ActionController::TestCase
  self.described_class = LibraryController
  self.rspec_metadata = { vcr: true }
  tests LibraryController



  context_ LibraryController, :vcr, type: :controller, inertia: true do
  ...
  test "doesn't show refunded purchases" do
          ...
          expect(product_names).not_to include("Refunded Product")
        end

  test "doesn't show charged back purchases" do
          ...
          product_names = inertia.props[:results].map { |r| r.dig(:product, :name) }
          expect(product_names).to include("Visible Product")
          expect(product_names).not_to include("Chargeback Product")
        end

  test "doesn't show gift sender purchases" do
          ...
          expect(response).to be_successful
          product_names = inertia.props[:results].map { |r| r.dig(:product, :name) }
          expect(product_names).not_to include(link.name)
        end
      end

  context_ "failed webhooks" do
        before do
  ...
  context_ "when an unconfirmed user attempts to access the library" do
        shared_examples "sends confirmation instructions" do
  test "disallows access and sends confirmation instructions" do
  ...
  context_ "when no confirmation instructions were sent to user" do
          before do
  ...
  context_ "when previous confirmation instructions were sent more than 24 hours ago" do
          before do
  ...
  context_ "when confirmation instructions were sent within the last 24 hours" do
          before do
            user.update_attribute(:confirmation_sent_at, 5.hours.ago)
          end

  test "doesn't send duplicate confirmation instructions" do
            allow(controller).to receive(:current_user).and_return(user)
  ...
  context_ "suspended (tos) user" do
        before do
  ...
  test "allows access" do
          get :index
          expect(response).to be_successful
        end
      end

  context_ "suspended (fraud) user" do
        before do
          @admin_user = create(:user)
  ...
  test "disallows access and redirects to the login page" do
          get :index
  ...
  test "unarchives the purchase" do
        expect do
