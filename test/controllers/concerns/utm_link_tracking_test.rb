...
class UtmLinkTrackingTest < ActionController::TestCase
  self.described_class = UtmLinkTracking



  context_ UtmLinkTracking, type: :controller do
  ...
  context_ "when a matching UTM link is found" do
  ...
  test "records UTM link visit", :sidekiq_inline do
        expect do
          expect do
  ...
  test "enqueues a job to update the UTM link stats" do
        get :action, params: { utm_source: utm_link.utm_source, utm_medium: utm_link.utm_medium, utm_campaign: utm_link.utm_campaign, utm_content: utm_link.utm_content, utm_term: utm_link.utm_term }
  ...
  context_ "when a matching UTM link is not found" do
  ...
  context_ "on the profile page" do
        before do
  ...
  context_ "on the subscribe page" do
        before do
  ...
  test "truncates UTM params that exceed the maximum length", :sidekiq_inline do
        long_source = "a" * 300
        request.host = "#{seller.subdomain}"
        request.path = "/"
        allow(controller).to receive(:root_path).and_return("/")
  ...
  test "does not auto-create UTM link when feature is disabled" do
  ...
  test "does not auto-create UTM link when cookies are disabled" do
        cookies[:_gumroad_guid] = nil
        request.host = "#{seller.subdomain}"
        request.path = "/"
