...
class CustomDomainRouteBuilderTest < ActionController::TestCase
  self.described_class = CustomDomainRouteBuilder



  context_ CustomDomainRouteBuilder, type: :controller do
  ...
  context_ "#build_view_post_route" do
      let!(:post) { create(:installment) }
      let!(:purchase) { create(:purchase) }

  test "returns the correct URL for requests from a custom domain" do
  ...
  test "returns the correct URL for requests from a non-custom domain" do
        get :action
  ...
  context_ "#seller_custom_domain_url" do
      let!(:user) { create(:user, username: "example") }

  context_ "when the request is through a custom domain" do
        before do
          @request.host = custom_domain
        end

  test "returns root path" do
          get :action
  ...
  context_ "when the request is through a product custom domain" do
        before do
  ...
  context_ "when the request is from a product custom domain page and there is an older dead matching domain" do
        before do
  ...
  context_ "when the request is not through a custom domain" do
  test "returns nil" do
          get :action
