...
class PurchasesDisputeEvidenceControllerTest < ActionController::TestCase
  self.described_class = Purchases::DisputeEvidenceController
  tests Purchases::DisputeEvidenceController

  # frozen_string_literal: false


  context_ Purchases::DisputeEvidenceController, type: :controller, inertia: true do
  ...
  context_ "when the seller has already submitted" do
        before do
  ...
  context_ "when the dispute has already been submitted" do
        before do
  ...
  test "redirects" do
      ...
      shared_examples "shows the dispute evidence page for the purchase" do
  test "shows the dispute evidence page for the purchase" do
          get :show, params: { purchase_id: purchase.external_id }
  ...
  context_ "when the dispute belongs to a charge" do
        let!(:charge) do
  ...
  context_ "GET success" do
  test "renders the success page" do
  ...
  test "updates the dispute evidence and redirects to success page" do
        put :update, params: {
  ...
  context_ "when a signed_id for a PNG file is provided" do
        let(:blob) do
          ActiveStorage::Blob.create_and_upload!(io: fixture_file_upload("smilie.png"), filename: "receipt_image.png", content_type: "image/png")
        end

  test "converts the file to JPG and attaches it to the dispute evidence" do
  ...
  context_ "when the dispute evidence is invalid" do
