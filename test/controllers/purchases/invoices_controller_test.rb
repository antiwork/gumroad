...
class PurchasesInvoicesControllerTest < ActionController::TestCase
  self.described_class = Purchases::InvoicesController
  self.rspec_metadata = { vcr: true }
  tests Purchases::InvoicesController



  context_ Purchases::InvoicesController, :vcr, type: :controller, inertia: true do
  ...
  test "renders inertia page and adds X-Robots-Tag response header to avoid page indexing" do
  ...
  test "assigns the charge as the chargeable for the presenter" do
            get :new, params: params
            expect(assigns(:_new_invoice_presenter).send(:chargeable)).to eq(charge)
          end

  context_ "when the second purchase is used as a param" do
  ...
  context_ "when the email does not match with purchase's email" do
  context_ "when the email is not present in params" do
  test "redirects to email confirmation path" do
  ...
  context_ "when the email is present in params" do
  test "redirects to email confirmation path" do
  ...
  context_ "for Purchase" do
  test "assigns the purchase as the chargeable" do
            post :create, params: payload
            expect(assigns(:chargeable)).to eq(purchase)
          end

  context_ "when user is issuing an invoice" do
  test "redirects to invoice page with success notice" do
              post :create, params: payload
  ...
  test "redirects with error alert if the process fails" do
  ...
  test "raises routing error if purchase doesn't exist" do
              expect do
                post :create, params: payload.merge!(purchase_id: "invalid")
              end.to raise_error(ActionController::RoutingError)
            end

  test "generates a PDF invoice with the purchase and payload details" do
              post :create, params: payload
  ...
  test "generates a PDF invoice with the submitted business name" do
  ...
  test "generates a PDF invoice with the purchase and payload details for non-US country" do
  ...
  test "generates a PDF invoice with the purchase and payload details for direct sales to AU customers" do
  ...
  context_ "when user provides additional notes" do
  test "includes additional notes in the invoice" do
  ...
  context_ "when user provides a vat id" do
            before do
  ...
  test "does not refund tax when provided an invalid vat id" do
  ...
  test "keeps the business id for broader supported countries without refunding tax" do
  ...
  test "refunds tax for a valid ABN id" do
              purchase_sales_tax_info = PurchaseSalesTaxInfo.new(country_code: Compliance::Countries::AUS.alpha2)
              @purchase.update!(purchase_sales_tax_info:)
  ...
  test "does not refund tax for an invalid ABN id" do
              purchase_sales_tax_info = PurchaseSalesTaxInfo.new(country_code: Compliance::Countries::AUS.alpha2)
              @purchase.update!(purchase_sales_tax_info:)
  ...
  test "refunds tax for a valid GST id" do
  ...
  test "does not refund tax for an invalid GST id" do
              purchase_sales_tax_info = PurchaseSalesTaxInfo.new(country_code: Compliance::Countries::SGP.alpha2)
              @purchase.update!(purchase_sales_tax_info:)
  ...
  test "refunds tax for a valid QST id" do
  ...
  test "does not refund tax for an invalid QST id" do
              purchase_sales_tax_info = PurchaseSalesTaxInfo.new(country_code: Compliance::Countries::CAN.alpha2, state_code: QUEBEC)
              @purchase.update!(purchase_sales_tax_info:)
  ...
  test "does not refund tax but still send receipt if already refunded" do
  ...
  context_ "when the email param is not set" do
  test "redirects to the email confirmation path" do
  ...
  context_ "when address_fields is missing" do
  test "redirects with an error alert" do
  ...
  test "assigns the charge as the chargeable" do
            post :create, params: payload
  ...
  test "generates a PDF invoice with the charge and payload details" do
              post :create, params: payload
  ...
  context_ "with a valid ABN id" do
              before do
                purchase_sales_tax_info = PurchaseSalesTaxInfo.new(country_code: Compliance::Countries::AUS.alpha2)
                purchase.update!(purchase_sales_tax_info:)
  ...
  context_ "with an invalid ABN id" do
  test "does not refund tax" do
                  expect do
  ...
  context_ "with a valid GST id" do
              before do
                purchase_sales_tax_info = PurchaseSalesTaxInfo.new(country_code: Compliance::Countries::SGP.alpha2)
  ...
  context_ "with an invalid GST id" do
  test "does not refund tax" do
                  expect do
  ...
  context_ "when already refunded" do
              before do
                purchase.refund_gumroad_taxes!(refunding_user_id: nil, note: "note")
  ...
  test "returns error if purchase is not successful" do
                post :create, params: payload.merge(vat_id: "IE6388047V", purchase_id: purchase.external_id)
  ...
  context_ "POST confirm_email" do
        let(:purchase) { create(:purchase) }

  test "redirects to invoice page with correct email" do
          post :confirm_email, params: { purchase_id: purchase.external_id, email: purchase.email }

          expect(response).to redirect_to(new_purchase_invoice_path(purchase.external_id, email: purchase.email))
        end

  test "redirects back with error for incorrect email" do
  ...
  test "redirects back with warning when email is missing" do
