...
class SettingsBeneficialOwnersControllerTest < ActionController::TestCase
  self.described_class = Settings::BeneficialOwnersController
  tests Settings::BeneficialOwnersController



  context_ Settings::BeneficialOwnersController, type: :controller do
  ...
  test "returns every beneficial owner including the representative" do
  ...
  test "returns 403 when the seller is not on a business account" do
  ...
  test "creates the Stripe person and returns 201 with the serialized record" do
  ...
  test "returns 422 with the Stripe message when ownership exceeds 100%" do
  ...
  test "returns 422 with a clear message when email or phone is missing" do
  ...
  test "updates the Stripe person and returns 200" do
  ...
  test "allows editing the representative's ownership and role flags" do
  ...
  test "deletes the Stripe person and returns 200" do
        allow(Stripe::Account).to receive(:retrieve_person).with(stripe_account_id, person_id).and_return(other_owner)
  ...
  test "returns 403 when attempting to delete the representative" do
        allow(Stripe::Account).to receive(:retrieve_person).with(stripe_account_id, representative_id).and_return(representative)
