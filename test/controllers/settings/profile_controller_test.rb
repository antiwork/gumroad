...
class SettingsProfileControllerTest < ActionController::TestCase
  self.described_class = Settings::ProfileController
  self.rspec_metadata = { vcr: true }
  tests Settings::ProfileController



  context_ Settings::ProfileController, :vcr, type: :controller, inertia: true do
  ...
  test "converts a blank username to nil" do
  ...
  test "performs model validations" do
  ...
  test "saves tabs and cleans up orphan sections" do
  ...
  test "returns an error if the corresponding blob for the provided 'profile_picture_blob_id' is already removed" do
        seller.avatar.attach(file_fixture("test.png"))
  ...
  test "handles duplicate attachment gracefully when avatar is already attached with the same blob" do
  ...
  test "handles concurrent avatar attachment race condition" do
        blob = ActiveStorage::Blob.create_and_upload!(
          io: fixture_file_upload("smilie.png"),
          filename: "smilie.png",
        )
  ...
  test "regenerates the subscribe preview when the avatar changes" do
