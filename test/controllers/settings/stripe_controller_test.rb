...
class SettingsStripeControllerTest < ActionController::TestCase
  self.described_class = Settings::StripeController
  self.rspec_metadata = { vcr: true }
  tests Settings::StripeController



  context_ Settings::StripeController, :vcr do
  context_ "POST disconnect" do
      before do
        @creator = create(:user)
  ...
  context_ "when stripe disconnect is allowed" do
  test "marks the connected Stripe merchant account as deleted" do
  ...
  test "reactivates creator's old gumroad-controlled Stripe account associated with their unpaid balance" do
  ...
  test "reactivates creator's old gumroad-controlled Stripe account that's associated with the active bank account" do
          stripe_account = create(:merchant_account_stripe_canada, user: @creator)
          stripe_account.delete_charge_processor_account!
