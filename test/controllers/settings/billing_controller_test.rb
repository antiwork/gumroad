...
class SettingsBillingControllerTest < ActionController::TestCase
  self.described_class = Settings::BillingController
  tests Settings::BillingController



  context_ Settings::BillingController, type: :controller, inertia: true do
  ...
  test "returns http success and renders the Inertia component" do
        get :show
  ...
  test "pre-fills with existing billing details when present" do
        create(
          :billing_detail,
          purchaser: seller,
  ...
  test "creates a BillingDetail when none exists and redirects with notice" do
  ...
  test "updates the existing BillingDetail instead of creating another" do
  ...
  test "redirects without persisting when validation fails" do
        expect do
