...
class SettingsTotpControllerTest < ActionController::TestCase
  self.described_class = Settings::TotpController
  tests Settings::TotpController



  context_ Settings::TotpController, type: :controller do
  ...
  test "creates a totp credential and returns setup data" do
        post :create
  ...
  context_ "when user already has a confirmed totp credential" do
        before do
  ...
  context_ "when user has an unconfirmed totp credential from a previous attempt" do
        before do
          create(:totp_credential, user:)
        end

  test "destroys the old credential and creates a new one" do
  ...
  test "confirms the credential with a valid code and returns formatted recovery codes" do
  ...
  test "returns an error with an invalid code" do
  ...
  context_ "when user has an already confirmed totp credential" do
        before do
          create(:totp_credential, :confirmed, user:)
        end
  ...
  test "destroys the totp credential" do
          delete :destroy
  ...
  context_ "POST regenerate_recovery_codes" do
  context_ "when user has a confirmed totp credential" do
        let!(:credential) { create(:totp_credential, :with_recovery_codes, user:) }

  test "regenerates formatted recovery codes" do
