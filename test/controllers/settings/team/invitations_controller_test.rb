...
class SettingsTeamInvitationsControllerTest < ActionController::TestCase
  self.described_class = Settings::Team::InvitationsController
  tests Settings::Team::InvitationsController



  context_ Settings::Team::InvitationsController do
  ...
  context_ "when payload is valid" do
  test "creates team invitation record" do
  ...
  context_ "when payload is not valid" do
  ...
  context_ "when the invitation email belongs to an existing team member" do
        before do
          member = create(:user, email: team_invitation.email)
          create(:team_membership, seller:, user: member)
        end

  test "returns an error instead of raising an exception" do
          put :update, params: { id: team_invitation.external_id, team_invitation: { role: TeamMembership::ROLE_ADMIN } }, as: :json
          expect(response).to be_successful
          expect(response.parsed_body["success"]).to eq(false)
  ...
  test "updates record as deleted" do
  ...
  context_ "with record belonging to other seller" do
  ...
  context_ "when the email is associated with an existing team member" do
        before do
          member = create(:user, email: team_invitation.email)
  ...
  test "successfully accepts the invitation" do
  ...
  context_ "when logged-in user email is missing" do
  ...
  test "renders email missing alert" do
          expect do
            get :accept, params: { id: team_invitation.external_id }
  ...
  context_ "when logged-in user email is not confirmed" do
        before { user.update!(confirmed_at: nil) }

  test "renders unconfirmed email alert" do
          expect do
            get :accept, params: { id: team_invitation.external_id }
          end.not_to change { seller.seller_memberships.count }
  ...
  context_ "when logged-in user email is different" do
        before { team_invitation.update!(email: "wrong.email@example.com") }

  test "renders email mismatch alert" do
          expect do
            get :accept, params: { id: team_invitation.external_id }
          end.not_to change { seller.seller_memberships.count }
  ...
  context_ "when invitation has expired" do
        before { team_invitation.update!(expires_at: 1.second.ago) }

  test "renders expired invitation alert" do
          expect do
            get :accept, params: { id: team_invitation.external_id }
          end.not_to change { seller.seller_memberships.count }
  ...
  context_ "when the invitation has already been accepted" do
        before { team_invitation.update_as_accepted! }

  test "renders invitation already accepted alert" do
          expect do
            get :accept, params: { id: team_invitation.external_id }
          end.not_to change { seller.seller_memberships.count }
  ...
  context_ "when the invitation has been deleted" do
  ...
  context_ "when the invitation email matches the owner's email" do
        before do
  ...
  test "deletes the invitation and renders invitation invalid alert" do
          expect do
            get :accept, params: { id: team_invitation.external_id }
          end.not_to change { seller.seller_memberships.count }
  ...
  context_ "PUT resend_invitation" do
  ...
  test "updates team invitation record and enqueues email" do
        allow(TeamMailer).to receive(:invite).and_call_original
