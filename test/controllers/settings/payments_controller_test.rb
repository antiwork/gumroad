...
class SettingsPaymentsControllerTest < ActionController::TestCase
  self.described_class = Settings::PaymentsController
  self.rspec_metadata = { vcr: true }
  tests Settings::PaymentsController



  context_ Settings::PaymentsController, :vcr, type: :controller, inertia: true do
  ...
  context_ "account_status prop" do
  test "does not show section for compliant user with no issues" do
  ...
  test "shows section for user on probation" do
  ...
  test "shows section for suspended user with reason" do
  ...
  test "shows section for user with fraud suspension" do
  ...
  test "shows section when payouts are paused internally" do
  ...
  test "shows section with compliance actions when there are pending requests" do
  ...
  context_ "tos" do
  context_ "with terms notice displayed" do
  context_ "with time" do
            let(:time_freeze) { Time.zone.local(2015, 4, 1) }

  test "updates the tos last agreed at" do
  ...
  test "updates the tos last agreed ip" do
              put :update, params: { user: params, terms_accepted: true }
              user.reload
  ...
  test "updates the payout threshold for valid amounts" do
          expect do
  ...
  test "returns an error for invalid amounts" do
  ...
  test "does not resubmit unchanged compliance info when only the payout threshold changed" do
  ...
  test "does not resubmit identical compliance info" do
          create_current_compliance_info_matching_form_params
          enqueue_identity_verification_email_if_compliance_info_is_submitted
          initial_compliance_info_id = user.alive_user_compliance_info.id
          initial_compliance_info_count = UserComplianceInfo.count
  ...
  context_ "payout frequency" do
  test "updates the payout frequency for valid values" do
          expect do
  ...
  test "updates the compliance information and return the proper response" do
  ...
  test "does not overwrite information for steps that the ui did not provide" do
          put :update, params: all_params
  ...
  test "does not overwrite information for steps that the ui did provide as blank" do
          put :update, params: all_params
  ...
  test "clears only the requests that are present" do
          put :update, params: all_params
          put :update, params: { user: { first_name: "newfirst", last_name: "newlast" } }
          request_1.reload
          request_2.reload
  ...
  context_ "immediate stripe account creation" do
          let(:all_params) { { user: params } }

  context_ "user has a bank account, and a merchant account already" do
            before do
              all_params.merge!(
                bank_account: {
                  type: AchAccount.name,
  ...
  test "does not try to create a new stripe account because user already has one" do
  ...
  context_ "user does not have a bank account, or a merchant account" do
  test "does not try to create a new stripe account because user does not have a bank account" do
              expect(StripeMerchantAccountManager).not_to receive(:create_account)
  ...
  context_ "user has a bank account but not a merchant account" do
  test "creates a new stripe merchant account for the user" do
              all_params.merge!(
                bank_account: {
                  type: AchAccount.name,
                  account_number: "000123456789",
  ...
  test "raises error if stripe account creation fails" do
              all_params.merge!(
                bank_account: {
                  type: AchAccount.name,
  ...
  test "handles Stripe::APIError gracefully instead of raising a 500" do
              all_params.merge!(
                bank_account: {
                  type: AchAccount.name,
                  account_number: "000123456789",
  ...
  test "handles MerchantRegistrationUserNotReadyError gracefully instead of raising a 500" do
              all_params.merge!(
                bank_account: {
                  type: AchAccount.name,
                  account_number: "000123456789",
  ...
  context_ "user enters a birthday accidentally that is under 13 years old given todays date" do
          before do
  ...
  test "leaves the previous user compliance info data unchanged" do
  ...
  context_ "creator enters an invalid zip code" do
          before do
            params.merge!(
  ...
  test "returns an error response" do
            put :update, params: { user: params }
            expect(response).to redirect_to(settings_payments_path)
            expect(response).to have_http_status :found
  ...
  test "the users current compliance info should contain the same data" do
              compliance_info = user.fetch_or_build_user_compliance_info
              expect(compliance_info.first_name).to eq "barnabas"
              expect(compliance_info.last_name).to eq "barnabastein"
              expect(compliance_info.street_address).to eq "123 barnabas st"
  ...
  context_ "user wishes to edit a frozen field (e.g. first name)" do
            before do
  ...
  test "returns an error with the actual Stripe error message" do
              expect(response).to redirect_to(settings_payments_path)
              expect(response).to have_http_status :found
  ...
  test "the users current compliance info should be changed" do
              compliance_info = user.fetch_or_build_user_compliance_info
  ...
  context_ "user wishes to edit a frozen field (e.g. dob that may be edited if nil)" do
            let(:params) do
              {
                first_name: "barnabas",
                last_name: "barnabastein",
  ...
  context_ "user wishes to edit a non-frozen feild (e.g. address)" do
            before do
  ...
  test "the users current compliance info should contain the new address" do
              compliance_info = user.fetch_or_build_user_compliance_info
              expect(compliance_info.first_name).to eq "barnabas"
              expect(compliance_info.last_name).to eq "barnabastein"
  ...
  test "allows the user to change the account type from individual to business" do
  ...
  context_ "user is verified, and their compliance info was old and is_business=nil when we created their merchant account" do
          before do
            params.merge!(
              is_business: nil
            )
  ...
  context_ "user submits their compliance info, and the new form submits is_business=off" do
            before do
              params.merge!(
                is_business: "off"
              )
  ...
  test "the users current compliance info should contain the same details" do
              compliance_info = user.fetch_or_build_user_compliance_info
              expect(compliance_info.first_name).to eq "barnabas"
              expect(compliance_info.last_name).to eq "barnabastein"
              expect(compliance_info.street_address).to eq "123 barnabas st"
  ...
  test "the users current compliance info should contain is_business=false" do
              compliance_info = user.fetch_or_build_user_compliance_info
              expect(compliance_info.is_business).to be(false)
            end
          end
  ...
  test "clears the requests that are present" do
          put :update, params: { user: business_params }
          request_1.reload
          request_2.reload
          request_3.reload
  ...
  test "allows the user to change the account type from business to individual after verification" do
  ...
  context_ "P.O. Box validation" do
        before do
          compliance_info = user.alive_user_compliance_info
  ...
  test "rejects an individual Ghana address that uses a P.O. Box" do
          expect do
            put :update, params: { user: params.merge(street_address: "P.O. Box 123, High street") }
          end.not_to change { user.reload.alive_user_compliance_info.street_address }
  ...
  test "allows unrelated full-form updates when a legacy Ghana P.O. Box address is unchanged" do
  ...
  test "allows unrelated full-form updates when a hidden legacy Ghana business P.O. Box is unchanged for an individual" do
          user.alive_user_compliance_info.dup_and_save! do |new_compliance_info|
  ...
  test "rejects a business Ghana address that uses a P.O. Box" do
          expect do
            put :update, params: {
              user: params.merge(
                is_business: "on",
          ...
          end.not_to change { user.reload.alive_user_compliance_info.business_street_address }
  ...
  test "rejects a submitted beneficiary Ghana P.O. Box even when the business address is valid" do
          expect do
            put :update, params: {
              user: params.merge(
                is_business: "on",
  ...
  test "rejects a Ghana beneficiary P.O. Box when the submitted country would not be persisted" do
          expect do
  ...
  test "rejects a Ghana beneficiary P.O. Box when the submitted country cannot be mapped" do
          expect do
  ...
  test "rejects a Ghana business beneficiary P.O. Box when is_business is omitted" do
          user.alive_user_compliance_info.dup_and_save! do |new_compliance_info|
  ...
  test "rejects toggling business mode on when a legacy Ghana beneficiary P.O. Box remains on file" do
          user.alive_user_compliance_info.dup_and_save! do |new_compliance_info|
          ...
          end.not_to change { user.reload.alive_user_compliance_info.is_business }
  ...
  test "rejects planting a Ghana business P.O. Box while business mode is off" do
          expect do
  ...
  test "rejects toggling business mode on when a dormant Ghana business P.O. Box remains on file" do
          user.alive_user_compliance_info.dup_and_save! do |new_compliance_info|
  ...
  test "creates the ach account" do
            put(:update, params:)
  ...
  test "clears the request for the bank account" do
            put(:update, params:)
  ...
  context_ "with invalid bank code" do
            before do
  ...
  context_ "success with dashes/hyphens and leading/trailing spaces" do
          let(:params) do
            {
              bank_account: {
                type: AchAccount.name,
  ...
  context_ "concurrent payout method change" do
  ...
  context_ "account number and repeated account number don't match" do
          let(:params) do
            {
              bank_account: {
                type: AchAccount.name,
  ...
  test "fails if the account numbers don't match" do
            put(:update, params:)
            expect(response).to redirect_to(settings_payments_path)
            expect(response).to have_http_status :found
  ...
  test "does not clear the request for the bank account" do
            put(:update, params:)
            request.reload
  ...
  context_ "account_number_confirmation is nil" do
          let(:params) do
            {
              bank_account: {
                type: AchAccount.name,
  ...
  test "returns a validation error instead of raising NoMethodError" do
            put(:update, params:)
            expect(response).to redirect_to(settings_payments_path)
            expect(response).to have_http_status :found
            expect(session[:inertia_errors][:base]).to include("The account numbers do not match.")
  ...
  context_ "canadian bank account" do
          let(:user) { create(:user) }
  ...
  context_ "australian bank account" do
          let(:user) { create(:user) }
  ...
  context_ "gibraltar bank account" do
          let(:user) { create(:user) }
  ...
  test "creates the gibraltar bank account" do
              put(:update, params:)
  ...
  context_ "when setting the PayPal payout address" do
        before do
          user.update!(user_risk_state: "compliant", payment_address: "sam@example.com")
        end

  test "fails if payout address contains non-ASCII characters" do
  ...
  test "fails if bank payouts are supported in seller's country" do
  ...
  test "succeeds if bank payouts are not supported in seller's country" do
  ...
  test "resumes payouts if account is not flagged or suspended" do
          user.update!(payment_address: "")
  ...
  test "does not resume payouts if account is flagged or suspended" do
  ...
  context_ "when setting a debit card as the payout method", :vcr do
        before do
  ...
  test "succeeds when the previous payout method was a bank account" do
  ...
  context_ "when updating country" do
  test "calls UpdateUserCountry service" do
  ...
  test "notifies error tracker if there is an error" do
  ...
  test "updates the country and returns the proper response" do
  ...
  context_ "user compliance info" do
  test "creates a new user compliance info" do
          expect do
            post :set_country, params:, as: :json
  ...
  test "creates compliance info without a country" do
          expect do
  ...
  context_ "user selects specific country" do
  context_ "US" do
  test "sets the default currency to USD" do
            post :set_country, params:, as: :json
  ...
  context_ "CA" do
  test "sets the default currency to CAD" do
  ...
  context_ "POST opt_in_to_au_backtax_collection" do
  ...
  test "creates the backtax agreement and returns the proper response" do
  ...
  test "returns an error if the signature is not the same length as the name in the creator's settings" do
  ...
  context_ "GET paypal_connect" do
  ...
  context_ "when the user has merchant migration enabled" do
        before do
          seller.check_merchant_account_is_linked = true
          seller.save
        end

  context_ "when PayPal account connection is successful" do
  test "creates a new MerchantAccount record and redirects the user on success" do
  ...
  context_ "when PayPal account connection is not successful" do
  test "redirects user to payments settings path" do
            get :paypal_connect, params: paypal_params(paypal_merchant_reference: nil)
            expect(response).to redirect_to(settings_payments_path)
          end

  test "allows same PayPal account to be connected even when it is already connected to the another Gumroad Account" do
  ...
  context_ "when there is some error connecting PayPal account" do
  test "flashes PayPal account connection error" do
              get :paypal_connect, params: paypal_params(paypal_merchant_reference: nil)
              expect(response).to redirect_to(settings_payments_path)
  ...
  context_ "when the user has merchant migration enabled by way of the feature flag" do
        before do
  ...
  context_ "when the PayPal account connection is successful" do
  test "does not set the `check_merchant_account_is_linked` property to `true` for the user on success" do
            expect do
              get :paypal_connect, params: paypal_params(paypal_merchant_reference: "A8RLJ7R5E389A")
            end.to change { MerchantAccount.count }.by(1)
  ...
  context_ "when the user's country is not supported by paypal commerce platform" do
          before do
            seller.alive_user_compliance_info.update_columns(country: "India")
          end

  test "still connects the paypal account if paypal account is from a supported country" do
            get :paypal_connect, params: paypal_params(paypal_merchant_reference: "A8RLJ7R5E389A")
            expect(response).to redirect_to(settings_payments_path)
  ...
  context_ "when PayPal returns C2 instead of CN as country code for Chinese accounts" do
          before do
            seller.alive_user_compliance_info.update_columns(country: "China")
          end

  test "still connects the paypal account" do
  ...
  context_ "when the user's paypal account country is not supported by paypal commerce platform" do
  test "redirects to payments settings page with proper error message" do
  ...
  context_ "POST remove_credit_card" do
  test "returns failure if credit card is required by the user, else removes the credit card and returns success" do
  ...
  test "does noting and redirects to payments settings page if there's no associated stripe account" do
        get :remediation

        expect(response).to redirect_to settings_payments_url
      end

  test "does noting and redirects to payments settings page if there's no pending stripe information request" do
  ...
  test "generates a remediation link for the associated Stripe account and redirects to it" do
  ...
  context_ "when Stripe::AccountLink.create raises Stripe::InvalidRequestError" do
  ...
  test "redirects back to settings with an alert instead of raising" do
  ...
  test "records the disabled_reason returned by Stripe so the rejection alert renders on the next page load" do
          stub_stripe_account_retrieve(disabled_reason: "rejected.listed")
  ...
  test "records the disabled_reason even when the Stripe error message does not contain 'has been rejected'" do
          allow(Stripe::AccountLink).to receive(:create).and_raise(
  ...
  test "does not overwrite an existing stripe_disabled_reason" do
  ...
  test "does not write a disabled_reason when Stripe returns none" do
  ...
  test "still redirects when Stripe::Account.retrieve itself raises" do
  ...
  context_ "GET verify_stripe_remediation" do
      let!(:user) { create(:user) }
      let!(:user_compliance_info) { create(:user_compliance_info, user:) }
      let!(:bank_account) { create(:ach_account_stripe_succeed, user:) }
      let!(:tos_agreement) { create(:tos_agreement, user:) }
  ...
  test "redirects to the payments settings page" do
