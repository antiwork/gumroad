...
class S3UtilityControllerTest < ActionController::TestCase
  self.described_class = S3UtilityController
  tests S3UtilityController



  context_ S3UtilityController do
  ...
  context_ "GET generate_multipart_signature" do
  test "doesn't allow sellers to sign request for buckets they do not own" do
  ...
  test "doesn't allow if an attacker splits the request with newlines" do
  ...
  test "allows sellers to sign request for buckets they own" do
        sign_string = "POST\n\nvideo/quicktime; charset=UTF-8\n\nx-amz-acl:private\nx-amz-date:Mon, 02 Mar 2015 17:21:19 \
  ...
  context_ "GET cdn_url_for_blob" do
  test "returns blob cdn url with valid key" do
  ...
  test "404s with an invalid key" do
        expect do
          get :cdn_url_for_blob, params: { key: "xxx" }
        end.to raise_error(ActionController::RoutingError)
      end

  test "returns the blob cdn url in JSON format" do
        blob = ActiveStorage::Blob.create_and_upload!(io: fixture_file_upload("smilie.png"), filename: "smilie.png")
