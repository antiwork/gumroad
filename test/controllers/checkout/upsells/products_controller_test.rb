...
class CheckoutUpsellsProductsControllerTest < ActionController::TestCase
  self.described_class = Checkout::Upsells::ProductsController
  tests Checkout::Upsells::ProductsController



  context_ Checkout::Upsells::ProductsController do
  ...
  test "returns the seller's visible products" do
        sign_in seller
        get :index
  ...
  test "limits results to MAX_PRODUCTS" do
        sign_in seller
  ...
  context_ "when no seller is found" do
  ...
  test "returns an empty array when the query times out" do
        sign_in seller
  ...
  test "returns the requested visible product" do
        sign_in seller
  ...
  test "returns thumbnail_url when product has a thumbnail" do
  ...
  test "raises ActiveRecord::RecordNotFound for an archived product" do
        sign_in seller
        expect do
  ...
  test "raises ActiveRecord::RecordNotFound for a non-existent product" do
        sign_in seller
        expect do
  ...
  test "returns a timeout error when the query times out" do
        sign_in seller
        allow(WithMaxExecutionTime).to receive(:timeout_queries).with(seconds: 10)
          .and_raise(WithMaxExecutionTime::QueryTimeoutError.new("maximum statement execution time exceeded"))
