...
class UrlRedirectsControllerTest < ActionController::TestCase
  self.described_class = UrlRedirectsController
  tests UrlRedirectsController



  context_ UrlRedirectsController, inertia: true do
    render_views
  ...
  context_ "GET 'download_page'", inertia: true do
      before do
  ...
  context_ "with access revoked for purchase" do
        before do
          @url_redirect.purchase.update!(is_access_revoked: true)
        end

  test "redirects to expired page" do
          expect do
            get :download_page, params: { id: @token }
  ...
  test "creates consumption event" do
        expect do
          get :download_page, params: { id: @token }
  ...
  context_ "when mobile view param is passed" do
  test "renders correctly" do
  ...
  test "displays the license key for the purchase" do
          get :download_page, params: { id: @token }
  ...
  test "returns updates from that purchase" do
            get :download_page, params: { id: token }
  ...
  test "returns updates from those other purchases if they've bought the same product multiple times" do
  ...
  test "does not break if the user has been sent a post for a product they have not purchased" do
  ...
  context_ "for subscriptions", :vcr do
  ...
  context_ "when it is deactivated" do
  ...
  context_ "and access is blocked on cancellation" do
  ...
  context_ "when access not blocked on cancellation" do
  ...
  context_ "with a bundle purchase" do
  ...
  test "redirects to the library page without creating consumption event" do
            expect do
  ...
  context_ "when the url_redirect doesn't belong to a product" do
            before do
  ...
  test "redirects to the library page" do
              expect do
                get :download_page, params: { id: purchase.url_redirect.token }
              end.not_to change(ConsumptionEvent, :count)
              expect(response).to redirect_to(library_url({ bundles: purchase.link.external_id }))
  ...
  context_ "when the receipt parameter is present" do
  test "includes the purchase_id parameter when redirecting" do
  ...
  context_ "with purchase purchaser is nil" do
  test "renders add to library" do
            get :download_page, params: { id: @token }
            expect(inertia.props[:add_to_library_option]).to eq "add_to_library_button"
          end
        end

  context_ "with purchase purchaser set to signed in user" do
          before do
            @url_redirect.purchase.purchaser = @user
            @url_redirect.purchase.save!
          end

  test "does not render add to library" do
            get :download_page, params: { id: @token }
            expect(inertia.props[:add_to_library_option]).to eq "none"
          end
        end

  test "redirects to check purchaser if signed in user is not purchaser" do
  ...
  context_ "when user does not exist with purchase email" do
  test "renders signup form" do
          get :download_page, params: { id: @token }
          expect(inertia.props[:add_to_library_option]).to eq "signup_form"
        end
      end

  test "increments the view count on the url_redirect" do
  ...
  test "renders the download page properly for a product installment with files" do
          get :download_page, params: { id: @token }
  ...
  test "renders the download page properly for a variant installment with files" do
  ...
  test "renders the download page properly for a follower installment with files" do
  ...
  test "renders the download page properly for an affiliate installment with files" do
  ...
  test "renders the download page properly for an audience installment with files" do
  ...
  test "renders the download page properly for a membership installment that used to have files and now does not" do
  ...
  test "returns a 404 if the url redirect is not found" do
          expect do
            get :download_page, params: { id: "some non-existent id" }
          end.to raise_error(ActionController::RoutingError)
        end

  test "returns 404 if the url redirect is for a membership installment that is now deleted" do
          @product.update!(is_recurring_billing: true)
          @url_redirect.update!(link: @product)
  ...
  test "returns 404 if the url redirect is for a creator (not product) installment that is now deleted" do
          @seller_installment.mark_deleted!
          expect do
            get :download_page, params: { id: @token }
          end.to raise_error(ActionController::RoutingError)
  ...
  test "404s if the url redirect is for a creator (not product) installment that used to have files and now does not" do
          @seller_installment.product_files.last.mark_deleted
          expect do
            get :download_page, params: { id: @token }
          end.to raise_error(ActionController::RoutingError)
  ...
  test "renders the download page properly for a product installment without files that was purchased" do
  ...
  test "renders the download page properly for a variant installment without files that was purchased" do
          category = create(:variant_category, link: @product, title: "Color")
          variant = create(:variant, variant_category: category, name: "Blue")
          @url_redirect.update(link: @product, purchase: create(:purchase, link: @product))
          @seller_installment.product_files.last.mark_deleted
  ...
  test "returns 404 error if the url redirect is for a product installment without files and was not purchased" do
          @url_redirect.update(link: @product)
          @seller_installment.product_files.last.mark_deleted
  ...
  test "returns 404 error if the url redirect is for a variant installment without files and was not purchased" do
          category = create(:variant_category, link: @product, title: "Color")
          variant = create(:variant, variant_category: category, name: "Blue")
          @url_redirect.update(link: @product)
          @seller_installment.product_files.last.mark_deleted
  ...
  test "returns 404 error if the url redirect is for a follower installment without files" do
          @seller_installment.product_files.last.mark_deleted
          @seller_installment.update!(installment_type: Installment::FOLLOWER_TYPE)
  ...
  test "returns 404 error if the url redirect is for an affiliate installment without files" do
          @seller_installment.product_files.last.mark_deleted
          @seller_installment.update!(installment_type: Installment::AFFILIATE_TYPE)
  ...
  test "returns 404 error if the url redirect is for an audience installment without files" do
          @seller_installment.product_files.last.mark_deleted
          @seller_installment.update!(installment_type: Installment::AUDIENCE_TYPE)
  ...
  test "returns 404 error if the url redirect is for a physical product installment that used to have files and now does not" do
  ...
  context_ "when 'product_file_id' is missing from the params", inertia: true do
          before do
            @product.product_files << create(:product_file, link: @product)
            stub_const("UrlRedirect::GUID_GETTER_FROM_S3_URL_REGEX", /(specs)/)
          end

  test "renders the stream page with the first streamable product file" do
            get :stream, params: { id: @url_redirect.token }
  ...
  context_ "when there are no streamable files" do
          before do
  ...
  context_ "when 'product_file_id' points to a non-streamable file" do
  test "raises 404 error" do
  ...
  context_ "when url redirect has a purchase", inertia: true do
          before do
            @url_redirect.update!(purchase: create(:purchase, link: @product))
            stub_const("UrlRedirect::GUID_GETTER_FROM_S3_URL_REGEX", /(specs)/)
          end

  test "includes purchase_id in stream props" do
            get :stream, params: { id: @url_redirect.token }
  ...
  context_ "with custom domain route", type: :request do
          let!(:custom_domain) { create(:custom_domain, user: @product.user) }

  test "renders stream page via custom domain" do
            stub_const("UrlRedirect::GUID_GETTER_FROM_S3_URL_REGEX", /(specs)/)
  ...
  context_ "with custom subdomain", inertia: true do
          before do
            stub_const("UrlRedirect::GUID_GETTER_FROM_S3_URL_REGEX", /(specs)/)
            @request.host = URI.parse(@product.user.subdomain_with_protocol).host
          end

  test "renders stream page via custom subdomain" do
            get :stream, params: { id: @url_redirect.token }
  ...
  test "renders stream page with product_file_id via custom subdomain" do
            get :stream, params: { id: @url_redirect.token, product_file_id: @product.product_files.first.external_id }
  ...
  test "redirects to the expiration page if the membership inactive" do
  ...
  test "does not redirect to login page if no user signed in" do
          ...
          get :download_page, params: { id: url_redirect.token }
          expect(response).not_to redirect_to(login_url(next: request.path))
        end
      end

  context_ "needs to be confirmed" do
        before do
  ...
  test "redirects to confirm page with correct parameters" do
  ...
  context_ "coffee product" do
        let(:url_redirect) { create(:url_redirect, purchase: create(:purchase, link: create(:coffee_product))) }

  test "redirects to the coffee page and forwards the purchase_email parameter" do
  ...
  context_ "mobile app webview" do
  ...
  test "grants access when the token owner is the impersonated user and mobile_token is present" do
              get :download_page, params: { id: @token, access_token: access_token.token, mobile_token: Api::Mobile::BaseController::MOBILE_TOKEN }
  ...
  test "requires mobile_token to match the expected value" do
  ...
  context_ "with valid token but user is not the purchaser" do
  ...
  test "redirects to confirm page" do
            @url_redirect.update!(has_been_seen: true)
  ...
  context_ "with invalid token" do
  test "returns 401" do
  ...
  context_ "with token that has wrong scope" do
  ...
  context_ "with valid token but no purchase is attached to the url redirect" do
          let(:access_token) { create("doorkeeper/access_token", application: oauth_app, resource_owner_id: purchaser.id, scopes: "mobile_api") }

  test "does not require redirect" do
  ...
  context_ "GET download_archive" do
  test "redirects to the download URL for the requested installment archive when the format is HTML" do
  ...
  test "returns the download URL for the requested folder archive" do
  ...
  test "returns nil if the folder archive is not ready" do
        folder_id = SecureRandom.uuid
  ...
  test "redirects to the download URL for the requested entity archive when the format is HTML" do
  ...
  test "redirects to the download URL for the requested folder archive when the format is HTML" do
        folder_id = SecureRandom.uuid
        folder_archive = @url_redirect.product_files_archives.new(folder_id:, product_files_archive_state: "ready")
        folder_archive.set_url_if_not_present
        folder_archive.save!
  ...
  test "triggers archive regeneration and redirects to download page when the S3 object is missing" do
        entity_archive = @url_redirect.product_files_archives.new(product_files_archive_state: "ready")
        entity_archive.set_url_if_not_present
        entity_archive.save!
  ...
  context_ "when accessed via custom domain and S3 object is missing", type: :request do
  test "redirects to download page without raising UnsafeRedirectError" do
          entity_archive = @url_redirect.product_files_archives.new(product_files_archive_state: "ready")
          entity_archive.set_url_if_not_present
          entity_archive.save!
  ...
  test "returns a 404 if no files exist" do
  ...
  context_ "when the S3 object is missing" do
  test "returns a 404 JSON error for JSON requests" do
          file = create(:product_file, link: @product)
  ...
  test "redirects to the download page with a warning for HTML requests" do
          file = create(:product_file, link: @product)
  ...
  context_ "when accessed via custom domain", type: :request do
  test "redirects to download page without raising UnsafeRedirectError" do
            file = create(:product_file, link: @product)
            url_redirect = UrlRedirect.find_by(token: @token)
            custom_domain = create(:custom_domain, user: @product.user)
  ...
  context_ "when a PDF must be stamped and stamped file is missing" do
  test "shows alert, enqueues job (critical, notify=true), and redirects to download page" do
  ...
  context_ "GET download_subtitle_file" do
      before do
  ...
  test "redirects to the subtitle file URL" do
  ...
  test "marks the url_redirect as seen" do
  ...
  test "renders confirm if has been seen and no cookie" do
  ...
  test "does not render confirm if has_been_seen is false and no cookie and different ip" do
        @url_redirect = create(:url_redirect, uses: 5, has_been_seen: false,
                                              purchase: create(:purchase, email: "abCabC@abc.com", link: create(:product_with_pdf_file)))
        cookies.encrypted[:confirmed_redirect] = nil
        @request.remote_ip = "123.4.5.6"
        get :show, params: { id: @url_redirect.token }
        expect(response).not_to redirect_to(confirm_page_path(id: @url_redirect.token))
      end

  test "increments uses if purchaser is the same" do
        @user = create(:user)
  ...
  test "increments uses if ip is the same" do
  ...
  test "increments the uses if ip_address or purchaser don't match" do
        @purchase = create(:purchase, ip_address: "123.0.0.1", link: create(:product_with_pdf_file))
        @url_redirect = create(:url_redirect, purchase: @purchase)
  ...
  test "404s if url id does not exist" do
        expect do
          get :show, params: { id: "monkey" }
        end.to raise_error(ActionController::RoutingError)
      end

  test "does not ask user to confirm if there is no purchase" do
  ...
  context_ "streaming" do
  test "renders correctly for stream-only files" do
  ...
  test "sets the m3u8 playlist url and the original video url in sources for hls-transcoded video" do
  ...
  test "sets the smil url and the original video url in sources if the video is not HLS-transcoded yet" do
  ...
  test "sets the correct source urls for all video files in the product and returns the index of the file that should be played on load" do
  ...
  test "sets the correct source urls for all video attachments of an installment and returns the index of the file that should be played on load" do
            pdf_file = create(:product_file, url: "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachment/manual.pdf")
            video_file_1 = create(:product_file, url: "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachments/2/original/chapter2.mp4", is_transcoded_for_hls: true, display_name: "Chapter 2", position: 2)
  ...
  test "replaces the links to the playlists with signed urls" do
  ...
  test "urls encode the playlist urls" do
  ...
  test "escapes RFC 3986 2.2 reserved characters in the file name" do
  ...
  test "sets the rental_first_viewed_at property" do
  ...
  test "show the proper download page for multiple files" do
          get :download_page, params: { id: @url_redirect.token }
          expect(response).to be_successful
  ...
  test "redirects to the correct url for downloading a specific file" do
  ...
  test "creates the proper consumption event" do
          get :download_product_files, params: { id: @url_redirect.token, product_file_ids: [@product.product_files.first.external_id] }
  ...
  test "displays license information" do
            get :download_page, params: { id: @url_redirect.token }
            expect(response).to be_successful
            file_names = inertia.props.dig(:content, :content_items).map { |item| item[:file_name] }
            expect(file_names).to include("chapter1")
  ...
  test "allows product to be accessed" do
          expect do
            expect do
              get :show, params: { id: @url_redirect.token }
              expect(response.code).not_to eq(404)
  ...
  context_ "GET 'confirm_page'", inertia: true do
      before do
  ...
  test "renders the confirm page correctly" do
        get :confirm_page, params: { id: @url_redirect.token }
        expect(response).to be_successful
  ...
  test "assigns the url_redirect correctly" do
        get :confirm_page, params: { id: @url_redirect.token }
        expect(assigns(:url_redirect)).to eq @url_redirect
      end

  context_ "when params[:destination] is set" do
  test "assigns @destination with params[:destination]" do
  ...
  context_ "when params[:destination] is not set" do
  context_ "when the purchased product has rich content" do
          before do
  ...
  test "assigns @destination with 'download_page'" do
            get :confirm_page, params: { id: @url_redirect.token }
  ...
  test "assigns @destination with nil value for an installment" do
          installment = create(:installment)
  ...
  context_ "POST 'confirm'" do
  context_ "download page" do
  test "redirects and reset counter if email is correct" do
  ...
  test "forwards the 'display' query parameter while redirecting" do
  ...
  test "renders confirm again if the email address is bad" do
  ...
  test "does not raise an exception when the email is absent" do
        expect { post :confirm, params: { id: @token, email: nil } }.not_to raise_error
        expect(response).to redirect_to confirm_page_path(id: @url_redirect.token)
      end
    end

  context_ "GET send_to_kindle" do
  ...
  test "queues send_to_kindle for a valid kindle email and creates consumption event" do
        expect do
          expect do
  ...
  context_ "when file_external_id belongs to a different product" do
  ...
  context_ "when file_external_id is missing" do
  test "returns not found" do
          expect do
            post :send_to_kindle, params: {
  ...
  context_ "when file_external_id is invalid" do
  test "returns not found" do
          expect do
            post :send_to_kindle, params: {
  ...
  context_ "when the file is not eligible for Kindle" do
        before do
  ...
  test "returns JSON not found" do
          expect do
            post :send_to_kindle, params: {
              id: url_redirect.token, file_external_id: product_file.external_id, email: "dude@kindle.com"
            }
  ...
  context_ "when purchase has chargeback" do
        before do
  ...
  context_ "when access is revoked" do
        before do
  ...
  context_ "when a different user is signed in" do
        before do
  ...
  context_ "when identity is not verified" do
        before do
          url_redirect.update!(has_been_seen: true)
  ...
  context_ "invalid kindle emails" do
  test "cannot create a user with a bad kindle email address" do
          expect do
            expect do
              expect do
  ...
  context_ "with invalid email address" do
          before do
            expect(CustomerMailer).not_to receive(:send_to_kindle)
          end

  context_ "when the email is not provided" do
  ...
  context_ "when the email provided is invalid" do
  test "returns error" do
              expect do
                post :send_to_kindle, params: {
                  id: url_redirect.token,
  ...
  test "returns the correct error" do
              expect do
                post :send_to_kindle, params: {
                  id: url_redirect.token,
                  file_external_id: product_file.external_id,
  ...
  context_ "reading", inertia: true do
  context_ "Product" do
        before do
          @product = create(:product_with_pdf_file)
  ...
  test "renders the download page with read url" do
            get :download_page, params: { id: @token }
            expect(response).to be_successful
            expect_inertia.to render_component("UrlRedirects/DownloadPage")
  ...
  test "has a a read button for a PDF product file" do
            get :download_page, params: { id: @token }
            expect(response).to be_successful
            expect_inertia.to render_component("UrlRedirects/DownloadPage")
            read_url = url_redirect_read_for_product_file_path(@token, @product.product_files.first.external_id)
  ...
  test "can be read with proper file download URL" do
  ...
  test "redirects to home if something goes awry and not logged in" do
            sign_out User.last
            get :read, params: { id: "I don't even have a beautiful URL Redirect Token" }
            expect(response).to redirect_to(root_path)
          end

  test "gets current product file if replaced" do
            @product.product_files.each(&:mark_deleted)
  ...
  test "recovers from an S3 error" do
  ...
  test "has a readable Product File for a PDF installment with no associated product" do
          get :download_page, params: { id: @token }
  ...
  context_ "change_purchaser" do
  test "changes the purchaser to current user and redirect" do
        user = create(:user)
  ...
  test "redirects to root path when next param is missing" do
        user = create(:user)
  ...
  test "redirects to the check_purchaser page if the email is incorrect" do
        user = create(:user)
  ...
  context_ "GET membership_inactive_page", inertia: true do
  ...
  test "renders the Inertia component" do
  ...
  test "includes purchase data for subscriptions that can be restarted" do
        get :membership_inactive_page, params: { id: url_redirect.token }
  ...
  test "includes purchase data for subscriptions that cannot be restarted" do
  ...
  context_ "GET rental_expired_page", inertia: true do
  test "renders the Inertia component" do
  ...
  context_ "GET expired", inertia: true do
  test "renders the Inertia component" do
  ...
  context_ "download_page polling", inertia: true do
  ...
  context_ "audio_durations partial prop" do
  test "returns audio durations via Inertia partial request" do
          product = create(:product)
  ...
  test "returns updated durations for processing files" do
          product = create(:product)
  ...
  context_ "latest_media_locations partial prop" do
  test "returns latest media locations via Inertia partial request" do
          product = create(:product)
  ...
  test "returns empty hash for installment url redirects" do
          seller = create(:user)
          product = create(:product, user: seller)
  ...
  context_ "combined polling" do
  test "returns both props when both are requested" do
          product = create(:product)
  ...
  context_ "side effects" do
  test "does not create consumption events during polling" do
          expect do
            request.headers.merge!(polling_headers.merge("X-Inertia-Partial-Data" => "audio_durations"))
            get :download_page, params: { id: @token }
          end.not_to change(ConsumptionEvent, :count)
  ...
  test "does not increment usage count during polling" do
          expect do
            request.headers.merge!(polling_headers.merge("X-Inertia-Partial-Data" => "audio_durations"))
            get :download_page, params: { id: @token }
  ...
  test "does not include dropbox_api_key during polling" do
          request.headers.merge!(polling_headers.merge("X-Inertia-Partial-Data" => "audio_durations"))
          get :download_page, params: { id: @token }
  ...
  context_ "non-polling requests" do
  test "includes dropbox_api_key on full load" do
          get :download_page, params: { id: @token }
  ...
  test "creates consumption events and increments usage on full load" do
          expect do
            get :download_page, params: { id: @token }
          end.to change(ConsumptionEvent, :count).by(1)
  ...
  test "treats partial requests with non-polling props as full page loads" do
          expect do
  ...
  test "returns empty hash if the 'file_ids' parameter is blank" do
  ...
  test "returns the media urls for the given file ids" do
        freeze_time do
  ...
  context_ "POST 'save_last_content_page'" do
      let(:product) { create(:product_with_pdf_file) }
      let(:purchase) { create(:purchase, link: product) }
      let!(:url_redirect) { create(:url_redirect, link: product, purchase:) }

  test "saves the last content page id for the purchase" do
        expect do
  ...
  test "updates an existing last content page id" do
  ...
  context_ "when there is no purchase" do
  ...
  test "handles missing page_id parameter by setting nil" do
  ...
  context_ "when access is revoked for purchase" do
        before do
          purchase.update!(is_access_revoked: true)
        end
  ...
  test "saves the last content page id via custom domain" do
          expect do
