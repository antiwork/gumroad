...
class SignupControllerTest < ActionController::TestCase
  self.described_class = SignupController
  tests SignupController



  context_ SignupController, type: :controller, inertia: true do
    render_views
  ...
  context_ "GET new" do
  context_ "Sign up and connect to OAuth app" do
        before do
  ...
  test "returns the page successfully, sets the props correctly and sets noindex header" do
  ...
  context_ "when an email is provided in the params" do
  test "sets the email in the props" do
            get :new, params: { email: "test@example.com" }
            expect(inertia.props[:email]).to eq "test@example.com"
          end
        end

  context_ "when an email is present in the next parameter" do
  test "sets the email in the props" do
  ...
  context_ "when a referrer is provided in the params" do
          let(:referrer) { create(:user, username: "testreferrer") }

  test "sets the referrer in the props" do
  ...
  context_ "when stats are present in Redis" do
          before do
  ...
  test "sets the stats in the props" do
            get :new
  ...
  context_ "when next param does not start with /oauth/authorize" do
  test "sets noindex header" do
  ...
  context_ "POST create", :vcr do
  context_ "user already exists" do
        before do
          @user = create(:user, password: "password")
        end

  context_ "when two factor authentication is disabled for the user" do
  test "signs in as user" do
  ...
  test "sets the user_id in session and redirects for two factor authentication" do
  ...
  test "creates a user and returns json response" do
        user = build(:user, password: "password")
        post "create", params: { user: { email: user.email, password: "password" } }, format: :json
  ...
  test "sets two factor authenticated" do
  ...
  test "redirects to the OAuth authorization path after successful login" do
  ...
  context_ "tos agreement" do
  context_ "signup on a page that displayed the terms notice" do
          let(:params) do
            {
              user: {
  ...
  test "saves a tos agreement record for the user with their IP" do
  ...
  context_ "signup on a page that did not display the terms notice" do
          let(:params) do
            {
              user: {
                email: generate(:email),
  ...
  test "does not save a tos agreement record for the user with their IP" do
            post("create", params:)
            user = User.last
  ...
  test "creates a global affiliate record" do
        user = build(:user, password: "password")
        post "create", params: { user: { email: user.email, password: "password" } }
        user = User.last
  ...
  test "saves the user even when a payment made with a transient client token is expired" do
  ...
  test "does not create user if user payload is not given" do
        post "create"
        expect(response).to redirect_to(signup_path)
        expect(flash[:warning]).to eq "Please provide a valid email address."
      end

  test "returns json error if user payload is not given" do
        post "create", format: :json
        expect(response.parsed_body["success"]).to be(false)
        expect(response.parsed_body["error_message"]).to eq "Please provide a valid email address."
      end

  test "turns notifications off the user if the user is from Canada" do
  ...
  test "creates a signup event" do
  ...
  test "saves the account created ip" do
        @request.remote_ip = "12.12.128.128"
        @user = build(:user, password: "password")
        post "create", params: { user: { email: @user.email, password: "password" } }
  ...
  test "doesn't redirect externally and deals with badly-formed referer URLs" do
  ...
  context_ "invites" do
  context_ "external_id" do
          before do
            @user = create(:user)
            create(:invite, sender_id: @user.id, receiver_email: "anish@gumroad.com")
          end

  test "updates invite to signed up and save receiver_id" do
  ...
  test "creates a new invite and make it signed up with receiver_id" do
  ...
  test "links user purchase and credit card", :vcr do
        ...
        expect(last_user.credit_card.id).not_to be(nil)
        expect(last_user.credit_card.expiry_month).to eq 12
        expect(last_user.credit_card.expiry_year).to eq 2023
      end

  test "links user purchase but not credit card if fingerprint different to purchase credit card", :vcr do
        # Provided a credit card and purchase made, a user that signs up for a
        # buyer side account will be linked to these other models.
        # Signup with CC is without CVC because it's done automatically in the background when user enters a password
        # after having completed a purchase (that clears the CVC).
  ...
  test "enqueues a background job to link past purchases by email" do
        user = build(:user, password: "password")
  ...
  test "rejects mass assignment of privileged attributes" do
        params = {
          user: {
            email: generate(:email),
            password: "password",
  ...
  test "associates the preorder with the newly created user", :vcr do
        purchase = create(:purchase)
  ...
  context_ "POST 'save_to_library'" do
  context_ "email already taken" do
        before do
          @user = create(:user)
          @purchase = create(:purchase)
  ...
  test "failed and has email taken error" do
  ...
  context_ "email not yet taken" do
        before do
          @purchase = create(:purchase)
        end

  test "signs up the user" do
  ...
  test "assigns them the correct purchase" do
  ...
  test "redirects to the library" do
          post :save_to_library, params: { user: { email: @purchase.email, password: "blah123", purchase_id: @purchase.external_id } }
          expect(response.parsed_body["success"]).to be(true)
          expect(response.parsed_body["error_message"]).to be(nil)
        end
