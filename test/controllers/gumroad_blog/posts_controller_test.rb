...
class GumroadBlogPostsControllerTest < ActionController::TestCase
  self.described_class = GumroadBlog::PostsController
  tests GumroadBlog::PostsController



  context_ GumroadBlog::PostsController, inertia: true do
  ...
  test "only includes posts that are visible on profile, order by published_at descending" do
        get :index
  ...
  context_ "when post is not found" do
  test "raises ActiveRecord::RecordNotFound" do
          expect { get :show, params: { slug: "nonexistent-slug" } }.to raise_error(ActiveRecord::RecordNotFound)
        end
      end

  context_ "when post belongs to different user" do
