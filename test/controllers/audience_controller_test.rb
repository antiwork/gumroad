...
class AudienceControllerTest < ActionController::TestCase
  self.described_class = AudienceController
  tests AudienceController



  context_ AudienceController, inertia: true do
  ...
  test "renders Inertia component with zero followers" do
        get :index
  ...
  test "renders Inertia component with correct follower count and deferred audience data" do
  ...
  context_ "when fetching the deferred audience data prop" do
        before do
  ...
  test "returns audience_data with expected structure", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  test "handles various timezone formats in date parameters" do
  ...
  test "enqueues a job for sending the CSV" do
