...
class CheckoutControllerTest < ActionController::TestCase
  self.described_class = CheckoutController
  tests CheckoutController



  context_ CheckoutController, type: :controller, inertia: true do
    let!(:seller) { create(:named_seller) }

  context_ "GET show" do
  test "return inertia component with correct props and force enables analytics" do
  ...
  test "does not raise when checkout params contain nested values" do
        get :show, params: { product: { foo: "bar" }, username: { baz: "qux" }, wishlist: { id: "1" } }

        expect(response).to be_successful
      end

  context_ "process_cart_id_param check" do
  ...
  test "does not redirect when cart_id is blank" do
            get :show
  ...
  test "redirects to the same path removing the `cart_id` query param" do
  ...
  test "redirects to the same path when `cart_id` is not found" do
            get :show, params: { cart_id: "no-such-cart" }

            expect(response).to redirect_to(checkout_path(referrer: UrlService.discover_domain_with_protocol))
          end

  test "redirects to the same path when an OLD/INSECURE external_id is used" do
  ...
  test "redirects to the same path when the cart for `cart_id` is deleted" do
            cart.mark_deleted!
  ...
  context_ "when the cart matching the `cart_id` query param belongs to a user" do
  test "redirects to the login page path with `next` param set to the checkout path" do
              get :show, params: { cart_id: secure_id }
  ...
  context_ "when the cart matching the `cart_id` query param has the `browser_guid` same as the current `_gumroad_guid` cookie value"  do
  test "redirects to the same path without modifying the cart" do
              browser_guid = SecureRandom.uuid
              cookies[:_gumroad_guid] = browser_guid
  ...
  context_ "when the cart matching the `cart_id` query param has the `browser_guid` different from the current `_gumroad_guid` cookie value" do
  test "merges the current guest cart with the cart matching the `cart_id` query param" do
  ...
  context_ "for partial visits" do
  ...
  test "calls CheckoutService and returns product cards" do
  ...
  test "returns empty array when recommendations time out" do
  ...
  test "creates and populates a cart" do
  ...
  test "updates an existing cart" do
  ...
  test "forces the cart email to the signed-in user's email regardless of the submitted email" do
  ...
  test "updates `browser_guid` with the value of the `_gumroad_guid` cookie" do
  ...
  test "does not change products that are already deleted" do
          product = create(:product)
  ...
  test "returns an error when cart contains more than allowed number of cart products" do
  ...
  test "creates an empty cart when the `items` key is missing from params" do
          expect do
  ...
  test "creates a cart when the `discountCodes` key is missing from params" do
          expect do
            patch :update, params: { cart: { items: [] } }, as: :json
          end.to change(Cart, :count).by(1)
  ...
  test "acquires a row lock on the cart to prevent deadlocks" do
  ...
  test "rescues ActiveRecord::Deadlocked and redirects with an error" do
  ...
  test "returns an error when the `cart` param is not a Hash" do
          expect do
