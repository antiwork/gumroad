...
class DropboxFilesControllerTest < ActionController::TestCase
  self.described_class = DropboxFilesController
  tests DropboxFilesController



  context_ DropboxFilesController do
  ...
  test "returns available files for product" do
  ...
  test "returns available files for user" do
  ...
  context_ "POST cancel_upload" do
      let(:product) { create(:product, user: seller) }
      let!(:dropbox_file) { create(:dropbox_file, link: product) }
