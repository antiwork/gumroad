# frozen_string_literal: true

require "test_helper"

class CareersControllerTest < ActionController::TestCase
  self.described_class = CareersController
  tests CareersController



  context_ CareersController do
    render_views

    before { allow(GithubStarsController).to receive(:cached_count).and_return(1234) }

  context_ "GET index" do
  context_ "when career_pages feature is active" do
        before { Feature.activate(:career_pages) }

  test "renders successfully" do
          get :index

          expect(response).to be_successful
          expect(assigns(:title)).to eq("Careers at Gumroad - Build the road with us")
          expect(assigns(:hide_layouts)).to be(true)
          expect(assigns(:jobs)).to eq(JOBS)
        end
      end

  context_ "when career_pages feature is inactive" do
        before { Feature.deactivate(:career_pages) }

  test "returns 404" do
          expect { get :index }.to raise_error(ActionController::RoutingError, "Not Found")
        end
      end
    end

  context_ "GET show" do
  context_ "when career_pages feature is active" do
        before { Feature.activate(:career_pages) }

  test "renders successfully for a valid job slug" do
          get :show, params: { slug: "design-engineer" }

          expect(response).to be_successful
          expect(assigns(:job)[:slug]).to eq("design-engineer")
          expect(assigns(:job)[:title]).to eq("Design Engineer")
          expect(assigns(:title)).to eq("Design Engineer - Gumroad Careers")
          expect(assigns(:hide_layouts)).to be(true)
        end

  test "raises not found for an invalid job slug" do
          expect { get :show, params: { slug: "invalid-job-slug" } }
            .to raise_error(ActionController::RoutingError, "Not Found")
        end
      end

  context_ "when career_pages feature is inactive" do
        before { Feature.deactivate(:career_pages) }

  test "returns 404" do
          expect { get :show, params: { slug: "design-engineer" } }
            .to raise_error(ActionController::RoutingError, "Not Found")
        end
      end
    end
  end
end
