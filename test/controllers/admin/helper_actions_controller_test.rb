...
class AdminHelperActionsControllerTest < ActionController::TestCase
  self.described_class = Admin::HelperActionsController
  tests Admin::HelperActionsController


  context_ Admin::HelperActionsController do
    let(:admin) { create(:admin_user) }
    let(:user) { create(:user) }

  context_ "GET impersonate" do
  test "redirects to admin impersonation when authenticated as admin" do
        sign_in(admin)
  ...
  test "redirects to root path when not authenticated as admin" do
        sign_in(create(:user))
  ...
  context_ "GET stripe_dashboard" do
  ...
  test "redirects to Stripe dashboard when authenticated as admin" do
        sign_in(admin)
  ...
  test "returns not found when user has no Stripe account" do
        sign_in(admin)
