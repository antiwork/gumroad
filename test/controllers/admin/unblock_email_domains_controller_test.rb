# frozen_string_literal: true

require "test_helper"
require "shared_examples/admin_base_controller_concern"

class AdminUnblockEmailDomainsControllerTest < ActionController::TestCase
  self.described_class = Admin::UnblockEmailDomainsController
  tests Admin::UnblockEmailDomainsController



  context_ Admin::UnblockEmailDomainsController, type: :controller, inertia: true do
    render_views

    it_behaves_like "inherits from Admin::BaseController"

    let(:non_admin_user) { create(:user) }
    let(:admin_user) { create(:admin_user) }

    before do
      sign_in admin_user
    end

  context_ "GET show" do
  test "renders the page to unsuspend users if admin" do
        get :show
        expect(response).to be_successful
        expect(inertia.component).to eq "Admin/UnblockEmailDomains/Show"
      end
    end

  context_ "PUT update" do
      let(:email_domains_to_unblock) { %w[example.com example.org] }
      let(:identifiers) { email_domains_to_unblock.join("\n") }

  test "enqueues a job to unsuspend the specified email domains" do
        put :update, params: { email_domains: { identifiers: } }
        expect(UnblockObjectWorker.jobs.size).to eq(2)
        expect(response).to redirect_to(admin_unblock_email_domains_url)
        expect(flash[:notice]).to eq "Email domains unblocked successfully!"
      end

  test "unblocks email domain", :sidekiq_inline do
        PlatformBlock.add!(object_type: PlatformBlock::TYPES[:email_domain], object_value: "example.com")

        put :update, params: { email_domains: { identifiers: } }
        record = PlatformBlock.find_by(object_value: "example.com")
        expect(record).to be_present
        expect(record.blocked_at).to be_nil
      end
    end
  end
end
