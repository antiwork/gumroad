...
class AdminPaydaysControllerTest < ActionController::TestCase
  self.described_class = Admin::PaydaysController
  tests Admin::PaydaysController



  context_ Admin::PaydaysController do
  ...
  test "pays the seller for balances up to and including the date passed in params" do
  ...
  test "does not pay the user if there are pending payments" do
  ...
  test "attempts to pay the user via Stripe if the `payout_processor` is 'STRIPE'" do
