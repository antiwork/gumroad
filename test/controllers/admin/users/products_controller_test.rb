...
class AdminUsersProductsControllerTest < ActionController::TestCase
  self.described_class = Admin::Users::ProductsController
  tests Admin::Users::ProductsController



  context_ Admin::Users::ProductsController, inertia: true do
    render_views
  ...
  context_ "when user has deleted products" do
        let(:product2) { create(:product, user:, deleted_at: Time.current) }

  test "includes deleted products in list" do
          expect(inertia.props[:products]).to contain_exactly(hash_including(external_id: product1.external_id), hash_including(external_id: product2.external_id))
        end
      end

  context_ "when user has banned products" do
        let(:product2) { create(:product, user:, banned_at: Time.current) }

  test "includes banned products in list" do
          expect(inertia.props[:products]).to contain_exactly(hash_including(external_id: product1.external_id), hash_including(external_id: product2.external_id))
        end
      end
    end
