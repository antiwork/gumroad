...
class AdminUsersMerchantAccountsControllerTest < ActionController::TestCase
  self.described_class = Admin::Users::MerchantAccountsController
  tests Admin::Users::MerchantAccountsController



  context_ Admin::Users::MerchantAccountsController do
  ...
  context_ "when user has no merchant accounts" do
  test "returns empty merchant accounts and false for stripe account" do
  ...
  test "returns true for has_stripe_account" do
          get :index, params: { user_external_id: user.external_id }, format: :json
  ...
  context_ "when user has deleted or inactive merchant accounts" do
  ...
  test "returns false for has_stripe_account" do
          get :index, params: { user_external_id: user.external_id }, format: :json
  ...
  context_ "when user has only paypal accounts" do
        let!(:paypal_account) { create(:merchant_account_paypal, user: user) }
