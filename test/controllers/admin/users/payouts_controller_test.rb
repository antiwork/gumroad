...
class AdminUsersPayoutsControllerTest < ActionController::TestCase
  self.described_class = Admin::Users::PayoutsController
  tests Admin::Users::PayoutsController



  context_ Admin::Users::PayoutsController, type: :controller, inertia: true do
  ...
  test "lists all the payouts for a user" do
        sign_in admin_user
  ...
  test "pauses payouts for seller, sets the pause source as admin, and saves the provided reason" do
  ...
  test "pauses payouts for seller and sets the pause source as admin even if no reason is provided" do
        expect(seller.payouts_paused_internally?).to be false
        expect(seller.payouts_paused_by_source).to be nil
        expect(seller.payouts_paused_for_reason).to be nil
  ...
  test "resumes payouts for seller and clears the payout pause source if payouts are paused by admin" do
  ...
  test "resumes payouts for seller and clears the payout pause source if payouts are paused by stripe" do
  ...
  test "resumes payouts for seller and clears the payout pause source if payouts are paused by the system" do
