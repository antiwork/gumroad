...
class AdminUsersEmailChangesControllerTest < ActionController::TestCase
  self.described_class = Admin::Users::EmailChangesController
  tests Admin::Users::EmailChangesController



  context_ Admin::Users::EmailChangesController do
  ...
  context_ "when user has email and payment_address changes", versioning: true do
        before do
  ...
  test "returns email changes and fields" do
  ...
  context_ "when user has no changes" do
  test "returns empty email changes" do
          get :index, params: { user_external_id: user.external_id }, format: :json
