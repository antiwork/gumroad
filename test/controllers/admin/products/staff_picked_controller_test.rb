...
class AdminProductsStaffPickedControllerTest < ActionController::TestCase
  self.described_class = Admin::Products::StaffPickedController
  tests Admin::Products::StaffPickedController



  context_ Admin::Products::StaffPickedController do
  ...
  context_ "when product doesn't have an associated staff_picked_product" do
  ...
  context_ "when product has a deleted staff_picked_product record" do
        before do
          product.create_staff_picked_product!(deleted_at: Time.current)
        end

  test "updates the record as not deleted" do
          post :create, params: { product_external_id: product.external_id }, format: :json
