...
class AdminUnreviewedUsersControllerTest < ActionController::TestCase
  self.described_class = Admin::UnreviewedUsersController
  tests Admin::UnreviewedUsersController



  context_ Admin::UnreviewedUsersController, type: :controller, inertia: true do
    render_views
  ...
  context_ "when logged in as non-admin" do
  ...
  context_ "when logged in as admin" do
        before do
          $redis.del(RedisKey.unreviewed_users_cutoff_date)
        end

  context_ "when no cached data exists" do
          before do
            $redis.del(RedisKey.unreviewed_users_data)
          end

  test "returns empty state with default cutoff_date" do
            get :index
  ...
  context_ "when cached data exists" do
  ...
  test "returns cached users with props from service" do
            get :index
  ...
  context_ "when user is reviewed after caching" do
          let!(:user) do
            create(:user, user_risk_state: "not_reviewed", created_at: 1.year.ago)
          end
  ...
  test "filters out users who are no longer not_reviewed" do
            get :index
