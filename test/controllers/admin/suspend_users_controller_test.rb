...
class AdminSuspendUsersControllerTest < ActionController::TestCase
  self.described_class = Admin::SuspendUsersController
  tests Admin::SuspendUsersController



  context_ Admin::SuspendUsersController, type: :controller, inertia: true do
    render_views
  ...
  context_ "when external IDs are provided" do
  ...
  context_ "when additional notes are provided" do
        let(:additional_notes) { "Some additional notes" }
        let(:specified_ids) { user_ids_to_suspend.join(", ") }

  test "passes the additional notes as job's param" do
          expect(SuspendUsersWorker).to have_enqueued_sidekiq_job(admin_user.id, user_ids_to_suspend, reason, additional_notes, expected_scheduled_payout)
          expect(flash[:notice]).to eq "User suspension in progress!"
          expect(response).to redirect_to(admin_suspend_users_url)
        end
  ...
  context_ "when a scheduled payout action is provided" do
        let(:specified_ids) { user_ids_to_suspend.join(", ") }
        let(:scheduled_payout_params) { { action: "payout", delay_days: "14" } }
        let(:expected_scheduled_payout) { { "action" => "payout", "delay_days" => "14" } }

  test "forwards the scheduled payout params to the worker" do
          expect(SuspendUsersWorker).to have_enqueued_sidekiq_job(admin_user.id, user_ids_to_suspend, reason, additional_notes, expected_scheduled_payout)
        end
      end

  context_ "when a hold action is provided without delay_days" do
        let(:specified_ids) { user_ids_to_suspend.join(", ") }
        let(:scheduled_payout_params) { { action: "hold", delay_days: "" } }
        let(:expected_scheduled_payout) { { "action" => "hold", "delay_days" => nil } }

  test "forwards the hold action and nil delay to the worker" do
          expect(SuspendUsersWorker).to have_enqueued_sidekiq_job(admin_user.id, user_ids_to_suspend, reason, additional_notes, expected_scheduled_payout)
        end
      end

  context_ "when scheduled payout action is blank" do
        let(:specified_ids) { user_ids_to_suspend.join(", ") }
        let(:scheduled_payout_params) { { action: "", delay_days: "14" } }

  test "does not forward scheduled payout params to the worker" do
