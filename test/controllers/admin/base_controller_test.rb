...
class AdminBaseControllerTest < ActionController::TestCase
  self.described_class = Admin::BaseController
  tests Admin::BaseController



  context_ Admin::BaseController, type: :controller, inertia: true do
    render_views
  ...
  context_ "when not logged in" do
        it_behaves_like "404 for xhr request"
        it_behaves_like "404 for json request format"

  context_ "with html request format" do
          before do
            @request.path = "/about"
          end

  test "redirects user to login when trying to access admin with proper next param value" do
            get :index
  ...
  context_ "with non-admin as current user" do
          before do
            sign_in not_admin_user
          end

  context_ "with self as current seller" do
            # current_seller = logged_in_user for user without team_memberhip
            it_behaves_like "404 for xhr request"
            it_behaves_like "404 for json request format"

  context_ "with html request format" do
  test "redirects to root_path and does not add next param" do
                get :index
  ...
  context_ "with admin as current seller" do
            before do
  ...
  context_ "with admin as current user" do
          before do
            sign_in admin_user
          end
  ...
  context_ "with self as current seller" do
            # current_seller = logged_in_user for user without team_memberhip
  test "returns the desired response" do
              get :index
  ...
  context_ "with non-admin as current seller" do
  test "returns the desired response" do
              get :index
  ...
  context_ "GET redirect_to_stripe_dashboard" do
      before do
        sign_in admin_user
      end

  context_ "when the seller has a Stripe account" do
        let(:seller) { create(:user) }
        let!(:merchant_account) { create(:merchant_account, user: seller) }

  test "redirects to Stripe dashboard" do
  ...
  context_ "when the seller is not found" do
  test "redirects to admin path with error" do
  ...
  context_ "when user has no Stripe account" do
