...
class AdminSearchPurchasesControllerTest < ActionController::TestCase
  self.described_class = Admin::Search::PurchasesController
  tests Admin::Search::PurchasesController



  context_ Admin::Search::PurchasesController, type: :controller, inertia: true do
    render_views
  ...
  context_ "when transaction_date is invalid" do
        let(:transaction_date) { "02/22" }

  test "shows error flash message and no purchases" do
  ...
  test "redirects to the admin purchase page when one purchase is found" do
  ...
  test "returns purchases from AdminSearchService" do
  ...
  context_ "when query is set" do
  test "filters by purchase status" do
