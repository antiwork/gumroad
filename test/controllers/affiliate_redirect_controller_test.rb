...
class AffiliateRedirectControllerTest < ActionController::TestCase
  self.described_class = AffiliateRedirectController
  tests AffiliateRedirectController



  context_ AffiliateRedirectController do
  ...
  context_ "set_cookie_and_redirect" do
  test "does not append anything to the redirect url if there were no url params" do
  ...
  context_ "when a custom destination URL is not set" do
  test "does not append anything to the redirect URL if there are no URL params" do
          get :set_cookie_and_redirect, params: { affiliate_id: direct_affiliate.external_id_numeric }
  ...
  test "preserves query parameters from the original request and appends them to the redirect URL but does not implicitly add the 'affiliate_id' query parameter" do
  ...
  test "redirects to the product URL" do
          get :set_cookie_and_redirect, params: { affiliate_id: direct_affiliate.external_id_numeric }
  ...
  context_ "when a custom destination URL is set" do
  test "implicitly adds 'affiliate_id' query parameter to the final destination URL during redirect" do
  ...
  test "adds the 'affiliate_id' query parameter along with the other URL params to the redirect URL when the destination URL already contains URL params" do
  ...
  test "redirects to the custom URL" do
          direct_affiliate.update!(destination_url: "https://gumroad.com/l/abc", apply_to_all_products: true)
  ...
  context_ "when destination URL has leading/trailing whitespace" do
  test "strips whitespace and redirects successfully" do
  ...
  context_ "when affiliate has no destination URL set and has multiple products" do
  ...
  test "redirects to the creator's profile page" do
          get :set_cookie_and_redirect, params: { affiliate_id: direct_affiliate.external_id_numeric }
