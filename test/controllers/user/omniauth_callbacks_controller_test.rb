...
class UserOmniauthCallbacksControllerTest < ActionController::TestCase
  self.described_class = User::OmniauthCallbacksController
  tests User::OmniauthCallbacksController



  context_ User::OmniauthCallbacksController do
    ...
    def fetch_json(service)
      JSON.parse(File.open("#{Rails.root}/test/support/fixtures/#{service}_omniauth.json").read)
    end
  ...
  context_ "#stripe_connect", :vcr do
  ...
  test "creates user if none exists" do
  ...
  test "signs in directly when user has no email" do
  ...
  test "signs in directly when user has email" do
          post :stripe_connect
  ...
  test "does not create a new user if the email is already taken" do
  ...
  context_ "when referer is payments settings" do
        before do
          request.env["omniauth.params"] = { "referer" => settings_payments_path }
        end

  test "throws error if stripe account is from an unsupported country" do
  ...
  test "throws error if creator already has another stripe account connected" do
          user = create(:user)
  ...
  test "does not allow user to login if the account is deleted" do
  ...
  context_ "when referer is signup" do
        let(:referer) { "signup" }
  ...
  context_ "when disable_stripe_signup feature flag is active" do
        before do
  ...
  test "does not allow new users to sign up via Stripe" do
          expect { post :stripe_connect }.not_to change { User.count }
  ...
  test "allows existing users with Stripe account to log in" do
  ...
  test "allows existing users with matching email (without Stripe connected) to log in" do
          create(:user, email: "stripe.connect@gum.co")
  ...
  test "allows existing users without email to log in" do
          user = create(:user)
  ...
  context_ "when user is admin" do
  test "allows admin user to log in with Apple" do
          admin = create(:admin_user)
  ...
  context_ "when user is marked as deleted" do
  ...
  test "does not allow user to login" do
          post :apple
  ...
  test "signs in the user without 2FA" do
          post :apple
          expect(controller.user_signed_in?).to be true
          expect(response).to redirect_to dashboard_path
        end

  test "keeps referral intact" do
  ...
  context_ "linking account" do
  test "links apple account to existing account" do
  ...
  context_ "when user is not created" do
        shared_examples "redirects to signup with error message" do
  test "redirects to the signup page with an error flash message" do
            post :apple
  ...
  context_ "when the user is not persisted" do
  ...
  context_ "when there's an error creating the user" do
  ...
  test "creates user if none exists", :vcr do
        expect do
          post :twitter
        end.to change { User.count }.by(1)
  ...
  test "does not allow user to login with Twitter only" do
          post :twitter
  ...
  context_ "linking account" do
  test "links twitter account to existing account", :vcr do
          ...
          expect(@user.twitter_oauth_token).not_to be(nil)
          expect(@user.twitter_oauth_secret).not_to be(nil)
          expect(@user.twitter_handle).not_to be(nil)
        end

  test "updates the Twitter OAuth credentials on account creation", :vcr do
          user = create(:user)
  ...
  test "redirects to login when no user is signed in and state is #{state}" do
  ...
  test "redirects to the settings page and ignores referrer", :vcr do
          post :twitter, params: { referer: balance_path }
  ...
  context_ "when the user has unconfirmed email" do
        before do
          user = create(:user)
          allow(User).to receive(:new).and_return(user)
          user.email = nil
  ...
  test "redirects to the settings page with the correct warning flash message", :vcr do
          post :twitter
  ...
  context_ "when user is admin" do
  test "allows admin user to log in with Google" do
          admin = create(:admin_user)
  ...
  test "sanitizes external domain in referral to a relative path" do
  ...
  context_ "linking account" do
  test "links google account to existing account", :vcr do
