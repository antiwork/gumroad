...
class UserPasswordsControllerTest < ActionController::TestCase
  self.described_class = User::PasswordsController
  tests User::PasswordsController



  context_ User::PasswordsController, type: :controller, inertia: true do
    render_views
  ...
  context_ "#new" do
  test "renders the Inertia password reset page" do
        get :new
  ...
  context_ "#create" do
  test "sends an email to the user and redirects with success message" do
        post(:create, params: { user: { email: @user.email } })
        expect(response).to redirect_to(login_url)
        expect(flash[:notice]).to eq("Password reset sent! Please make sure to check your spam folder.")
      end

  test "redirects with warning if email is blank even if matching user exists" do
  ...
  test "redirects with warning if email is not valid" do
  ...
  context_ "#edit" do
  test "shows a form for a valid token" do
        get(:edit, params: { reset_password_token: @user.send_reset_password_instructions })
        expect(response).to be_successful
      end

  context_ "should fail when errors" do
  test "shows an error for an invalid token" do
  ...
  context_ "#update" do
  test "logs in after successful pw reset" do
  ...
  test "invalidates all active sessions after successful password reset" do
  ...
  context_ "when the user has email two-factor authentication enabled" do
        before do
          @user = create(:user, skip_enabling_two_factor_authentication: false)
        end

  test "redirects to two-factor authentication instead of signing the user in" do
          token = @user.send_reset_password_instructions
  ...
  context_ "when the user has authenticator-app two-factor authentication enabled" do
        before do
          @user = create(:user, skip_enabling_two_factor_authentication: false)
  ...
  test "redirects to two-factor authentication and keeps the authenticator method" do
          token = @user.send_reset_password_instructions
  ...
  context_ "should fail when there are errors" do
        let(:old_password) { @user.password }

  test "shows error after unsuccessful pw reset" do
          token = @user.send_reset_password_instructions
  ...
  context_ "when specifying a compromised password", :vcr do
  test "fails with an error" do
            token = @user.send_reset_password_instructions
