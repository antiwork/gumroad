...
class CustomersControllerTest < ActionController::TestCase
  self.described_class = CustomersController
  self.rspec_metadata = { vcr: true }
  tests CustomersController



  context_ CustomersController, :vcr, type: :controller, inertia: true do
    render_views
  ...
  test "returns HTTP success and renders the correct inertia component and props" do
        get :index
  ...
  context_ "for a specific product" do
  test "renders the correct inertia component and props" do
  ...
  context_ "when seller is suspended for TOS violation" do
  ...
  test "returns HTTP success and assigns the correct instance variables" do
  ...
  context_ "minimum license uses filter" do
        before do
  ...
  context_ "when the :license_uses_sales_filter feature is active for the seller" do
          before { Feature.activate_user(:license_uses_sales_filter, seller) }

  test "filters by minimum license uses" do
  ...
  context_ "when the :license_uses_sales_filter feature is inactive for the seller" do
  test "ignores the minimum_license_uses param" do
  ...
  context_ "GET paged with active_customers_only filter" do
      let(:product) { create(:product, user: seller, name: "Product 1", price_cents: 100) }
      let(:customer_ids) { -> (res) { res.parsed_body.deep_symbolize_keys[:customers].map { _1[:id] } } }

  test "excludes customers with deactivated or cancelled subscriptions" do
  ...
  test "returns all customers when filter is not active" do
        active_purchase = create(:membership_purchase, seller:, link: product)
        cancelled_purchase = create(:membership_purchase, seller:, link: product)
        cancelled_purchase.subscription.update!(cancelled_at: 2.days.ago)
        index_model_records(Purchase)
  ...
  context_ "GET paged when Elasticsearch times out" do
  test "returns 504" do
  ...
  context_ "when purchase is an original subscription purchase" do
  test "returns all recurring purchases" do
  ...
  context_ "when purchase is a commission deposit purchase", :vcr do
        let!(:commission) { create(:commission) }

        before { commission.create_completion_purchase! }

  test "returns the deposit and completion purchases" do
  ...
  context_ "when the purchase isn't found" do
  test "returns 404" do
          expect do
  ...
  context_ "GET customer_emails" do
  ...
  context_ "with classic product" do
        before do
          @product = create(:product, user: seller)
          now = Time.current
  ...
  test "returns 404 if no purchase" do
          expect do
            get :customer_emails, params: { purchase_id: "hello" }
          end.to raise_error(ActiveRecord::RecordNotFound)
        end

  test "returns success true with only receipt default values" do
  ...
  test "returns success true with only receipt" do
  ...
  test "returns success true with receipt and posts" do
          create(:customer_email_info_opened, purchase: @purchase)
  ...
  context_ "with subscription product" do
  test "returns all receipts and posts ordered by date" do
  ...
  test "includes receipts for free trial original purchases" do
  ...
  test "returns EmailInfo from charge" do
          get :customer_emails, params: { purchase_id: purchase.external_id }
          expect(response).to be_successful
  ...
  test "returns success true with missed updates" do
  ...
  context_ "GET product_purchases" do
  ...
  test "returns product purchases" do
  ...
  context_ "no product purchases" do
