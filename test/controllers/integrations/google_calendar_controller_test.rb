...
class IntegrationsGoogleCalendarControllerTest < ActionController::TestCase
  self.described_class = Integrations::GoogleCalendarController
  tests Integrations::GoogleCalendarController



  context_ Integrations::GoogleCalendarController do
    before do
      sign_in create(:user)
    end
  ...
  context_ "GET account_info" do
  test "returns user account information for a valid oauth code" do
  ...
  test "fails if oauth authorization response does not have refresh token" do
        WebMock.stub_request(:post, "#{GoogleCalendarApi::GOOGLE_CALENDAR_OAUTH_URL}/token").with(body: oauth_request_body).
  ...
  test "fails if user info request fails" do
        WebMock.stub_request(:post, "#{GoogleCalendarApi::GOOGLE_CALENDAR_OAUTH_URL}/token").with(body: oauth_request_body).
          to_return(status: 200, body: { access_token:, refresh_token: }.to_json, headers: { content_type: "application/json" })
  ...
  test "fails if user info response does not have email" do
        WebMock.stub_request(:post, "#{GoogleCalendarApi::GOOGLE_CALENDAR_OAUTH_URL}/token").with(body: oauth_request_body).
          to_return(status: 200, body: { access_token:, refresh_token: }.to_json, headers: { content_type: "application/json" })
  ...
  context_ "GET calendar_list" do
  test "returns list of calendars from the google account associated with the access_token" do
  ...
  test "fails if the Google API for calendar list returns an error" do
        WebMock.stub_request(:get, "#{GoogleCalendarApi.base_uri}/calendar/v3/users/me/calendarList").with(headers: { "Authorization" => "Bearer #{access_token}" }).
  ...
  context_ "uses refresh_token if access_token is expired" do
  test "returns list of calendars from the google account associated with the access_token" do
          WebMock.stub_request(:get, "#{GoogleCalendarApi.base_uri}/calendar/v3/users/me/calendarList").with(headers: { "Authorization" => "Bearer #{access_token}" }).
  ...
  test "fails if request for refresh token fails" do
          WebMock.stub_request(:get, "#{GoogleCalendarApi.base_uri}/calendar/v3/users/me/calendarList").with(headers: { "Authorization" => "Bearer #{access_token}" }).
            to_return(status: 401, headers: { content_type: "application/json" })
  ...
  test "fails if refresh token response does not have access token" do
          WebMock.stub_request(:get, "#{GoogleCalendarApi.base_uri}/calendar/v3/users/me/calendarList").with(headers: { "Authorization" => "Bearer #{access_token}" }).
            to_return(status: 401, headers: { content_type: "application/json" })
