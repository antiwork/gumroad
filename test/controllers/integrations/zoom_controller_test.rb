...
class IntegrationsZoomControllerTest < ActionController::TestCase
  self.described_class = Integrations::ZoomController
  tests Integrations::ZoomController



  context_ Integrations::ZoomController do
    before do
      sign_in create(:user)
    end
  ...
  test "fails if user id is not present" do
        WebMock.stub_request(:post, ZoomApi::ZOOM_OAUTH_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
                    body: { access_token:, refresh_token: }.to_json,
  ...
  test "fails if user email is not present" do
        WebMock.stub_request(:post, ZoomApi::ZOOM_OAUTH_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
                    body: { access_token:, refresh_token: }.to_json,
