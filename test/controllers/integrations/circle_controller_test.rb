...
class IntegrationsCircleControllerTest < ActionController::TestCase
  self.described_class = Integrations::CircleController
  self.rspec_metadata = { vcr: true }
  tests Integrations::CircleController



  context_ Integrations::CircleController, :vcr, :without_circle_rate_limit do
  ...
  context_ "GET communities" do
  test "returns communities for a valid api_key" do
  ...
  test "fails if request to circle API fails" do
  ...
  test "fails if api_key is not passed" do
        get :communities, format: :json
        expect(response.status).to eq(200)
        expect(response.parsed_body).to eq({ "success" => false })
      end

  test "fails if invalid response is received from circle API" do
  ...
  test "fails if invalid API key is passed" do
  ...
  context_ "GET space_groups" do
  test "returns space_groups for a valid api_key and community_id" do
  ...
  test "fails if community_id is not passed" do
  ...
  context_ "GET communities_and_space_groups" do
  test "returns communities_and_space_groups for a valid api_key and community_id" do
  ...
  test "fails if request to circle API space_groups fails" do
        WebMock.stub_request(:get, space_groups_url).with(query: community_id_param).to_return(status: 500)
  ...
  test "fails if request to circle API communities fails" do
        WebMock.stub_request(:get, communities_url).to_return(status: 500)
  ...
  test "fails if invalid response is received from circle API communities" do
        WebMock.stub_request(:get, communities_url).to_return(status: 200, body: "invalid_error_response")
  ...
  test "fails if invalid response is received from circle API space_groups" do
        WebMock.stub_request(:get, space_groups_url).with(query: community_id_param).to_return(status: 200, body: "invalid_error_response")
