...
class IntegrationsDiscordControllerTest < ActionController::TestCase
  self.described_class = Integrations::DiscordController
  tests Integrations::DiscordController



  context_ Integrations::DiscordController do
    before do
      sign_in create(:user)
    end
  ...
  context_ "GET server_info" do
  test "returns server information for a valid oauth code" do
  ...
  test "fails if oauth authorization fails" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
  ...
  test "fails if oauth authorization response does not have access token" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
  ...
  test "fails if oauth authorization response does not have guild information" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
  ...
  test "fails if user identification fails" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
                    body: { access_token: "test_access_token", guild: { id: "0", name: "Gaming" } }.to_json,
  ...
  test "fails if Discord returns a non-JSON response" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
                    body: { access_token: "test_access_token", guild: { id: "0", name: "Gaming" } }.to_json,
  ...
  test "fails if oauth token endpoint returns a non-JSON response" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
  ...
  context_ "GET oauth_redirect" do
  test "returns successful response if code is present" do
        get :oauth_redirect, format: :json, params: { code: "test_code" }

        expect(response.status).to eq(200)
      end

  test "fails if code is not present" do
        get :oauth_redirect, format: :json

        expect(response.status).to eq(400)
      end

  test "redirects to subdomain if specified in state" do
  ...
  test "redirects to custom domain if specified in state" do
        user = create(:user, username: "test")
  ...
  context_ "GET join_server" do
      let(:user_id) { "user-0" }
      let(:integration) { create(:discord_integration) }
      let(:product) { create(:product, active_integrations: [integration]) }
      let(:purchase) { create(:purchase, link: product) }

  test "adds member to server for a purchase with an enabled integration and a valid code" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
                    body: { access_token: "test_access_token" }.to_json,
  ...
  test "adds member to server for a variant purchase with an enabled integration and a valid code" do
  ...
  test "fails if code is not passed" do
        expect do
  ...
  test "fails if purchase_id is not passed" do
        expect do
  ...
  test "fails if oauth response does not have access token" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
  ...
  test "fails if access_token is invalid" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
                    body: { access_token: "test_access_token" }.to_json,
  ...
  test "fails if purchased product does not have an integration" do
  ...
  test "fails if purchase_integration record creation fails" do
        WebMock.stub_request(:post, DISCORD_OAUTH_TOKEN_URL).
          with(body: oauth_request_body, headers: oauth_request_header).
          to_return(status: 200,
                    body: { access_token: "test_access_token" }.to_json,
  ...
  context_ "GET leave_server" do
      let(:user_id) { "user-0" }
      let(:integration) { create(:discord_integration) }
      let(:product) { create(:product, active_integrations: [integration]) }
      let(:purchase) { create(:purchase, link: product) }

  test "removes member from server if integration is active" do
  ...
  test "marks the purchase integration as deleted if the Discord server is deleted" do
        create(:purchase_integration, integration:, purchase:, discord_user_id: user_id)
  ...
  test "fails if purchased product does not have an activated integration" do
        expect do
          get :leave_server, format: :json, params: { purchase_id: purchase.external_id }
        end.to change { purchase.live_purchase_integrations.reload.count }.by(0)
  ...
  test "fails if call to discord api fails" do
  ...
  test "fails if Discord raises a permissions error" do
        create(:purchase_integration, integration:, purchase:, discord_user_id: user_id)
