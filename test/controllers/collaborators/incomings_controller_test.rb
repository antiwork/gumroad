...
class CollaboratorsIncomingsControllerTest < ActionController::TestCase
  self.described_class = Collaborators::IncomingsController
  tests Collaborators::IncomingsController



  context_ Collaborators::IncomingsController, inertia: true do
  ...
  test "returns the pending collaborations for the signed in user" do
  ...
  context_ "POST accept" do
      it_behaves_like "authentication required for action", :post, :accept do
        let(:request_params) { { id: collaborator.external_id } }
      end

  context_ "when logged in as the invited user" do
        before { sign_in invited_user }

  test "accepts the invitation when found and redirects to collaborations page" do
  ...
  test "returns not found when there is no invitation" do
          invitation.destroy!
          expect { post :accept, params: { id: collaborator.external_id } }.to raise_error(ActionController::RoutingError)
        end
      end

  context_ "when logged in as a different user" do
        let(:different_user) { create(:user) }

        before { sign_in different_user }

  test "redirects when invitation isn't for the current user" do
          post :accept, params: { id: collaborator.external_id }
  ...
  test "soft-deletes the collaborator when found and redirects to collaborations page" do
  ...
  test "deletes the collaborator, sends the appropriate email, and redirects to the collaborations page" do
        expect do
