...
class ApiMobileUrlRedirectsControllerTest < ActionController::TestCase
  self.described_class = Api::Mobile::UrlRedirectsController
  tests Api::Mobile::UrlRedirectsController



  context_ Api::Mobile::UrlRedirectsController do
    before do
  ...
  context_ "GET fetch_placeholder_products" do
  test "does not provide any placeholder products" do
  ...
  context_ "GET url_redirect_attributes" do
      before do
  ...
  test "provides purchase link and file data if the url redirect is still valid" do
  ...
  test "correctly marks if the url_redirect's purchase is invalid" do
  ...
  test "returns success when the url redirect has no associated product" do
  ...
  test "does not return purchase link and file data redirect external id is invalid" do
  ...
  test "returns only files for specific version purchase" do
  ...
  test "returns an unauthorized response if the user does not provide the correct mobile token" do
  ...
  test "returns an unauthorized response if the rental has expired" do
  ...
  test "returns a progressive download url if a m3u8 location does not exist for the product file" do
  ...
  test "returns an m3u8 url if there is an m3u8 file for the product file" do
        stub_playlist_s3_object_get.call
        travel_to(Date.parse("2014-01-27")) do
          get :stream, params: { token: url_redirect.token, product_file_id: file_1.external_id, mobile_token: Api::Mobile::BaseController::MOBILE_TOKEN, format: :json }
        end
  ...
  test "returns subtitle file information if any subtitle exists" do
        stub_playlist_s3_object_get.call
  ...
  test "creates the proper consumption events for watching" do
        stub_playlist_s3_object_get.call
  ...
  test "increments the uses count on the url_redirect" do
        stub_playlist_s3_object_get.call
  ...
  test "sets the rental_first_viewed_at property for a rental" do
        stub_playlist_s3_object_get.call
  ...
  test "never displays a confirmation page for mobile stream urls" do
  ...
  test "never displays a confirmation page for download urls" do
