...
class ApiMobileMediaLocationsControllerTest < ActionController::TestCase
  self.described_class = Api::Mobile::MediaLocationsController
  tests Api::Mobile::MediaLocationsController



  context_ Api::Mobile::MediaLocationsController do
  ...
  test "successfully creates media_location" do
  ...
  test "fails to create consumption event if access token is invalid" do
  ...
  test "fails to create consumption event if mobile token is invalid" do
  ...
  test "creates a media_location with consumed_at set to the current_time now if one is not provided" do
        media_location_params = {
          product_file_id: @product_file.external_id,
          url_redirect_id: @url_redirect.external_id,
          purchase_id: @purchase.external_id,
  ...
  context_ "avoid creating new media_location if valid existing media_location is present" do
  test "updates existing media_location instead of creating a new one if it exists" do
  ...
  test "creates new media_location if existing media_location is present but on different platform" do
          MediaLocation.create!(product_file_id: @product_file.id, product_id: @product.id, url_redirect_id: @url_redirect.id,
  ...
  test "ignores creating media locations for non consumable files" do
  ...
  test "does not update media location if event is older than the one in db" do
        MediaLocation.create!(product_file_id: @product_file.id, product_id: @product.id, url_redirect_id: @url_redirect.id,
                              purchase_id: @purchase.id, platform: "android", consumed_at: "2015-09-09T17:26:50PDT", location: 1)
        params = {
          product_file_id: @product_file.external_id,
  ...
  test "updates media location if event is newer than the one in db" do
        MediaLocation.create!(product_file_id: @product_file.id, product_id: @product.id, url_redirect_id: @url_redirect.id,
                              purchase_id: @purchase.id, platform: "android", consumed_at: "2015-09-09T17:26:50PDT", location: 1)
        params = {
          product_file_id: @product_file.external_id,
