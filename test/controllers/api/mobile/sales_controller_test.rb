...
class ApiMobileSalesControllerTest < ActionController::TestCase
  self.described_class = Api::Mobile::SalesController
  self.rspec_metadata = { vcr: true }
  tests Api::Mobile::SalesController



  context_ Api::Mobile::SalesController, :vcr do
    before do
      @seller = create(:user)
  ...
  context_ "PATCH refund" do
  context_ "when the purchase is not found" do
  test "responds with HTTP 404" do
  ...
  context_ "when the purchase is not paid" do
  test "responds with HTTP 404" do
  ...
  context_ "when the purchase is already refunded" do
  test "responds with HTTP 404" do
  ...
  context_ "when the amount contains a comma" do
  test "responds with invalid request error" do
  ...
  context_ "when the purchase is refunded" do
  test "responds with HTTP success" do
  ...
  context_ "when there's a refunding error" do
        before do
  ...
  test "response with error message" do
