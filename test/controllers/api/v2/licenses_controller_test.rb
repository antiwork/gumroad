...
class ApiV2LicensesControllerTest < ActionController::TestCase
  self.described_class = Api::V2::LicensesController
  tests Api::V2::LicensesController



  context_ Api::V2::LicensesController do
  ...
  test "returns the correct JSON when it modifies the license state" do
  ...
  test "returns an error response when a user provides a license key that does not exist for the provided product" do
  ...
  test "returns an error response when a user provides a non existent product" do
          put action, params: { access_token: @token.token, license_key: "Does not exist" }.merge(@product_identifier)
  ...
  test "does not allow modifying someone else's license" do
  ...
  test "responds with 403 forbidden" do
          put action, params: { access_token: @token.token, license_key: @purchase.license.serial }.merge(@product_identifier)
  ...
  test "it enables the license" do
  ...
  test "returns a 404 error if the license user is not the current resource owner" do
            token = create("doorkeeper/access_token", application: @app,
  ...
  test "it disables the license" do
  ...
  test "rotates the license key" do
  ...
  test "returns the correct json when a valid product and license key are provided" do
  ...
  test "returns correct json for purchase with quantity" do
  ...
  test "returns correct json for refunded and chargebacked purchases" do
  ...
  test "doesn't increment uses count when 'increase_uses_count' parameter is set to false" do
          post :verify, params: { license_key: @purchase.license.serial }.merge(@product_identifier)
          expect(response.code.to_i).to eq(200)
  ...
  test "increments the uses column as needed" do
          post :verify, params: { license_key: @purchase.license.serial }.merge(@product_identifier)
          post :verify, params: { license_key: @purchase.license.serial }.merge(@product_identifier)
          post :verify, params: { license_key: @purchase.license.serial }.merge(@product_identifier)
  ...
  test "accepts license keys from imported customers" do
  ...
  test "indicates that a license has been disabled" do
  ...
  test "doesn't verify authenticity token" do
  ...
  context_ "when access to purchase is revoked" do
          before do
            @purchase.update!(is_access_revoked: true)
          end

  test "responds with an error message" do
            post :verify, params: { license_key: @purchase.license.serial }.merge(@product_identifier)
  ...
  test "indicates if a subscription is ended for the license key", :vcr do
  ...
  test "indicates if a subscription is cancelled for the license key", :vcr do
  ...
  test "accepts license keys from subscription products and returns relevant information", :vcr do
  ...
  test "returns successful response for case-insensitive product_permalink param" do
  ...
  context_ "legacy params" do
  test "returns successful response when product permalink is passed via `id` param" do
  ...
  test "returns successful response when product permalink is passed via `link_id` param" do
  ...
  context_ "when product_id is blank" do
        before do
          create(:product, custom_permalink: @product.unique_permalink)
        end

  context_ "when product_id check is not skipped for the product" do
  context_ "when product_id param is enforced for the license verification of the product" do
            before do
              @redis_namespace = Redis::Namespace.new(:license_verifications, redis: $redis)
            end

  context_ "when product's created_at is after the set timestamp" do
              before do
  ...
  context_ "when the permalink doesn't match with the product" do
  test "responds with error without double rendering the response" do
  ...
  context_ "when product's created_at is before the set timestamp" do
              before do
                $redis.set(RedisKey.force_product_id_timestamp, @product.created_at + 1.day)
              end

  test "verifies the license" do
                post :verify, params: { product_permalink: @product.unique_permalink, license_key: @purchase.license.serial }
  ...
  context_ "when the product_id check is skipped for the product" do
          before do
            redis_namespace = Redis::Namespace.new(:license_verifications, redis: $redis)
  ...
  context_ "when product_id param is not blank" do
  test "verifies the license" do
  ...
  test "decreases the license uses count and returns the correct json when a valid product and license key are provided" do
  ...
  test "does not decrease the license uses count if the uses count is 0" do
            put :decrement_uses_count, params: { access_token: @token.token, license_key: @purchase.license.serial }.merge(@product_identifier)
            expect(response.code.to_i).to eq(200)
