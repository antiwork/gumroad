...
class ApiV2SalesControllerTest < ActionController::TestCase
  self.described_class = Api::V2::SalesController
  tests Api::V2::SalesController



  context_ Api::V2::SalesController do
    before do
      @seller = create(:user)
  ...
  test "returns a link to the next page if there are more than 10 sales" do
  ...
  test "returns the correct link to the next pages from second page onwards" do
          per_page = Api::V2::SalesController::RESULTS_PER_PAGE
  ...
  test "does not return sales outside of date range" do
  ...
  test "filters sales by email if one is specified" do
          create(:purchase, link: @product, created_at: 7.days.ago)
  ...
  test "filters sales by order_id if one is specified" do
          create(:purchase, link: @product, created_at: 3.days.ago)
  ...
  test "filters sales by customer name (case-insensitive, partial match) when name is specified" do
  ...
  test "filters sales by license key when license_key is specified" do
  ...
  test "returns a 400 error if date format is incorrect" do
  ...
  test "returns a 400 error if page number is invalid" do
  ...
  test "filters sales by product if one is specified" do
  ...
  test "returns empty result set when filtered by non-existing product ID" do
  ...
  test "returns empty result set when filtered by non-existing purchase ID" do
  ...
  test "returns a 400 error if order_id ID cannot be decrypted" do
  ...
  test "returns the correct dispute information" do
  ...
  test "the response is 403 forbidden for incorrect scope" do
          get :index, params: @params
  ...
  test "returns a sale that belongs to the seller" do
  ...
  test "includes invoice_url in the response" do
          get :show, params: @params
  ...
  test "includes license_uses in the response for a purchase with a license key" do
          @product.update!(is_licensed: true)
  ...
  test "does not return a sale that does not belong to the seller" do
  ...
  test "queues a sales export for the current seller" do
  ...
  test "queues a sales export without optional filters" do
  ...
  test "returns a 400 error if from date format is incorrect" do
  ...
  test "returns a 400 error when both dates are incorrectly formatted" do
  ...
  test "returns a 400 error if to date format is incorrect" do
  ...
  test "returns a 400 error if product ID is invalid" do
  ...
  test "returns a sales summary for the requested date range and grouping" do
  ...
  test "defaults to the last 30 days in the seller's timezone" do
  ...
  test "defaults the end date to today in the seller's timezone when only from is provided" do
  ...
  test "returns a 400 error if from is after to" do
  ...
  test "returns a 400 error if date range is too wide" do
  ...
  test "returns a 400 error if group_by is invalid" do
  ...
  context_ "PUT 'mark_as_shipped'" do
      before do
        @product = create(:product, user: @seller)
        @params = { id: @purchase.external_id }
      end

  context_ "when logged in with mark sales as shipped scope" do
        before do
  ...
  test "marks shipment as shipped" do
  ...
  test "marks shipment as shipped and includes tracking url" do
  ...
  test "does not allow you to mark someone else's sale as shipped" do
          @params.merge!(id: @purchase_by_seller.external_id)
          put :mark_as_shipped, params: @params
          expect(response.parsed_body).to eq({
            success: false,
  ...
  context_ "when logged in with view sales scope" do
        before do
          @token = create("doorkeeper/access_token", application: @app,
                                                     resource_owner_id: @seller.id,
                                                     scopes: "view_sales")
  ...
  context_ "when logged in with edit_sales scope" do
        before do
          @token = create("doorkeeper/access_token", application: @app,
                                                     resource_owner_id: @seller.id,
                                                     scopes: "edit_sales")
  ...
  context_ "when request for a full refund" do
  test "refunds a sale fully" do
  ...
  context_ "when request for a partial refund" do
  test "refunds partially if refund amount is a fraction of the sale price" do
            expect(@purchase.price_cents).to eq 100_00
            expect(@purchase.refunded?).to be false
  ...
  test "refunds fully if refund amount matches the price of the sale" do
            expect(@purchase.price_cents).to eq 100_00
            expect(@purchase.refunded?).to be false
  ...
  test "correctly processes multiple partial refunds" do
            expect(@purchase.price_cents).to eq 100_00
            expect(@purchase.refunded?).to be false
  ...
  test "does nothing if refund amount is negative" do
            expect(@purchase.refunded?).to be false
  ...
  test "does nothing if refund amount is too high" do
            expect(@purchase.price_cents).to eq 100_00
            expect(@purchase.refunded?).to be false
  ...
  test "does not divide by 100 for JPY (unit_scaling_factor is 1)" do
  ...
  test "divides by 100 for USD purchases" do
  ...
  test "does nothing if refund amount is more than the available balance" do
  ...
  test "does not refund an already refunded sale" do
  ...
  test "does not refund a chargebacked sale" do
  ...
  test "does not refund if the sale is not successful" do
  ...
  test "does not refund a free purchase" do
  ...
  test "does not allow to refund someone else's sale" do
  ...
  context_ "when logged in with refund_sales scope" do
        before do
          @token = create("doorkeeper/access_token", application: @app,
                                                     resource_owner_id: @seller.id,
                                                     scopes: "refund_sales")
  ...
  test "returns a not found error when sale does not exist" do
  ...
  test "returns a not found error when sale belongs to another user" do
  ...
  context_ "when logged in with incorrect scope" do
        before do
          @token = create("doorkeeper/access_token", application: @app, resource_owner_id: @seller.id, scopes: "view_sales")
          @params.merge!(format: :json, access_token: @token.token)
        end
