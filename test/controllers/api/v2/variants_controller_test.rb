...
class ApiV2VariantsControllerTest < ActionController::TestCase
  self.described_class = Api::V2::VariantsController
  tests Api::V2::VariantsController



  context_ Api::V2::VariantsController do
    before do
      @user = create(:user)
  ...
  test "returns error for nonexistent variant_category_id" do
  ...
  test "shows the 0 variants in that variant category" do
          get @action, params: @params
          expect(response.parsed_body["variants"]).to eq []
        end

  test "shows the 1 variant in that variant category" do
  ...
  context_ "usd" do
  test "works if variants passed in" do
  ...
  test "shows the variant in that variant category" do
          get @action, params: @params
          variant = @product.variant_categories.first.variants.first
  ...
  test "returns rich_content in response" do
            put @action, params: @params
  ...
  test "rejects variant rich content updates when shared-content mode is enabled" do
  ...
  test "updates variant with rich content pages" do
            put @action, params: @params.merge(
              rich_content: [
  ...
  test "replaces existing variant rich content" do
  ...
  test "updates existing rich content page by id" do
  ...
  test "deletes all pages when empty array is sent as JSON body" do
            create(:rich_content, entity: @variant, title: "Page 1", description: [], position: 0)
  ...
  test "deletes all pages when empty array is sent as JSON string" do
            create(:rich_content, entity: @variant, title: "Page 1", description: [], position: 0)
            put @action, params: @params.merge(rich_content: "[]")
            expect(response.parsed_body["success"]).to eq true
            @variant.reload
  ...
  test "does not include deleted pages in response after replacement" do
  ...
  test "returns rich_content in response after update" do
            put @action, params: @params.merge(
              rich_content: [
  ...
  test "does not touch rich content when rich_content param is not provided" do
  ...
  test "processes upsellCard nodes via SaveContentUpsellsService" do
  ...
  test "wires up variant.product_files from file embeds" do
  ...
  test "rejects file embeds referencing files from another product" do
  ...
  test "rejects file embeds referencing deleted files" do
  ...
  test "sets product.is_licensed when licenseKey node is added" do
  ...
  test "clears product.is_licensed and is_multiseat_license when last licenseKey is removed" do
