...
class ApiV2FilesControllerTest < ActionController::TestCase
  self.described_class = Api::V2::FilesController
  tests Api::V2::FilesController



  context_ Api::V2::FilesController do
    let(:user) { create(:user) }
    let(:app) { create(:oauth_application, owner: create(:user)) }

  context_ "POST 'presign'" do
      let(:action) { :presign }
  ...
  context_ "with a token missing edit_products scope" do
  ...
  test "includes the user external_id and filename in the key and file_url" do
          post action, params: params
          body = response.parsed_body
  ...
  test "returns 400 when file_size is missing" do
          post action, params: params.except(:file_size)
          expect(response.status).to eq(400)
          expect(response.parsed_body["error"]).to be_present
        end

  test "returns 400 when file_size is sent as a file upload" do
  ...
  test "returns 400 when S3 raises a service error" do
          allow_any_instance_of(Aws::S3::Client).to receive(:create_multipart_upload)
  ...
  test "returns 400 when file_size exceeds the 20 GB maximum" do
  ...
  context_ "with account scope" do
  ...
  test "returns success with file_url" do
          post action, params: params
          body = response.parsed_body
          expect(body["success"]).to be true
  ...
  test "returns 400 when upload_id is missing" do
          post action, params: params.except(:upload_id)
          expect(response.status).to eq(400)
        end

  test "returns 400 when key is missing" do
          post action, params: params.except(:key)
          expect(response.status).to eq(400)
        end

  test "returns 400 when parts is missing" do
          post action, params: params.except(:parts)
          expect(response.status).to eq(400)
        end

  test "returns 400 for a key scoped to a different user" do
  ...
  test "returns 400 when part_number is an array instead of a scalar" do
  ...
  test "returns 400 when etag is an array instead of a scalar" do
          post action, params: params.merge(parts: [{ part_number: 1, etag: ['"etag-abc"'] }])
          expect(response.status).to eq(400)
          expect(response.parsed_body["error"]).to eq("each part must have scalar part_number and etag values")
        end

  test "returns 400 with the S3 error message when the upload_id no longer exists" do
          allow_any_instance_of(Aws::S3::Client).to receive(:complete_multipart_upload)
  ...
  test "aborts the multipart upload and returns status=accepted" do
  ...
  test "returns 200 with status=already_gone when S3 reports NoSuchUpload" do
          allow_any_instance_of(Aws::S3::Client).to receive(:abort_multipart_upload)
            .and_raise(Aws::S3::Errors::NoSuchUpload.new(nil, "The specified upload does not exist."))
          post action, params: params
          body = response.parsed_body
  ...
  test "returns success with status=accepted" do
          post action, params: params
          expect(response.parsed_body["success"]).to be true
