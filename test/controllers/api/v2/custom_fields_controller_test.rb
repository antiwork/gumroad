...
class ApiV2CustomFieldsControllerTest < ActionController::TestCase
  self.described_class = Api::V2::CustomFieldsController
  tests Api::V2::CustomFieldsController



  context_ Api::V2::CustomFieldsController do
    before do
      @user = create(:user)
  ...
  test "returns the custom fields" do
  ...
  test "creates a new custom field with default type" do
  ...
  test "creates a new custom checkbox field with label" do
          post @action, params: @params.merge({ type: "checkbox" })
          expect(@product.custom_fields.reload.last.name).to eq(@new_custom_field_name)
          expect(@product.custom_fields.last.type).to eq("checkbox")
        end

  test "creates a new custom terms field with url" do
  ...
  test "creates a custom field on an unpublished API-created product" do
  ...
  test "returns validation errors when save fails" do
  ...
  test "returns validation errors for terms field with invalid URL" do
  ...
  test "updates the custom field" do
  ...
  test "deletes a custom field" do
  ...
  test "only deletes the association if there are multiple products attached" do
  ...
  context_ "when there are multiple of a field" do
          before do
            @product.custom_fields << create(:custom_field, name: "country")
          end

  test "deletes the last instance of the duplicated field" do
            delete @action, params: @params
            expect(@product.reload.custom_fields).to eq(@custom_fields)
          end
        end
