...
class ApiV2SubscribersControllerTest < ActionController::TestCase
  self.described_class = Api::V2::SubscribersController
  tests Api::V2::SubscribersController



  context_ Api::V2::SubscribersController do
    before do
      @seller = create(:user)
  ...
  context_ "when a subscriber does not have a user account" do
  test "filters subscribers by email if one is specified" do
  ...
  test "does not return subscribers for another user's product" do
  ...
  test "returns a link to the next page if there are more than the limit of sales" do
  ...
  test "returns a subscriber that belongs to the seller's product" do
          get @action, params: @params
          expect(response.parsed_body).to eq({
            success: true,
  ...
  test "does not return a subscriber that does not belong to the seller's product" do
