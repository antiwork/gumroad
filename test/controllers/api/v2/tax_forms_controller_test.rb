...
class ApiV2TaxFormsControllerTest < ActionController::TestCase
  self.described_class = Api::V2::TaxFormsController
  tests Api::V2::TaxFormsController



  context_ Api::V2::TaxFormsController do
  ...
  context_ "when authenticated with view_tax_data scope" do
        before do
          @params.merge!(format: :json, access_token: token.token)
        end

  test "returns 403 with empty body when token is missing the view_tax_data scope" do
  ...
  test "returns 403 when tax_center is not enabled for the seller" do
  ...
  test "returns 403 when seller is not US-based" do
  ...
  test "returns an empty list when the seller has no forms" do
          get :index, params: @params
  ...
  test "returns all forms across years when year is omitted" do
  ...
  test "returns filed_at as ISO-8601 when the form has been filed" do
  ...
  test "returns 404 when the year is outside the available range" do
  ...
  test "returns 404 when year is a non-scalar param" do
          get :index, params: @params.merge(year: ["2025"])
  ...
  test "returns 404 when the year is the current year" do
          get :index, params: @params.merge(year: 2026)

          expect(response.status).to eq(404)
        end

  test "returns 404 when the year is before the seller account-creation year" do
          get :index, params: @params.merge(year: 2021)

          expect(response.status).to eq(404)
        end

  test "does not return tax forms belonging to other sellers" do
  ...
  test "returns 403 when the token is missing view_tax_data" do
          other_token = create("doorkeeper/access_token", application: app, resource_owner_id: seller.id, scopes: "view_sales")
  ...
  test "returns 403 when tax_center is not enabled" do
          Feature.deactivate_user(:tax_center, seller)
  ...
  test "streams the Stripe PDF when the Stripe API returns one" do
  ...
  test "proxies the S3 fallback bytes when Stripe returns nil for 1099-K" do
  ...
  test "returns 404 for 1099-MISC when Stripe returns nil (no S3 fallback for MISC)" do
  ...
  test "returns 404 when neither Stripe nor S3 has the form" do
          allow_any_instance_of(StripeTaxFormsApi).to receive(:download_tax_form).and_return(nil)
  ...
  test "returns 404 when the tax form row does not exist for the seller" do
  ...
  test "returns 404 when tax_form_type is invalid" do
          get :download, params: @params.merge(tax_form_type: "us_1099_bogus")

          expect(response.status).to eq(404)
        end

  test "returns 404 when the stored stripe_account_id does not belong to the seller" do
