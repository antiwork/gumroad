...
class ApiInternalAdminBaseControllerTest < ActionController::TestCase
  self.described_class = Api::Internal::Admin::BaseController
  tests Api::Internal::Admin::BaseController



  context_ Api::Internal::Admin::BaseController do
  ...
  context_ "admin token authorization" do
  ...
  test "allows requests with the configured bearer token" do
  ...
  test "allows requests with a per-actor admin token" do
  ...
  test "rejects requests with a revoked per-actor admin token" do
        actor = create(:admin_user)
        plaintext_token = AdminApiToken.mint!(actor_user_id: actor.id)
        admin_api_token = AdminApiToken.find_by!(actor_user: actor, token_hash: AdminApiToken.hash_token(plaintext_token))
  ...
  test "rejects requests with an expired per-actor admin token" do
        actor = create(:admin_user)
  ...
  test "uses the admin token row as the rotation point" do
        request.headers["Authorization"] = "Bearer test-admin-token"
  ...
  context_ "per-actor admin token requirement" do
      let!(:legacy_admin_token) do
        create(:admin_api_token, actor_user: legacy_admin_actor, token_hash: AdminApiToken.hash_token("test-admin-token"))
      end

  test "allows per-actor admin tokens" do
        actor = create(:admin_user)
        plaintext_token = AdminApiToken.mint!(actor_user_id: actor.id)
        request.headers["Authorization"] = "Bearer #{plaintext_token}"
  ...
  test "rejects the legacy admin token" do
        request.headers["Authorization"] = "Bearer test-admin-token"
  ...
  context_ "admin write auditing" do
      let!(:legacy_admin_token) do
        create(:admin_api_token, actor_user: legacy_admin_actor, token_hash: AdminApiToken.hash_token("test-admin-token"))
      end
  ...
  test "records the actor, token, target, route, request id, and redacted params for write requests" do
        patch :update, params: {
  ...
  test "records handled client errors with their response status" do
  ...
  test "records unhandled server errors with the exception class" do
        expect do
  ...
  test "does not leave an audit row when the wrapped transaction rolls back" do
        expect do
          get :new
  ...
  test "does not fail the write request when audit logging fails" do
