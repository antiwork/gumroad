...
class ApiInternalAdminUsersControllerTest < ActionController::TestCase
  self.described_class = Api::Internal::Admin::UsersController
  tests Api::Internal::Admin::UsersController



  context_ Api::Internal::Admin::UsersController do
  ...
  context_ "user_id lookup" do
  test "looks up the user by user_id" do
  ...
  test "returns 404 when user_id does not match any user" do
  ...
  test "prefers user_id over email when both are provided" do
  ...
  test "returns 400 when only email is provided" do
        user = create(:user)
  ...
  test "rejects mismatched expected_email without mutating the user" do
        user = instance_exec(&build_user)
  ...
  test "returns not found when the user does not exist" do
  ...
  test "returns a comprehensive info payload for a compliant seller" do
  ...
  test "includes populated social handles and OAuth provider" do
        user = create(:compliant_user,
                      twitter_user_id: "1",
  ...
  test "includes blank social fields with an empty external authentications list" do
        user = create(:compliant_user,
                      twitter_user_id: nil,
                      twitter_handle: nil,
                      facebook_uid: nil,
  ...
  test "includes linked external authentications" do
        user = create(:compliant_user)
  ...
  test "orders linked external authentications by creation time" do
        user = create(:compliant_user)
  ...
  test "does not expose credential fields or token values" do
        user = create(:compliant_user)
        user.update_columns(
  ...
  test "includes populated sign-in tracking fields" do
  ...
  test "reports the suspension status and latest status timestamp for a suspended user" do
  ...
  test "reflects two-factor authentication when enabled" do
  ...
  test "surfaces the country from the alive user compliance info" do
  ...
  test "reports admin-paused payouts with the latest pause comment as the reason" do
  ...
  test "reports system-paused payouts without exposing a paused_for_reason" do
  ...
  test "reports paused_by_user when the seller has self-paused via Settings" do
  ...
  test "surfaces a deactivated user with a populated deleted_at" do
  ...
  test "looks up a soft-deleted user by user_id" do
  ...
  test "uses the latest risk-state comment for last_status_changed_at, including on_probation transitions" do
  ...
  test "computes sales_count and total_earnings_formatted from the seller's successful sales" do
  ...
  test "includes the active watched user" do
  ...
  test "returns bad request when direction is missing" do
        user = create(:user)
  ...
  test "returns bad request when direction is invalid" do
        user = create(:user)
  ...
  test "returns bad request when neither user_id nor email is provided" do
  ...
  test "returns an empty list with cursor pagination metadata" do
        user = create(:user)
  ...
  test "looks up soft-deleted users" do
  ...
  test "does not write an admin audit log" do
        user = create(:user)
  ...
  test "lists granted direct affiliates and collaborators with affiliate users as counterparties" do
  ...
  test "lists received affiliates with sellers as counterparties" do
  ...
  test "excludes global affiliates" do
        user = create(:user)
  ...
  test "includes soft-deleted affiliates" do
        seller = create(:user)
  ...
  test "falls back to the parent basis points when a product row has no override" do
        seller = create(:user)
        affiliate_user = create(:user)
        product = create(:product, user: seller)
  ...
  test "uses product basis points when a product row has an override" do
        seller = create(:user)
        affiliate_user = create(:user)
        product = create(:product, user: seller)
  ...
  test "reflects apply_to_all_products without expanding blanket relationships into products" do
        seller = create(:user)
        affiliate_user = create(:user)
  ...
  test "paginates affiliates with a cursor" do
        seller = create(:user)
  ...
  test "returns bad request when the cursor is invalid" do
        user = create(:user)
  ...
  test "applies a granted cursor to the received scope without binding the cursor to the direction" do
        user = create(:user)
  ...
  test "scopes results to the requested user" do
        seller = create(:user)
  ...
  test "preloads received affiliate products and sellers instead of issuing one query per affiliate" do
        affiliate_user = create(:user)
        3.times do
          seller = create(:user)
  ...
  test "lists comments for the user ordered by created_at and id descending" do
        user = create(:user)
  ...
  test "falls back to the author name and then System when the snapshot is blank" do
        user = create(:user)
  ...
  test "filters to a single comment_type" do
        user = create(:user)
  ...
  test "filters to multiple comment types from a comma-separated list" do
        user = create(:user)
  ...
  test "rejects an unknown comment_type value" do
        user = create(:user)
  ...
  test "rejects a comma-separated comment_type list with any invalid value" do
        user = create(:user)
  ...
  test "rejects a comma-only comment_type value instead of silently returning all comments" do
        user = create(:user)
  ...
  test "includes soft-deleted comments with deletion state exposed" do
        user = create(:user)
  ...
  test "scopes results to the requested user and excludes comments on other records" do
        user = create(:user)
  ...
  test "paginates comments with a cursor" do
        user = create(:user)
  ...
  test "preloads authors instead of issuing one query per comment" do
        user = create(:user)
        3.times do |index|
  ...
  context_ "GET compliance_info" do
  ...
  test "returns null compliance_info and an empty info_requests list when the user has never submitted KYC" do
        user = create(:user)
  ...
  test "returns submitted KYC fields for an individual seller with the tax ID masked to last four" do
        user = create(:user)
  ...
  test "returns business KYC fields with the business address and business tax ID last four" do
        user = create(:user)
  ...
  test "returns the latest alive compliance info when older records are soft-deleted" do
        user = create(:user)
  ...
  test "returns the trailing characters of an alphanumeric non-US business_tax_id verbatim" do
        user = create(:user)
  ...
  test "omits last_four for tax IDs that were never set" do
        user = create(:user)
  ...
  test "lists only requested info_requests and excludes provided ones, marking overdue rows" do
        user = create(:user)
  ...
  test "exposes the most recent email reminder timestamp when present" do
        user = create(:user)
  ...
  test "scopes info_requests to the requested user" do
        user = create(:user)
  ...
  test "returns empty stats and cursor pagination metadata" do
        user = create(:user)
  ...
  test "returns seller Radar aggregates with cursor-paginated recent EFWs" do
        seller = create(:user)
        product = create(:product, user: seller)
  ...
  test "returns purchases matched by purchaser_id and by email, newest first" do
  ...
  test "serializes each row with the shared purchase payload" do
        buyer = create(:user, email: "buyer@example.com")
        seller = create(:user, email: "seller@example.com")
  ...
  test "serializes nil seller fields when a purchase has no seller" do
        buyer = create(:user, email: "buyer@example.com")
  ...
  test "filters by a single purchase_state" do
        buyer = create(:user)
  ...
  test "filters by multiple purchase states from a comma-separated list" do
        buyer = create(:user)
  ...
  test "rejects a comma-only status value instead of silently returning an empty list" do
        buyer = create(:user)
  ...
  test "accepts every state defined on the Purchase state machine, including preorder_concluded_unsuccessfully" do
        buyer = create(:user)
  ...
  test "filters by a created_at window using ISO 8601 timestamps" do
        buyer = create(:user)
  ...
  test "rejects an unparseable start_at" do
        user = create(:user)
  ...
  test "rejects an unparseable end_at" do
        user = create(:user)
  ...
  test "filters by chargedback=true" do
        buyer = create(:user)
  ...
  test "filters by chargedback=false" do
        buyer = create(:user)
        clean = create(:purchase, purchaser: buyer)
  ...
  test "rejects an empty chargedback value instead of silently filtering to chargedback=false" do
        user = create(:user)
  ...
  test "rejects an empty has_early_fraud_warning value" do
        user = create(:user)
  ...
  test "rejects an unparseable has_affiliate value" do
        user = create(:user)
  ...
  test "filters by has_early_fraud_warning=true" do
        buyer = create(:user)
  ...
  test "filters by has_early_fraud_warning=false" do
        buyer = create(:user)
        flagged = create(:purchase, purchaser: buyer)
        create(:early_fraud_warning, purchase: flagged)
        clean = create(:purchase, purchaser: buyer)
  ...
  test "filters by has_affiliate=true" do
        buyer = create(:user)
        seller = create(:user)
        product = create(:product, user: seller)
  ...
  test "filters by stripe_fingerprint" do
        buyer = create(:user)
  ...
  test "skips the email branch when the user has no email so NULL-email purchases are not leaked" do
  ...
  test "paginates purchases with a cursor" do
        buyer = create(:user)
  ...
  test "rejects unknown signal values" do
        user = create(:user)
  ...
  test "rejects a comma-only signals value" do
        user = create(:user)
  ...
  test "returns the full default-signals response" do
        target = create(:user,
                        account_created_ip: "1.2.3.4",
                        current_sign_in_ip: nil,
                        last_sign_in_ip: nil,
  ...
  test "scopes related users to the requested signals" do
  ...
  test "applies the per-signal limit and reports truncation" do
  ...
  test "includes each related user's risk state" do
        target = create(:user, account_created_ip: "1.2.3.4", current_sign_in_ip: nil, last_sign_in_ip: nil, payment_address: nil)
  ...
  test "keeps related user enrichment queries bounded" do
        target = create(:user,
                        account_created_ip: "1.2.3.4",
                        current_sign_in_ip: nil,
                        last_sign_in_ip: nil,
  ...
  context_ "GET suspension" do
      include_examples "admin api authorization required", :get, :suspension

  test "returns compliant status for an unsuspended user" do
  ...
  test "returns suspended status with the latest status comment timestamp" do
        user = create(:tos_user, email: "suspended@example.com")
        comment = create(:comment, commentable: user, comment_type: Comment::COMMENT_TYPE_SUSPENDED, created_at: 2.days.ago)
  ...
  test "returns not found when user_id matches a soft-deleted user" do
        user = create(:compliant_user)
        user.mark_deleted!
  ...
  context_ "POST reset_password" do
      let(:user) { create(:user) }

      include_examples "admin api authorization required", :post, :reset_password

  test "returns 400 when user_id is missing" do
        post :reset_password
  ...
  test "returns 404 when the user does not exist" do
  ...
  test "sends reset password instructions and returns success" do
  ...
  context_ "with a user lookup helper stubbed" do
  ...
  test "returns 400 when new_email is missing" do
  ...
  test "returns 400 when new_email is malformed" do
  ...
  test "returns 404 when user_id does not match a user" do
  ...
  test "updates the email and returns the pending confirmation state" do
  ...
  test "returns 422 when the new email collides with an existing user" do
        other_user = create(:user)
  ...
  test "returns 422 when the new email matches the current email" do
  ...
  test "rejects same-email submissions case-insensitively" do
  ...
  test "accepts expected_email when it matches the target user" do
  ...
  test "returns 400 when only current_email is provided" do
  ...
  test "returns 404 when user_id matches a soft-deleted user" do
        user.mark_deleted!
  ...
  test "uses user_id as the lookup key when current_email is also provided" do
        other_user = create(:user)
  ...
  context_ "POST two_factor_authentication" do
      let(:user) { create(:user) }
  ...
  test "returns 400 when enabled is missing" do
  ...
  test "returns 400 when enabled is an empty string and does not modify the user" do
        user.update!(two_factor_authentication_enabled: true)
  ...
  test "treats Ruby false as a valid disable request" do
        user.update!(two_factor_authentication_enabled: true)
  ...
  test "enables two-factor authentication" do
  ...
  test "disables two-factor authentication and destroys the totp credential" do
        user.update!(two_factor_authentication_enabled: true)
        totp_credential = TotpCredential.create!(user: user)
  ...
  context_ "POST create_comment" do
      let(:user) { create(:user) }
  ...
  test "creates a comment attributed to GUMROAD_ADMIN_ID" do
        expect do
  ...
  test "creates comments and audit rows attributed to a per-actor token" do
  ...
  test "returns the existing comment when called twice with the same key and matching content" do
  ...
  test "returns 409 conflict when the same key is reused with different content" do
  ...
  test "returns 422 when the content fails validation" do
  ...
  context_ "POST mark_compliant" do
  ...
  test "marks the user compliant and creates separate audit and note comments attributed to GUMROAD_ADMIN_ID" do
        expect do
  ...
  test "returns 422 without marking the user compliant when the note is invalid" do
        expect do
  ...
  test "keeps the existing sibling-account compliant side effect" do
  ...
  test "returns success without creating another comment when the user is already compliant" do
  ...
  context_ "POST suspend_for_fraud" do
  ...
  test "suspends the user for fraud and creates an audit comment attributed to GUMROAD_ADMIN_ID" do
        expect do
  ...
  test "creates an extra suspension note when one is provided" do
        expect do
  ...
  test "returns 422 without suspending the user when the suspension note is invalid" do
        expect do
  ...
  test "returns success without creating another comment when the user is already suspended" do
  ...
  test "returns 422 when the user is suspended for a different reason" do
  ...
  test "returns 422 when the state machine rejects the suspension" do
  ...
  context_ "POST watch" do
      let(:user) { create(:user) }

      include_examples "admin api authorization required", :post, :watch

  test "returns bad request when user_id is missing" do
  ...
  test "returns bad request when revenue_threshold is missing" do
  ...
  test "returns bad request when revenue_threshold is not positive" do
  ...
  test "returns bad request when revenue_threshold is non-finite" do
  ...
  test "creates a watched user record" do
        expect do
  ...
  test "returns 422 when user is already being watched" do
  ...
  test "removes the user from the watchlist" do
  ...
  test "returns 422 when user is not being watched" do
        post :unwatch, params: { user_id: user.external_id }
  ...
  test "updates the active watched user" do
  ...
  test "preserves notes when notes is omitted" do
  ...
  test "clears notes when notes is blank" do
