...
class ApiInternalAdminProductsControllerTest < ActionController::TestCase
  self.described_class = Api::Internal::Admin::ProductsController
  tests Api::Internal::Admin::ProductsController



  context_ Api::Internal::Admin::ProductsController do
  ...
  test "returns a bad request when neither email nor external_id is provided" do
        get :index
  ...
  test "looks up the seller by external_id when provided" do
  ...
  test "returns not found when the external_id does not match any user" do
  ...
  test "prefers external_id over email when both are provided" do
  ...
  test "returns products for a soft-deleted seller looked up by external_id" do
        product = create(:product, user: seller)
        seller.mark_deleted!
  ...
  test "returns an empty list with pagination metadata when the seller has no products" do
  ...
  test "returns alive and soft-deleted products with their deletion state exposed" do
  ...
  test "orders alive products before deleted ones, then by created_at desc" do
  ...
  test "surfaces external-link files with their URL and a URL extension" do
        product = create(:product, user: seller)
  ...
  test "preloads product files instead of issuing one query per product" do
        3.times do
          product = create(:product, user: seller)
  ...
  test "resolves taxonomy ancestry with a single batched query across distinct taxonomies" do
  ...
  test "exposes file metadata including soft-deleted files" do
        product = create(:product, user: seller)
  ...
  test "returns the cover image url when one is present" do
  ...
  test "paginates with the default per_page" do
  ...
  test "treats non-positive or non-numeric page as page 1 (rather than raising 500)" do
        product = create(:product, user: seller)
  ...
  test "returns an empty page in the JSON envelope when page is past the end (rather than raising 500)" do
  ...
  test "orders product files with NULL position first to match MySQL ORDER BY ASC" do
        product = create(:product, user: seller)
  ...
  test "honors per_page and caps it at the maximum" do
  ...
  test "scopes results to the requested seller and excludes other sellers' products" do
        other_seller = create(:user, email: "other@example.com")
  ...
  test "returns not found when no product matches" do
  ...
  test "returns the full product payload with file metadata" do
  ...
  test "returns a soft-deleted product so admins can inspect tombstones" do
        product = create(:product, user: seller)
  ...
  test "includes soft-deleted files with deleted_at populated" do
        product = create(:product, user: seller)
  ...
  test "exposes banned_at and purchase_disabled_at when set, nil when not" do
  ...
  test "returns the bad-card counter value as stored on the product" do
        product = create(:product, user: seller)
  ...
  test "returns the taxonomy slug and ancestry path when assigned" do
  ...
  test "returns null taxonomy when none is assigned" do
  ...
  test "lists attached direct affiliates with the fallback basis points from the parent affiliate" do
  ...
  test "surfaces soft-deleted affiliates with their lifecycle state so reviewers see recently removed ones" do
        product = create(:product, user: seller)
  ...
  test "lists collaborators with per-product basis-point overrides" do
  ...
  test "excludes global affiliates from the attached affiliates list" do
        product = create(:product, user: seller)
        direct_user = create(:user)
        direct = create(:direct_affiliate, seller:, affiliate_user: direct_user, products: [product])
  ...
  test "returns an empty affiliates list when none are attached" do
        product = create(:product, user: seller)
  ...
  test "computes recent_chargeback_rate over a 90 day window from successful sales" do
  ...
  test "excludes reversed chargebacks from recent_chargeback_rate, matching the rest of the fraud-signal stack" do
        create(:merchant_account, user: nil)
        product = create(:product, user: seller)
        4.times { create(:purchase, link: product, seller:, created_at: 10.days.ago) }
  ...
  test "excludes bundle sub-purchases from both successful_count and chargedback_count" do
        create(:merchant_account, user: nil)
        product = create(:product, user: seller)
  ...
  test "returns a recent_chargeback_rate with nil rate when there are no recent successful purchases" do
        product = create(:product, user: seller)
  ...
  test "omits recent_chargeback_rate from index rows to keep the listing cheap" do
        create(:merchant_account, user: nil)
        product = create(:product, user: seller)
