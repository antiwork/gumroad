...
class ApiInternalAdminPayoutsControllerTest < ActionController::TestCase
  self.described_class = Api::Internal::Admin::PayoutsController
  tests Api::Internal::Admin::PayoutsController



  context_ Api::Internal::Admin::PayoutsController do
  ...
  test "rejects mismatched expected_email without mutating payouts" do
  ...
  test "returns the user's recent payouts and next payout information" do
  ...
  test "paginates recent payouts with a cursor" do
  ...
  test "scopes recent payouts to the requested user" do
  ...
  test "returns a bad request when email and user_id are missing" do
        get :index
  ...
  test "lists payouts by user_id" do
        get :index, params: { user_id: user.external_id }
  ...
  test "pauses payouts and records the admin as the pause source" do
  ...
  test "creates a COMMENT_TYPE_PAYOUTS_PAUSED comment when reason is provided" do
  ...
  test "records the legacy token in the audit log" do
  ...
  test "attributes payout comments and audit rows to a per-actor token" do
  ...
  test "does not create a comment when reason is blank" do
  ...
  test "short-circuits when payouts are already paused by admin" do
  ...
  test "asserts admin attribution when payouts were previously paused by the system" do
  ...
  test "asserts admin attribution when payouts were previously paused by Stripe" do
  ...
  test "resumes payouts, clears payouts_paused_by, and records a resume comment" do
        user.update!(payouts_paused_internally: true, payouts_paused_by: GUMROAD_ADMIN_ID)
  ...
  test "reports payouts_paused: true after admin resume when the seller is still self-paused" do
  ...
  test "short-circuits when payouts are not paused by admin" do
        expect { post :resume, params: { user_id: user.external_id } }
          .not_to change { user.comments.count }
  ...
  test "reports payouts_paused: true on short-circuit when the seller has self-paused" do
  ...
  test "returns 400 when payout_processor is invalid" do
  ...
  test "returns 400 when payout_period_end_date is missing" do
  ...
  test "returns 400 when payout_period_end_date is invalid" do
  ...
  test "returns 400 without writing an audit row when payout_period_end_date is today or in the future" do
  ...
  test "issues a stripe payout via Payouts.create_payments_for_balances_up_to_date_for_users" do
  ...
  test "sets should_paypal_payout_be_split when paypal split is requested" do
  ...
  test "returns 422 when no payment is created" do
        date = 1.day.ago.to_date
  ...
  test "returns 422 when the payment failed" do
  ...
  test "writes an admin audit log" do
        payment = create(:payment_completed, user:)
        date = 1.day.ago.to_date
        allow(Payouts).to receive(:create_payments_for_balances_up_to_date_for_users).and_return([[payment]])
