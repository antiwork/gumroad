...
class ApiInternalAdminScheduledPayoutsControllerTest < ActionController::TestCase
  self.described_class = Api::Internal::Admin::ScheduledPayoutsController
  tests Api::Internal::Admin::ScheduledPayoutsController



  context_ Api::Internal::Admin::ScheduledPayoutsController do
  ...
  test "returns scheduled payouts ordered by id desc" do
  ...
  test "returns 400 when status is invalid" do
  ...
  test "caps the limit at MAX_LIMIT" do
  ...
  test "uses the default limit when limit is missing or non-positive" do
        get :index
  ...
  test "filters by email when provided" do
        mine = create(:scheduled_payout, user:)
        create(:scheduled_payout, user: create(:user))
  ...
  test "returns 404 when the requested user does not exist" do
  ...
  test "filters by an array of statuses" do
  ...
  test "returns 400 when any status in the array is invalid" do
  ...
  test "combines user and status filters" do
  ...
  test "includes enrichment keys for each scheduled payout" do
  ...
  test "reports alive product count" do
  ...
  test "reports incoming affiliate count without global or deleted affiliates" do
        seller = create(:compliant_user)
  ...
  test "reports top categories by alive product count" do
        seller = create(:compliant_user)
  ...
  test "reports unpaid balance" do
        seller = create(:compliant_user)
  ...
  test "keeps scheduled payout list enrichment queries bounded" do
  ...
  test "returns 404 when the scheduled payout is not found" do
  ...
  test "promotes a flagged scheduled payout to pending before executing" do
  ...
  test "returns 422 when the scheduled payout is already executed" do
  ...
  test "returns 422 and the error message when execute! raises" do
        scheduled_payout = create(:scheduled_payout, user:)
  ...
  test "writes an admin audit log targeting the scheduled payout" do
        scheduled_payout = create(:scheduled_payout, user:)
        allow_any_instance_of(ScheduledPayout).to receive(:execute!).and_return(:executed)
  ...
  test "returns 422 and the error message when cancel! raises" do
        scheduled_payout = create(:scheduled_payout, user:)
