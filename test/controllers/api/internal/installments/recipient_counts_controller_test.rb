...
class ApiInternalInstallmentsRecipientCountsControllerTest < ActionController::TestCase
  self.described_class = Api::Internal::Installments::RecipientCountsController
  tests Api::Internal::Installments::RecipientCountsController



  context_ Api::Internal::Installments::RecipientCountsController do
  ...
  test "returns counts for audience installment type with created_before filter" do
        get :show, params: { installment_type: "audience", created_before: 5.days.ago.to_s }
        expect(response).to be_successful
        expect(response.parsed_body).to eq("recipient_count" => 3, "audience_count" => 5)
      end

  test "ignores out-of-range year in created_before filter without raising a database error" do
        get :show, params: { installment_type: "audience", created_before: "252026-05-06" }
        expect(response).to be_successful
        expect(response.parsed_body).to eq("recipient_count" => 5, "audience_count" => 5)
      end

  test "ignores out-of-range year in created_after filter without raising a database error" do
        get :show, params: { installment_type: "audience", created_after: "252026-05-06" }
        expect(response).to be_successful
        expect(response.parsed_body).to eq("recipient_count" => 5, "audience_count" => 5)
      end

  test "ignores unparseable created_before filter without raising" do
        get :show, params: { installment_type: "audience", created_before: "not-a-date" }
        expect(response).to be_successful
        expect(response.parsed_body).to eq("recipient_count" => 5, "audience_count" => 5)
      end

  test "returns counts for seller installment type with paid_more_than_cents filter" do
  ...
  test "returns counts for seller installment type with paid_less_than_cents filter" do
  ...
  test "returns counts for seller installment type with bought_products filter" do
        get :show, params: { installment_type: "seller", bought_products: [product2.unique_permalink] }
        expect(response).to be_successful
        expect(response.parsed_body).to eq("recipient_count" => 2, "audience_count" => 5)
      end

  test "returns counts for audience installment type with not_bought_products filter" do
        get :show, params: { installment_type: "audience", not_bought_products: [product2.unique_permalink] }
        expect(response).to be_successful
        expect(response.parsed_body).to eq("recipient_count" => 3, "audience_count" => 5)
      end

  test "returns counts for seller installment type with bought_variants filter" do
  ...
  test "returns counts for audience installment type with not_bought_variants filter" do
        get :show, params: { installment_type: "audience", not_bought_variants: [product1_variant.external_id] }
        expect(response).to be_successful
        expect(response.parsed_body).to eq("recipient_count" => 4, "audience_count" => 5)
      end

  test "returns counts for seller installment type with bought_from filter" do
