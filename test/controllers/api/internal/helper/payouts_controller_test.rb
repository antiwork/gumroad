...
class ApiInternalHelperPayoutsControllerTest < ActionController::TestCase
  self.described_class = Api::Internal::Helper::PayoutsController
  tests Api::Internal::Helper::PayoutsController



  context_ Api::Internal::Helper::PayoutsController do
  ...
  test "returns last 5 payouts and next payout information" do
  ...
  test "returns nil for payout_note when no note exists" do
          get :index, params: { email: user.email }
  ...
  context_ "when user exists" do
  context_ "when last successful payout was less than a week ago" do
          before do
  ...
  context_ "when user is not compliant" do
  ...
  test "returns error message", :vcr do
            post :create, params: params
            expect(response.status).to eq(422)
            parsed_response = JSON.parse(response.body)
            expect(parsed_response["success"]).to be(false)
  ...
  context_ "when last successful payout was more than a week ago" do
          before do
            create(:payment_completed, user:, created_at: 8.days.ago)
            create(:balance, user:, date: 10.days.ago, amount_cents: 200_00)
          end

  context_ "when user is payable" do
            before do
              create(:ach_account_stripe_succeed, user:)
              create(:merchant_account_stripe, user:)
            end

  context_ "when payout creation succeeds", :vcr do
  test "creates a new payout and returns payout information" do
                post :create, params: params
                expect(response.status).to eq(200)
                parsed_response = JSON.parse(response.body)
                expect(parsed_response["success"]).to be(true)
  ...
  context_ "when payout creation fails", :vcr do
              before do
  ...
  context_ "when user is not payable", :vcr do
            before do
  ...
  context_ "when user is payable via PayPal", :vcr do
  test "creates a new payout via PayPal and returns payout information" do
  ...
  context_ "when user does not have any payment method", :vcr do
  test "returns error message" do
