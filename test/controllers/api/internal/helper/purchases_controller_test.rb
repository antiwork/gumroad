...
class ApiInternalHelperPurchasesControllerTest < ActionController::TestCase
  self.described_class = Api::Internal::Helper::PurchasesController
  self.rspec_metadata = { vcr: true }
  tests Api::Internal::Helper::PurchasesController



  context_ Api::Internal::Helper::PurchasesController, :vcr do
  ...
  context_ "POST reassign_purchases" do
  ...
  context_ "when both emails are provided" do
  test "reassigns purchases and updates purchaser_id when target user exists" do
  ...
  test "sends grouped receipt to new email" do
  ...
  test "updates original_purchase email for subscription purchases" do
          subscription = create(:subscription, user: buyer)
  ...
  test "updates the original purchase once only when there are multiple recurring purchases sharing the same original purchase" do
  ...
  test "reassigns purchases and sets purchaser_id to nil when target user doesn't exist" do
          subscription = create(:subscription, user: buyer)
          subscription_purchase = create(:purchase, email: from_email, purchaser: buyer, is_original_subscription_purchase: true, subscription: subscription, merchant_account: merchant_account)
  ...
  context_ "when parameters are missing" do
  test "returns an error when 'from' email is missing" do
  ...
  test "returns an error when 'to' email is missing" do
  ...
  context_ "when no purchases are found" do
  ...
  context_ "POST refund_last_purchase" do
  ...
  context_ "when the last purchase can be refunded" do
  test "refunds the purchase" do
          expect do
  ...
  context_ "when the purchase cannot be refunded" do
        before do
  ...
  context_ "POST resend_last_receipt" do
  ...
  test "resends the last purchase's receipt" do
  ...
  context_ "POST resend_receipt_by_number" do
  ...
  test "resends the receipt when purchase is found" do
  ...
  test "returns 404 when purchase is not found" do
  ...
  test "returns error if no parameters are provided" do
        post :search, params: {}
  ...
  test "returns purchase data if found" do
  ...
  test "does not return purchase data if no purchase is found" do
  ...
  context_ "when searching by paypal email" do
  test "returns purchase data if found" do
  ...
  context_ "when searching by card's last 4" do
  test "returns purchase data if found" do
  ...
  context_ "when purchase_date is invalid" do
  ...
  context_ "when searching by charge amount" do
  test "returns purchase data if found" do
  ...
  context_ "with refunded purchases" do
  test "returns fully refunded purchase data" do
  ...
  test "returns partially refunded purchase data" do
  ...
  test "returns purchase data when searching by order ID (external_id) via query param" do
  ...
  test "returns purchase data when searching by order ID and email together" do
  ...
  test "returns not found when order ID doesn't match the customer email" do
        purchase = create(:free_purchase, email: "customer@example.com")
  ...
  context_ "POST auto_refund_purchase" do
  ...
  context_ "when the purchase exists and email matches" do
  test "processes the refund when within policy timeframe and no fine print" do
  ...
  test "returns an error when outside refund policy timeframe" do
  ...
  test "returns an error when fine print exists" do
          allow(purchase).to receive(:within_refund_policy_timeframe?).and_return(true)
  ...
  test "returns an error if the refund fails" do
          allow(purchase).to receive(:within_refund_policy_timeframe?).and_return(true)
  ...
  context_ "when the purchase does not exist or email doesn't match" do
  test "returns a not found error when purchase doesn't exist" do
  ...
  test "returns a not found error when email doesn't match" do
  ...
  context_ "POST refund_taxes_only" do
  ...
  context_ "when the purchase exists and email matches" do
  test "successfully refunds taxes when refundable taxes are available" do
  ...
  test "includes note and business_vat_id when provided" do
  ...
  test "returns an error when no refundable taxes are available" do
          allow(purchase).to receive(:refund_gumroad_taxes!).with(
            refunding_user_id: admin_user.id,
            note: nil,
            business_vat_id: nil
  ...
  test "returns specific error message when purchase has validation errors" do
  ...
  test "returns a not found error when email doesn't match (case insensitive)" do
          mismatched_purchase = instance_double(Purchase, email: "different@example.com")
          allow(Purchase).to receive(:find_by_external_id_numeric).with(54321).and_return(mismatched_purchase)
  ...
  test "handles email matching case insensitively" do
  ...
  context_ "when required parameters are missing" do
  test "handles missing purchase_id gracefully" do
