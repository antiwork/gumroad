...
class ProductsAvailableOfferCodesControllerTest < ActionController::TestCase
  self.described_class = Products::AvailableOfferCodesController
  tests Products::AvailableOfferCodesController



  context_ Products::AvailableOfferCodesController do
  ...
  context_ "with product offer codes" do
  ...
  test "returns product and universal offer codes" do
  ...
  test "returns offer codes with correct structure" do
            get :index, format: :json, params: { product_id: product.unique_permalink }
  ...
  test "returns configured discounts for existing-customer offer codes" do
  ...
  test "returns only matching offer codes" do
  ...
  context_ "with no offer codes" do
  ...
  context_ "with MAX_OFFER_CODES_LIMIT" do
          before do
  ...
  test "enforces the limit when query is present" do
  ...
  context_ "with INITIAL_OFFER_CODES_LIMIT" do
          before do
  ...
  test "enforces the initial limit when no query is present" do
            get :index, format: :json, params: { product_id: product.unique_permalink }
  ...
  test "includes universal offer codes with matching currency" do
            get :index, format: :json, params: { product_id: product.unique_permalink }
  ...
  test "returns offer codes ordered by newest first when no query is provided" do
            get :index, format: :json, params: { product_id: product.unique_permalink }
  ...
  test "returns offer codes ordered by newest first when query is provided" do
            get :index, format: :json, params: { product_id: product.unique_permalink, query: "Code" }
  ...
  test "returns universal offer codes ordered by newest first when query matches universal codes" do
  ...
  context_ "for an invalid product ID" do
  test "returns 404" do
          expect do
