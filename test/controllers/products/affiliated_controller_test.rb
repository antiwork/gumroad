...
class ProductsAffiliatedControllerTest < ActionController::TestCase
  self.described_class = Products::AffiliatedController
  tests Products::AffiliatedController



  context_ Products::AffiliatedController, inertia: true do
  ...
  test "renders affiliated products and stats" do
        get :index
  ...
  context_ "pagination" do
        before { stub_const("AffiliatedProductsPresenter::PER_PAGE", 1) }

  test "returns paginated affiliate products" do
  ...
  test "returns an empty list if query does not match" do
  ...
  test "sorts affiliated products by #{sort_key}" do
