...
class WorkflowsEmailsControllerTest < ActionController::TestCase
  self.described_class = Workflows::EmailsController
  tests Workflows::EmailsController



  context_ Workflows::EmailsController, type: :controller, inertia: true do
  ...
  test "303 redirects to emails page with a success message" do
        installment_params = [{
          id: "test-id",
          name: "Test Email",
  ...
  test "redirects with published message when save_action_name is save_and_publish" do
  ...
  test "redirects with unpublished message when save_action_name is save_and_unpublish" do
        allow_any_instance_of(Workflow::SaveInstallmentsService).to receive(:process).and_return([true, nil])
  ...
  test "redirects with Inertia errors and specific alert message" do
          patch :update, params: {
            workflow_id: workflow.external_id,
            workflow: {
              send_to_past_customers: true,
