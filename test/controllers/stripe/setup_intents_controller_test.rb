...
class StripeSetupIntentsControllerTest < ActionController::TestCase
  self.described_class = Stripe::SetupIntentsController
  self.rspec_metadata = { vcr: true }
  tests Stripe::SetupIntentsController



  context_ Stripe::SetupIntentsController, :vcr do
  context_ "POST create" do
  context_ "when card params are invalid" do
  ...
  context_ "when card handling error occurred" do
  test "responds with an error" do
  ...
  context_ "when card params are valid" do
        let(:card_with_sca) { StripePaymentMethodHelper.success_indian_card_mandate }

  test "creates a Stripe customer and sets up future usage" do
  ...
  test "uses the logged in user when deriving a subscription update mandate amount" do
  ...
  context_ "when setup intent succeeds" do
  test "renders a successful response" do
  ...
  context_ "when setup intent requires action" do
  test "renders a successful response" do
            post :create, params: StripePaymentMethodHelper.success_with_sca.to_stripejs_params
  ...
  context_ "when charge processor error occurs" do
          before do
