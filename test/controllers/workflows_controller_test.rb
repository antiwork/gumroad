...
class WorkflowsControllerTest < ActionController::TestCase
  self.described_class = WorkflowsController
  tests WorkflowsController



  context_ WorkflowsController, type: :controller, inertia: true do
  ...
  test "renders successfully with Inertia" do
        get :index
  ...
  context_ "when workflow doesn't exist" do
  test "returns 404" do
  ...
  test "303 redirects to workflow emails page with a success message" do
  ...
  test "redirects back to new workflow page with errors when service fails" do
  ...
  test "handles validation errors from the service" do
  ...
  context_ "with abandoned cart workflow" do
  test "returns error when seller is not eligible for abandoned cart workflows" do
  ...
  test "redirects to workflow emails page with publish message when save_and_publish" do
  ...
  test "redirects to workflow emails page with unpublish message when save_and_unpublish" do
  ...
  context_ "with invalid params" do
  test "redirects back to edit workflow page with errors when service fails" do
          allow_any_instance_of(Workflow::ManageService).to receive(:process).and_return([false, "Name can't be blank"])
  ...
  test "handles ActiveRecord::RecordInvalid errors" do
  ...
  test "303 redirects to workflows page with a success message" do
