...
class OauthAccessTokensControllerTest < ActionController::TestCase
  self.described_class = Oauth::AccessTokensController
  tests Oauth::AccessTokensController



  context_ Oauth::AccessTokensController do
  ...
  context_ "when user owns the application" do
        before do
  ...
  context_ "when user does not own the application" do
        before do
          @oauth_application = create(:oauth_application)
        end

  test "returns 401 Unauthorized" do
          expect do
            post :create, params: { application_id: @oauth_application.external_id }, session: { format: :json }
  ...
  test "does not create an access token" do
          expect do
            post :create, params: { application_id: @oauth_application.external_id }, session: { format: :json }
          end.not_to change { Doorkeeper::AccessToken.count }
