...
class ThirdPartyAnalyticsControllerTest < ActionController::TestCase
  self.described_class = ThirdPartyAnalyticsController
  tests ThirdPartyAnalyticsController



  context_ ThirdPartyAnalyticsController do
    render_views
  ...
  context_ "index" do
  context_ "when location is product" do
  test "includes all applicable snippets" do
          ...
          expect(response.body).not_to include @receipt_user_snippet.analytics_code
          expect(response.body).not_to include @receipt_product_snippet.analytics_code
        end
      end

  context_ "when location is receipt" do
  test "includes all applicable snippets" do
          ...
          expect(response.body).to include @global_user_snippet.analytics_code
          expect(response.body).to include @global_product_snippet.analytics_code
          expect(response.body).not_to include @product_product_snippet.analytics_code
          expect(response.body).not_to include @product_user_snippet.analytics_code
        end
      end

  test "successfully returns replaced analytics code" do
  ...
  test "successfully returns analytics code with order id" do
  ...
  test "doesn't return any analytics code if none exist for product or user" do
        ...
        expect(response.body).not_to include @product_product_snippet.analytics_code
        expect(response.body).not_to include @product_user_snippet.analytics_code
        expect(response.body).not_to include @global_user_snippet.analytics_code
        expect(response.body).not_to include @global_product_snippet.analytics_code
        expect(response.body).not_to include @receipt_user_snippet.analytics_code
        expect(response.body).not_to include @receipt_product_snippet.analytics_code
      end

  test "raises an e404 if the purchase does not belong to the product" do
  ...
  test "raises an e404 if the purchase does not exist" do
        expect { get :index, params: { link_id: @product.unique_permalink, purchase_id: "@purchase.external_id" } }.to raise_error(ActionController::RoutingError)
      end
    end
  end
