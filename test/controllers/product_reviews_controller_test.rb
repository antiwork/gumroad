...
class ProductReviewsControllerTest < ActionController::TestCase
  self.described_class = ProductReviewsController
  tests ProductReviewsController



  context_ ProductReviewsController do
  ...
  test "creates a product review" do
  ...
  test "updates an existing review" do
  ...
  test "allows saving the same rating" do
          put :set, params: valid_params
          expect(response.parsed_body["success"]).to eq(true)
  ...
  test "allows creating and destroying a video review" do
  ...
  test "rejects invalid email digest" do
  ...
  test "rejects missing email digest" do
  ...
  test "rejects mismatched purchase and product" do
  ...
  test "rejects ineligible purchases" do
  ...
  context_ "when purchaser is suspended" do
  test "rejects review creation from a suspended user" do
  ...
  test "rejects review edits from a suspended user" do
          review = create(:product_review, purchase: purchase, rating: 3)
  ...
  test "allows review creation after user is unsuspended" do
          purchaser.update!(user_risk_state: "suspended_for_fraud")
  ...
  test "allows reviews from guest purchases with no linked user" do
  ...
  context_ "when seller has reviews disabled after 1 year" do
        before { product.user.update!(disable_reviews_after_year: true) }

  test "rejects reviews for old purchases" do
          put :set, params: valid_params
  ...
  test "rejects updates to old reviews" do
  ...
  test "rejects updating an existing review with a JSON error" do
          review = create(:product_review, purchase: purchase, rating: 2)
          put :set, params: valid_params
  ...
  test "rejects creating a new review with a JSON error" do
          put :set, params: valid_params
  ...
  context_ "when the product has #{attribute} set but is not deleted" do
          before { product.update!(attribute => 1.day.ago) }

  test "still allows updating an existing review — only deleted products are blocked" do
            review = create(:product_review, purchase: purchase, rating: 2)
  ...
  context_ "when product doesn't exist" do
  ...
  context_ "when product reviews are hidden" do
  ...
  test "allows product owner to view reviews" do
  ...
  test "returns paginated product reviews" do
        get :index, params: { product_id: product.external_id }
  ...
  context_ "when review doesn't exist" do
  test "returns not found" do
  ...
  context_ "when review is deleted" do
  test "returns not found" do
  ...
  context_ "when the review has no message nor approved videos" do
  ...
  context_ "when the review only has an approved video" do
  ...
  test "allows product owner to view the review" do
          sign_in product.user
          get :show, params: { id: review.external_id }
          expect(response).to be_successful
        end
