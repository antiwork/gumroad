...
class DashboardControllerTest < ActionController::TestCase
  self.described_class = DashboardController
  tests DashboardController



  context_ DashboardController, type: :controller, inertia: true do
    render_views
  ...
  context_ "when seller has no activity" do
  ...
  test "creates LargeSeller record for sellers with many sales" do
  ...
  context_ "when seller has products, but no sales" do
        before do
          create(:product, user: seller)
        end

  test "renders no sales text" do
          get :index
  ...
  context_ "when seller has purchases" do
  ...
  test "renders the purchase", :sidekiq_inline, :elasticsearch_wait_for_refresh do
          get :index
  ...
  context_ "when seller has no alive products" do
  ...
  test "renders appropriate text" do
          get :index
  ...
  context_ "when seller has completed all 'Getting started' items" do
        before do
          create(:product, user: seller)
  ...
  test "doesn't render `Getting started` text"  do
          get :index

          expect(response.body).not_to have_text("We're here to help you get paid for your work.")
          expect(response.body).not_to have_text("Getting started")
        end
      end

  context_ "when seller has dismissed the getting started checklist" do
        before do
          seller.update!(has_dismissed_getting_started_checklist: true)
        end

  test "returns only first_product in getting started stats and marks checklist as dismissed" do
          get :index
  ...
  context_ "when seller is suspended for TOS" do
  ...
  test "redirects to the products_path" do
          get :index
  ...
  context_ "GET customers_count" do
      it_behaves_like "authorize called for action", :get, :customers_count do
        let(:record) { :dashboard }
      end

  test "returns the formatted number of customers" do
  ...
  context_ "GET total_revenue" do
      it_behaves_like "authorize called for action", :get, :total_revenue do
        let(:record) { :dashboard }
      end

  test "returns the formatted revenue" do
  ...
  context_ "GET active_members_count" do
  ...
  context_ "GET monthly_recurring_revenue" do
  ...
  context_ "POST dismiss_getting_started_checklist" do
      it_behaves_like "authorize called for action", :post, :dismiss_getting_started_checklist do
        let(:record) { :dashboard }
      end

  test "dismisses the getting started checklist" do
  ...
  test "succeeds when checklist is already dismissed" do
        seller.update!(has_dismissed_getting_started_checklist: true)
  ...
  context_ "GET download_tax_form" do
      it_behaves_like "authorize called for action", :get, :download_tax_form do
        let(:record) { :dashboard }
      end

  test "redirects to the 1099 form download url if present" do
  ...
  test "redirects to dashboard if form download url is not present" do
