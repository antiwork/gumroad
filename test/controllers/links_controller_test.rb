...
class LinksControllerTest < ActionController::TestCase
  self.described_class = LinksController
  ...
  test "renders the Products/Index component with correct props" do
          get :index
  ...
  test "enables a disabled link" do
  ...
  context_ "when link is not publishable" do
          before do
  ...
  test "does not publish the link" do
            post :publish, params: { id: @disabled_link.unique_permalink }
  ...
  context_ "when user email is not confirmed" do
          before do
  ...
  context_ "when a temp file is missing during publish" do
          before do
  ...
  test "returns a retry-friendly error message" do
            post :publish, params: { id: @disabled_link.unique_permalink }
  ...
  context_ "when an unknown exception is raised" do
          before do
  ...
  test "succeeds when the product has an expired default offer code" do
  ...
  test "updates the SellerProfileSections attached to the product and cleans up orphaned sections" do
          sections = create_list(:seller_profile_products_section, 2, seller:, product:)
  ...
  context_ "suspended tos violation user" do
          before do
  ...
  test "allows deletion if user suspended (tos)" do
            delete :destroy, params: { id: @product.unique_permalink }
            expect(@product.reload.deleted_at.present?).to be(true)
          end
        end

  test "allows deletion when default_offer_code is no longer associated with the product" do
          product = create(:product, user: seller)
          offer_code = create(:offer_code, user: seller, products: [product])
  ...
  test "renders the Inertia product edit page" do
  ...
  context_ "with other user not owning the product" do
  ...
  test "redirects to product page" do
            get :edit, params: { id: product.unique_permalink }
            expect(response).to redirect_to(short_link_path(product))
          end
        end

  context_ "with admin user signed in" do
  ...
  context_ "when the product is a bundle" do
          let(:bundle) { create(:product, :bundle) }

  test "redirects to the bundle edit page" do
            sign_in bundle.user
  ...
  context_ "with wildcard sub-path" do
  test "renders the Inertia page for sub-routes" do
  ...
  test "returns 404 when the price_checker feature flag is disabled" do
  ...
  test "returns 504 when the service raises TimeoutError" do
  ...
  test "returns the price distribution payload as JSON" do
          payload = {
            status: "ok",
            tier: "broadened",
            match_count: 25,
  ...
  test "passes force_refresh when refresh param is present" do
  ...
  test "passes sanitized overrides to the service" do
  ...
  test "drops an unknown currency_code override" do
          expect(PriceCheckerService).to receive(:call).with(
            product: product,
            overrides: { name: "ok" },
            force_refresh: false,
  ...
  test "drops invalid overrides instead of erroring" do
          expect(PriceCheckerService).to receive(:call).with(
            product: product,
            overrides: { name: "ok" },
            force_refresh: false,
  ...
  context_ "when the user does not own the product" do
          let(:other_user) { create(:user) }
  ...
  test "returns the existing validation error when suggested price is set but the default price record is missing" do
  ...
  context_ "when user email is empty" do
          before do
            seller.email = ""
            seller.save(validate: false)
          end

  test "includes error_message when publishing" do
  ...
  context_ "when license key is embedded in the product-level rich content" do
  test "sets is_licensed to true" do
  ...
  context_ "when license key is embedded in the rich content of at least one version" do
  test "sets is_licensed to true" do
  ...
  test "sets is_licensed to false when no license key is embedded in the rich content" do
            expect(@product.is_licensed).to be(false)
  ...
  test "sets suggested_price_cents to the maximum price_difference_cents of variants" do
  ...
  context_ "content_updated_at" do
  test "is updated when a new file is uploaded" do
            freeze_time do
  ...
  test "is not updated when irrelevant attributes are changed" do
            freeze_time do
  ...
  context_ "invalidate_action" do
          before do
            Rails.cache.write("views/#{@product.cache_key_prefix}_en_displayed_switch_ids_.html", "<html>hello</html>")
          end

  test "invalidates the action" do
            expect(Rails.cache.read("views/#{@product.cache_key_prefix}_en_displayed_switch_ids_.html")).not_to be_nil
  ...
  test "updates the product refund policy" do
            put :update, params: @params, as: :json
            @product.reload
  ...
  context_ "when seller refund policy is set to false" do
          before do
  ...
  context_ "with product refund policy enabled" do
            before do
              @product.update!(product_refund_policy_enabled: true)
            end

  test "disables the product refund policy" do
  ...
  test "updates a physical product" do
  ...
  test "appends removed_file_info_attributes when additional keys are provided" do
  ...
  context_ "currency and price updates" do
          before do
            @product.update!(price_currency_type: "usd", price_cents: 1000)
          end

  test "changes product from USD $10 to EUR €12 and back to USD $11" do
  ...
  test "sets the correct value for removed_file_info_attributes if there are none" do
          post :update, params: @params.merge({
                                                file_attributes: [
                                                  {
                                                    name: "Length",
  ...
  test "marks the product as allowing display of reviews if the display_product_reviews param is true" do
  ...
  test "marks the product as not allowing display of reviews if the display_product_reviews param is false" do
  ...
  test "marks the product as allowing display of sales count if the should_show_sales_count param is true" do
  ...
  test "marks the product as not allowing display of sales count if the should_show_sales_count param is false" do
  ...
  context_ "adding variants" do
  context_ "variants" do
  test "adds variants to the product" do
              variants = [
  ...
  context_ "removing a variant from an existing category" do
  ...
  test "persists the variants correctly" do
              variants = [
  ...
  context_ "when all variants are removed" do
            let(:category) { create(:variant_category, title: "sizes", link: @product) }
            let!(:variant1) { create(:variant, variant_category: category, name: "small", price_difference_cents: 200, max_purchase_count: 100) }
  ...
  test "updates profile sections" do
  ...
  context_ "subscription pricing" do
          let(:membership_product) { create(:membership_product, user: seller) }

  context_ "changing membership price update settings for a tier" do
  ...
  test "enables existing membership price upgrades" do
  ...
  context_ "when existing membership price upgrades are enabled" do
              before do
  ...
  test "changes effective date to a later date and schedules emails to subscribers" do
  ...
  test "changes effective date to an earlier date and schedules emails to subscribers" do
  ...
  context_ "setting recurring prices on a variant" do
          before :each do
  ...
  test "sets the prices on the variants" do
  ...
  context_ "cancellation discounts" do
            before do
  ...
  context_ "when cancellation_discounts feature flag is off" do
  test "does not update the cancellation discount" do
                expect(Product::SaveCancellationDiscountService).not_to receive(:new)
                post :update, params: @params, format: :json
              end
            end

  context_ "when cancellation_discounts feature flag is on" do
              before do
                Feature.activate_user(:cancellation_discounts, @product.user)
              end

  test "updates the cancellation discount" do
  ...
  context_ "default discount code" do
  ...
  test "does not set the default offer code when offer code is not associated with the product" do
  ...
  test "clears the default offer code when nil is provided" do
              @product.update!(default_offer_code: offer_code)
  ...
  test "clears the default offer code when empty string is provided" do
              @product.update!(default_offer_code: offer_code)
  ...
  context_ "with pay-what-you-want pricing" do
  test "sets the suggested prices" do
              @params.merge!(
                id: @product.unique_permalink,
                variants: [
                  {
  ...
  test "sets the shipping rates as configured with no duplicates on the product" do
            post :update, params: {
              id: @product.unique_permalink,
              shipping_destinations: [
  ...
  test "does not accept duplicate submission for the same country for a product" do
            post :update, params: {
              id: @product.unique_permalink,
              shipping_destinations: [
                { country_code: "US", one_item_rate_cents: 1200, multiple_items_rate_cents: 600 },
  ...
  test "does not allow link to be saved if there are no shipping destinations" do
            post :update, params: {
              id: @product.unique_permalink,
              shipping_destinations: []
            }, format: :json
  ...
  context_ "Tags and Categories" do
  context_ "Adding tags" do
            let(:tags) { ["some sort of tàg!", "tagme", "🐗🐗"] }

  test "adds tags when there are none" do
              expect do
  ...
  test "adds tags when they exist" do
  ...
  test "does not remove tags if unchanged" do
              @product.tag!("one tag")
              @product.tag!("another tag")
              expect do
  ...
  context_ "custom attributes" do
  test "saves the custom attributes properly" do
  ...
  context_ "without files" do
  test "allows updating a published product to have no files" do
            expect do
  ...
  test "updates existing files and the product description appropriately" do
            files_params = [
  ...
  test "schedules unused files for deletion" do
            ...
            expect(@product.description).not_to include(public_file2.public_id)
            expect(@product.description).not_to include(unused_file.public_id)
  ...
  test "removes invalid file embeds from content" do
  ...
  test "handles missing public_files params" do
  ...
  context_ "multiple files" do
          def files_data_from_urls(urls)
            urls.map { { id: SecureRandom.uuid, url: _1 } }
          end

  test "preserves correct s3 key for s3 files containing percent and ampersand" do
  ...
  test "saves the files properly" do
  ...
  test "supports deleting and adding files" do
  ...
  test "allows 0 files for unpublished product" do
  ...
  test "updates product's rich content when file embed IDs exist in product_rich_content" do
  ...
  test "does not produce transitive ID collisions when a new file's external_id matches another file's placeholder ID" do
            rich_content_node = {
              "type" => "doc",
              "content" => [
  ...
  test "handles nil nodes in rich content without crashing" do
            rich_content_node = {
              "type" => "doc",
              "content" => [
                { "type" => "fileEmbed", "attrs" => { "id" => "placeholder_a" } },
  ...
  test "saves variant-level rich content containing file embeds with the persisted IDs" do
            external_id1 = "ext1"
            external_id2 = "ext2"
            category = create(:variant_category, link: @product, title: "Versions")
            version1 = create(:variant, variant_category: category, name: "Version 1")
  ...
  test "calls SaveContentUpsellsService when rich content or description changes" do
  ...
  test "saves the product file thumbnails" do
  ...
  context_ "adding integrations" do
          shared_examples "manages integrations" do
  test "adds a new integration" do
              expect do
                post :update, params: @params.merge(
  ...
  test "modifies an existing integration" do
  ...
  context_ "circle integration" do
  ...
  context_ "discord integration" do
            let(:server_id) { "0" }
  ...
  test "succeeds if bot is successfully removed from server" do
  ...
  test "fails if removing bot from server fails" do
                WebMock.stub_request(:delete, "#{Discordrb::API.api_base}/users/@me/guilds/#{server_id}").
                  with(headers: request_header).
  ...
  context_ "google calendar integration" do
  ...
  test "succeeds if the gumroad app is successfully disconnected from google account" do
  ...
  test "fails if disconnecting the gumroad app from google fails" do
                WebMock.stub_request(:post, "#{GoogleCalendarApi::GOOGLE_CALENDAR_OAUTH_URL}/revoke").
  ...
  context_ "custom domains" do
  context_ "with an existing domain" do
            let(:new_domain_name) { "example2.com" }

  context_ "when product has an existing custom domain" do
              before do
  ...
  context_ "when the product doesn't have an existing custom_domain" do
  ...
  test "enqueues a RenameProductFileWorker job" do
          @product.product_files << create(:product_file, link: @product, url: "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachment/pencil.png")
          @product.save!
          post :update, params: {
            id: @product.unique_permalink,
  ...
  context_ "rich content" do
          let(:product) { create(:product, user: seller) }
          let(:product_content) { [{ "type" => "paragraph", "content" => [{ "type" => "text", "text" => "Hello" }] }] }

  test "saves the rich content pages in the given order" do
  ...
  context_ "product_files_archive generation" do
  test "deletes all product-level archives when switching to variant-level archives" do
            ...
            end.not_to change { ProductFilesArchive.count }
  ...
  test "deletes all variant-level archives when switching to product-level archives" do
            category = create(:variant_category, link: @product, title: "Versions")
            version1 = create(:variant, variant_category: category, name: "Version 1")
  ...
  test "does not generate a folder archive when nothing has changed" do
  ...
  test "does not generate a folder archive when there are no folders" do
            file1 = create(:product_file, display_name: "File 1")
            ...
            end.not_to change { @product.product_files_archives.folder_archives.alive.count }
          end

  test "does not generate a folder archive when a folder only contains 1 file" do
            file1 = create(:product_file, display_name: "File 1")
            @product.product_files = [file1]
            description = [
  ...
  test "does not generate an updated folder archive when the product name or page name is changed" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
            @product.product_files = [file1, file2]
  ...
  test "does not generate an updated folder archive when top-level files are modified" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
  ...
  test "generates a folder archive for every valid folder on a page" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
            ...
            end.not_to change { @product.product_files_archives.folder_archives.count }

            expect([folder1_archive.reload, folder2_archive.reload, folder3_archive.reload].all?(&:alive?)).to eq(true)
          end

  test "generates a folder archive when a folder is added to an existing page" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
            @product.product_files = [file1, file2]
            folder1_id = SecureRandom.uuid
  ...
  test "generates a new folder archive and deletes the old archive for an existing folder that gets modified" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
            @product.product_files = [file1, file2]
            folder1_id = SecureRandom.uuid
  ...
  test "generates new folder archives when a file is moved from one folder to another folder" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
            file3 = create(:product_file, display_name: "File 3")
            file4 = create(:product_file, display_name: "File 4")
  ...
  test "deletes the corresponding folder archive when a folder gets deleted" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
            @product.product_files = [file1, file2]
  ...
  test "deletes a folder archive if the folder is updated to contain only 1 file" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
            @product.product_files = [file1, file2]
            folder_id = SecureRandom.uuid
  ...
  test "updates all folder archives when multiple changes occur to a product's rich content across multiple pages" do
            file1 = create(:product_file, display_name: "File 1")
            file2 = create(:product_file, display_name: "File 2")
            file3 = create(:product_file, display_name: "File 3")
            file4 = create(:product_file, display_name: "File 4")
  ...
  context_ "product variants" do
  test "generates folder archives for a new variant when has_same_rich_content_for_all_variants is false" do
              category = create(:variant_category, link: @product, title: "Versions")
              version1 = create(:variant, variant_category: category, name: "Version 1")
  ...
  test "generates folder archives for the file embed groups in product-level content when has_same_rich_content_for_all_variants is true" do
              file1 = create(:product_file, display_name: "File 1")
              file2 = create(:product_file, display_name: "File 2")
              @product.product_files = [file1, file2]
  ...
  context_ "error handling on save" do
  context_ "when Link::LinkInvalid is raised" do
            let(:product) { create(:product, user: seller) }

  test "logs and renders error message" do
  ...
  context_ "when product is eligible for installment plans" do
            let(:product) { create(:product, user: seller, price_cents: 1000) }

  context_ "no existing plans" do
  ...
  context_ "updating an existing plan" do
  ...
  context_ "has existing payment_options" do
                before do
                  create(:payment_option, installment_plan: existing_plan)
                  create(:installment_plan_purchase, link: product)
                end

  test "soft deletes the existing plan and creates a new plan" do
                  expect do
                    post :update, params: {
                      id: product.unique_permalink,
                      installment_plan: {
  ...
  context_ "no existing payment_options" do
  test "destroys the existing plan and creates a new plan" do
                  expect do
                    post :update, params: {
                      id: product.unique_permalink,
                      installment_plan: {
  ...
  context_ "removing an existing plan" do
              let!(:existing_plan) do
                create(
                  :product_installment_plan,
                  link: product,
  ...
  test "soft deletes the existing plan even if product is no longer eligible for installment plans" do
                  expect do
                    post :update, params: {
                      id: product.unique_permalink,
                      price_cents: 0,
  ...
  context_ "no existing payment_options" do
  test "destroys the existing plan" do
                  expect do
                    post :update, params: {
                      id: product.unique_permalink,
                      installment_plan: nil
  ...
  test "does not create an installment plan" do
              expect do
                post :update, params: {
                  id: membership_product.unique_permalink,
                  installment_plan: {
  ...
  context_ "community chat" do
  context_ "when communities feature is enabled" do
            before do
              Feature.activate_user(:communities, seller)
            end

  test "enables community chat when requested" do
  ...
  test "disables community chat when requested" do
  ...
  test "does not enable community chat for coffee products" do
  ...
  test "does not enable community chat for bundle products" do
  ...
  test "reactivates existing community when enabling chat" do
  ...
  context_ "when communities feature is disabled" do
            before do
              Feature.deactivate_user(:communities, seller)
            end

  test "does not enable community chat" do
              post :update, params: { id: @product.unique_permalink, community_chat_enabled: true }, as: :json
  ...
  test "shows the introduction text if the user has no memberships or products" do
          get :new
  ...
  test "does not show the introduction text if the user has memberships" do
  ...
  test "does not show the introduction text if the user has products" do
  ...
  test "creates link with display_product_reviews set to true" do
  ...
  test "ignores is_in_preorder_state param" do
  ...
  test "is able to set currency type" do
  ...
  context_ "when the product's native type is bundle" do
  test "sets is_bundle to true" do
  ...
  context_ "the product is a coffee product" do
          let(:seller) { create(:user, :eligible_for_service_products) }

  test "sets custom_button_text_option to 'donate_prompt'" do
  ...
  test "defaults should_show_all_posts to true for recurring billing products" do
  ...
  test "sets is_recurring_billing correctly" do
              expect(@product.is_recurring_billing).to be(true)
            end

  test "sets the correct duration" do
              expect(@product.subscription_duration).to eq "monthly"
            end
          end

  context_ "yearly duration" do
            before do
  ...
  context_ "physical" do
          before do
            @params = { price_cents: 100, name: "test physical link", is_physical: true }
          end

  context_ "when physical products are enabled" do
            before do
              seller.update!(can_create_physical_products: true)
            end

  test "allows users to create physical products" do
              post :create, params: { link: @params }
              expect(response).to redirect_to(edit_link_path(Link.last))
              product = Link.last
  ...
  context_ "when physical products are disabled" do
  ...
  test "does not enable community chat by default" do
              params = { price_cents: 100, name: "test link" }
  ...
  context_ "Product details generation using AI" do
          let(:params) do
            {
  ...
  test "calls AI service when ai_prompt is present and feature is active" do
  ...
  test "does not call AI service when feature is inactive" do
  ...
  test "does not call AI service when ai_prompt is blank" do
            service_double = instance_double(Ai::ProductDetailsGeneratorService)
            allow(Ai::ProductDetailsGeneratorService).to receive(:new).and_return(service_double)
            expect(service_double).not_to receive(:generate_cover_image)
            expect(service_double).not_to receive(:generate_rich_content_pages)
  ...
  context_ "POST release_preorder" do
        before do
  ...
  test "returns the right success value" do
  ...
  test "releases the preorder even though the release date is in the future" do
          post :release_preorder, params: @params
          expect(response.parsed_body["success"]).to be(true)
  ...
  context_ "POST send_sample_price_change_email" do
  ...
  test "returns an error if the tier ID is incorrect" do
  ...
  test "raises an error if required params are missing" do
          expect do
            post :send_sample_price_change_email, params: { id: product.unique_permalink, tier_id: tier.external_id }
          end.to raise_error(ActionController::ParameterMissing)
        end

  test "sends a sample price change email to the user" do
          expect do
  ...
  context_ "layout variants" do
  test "renders Products/Show with product props for default layout" do
  ...
  test "renders Products/Profile/Show with creator_profile for profile layout" do
            link = create(:product, user: @user)
  ...
  test "renders Products/Discover/Show with taxonomy props for discover layout" do
            link = create(:product, user: @user)
  ...
  test "renders Products/Iframe/Show with product props for embed param" do
            link = create(:product, user: @user)
  ...
  test "renders Products/Iframe/Show with product props for overlay param" do
            link = create(:product, user: @user)
  ...
  context_ "wanted=true parameter" do
  test "passes pay_in_installments parameter to checkout when wanted=true" do
  ...
  test "doesn't redirect to checkout for PWYW products without price" do
  ...
  context_ "discount code selection" do
  ...
  context_ "when URL code has better discount than default code" do
  ...
  test "uses the URL code in checkout redirect" do
  ...
  context_ "when default code has better discount than URL code" do
              let(:url_offer_code) { create(:offer_code, products: [product], code: "URL10", amount_cents: 200, currency_type: product.price_currency_type) }
  ...
  test "uses the default code in checkout redirect" do
                get :show, params: { id: product.to_param, wanted: "true", code: url_offer_code.code }
  ...
  context_ "when only URL code is provided" do
              before do
  ...
  context_ "when only default code is provided" do
  test "uses the default code in checkout redirect" do
                get :show, params: { id: product.to_param, wanted: "true" }
  ...
  context_ "when URL code is invalid and default code is valid" do
  test "uses the default code in checkout redirect" do
  ...
  context_ "when both codes are invalid" do
              before do
                product.update!(default_offer_code: nil)
              end

  test "does not include code in checkout redirect" do
                get :show, params: { id: product.to_param, wanted: "true", code: "INVALID" }
  ...
  context_ "when discount code is passed as offer_code param (embed/legacy URLs)" do
  test "picks up offer_code and uses the better of URL code and default in checkout redirect" do
  ...
  test "includes code exactly once in redirect query string when code param is passed" do
              get :show, params: { id: product.to_param, wanted: "true", code: url_offer_code.code }
  ...
  test "includes code exactly once in redirect query string when offer_code param is passed" do
              get :show, params: { id: product.to_param, wanted: "true", offer_code: url_offer_code.code }
  ...
  context_ "with user signed in" do
  ...
  test "assigns the correct props" do
  ...
  context_ "meta tags sanitization" do
  test "properly escapes double quote in content" do
            ...
            expect(html_doc.css("meta[name='description'][content='I like pie.\"']")).not_to be_empty
          end

  test "scrubs tags in content" do
  ...
  test "escapes new lines and html tags" do
            ...
            # Can't use assert_selector, it doesn't work for tags in head
            html_doc = Nokogiri::HTML(response.body)
            expect(html_doc.css("meta[name='description'][content='I like pie. This is not what we had estimated! ~']")).not_to be_empty
          end
        end

  context_ "asset previews" do
          before do
            @product = create(:product_with_file_and_preview, user: @user)
          end

  test "includes asset preview data in Inertia props" do
            get(:show, params: { id: @product.to_param })
  ...
  context_ "when custom_permalink exists" do
          let(:product) { create(:product, user: @user, custom_permalink: "custom") }

  test "redirects from unique_permalink to custom_permalink URL preserving the original query parameter string" do
  ...
  context_ "redirection to creator's subdomain" do
          before do
            @request.host = DOMAIN
          end

  context_ "when requested with unique permalink" do
  context_ "when custom permalink is not present" do
  test "redirects to the subdomain product URL with original query params" do
  ...
  context_ "when custom permalink is present" do
  test "redirects to the subdomain product URL using custom permalink with original query params" do
  ...
  context_ "when offer code is present" do
  test "redirects to subdomain product URL with offer code and original query params" do
                product = create(:product)
  ...
  context_ "when requested with custom permalink" do
  test "redirects to the subdomain product URL using custom permalink with original query params" do
              product = create(:product, custom_permalink: "abcd")
  ...
  context_ "when the product is deleted" do
  ...
  context_ "when the product is a coffee product" do
          let!(:product) { create(:product, user: @user, native_type: Link::NATIVE_TYPE_COFFEE) }

  test "redirects to the coffee page" do
  ...
  test "does not 404 if user is not suspended" do
          link = create(:product, user: @user)
          expect { get :show, params: { id: link.to_param } }.not_to raise_error
        end

  test "404s on an unsupported format" do
          link = create(:product, user: @user)
          expect do
  ...
  context_ "canonical urls" do
  test "renders the canonical meta tag" do
  ...
  context_ "product information markup" do
  test "sets server-side meta tags for classic product" do
  ...
  test "sets server-side meta tags for product over $1000" do
            ...
            expect(response).to be_successful
            html_doc = Nokogiri::HTML(response.body)
            expect(html_doc.css("meta[property='product:retailer_item_id'][content='#{product.unique_permalink}']")).not_to be_empty
            expect(html_doc.css("meta[content='#{product.long_url}'][property='og:url']")).not_to be_empty
            expect(html_doc.css("meta[property='product:price:amount'][content='1000.0']")).not_to be_empty
            expect(html_doc.css("meta[property='product:price:currency'][content='USD']")).not_to be_empty
          end

  test "sets canonical and og:url meta tags for product without reviews" do
            product = create(:product, user: @user)
            get :show, params: { id: product.unique_permalink }
            ...
            html_doc = Nokogiri::HTML(response.body)
            expect(html_doc.css("meta[content='#{product.long_url}'][property='og:url']")).not_to be_empty
            expect(html_doc.css("link[rel='canonical'][href='#{product.long_url}']")).not_to be_empty
          end

  test "sets server-side meta tags for membership product" do
  ...
  test "includes product data in Inertia props" do
            product = create(:product, user: @user, price_currency_type: "usd", price_cents: 525)
            get :show, params: { id: product.unique_permalink }
  ...
  test "renders seller custom_styles in the head as a style tag" do
  ...
  test "does not set no index header by default" do
          product = create(:product, user: @user)
          get :show, params: { id: product.unique_permalink }
          expect(response.headers["X-Robots-Tag"]).to be_nil
        end

  context_ "adult products" do
  test "does not set the noindex header" do
  ...
  context_ "non-alive products" do
  test "sets the noindex header" do
            product = create(:product, user: @user)
  ...
  test "sets paypal_merchant_currency as merchant account's currency if native paypal payments are enabled else as usd" do
          product = create(:product, user: @user)
  ...
  context_ "when requests come from custom domains" do
  ...
  context_ "when the custom domain matches a product's custom domain" do
  test "assigns the product and renders the Inertia page" do
              get :show
              expect(response).to be_successful
  ...
  context_ "when the custom domain matches a deleted product" do
            before do
              product.mark_deleted!
            end

  test "raises ActionController::RoutingError" do
              expect { get :show }.to raise_error(ActionController::RoutingError)
            end
          end

  context_ "when the same domain name is used for a user's deleted custom domain and a product's active custom domain" do
            before do
  ...
  context_ "when a product's custom domain is deleted" do
            before do
  ...
  context_ "when a product's saved custom domain does not use the www prefix" do
            before do
  ...
  context_ "when requests come from subdomains" do
          before do
  ...
  context_ "when the subdomain and unique permalink are valid and present" do
            before do
              @product = create(:product, user: @user)
            end
  ...
  context_ "when the product has custom permalink but accessed through unique permalink" do
            before do
              @product = create(:product, user: @user, custom_permalink: "onetwothree")
            end

  test "redirects unique permalink to custom permalink" do
              get :show, params: { id: @product.unique_permalink }
              expect(response).to redirect_to(@product.long_url)
            end
          end

  context_ "when the subdomain and custom permalink are valid and present" do
            before do
  ...
  context_ "when the seller from subdomain is different from product's seller" do
            before do
  ...
  context_ "when request comes from a legacy product URL" do
          before do
  ...
  context_ "when looked up by unique permalink" do
  test "redirects to a product URL with subdomain and custom permalink" do
  ...
  context_ "when looked up by custom permalink" do
  test "redirects to a full product URL of the oldest product matched by custom permalink" do
  ...
  context_ "legacy products lookup" do
          before do
            @user = create(:user)
  ...
  context_ "when request comes from a legacy URL" do
            before do
              @request.host = DOMAIN
            end

  test "redirects to a product defined by legacy permalink" do
              get :show, params: { id: "custom" }

              expect(response).to redirect_to(@product_with_legacy_mapping.long_url)
            end

  context_ "when legacy permalink points to a deleted product" do
              before do
                @product_with_legacy_mapping.mark_deleted!
              end

  test "redirects to an earlier product matched by permalink" do
                get :show, params: { id: "custom" }
  ...
  context_ "when request comes from a custom domain" do
            before do
              @domain = CustomDomain.create(domain: "www.example1.com", user: @user)
              @request.host = "www.example1.com"
            end

  test "renders the user's product" do
              get :show, params: { id: "custom" }
  ...
  context_ "when request comes from a subdomain URL" do
            before do
              @request.host = "#{@user.username}.test.gumroad.com"
              stub_const("ROOT_DOMAIN", "test.gumroad.com")
            end
  ...
  context_ "setting affiliate cookie" do
          let(:product) { create(:product) }
  ...
  context_ "with `#{query_param}` query param" do
  ...
  test "adds X-Robots-Tag response header to avoid page indexing only if the url contains an offer code" do
  ...
  context_ "Discover tracking" do
  test "stores click when coming from discover" do
  ...
  test "does not store click when not coming from discover" do
            expect do
              get :show, params: { id: product.to_param }
  ...
  context_ "GET cart_items_count" do
  test "returns 0 when no cart exists" do
  ...
  test "returns the count of alive cart products" do
          sign_in @user
          product = create(:product)
  ...
  test "does not count deleted cart products" do
          sign_in @user
          product = create(:product)
          cart = create(:cart, user: @user, email: @user.email)
          create(:cart_product, cart:, product:)
  ...
  context_ "POST increment_views" do
        before do
          @product = create(:product)
  ...
  context_ "with a logged out visitor" do
          before do
            sign_out @user
          end
  ...
  context_ "with a logged out user" do
          include_examples "records page view"
        end
  ...
  context_ "data recorded", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  test "sets country and state from custom IP address" do
  ...
  test "sets referrer via HTTP header" do
            @request.env["HTTP_REFERER"] = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
            post :increment_views, params: { id: @product.to_param }
            expect(last_page_view_data.with_indifferent_access).to include(
              referrer_domain: "youtube.com",
  ...
  test "sets referrer via params" do
  ...
  test "sets custom browser_guid" do
            cookies[:_gumroad_guid] = "custom_guid"
            post :increment_views, params: { id: @product.to_param }
            expect(last_page_view_data[:browser_guid]).to eq("custom_guid")
          end

  test "sets user_id to nil when the user is signed out" do
            sign_out @user
            post :increment_views, params: { id: @product.to_param }
            expect(last_page_view_data[:user_id]).to eq(nil)
          end

  test "sets correct referrer_domain when product is not recommended" do
            @request.env["HTTP_REFERER"] = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
            post :increment_views, params: {
              id: @product.to_param,
  ...
  test "sets correct referrer_domain when product is recommended" do
            @request.env["HTTP_REFERER"] = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
            post :increment_views, params: {
              id: @product.to_param,
  ...
  test "does not record page view for the seller of the product" do
  ...
  test "does not record page view for an admin user" do
  ...
  test "does not record page view" do
            post :increment_views, params: { id: @product.to_param }
  ...
  test "does not record page view for bots" do
  ...
  test "does not record page view for an admin becoming user" do
  ...
  context_ "POST track_user_action" do
  context_ "with signed in user" do
          before do
            sign_in @user
          end

          shared_examples "creates an event" do
  test "writes the event to the events table" do
  ...
  context_ "create_purchase_event" do
  test "creates a purchase event" do
  ...
  test "accepts a string ids param when searching by user" do
  ...
  context_ "Setting and ordering" do
          before do
            Link.__elasticsearch__.create_index!(force: true)
            @creator = create(:compliant_user, username: "creatordudey", name: "Creator Dudey")
            @section = create(:seller_profile_products_section, seller: @creator)
  ...
  test "returns the expected JSON response when no search parameters are specified" do
            res = {
              "total" => 1,
              "filetypes_data" => [],
              "tags_data" => [],
  ...
  test "returns the expected JSON response when searching by a user" do
  ...
  test "returns products in page layout order when applicable if searching by user" do
            @recommended_by = nil
            @on_profile = true
  ...
  test "returns an empty response when searching by non-existent user" do
  ...
  test "returns an empty response when searching by non-existent section" do
  ...
  test "searches only for recommendable products" do
  ...
  test "returns product in fee revenue order" do
  ...
  test "searches successfully for a product with a regex character" do
  ...
  context_ "Loose and exact matching" do
          before do
            Link.__elasticsearch__.create_index!(force: true)
            @products = {
  ...
  test "finds all matches if exact match not specified" do
  ...
  test "finds exact match if double-quotes used" do
            get :search, params: { query: '" north american river otter  "' }
            expect(response.parsed_body["products"]).to match_array(%i[name desc creator].map { |key| product_json(@products[key], "discover") })
          end

  test "finds compound match when double-quotes used in combination with another term" do
  ...
  test "finds results for a complex match across different fields" do
  ...
  test "handles potentially malformed query" do
            get :search, params: { query: "\\" }
            expect(response.parsed_body["products"]).to eq([])
          end
        end

  context_ "Filtering" do
  context_ "for products with no reviews" do
            before do
  ...
  context_ "Discover tracking" do
  test "stores the search query along with useful metadata" do
            cookies[:_gumroad_guid] = "custom_guid"
            sign_in @user
  ...
  test "does not store search when querying user products" do
            expect do
