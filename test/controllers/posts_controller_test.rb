...
class PostsControllerTest < ActionController::TestCase
  self.described_class = PostsController
  tests PostsController



  context_ PostsController, type: :controller, inertia: true do
    let(:seller) { create(:named_seller) }

  context_ "within seller area" do
      include_context "with user signed in as admin for seller"

  context_ "GET redirect_from_purchase_id" do
        before do
  ...
  test "redirects old /library/purchase/purchase_id paths to the new path" do
  ...
  context_ "GET send_for_purchase" do
        before do
  ...
  test "returns an error if seller is not eligible to send emails" do
  ...
  test "returns success and redelivers the installment" do
          @purchase.create_url_redirect!
  ...
  test "renders a non-public product installment with a valid purchase_id" do
  ...
  test "sets the post page meta tags" do
          installment = create(:published_installment, link: @product, installment_type: "product", shown_on_profile: false)
          get :show, params: { username: @user.username, slug: installment.slug, purchase_id: @purchase.external_id }
  ...
  test "sets @user instance variable to load third-party analytics config" do
          installment = create(:published_installment, link: @product, installment_type: "product", shown_on_profile: false)
          get :show, params: { username: @user.username, slug: installment.slug, purchase_id: @purchase.external_id }
  ...
  context_ "with user signed in as support for seller" do
  ...
  test "renders post" do
            get :show, params: { username: seller.username, slug: post.slug }
            expect(response).to be_successful
          end
        end

  test "renders a publicly visible installment even if it doesn't have a purchase_id" do
  ...
  test "does not render a non-public installment if it doesn't have a valid purchase_id" do
  ...
  context_ "when requested through custom domain" do
          before do
  ...
  context_ "when requested through app domain" do
          before do
            @request.host = DOMAIN
            @post = create(:published_installment, installment_type: Installment::AUDIENCE_TYPE, seller: @user, shown_on_profile: true)
          end

  test "redirects to subdomain url of the post" do
  ...
  context_ "increment_post_views" do
  context_ "page view incrementor" do
  test "increments the post page view with a new Event and InstallmentEvent record" do
  ...
  test "creates a post page view event associated with a link" do
            link = create(:product)
  ...
  test "increments the page view for other user" do
            user_agent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_3) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.165 Safari/535.19"
            @request.env["HTTP_USER_AGENT"] = user_agent
            create(:user)
  ...
  test "increments the page view if user is anonymous" do
            user_agent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_3) AppleWebKit/535.19 (KHTML, like Gecko) Chrome/18.0.1025.165 Safari/535.19"
            @request.env["HTTP_USER_AGENT"] = user_agent
            create(:user)
            sign_out seller
  ...
  test "does not increment page views for bots" do
  ...
  test "does not increment page views for same user" do
            user = create(:user)
  ...
  test "does not increment page views for admin user" do
  ...
  test "does not increment page views for team member" do
              # user = create(:admin_user)
              # allow(controller).to receive(:current_user).and_return(user)
  ...
  test "does not increment page views for bots that pretends to be same user" do
            user = create(:user)
            allow(controller).to receive(:current_user).and_return(user)
            create(:product, user:)
            @request.env["HTTP_USER_AGENT"] = "EventMachine HttpClient"
