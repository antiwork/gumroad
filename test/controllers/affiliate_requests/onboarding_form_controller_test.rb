...
class AffiliateRequestsOnboardingFormControllerTest < ActionController::TestCase
  self.described_class = AffiliateRequests::OnboardingFormController
  tests AffiliateRequests::OnboardingFormController



  context_ AffiliateRequests::OnboardingFormController do
  ...
  context_ "when the request payload contains invalid information" do
  context_ "such as an invalid fee percent" do
          let(:params) do
            {
              products: [
  ...
  test "responds with an error without persisting any changes" do
            ...
            end.not_to change { seller.self_service_affiliate_products.reload }
  ...
  context_ "such as an ineligible product" do
          let(:params) do
            {
              products: [
  ...
  test "upserts the self service affiliate products, ignoring ineligible products and updates the global affiliate status" do
          expect do
            patch :update, params:, format: :json
            seller.reload
  ...
  context_ "when a pending affiliate request exists and all products are being disabled" do
  ...
  test "responds with an error and does not persist the requested changes" do
            expect do
              expect do
                patch :update, params:, as: :json
              end.not_to change { seller.self_service_affiliate_products.count }
            end.not_to change { seller.self_service_affiliate_products.enabled.count }
