...
class EmailsControllerTest < ActionController::TestCase
  self.described_class = EmailsController
  tests EmailsController



  context_ EmailsController, type: :controller, inertia: true do
  ...
  test "redirects to the published tab" do
        get :index

        expect(response).to redirect_to(published_emails_path)
      end

  test "redirects to the scheduled tab if there are scheduled installments" do
  ...
  test "includes copy_from installment when copy_from param is present" do
  ...
  context_ "when installment doesn't exist" do
  test "returns 404" do
  ...
  context_ "when save is successful" do
  ...
  test "redirects to edit email path with success notice" do
  ...
  test "redirects to edit path when save_and_preview_post is clicked" do
          installment = create(:installment, seller:)
  ...
  test "redirects to new email path with alert" do
          post :create, params: { installment: { subject: "Test Email" } }
  ...
  test "redirects to edit email path with alert" do
          patch :update, params: { id: installment.external_id, installment: { subject: "Updated Email" } }
