...
class WishlistProductPolicyTest < ActiveSupport::TestCase
  self.described_class = WishlistProductPolicy



  context_ WishlistProductPolicy do
  ...
  test "denies access to accountant" do
        ...
        expect(subject).not_to permit(seller_context, wishlist_product)
      end
  ...
  test "denies access to another user's wishlist product" do
        seller_context = SellerContext.new(user: seller, seller:)
        expect(subject).not_to permit(seller_context, wishlist_product)
      end
    end
