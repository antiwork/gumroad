...
class SettingsPaymentsUserPolicyTest < ActiveSupport::TestCase
  self.described_class = Settings::Payments::UserPolicy



  context_ Settings::Payments::UserPolicy do
  ...
  context_ "with owner as seller" do
  ...
  test "denies access to owner when record is other user" do
          expect(subject).not_to permit(seller_context, create(:user))
        end
      end

  context_ "with admin for seller" do
  ...
  context_ "with marketing for seller" do
  ...
  context_ "with accountant for seller" do
  ...
  context_ "with support for seller" do
        let(:seller_context) { SellerContext.new(user: marketing_for_seller, seller:) }
