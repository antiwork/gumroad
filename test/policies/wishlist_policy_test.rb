...
class WishlistPolicyTest < ActiveSupport::TestCase
  self.described_class = WishlistPolicy



  context_ WishlistPolicy do
  ...
  test "denies access to accountant" do
        ...
        expect(subject).not_to permit(seller_context, wishlist)
      end
  ...
  test "denies access to another user's wishlist" do
        seller_context = SellerContext.new(user: seller, seller:)
        expect(subject).not_to permit(seller_context, wishlist)
      end
    end
