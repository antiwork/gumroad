...
class GumroadBlogPostsPolicyTest < ActiveSupport::TestCase
  self.described_class = GumroadBlog::PostsPolicy



  context_ GumroadBlog::PostsPolicy do
  ...
  test "grants access to anonymous users" do
        expect(subject).to permit(SellerContext.logged_out, record)
      end
    end

    permissions :show? do
  context_ "when the post is published and meets all criteria" do
        let(:record) { published_post }
  ...
  context_ "when the post is unpublished" do
  ...
  context_ "for another seller" do
  ...
  test "denies access to anonymous users" do
          expect(subject).not_to permit(SellerContext.logged_out, record)
        end
      end

  context_ "when the post is not alive" do
  ...
  context_ "when the post is not shown on profile" do
  ...
  context_ "when the post is a workflow post" do
  ...
  context_ "when the post is not an audience post" do
