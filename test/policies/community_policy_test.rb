...
class CommunityPolicyTest < ActiveSupport::TestCase
  self.described_class = CommunityPolicy



  context_ CommunityPolicy do
  ...
  test "grants access to buyer with purchased product" do
          seller_context = SellerContext.new(user: buyer, seller: other_product.user)
          expect(subject).to permit(seller_context, Community)
        end

  test "denies access to seller who has at least one product but no active communities" do
  ...
  test "denies access to buyer with purchased product" do
          purchase
          seller_context = SellerContext.new(user: buyer, seller: other_product.user)
          expect(subject).not_to permit(seller_context, Community)
        end
    ...
    permissions :show? do
  context_ "when user has access to the community" do
        before do
          Feature.activate_user(:communities, seller)
          Feature.activate_user(:communities, other_product.user)
        end
  ...
  test "grants access to own community" do
            seller_context = SellerContext.new(user: seller, seller:)
            expect(subject).to permit(seller_context, community)
          end

  test "denies access to other seller's community" do
            seller_context = SellerContext.new(user: seller, seller:)
  ...
  test "grants access to community of purchased product" do
            purchase
            seller_context = SellerContext.new(user: buyer, seller:)
            expect(subject).to permit(seller_context, other_community)
          end

  test "denies access to community of unpurchased product" do
            seller_context = SellerContext.new(user: buyer, seller:)
            expect(subject).not_to permit(seller_context, community)
          end
        end
  ...
  test "denies access to seller's community" do
            seller_context = SellerContext.new(user: admin_for_seller, seller:)
            expect(subject).not_to permit(seller_context, community)
          end
  ...
  context_ "when user has no access to the community" do
        before do
          Feature.deactivate_user(:communities, seller)
          Feature.deactivate_user(:communities, other_product.user)
        end
  ...
  test "denies access to team members" do
          seller_context = SellerContext.new(user: admin_for_seller, seller:)
          expect(subject).not_to permit(seller_context, community)
        end
      end

  context_ "when community's resource is deleted" do
        before do
          Feature.activate_user(:communities, seller)
  ...
  context_ "when community chat is disabled" do
        before do
          Feature.activate_user(:communities, seller)
