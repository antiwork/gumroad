...
class PublicFileTest < ActiveSupport::TestCase
  self.described_class = PublicFile



  context_ PublicFile do
  ...
  context_ "#set_original_file_name" do
  test "sets original_file_name from file if not present" do
          public_file = build(:public_file, :with_audio, original_file_name: nil)
          public_file.valid?
          expect(public_file.original_file_name).to eq("test.mp3")
        end

  test "keeps existing original_file_name if present" do
  ...
  test "does not set original_file_name if file is not attached" do
  ...
  context_ "#set_default_display_name" do
  test "sets display_name from original_file_name if not present" do
  ...
  test "sets display_name to Untitled if original_file_name is a dotfile" do
  ...
  context_ "#set_file_group_and_file_type" do
  test "sets file_type and file_group based on original_file_name" do
  ...
  test "does not set file_type and file_group if original_file_name is nil" do
          public_file = build(:public_file, original_file_name: nil)
          public_file.valid?
  ...
  context_ "#set_public_id" do
  test "generates a public_id if not present" do
  ...
  test "keeps existing public_id if present" do
  ...
  context_ ".generate_public_id" do
  test "generates a unique 16-character long alphanumeric public_id" do
        public_id = described_class.generate_public_id
        expect(public_id).to match(/^[a-z0-9]{16}$/)
      end

  test "generates unique public_ids" do
  ...
  test "retries until finding a unique public_id" do
  ...
  test "returns empty hash if blob is nil" do
        public_file = build(:public_file)
        expect(public_file.metadata).to eq({})
      end
    end

  context_ "#scheduled_for_deletion?" do
  test "returns true if scheduled_for_deletion_at is present" do
        public_file = build(:public_file, scheduled_for_deletion_at: Time.current)
        expect(public_file).to be_scheduled_for_deletion
      end

  test "returns false if scheduled_for_deletion_at is nil" do
        public_file = build(:public_file, scheduled_for_deletion_at: nil)
        expect(public_file).not_to be_scheduled_for_deletion
      end
    end

  context_ "#schedule_for_deletion!" do
  test "sets scheduled_for_deletion_at to 10 days from now" do
