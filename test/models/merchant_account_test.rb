...
class MerchantAccountTest < ActiveSupport::TestCase
  self.described_class = MerchantAccount



  context_ MerchantAccount do
  context_ ".paypal" do
  test "returns records with the paypal charge processor id" do
  ...
  context_ ".stripe" do
  test "returns records with the stripe charge processor id" do
        MerchantAccount.destroy_all
  ...
  test "validates uniqueness of charge_processor_merchant_id when charge processor is stripe and is not a stripe connect account" do
  ...
  context_ "#is_managed_by_gumroad?" do
  test "returns true if user_id is not assigned" do
        merchant_account = create(:merchant_account, user_id: nil)
        expect(merchant_account.is_managed_by_gumroad?).to be(true)
      end

  test "returns false if user_id is assigned" do
  ...
  context_ "#can_accept_charges?" do
  test "returns true if account is not from one of the cross-border payouts countries" do
        merchant_account = create(:merchant_account)
        expect(merchant_account.can_accept_charges?).to be(true)
      end

  test "returns false if account is from one of the cross-border payouts countries" do
        merchant_account = create(:merchant_account, country: "TH")
        expect(merchant_account.can_accept_charges?).to be(false)
      end
    end

  context_ "#delete_charge_processor_account!", :vcr do
  test "marks the merchant account as deleted and clears the meta field" do
  ...
  test "marks the merchant account as deleted and does not clear the meta field if it is a stripe connect account" do
  ...
  context_ "#is_a_paypal_connect_account?" do
  test "returns true if charge_processor_id is PayPal otherwise false" do
  ...
  context_ "#stripe_rejected?" do
  test "returns true when stripe_disabled_reason begins with rejected." do
  ...
  test "returns false for non-rejected disabled reasons" do
  ...
  test "returns false when stripe_disabled_reason is blank" do
  ...
  test "returns the holder of funds for a known charge processor" do
  ...
  test "returns gumroad for a removed charge processor" do
