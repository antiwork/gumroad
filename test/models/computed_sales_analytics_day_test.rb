...
class ComputedSalesAnalyticsDayTest < ActiveSupport::TestCase
  self.described_class = ComputedSalesAnalyticsDay



  context_ ComputedSalesAnalyticsDay do
  context_ ".read_data_from_keys" do
  test "returns hash with sorted existing keys and parsed values" do
  ...
  context_ ".fetch_data_from_key" do
  test "creates record if the key does not exist, returns existing  data if it does" do
        expect do
  ...
  context_ ".upsert_data_from_key" do
  test "creates a record if it doesn't exist, update data if it does" do
        expect do
