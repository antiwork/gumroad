...
class UpsellPurchaseTest < ActiveSupport::TestCase
  self.described_class = UpsellPurchase



  context_ UpsellPurchase do
  context_ "validations" do
  context_ "when the upsell doesn't belong to the purchase's product" do
  ...
  context_ "when the upsell belongs to the purchase's product" do
  ...
  context_ "when the upsell purchase doesn't have an upsell variant for its upsell" do
  ...
  context_ "when the upsell purchase has an upsell variant for its upsell" do
  ...
  test "returns the upsell purchase encoded in an object" do
  ...
  context_ "when the upsell is a content upsell" do
          let(:seller) { create(:named_seller) }
  ...
  test "returns nil for selected_product" do
