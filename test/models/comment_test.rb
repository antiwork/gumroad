...
class CommentTest < ActiveSupport::TestCase
  self.described_class = Comment



  context_ Comment do
  context_ "validations" do
  context_ "content length" do
  context_ "when content is within the configured character limit" do
          subject(:comment) { build(:comment, commentable: create(:published_installment), content: "a" * 10_000) }

  test "marks the comment as valid" do
            expect(comment).to be_valid
          end
        end

  context_ "when content is bigger than the configured character limit" do
          subject(:comment) { build(:comment, commentable: create(:published_installment), content: "a" * 10_001) }

  test "marks the comment as invalid" do
  ...
  context_ "when content contains an adult keyword" do
  context_ "when the author is not a team member" do
  ...
  context_ "when the author is a team member" do
  ...
  context_ "when author name is ContentModeration" do
  ...
  context_ "when content does not contain adult keywords" do
  ...
  context_ "depth numericality" do
  ...
  context_ "when a comment exceeds maximum allowed depth" do
  ...
  context_ "when depth is set manually" do
          subject(:comment) { build(:comment, commentable:, parent: root_comment, ancestry_depth: 3) }

  test "marks the comment as valid and sets the depth according to its actual position in its ancestry tree" do
            expect(comment).to be_valid
  ...
  context_ "before_save" do
  context_ "#trim_extra_newlines" do
          subject(:comment) { build(:comment, commentable: create(:published_installment)) }

  test "trims unnecessary additional newlines" do
  ...
  context_ "after_commit" do
  context_ "#notify_seller_of_new_comment" do
  ...
  context_ "when a new comment is added" do
  context_ "when it is not a user submitted comment" do
              before do
                comment.comment_type = :flagged
              end

  test "does not send a notification to the seller" do
                expect do
                  comment.save!
                end.not_to have_enqueued_mail(CommentMailer, :notify_seller_of_new_comment)
              end
            end

  context_ "when it is not a root comment" do
              before do
  ...
  context_ "when it is authored by the seller" do
              before do
  ...
  context_ "when the seller has opted out of comments email notifications" do
              before do
  ...
  context_ "when a comment gets updated" do
  ...
  context_ "#mark_subtree_deleted!" do
      let(:commentable) { create(:published_installment) }
      let(:root_comment) { create(:comment, commentable:) }
      let!(:reply1) { create(:comment, parent: root_comment, commentable:) }
  ...
  test "soft deletes the comment along with its descendants" do
        expect do
          expect do
            expect do
              expect do
            ...
            end.not_to change { root_comment.reload.alive? }
          end.not_to change { reply2.reload.alive? }
        end.not_to change { reply_at_depth_3.reload.alive? }
  ...
  context_ "#mark_subtree_undeleted!" do
      let(:commentable) { create(:published_installment) }
      let(:root_comment) { create(:comment, commentable:) }
      let!(:reply1) { create(:comment, parent: root_comment, commentable:) }
      let!(:reply2) { create(:comment, parent: root_comment, commentable:) }
  ...
  test "marks the comment and its descendants undeleted" do
        expect do
          expect do
            expect do
