...
class AdminApiTokenTest < ActiveSupport::TestCase
  self.described_class = AdminApiToken



  context_ AdminApiToken do
  context_ ".mint!" do
  test "stores the token hash and returns the plaintext token once" do
  ...
  context_ ".mint_with_plaintext!" do
  test "returns the plaintext token and token row" do
        actor = create(:admin_user)
  ...
  context_ ".seed_legacy_admin_token!" do
  test "creates the legacy admin token from the configured shared token" do
        actor = create(:admin_user)
  ...
  test "does not create a token when the configured shared token is blank" do
  ...
  context_ ".authenticate" do
  test "returns an active token for the matching plaintext token" do
        actor = create(:admin_user)
        plaintext_token = described_class.mint!(actor_user_id: actor.id)
        admin_api_token = described_class.find_by!(actor_user: actor, token_hash: described_class.hash_token(plaintext_token))
  ...
  test "does not authenticate revoked or expired tokens" do
        actor = create(:admin_user)
  ...
  context_ "#legacy_admin_token?" do
  test "only treats the seeded legacy row as the legacy admin token" do
  ...
  context_ "#record_used!" do
  test "extends expiring tokens by 30 days capped at 90 days from creation" do
        actor = create(:admin_user)
  ...
  test "does not add expiry to service tokens" do
