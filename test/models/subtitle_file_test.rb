...
class SubtitleFileTest < ActiveSupport::TestCase
  self.described_class = SubtitleFile



  context_ SubtitleFile do
  context_ "validations" do
  context_ "file types" do
  ...
  test "displays an unsupported file type error message" do
            @subtitle.save
  ...
  context_ "when uploading an invalid type" do
  ...
  context_ "and subtitle is an S3 URL" do
            before do
  ...
  context_ "when uploading a valid type" do
  ...
  context_ "when updating an invalid type" do
          before do
