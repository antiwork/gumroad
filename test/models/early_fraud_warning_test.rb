...
class EarlyFraudWarningTest < ActiveSupport::TestCase
  self.described_class = EarlyFraudWarning



  context_ EarlyFraudWarning do
  context_ "validations" do
  context_ "for Purchase" do
        let(:purchase_efw) { create(:early_fraud_warning) }

  test "validates uniqueness of processor_id" do
  ...
  test "validates uniqueness of purchase" do
  ...
  context_ "when both a purchase and charge are present" do
  ...
  test "validates uniqueness of charge" do
  ...
  context_ "#update_from_stripe!" do
      let(:early_fraud_warning) { create(:early_fraud_warning) }

  test "calls the service" do
  ...
  context_ "when EarlyFraudWarning::UpdateService::AlreadyResolvedError is raised" do
  test "ignores the error" do
  ...
  context_ "#chargeable_refundable_for_fraud?" do
      shared_examples "for a Chargeable" do
  context_ "when eligible" do
  test "returns true" do
            expect(early_fraud_warning.chargeable_refundable_for_fraud?).to eq(true)
          end
        end

  context_ "when the purchase was created before the dispute window" do
          before do
  ...
  context_ "#purchase_for_subscription_contactable?" do
  ...
  context_ "when the purchase doesn't have a CustomerEmailInfo record" do
  test "returns false" do
            expect(early_fraud_warning.purchase_for_subscription_contactable?).to eq(false)
          end
        end

  context_ "when the purchase has a CustomerEmailInfo record" do
  context_ "when the CustomerEmailInfo has an eligible state" do
            before do
  ...
  context_ "when the risk level is not eligible" do
              before do
  ...
  context_ "when the fraud_type is not unauthorized use of card" do
              before do
  ...
  context_ "when the CustomerEmailInfo doesn't have an eligible state" do
            before do
  ...
  context_ "#associated_early_fraud_warning_ids_for_subscription_contacted" do
      let(:purchase) { create(:membership_purchase) }
  ...
  context_ "without associated purchases" do
  ...
  context_ "with associated purchases" do
  ...
  context_ "when the associated purchase early fraud warning is not resolved" do
  test "returns an empty array" do
              expect(
                early_fraud_warning.associated_early_fraud_warning_ids_for_subscription_contacted
              ).to eq([])
  ...
  context_ "when the associated purchase early fraud warning is resolved as ignored" do
            before do
  ...
  context_ "when the associated purchase early fraud warning is resolved as contacted" do
            before do
  ...
  test "returns the associated purchase early fraud warning ids" do
              expect(
                early_fraud_warning.associated_early_fraud_warning_ids_for_subscription_contacted
  ...
  context_ "when the associated purchase early fraud warning is associated with a charge" do
  ...
  context_ "#purchase_for_subscription" do
  context_ "for a Purchase" do
  ...
  context_ "when the purchase is for membership" do
          let(:purchase) { create(:membership_purchase) }
  ...
  context_ "when the charge has no membership purchases" do
  test "returns nil" do
            expect(early_fraud_warning.purchase_for_subscription).to be_nil
          end
        end

  context_ "when the charge has a membership purchase" do
  ...
  context_ "when there are no purchase email infos" do
  test "returns nil" do
          expect(early_fraud_warning.send(:receipt_email_info)).to be_nil
        end
      end

  context_ "when there are purchase email infos" do
  ...
  test "returns the last receipt email info" do
