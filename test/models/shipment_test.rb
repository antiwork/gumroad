...
class ShipmentTest < ActiveSupport::TestCase
  self.described_class = Shipment



  context_ Shipment do
  context_ "#shipped?" do
  test "returns false is shipped_at is nil" do
        expect(create(:shipment).shipped?).to be(false)
      end

  test "returns true is shipped_at is present" do
        expect(create(:shipment, shipped_at: 1.day.ago).shipped?).to be(true)
      end
    end

  context_ "#mark_as_shipped" do
  test "marks a shipment as shipped" do
  ...
  context_ "notify_sender_of_sale" do
      before do
        user = create(:user)
  ...
  test "sends sender email of receiver sale" do
        mail_double = double
  ...
  context_ "#calculated_tracking_url" do
      before do
        user = create(:user)
        link = create(:physical_product, user:)
        purchase = create(:physical_purchase, link:)
  ...
  test "returns the tracking_url if present" do
  ...
  test "returns the right url based on carrier and tracking_number when tracking_url is not present" do
  ...
  test "does not return anything if no tracking_url and no carrier" do
        @shipment.update(carrier: nil)
        expect(@shipment.calculated_tracking_url).to eq(nil)
      end

  test "does not return anything if no tracking_url and no tracking number" do
        @shipment.update(tracking_number: nil)
        expect(@shipment.calculated_tracking_url).to eq(nil)
      end

  test "does not return anything if no tracking_url and unrecognized carrier" do
