...
class PreorderLinkTest < ActiveSupport::TestCase
  self.described_class = PreorderLink



  context_ PreorderLink do
  context_ "#release!" do
      before do
  ...
  test "does not allow the product to be released if the release date hasn't arrived yet" do
        expect(@preorder_link.release!).to be(false)
        expect(@preorder_link.reload.state).to eq "unreleased"
      end

  test "allows the product to be released if the release date is within a minute" do
  ...
  test "does not allow the product to be released if the release date is not within a minute" do
        @preorder_link.update_attribute(:release_at, 70.seconds.from_now)
        expect(@preorder_link.release!).to be(false)
        expect(@preorder_link.reload.state).to eq "unreleased"
      end

  test "does not allow the product to be released if the product is banned" do
  ...
  test "doesn't allow the product to be released if the product is unpublished" do
        @preorder_link.update_attribute(:release_at, Time.current)
  ...
  test "allows the unpublished product to be released if it's being released manually" do
        @preorder_link.update_attribute(:release_at, Time.current)
        @preorder_link.link.update_column(:purchase_disabled_at, Time.current)
  ...
  test "doesn't allow the product to be released if the product is deleted" do
        @preorder_link.update_attribute(:release_at, Time.current)
  ...
  test "allows the product to be released if the release date is in the future but it's being released by the seller" do
        @preorder_link.is_being_manually_released_by_the_seller = true
        expect(@preorder_link.release!).to be(true)
        expect(@preorder_link.reload.state).to eq "released"
      end

  context_ "regular preorder" do
  test "does not allow the product to be released if the product does not have rich content" do
          @preorder_link.update_attribute(:release_at, Time.current)
  ...
  context_ "physical preorder" do
        before do
          @product.update!(is_physical: true, require_shipping: true)
          @product.save_files!([])
        end

  test "allows releasing the product even if it does not have any delivery content" do
          @preorder_link.update_attribute(:release_at, Time.current)
  ...
  context_ "when purchasing a preorder twice with different version selections", :vcr do
        before do
  ...
  test "releases the preorder and charges both purchases", :sidekiq_inline do
          @preorder_link.is_being_manually_released_by_the_seller = true
  ...
  test "marks the link as released" do
  ...
  test "adds the proper job to the queue when it comes time to charge the preorders" do
        @preorder_link.update_attribute(:release_at, Time.current)
        @preorder_link.release!
  ...
  test "returns the correct revenue", :vcr do
