...
class AssetPreviewTest < ActiveSupport::TestCase
  self.described_class = AssetPreview
  self.rspec_metadata = { vcr: true }



  context_ AssetPreview, :vcr do
  context_ "Attachment" do
  test "scales down a big image and keeps original" do
  ...
  test "fails if URL is not of a supported provider" do
        expect do
  ...
  test "works as expected with a public URL" do
        expect do
  ...
  test "works as expected when a URL with square brackets is encoded and passed as an argument" do
        expect do
  ...
  test "prevents non-http urls from being downloaded" do
        expect do
  ...
  test "blocks SSRF attempts to localhost", :skip_ssrf_stub do
        expect do
  ...
  test "blocks SSRF attempts to cloud metadata endpoint", :skip_ssrf_stub do
        expect do
  ...
  test "blocks SSRF attempts to private IP ranges", :skip_ssrf_stub do
        expect do
  ...
  context_ "product update on save" do
  test "updates the updated_at timestamp of the product after creating asset_preview" do
  ...
  test "properly sets position on creation if the previous preview is missing position" do
        product = create(:product)
  ...
  context_ "file attachment" do
      let!(:asset_preview) { create(:asset_preview) }

  context_ "with file attached" do
  test "returns proper width" do
          expect(asset_preview.width).to eq(1633)
          expect(asset_preview.display_width).to eq(670)
          expect(asset_preview.retina_width).to eq(1005)
        end

  test "returns proper height" do
          expect(asset_preview.height).to eq(512)
          expect(asset_preview.display_height).to eq(210)
        end

  context_ "#retina_variant" do
  test "falls back to original file URL when image processing times out" do
  ...
  context_ "#url" do
  test "returns retina variant" do
            expect(asset_preview.url).to match(asset_preview.retina_variant.key)
          end

  test "returns original file for non-image files" do
  ...
  test "works well with video types" do
            asset_preview.file.attach(Rack::Test::UploadedFile.new(
  ...
  context_ "#image_url?" do
  test "returns true for image assets" do
  ...
  test "returns nil for dangerous URLs" do
  ...
  context_ "#oembed_thumbnail_url" do
      let(:asset_preview) { build(:asset_preview) }

  test "returns nil when oembed is not present" do
        expect(asset_preview.oembed_thumbnail_url).to be_nil
      end

  test "returns nil for blank thumbnail URLs" do
        ["", " "].each do |blank_url|
  ...
  test "returns safe thumbnail URLs unchanged" do
  ...
  context_ "#oembed_url" do
      let(:asset_preview) { build(:asset_preview) }

  test "returns nil when oembed is not present or has no iframe" do
  ...
  test "handles protocol-relative and absolute URLs" do
        {
  ...
  test "adds platform-specific parameters" do
        {
