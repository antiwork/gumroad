...
class DropboxFileTest < ActiveSupport::TestCase
  self.described_class = DropboxFile



  context_ DropboxFile do
  ...
  context_ "#multipart_transfer_to_s3", :vcr do
  ...
  test "copies the file from Dropbox to S3" do
  ...
  context_ "#schedule_dropbox_file_analyze" do
  test "enqueues the job to transfer the file to S3" do
          create(:dropbox_file)
  ...
  context_ "#validate_dropbox_url!" do
  test "allows valid Dropbox URLs" do
        valid_urls = [
  ...
  test "rejects non-Dropbox URLs" do
        invalid_urls = [
