...
class CountryTest < ActiveSupport::TestCase
  self.described_class = Country


  context_ Country do
  context_ "#supports_stripe_cross_border_payouts?" do
  test "returns true if country only supports cross-border payouts via stripe otherwise returns false" do
  ...
  context_ "#can_accept_stripe_charges?" do
  test "returns true if country supports payments via stripe otherwise returns false" do
  ...
  context_ "#stripe_capabilities" do
  test "returns country-specific stripe capabilities" do
  ...
  context_ "#stripe_country_code" do
  test "maps US territory ISO codes to their parent country code expected by Stripe" do
        expect(Country.new("PR").stripe_country_code).to eq "US"
      end

  test "passes through every other country code unchanged" do
  ...
  context_ "#default_currency" do
  test "returns the currency which is set as default currency for all the accounts from the country" do
  ...
  context_ "#payout_currency" do
  test "returns the currency which is used for sending stripe payouts to the country" do
  ...
  context_ "#min_cross_border_payout_amount_local_cents" do
  test "returns a minimum payout amount cents in local currency for all supported cross-border countries" do
  ...
  test "returns the correct minimum payout amount in local currency cents for supported cross-border countries" do
  ...
  test "returns nil for regular (non cross-border) payout and unsupported countries" do
  ...
  context_ "#min_cross_border_payout_amount_usd_cents" do
  test "returns the min_cross_border_payout_amount_local_cents converted to usd cents " do
  ...
  test "returns 0 if min_cross_border_payout_amount_local_cents is nil" do
        regular_payout_countries = User::Compliance.const_get(:SUPPORTED_COUNTRIES) - Country.const_get(:CROSS_BORDER_PAYOUTS_COUNTRIES)
        regular_payout_countries.each do |country|
