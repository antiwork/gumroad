...
class ProductFilesArchiveTest < ActiveSupport::TestCase
  self.described_class = ProductFilesArchive



  context_ ProductFilesArchive do
  context_ "callbacks" do
  test "saves `digest` when transitioning to the 'in_progress' state" do
  ...
  context_ "#belongs_to_product_or_installment_or_variant" do
      before do
  ...
  test "raises no errors if only one of link/variant/installment is present" do
  ...
  test "raises an error if there are no associations or more associations than expected" do
  ...
  context_ ".latest_ready_entity_archive" do
  test "returns the correct entity archive" do
        product = create(:product)
  ...
  context_ ".latest_ready_folder_archive" do
  test "returns the correct folder archive" do
        product = create(:product)
        file1 = create(:product_file, display_name: "File 1")
        file2 = create(:product_file, display_name: "File 2")
        file3 = create(:product_file, display_name: "File 3")
  ...
  test "only returns true for folder archives" do
        product = create(:product)
        file1 = create(:product_file, display_name: "File 1")
        file2 = create(:product_file, display_name: "File 2")
        file3 = create(:product_file, display_name: "File 3")
  ...
  context_ "#files_digest" do
  context_ "when there is no rich content" do
  ...
  context_ "entity archives" do
  test "returns the same digest when nothing has changed" do
          product = create(:product_with_files)
  ...
  context_ "folder archives" do
  test "returns the correct digests" do
          product = create(:product)
          file1 = create(:product_file, display_name: "File 1")
          file2 = create(:product_file, display_name: "File 2")
          file3 = create(:product_file, display_name: "File 3")
  ...
  context_ ".entity_archives" do
  test "only returns entity archives" do
  ...
  context_ ".folder_archives" do
  test "only returns folder archives" do
  ...
  test "is create-able through an installment" do
      post = create(:installment)
  ...
  test "is create-able through a variant" do
      variant = create(:variant)
  ...
  test "schedules an UpdateProductFilesArchiveWorker job" do
      link = create(:product)
      link.product_files << create(:product_file)
      link.product_files << create(:product_file)
      link.save
  ...
  context_ "when s3 directory is empty" do
  test "has unique s3_key for the same product" do
        product = create(:product)
  ...
  test "is unique for different archives of the same product" do
        product = create(:product)
  ...
  context_ "#set_url_if_not_present" do
  test "sets the url if not present" do
        product = create(:product)
        product_files_archive = create(:product_files_archive_without_url, link: product)
        product_files_archive.set_url_if_not_present
        expect(product_files_archive.url).to start_with("#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachments_zipped/")
        expect(product_files_archive.url.split("/").last).to eq("The_Works_of_Edgar_Gumstein.zip")
      end
    end

  context_ "#construct_url" do
  test "uses the entity name for an entity archive" do
        ...
        expect(entity_archive.url).to start_with("#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachments_zipped/")
        expect(entity_archive.url.split("/").last).to eq("Product_name.zip")
      end

  test "uses the folder name for a folder archive" do
        product = create(:product)
        file1 = create(:product_file)
        file2 = create(:product_file)
        product.product_files = [file1, file2]
        ...
        expect(folder_archive.url).to start_with("#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachments_zipped/")
        expect(folder_archive.url.split("/").last).to eq("Folder_1.zip")
      end
    end

  context_ "#needs_updating?" do
      before do
        @product = create(:product)
  ...
  test "returns false when the files and folders in the rich content are unchanged" do
        expect(@product_files_archive.needs_updating?(@files)).to be(false)
      end

  test "returns true when a file changes but the filename stays the same" do
        folder_id = SecureRandom.uuid
  ...
  test "returns true when a file is renamed" do
        @product_file1.update!(display_name: "New file name")

        expect(@product_files_archive.reload.needs_updating?(@files)).to be(true)
      end

  test "returns true when a page containing files is renamed" do
        @page1.update!(title: "New title")

        expect(@product_files_archive.reload.needs_updating?(@files)).to be(true)
      end

  test "returns true when a page containing files is deleted" do
        @page1.mark_deleted!

        expect(@product_files_archive.reload.needs_updating?(@files)).to be(true)
      end

  test "returns true when a page is added containing files" do
  ...
  test "returns false when a page is added without files" do
        create(:rich_content, entity: @product, description: [
  ...
  test "returns true when a file group's name is changed" do
  ...
  test "returns true when a file group is deleted" do
  ...
  test "returns true when a file group is added" do
        @product_file6 = create(:product_file, display_name: "Sixth file")
        @product.product_files << @product_file6
        @product_files_archive.product_files << @product_file6
  ...
  test "returns true when a file is added to a file group" do
        @product_file6 = create(:product_file, display_name: "Sixth file")
        @product.product_files << @product_file6
        @product_files_archive.product_files << @product_file6
  ...
  test "returns true when a file is removed from a file group" do
  ...
  test "returns true when a file is moved from one file group to another" do
  ...
  test "returns true when a file is moved out of a file group " do
  ...
  test "returns false when a file is reordered within a file group" do
  ...
  test "returns false on page name changes" do
  ...
  test "returns false when unrelated folders change" do
  ...
  test "returns false when top-level files are modified" do
          @product_file1.update!(display_name: "a new name")

          expect(@folder1_archive.reload.needs_updating?(@files)).to be(false)
        end

  test "returns true when files are added" do
          @product_file6 = create(:product_file, display_name: "Sixth file")
          @product.product_files << @product_file6
  ...
  test "returns true when folder files are deleted" do
  ...
  test "returns true if the folder name changes" do
          rich_content = RichContent.find_by(id: @page2.id)
          rich_content.description.first["attrs"]["name"] = "New folder name!"
          rich_content.save!
  ...
  context_ "#rich_content_provider" do
  context_ "when associated with a product" do
  test "returns the associated product" do
          product_files_archive = create(:product_files_archive)
  ...
  test "returns the associated variant" do
  ...
  context_ "when associated with an installment" do
  test "returns nil" do
