...
class UtmLinkTest < ActiveSupport::TestCase
  ...
  context_ "target_resource_id" do
  context_ "when target_resource_type is product_page" do
          subject(:utm_link) { build(:utm_link, target_resource_type: :product_page) }

  test "is invalid when absent" do
            expect(subject).to be_invalid
            expect(subject.errors[:target_resource_id]).to include("can't be blank")
          end

  test "is valid when present" do
            subject.target_resource_id = create(:product).id
            expect(subject).to be_valid
          end
        end

  context_ "when target_resource_type is post_page" do
  ...
  context_ "when target_resource_type is profile_page" do
  ...
  context_ "when target_resource_type is subscribe_page" do
          subject(:utm_link) { build(:utm_link, target_resource_type: :subscribe_page) }

  test "is valid when absent" do
            expect(subject).to be_valid
          end
        end
      end
  ...
  context_ "last_click_at_is_same_or_after_first_click_at" do
  context_ "when last_click_at is not present" do
  test "is valid" do
  ...
  context_ "when last_click_at is present" do
  test "is invalid if first_click_at is nil" do
  ...
  test "is invalid if last_click_at is before first_click_at" do
  ...
  test "is valid if last_click_at is same as first_click_at" do
  ...
  test "is valid if last_click_at is after first_click_at" do
  ...
  context_ "utm_fields_are_unique" do
        let(:seller) { create(:user) }
        let!(:product) { create(:product, user: seller) }

  test "prevents duplicate UTM parameters for the same target resource and seller" do
          create(:utm_link,
                 seller:,
  ...
  test "allows same UTM parameters for different target resources of the same seller" do
          create(:utm_link,
                 seller:,
                 target_resource_type: "product_page",
                 target_resource_id: product.id,
  ...
  test "allows duplicate UTM parameters for the same target resource for different sellers" do
          create(:utm_link,
                 seller:,
                 target_resource_type: "product_page",
                 target_resource_id: product.id,
  ...
  test "allows duplicate UTM parameters if original is deleted" do
          create(:utm_link,
                 seller:,
                 target_resource_type: "product_page",
                 target_resource_id: product.id,
  ...
  test "allows updating a UTM link without triggering UTM parameters' uniqueness validation" do
  ...
  context_ "scopes" do
  context_ "enabled" do
  test "returns only enabled utm links" do
  ...
  context_ "active" do
  test "returns only alive and enabled utm links" do
  ...
  context_ "#set_permalink" do
  test "sets the permalink before validation" do
          utm_link = build(:utm_link)
          utm_link.valid?
          expect(utm_link.permalink).to be_present
        end

  test "does not set the permalink if it is already set" do
          utm_link = create(:utm_link)
  ...
  context_ "#enabled?" do
  test "returns true when utm link is not disabled" do
        expect(create(:utm_link)).to be_enabled
      end

  test "returns false when utm link is disabled" do
        expect(create(:utm_link, disabled_at: Time.current)).not_to be_enabled
      end
    end

  context_ "#active?" do
  test "returns true when utm link is enabled and not deleted" do
  ...
  test "returns false when utm link is deleted" do
        expect(create(:utm_link, deleted_at: Time.current)).not_to be_active
      end
    end

  context_ "#mark_disabled!" do
  test "marks the utm link as disabled" do
        utm_link = create(:utm_link)
  ...
  context_ "#mark_enabled!" do
  test "marks the utm link as enabled" do
  ...
  context_ "#short_url" do
  test "returns the short URL" do
        utm_link = create(:utm_link)
  ...
  context_ "#utm_url" do
  context_ "when target_resource_type is profile_page" do
  test "returns the seller's profile URL with UTM parameters" do
  ...
  context_ "when target_resource_type is product_page" do
  test "returns the product's long URL with UTM parameters" do
  ...
  context_ "when target_resource_type is post_page" do
  test "returns the post's full URL with UTM parameters" do
  ...
  context_ "#target_resource" do
  test "returns correct target resource" do
        seller = create(:user)
  ...
  context_ "#default_title" do
  context_ "when target is product page" do
  ...
  test "returns appropriate title" do
          expect(utm_link.default_title).to eq("Product — My Product (auto-generated)")
        end
      end

  context_ "when target is post page" do
  ...
  context_ "when target is profile page" do
  ...
  context_ "when target is subscribe page" do
  ...
  context_ ".generate_permalink" do
  test "generates an 8-character alphanumeric permalink" do
        permalink = described_class.generate_permalink
        expect(permalink).to match(/^[a-z0-9]{8}$/)
      end

  test "generates unique permalinks" do
  ...
  test "retries until finding a unique permalink" do
  ...
  test "raises an error after max retries" do
  ...
  context_ ".polymorphic_class_for" do
  context_ "when target_resource_type is product_page" do
  test "returns Link" do
          expect(described_class.polymorphic_class_for("product_page")).to eq(Link)
        end
      end

  context_ "when target_resource_type is post_page" do
  test "returns Installment" do
