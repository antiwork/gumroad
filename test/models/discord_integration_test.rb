...
class DiscordIntegrationTest < ActiveSupport::TestCase
  self.described_class = DiscordIntegration



  context_ DiscordIntegration do
  ...
  test "returns true if a discord integration is enabled on the product" do
  ...
  test "returns false if a discord integration is not enabled on the product" do
  ...
  test "returns false if a deleted discord integration exists on the product" do
        product = create(:product, active_integrations: [create(:discord_integration)])
        purchase = create(:purchase, link: product)
  ...
  context_ ".discord_user_id_for" do
  test "returns discord_user_id for a purchase with an enabled discord integration" do
  ...
  test "returns nil for a purchase without an enabled discord integration" do
  ...
  test "returns nil for a purchase without an active discord integration" do
  ...
  test "returns nil for a purchase with a deleted discord integration" do
        purchase_integration = create(:discord_purchase_integration)
  ...
  test "disconnects bot from server if server id is valid" do
  ...
  test "fails if bot is not added to server" do
        WebMock.stub_request(:delete, "#{Discordrb::API.api_base}/users/@me/guilds/#{server_id}").
          with(headers: request_header).
  ...
  test "returns true if the server has been deleted" do
        WebMock.stub_request(:delete, "#{Discordrb::API.api_base}/users/@me/guilds/#{server_id}").
          with(headers: request_header).
  ...
  test "returns true if both integrations have the same server id" do
        expect(discord_integration.same_connection?(same_connection_discord_integration)).to eq(true)
      end

  test "returns false if both integrations have different server ids" do
