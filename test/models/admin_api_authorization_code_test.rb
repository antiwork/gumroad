...
class AdminApiAuthorizationCodeTest < ActiveSupport::TestCase
  self.described_class = AdminApiAuthorizationCode



  context_ AdminApiAuthorizationCode do
  context_ ".code_challenge_for" do
  test "uses S256 raw URL-safe base64 to match the Gumroad CLI" do
  ...
  context_ ".create_for!" do
  test "stores a hashed code with a 60 second expiry" do
  ...
  context_ ".exchange!" do
  test "mints an admin token and marks the code used when the PKCE verifier matches" do
        actor = create(:admin_user)
  ...
  test "rejects mismatched, expired, and already-used codes" do
