...
class SentPostEmailTest < ActiveSupport::TestCase
  self.described_class = SentPostEmail



  context_ SentPostEmail do
  ...
  context_ ".missing_emails" do
  test "returns array of emails currently not stored" do
        create(:sent_post_email, post:, email: "foo")
  ...
  context_ ".ensure_uniqueness" do
  test "runs block if we successfully created a unique record for that post and email" do
  ...
  test "does not raise error if email is blank" do
        counter = 0
  ...
  context_ ".insert_all_emails" do
  test "inserts all emails, even if some already exist, and returns newly inserted" do
        create(:sent_post_email, post:, email: "foo")
