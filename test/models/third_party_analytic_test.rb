...
class ThirdPartyAnalyticTest < ActiveSupport::TestCase
  self.described_class = ThirdPartyAnalytic



  context_ ThirdPartyAnalytic do
    before do
      @user = create(:user)
      @product = create(:product, user: @user)
    end

  context_ "#save_third_party_analytics" do
  context_ "new analytics" do
  ...
  context_ "when two snippets have the same location" do
  test "doesn't allow them to belong to the same user" do
            params = [
              {
                name: "Snippet 1",
                location: "product",
  ...
  test "doesn't allow them to belong to the same product" do
            params = [
              {
                name: "Snippet 1",
                location: "product",
  ...
  context_ "when two snippets have different locations" do
  test "allows them to belong to the same user" do
            params = [
              {
                name: "Snippet 1",
                location: "product",
  ...
  context_ "existing analytics" do
        before do
          @third_party_analytics = create(:third_party_analytic, user: @user, link: nil)
        end

  test "updates the new third_party_analytic for creator" do
          params = [{
  ...
  test "updates the new third_party_analytic for product" do
  ...
  test "deletes the users third_party_analytics" do
  ...
  context_ "#clear_related_products_cache" do
      before do
        create(:product, user: @user)
        @tpa = create(:third_party_analytic, user: @user, link: @product)
      end

  test "calls cache invalidate on all user's links" do
