...
class ModelsInstallmentInstallmentClassMethodsTest < ActiveSupport::TestCase



  context_ "InstallmentClassMethods"  do
    before do
  ...
  context_ ".product_or_variant_with_sent_emails_for_purchases" do
  test "returns live product or variant installments that have been emailed to those purchasers" do
  ...
  context_ ".seller_with_sent_emails_for_purchases" do
  test "returns live seller installments that have been emailed to those purchasers" do
        product = create(:product)
  ...
  context_ ".profile_only_for_products" do
  test "returns live profile-only product posts for the given product IDs" do
  ...
  context_ ".profile_only_for_variant_ids" do
  test "returns live profile-only variant posts for the given variant IDs" do
        product = create(:product)
  ...
  context_ ".profile_only_for_sellers" do
  test "returns live profile-only seller posts for the given seller IDs" do
        seller1 = create(:user)
        seller2 = create(:user)
  ...
  context_ ".for_products" do
  test "returns live, non-workflow product-posts for the given products" do
        product1 = create(:product)
        product2 = create(:product)
  ...
  context_ ".for_variants" do
  test "returns live, non-workflow variant-posts for the given variants" do
  ...
  context_ ".for_sellers" do
  test "returns live, non-workflow seller-posts for the given sellers" do
        seller1 = create(:user)
        seller2 = create(:user)
  ...
  context_ ".past_posts_to_show_for_products" do
      before do
  ...
  test "returns live product posts for products with should_show_all_posts enabled" do
  ...
  test "excludes certain post IDs, if provided" do
  ...
  context_ ".past_posts_to_show_for_variants" do
      before do
        enabled_product = create(:product, should_show_all_posts: true)
  ...
  test "returns live variant posts for variants whose products have should_show_all_posts enabled" do
  ...
  context_ ".seller_posts_for_sellers" do
      before do
        @seller = create(:user)
  ...
  test "returns live seller posts for the given seller IDs" do
  ...
  context_ ".emailable_posts_for_purchase" do
  test "returns the product-, variant-, and seller-type posts for the purchase where send_emails is true" do
        product = create(:product)
        variant = create(:variant, variant_category: create(:variant_category, link: product))
        purchase = create(:purchase, link: product, variant_attributes: [variant])
        posts = [
  ...
  context_ ".filter_by_product_id_if_present" do
      before do
  ...
  test "returns the proper product updates if filtered by product ID" do
        ...
        expect(product_filtered_posts).not_to include(@audience_post)
        expect(product_filtered_posts).not_to include(@another_product_post)
      end

  test "does not apply any scope if no product_id present" do
  ...
  context_ ".missed_for_purchase" do
      before do
        @creator = create(:user)
  ...
  test "returns only the posts sent by seller in case there are posts belonging to other user" do
  ...
  test "includes posts sent to customers of multiple products if it includes the bought product" do
  ...
  test "includes posts sent to all customers" do
  ...
  test "includes posts sent to audience" do
  ...
  test "does not include post sent to customers of multiple products if it is already sent to the purchase email" do
  ...
  test "does not include profile-only posts" do
  ...
  test "does not include workflow installments for different variants" do
  ...
  test "includes workflow installments for matching variants" do
        variant_basic = create(:variant, variant_category: create(:variant_category, link: @product))
