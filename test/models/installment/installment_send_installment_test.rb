...
class ModelsInstallmentInstallmentSendInstallmentTest < ActiveSupport::TestCase



  context_ "InstallmentSendInstallment"  do
    before do
  ...
  context_ "published installments" do
          before do
  ...
  context_ "send_installment_from_workflow_for_purchase" do
      before do
        @seller = create(:user)
  ...
  test "sends a purchase_installment email for the purchase" do
  ...
  test "respects can_contact for purchase installments" do
  ...
  test "does not send a purchase_installment if buyer got the installment from another purchase" do
  ...
  test "sends a purchase_installment if buyer got the installment from another purchase but it has since been refunded" do
        link2 = create(:product, user: @seller)
        purchase3 = create(:purchase, link: link2, email: "tuhinA@gumroad.com", created_at: 2.weeks.ago, price_cents: 100)
        create(:creator_contacting_customers_email_info, installment: @installment, purchase: purchase3)
  ...
  test "sends a purchase_installment email for a purchase of a variant with a workflow" do
          expect(PostSendgridApi).to receive(:process).with(
  ...
  test "does not send a purchase_installment email for a variant that was not purchased" do
  ...
  context_ "multi-product or variant workflows" do
        before do
  ...
  test "sends a purchase_installment email if the purchase is for the targeted product" do
  ...
  test "sends a purchase_installment email if the purchase is for the targeted variant" do
  ...
  test "does not send a purchase_installment email if the purchase is not for the targeted product" do
  ...
  test "does not send a purchase_installment email if the purchase is not for the targeted variant" do
  ...
  test "does not do anything if purchase is recurring payment" do
  ...
  test "does not do anything if subscription is cancelled" do
          @sub.unsubscribe_and_fail!
          expect(PostSendgridApi).not_to receive(:process)
          @installment.send_installment_from_workflow_for_purchase(@purchase.id)
        end

  test "queues installment for original subscription purchase and alive subscription" do
          expect(PostSendgridApi).to receive(:process).with(
            post: @installment,
  ...
  context_ "when subscription plan has changed" do
          before do
  ...
  test "queues installment for the current original purchase" do
            expect(PostSendgridApi).to receive(:process).with(
              post: @installment,
  ...
  context_ "and tier has changed" do
  test "does not queue installment for a workflow belonging to the old tier" do
  ...
  test "sends a purchase_installment if buyer got the installment from another subscription but it has since been deactivated" do
  ...
  context_ "when subscription was reactivated" do
          before do
  ...
  context_ "when it is too early to send the workflow post" do
            before do
  ...
  test "schedules it to be sent in the future" do
              @installment_2.send_installment_from_workflow_for_purchase(@purchase.id)
  ...
  test "sends the next workflow post email when the previous one was deleted" do
  ...
  test "sends the next workflow post email when the previous one was sent on time" do
            expect(PostSendgridApi).to receive(:process).with(
              post: @installment,
              recipients: [{ email: @purchase.email, purchase: @purchase }],
              cache: anything
  ...
  test "does not send purchase_installment to chargedback purchases" do
  ...
  test "sends a message to the giftee" do
          expect(PostSendgridApi).to receive(:process).with(
            post: @gift_post,
  ...
  context_ "send_installment_from_workflow_for_follower" do
      before do
  ...
  test "sends a follower_installment email for the purchase" do
  ...
  test "doesn't send email if following was deleted" do
        @follower.mark_deleted!
        expect(PostSendgridApi).not_to receive(:process)
        @installment.send_installment_from_workflow_for_follower(@follower.id)
      end

  test "does not send email if follower has not confirmed" do
