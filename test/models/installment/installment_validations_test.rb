...
class ModelsInstallmentInstallmentValidationsTest < ActiveSupport::TestCase



  context_ "InstallmentValidations" do
  context_ "#validate_sending_limit_for_sellers" do
      before do
  ...
  context_ "when seller is sending less than 100 emails" do
  ...
  context_ "when seller has less than $100 revenue" do
  test "allows to delete the post that was sent to more than 100 email addresses" do
          @installment.save!
  ...
  context_ "when seller is sending more than 100 emails" do
  context_ "when seller has less than $100 sales" do
  test "returns minimum sales error" do
            allow(@installment).to receive(:audience_members_count).and_return(Installment::SENDING_LIMIT + 1)
  ...
  context_ "for an abandoned cart workflow installment" do
  test "returns true with no errors" do
  ...
  context_ "when seller has $100 sales" do
  test "returns true with no errors" do
  ...
  context_ "field validations" do
  test "when name is longer than 255 characters" do
  ...
  test "disallows records with no message" do
  ...
  test "disallows records with empty HTML-only message content" do
        installment = build(
          :installment,
          name: "installment1",
          message: "<p><br></p>",
  ...
  context_ "#shown_on_profile_only_for_confirmed_users" do
  test "allows creating, updating, and deleting non-profile posts belonging to unconfirmed users" do
  ...
  test "disallows creating profile posts by unconfirmed users" do
        product = create(:product, user: create(:unconfirmed_user))
  ...
  test "disallows updating a post to be a profile post belonging to an unconfirmed user" do
        product = create(:product, user: create(:unconfirmed_user))
        post = create(:installment, link: product)
        expect(post).to be_valid
  ...
  test "allows soft-deleting a profile post belonging to an unconfirmed user" do
        post = create(:installment, shown_on_profile: true)
        post.seller.update!(confirmed_at: nil)
  ...
  test "allows deleting a profile post belonging to an unconfirmed user" do
        post = create(:installment, shown_on_profile: true)
        post.seller.update!(confirmed_at: nil)
  ...
  test "allows creating, updating, and deleting profile posts belonging to confirmed users" do
        post = create(:installment, shown_on_profile: true)
        expect(post).to be_valid
  ...
  context_ "#published_at_cannot_be_in_the_future" do
      before do
        @post = build(:installment, name: "sample name", message: "sample message")
      end

  test "allows published_at to be nil" do
        expect(@post.published_at).to be_nil
        expect(@post).to be_valid
      end

  test "allows published_at to be in the past" do
        @post.published_at = Time.current
        expect(@post).to be_valid
      end

  test "disallows published_at to be in the future" do
