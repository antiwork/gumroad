...
class CallTest < ActiveSupport::TestCase
  ...
  context_ "normalizations", :freeze_time do
  ...
  test "validates that start_time is before end_time" do
  ...
  test "validates that start_time and end_time are present" do
  ...
  test "validates that the purchased product is a call" do
  ...
  test "validates that the selected time is allowed by #call_limitation_info" do
  ...
  test "validates that the selected time is available and not yet taken" do
        call = build(:call, link:)
  ...
  test "does not validate selected time availability for gift receiver purchases" do
  ...
  context_ "scopes", :freeze_time do
      before { travel_to(DateTime.parse("May 1 2024 UTC")) }
  ...
  context_ ".occupies_availability" do
  ...
  test "returns calls that haven't ended" do
          expect(Call.upcoming).to contain_exactly(not_started, started_but_not_ended)
        end
      end

  context_ ".ordered_chronologically" do
  ...
  test "returns calls that start on the given time's date in the given time zone" do
  ...
  context_ ".overlaps_with" do
        let!(:call_1_to_2) { create(:call, start_time: time_1, end_time: time_2, link:) }
        let!(:call_3_to_4) { create(:call, start_time: time_3, end_time: time_4, link:) }

  test "returns calls that overlap with the given time range" do
  ...
  context_ "#formatted_time_range" do
  ...
  test "returns the formatted time range" do
        expect(call.formatted_time_range).to eq("02:00 AM - 03:00 AM PST")
      end
    end

  context_ "#formatted_date_range" do
  context_ "when start and end times are on the same day" do
        let(:call) { create(:call, :skip_validation, start_time: DateTime.parse("January 1 2024 10:00"), end_time: DateTime.parse("January 1 2024 11:00")) }
  ...
  context_ "when the purchase has been refunded" do
  ...
  context_ "when the purchase failed" do
  ...
  context_ "when the purchase is successful" do
  ...
  context_ "scheduling reminder emails" do
  context_ "when the call is less than 24 hours away" do
        let(:start_time) { 23.hours.from_now }
        let(:end_time) { start_time + 1.hour }
        let(:call) { build(:call, start_time:, end_time:, link:) }

  test "does not schedule reminder emails" do
          expect do
            call.save!
  ...
  context_ "when the call is more than 24 hours away" do
        let(:start_time) { 25.hours.from_now }
        let(:end_time) { start_time + 1.hour }
        let(:call) { build(:call, start_time:, end_time:, link:) }

  test "schedules reminder emails" do
          expect do
            call.save!
  ...
  context_ "when the call is for a gift sender purchase" do
        let(:start_time) { 25.hours.from_now }
        let(:end_time) { start_time + 1.hour }
  ...
  test "schedules google calendar invites" do
        expect do
          call.save!
  ...
  test "does not schedule google calendar invites if the call is not linked to a google calendar integration" do
        expect do
          call2.save!
