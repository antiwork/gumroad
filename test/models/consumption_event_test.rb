...
class ConsumptionEventTest < ActiveSupport::TestCase
  self.described_class = ConsumptionEvent



  context_ ConsumptionEvent do
  context_ ".create_event!" do
  ...
  test "creates an event with required parameters" do
  ...
  test "raises an error if a required parameter is missing" do
        required_params.each_key do |key|
          expect { described_class.create_event!(**required_params.except(key)) }.to raise_error(KeyError)
        end
      end

  test "assigns default values to optional parameters if they are not provided" do
  ...
  test "uses provided values for optional parameters when available" do
        other_params = {
