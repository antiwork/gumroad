...
class PaymentTest < ActiveSupport::TestCase
  self.described_class = Payment



  context_ Payment do
  context_ "mark" do
  test "sets the appropriate state" do
  ...
  test "raises an error on invalid state" do
        payment = create(:payment)
        expect do
  ...
  context_ "when the processor is PAYPAL" do
  test "allows a transition from processing to unclaimed" do
          payment = create(:payment, processor: PayoutProcessorType::PAYPAL)
          payment.mark("unclaimed")
          expect(payment.reload.state).to eq "unclaimed"
        end

  test "allows a transition from unclaimed to cancelled and marks balances as paid" do
          creator = create(:user)
  ...
  test "marks balances as unpaid when a processing payment transitions directly to reversed" do
          creator = create(:user)
          merchant_account = create(:merchant_account_paypal, user: creator)
          balance = create(:balance, user: creator, state: "processing", merchant_account:)
  ...
  test "marks balances as unpaid when a processing payment transitions directly to returned" do
          creator = create(:user)
          merchant_account = create(:merchant_account_paypal, user: creator)
          balance = create(:balance, user: creator, state: "processing", merchant_account:)
          payment = create(:payment, state: "processing", balances: [balance], processor: PayoutProcessorType::PAYPAL)
  ...
  context_ "when the processor is STRIPE" do
  test "prevents a transition from processing to unclaimed" do
  ...
  context_ "when transitioning to completed" do
  ...
  test "generates default abandoned cart workflow for the user" do
  ...
  test "does not generate workflow if user is nil" do
          payment.user = nil
  ...
  context_ "send_cannot_pay_email" do
  ...
  test "sends the cannot pay email to the creator and sets the payout_date_of_last_payment_failure_email for user" do
        expect do
  ...
  test "does not send the cannot pay email if payout_date_of_last_payment_failure_email is same or newer than current payout date" do
        ...
        end.not_to have_enqueued_mail(ContactingCreatorMailer, :cannot_pay).with(payment.id)

        expect(compliant_creator.reload.payout_date_of_last_payment_failure_email.to_s).to eq(payment.payout_period_end_date.to_s)
      end
    end

  context_ "send_payout_failure_email" do
      let(:compliant_creator) { create(:user, user_risk_state: "compliant") }
  ...
  test "sends the payout failure email to the creator and sets the payout_date_of_last_payment_failure_email for user" do
        expect do
  ...
  test "does not send the payout failure email if failure_reason is cannot_pay" do
  ...
  context_ ".failed scope" do
  test "responds" do
        expect(Payment).to respond_to(:failed)
      end

  test "only returns failed payments" do
        create(:payment)
  ...
  test "returns failed payments sorted descending by id" do
  ...
  context_ "emails" do
  context_ "mark returned" do
  context_ "if already completed" do
  ...
  context_ "if not yet completed" do
          let(:payment) { create(:payment, state: "processing", processor: PayoutProcessorType::ACH, processor_fee_cents: 0) }

  test "does not send an email to the creator" do
            expect do
              payment.mark_returned!
            end.not_to have_enqueued_mail(ContactingCreatorMailer, :payment_returned).with(payment.id)
          end
        end
      end

  context_ "mark failed with no reason" do
        let(:payment) { create(:payment, state: "processing", processor: PayoutProcessorType::ACH, processor_fee_cents: 0) }
  ...
  context_ "mark failed with reason cannot pay" do
  ...
  context_ "#humanized_failure_reason" do
  context_ "when the processor is STRIPE" do
  test "returns the value from failure_reason" do
  ...
  context_ "when the processor is PAYPAL" do
  test "returns the full failure message" do
  ...
  test "returns `nil` when the failure_reason value is absent" do
  ...
  context_ "#credit_amount_cents" do
  test "does not include credits created for refund fee retention" do
        creator = create(:user)
  ...
  context_ "#sync_with_payout_processor" do
  context_ "when processor is PayPal" do
        before do
          @payment = create(:payment, processor: PayoutProcessorType::PAYPAL)
        end

  test "calls #sync_with_paypal if state is non-terminal" do
  ...
  test "calls #sync_with_stripe if state is non-terminal" do
          %w(creating processing unclaimed completed failed cancelled returned reversed).each do |payment_state|
            if payment_state == "completed"
              @payment.txn_id = "12345"
              @payment.processor_fee_cents = 10
  ...
  context_ "#sync_with_stripe" do
  context_ "when payment is stuck in creating for over 24 hours without Stripe IDs" do
  test "fails the payment and reverses internal transfer without sending failure email" do
  ...
  context_ "when payment is stuck in creating for over 24 hours with Stripe IDs" do
  test "retrieves Stripe payout status and transitions accordingly" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "creating",
  ...
  context_ "when Stripe payout status is paid" do
  test "marks the payment as completed with arrival date" do
  ...
  context_ "when Stripe payout status is failed" do
  test "marks the payment as failed with failure code, reverses transfer, and sends failure email" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  test "uses a default failure reason when no failure_code is present" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  context_ "when Stripe payout status is canceled" do
  test "marks the payment as cancelled and reverses transfer without sending failure email" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  context_ "when Stripe payout is still in transit" do
  test "does not change the payment state" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  context_ "when Stripe API raises an error" do
  test "logs the error and adds it to payment errors" do
          payment = create(:payment, processor: PayoutProcessorType::STRIPE, state: "processing",
  ...
  context_ "when processor is not Stripe" do
  ...
  context_ "when payment is in a terminal state" do
  test "does nothing for failed payments" do
  ...
  context_ "#sync_with_paypal" do
  context_ "when the payout is not created in the split mode" do
  test "fetches and sets the new payment state, txn_id, correlation_id, and fee from PayPal" do
  ...
  test "marks the payment as failed if no corresponding txn is found on PayPal" do
  ...
  test "does not change the payment if multiple txns are found on PayPal" do
  ...
  context_ "when the payout is created in the split mode" do
        let(:payment) do
          # Payout was sent out
  ...
  test "fetches and sets the new payment status from PayPal for all split parts" do
          expect(PaypalPayoutProcessor).to(
  ...
  test "adds an error if not all split parts statuses are same" do
          expect(PaypalPayoutProcessor).to(
            receive(:get_latest_payment_state_from_paypal).with(100,
                                                                "02P",
                                                                payment.created_at.beginning_of_day - 1.day,
  ...
  test "includes all successful sales" do
  ...
  test "returns sales sorted by created_at desc" do
  ...
  test "returns empty array when no successful sales" do
        sales = payment.successful_sales
        expect(sales).to be_empty
      end
    end

  context_ "#refunded_sales" do
      let(:user) { create(:user) }
      let(:product) { create(:product, user: user) }
      let(:balance) { create(:balance, user: user) }
      let(:payment) { create(:payment, user: user, balances: [balance], payout_period_end_date: 3.days.ago) }

  test "includes only refunded sales from associated balances" do
        successful_sale = create(:purchase, seller: user, link: product, purchase_success_balance: balance)
  ...
  test "returns empty array when no refunded sales" do
        sales = payment.refunded_sales
        expect(sales).to be_empty
      end
    end

  context_ "#disputed_sales" do
      let(:user) { create(:user) }
      let(:product) { create(:product, user: user) }
      let(:balance) { create(:balance, user: user) }
      let(:payment) { create(:payment, user: user, balances: [balance], payout_period_end_date: 3.days.ago) }

  test "includes only chargedback sales from associated balances" do
        successful_sale = create(:purchase, seller: user, link: product, purchase_success_balance: balance)
  ...
  test "returns empty array when no chargedback sales" do
        sales = payment.disputed_sales
        expect(sales).to be_empty
      end
    end
  ...
  test "returns external_id for the id field" do
        json = @payment.as_json
        expect(json[:id]).to eq("mocked_external_id")
      end

  test "returns the correct values for basic fields" do
        json = @payment.as_json
  ...
  test "returns correct formatted amount" do
  ...
  context_ "when payment is not completed" do
  test "returns nil for processed_at in processing state" do
          @payment.update!(state: Payment::PROCESSING)
          expect(@payment.as_json[:processed_at]).to be_nil
        end

  test "returns nil for processed_at in failed state" do
          @payment.update!(state: Payment::FAILED)
          expect(@payment.as_json[:processed_at]).to be_nil
        end

  test "returns nil for processed_at in creating state" do
          @payment.update!(state: Payment::CREATING)
          expect(@payment.as_json[:processed_at]).to be_nil
        end
      end

  context_ "when payment is completed" do
  test "returns correct processed_at timestamp" do
  ...
  test "works with different payment processors" do
  ...
  test "handles zero amount correctly" do
        @payment.update!(amount_cents: 0)
        expect(@payment.as_json[:amount]).to eq("0.00")
      end

  context_ "bank_account_visual field" do
  test "includes bank_account_visual when payment has bank account" do
  ...
  test "returns nil when payment has no bank account" do
          json = @payment.as_json
          expect(json[:bank_account_visual]).to be_nil
        end
      end

  context_ "paypal_email field" do
  test "includes paypal_email when payment has payment_address" do
  ...
  test "returns nil when payment has no payment_address" do
          json = @payment.as_json
          expect(json[:paypal_email]).to be_nil
        end
      end

  context_ "with include_sales option" do
        let(:user) { create(:user) }
        let(:product) { create(:product, user: user) }
        let(:balance) { create(:balance, user: user) }
  ...
  context_ "when include_sales is true" do
  test "includes sales, refunded_sales, and disputed_sales ids" do
            successful_sale = create(:purchase, seller: user, link: product, purchase_success_balance: balance)
            refunded_sale = create(:purchase, :refunded, seller: user, link: product, purchase_refund_balance: balance)
            chargedback_sale = create(:purchase, seller: user, link: product, purchase_chargeback_balance: balance, chargeback_date: 1.day.ago)
  ...
  test "includes empty arrays when no sales of each type exist" do
            json = payment.as_json(include_sales: true)
  ...
  context_ "when include_sales is false" do
  test "does not include sales keys in the response" do
  ...
  context_ "when include_sales option is not provided" do
  test "does not include sales keys in the response" do
            json = payment.as_json
