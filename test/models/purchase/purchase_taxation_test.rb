...
class ModelsPurchasePurchaseTaxationTest < ActiveSupport::TestCase
  self.rspec_metadata = { vcr: true }



  context_ "PurchaseTaxation", :vcr do
  ...
  context_ "sales tax with merchant_migration enabled" do
      let(:price) { 10_00 }
  ...
  context_ "sales tax with native_paypal_payments enabled" do
  ...
  test "stores the tax attributes" do
        expect(@purchase.tax_cents).to eq(0)
        expect(@purchase.gumroad_tax_cents).to eq(0)
      end
