...
class ModelsPurchaseInventoryCounterCacheTest < ActiveSupport::TestCase



  context_ "Purchase inventory counter cache" do
  ...
  test "decrements cache when a counting purchase transitions to a non-counting state" do
  ...
  test "decrements cache when a counting purchase is destroyed" do
  ...
  test "does not double-count when unrelated attributes change" do
        purchase = create(:purchase, link: product, variant_attributes: [variant], purchase_state: "successful", quantity: 1)
        variant.reload
        product.reload
        expect do
  ...
  context_ "subscription deactivation" do
      let(:membership) { create(:membership_product) }
      let(:membership_variant) { membership.variant_categories_alive.first.variants.first }

  test "subtracts counted quantity from variant and product when subscription deactivates, restores on reactivation" do
  ...
  context_ "refund flow" do
  test "keeps cached value unchanged when a successful purchase is partially refunded" do
        purchase = create(:purchase, link: product, variant_attributes: [variant], purchase_state: "successful", quantity: 1)
        variant.reload
        product.reload
        expect do
  ...
  context_ "per purchase type" do
  context_ "gift purchases" do
        let(:gift_product) { create(:product) }

  test "counts the gift sender purchase but not the gift receiver purchase" do
  ...
  test "counts a gift receiver purchase for a subscription" do
  ...
  context_ "preorder authorization purchases" do
  test "counts a preorder authorization as part of inventory" do
  ...
  context_ "membership (subscription) purchases" do
        let(:membership) { create(:membership_product) }

  test "counts the original subscription purchase" do
          purchase = create(:membership_purchase, link: membership, quantity: 1)
          expect(purchase.counts_towards_inventory?).to eq(true)
        end

  test "does not count a recurring (non-original) subscription charge" do
          sub = create(:subscription, link: membership)
  ...
  test "counts both bundle parent and bundle product purchases" do
  ...
  context_ "combined charge purchases" do
  test "counts combined charge child purchases" do
          purchase = create(:purchase, link: product, is_part_of_combined_charge: true, purchase_state: "successful", quantity: 1)
          expect(purchase.counts_towards_inventory?).to eq(true)
        end
      end

  context_ "installment plan purchases" do
  test "counts the original installment plan purchase" do
          purchase = create(:installment_plan_purchase, purchase_state: "successful", quantity: 1)
          expect(purchase.counts_towards_inventory?).to eq(true)
        end

  test "does not count a recurring installment payment" do
  ...
  context_ "multi-save in a single outer transaction" do
  test "increments cache by quantity when purchase_state goes failed -> in_progress -> successful in one transaction" do
  ...
  test "leaves cache unchanged when purchase_state goes failed -> in_progress -> failed in one transaction" do
        purchase = create(:purchase, link: product, variant_attributes: [variant], purchase_state: "failed", quantity: 2)
        variant.reload
        product.reload
  ...
  test "leaves cache unchanged when archive flag flips on then off on the same purchase in one transaction" do
        membership = create(:membership_product)
  ...
  test "does not drift when an original purchase is archived and the subscription is reactivated in the same transaction" do
        membership = create(:membership_product)
        v = membership.variant_categories_alive.first.variants.first
  ...
  test "returns the cached column when flag is on" do
