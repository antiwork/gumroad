...
class ModelsPurchasePurchaseBalancesTest < ActiveSupport::TestCase
  self.rspec_metadata = { vcr: true }



  context_ "PurchaseBalances", :vcr  do
  ...
  context_ "proper Balance creation and association with purchases" do
      let(:physical) { false }
  ...
  test "associates the balance with the purchase and have the proper amount" do
  ...
  test "uses the same balance for refund/chargeback as the one used for the original purchase (if unpaid)" do
  ...
  test "uses the oldest unpaid balance for refund/chargeback if the original purchase balance is not unpaid" do
          purchase_balance_1 = @purchase_1.purchase_success_balance
  ...
  test "uses today's balance for refund/chargeback if the original purchase balance is not unpaid and there are no older unpaid balances" do
          purchase_balance_1 = @purchase_1.purchase_success_balance
  ...
  context_ "proper Balance creation and association with affiliate_credit" do
      let(:physical) { false }
  ...
  test "associates the balance with the purchase and affiliate_credit and have the proper amount for both" do
        travel_to(old_date) do
        ...
        expect(@purchase_1.affiliate_credit).not_to be(nil)
  ...
  context_ "when the app owner and creator get their own merchant accounts" do
        let(:product_2) do
  ...
  test "associates the balance with the purchase and affiliate_credit and have the proper amount for both" do
            ...
            expect(@purchase_2.affiliate_credit).not_to be(nil)
  ...
  context_ "proper Balance creation and association with affiliate_credit with merchant_migration enabled" do
      let(:physical) { false }
      let(:user) { create(:user) }
      let(:product) do
        create(:product, user:, is_physical: physical, require_shipping: physical,
  ...
  context_ "increment_sellers_balance!" do
  test "adds a balance to the user" do
        user = create(:user)
  ...
  context_ "increment_sellers_balance! without affiliate_credit without merchant account" do
        before do
          @seller = create(:user)
  ...
  test "creates a balance transaction" do
          @purchase.increment_sellers_balance!
  ...
  context_ "increment_sellers_balance! without affiliate_credit with merchant account" do
        before do
          @seller = create(:user)
  ...
  test "creates one balance transaction for the purchase" do
          @purchase.increment_sellers_balance!
          balance_transaction = BalanceTransaction.last
  ...
  context_ "increment_sellers_balance! with affiliate_credit with merchant account" do
        before do
          @seller = create(:user)
  ...
  test "creates an instance of affiliate_credit" do
          expect { @purchase.increment_sellers_balance! }.to change { AffiliateCredit.count }.by(1)
        end

  test "increases each affiliate application owners and sellers balances accordingly" do
          @purchase.increment_sellers_balance!
  ...
  test "creates two balance transactions for the purchase" do
          @purchase.increment_sellers_balance!
  ...
  context_ "increment_sellers_balance! with merchant_migration_enabled" do
  test "adds a balance to the user" do
        user = create(:user)
        Feature.activate_user(:merchant_migration, user)
        create(:user_compliance_info, user:)
  ...
  test "does not create balance transaction for the purchase" do
          @purchase.increment_sellers_balance!
          balance_transaction = BalanceTransaction.last
  ...
  test "creates only one balance transactions for the purchase" do
