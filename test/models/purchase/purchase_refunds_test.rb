...
class ModelsPurchasePurchaseRefundsTest < ActiveSupport::TestCase
  self.rspec_metadata = { vcr: true }



  context_ "PurchaseRefunds", :vcr do
  ...
  test "updates refund status" do
        ...
        expect(@purchase.stripe_refunded).not_to be(true)
        @purchase.refund_and_save!(@user.id)
        @purchase.reload
        expect(@purchase.refunds.first.status).to eq("succeeded")
      end

  test "updates refund processor refund id" do
        expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
        ...
        expect(@purchase.reload.refunds.first.processor_refund_id).not_to be(nil)
      end

  test "refunds idempotent" do
        expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
        expect(@purchase.stripe_refunded).not_to be(true)
        @purchase.refund_and_save!(@user.id)
        @purchase.reload
        expect(@purchase.stripe_refunded).not_to be(false)
        @purchase.refund_and_save!(@user.id)
      end

  test "refunds when stripe_partially_refunded" do
  ...
  test "creates a balance transaction for the refund" do
        charge_refund = nil
  ...
  test "updates balance of seller and # paid downloads" do
        expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
        @purchase.refund_and_save!(@user.id)
        @user.reload
        ...
        @product.reload
        expect(@product.sales.paid.count).not_to eq @initial_num_paid_download
        @product.sales.paid.count == @initial_num_paid_download - 1
      end

  context_ "partial refund with amount" do
  test "updates refund status" do
          expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
          expect(@purchase.stripe_refunded).not_to be(true)
          expect(@purchase.stripe_partially_refunded).not_to be(true)
  ...
  test "updates refund processor refund id" do
          ...
          @purchase.reload
          expect(@purchase.stripe_partially_refunded).to be(true)
          expect(@purchase.refunds.first.processor_refund_id).not_to be(nil)
        end

  test "allows refund multiple times" do
  ...
  test "fully refunds if amount goes over total transaction cents" do
          expect(ChargeProcessor).to receive(:refund!).twice.with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
          expect(@purchase.stripe_refunded).not_to be(true)
          expect(@purchase.stripe_partially_refunded).not_to be(true)
          @purchase.refund_and_save!(@user.id, amount_cents: @purchase.total_transaction_cents - 50)
  ...
  test "updates balance of seller" do
          expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
          @purchase.refund_and_save!(@user.id, amount_cents: 50)
          @user.reload
  ...
  test "updates balance of seller for multiple refunds" do
          expect(ChargeProcessor).to receive(:refund!).twice.with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
          @purchase.refund_and_save!(@user.id, amount_cents: 10)
          @user.reload
  ...
  test "updates balance of seller for multiple refunds finally marking it as fully refunded" do
          expect(ChargeProcessor).to receive(:refund!).twice.with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
          @purchase.refund_and_save!(@user.id, amount_cents: 10)
          @user.reload
          verify_balance(@user, @initial_balance - @purchase.amount_refunded_cents + @purchase.refunds.sum { |refund| refund.fee_cents - refund.retained_fee_cents })
  ...
  test "notifies customer about the refund" do
          expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
  ...
  context_ "with non USD currency" do
          before do
  ...
  test "handles partial refunds with passed amount" do
            expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
            expect(@purchase.stripe_refunded).not_to be(true)
            expect(@purchase.stripe_partially_refunded).not_to be(true)
  ...
  context_ "user has a merchant account" do
  ...
  test "does not try to reverse the associated transfer if purchase is chargedback and chargeback is won" do
  ...
  context_ "user has a merchant account not charge processor alive" do
  ...
  test "refunds successfully a single purchase which is part of a combined charge on a non-usd PayPal merchant account" do
  ...
  test "works when link is sold out" do
        link = create(:product, max_purchase_count: 1)
        purchase = create(:purchase, link:, seller: link.user)
        expect(-> { purchase.refund_and_save!(link.user.id) }).not_to raise_error
      end

  test "creates a refund event" do
        expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
  ...
  test "creates a refund object" do
        expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
        calculated_fingerprint = "3dfakl93klfdjsa09rn"
        allow(Digest::MD5).to receive(:hexdigest).and_return(calculated_fingerprint)
        @purchase.refund_and_save!(@user.id)
  ...
  test "returns true if refunds without error" do
        expect(ChargeProcessor).to receive(:refund!).and_call_original
        expect(@purchase.refund_and_save!(@user.id)).to be(true)
      end

  test "returns false if charge processor indicates request invalid" do
  ...
  test "returns false if charge processor unavailable" do
        expect(ChargeProcessor).to receive(:refund!).and_raise(ChargeProcessorUnavailableError)
        expect(@purchase.refund_and_save!(@user.id)).to be(false)
      end

  test "returns false if charge processor indicates already refunded" do
        expect(ChargeProcessor).to receive(:refund!).and_raise(ChargeProcessorAlreadyRefundedError)
        expect(@purchase.refund_and_save!(@user.id)).to be(false)
      end

  context_ "refund with tax" do
  context_ "with sales tax" do
          before do
            @purchase = create(:purchase_in_progress, link: @product, chargeable: create(:chargeable))
            @purchase.process!
            @purchase.mark_successful!
  ...
  test "refunds total transaction amount" do
            expect(@purchase).to receive(:debit_processor_fee_from_merchant_account!).and_call_original
  ...
  test "refunds with given amount cents" do
            expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
            expect(@purchase).to receive(:debit_processor_fee_from_merchant_account!).and_call_original
  ...
  test "refunds with given amount_refundable_cents" do
          expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
  ...
  context_ "refund Gumroad taxes" do
  test "refunds all taxes collected by Gumroad" do
  ...
  test "does not deduct the refunded tax amount from the connect account" do
  ...
  test "does not refund in excess if Gumroad taxes were already refunded - full refund" do
            expect(ChargeProcessor).to receive(:refund!)
  ...
  test "does not refund anything if purchase is already refunded" do
            expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
  ...
  test "does not refund anything if purchase already stripe refunded" do
            @purchase.stripe_refunded = true
            @purchase.save!
            refund_count = Refund.count
  ...
  test "saves business vat id along with refund information" do
  ...
  context_ "PayPal Connect sales" do
            before do
  ...
  context_ "refund Gumroad taxes" do
  context_ "when purchase is NOT partially refunded" do
                before do
                  expect(ChargeProcessor).to receive(:refund!)
  ...
  test "credits the creator account with the refunded VAT amount minus the fee that is returned" do
                  expect do
                    expect do
                      @paypal_purchase.refund_gumroad_taxes!(refunding_user_id: @product.user.id, note: "VAT_ID_1234_Dummy")
  ...
  context_ "when purchase is partially refunded" do
                before do
  ...
  test "refunds only the remaining taxes" do
  ...
  context_ "further refunds after refunding Gumroad taxes" do
  context_ "when gumroad taxes have NOT been refunded" do
  test "does not debit the creator's account" do
                  expect do
  ...
  context_ "when gumroad taxes have been refunded" do
                before do
                  expect do
                    @paypal_purchase.refund_gumroad_taxes!(refunding_user_id: @product.user_id, note: "VAT_ID_1234_Dummy")
                  end.to change { Credit.count }.by(1)
  ...
  test "debits the creator account with the same amount that was credited during gumroad VAT refund" do
                  expect do
                    expect do
                      @paypal_purchase.refund_and_save!(@product.user_id, amount_cents: @paypal_purchase.price_cents / 2)
                    end.to change { Credit.count }.by(1)
  ...
  context_ "do not decrement seller balance twice" do
        let(:purchase) do
  ...
  context_ "refund after a dispute event which is functionally treated as a chargeback on our side" do
          before do
  ...
  test "does not issue a refund while the dispute is still active" do
            expect(ChargeProcessor).not_to receive(:refund!)
  ...
  test "does not refund Gumroad taxes while the dispute is still active" do
  ...
  test "does not decrement balance from the user on such an event" do
            expect(purchase).not_to receive(:process_refund_or_chargeback_for_purchase_balance)
            expect(purchase).not_to receive(:process_refund_or_chargeback_for_affiliate_credit_balance)

            purchase.refund_and_save!(create(:admin_user).id)
          end
        end

  context_ "dispute after a refund event does not decrement seller balance" do
          before do
            purchase.refund_and_save!(create(:admin_user).id)
          end
  ...
  test "calls 'send_refunded_notification_webhook' to send sale refunded notification to the seller" do
        expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
  ...
  context_ "when refunds are disabled for the creator" do
        before do
          @user.disable_refunds!
        end

  context_ "when the refunding user is not an admin"  do
  test "doesn't issue a refund" do
            expect(ChargeProcessor).not_to receive(:refund!)
  ...
  context_ "when the refunding user is an admin" do
          before do
            @admin_user = create(:admin_user)
          end

  test "issues a refund" do
            expect(ChargeProcessor).to receive(:refund!).with(@purchase.charge_processor_id, @purchase.stripe_transaction_id, anything).and_call_original
  ...
  context_ "when creator's balance is less than the refund amount", :vcr do
  ...
  context_ "when the refunding user is not an admin"  do
  test "doesn't issue a refund if the purchase was made on Gumroad's Stripe account" do
  ...
  test "doesn't issue a refund if the purchase was made on Gumroad-managed Stripe account" do
  ...
  test "issues a refund if the purchase was made on creator's Stripe Connect account" do
  ...
  test "issues a partial refund if the refund amount is not more than creator's balance" do
            stripe_account = create(:merchant_account_stripe, user: @user)
            purchase.chargeable = create(:chargeable, product_permalink: purchase.link.unique_permalink)
            purchase.process!
            expect(purchase.merchant_account).to eq(stripe_account)
  ...
  test "issues a refund if the purchase was made on Gumroad's Stripe account" do
            purchase.chargeable = create(:chargeable, product_permalink: purchase.link.unique_permalink)
            purchase.process!
            expect(purchase.merchant_account).to eq(MerchantAccount.gumroad(StripeChargeProcessor.charge_processor_id))
            purchase.mark_successful!
  ...
  test "issues a refund if the purchase was made on Gumroad-managed Stripe account" do
            stripe_account = create(:merchant_account_stripe, user: @user)
            purchase.chargeable = create(:chargeable, product_permalink: purchase.link.unique_permalink)
            purchase.process!
            expect(purchase.merchant_account).to eq(stripe_account)
  ...
  context_ "partial refund after vat refund" do
        before do
  ...
  test "does not try to create a transfer reversal if purchase does not have vat already refunded" do
          ...
          expect_any_instance_of(Purchase).not_to receive(:reverse_excess_amount_from_stripe_transfer)

          purchase.refund!(refunding_user_id: purchase.seller.id, amount: 500)
        end

  test "does not try to create a transfer reversal if this is not a partial refund" do
          purchase = build(:purchase_in_progress, link: @product, gumroad_tax_cents: 200, country: "Germany", ip_country: "Germany", chargeable: create(:chargeable))
          purchase.process!
          purchase.update_balance_and_mark_successful!
          expect(purchase.merchant_account_id).to eq(@merchant_account.id)
  ...
  test "does not try to create a transfer reversal if holder of funds is not Stripe" do
  ...
  test "reverses the correct amount from the transfer in case of partial refund on a stripe purchase with vat already refunded" do
          purchase = build(:purchase_in_progress, link: @product, gumroad_tax_cents: 200, country: "Germany", ip_country: "Germany", chargeable: create(:chargeable))
          purchase.process!
          purchase.update_balance_and_mark_successful!
          expect(purchase.merchant_account_id).to eq(@merchant_account.id)
  ...
  context_ "#reverse_excess_amount_from_stripe_transfer" do
      before do
        @product = create(:product, price_cents: 1000)
        @merchant_account = create(:merchant_account, user: @product.user, country: "CA", currency: "cad",
                                                      charge_processor_merchant_id: "acct_1MbQQ6S2yTRm7HHQ")
  ...
  test "does not try to create a transfer reversal if total amount to be reversed is already reversed" do
  ...
  context_ "refund subscription purchase" do
  context_ "excluding reviews on subscription charges" do
        let(:subscriber) { create(:user, credit_card: create(:credit_card)) }

  context_ "free trial subscription" do
  ...
  test "excludes the subscriber's review when refunding the first charge" do
            expect do
  ...
  test "does not exclude the subscriber's review when refunding a subsequent charge" do
  ...
  context_ "non-free trial subscription" do
          let(:original_purchase) { create(:membership_purchase, purchaser: subscriber, price_cents: 100) }
          let!(:first_charge) { original_purchase.subscription.charge! }

  test "does not exclude the subscriber's review" do
            expect do
  ...
  context_ "#refund_purchase!" do
      before do
        @purchase = create(:purchase)
        @refunding_user = create(:user)
      end

  test "enqueues a UpdateSellerRefundEligibilityJob" do
  ...
  context_ "when partial refund amount is zero" do
  test "does not process a refund" do
  ...
  context_ "when called directly via external webhook on a chargedback purchase" do
  test "creates the Refund record but does not decrement seller balance again" do
          ...
          expect(@purchase.reload.refunds).not_to be_empty
          expect(@purchase.stripe_refunded).to be(true)
        end
      end

  context_ "when called on a chargeback-reversed purchase (seller won dispute)" do
  test "decrements seller balance normally because the dispute reversal already credited them back" do
  ...
  context_ "Low balance related sidekiq jobs" do
        before do
          @flow_of_funds = FlowOfFunds.build_simple_flow_of_funds(Currency::USD, @purchase.price_cents)
        end

  context_ "when refunding user is not admin" do
          before do
            @purchase.refund_purchase!(@flow_of_funds, @refunding_user.id)
          end

  test "enqueues LowBalanceFraudCheckWorker" do
            expect(LowBalanceFraudCheckWorker).to have_enqueued_sidekiq_job(@purchase.id)
          end
        end

  context_ "when refunding user is admin" do
          before do
            admin_user = create(:admin_user)
            @purchase.refund_purchase!(@flow_of_funds, admin_user.id)
          end

  test "doesn't enqueue LowBalanceFraudCheckWorker" do
  ...
  context_ "when gifter purchase is partially refunded" do
          before do
  ...
  test "sets the stripe_partially_refunded of giftee purchase to true" do
            expect(@giftee_purchase.reload.stripe_partially_refunded).to eq true
          end

  test "doesn't change the stripe_refunded of giftee purchase" do
            expect(@giftee_purchase.reload.stripe_refunded).to be_nil
          end
        end

  context_ "when gifter purchase is fully refunded" do
          before do
            flow_of_funds = FlowOfFunds.build_simple_flow_of_funds(Currency::USD, @gifter_purchase.price_cents)
            @gifter_purchase.refund_purchase!(flow_of_funds, @gifter_purchase.link.user.id)
          end

  test "sets the stripe_refunded of giftee purchase to true" do
            expect(@giftee_purchase.reload.stripe_refunded).to eq true
          end

  test "sets the stripe_partially_refunded of giftee purchase to false" do
  ...
  test "refunds the original purchase" do
        ...
        @purchase.reload
        expect(@purchase.stripe_refunded).not_to be(false)
        expect(@purchase.refunds).not_to be_empty
        expect(@purchase.refunds.first.is_for_fraud).to be(true)
      end

  test "does not retain the processor fee" do
        expect(@purchase.stripe_refunded).to be(false)
  ...
  test "queues an email to the seller informing them of the refund" do
  ...
  test "does not cancel the subscription or queue an email when the refund fails" do
  ...
  test "still runs side effects when refund_and_save! is an idempotent no-op" do
        subscription = double(deactivated?: false)
        allow(@purchase).to receive(:subscription).and_return(subscription)
  ...
  test "cancels the subscription effective immediately" do
  ...
  test "calls refund_for_fraud! and block_buyer!" do
  ...
  test "does not block the buyer if the refund fails" do
  ...
  context_ "refund purchase partially from stripe" do
      let(:merchant_account) { nil }
  ...
  test "changes stripe_partially_refunded status" do
        expect(@purchase.stripe_partially_refunded).not_to be(true)
        expect(@purchase).to receive(:debit_processor_fee_from_merchant_account!).and_call_original
  ...
  test "debits the gumroad fee from merchant account" do
        expect(@purchase).to receive(:debit_processor_fee_from_merchant_account!).and_call_original
  ...
  test "handles multiple partial refunds" do
        expect(@purchase.stripe_partially_refunded).not_to be(true)
        @purchase.refund_partial_purchase!(10, @user.id)
        @purchase.refund_partial_purchase!(10, @user.id)
        @purchase.reload
  ...
  test "marks fully refunded if subsequent refund exceeds price_cents" do
        expect(@purchase.stripe_partially_refunded).not_to be(true)
        @purchase.refund_partial_purchase!(10, @user.id)
        @purchase.refund_partial_purchase!(@purchase.price_cents - 10, @user.id)
        @purchase.reload
  ...
  context_ "refund purchase with 0 fee_cents" do
  ...
  test "updates the balance of the seller" do
  ...
  context_ "refund purchase with affiliate_credit" do
      let!(:merchant_account) { nil }
  ...
  test "updates balance of affiliate user as well as seller", :vcr do
        purchase.refund_and_save!(create(:admin_user).id)
        seller.reload
  ...
  test "creates two balance transactions for the refund" do
        charge_refund = nil
        original_charge_processor_refund = ChargeProcessor.method(:refund!)
        expect(ChargeProcessor).to receive(:refund!) do |*args, **kwargs|
          charge_refund = original_charge_processor_refund.call(*args, **kwargs)
  ...
  context_ "when the affiliate paid part of the Gumroad fees" do
        let(:affiliate) { create(:collaborator, affiliate_user:, seller:, affiliate_basis_points: 5000, products: [product]) }

  test "refunds the full affiliate credit (net of fees)" do
          purchase.refund_and_save!(create(:admin_user).id)
          verify_balance(affiliate_user.reload, 0)
        end
      end

  context_ "partially" do
  test "updates balance of affiliate user as well as seller" do
          expect(purchase).to receive(:debit_processor_fee_from_merchant_account!).and_call_original
  ...
  test "creates affiliate_partial_refunds and balances" do
          seller_balance = seller.unpaid_balance_cents
  ...
  test "cents calculation for balances tally up with affiliate_partial_refund" do
          seller_balance = seller.unpaid_balance_cents
  ...
  test "processes partial and then full refund" do
          purchase.refund_and_save!(seller.id, amount_cents: 400)
          seller.reload
          affiliate_user.reload
  ...
  test "refunds part of the fees" do
            seller_balance = seller.unpaid_balance_cents
  ...
  context_ "when the affiliate cut has changed since the purchase was made" do
  test "uses the cut at the time of the purchase when determining how much to refund" do
  ...
  context_ "refund purchase with affiliate_credit with merchant_migration enabled" do
  ...
  context_ "user has no merchant account" do
  test "does not update balance of affiliate user or seller" do
  ...
  context_ "user has a merchant account" do
  test "updates balance of affiliate user but not the seller" do
  ...
  test "creates a balance transaction for the affiliate user" do
          original_charge_processor_refund = ChargeProcessor.method(:refund!)
          expect(ChargeProcessor).to receive(:refund!) do |*args, **kwargs|
            charge_refund = original_charge_processor_refund.call(*args, **kwargs)
            charge_refund
  ...
  context_ "#reverse_the_transfer_made_for_dispute_win!" do
  test "does nothing and returns if holder of funds is not Stripe" do
  ...
  test "does nothing and returns if purchase is not disputed" do
  ...
  test "does nothing and returns if purchase dispute is not won" do
        merchant_account = create(:merchant_account, charge_processor_merchant_id: "acct_1MABWa2noRrbY6cK")
        purchase = create(:purchase, link: create(:product, user: merchant_account.user), merchant_account:)
  ...
  test "tries to reverse the dispute transfer if purchase dispute is won and holder of funds is Stripe" do
        merchant_account = create(:merchant_account, charge_processor_merchant_id: "acct_1MABWa2noRrbY6cK")
        purchase = create(:purchase, link: create(:product, user: merchant_account.user), merchant_account:)
        create(:dispute, purchase:, state: "won", won_at: Time.at(1669749973).utc)
