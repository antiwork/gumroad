...
class ModelsPurchasePurchaseInstallmentsTest < ActiveSupport::TestCase
  self.rspec_metadata = { vcr: true }



  context_ "PurchaseInstallments", :vcr do
    include CurrencyHelper
    include ProductsHelper

  context_ ".product_installments" do
  test "can return installments for several purchases" do
  ...
  context_ "when purchased product(s) have should_show_all_posts enabled" do
  ...
  test "returns all posts for purchases of products" do
  ...
  test "excludes past posts that are not directly targeted at the purchased product or variant" do
  ...
  context_ "with a restarted subscription" do
  ...
  test "shows non-workflow posts for restarted subscriptions" do
            installments = Purchase.product_installments(purchase_ids: [purchase.id])
            expect(installments.map(&:id)).to include(regular_post.id)
          end

  test "shows workflow installments that were already emailed" do
            installments = Purchase.product_installments(purchase_ids: [purchase.id])
            expect(installments.map(&:id)).to include(emailed_workflow_installment.id)
          end

  test "does not show workflow installments that have not been emailed" do
            installments = Purchase.product_installments(purchase_ids: [purchase.id])
  ...
  test "only includes published installments" do
  ...
  context_ "variant installments" do
  test "only includes installments from the purchased variant" do
  ...
  test "gets installments only from product workflow" do
  ...
  context_ "link and workflow" do
  test "includes installments in the correct order" do
          @product.update!(should_show_all_posts: true)
  ...
  test "does not throw an exception when a url redirect for a purchase is nil" do
          @purchase.url_redirect = nil
          @purchase.save!
          expect { @purchase.json_data_for_mobile }.not_to raise_error
        end

  test "returns update information if available" do
  ...
  test "returns subscription expiration information for a subscription purchase" do
  ...
  test "returns the correct json format for free purchase" do
  ...
  test "returns the correct json format for nonfree purchase" do
          @paid_purchase = create(:purchase)
  ...
  test "calls class method .product_installments for current purchase" do
  ...
  context_ "#update_json_data_for_mobile" do
      let(:user) { create(:user, credit_card: create(:credit_card)) }

  context_ "for subscriptions" do
  ...
  test "returns posts for all purchases of the product" do
  ...
  context_ "with chargedback purchase" do
  ...
  context_ "lapsed subscriptions" do
        before do
          @purchase = create(:membership_purchase)
  ...
  test "returns an empty array when user should lose access after cancellation" do
          @product.update!(block_access_after_membership_cancellation: true)
          expect(@purchase.update_json_data_for_mobile).to eq []
        end

  test "returns content when user should not lose access after cancellation" do
          expect(@purchase.update_json_data_for_mobile).not_to be_empty
        end
      end

  context_ "updated subscriptions" do
  test "returns posts associated with all purchases, including the updated original purchase" do
  ...
  context_ "Purchase.preload_product_updates_data!" do
        def capture_queries
          queries = []
  ...
  test "produces output byte-identical to the single-purchase path" do
          product = create(:membership_product)
          subscription = create(:subscription, link: product)
  ...
  test "issues a bounded number of queries regardless of installment count" do
          product = create(:membership_product)
          subscription = create(:subscription, link: product)
          purchase = create(:membership_purchase, link: product, subscription:)
          subscription.reload
  ...
  test "matches the single-purchase path for subscription renewals (email_info keyed on original_purchase.id)" do
          product = create(:membership_product)
          subscription = create(:subscription, link: product)
  ...
  test "picks the lowest-id UrlRedirect when duplicates exist for the same (purchase, installment)" do
          product = create(:membership_product)
          subscription = create(:subscription, link: product)
          purchase = create(:membership_purchase, link: product, subscription:)
          subscription.reload
  ...
  context_ "#schedule_workflows" do
      before do
        @product = create(:product)
  ...
  test "enqueues SendWorkflowInstallmentWorker for the workflow installments" do
  ...
  test "schedules matching non-abandoned cart workflow installments" do
        ...
        expect(SendWorkflowInstallmentWorker).not_to have_enqueued_sidekiq_job(abandoned_cart_workflow_installment.id, abandoned_cart_workflow_installment_rule.version, @purchase.id, nil, nil)
      end
    end

  context_ "#schedule_workflows_for_variants" do
      before do
        @product = create(:product)
  ...
  test "enqueues SendWorkflowInstallmentWorker for the purchased variant's workflow installments" do
  ...
  test "enqueues SendWorkflowInstallmentWorker for seller workflows targeted at the purchased variant" do
        @purchase.schedule_workflows_for_variants(excluded_variants: [@variant2])

        expect(SendWorkflowInstallmentWorker).to have_enqueued_sidekiq_job(@seller_variant_1_installment.id, 1, @purchase.id, nil, nil)
      end

  test "does not enqueue SendWorkflowInstallmentWorker for workflows that should already have been scheduled" do
        @purchase.schedule_workflows_for_variants(excluded_variants: [@variant2])
  ...
  test "does not enqueue SendWorkflowInstallmentWorker for workflows that don't target the variant" do
        @purchase.schedule_workflows_for_variants(excluded_variants: [@variant2])
  ...
  test "does not enqueue SendWorkflowInstallmentWorker if the workflow is deleted" do
  ...
  test "does not enqueue SendWorkflowInstallmentWorker if old variants are the same as the existing ones" do
  ...
  context_ "#schedule_all_workflows" do
  test "schedules all workflows that apply to the purchase" do
        user = create(:user)
  ...
  context_ "#reschedule_workflow_installments" do
      before do
        product = create(:product)
  ...
  context_ "when it is a quick unsubscribe and resubscribe in less than a minute" do
  test "does not reschedule any workflow installments" do
          unsubscribed_interval = 30.seconds

          travel_to(@purchase.created_at + unsubscribed_interval) do
            expect_any_instance_of(Purchase).not_to receive(:all_workflows)

            @purchase.reschedule_workflow_installments(send_delay: unsubscribed_interval)

            expect(SendWorkflowInstallmentWorker).not_to have_enqueued_sidekiq_job(@past_installment_1.id, 1, @purchase.id, nil)
            expect(SendWorkflowInstallmentWorker).not_to have_enqueued_sidekiq_job(@past_installment_2.id, 1, @purchase.id, nil)
            expect(SendWorkflowInstallmentWorker).not_to have_enqueued_sidekiq_job(@future_installment.id, 1, @purchase.id, nil)
          end
        end
      end

  context_ "when resubscribed after 1 hour and did not miss any workflow installments" do
  test "does not reschedule the workflow installments" do
  ...
  context_ "when resubscribed after 4 days and missed workflow installments" do
  test "reschedules all non-abandoned cart workflow installments in the future" do
          ...
          travel_to(@purchase.created_at + unsubscribed_interval) do
            ...
            expect(SendWorkflowInstallmentWorker).not_to have_enqueued_sidekiq_job(abandoned_cart_workflow_installment.id, abandoned_cart_workflow_installment_rule.version, @purchase.id, nil)
          end
        end
      end
    end
