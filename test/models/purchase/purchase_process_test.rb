...
class ModelsPurchasePurchaseProcessTest < ActiveSupport::TestCase
  self.rspec_metadata = { vcr: true }



  context_ "Purchase Process", :vcr do
  ...
  test "does not call Stripe" do
        expect(Stripe::PaymentIntent).not_to receive(:create)
        purchase.process!
      end

  test "sets fee_cents" do
        purchase.process!
        purchase.save!
  ...
  test "runs the taxation logic" do
          purchase.process!
          purchase.save!
          purchase.reload
  ...
  context_ "physical product with paid shipping" do
          before do
  ...
  test "runs the shipping calculation logic" do
            purchase.process!
  ...
  context_ "card values" do
  test "has correct card values for normal product" do
  ...
  context_ "major steps involving the chargeable" do
  test "gives chargeable from load_chargeable_for_charging --> prepare_chargeable_for_charge!" do
  ...
  test "gives chargeable from prepare_chargeable_for_charge! --> create_charge_intent" do
          chargeable = build(:chargeable)
          purchase = build(:purchase_with_balance, chargeable:)
  ...
  context_ "#load_chargeable_for_charging" do
  test "sets a card error in the errors if one is set as a card params error" do
  ...
  test "sets a stripe unavailable error in the errors if some other error is set as a card params error" do
  ...
  test "uses the chargeable if one is set" do
            chargeable = build(:chargeable)
            purchase = build(:purchase_with_balance, chargeable:)
            expect(purchase).to receive(:prepare_chargeable_for_charge!).with(chargeable).and_call_original
            purchase.process!
          end
  test "uses a chargeable credit card if no chargeable is set, but purchaser with saved card exists" do
            user = build(:user)
  ...
  test "uses a chargeable credit card if no chargeable is set, no purchaser with saved card exists, but credit card is already set on purchase" do
            credit_card = build(:credit_card)
            purchase = build(:purchase_with_balance, credit_card:)
            expect(purchase).to receive(:prepare_chargeable_for_charge!).and_call_original
            purchase.process!
          end
  test "sets errors if no chargeable is set, no purchaser with saved card exists, and no credit card is set on purchase" do
  ...
  context_ "#validate_chargeable_for_charging" do
  test "errors if a chargeable is set backed by multiple charge processors" do
  ...
  test "does not error if chargeable is backed by a single charge processor" do
            chargeable = build(:chargeable)
            purchase = build(:purchase, purchase_state: "in_progress", chargeable:)
  ...
  context_ "#prepare_chargeable_for_charge!" do
  test "does not try to resave chargeable if it's a saved card we're using for the purchase" do
  ...
  test "saves card if purchaser has asked to save and return new chargeable for that saved card" do
            user = create(:user)
            chargeable = build(:chargeable)
  ...
  test "saves the card to the user if the purchase is made with PayPal" do
            user = create(:user)
  ...
  test "saves card if preorder and return new chargeable for that saved card" do
            chargeable = build(:chargeable)
            purchase = build(:purchase_with_balance, chargeable:, is_preorder_authorization: true)
            expect(purchase).to receive(:prepare_chargeable_for_charge!).with(chargeable).and_call_original
            expect(purchase).not_to receive(:create_charge_intent)
            purchase.process!
            expect(purchase.credit_card).to eq CreditCard.last
          end
  test "saves card if subscription and return new chargeable for that saved card" do
            chargeable = build(:chargeable)
  ...
  test "prepares chargeable" do
            chargeable = build(:chargeable)
            purchase = build(:purchase_with_balance, chargeable:)
            expect(chargeable).to receive(:prepare!).and_call_original
            purchase.process!
          end
  test "sets info on purchase about card" do
            chargeable = build(:chargeable)
            purchase = build(:purchase_with_balance, chargeable:)
  ...
  context_ "card country" do
  test "leaves card country unset if card country on chargeable is blank" do
              chargeable = build(:chargeable)
  ...
  context_ "charge processor errors" do
  context_ "new chargeable" do
              let(:chargeable) { build(:chargeable) }
              let(:purchase) { build(:purchase_with_balance, chargeable:) }

  context_ "charge processor unavailable" do
                before do
  ...
  context_ "charge processor invalid request" do
                before do
  ...
  context_ "new chargeable and save card" do
              let(:chargeable) { build(:chargeable) }
  ...
  context_ "#create_charge_intent" do
  test "sends charge chargeable" do
            chargeable = build(:chargeable)
            purchase = build(:purchase_with_balance, chargeable:)
            expect(purchase).to receive(:create_charge_intent).with(chargeable, anything).and_call_original
            purchase.process!
  ...
  test "does not update payment intent id on the associated credit card when mandate options are not present" do
  ...
  test "updates payment intent id on the associated credit card when mandate options are present" do
            purchase = build(:purchase_with_balance, chargeable: build(:chargeable), credit_card: create(:credit_card, card_country: "IN"))
  ...
  context_ "purchase with perceived_price_cents too low" do
  ...
  test "creates the purchase object with the right error code" do
          expect(purchase.errors).to be_present
          expect(purchase.reload.error_code).to eq "perceived_price_cents_not_matching"
        end
      end

  context_ "purchase with high price" do
  ...
  context_ "purchase with low contirbution amount" do
  ...
  context_ "purchase with negative seller revenue" do
  ...
  test "does not block the purchase" do
          expect(affiliate_purchase.error_code).to be_nil
          expect(affiliate_purchase.errors).to be_empty
        end
      end

  context_ "purchase of product with customizable price and offer codes" do
        before do
  ...
  context_ "intended purchase price greater than minimum after offer codes" do
          before(:each) do
  ...
  test "allows / creates a purchase and does not return an error code" do
            expect(@purchase.errors).not_to be_present
          end

  test "records discount details" do
            @purchase.process!
  ...
  context_ "with commas" do
        before do
          @product = create(:product, price_cents: 100)
          @chargeable = build(:chargeable)
        end

  test "sets the correct price on the purchase" do
  ...
  context_ "setting price_cents" do
  test "sets the correct price cents based off the link price and quantity" do
  ...
  context_ "validating product quantity" do
  test "lets the purchase go through when quantity is less than products available" do
            purchase = create(:physical_purchase, link: @product, chargeable: @chargeable, perceived_price_cents: 500, save_card: false, ip_address:, quantity: 5)
            purchase.process!
            expect(purchase.errors).to be_empty
  ...
  test "lets the purchase go through when quantity is equal to products available" do
  ...
  test "does not let the purchase go through when quantity is greater than products available" do
  ...
  context_ "validating variant quantity" do
          before do
  ...
  test "lets the purchase go through when quantity is less than variants available" do
  ...
  test "lets the purchase go through when quantity is equal to variants available" do
  ...
  test "does not let the purchase go through when quantity is greater than variants available" do
  ...
  context_ "validating offer code quantity" do
          before do
            @offer = create(:offer_code, products: [@product], code: "sxsw", amount_cents: 100, max_purchase_count: 5)
          end

  test "lets the purchase go through when quantity is less than offer codes available" do
  ...
  test "lets the purchase go through when quantity is equal to offer codes available" do
  ...
  test "does not let the purchase go through when quantity is greater than offer codes available" do
  ...
  test "lets the purchase go through if the proper sku is attached to it" do
  ...
  test "does not let the purchase go through if the proper sku is not attached to it" do
          @purchase = build(:physical_purchase, link: @product, chargeable: @chargeable, perceived_price_cents: 100, save_card: false, ip_address:, price_range: 1)
          @purchase.process!
          expect(@purchase.errors).to be_present
  ...
  context_ "purchase with price_range too small for variants" do
          before do
  ...
  context_ "purchase with perceived_price_cents too small for variants" do
          before do
  ...
  context_ "purchase with dollars and cents price difference" do
          before do
  ...
  context_ "purchase with sufficient price_range" do
          before do
            @purchase = build(:purchase, link: @product, chargeable: @chargeable, perceived_price_cents: 400, save_card: false,
  ...
  context_ "User's first purchase" do
  ...
  test "assigns user credit card on first purchase if save_card is true" do
          purchase.perceived_price_cents = perceived_price_cents
          purchase.save_card = true
          purchase.ip_address = ip_address
          purchase.process!(off_session: false)
          expect(@user.credit_card).not_to be(nil)
        end

  test "does not assign credit card on first purchase if save_card is false" do
          purchase.perceived_price_cents = perceived_price_cents
          purchase.ip_address = ip_address
          purchase.save_card = false
          purchase.process!(off_session: false)
  ...
  context_ "cvc check failing for a new credit card" do
        before do
          @user = create(:user)
          @product = create(:product, user: @user)
        end

  test "fails the purchase if cvc check failed" do
          ...
          expect(bad_purchase.stripe_fingerprint).not_to be(nil)
        end
      end

  context_ "cvc check failing for new saved credit card" do
        before do
  ...
  context_ "zero dollars" do
        before do
          @l = create(:product, price_range: "$0+")
          @p = build(:purchase, link: @l, price_cents: 0, stripe_fingerprint: nil, stripe_transaction_id: nil)
        end

  test "does not charge" do
          expect(@p).not_to receive(:prepare_chargeable_for_charge!)
          expect(@p).not_to receive(:create_charge_intent)
          @p.process!
        end

  test "has 0 fee_cents" do
          @p.process!
          @p.save!
          expect(@p.reload.fee_cents).to eq 0
        end

  test "does not have a charge processor id set" do
          @p.process!
          @p.save!
  ...
  context_ "skip_preparing_for_charge purchase" do
        let(:product) { create(:product) }
        let(:purchase) do
  ...
  context_ "preorder credit card creation" do
        before do
  ...
  test "creates the credit card and associates it with the purchase but does not charge it" do
          expect(Stripe::PaymentIntent).not_to receive(:create)
  ...
  test "doesn't create the credit card when given a bad card" do
  ...
  test "creates the credit card and associates it with the purchase and the buyer, but does not charge it" do
          expect(Stripe::PaymentIntent).not_to receive(:create)
  ...
  test "creates a url redirect for a purchase of a physical product if it has a file" do
  ...
  test "creates a url redirect for a purchase of a physical product even if it does not have a file" do
        @product = create(:physical_product)
        @purchase = create(:physical_purchase, link: @product, seller: @product.user, purchase_state: "in_progress")
        @purchase.update_balance_and_mark_successful!
        expect(@purchase.url_redirect).to be_present
  ...
  test "creates a url redirect for a subscription even if it does not have a file" do
  ...
  context_ "recommended products" do
        before do
  ...
  test "does not charge extra 10% fee if the product was recommended" do
  ...
  test "uses the provided card, even if a default is present" do
  ...
  test "falls back to the customer and his card if none is provided" do
