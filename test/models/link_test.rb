...
class LinkTest < ActiveSupport::TestCase
  self.described_class = Link
  self.rspec_metadata = { vcr: true }



  context_ Link, :vcr do
  ...
  context_ "max_purchase_count validations" do
  test "can be set on new records with no purchases" do
        expect(build(:product, max_purchase_count: nil).valid?).to eq(true)
      end

  test "prevents to change when inventory sold is greater than the new value" do
  ...
  test "does not make the record invalid when inventory sold is greater" do
  ...
  test "allows it to be set on new records with no purchases" do
        expect(build(:product, max_purchase_count: 100).valid?).to eq(true)
      end
    end

  test "allows > $1000 links for verified users" do
      expect(build(:product, user: create(:user, verified: true), price_cents: 100_100).valid?).to be(true)
    end

  context_ "price_must_be_within_range validation" do
  ...
  test "fails if price is too high" do
  ...
  test "fails if price exceeds the maximum storable value" do
        expect do
  ...
  test "fails if price is too low" do
  ...
  context_ "when product has prices in multiple currencies" do
        let(:product) { create(:product, price_currency_type: "usd", price_cents: 100) }

  test "validates prices for the current currency against correct thresholds when switching currencies" do
  ...
  test "adds an error for unsupported currency type" do
  ...
  context_ "native_type inclusion validation" do
  test "fails if native_type is nil" do
  ...
  test "succeeds if native_type is in the allowed list" do
        link = build(:product, native_type: "digital")

        expect(link).to be_valid
      end

  test "fails if native_type is not in the allowed list" do
  ...
  context_ "discover_fee_per_thousand inclusion validation" do
      let(:product) { build(:product) }

  test "succeeds if discover_fee_per_thousand is in the allowed list" do
  ...
  test "fails if discover_fee_per_thousand is not in the allowed list" do
  ...
  context_ "alive_category_variants_presence validation" do
  context_ "for physical products" do
        let(:product) { create(:physical_product) }

  test "succeeds when the product has no versions" do
  ...
  test "succeeds when the product has non-empty versions" do
  ...
  test "fails when the product has empty versions" do
          category_one = create(:variant_category, link: product)
  ...
  context_ "for non-physical products" do
  ...
  context_ "free trial validation" do
  context_ "when product is recurring billing" do
  test "allows free_trial_enabled to be set" do
          product = build(:subscription_product, free_trial_enabled: true, free_trial_duration_unit: :week, free_trial_duration_amount: 1)
          expect(product).to be_valid
        end

  test "validates presence of free trial properties if free trial is enabled" do
  ...
  test "skips validating free_trial_duration_amount unless changed" do
  ...
  test "does not validate presence of free trial properties if free trial is disabled" do
          product = build(:subscription_product, free_trial_enabled: false)
          expect(product).to be_valid
        end

  test "only allows permitted free trial durations" do
          product = build(:subscription_product, free_trial_enabled: true, free_trial_duration_unit: :week, free_trial_duration_amount: 1)
          expect(product).to be_valid
  ...
  context_ "when product is not recurring billing" do
  test "does not allow free_trial_enabled to be set" do
  ...
  test "does not allow free trial properties to be set" do
  ...
  context_ "set_default_discover_fee_per_thousand" do
  test "sets the boosted discover fee when user has discover_boost_enabled" do
  ...
  test "doesn't set the boosted discover fee when user doesn't have discover_boost_enabled" do
          user = create(:user)
  ...
  context_ "initialize_tier_if_needed" do
  test "creates a Tier variant category and default tier" do
  ...
  test "creates a default price for the default tier" do
  ...
  test "creates a price with price_cents 0 for the product" do
          product = create(:membership_product, price_cents: 600)
  ...
  test "sets subscription_duration to the default if not set" do
  ...
  context_ "single-unit currencies" do
  test "sets prices correctly" do
  ...
  context_ "content moderation on publish" do
  ...
  test "blocks publishing when ContentModeration::ModerateRecordService.check fails" do
  ...
  test "skips the content moderation check for VIP creators" do
  ...
  test "publishes successfully when the content moderation check passes" do
          allow(ContentModeration::ModerateRecordService).to receive(:check).with(product, :product).and_return(
  ...
  test "clears the publishing flag after publish! completes" do
  ...
  test "clears the publishing flag even when publish! raises" do
          allow(ContentModeration::ModerateRecordService).to receive(:check).and_return(
  ...
  context_ "content moderation on edits to a published product" do
        let(:product) { create(:product, purchase_disabled_at: Time.current) }
  ...
  test "re-checks moderation when the name changes" do
  ...
  test "re-checks moderation when the description changes" do
          expect(ContentModeration::ModerateRecordService).to receive(:check).with(product, :product).and_return(
  ...
  test "does not re-check moderation when unrelated attributes change" do
          expect(ContentModeration::ModerateRecordService).not_to receive(:check)
  ...
  context_ "content moderation on edits to a draft product" do
        let(:product) { create(:product, draft: true) }

  test "does not run moderation on name/description edits" do
          expect(ContentModeration::ModerateRecordService).not_to receive(:check)
  ...
  context_ "native type is a coffee" do
          before { product.native_type = Link::NATIVE_TYPE_COFFEE }

  test "creates a suggested amount variant category and variant and resets the base price" do
            product.save!
            product.reload
  ...
  context_ "initialize_call_limitation_info_if_needed!" do
        let(:seller) { create(:user, :eligible_for_service_products) }
        let(:product) { build(:product, user: seller, price_cents: 200) }

  context_ "native type is not call" do
  test "does not create a call limitations record" do
            product.save
            expect(product.call_limitation_info).to be_nil
          end
        end

  context_ "native type is call" do
          before { product.native_type = Link::NATIVE_TYPE_CALL }

  test "creates a call limitations record" do
            product.save!
  ...
  context_ "initialize_duration_variant_category_for_calls!" do
  context_ "native type is call" do
          let(:call) { create(:call_product) }

  test "creates a duration variant category" do
  ...
  test "does not create a duration variant category" do
  ...
  context_ "#purchase_type=" do
  test "accepts valid purchase_type values" do
  ...
  test "defaults to buy_only when given an invalid value" do
          link.purchase_type = "buy"
          expect(link.purchase_type).to eq("buy_only")
        end

  test "does not raise ArgumentError for invalid values" do
          expect { link.purchase_type = "invalid" }.not_to raise_error
          expect(link.purchase_type).to eq("buy_only")
        end
      end

  context_ "delete_unused_prices" do
  ...
  context_ "when switching to a buy_only product" do
  test "deletes any rental prices" do
            expect do
  ...
  context_ "when switching to a rent_only product" do
  test "deletes any buy prices" do
            expect do
  ...
  context_ "when switching to a buy_and_rent product" do
  test "does nothing" do
  ...
  context_ "when leaving purchase_type unchanged" do
  test "does not run the callback" do
  ...
  context_ "adding to profile sections" do
  test "adds newly created products to all sections that have add_new_products set" do
          seller = create(:user)
          ...
          other_sections.each do |section|
            expect(section.reload.shown_products).not_to include link.id
          end
        end
      end
    end
  ...
  context_ "#confirmed_collaborators" do
  test "returns all confirmed collaborators" do
          product = create(:product)
  ...
  test "returns the live collaborator" do
          product = create(:product)
  ...
  context_ "#collaborator_for_display" do
  test "returns the collaborating user if they should be shown as a co-creator" do
          product = create(:product)
  ...
  context_ "#current_base_variants" do
  test "returns variants and SKUs that have not been deleted and whose variant category has not been deleted" do
  ...
  context_ "#public_files" do
  test "returns all public files associated with the product" do
          product = create(:product)
  ...
  context_ "#alive_public_files" do
  test "returns all alive public files associated with the product" do
          product = create(:product)
          public_file = create(:public_file, resource: product)
  ...
  context_ "#communities" do
  test "returns all communities associated with the product" do
          product = create(:product)
          communities = [
  ...
  context_ "#active_community" do
  test "returns the live community" do
          product = create(:product)
          create(:community, resource: product, deleted_at: 1.minute.ago)
  ...
  test "returns the correct products do" do
  ...
  context_ "by_general_permalink" do
        before do
  ...
  test "matches products by unique permalink" do
          expect(Link.by_general_permalink("xxx")).to match_array([@product_1])
        end

  test "matches products by custom permalink" do
          expect(Link.by_general_permalink("custom")).to match_array([@product_2])
        end

  test "does not match products if empty permalink is passed" do
  ...
  context_ "by_unique_permalinks" do
        before do
          @product_1 = create(:product, unique_permalink: "xxx")
          @product_2 = create(:product, unique_permalink: "yyy", custom_permalink: "custom")
          @product_3 = create(:product, unique_permalink: "zzz", custom_permalink: "awesome")
  ...
  test "does not match products by custom permalink" do
          expect(Link.by_unique_permalinks(["awesome", "custom"])).to be_empty
        end

  test "returns matched products and ignores permalinks that did not match" do
          expect(Link.by_unique_permalinks(["xxx", "custom"])).to match_array([@product_1])
        end

  test "does not match any products if no permalinks are provided" do
          expect(Link.by_unique_permalinks([])).to be_empty
        end
      end

  context_ "unpublished" do
        before do
          create(:product, user:)
  ...
  test "retries on ActiveRecord::Deadlocked and succeeds", :vcr do
          call_count = 0
  ...
  test "re-raises ActiveRecord::Deadlocked after exhausting retries", :vcr do
  ...
  context_ "when the user has not confirmed their email address" do
          before do
            @user.update!(confirmed_at: nil)
          end

  test "raises a Link::LinkInvalid error" do
            expect do
              @product.publish!
  ...
  context_ "when a bundle has no alive products" do
          before do
  ...
  context_ "when the seller has universal affiliates" do
  test "associates those affiliates with the product and notifies them" do
  ...
  context_ "who are already associated with the product" do
  test "does not add or notify them" do
              direct_affiliate = create(:direct_affiliate, seller: @user, apply_to_all_products: true, products: [@product])

              expect do
                @product.publish!
              end.not_to have_enqueued_mail(AffiliateMailer, :notify_direct_affiliate_of_new_product).with(direct_affiliate.id, @product.id)
  ...
  context_ "when affiliate has been removed" do
  test "does not add or notify them" do
              direct_affiliate = create(:direct_affiliate, seller: @user, apply_to_all_products: true)
  ...
  test "transcodes video files on publishing the product only if `queue_for_transcoding?` is true for the product file" do
            ...
            @video_link.publish!
            expect(TranscodeVideoForStreamingWorker).not_to have_enqueued_sidekiq_job(video_file_1.id, video_file_1.class.name)
            expect(TranscodeVideoForStreamingWorker).not_to have_enqueued_sidekiq_job(video_file_2.id, video_file_2.class.name)
            expect(TranscodeVideoForStreamingWorker).not_to have_enqueued_sidekiq_job(video_file_3.id, video_file_3.class.name)
  ...
  test "transcodes video when the link is already published" do
  ...
  test "doesn't transcode when the link is unpublished" do
              @video_link.unpublish!
  ...
  context_ "when auto-transcode is disabled" do
              before do
                allow(@product).to receive(:auto_transcode_videos?).and_return(false)
              end

  test "doesn't transcode the video" do
                expect do
                  @product.publish!
                end.not_to change { TranscodeVideoForStreamingWorker.jobs.size }
              end

  test "enables transcode on purchase" do
  ...
  context_ "Merchant migration enabled" do
          before do
  ...
  test "allows publishing if grandfathered account" do
            @user.save!
  ...
  test "allows publishing if new account and has a valid merchant account connected" do
            @user.save!
  ...
  context_ "when new account and no valid merchant account connected" do
          before do
  ...
  context_ "when the seller has universal affiliates" do
  test "does not associate those affiliates with the product and notifies them" do
              direct_affiliate = create(:direct_affiliate, seller: @user, apply_to_all_products: true)
  ...
  context_ "has_paid_sales_between" do
        before do
  ...
  test "returns link with sales made after 1.week.ago" do
          expect(Link.has_paid_sales_between(1.week.ago, Time.current)).to include @product_with_recent_sales
        end

  test "does not return link with sales from before that" do
          expect(Link.has_paid_sales_between(1.week.ago, Time.current)).not_to include @product_no_recent_sales
        end
      end
  ...
  test "returns memberships" do
          expect(Link.membership).to eq([@membership])
        end
      end

  context_ "non_membership" do
        before do
          @product = create(:product)
  ...
  context_ "collabs_as_collaborator" do
  test "returns products that the user is a collaborator on" do
          user = create(:user)
  ...
  context_ "collabs_as_seller_or_collaborator" do
  test "returns products that the user is a collaborator on and collabs they've created" do
          user = create(:user)
  ...
  context_ "for_balance_page" do
  test "returns all the user's own products and collab products" do
          user = create(:user)
  ...
  context_ "not_call" do
        let!(:call_product) { create(:call_product) }
        let!(:product) { create(:product) }

  test "returns products that are not calls" do
          expect(Link.not_call).to contain_exactly(product)
        end
      end

  context_ "can_be_bundle" do
  ...
  test "returns products that can be bundles" do
          expect(Link.can_be_bundle).to match_array([product, bundle, *bundle.bundle_products.map(&:product)])
        end
      end

  context_ "with_latest_product_cached_values" do
  test "joins a product_cached_values row for each product" do
          user = create(:user)
  ...
  test "is invalid if duplicates a custom permalink of another product by the same user" do
          user = create(:user)
  ...
  test "is invalid if duplicates a unique permalink of another product by the same user" do
          user = create(:user)
  ...
  test "is valid if duplicates a unique permalink of another user's product" do
  ...
  test "is valid if duplicates a custom permalink of another user's product" do
  ...
  context_ "uniqueness validation for licensed products" do
          before do
  ...
  test "marks the product as valid" do
  ...
  context_ "when the product is licensed" do
  context_ "when product is not persisted" do
  test "marks the product as valid" do
  ...
  context_ "when the product is persisted" do
  context_ "when the created_at is after force_product_id_timestamp" do
                before do
  ...
  context_ "when the created_at is before the force_product_id_timestamp" do
                before do
                  @created_at_timestamp = @force_product_id_timestamp - 1.day
                end

  context_ "when the product is not licensed" do
                  before do
                    @licensed = false
                  end
  ...
  context_ "when other sellers have licensed products with same custom or unique permalinks" do
  context_ "when products of other sellers were created before force_product_id_timestamp" do
  test "marks the product as invalid" do
  ...
  context_ "when products of other sellers were created after force_product_id_timestamp" do
                    before do
  ...
  context_ "is_licensed" do
  context_ "validation for products with custom permalinks overlap" do
          before do
            @force_product_id_timestamp = Time.current
            @licensed = true
            @other_product = create(:product, is_licensed: true, custom_permalink: "abc", unique_permalink: "xyz", created_at: @force_product_id_timestamp - 1.day)
  ...
  context_ "unique_permalink" do
  test "is invalid if it has numbers in it" do
        expect(build(:product, unique_permalink: "a23f").valid?).to be(false)
      end

  test "is valid with underscores" do
  ...
  context_ "automatic generation of unique permalinks" do
  context_ "conflicts with other unique permalinks" do
          before do
            # Take up all possible one-letter unique permalinks
            ("a".."z").to_a.each { |ch| create :product, unique_permalink: ch }
          end

  test "generates the shortest possible permalink that does not conflict with other unique permalinks" do
            # Does not take any of the one-letter permalinks
            expect(create(:product).unique_permalink.length).to eq(2)
          end
        end

  context_ "conflicts with custom permalinks by other users" do
          before do
  ...
  test "generates the shortest possible permalink that may conflict with custom permalinks" do
            expect(create(:product).unique_permalink.length).to eq(1)
          end
        end

  context_ "conflicts with custom permalinks by the same user" do
          let(:user) { create(:user) }
  ...
  test "generates the shortest possible permalink that does not conflict with custom permalinks" do
            expect(create(:product, user:).unique_permalink.length).not_to eq(1)
          end
        end

  context_ "case sensitivity" do
          before do
            # Take up all possible one-letter permalinks with uppercase chars
            ("A".."Z").to_a.each { |ch| create :product, unique_permalink: ch }
          end

  test "does not duplicate uppercase permalinks and generates a lowercase permalink" do
            product = create(:product)
  ...
  context_ "by unique permalink" do
  test "fetches a product by unique permalink" do
          expect(Link.fetch_leniently("aaa")).to eq(product_1)
        end

  test "scopes search to specific user if provided" do
          expect(Link.fetch_leniently("aaa", user: user_1)).to eq(product_1)
          expect(Link.fetch_leniently("aaa", user: user_2)).to eq(nil)
        end

  test "does not fetch a deleted product" do
          expect(Link.fetch_leniently("ccc")).to eq(nil)
        end
      end

  context_ "by custom permalink" do
        # Support for legacy URLs
  test "fetches an oldest product if not scoped to a specific user" do
          expect(Link.fetch_leniently("custom")).to eq(product_2)
        end

  test "fetches a product for the correct user" do
  ...
  context_ "legacy permalink mapping" do
  context_ "when no mapping exists" do
  test "fetches the oldest product if no user is provided" do
            expect(Link.fetch_leniently("custom")).to eq(product_2)
          end

  test "fetches a product for the correct user if one is provided" do
            expect(Link.fetch_leniently("custom", user: user_2)).to eq(product_6)
          end
        end

  context_ "when legacy permalink mapping exists" do
          before do
            create(:legacy_permalink, permalink: "custom", product: product_6)
          end

  test "fetches a mapped product when no user provided" do
  ...
  test "fetches a user's product when user is provided" do
  ...
  test "does not fetch a product by custom permalink" do
  ...
  context_ "#matches_permalink?" do
      let(:product) { build(:product, unique_permalink: "aB1", custom_permalink: "custom") }

  test "returns false on no match" do
        expect(product.matches_permalink?("invalid")).to be(false)
      end

  test "returns true on exact match via unique permalink" do
        expect(product.matches_permalink?("aB1")).to be(true)
      end

  test "returns true on case-insensitive match via unique permalink" do
        expect(product.matches_permalink?("ab1")).to be(true)
      end

  test "returns false on partial match via unique permalink" do
        expect(product.matches_permalink?("aB")).to be(false)
      end

  test "returns true on exact match via custom permalink" do
  ...
  context_ "when custom permalink is blank" do
        let(:product) { build(:product) }

  test "returns false if empty permalink is passed" do
  ...
  context_ "name" do
  test "is invalid if super long" do
        expect(build(:product, name: "hi there" * 255).valid?).to be(false)
      end
    end

  context_ "#checkout_custom_fields" do
      let(:product) { create(:product) }
  ...
  test "returns all non-post-purchase custom fields" do
        expect(product.checkout_custom_fields).to eq([global_custom_field, custom_field])
      end
    end

  context_ "#custom_field_descriptors" do
      let(:product) { create(:product) }

  test "returns formatted custom fields" do
  ...
  context_ "#custom_view_content_button_text" do
  ...
  test "errors if text is longer than 26 characters" do
        product = create(:product)
  ...
  context_ "#content_cannot_contain_adult_keywords" do
      let(:product) { create(:product) }

  context_ "content is safe" do
  test "does not add errors to the product" do
  ...
  context_ "description contains adult keywords" do
  test "adds an error to the product" do
  ...
  context_ "name contains adult keywords" do
  test "adds an error to the product" do
          product.name = "fetish"
          product.save
          expect(product.errors.full_messages).to include("Adult keywords are not allowed")
  ...
  context_ "#bundle_is_not_in_bundle" do
      let!(:product) { create(:product, :unpublished) }

  context_ "product is not in bundle" do
  test "does not add an error to the product" do
  ...
  context_ "product is in bundle" do
        before do
  ...
  context_ "product was in bundle" do
        before do
          bundle = create(:product, user: product.user, is_bundle: true)
  ...
  context_ "multifile_aware_product_file_info" do
      before do
  ...
  test "returns the right file info for each type of product" do
  ...
  context_ "removed_file_info_attributes" do
  test "sets removed_file_info_attributes correctly" do
        link = build(:product)
  ...
  context_ "when attributes have S3 URLs" do
      before do
  ...
  context_ "#description" do
  test "replaces S3 URLs with CDN Proxy URLs" do
  ...
  context_ "description formatting" do
      before do
        @product = create(:product)
      end

  test "removes xml tags from the description field if there are any" do
  ...
  test "is a single-unit currency" do
        expect(subject.send(:single_unit_currency?)).to be(true)
      end
    end

  context_ "#remaining_for_sale_count" do
  test "defaults to nil" do
        expect(link.max_purchase_count).to be(nil)
        expect(link.remaining_for_sale_count).to be(nil)
      end

  test "does not return nil if there's no max purchase count if there is a tier with a max purchase count" do
  ...
  context_ "less than infinity" do
  ...
  context_ "purchases were made" do
  ...
  test "decrements the remaining count" do
            expect(link.remaining_for_sale_count).to eq 48
          end

  context_ "user attempts to set remaining purchase count below the number of purchases" do
  ...
  context_ "bundle product" do
        let(:bundle) { create(:product, :bundle, max_purchase_count: 3) }

  test "returns the minimum quantity remaining out of the bundle and the bundle products" do
  ...
  test "excludes deleted bundle products" do
          expect(bundle.remaining_for_sale_count).to eq(3)
          bundle.bundle_products.second.product.update!(max_purchase_count: 2)
          expect(bundle.remaining_for_sale_count).to eq(2)
  ...
  context_ "#remaining_call_availabilities" do
      let(:call_product) { create(:call_product) }

  test "calls Product::ComputeCallAvailabilitiesService with self as parameter" do
  ...
  context_ "plaintext description" do
  test "keeps a normal description the same" do
        link = create(:product, description: "I like pie.")
        expect(link.plaintext_description).to eq "I like pie."
      end

  test "properly cleans a html description" do
  ...
  test "encodes lone `<`, `>` characters" do
  ...
  context_ "default_price_recurrence" do
  test "returns the product price that has the product's subscription duration" do
  ...
  context_ "for a non-recurring billing product" do
  test "returns the last price" do
          product = create(:product)
  ...
  context_ "for a recurring billing product that is not a tiered membership" do
  test "returns the price with the correct recurrence" do
  ...
  test "returns the product price with the correct recurrence" do
  ...
  context_ "#validate_product_price_against_all_offer_codes?" do
  test "uses tiered discount amounts across membership tier prices" do
  ...
  context_ "suggested_price_cents" do
  test "suggested_price should set suggested_price_cents" do
        link = create(:product)
  ...
  test "suggested_price_formatted should be correct" do
        link = create(:product, suggested_price_cents: 400)
        expect(link.suggested_price_formatted).to eq "4"
      end

  test "suggested_price_cents cannot be less than price cents" do
        link = create(:product)
        link.price_range = "2+"
  ...
  context_ "non-customizable price" do
  test "does not validate suggested price" do
  ...
  context_ "#price_range" do
  test "can be assigned a number" do
        link.price_range = 1
  ...
  test "treats a tailing plus sign as customizable" do
  ...
  context_ "american dollar" do
        before { link.price_currency_type = :usd }

  test "sets price cents" do
          link.price_range = "1"
          expect(link.price_cents).to eq 100
  ...
  test "saves price cents correctly too" do
          link.price_range = "10.01"
          link.save!
          expect(link.price_cents).to eq 1001
        end
  ...
  test "sets price cents correctly for yen if they try to do point values" do
          link.price_range = "¥100.01"
          expect(link.price_cents).to eq 100
        end

  test "saves price cents correctly" do
          link.save!
          expect(link.price_cents).to eq 100
        end

  test "saves price cents from price range if the yen symbol is not included" do
          link.price_range = "100"
          expect(link.price_cents).to eq 100
          link.save!
          expect(link.price_cents).to eq 100
  ...
  context_ "zero+" do
  test "is valid with 0+ price_range" do
          link.price_range = "0+"
          expect(link.save).to be(true)
        end

  test "is invalid with 0.5+ price_range" do
          link.price_range = "0.50+"
          expect(link.save).to be(false)
        end

  test "is valid with 1+ price_range" do
          link.price_range = "1+"
          expect(link.save).to be(true)
        end
      end

  context_ "euro style" do
  ...
  test "treats a tailing plus sign in rental price as customizable price only if product is rent-only" do
        link.purchase_type = :rent_only
  ...
  test "creates a permalink" do
      link.save!
      expect(link.unique_permalink).not_to be(nil)
    end

  context_ "#price_formatted" do
      before(:each) { @product = create(:product) }
  context_ "usd price formatted standard" do
  ...
  context_ "usd price formatted non-standard" do
        before(:each) do
  ...
  context_ "usd price formatted with customizable price" do
        before(:each) do
  ...
  context_ "jpy price formatted standard" do
        before(:each) do
  ...
  context_ "jpy price formatted non-standard" do
        before(:each) do
  ...
  context_ "jpy price formmated with customizable price" do
        before(:each) do
  ...
  context_ "total_usd_cents_earned_by_user", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  test "sums up earnings for the given user" do
        user = create(:user)
  ...
  context_ "compliance_blocked" do
  test "is false if a good ip" do
        ip = "199.21.86.138" # San Francisco WebPass
        expect(build(:product).compliance_blocked(ip)).to be(false)
      end

  test "blocks ips from 'bad' countries, like Libya" do
        ip = "41.208.70.70" # Tripoly Libya Telecom
        expect(build(:product).compliance_blocked(ip)).to be(true)
      end

  test "does not block ips when we do not know the ip" do
        ip = nil
        expect(build(:product).compliance_blocked(ip)).to be(false)
      end

  test "does not block ips when we cannot identify the country" do
        ip = "10.0.1.1"
        expect(build(:product).compliance_blocked(ip)).to be(false)
      end
    end

  context_ "#long_url" do
      before :each do
        @product = create(:product)
      end

  test "returns long_url of the product with seller's subdomain" do
        expect(@product.long_url).to eq "#{@product.user.subdomain_with_protocol}/l/#{@product.general_permalink}"
      end

  test "appends the 'recommended_by' query parameter if one is present" do
  ...
  test "appends the 'code' query parameter if one is present" do
  ...
  test "does not append the 'recommended_by' query parameter if the value is blank" do
  ...
  test "doesn't include protocol if include_protocol is set to false" do
        expect(@product.long_url(include_protocol: false)).to eq "#{@product.user.subdomain}/l/#{@product.general_permalink}"
      end
    end

  context_ "#thumbnail_or_cover_url" do
      let(:product) { create(:product) }

  test "returns nil when the product has no thumbnail or covers" do
        expect(product.thumbnail_or_cover_url).to be_nil
      end

  test "returns the thumbnail or falls back to the first cover image" do
  ...
  context_ "#for_email_thumbnail_url" do
      let(:product) { create(:product) }

  context_ "when the product doesn't have a thumbnail" do
  test "returns product type thumbnail" do
  ...
  context_ "when the product has an active thumbnail" do
        before do
  ...
  context_ "#release_custom_permalink_if_possible" do
      before do
        @user = create(:user)
  ...
  test "releases the custom permalink of the user's deleted product" do
  ...
  test "does not release the custom permalink of a non-deleted product" do
  ...
  test "does not release the custom permalink of another user's deleted product" do
  ...
  context_ "#has_stampable_pdfs?" do
      before do
        @product = create(:product)
      end

  test "returns false if product has no product files" do
        expect(@product.has_stampable_pdfs?).to eq(false)
      end

  test "returns false if product has no stampable pdf files" do
  ...
  test "returns true if product has at least one stampable pdf file" do
        @product.product_files << create(:non_readable_document)
  ...
  context_ "#customize_file_per_purchase?" do
      before do
        @product = create(:product)
      end
  ...
  context_ "#allow_parallel_purchases?" do
  test "returns false if the product is a call" do
        expect(create(:call_product).allow_parallel_purchases?).to eq(false)
      end

  test "returns false if the product has a max purchase count" do
  ...
  test "returns false if product has at least one stampable pdf file" do
        @product.product_files << create(:non_readable_document)
        @product.product_files << create(:readable_document, pdf_stamp_enabled: true)
  ...
  test "returns false if product has files that are all marked stream-only" do
  ...
  test "returns true if product has unstampable files that are not marked stream-only" do
        @product.product_files << create(:non_readable_document)
        @product.product_files << create(:streamable_video, stream_only: true)
        @product.product_files << create(:readable_document, pdf_stamp_enabled: false)
  ...
  context_ "#create_licenses_for_existing_customers" do
  test "queues the CreateLicensesForExistingCustomersWorker job once when licensing is enabled" do
  ...
  test "does not queue the CreateLicensesForExistingCustomersWorker job when licensing is disabled" do
  ...
  test "does not queue the CreateLicensesForExistingCustomersWorker job when a non-licensing attribute is updated" do
        product = create(:product_with_pdf_file)
  ...
  context_ "subscription_duration" do
      let(:link) { create(:product, subscription_duration: :monthly) }

  test "persists integer correctly" do
  ...
  context_ "preorders" do
  test "allows a product to be created if it's for a preorder" do
  ...
  context_ "offer_code creation" do
      before :each do
        @product_for_code = create(:product, price_currency_type: "eur", price_cents: 240)
      end

  test "can create an offer code that lowers the price to 0" do
  ...
  test "can create an offer code that keeps the price above the minimum" do
  ...
  test "cannot create an offer code that brings the price to below the minimum" do
  ...
  context_ "#delete!" do
  test "marks custom domain as deleted" do
        product = create(:product)
  ...
  test "enqueues subscription cancellations" do
  ...
  test "enqueues rich content deletions" do
        product = create(:product)
  ...
  test "deletes all product files and archives if there are no successful sales" do
        link = create(:product)
  ...
  test "enqueues wishlist product deletions" do
        product = create(:product)
        product.delete!
        expect(DeleteWishlistProductsJob).to have_enqueued_sidekiq_job(product.id).in(10.minutes)
      end

  test "schedules associated public files for deletion" do
        product = create(:product)
  ...
  test "deletes a tiered membership even when tier categories are in an inconsistent state" do
        product = create(:membership_product_with_preset_tiered_pricing)
  ...
  context_ "#ordered_by_ids" do
  test "returns a list of users' products ordered as per the input list of ids" do
        creator = create(:user)
  ...
  test "returns proper variants" do
          product = create(:membership_product)
  ...
  context_ "for a non-membership product" do
  test "returns nil" do
          product = create(:product)
  ...
  context_ "#default_tier" do
  context_ "for a tiered membership" do
  test "returns the first tier" do
          product = create(:membership_product)
  ...
  context_ "#tier_category" do
  context_ "for tiered membership" do
  test "returns the tier category" do
          product = create(:membership_product)
  ...
  context_ "has_downloadable_content?" do
      let(:product) { create(:product) }
      before do
  ...
  test "returns false if product has no files" do
        expect(product.has_downloadable_content?).to eq(false)
      end

  test "returns false if product is a pre-order" do
  ...
  test "returns true if product has at least one file that is not stream-only" do
  ...
  context_ "#save_shipping_destinations" do
      before do
        @product = create(:product)
      end

  test "clears all entries if the input is empty for an unpublished product" do
  ...
  test "raises an exception if the input is empty for an product" do
        expect(@product.shipping_destinations.size).to eq(0)
  ...
  test "saves the entry if input has unique country values" do
  ...
  test "saves the entry if the values are specified in cents" do
        shipping_destination_inputs = []
  ...
  test "rejects duplicated and raises an error" do
        shipping_destination_inputs = []
        shipping_destination_inputs << { "country_code" => Compliance::Countries::USA.alpha2, "one_item_rate" => 20, "multiple_items_rate" => 10 }
  ...
  test "removes entries that do not exist in the input" do
        shipping_destination1 = ShippingDestination.new(country_code: Product::Shipping::ELSEWHERE, one_item_rate_cents: 20, multiple_items_rate_cents: 10)
        shipping_destination2 = ShippingDestination.new(country_code: Compliance::Countries::DEU.alpha2, one_item_rate_cents: 10, multiple_items_rate_cents: 5)
  ...
  test "resurrects deactivated entries if input reconfigures them" do
        shipping_destination1 = ShippingDestination.new(country_code: Product::Shipping::ELSEWHERE, one_item_rate_cents: 20, multiple_items_rate_cents: 10)
        shipping_destination2 = ShippingDestination.new(country_code: Compliance::Countries::DEU.alpha2, one_item_rate_cents: 10, multiple_items_rate_cents: 5)
  ...
  test "saves the entry if input has unique country values and sets is_virtual_country" do
          shipping_destination_inputs = []
  ...
  test "has the proper buy price" do
  ...
  test "has the proper buy price when the price changes" do
  ...
  test "has the proper rent price" do
        expect do
  ...
  test "only has the rental price if it's rent-only" do
        expect do
          @product.rental_price_cents = 100
          @product.purchase_type = :buy_and_rent
          @product.save!
  ...
  test "has the proper price for subscriptions" do
  ...
  test "has the proper price for subscriptions when the price changes" do
  ...
  test "has the proper currency" do
  ...
  context_ "require_shipping_for_physical" do
  test "does not create a physical product with require_shipping false" do
        expect(build(:product, is_physical: true, require_shipping: false).valid?).to be(false)
      end

  test "has error if require shipping is false for a physical product" do
        product = create(:physical_product)
  ...
  context_ "twitter_share_url" do
  test "uris escape the product name" do
  ...
  context_ "duration_multiple_of_price_options" do
      before do
        @product = create(:subscription_product, subscription_duration: "yearly", duration_in_months: 12)
      end

  test "allows a null duration_in_months" do
  ...
  test "allows a duration_in_months that is a multiple of 12 for yearly" do
  ...
  test "errors if duration_in_months is 0" do
  ...
  test "errors if duration_in_months is not a multiple of 12 for yearly" do
  ...
  test "updates positions of previews" do
  ...
  context_ "#rated_as_adult?" do
  test "returns true if the product is set to is_adult" do
        product = create(:product, is_adult: true)
        expect(product.rated_as_adult?).to eq(true)
      end

  test "returns true if the user profile is set to all_adult_products" do
        product = create(:product, user: create(:user, all_adult_products: true))
        expect(product.rated_as_adult?).to eq(true)
      end

  test "returns true if the product contains any adult keywords" do
        product = create(:product)
  ...
  context_ "#has_adult_keywords?" do
  test "returns true if product fields have adult keywords" do
  ...
  context_ "classifies description as expected" do
        shared_examples_for "test" do |description, adult|
  test "\"#{description}\" as #{'non-' unless adult}adult" do
  ...
  context_ "for a product with rich content" do
        let(:product) { create(:product) }

  context_ "when the product has no rich content" do
  ...
  context_ "when the product has rich content" do
  ...
  test "formats #{currency_type.to_s.upcase} currency properly" do
  ...
  test "returns creator's name if present, otherwise the username" do
  ...
  context_ "#gumroad_amount_for_paypal_order" do
      let(:creator) { create(:user) }
      let(:product) { create(:product, user: creator) }

  test "returns 10% of the given amount" do
  ...
  test "adds discover fee minus 10%" do
  ...
  test "adds the affiliate fee" do
  ...
  test "does not add the affiliate fee for Discover sales" do
  ...
  test "adds the affiliate fee for a global affiliate" do
            expect(product.gumroad_amount_for_paypal_order(amount_cents: 10_00, affiliate_id: affiliate.id)).to eq(gumroad_fee + affiliate_fee)
          end

  test "still adds the affiliate fee for Discover sales" do
  ...
  test "adds vat amount if vat is present" do
  ...
  context_ "#free_trial_duration" do
  test "returns nil if free trial is not enabled" do
        product = build(:product)
        expect(product.free_trial_duration).to eq nil
      end

  test "returns the free trial duration if free trial is enabled" do
  ...
  context_ "#has_customizable_price_option?" do
  context_ "for a non-tiered membership product" do
  test "returns true if the product has a customizable price" do
          product = build(:product, customizable_price: true)
          expect(product.has_customizable_price_option?).to eq true
        end

  test "returns false if the product does not have a customizable price" do
  ...
  test "returns false if the product has no customizable price tiers" do
          expect(product.has_customizable_price_option?).to eq false
        end

  test "returns true if the product has a customizable price tier" do
          product.default_tier.update!(customizable_price: true)
          expect(product.has_customizable_price_option?).to eq true
        end

  test "returns false if the product has only deleted customizable price tiers" do
  ...
  context_ "#recurrence_price_enabled?" do
  test "returns true if the product has a live price for the given recurrence" do
        product = create(:product)
  ...
  test "returns false if the product does not have a live price for the given recurrence" do
        product = create(:product)
  ...
  context_ "#has_multiple_variants?" do
  context_ "for a physical product" do
        let(:product) { create(:physical_product) }

  test "returns false if the product has only a default SKU" do
          expect(product.has_multiple_variants?).to eq false
        end

  test "returns false if the product has only a default SKU and deleted custom SKUs" do
          create(:sku, link: product, deleted_at: Time.current)
          expect(product.has_multiple_variants?).to eq false
        end

  test "returns true if the product has a default SKU and a single live custom SKU" do
          # if there are any custom SKUs, those SKUs are used and the default SKU is not
          create(:sku, link: product)
          expect(product.has_multiple_variants?).to eq false
        end

  test "returns true if the product has a default SKU and multiple live custom SKUs" do
          create_list(:sku, 2, link: product)
          expect(product.has_multiple_variants?).to eq true
        end
      end

  context_ "for a non-physical product" do
        let(:product) { create(:product) }
        let(:category) { create(:variant_category, link: product) }

  test "returns false if the product has no live variants" do
          create(:variant, variant_category: category, deleted_at: Time.current)
          expect(product.has_multiple_variants?).to eq false
        end

  test "returns false if the product has a single live variant" do
          create(:variant, variant_category: category)
          expect(product.has_multiple_variants?).to eq false
        end

  test "returns true if the product has a single variant category with multiple live variants" do
          create_list(:variant, 2, variant_category: category)
          expect(product.has_multiple_variants?).to eq true
        end

  test "returns true if the product has multiple variant categories with live variants" do
  ...
  context_ "associations" do
  context_ "has many `product_integrations`" do
  test "returns alive and deleted product_integrations" do
  ...
  context_ "has many `live_product_integrations`" do
  test "does not return deleted product_integrations" do
          integration_1 = create(:circle_integration)
          integration_2 = create(:circle_integration)
          product = create(:product, active_integrations: [integration_1, integration_2])
          expect do
  ...
  context_ "has many `product_cached_values`" do
  test "returns all product cached values" do
          product = create(:product)
  ...
  test "has many `alive_variants`" do
          expect(product.alive_variants).to match_array [alive_variant]
        end
      end
    end

  context_ "#has_active_paid_variants?" do
      before do
  ...
  test "returns false when there are no active paid variants" do
        expect(@product.has_active_paid_variants?).to eq(false)
      end

  test "returns true when there are active paid variants" do
  ...
  context_ "when description is empty" do
        let(:product) { create(:product_with_pdf_file, description: "") }

        it { is_expected.to be_nil }
      end

  context_ "when description contains unsafe tags and attributes" do
  ...
  test "removes unsafe and unknown tags and attributes from the description" do
  ...
  context_ "#sku_title" do
  test "is \"Version\" when there are no current categories" do
        product = create(:product)
        expect(product.sku_title).to eq("Version")
      end

  test "is the categories concatenated together when there are current categories" do
        product = create(:product)
  ...
  context_ "#options" do
  test "returns SKUs if the product has SKUs enabled" do
        product = create(:physical_product)
  ...
  test "returns variants if the product does not have SKUs" do
  ...
  context_ "#enable_transcode_videos_on_purchase!" do
  test "sets transcode_videos_on_purchase to true" do
        product = create(:product)
  ...
  context_ "#auto_transcode_videos?" do
      before do
        @product = create(:product)
      end

  context_ "when user.auto_transcode_videos? returns true" do
        before do
  ...
  context_ "when product has successful sales" do
        before do
  ...
  context_ "#permalink_overlaps_with_other_sellers?" do
      before do
        create(:product, unique_permalink: "abc", custom_permalink: "xyz")
      end

  test "returns true when custom permalink overlaps with products from other sellers" do
        product = create(:product, custom_permalink: "abc")

        expect(product.permalink_overlaps_with_other_sellers?).to eq true
      end

  test "returns true when unique permalink overlaps with products from other sellers" do
        product = create(:product, unique_permalink: "xyz")

        expect(product.permalink_overlaps_with_other_sellers?).to eq true
      end
  test "returns false when permalinks don't overlap with products from other sellers" do
  ...
  context_ "when the PPP factor doesn't exist" do
        before do
  ...
  context_ "with purchasing_power_parity_disabled" do
        before do
  ...
  context_ "when the PPP factor is 1" do
  test "returns nil" do
          expect(@product.ppp_details(@us_ip)).to eq(nil)
        end
      end

  context_ "when the PPP factor exists and isn't 1" do
  test "returns the PPP details" do
  ...
  context_ "purchasing_power_parity_enabled?" do
      before do
        @product = create(:product, user: create(:user))
      end

  context_ "when the user doesn't have purchasing_power_parity_enabled" do
  test "returns false when purchasing_power_parity_disabled is false" do
          expect(@product.purchasing_power_parity_enabled?).to eq(false)
        end

  test "returns false when purchasing_power_parity_disabled is true" do
          @product.update! purchasing_power_parity_disabled: true
  ...
  context_ "when the user has purchasing_power_parity_enabled" do
        before do
          @product.user.update! purchasing_power_parity_enabled: true
        end

  test "returns true when purchasing_power_parity_disabled is false" do
  ...
  context_ "#has_offer_codes?" do
      let(:product) { create(:product) }

  context_ "when the product has offer codes" do
        let!(:offer_code) { create(:offer_code, user: product.user, products: [product]) }

  context_ "when the user-level flag is enabled" do
          before do
  ...
  context_ "when the user-level flag is disabled" do
  test "returns false" do
  ...
  context_ "when the product doesn't have offer codes" do
  context_ "when the user-level flag is enabled" do
          before do
            product.user.update!(display_offer_code_field: true)
          end
  ...
  context_ "#available_cross_sells" do
  ...
  test "returns cross-sells that are are either for the product or universal" do
        expect(product.available_cross_sells).to contain_exactly(cross_sell_for_product, cross_sell_universal)
      end

  test "does not return cross-sells that are paused or deleted" do
  ...
  context_ "#find_or_initialize_product_refund_policy" do
  context_ "when the product has a refund policy" do
        let(:product_refund_policy) { create(:product_refund_policy) }

  test "returns the product's refund policy" do
          expect(product_refund_policy.product.find_or_initialize_product_refund_policy).to eq(product_refund_policy)
        end
      end

  context_ "when the product doesn't have a refund policy" do
        let(:product) { create(:product) }

  test "returns a new refund policy" do
  ...
  context_ "#purchase_info_for_product_page" do
      let(:product) { create(:product, is_in_preorder_state:) }

  context_ "when there is a matching user" do
        let(:user) { create(:user) }

  context_ "when the purchase is not a preorder" do
          let(:is_in_preorder_state) { false }

  context_ "when the user has a previous purchase" do
  ...
  test "returns purchase_info" do
              expect(product.purchase_info_for_product_page(user, nil)).to eq(purchase.purchase_info)
            end
          end

  context_ "when the user has a previous gift sender purchase" do
  ...
  context_ "when the user has a previous gift receiver purchase" do
  ...
  context_ "when the purchase is a preorder" do
  ...
  context_ "when there is no user but a browser guid" do
        let(:user) { nil }

  context_ "when the purchase is not a preorder" do
          let(:is_in_preorder_state) { false }

  context_ "when there is a previous purchase with the browser guid" do
  ...
  context_ "when there is a previous gift sender purchase with the browser guid" do
  ...
  context_ "when there is a previous purchase received as a gift with the browser guid" do
  ...
  context_ "when there is a non-matching user and a matching browser guid" do
        let(:user) { create(:user) }
  ...
  context_ "when there is no user or browser guid" do
        let(:purchase) { create(:purchase, link: product) }
        let(:is_in_preorder_state) { false }
  ...
  context_ "service product validation" do
  context_ "seller of a service product is not eligible for service products" do
        let(:commission) { build(:product, native_type: "commission") }

  test "adds a validation error for a service product" do
          commission.save
  ...
  context_ "seller of a service product is eligible for service products" do
  ...
  test "does not add a validation error for a service product" do
          commission.save
  ...
  test "updates the profile sections correctly" do
        seller.reload
  ...
  context_ "#variants_or_skus" do
  context_ "SKUs are enabled" do
        let(:product) { create(:physical_product) }

  context_ "no non-default SKUs" do
  test "returns an empty array" do
            expect(product.variants_or_skus).to eq([])
          end
        end

  context_ "one or more SKUs" do
          let!(:sku) { create(:sku, link: product) }

  test "returns the product's SKUs" do
            expect(product.variants_or_skus).to eq([sku])
          end
        end
      end

  context_ "SKUs are disabled" do
        let(:product) { create(:product) }
  ...
  context_ "one or more variants" do
  test "returns those variants" do
  ...
  context_ "#has_embedded_license_key?" do
  test "returns false when the product-level rich content does not have an embedded license key" do
        product = create(:product)
  ...
  test "returns true when the product-level rich content has an embedded license key" do
        product = create(:product)
  ...
  test "returns false when none of the variant-level rich content has an embedded license key" do
        product = create(:product)
  ...
  test "returns true when at least one of the variant-level rich content has an embedded license key" do
        product = create(:product)
        variant_category = create(:variant_category, link: product)
  ...
  context_ "#has_another_collaborator?" do
      let(:product) { create(:product) }
  ...
  test "returns true if the product has any live collaborators, regardless of invitation status" do
  ...
  context_ "#generate_product_files_archives!" do
  test "generates file group archives for a product with product-level rich content" do
        product = create(:product)
  ...
  test "generates file group archives for a product with variant-level rich content" do
        product = create(:product)
  ...
  test "regenerates file group archives containing the provided files" do
        product = create(:product)
        file1 = create(:product_file, link: product)
        file2 = create(:product_file, link: product)
        ...
        expect { product.generate_product_files_archives! }.not_to change { archive.reload.deleted? }
  ...
  context_ "#has_product_level_rich_content?" do
  test "returns true for products with product-level rich content, and false otherwise" do
        product = create(:product)
  ...
  context_ "#percentage_revenue_cut_for_user" do
      let(:product) { create(:product, is_collab: false) }

  context_ "for a non-collab" do
  test "returns 100 for the creator" do
          expect(product.percentage_revenue_cut_for_user(product.user)).to eq(100)
        end

  test "returns 0 for other users" do
          expect(product.percentage_revenue_cut_for_user(create(:user))).to eq(0)
        end
      end

  context_ "for a collab" do
        let(:product) { create(:product, :is_collab, collaborator_cut: 45_00) }

  test "returns the collaborator's cut for the collaborator, only if the invitation has been accepted" do
          seller = product.user
  ...
  context_ "when a tiered membership has an invalid tier structure" do
  ...
  test "allows unpublishing despite invalid tier structure" do
          expect { membership.unpublish! }.not_to raise_error
          expect(membership.reload.purchase_disabled_at).to be_present
        end

  test "still validates tier structure on other updates" do
  ...
  context_ "#alive?" do
  test "returns true if the product is available for purchase" do
        expect(build(:product).alive?).to eq true
      end

  test "returns false if the product has been banned, deleted, or purchase is disabled" do
  ...
  test "returns true if the product is available for purchase and not a draft" do
        expect(build(:product).published?).to eq true
      end

  test "returns false if the product is a draft, deleted, or unavailable for purchase" do
  ...
  context_ "price is at least double the currency minimum" do
  ...
  context_ "user doesn't have a coffee product" do
  ...
  context_ "unarchiving coffee product when another coffee product exists" do
  ...
  test "prevents unarchiving" do
          coffee_a.archived = false
          expect(coffee_a).not_to be_valid
          expect(coffee_a.errors.full_messages.first).to eq("You can only have one coffee product.")
        end
      end

  context_ "unarchiving coffee product when no other coffee product exists" do
        let!(:coffee_a) { create(:product, user: seller, native_type: Link::NATIVE_TYPE_COFFEE, archived: true) }

  test "allows unarchiving" do
          coffee_a.archived = false
          expect(coffee_a).to be_valid
        end
      end

  context_ "with zero price" do
  ...
  context_ "calls validations" do
  context_ "call has no durations" do
  ...
  test "does not add an error if product is deleted" do
  ...
  context_ "call has durations" do
  ...
  context_ "support email validations" do
      let(:product) { build(:product) }

  context_ "when support_email is nil" do
  test "is valid" do
  ...
  context_ "when support_email is blank" do
  test "is invalid" do
  ...
  context_ "when support_email has a valid email format" do
  test "is valid" do
  ...
  context_ "when support_email has an invalid email format" do
  test "is invalid without @ symbol" do
          product.support_email = "invalidemail"
          expect(product).not_to be_valid
          expect(product.errors[:support_email]).to include("is invalid")
        end

  test "is invalid without domain" do
  ...
  context_ "#can_gift?" do
      let(:product) { build(:product) }

  context_ "for a regular product" do
  test "returns true" do
          expect(product.can_gift?).to eq true
        end
      end

  context_ "when has a preorder state" do
  ...
  context_ "when it is a recurring product" do
  ...
  context_ "#quantity_enabled" do
  test "can only be false for membership product" do
  ...
  test "can only be false for call products" do
  ...
  test "can be true for other products" do
        product = build(:product)
  ...
  context_ "#can_enable_quantity?" do
  ...
  test "returns true for non-membership and non-call products" do
  ...
  context_ "#require_captcha?" do
      let(:seller) { create(:user) }
      let(:product) { create(:product, user: seller) }

  context_ "seller is older than 6 months" do
  ...
  context_ "seller is younger than 6 months" do
        before { seller.update(created_at: 6.months.ago + 1.day) }
  ...
  context_ ".eligible_for_content_upsells" do
  ...
  test "returns visible non-membership products including those with variants" do
        expect(Link.eligible_for_content_upsells).to match_array([regular_product, product_with_variants])
      end
    end

  context_ "installment plan" do
  ...
  test "re-validates the installment plan whenever the product is updated" do
  ...
  context_ "#toggle_community_chat" do
      let(:product) { create(:product) }

  context_ "when enabling community chat" do
  test "enables community chat and creates a new community if none exists" do
          expect do
  ...
  test "enables community chat and restores a deleted community if one exists" do
  ...
  test "does nothing if community chat is already enabled" do
  ...
  context_ "when disabling community chat" do
        before do
          product.update!(community_chat_enabled: true)
          create(:community, resource: product)
        end

  test "disables community chat and marks the associated active community as deleted" do
          expect(product.active_community).to eq(product.communities.last)
  ...
  test "does nothing if community chat is already disabled" do
  ...
  context_ "#cart_item" do
  context_ "when product is a tiered membership and the variant record is missing" do
        let(:product) { create(:membership_product) }
  ...
  test "falls back to the product prices instead of raising NoMethodError" do
