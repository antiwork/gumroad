...
class ShippingDestinationTest < ActiveSupport::TestCase
  self.described_class = ShippingDestination



  context_ ShippingDestination do
    before :each do
      @product = create(:product)
    end

  test "it does not allow saving if the country code is nil or invalid" do
  ...
  context_ "#calculate_shipping_rate" do
      before do
  ...
  test "returns nil for quantity < 1" do
        expect(@shipping_destination.calculate_shipping_rate(quantity: -1)).to be_nil
      end

  test "returns one_item_rate_cents for quantity = 1" do
        expect(@shipping_destination.calculate_shipping_rate(quantity: 1)).to eq(20)
      end

  test "returns one_item_rate_cents + (quantity -1)*multiple_items_rate_cents for quantity > 1" do
  ...
  context_ "#for_product_and_country_code" do
