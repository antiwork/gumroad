...
class PurchaseRefundPolicyTest < ActiveSupport::TestCase
  self.described_class = PurchaseRefundPolicy



  context_ PurchaseRefundPolicy do
  ...
  context_ "when created after MAX_REFUND_PERIOD_IN_DAYS_INTRODUCED_ON" do
          before do
  ...
  test "requires max_refund_period_in_days to be present" do
  ...
  test "allows valid max_refund_period_in_days values" do
  ...
  context_ "when created before MAX_REFUND_PERIOD_IN_DAYS_INTRODUCED_ON" do
          before do
            travel_to PurchaseRefundPolicy::MAX_REFUND_PERIOD_IN_DAYS_INTRODUCED_ON - 1.day
          end
  ...
  test "does not require max_refund_period_in_days to be present" do
            refund_policy = build(:purchase_refund_policy, purchase:, title: "30-day money back guarantee", max_refund_period_in_days: nil)

            expect(refund_policy.valid?).to be true
          end

  test "allows max_refund_period_in_days to be nil" do
            refund_policy = build(:purchase_refund_policy, purchase:, title: "30-day money back guarantee", max_refund_period_in_days: nil)
  ...
  context_ "when created_at is nil (new record)" do
  test "requires max_refund_period_in_days to be present" do
            refund_policy = build(:purchase_refund_policy, purchase:, title: "30-day money back guarantee", max_refund_period_in_days: nil)
  ...
  context_ "product_refund_policy helper methods" do
      let(:product) { create(:product) }
      let(:product_refund_policy) { create(:product_refund_policy, product:) }
      let(:purchase) { create(:purchase, link: product) }


  context_ "#different_than_product_refund_policy?" do
  context_ "when no product refund policy exists" do
  ...
  context_ "when max_refund_period_in_days is present" do
  ...
  context_ "when max_refund_period_in_days matches product refund policy" do
            let(:refund_policy) do
              purchase.create_purchase_refund_policy!(
  ...
  context_ "when max_refund_period_in_days differs from product refund policy" do
            let(:refund_policy) do
              purchase.create_purchase_refund_policy!(
                title: "Same title",
  ...
  context_ "when max_refund_period_in_days is not present" do
  ...
  context_ "when title matches product refund policy title" do
            let(:refund_policy) do
              build(
  ...
  context_ "when title differs from product refund policy title" do
            let(:refund_policy) do
              build(
                :purchase_refund_policy,
                purchase:,
