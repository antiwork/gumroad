...
class ModelsPaymentOptionInstallmentPlanSnapshotTest < ActiveSupport::TestCase



  context_ "PaymentOption installment plan snapshots" do
  ...
  context_ "when product price increases" do
  test "maintains original installment amounts" do
          expect(snapshot.total_price_cents).to eq(14700)
          expect(snapshot.number_of_installments).to eq(3)
  ...
  context_ "when product price decreases" do
  test "maintains original installment amounts" do
  ...
  context_ "installment configuration protection" do
      let!(:snapshot) do
        create(:installment_plan_snapshot,
               payment_option: payment_option,
               number_of_installments: 3,
  ...
  context_ "when installment count changes" do
  test "maintains original count when changed from 3 to 2" do
  ...
  test "maintains original count when changed from 3 to 5" do
  ...
  context_ "when no snapshot exists" do
  test "can still access installment_plan through payment_option" do
  ...
  context_ "when snapshot exists" do
        let!(:snapshot) do
          create(:installment_plan_snapshot,
                 payment_option: payment_option,
  ...
  test "can access both snapshot and live plan" do
