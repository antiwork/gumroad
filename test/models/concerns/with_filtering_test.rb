...
class WithFilteringTest < ActiveSupport::TestCase
  self.described_class = WithFiltering



  context_ WithFiltering do
  context_ "scopes" do
  context_ "abandoned_cart_type" do
  test "returns only installments with the 'abandoned_cart' type" do
  ...
  test "returns only workflows with the 'abandoned_cart' type" do
  ...
  context_ "#purchase_passes_filters" do
  context_ "for created filters" do
        before do
  ...
  context_ "when created_before filter is set" do
          before do
            @post = create(:seller_installment, seller: @product.user, json_data: { created_before: 1.day.ago })
          end

  test "returns true for purchase older than the created_before date" do
            expect(@post.purchase_passes_filters(@old_purchase)).to eq(true)
          end

  test "returns false for purchase newer than the created_before date" do
            expect(@post.purchase_passes_filters(@purchase)).to eq(false)
          end
        end

  context_ "when created_after filter is set" do
          before do
            @post = create(:seller_installment, seller: @product.user, json_data: { created_after: 1.day.ago })
          end

  test "returns true for purchase newer than the created_after date" do
            expect(@post.purchase_passes_filters(@purchase)).to eq(true)
          end

  test "returns false for purchase older than the created_after date" do
            expect(@post.purchase_passes_filters(@old_purchase)).to eq(false)
          end
        end

  context_ "when created_before and created_after filters are set" do
          before do
  ...
  test "returns true for purchase within the created dates" do
            expect(@post.purchase_passes_filters(@purchase_2)).to eq(true)
          end

  test "returns false for purchases outside the created dates" do
            expect(@post.purchase_passes_filters(@purchase)).to eq(false)
            expect(@post.purchase_passes_filters(@old_purchase)).to eq(false)
          end
        end
  ...
  context_ "when paid_more_than_cents filter is set" do
          before do
            @post = create(:seller_installment, seller: @product.user, json_data: { paid_more_than_cents: 100 })
          end

  test "returns true for purchases higher than the paid_more_than_cents filter" do
  ...
  test "returns false for purchases lower than the paid_more_than_cents filter" do
            expect(@post.purchase_passes_filters(@small_purchase)).to eq(false)
          end
        end

  context_ "when paid_less_than_cents filter is set" do
          before do
            @post = create(:seller_installment, seller: @product.user, json_data: { paid_less_than_cents: 100 })
          end

  test "returns false for purchases higher than the paid_more_than_cents filter" do
  ...
  test "returns true for purchases lower than the paid_more_than_cents filter" do
            expect(@post.purchase_passes_filters(@small_purchase)).to eq(true)
          end
        end

  context_ "when paid_more_than_cents and paid_less_than_cents filters are set" do
          before do
            @post = create(:seller_installment, seller: @product.user, json_data: { paid_more_than_cents: 100, paid_less_than_cents: 500 })
          end

  test "returns true for purchase within the price range" do
            expect(@post.purchase_passes_filters(@medium_purchase)).to eq(true)
          end

  test "returns false for purchases outside the price range" do
            expect(@post.purchase_passes_filters(@small_purchase)).to eq(false)
            expect(@post.purchase_passes_filters(@big_purchase)).to eq(false)
          end
        end
  ...
  context_ "when bought from country filter is set" do
        before do
          @product = create(:product)
  ...
  test "returns true for purchases from selected bought_from country filter" do
          expect(@post.purchase_passes_filters(@us_purchase_1)).to eq(true)
          expect(@post.purchase_passes_filters(@us_purchase_2)).to eq(true)
        end

  test "returns false for purchase not matching the bought_from country filter" do
          expect(@post.purchase_passes_filters(@non_us_purchase)).to eq(false)
        end
      end

  context_ "bought products and variants filters" do
        before do
          @product = create(:product)
  ...
  context_ "not bought filters" do
  ...
  context_ "not bought products only" do
  context_ "when the purchased product is not included in the filter" do
  ...
  context_ "when the purchased product is included in the filter" do
  ...
  context_ "not bought variants only" do
          before do
            @purchase.variant_attributes = [@variant]
          end

  context_ "when the purchased variant is not included in the filter" do
  ...
  context_ "not bought products and variants" do
  context_ "when the purchased product is included in the filter" do
            let(:not_bought_products) { [@product.unique_permalink] }
            let(:not_bought_variants) { [create(:variant).external_id] }
  ...
  context_ "when the purchased variant is included in the filter" do
            let(:not_bought_products) { [create(:product).unique_permalink] }
            let(:not_bought_variants) { [@variant.external_id] }
  ...
  context_ "bought and not bought filters" do
  context_ "memberships that have changed tier" do
          before do
  ...
  test "returns true if the membership used to have the 'not bought' variant and currently has the 'bought' variant" do
  ...
  test "returns false if the membership used to have the 'bought' variant and currently has the 'not bought' variant" do
  ...
  context_ "#seller_post_passes_filters" do
      before do
        @creator = create(:user)
      end
  ...
  test "returns true for max_created_at older than the created_before date" do
            expect(@post.seller_post_passes_filters(max_created_at: 1.week.ago)).to eq(true)
          end

  test "returns false for max_created_at newer than the created_before date" do
            expect(@post.seller_post_passes_filters(max_created_at: Time.current)).to eq(false)
          end

  test "returns false when passed filters are missing" do
  ...
  test "returns true for min_created_at newer than the created_after date" do
            expect(@post.seller_post_passes_filters(min_created_at: Time.current)).to eq(true)
          end

  test "returns false for min_created_at older than the created_after date" do
  ...
  test "returns true when created timestamp filters are within the created dates" do
            expect(@post.seller_post_passes_filters(min_created_at: 1.week.ago, max_created_at: 2.days.ago)).to eq(true)
          end

  test "returns false for created timestamp filters are outside the created dates" do
  ...
  test "returns true for min_price_cents higher than the paid_more_than_cents" do
            expect(@post.seller_post_passes_filters(min_price_cents: 150)).to eq(true)
          end

  test "returns false for min_price_cents lower than the paid_more_than_cents" do
  ...
  test "returns true for max_price_cents lower than the paid_less_than_cents" do
            expect(@post.seller_post_passes_filters(max_price_cents: 50)).to eq(true)
          end

  test "returns false for max_price_cents higher than the paid_less_than_cents" do
  ...
  test "returns true when price filters are within the post's price range" do
            expect(@post.seller_post_passes_filters(min_price_cents: 200, max_price_cents: 200)).to eq(true)
          end

  test "returns false when price filters are outside the post's price range" do
  ...
  test "returns true for country matching the selected bought_from country filter" do
          expect(@post.seller_post_passes_filters(country: "United States")).to eq(true)
        end

  test "returns true for ip_country matching the selected bought_from country filter" do
          expect(@post.seller_post_passes_filters(ip_country: "United States")).to eq(true)
        end

  test "returns false for country different than the selected bought_from country filter" do
          expect(@post.seller_post_passes_filters(country: "Canada")).to eq(false)
        end

  test "returns false for ip_country different than the selected bought_from country filter" do
          expect(@post.seller_post_passes_filters(country: "Canada")).to eq(false)
        end
  ...
  test "returns false when product permalinks don't match the bought_products filter" do
  ...
  test "returns true when one product permalink matches the bought_products filter" do
            post = create(:seller_installment, seller: @creator, json_data: { bought_products: %w[a b] })
  ...
  context_ "#affiliate_passes_filters" do
  context_ "for created filters" do
        before do
          @product = create(:product)
  ...
  test "returns true for affiliates older than the created_before date" do
            expect(@post.affiliate_passes_filters(@old_direct_affiliate)).to eq(true)
          end

  test "returns false for affiliates newer than the created_before date" do
  ...
  test "returns true for affiliates newer than the created_after date" do
            expect(@post.affiliate_passes_filters(@direct_affiliate)).to eq(true)
          end

  test "returns false for affiliates older than the created_after date" do
  ...
  test "returns true for affiliates within the created dates" do
            expect(@post.affiliate_passes_filters(@direct_affiliate_2)).to eq(true)
          end

  test "returns false for affiliates outside the created dates" do
            expect(@post.affiliate_passes_filters(@direct_affiliate)).to eq(false)
            expect(@post.affiliate_passes_filters(@old_direct_affiliate)).to eq(false)
          end
        end
  ...
  context_ "#follower_passes_filters" do
  context_ "for created filters" do
        before do
          @product = create(:product)
  ...
  test "returns true for followers older than the created_before date" do
            expect(@post.follower_passes_filters(@old_follower)).to eq(true)
          end

  test "returns false for followers newer than the created_before date" do
  ...
  test "returns true for followers newer than the created_after date" do
            expect(@post.follower_passes_filters(@follower)).to eq(true)
          end

  test "returns false for followers older than the created_after date" do
  ...
  test "returns true for followers within the created dates" do
            expect(@post.affiliate_passes_filters(@follower_2)).to eq(true)
          end

  test "returns false for followers outside the created dates" do
