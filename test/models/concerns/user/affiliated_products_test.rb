...
class UserAffiliatedProductsTest < ActiveSupport::TestCase
  self.described_class = User::AffiliatedProducts
  self.rspec_metadata = { vcr: true }



  context_ User::AffiliatedProducts, :vcr do
  context_ "#directly_affiliated_products" do
  ...
  context_ "when alive flag is set to true" do
        let(:alive) { true }

  test "returns only alive affiliated products" do
  ...
  context_ "when alive flag is set to false" do
        let(:alive) { false }

  test "returns both alive and non-alive affiliated products" do
          expect(affiliated_products).to match_array(
            [
              { "name" => "Product 1", "affiliate_id" => direct_affiliate_one.id },
              { "name" => "Product 2", "affiliate_id" => direct_affiliate_one.id },
