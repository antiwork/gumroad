...
class UserFollowersTest < ActiveSupport::TestCase
  self.described_class = User::Followers



  context_ User::Followers do
  context_ "#following" do
  ...
  test "returns users being followed" do
  ...
  context_ "#follower_by_email" do
  test "returns the active follower matching the provided email" do
        user = create(:user)
  ...
  test "returns true if user has confirmed follower with that email" do
        user = create(:user)
        active_follower = create(:active_follower, user:)
  ...
  test "calls Follower::CreateService and returns the follower object" do
  ...
  test "updates source when the user is already following the same creator using the same email" do
