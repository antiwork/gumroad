...
class UserStripeConnectTest < ActiveSupport::TestCase
  self.described_class = User::StripeConnect



  context_ User::StripeConnect do
  context_ ".find_or_create_for_stripe_connect_account" do
      before do
        @data = {
  ...
  test "does not return the user associated with the email and does not create a new one as email is already taken" do
  ...
  context_ "#has_brazilian_stripe_connect_account?" do
      let(:user) { create(:user_compliance_info).user }

  test "returns true if user has a Brazilian Stripe Connect merchant account" do
  ...
  test "returns false if user has a non-Brazilian Stripe Connect merchant account" do
  ...
  test "returns false if user has no Stripe Connect merchant account" do
        expect(user.has_brazilian_stripe_connect_account?).to be false
      end
    end
  end
