...
class UserTaxationTest < ActiveSupport::TestCase
  self.described_class = User::Taxation



  context_ User::Taxation do
  ...
  context_ "#eligible_for_1099_k?", :vcr do
  ...
  context_ "when user is not from the US" do
        before do
  ...
  context_ "when user is from an invalid compliance country" do
        before do
  ...
  context_ "when user doesn't meet the minimum sales amount" do
  test "returns false" do
  ...
  context_ "when user doesn't meet the minimum sales count" do
  test "returns false" do
  ...
  context_ "for an year with different sales threshold than default" do
  context_ "when user doesn't meet the minimum sales amount" do
  test "returns false" do
  ...
  context_ "when user meets the minimum sales amount and count" do
  ...
  context_ "when user is compliant" do
        before do
          create(:user_compliance_info, user: @user)
        end
  ...
  context_ "when sales amount is above threshold but non-refunded sales amount is not" do
        before do
          create(:user_compliance_info, user: @user)
  ...
  context_ "when sales amount is above threshold but non-disputed sales amount is not" do
        before do
          create(:user_compliance_info, user: @user)
  ...
  context_ "when sales amount is above threshold but non-Connect sales amount is not" do
        before do
          create(:user_compliance_info, user: @user)
  ...
  context_ "when sales amount is above state filing threshold" do
        before do
  ...
  context_ "#eligible_for_1099_misc?", :vcr do
      let(:year) { Date.current.year }
  ...
  context_ "when user doesn't meet the minimum affiliate sales amount" do
  test "returns false" do
  ...
  context_ "when affiliate sales amount is above threshold but non-refunded amount is not" do
        before do
          create(:user_compliance_info, user: @user)
  ...
  context_ "when affiliate sales amount is above threshold but non-disputed amount is not" do
        before do
          create(:user_compliance_info, user: @user)
  ...
  context_ "when affiliate amount is above state filing threshold" do
        before do
  ...
  context_ "#eligible_for_1099?", :vcr do
      let(:year) { Date.current.year }
      before do
        allow_any_instance_of(User).to receive(:from_us?).and_return(true)
      end

  test "returns true if eligible for 1099-K and not 1099-MISC" do
  ...
  test "returns true if eligible for 1099-MISC and not 1099-K" do
  ...
  test "returns false if eligible for neither 1099-K nor 1099-MISC" do
        allow_any_instance_of(User).to receive(:eligible_for_1099_k?).and_return(false)
        allow_any_instance_of(User).to receive(:eligible_for_1099_misc?).and_return(false)
  ...
  test "returns true if eligible for both 1099-K and 1099-MISC" do
        allow_any_instance_of(User).to receive(:eligible_for_1099_k?).and_return(true)
        allow_any_instance_of(User).to receive(:eligible_for_1099_misc?).and_return(true)
  ...
  context_ "#from_us?" do
  context_ "when user is from the United States" do
        before do
          create(:user_compliance_info, user: @user)
        end
  ...
  context_ "when user is from Singapore" do
        before do
          create(:user_compliance_info_singapore, user: @user)
        end
  ...
  context_ "when user compliance is empty" do
        before do
  ...
  context_ "when user compliance is missing" do
  test "returns false" do
          expect(@user.from_us?).to eq(false)
        end
      end
