...
class UserMoneyBalanceTest < ActiveSupport::TestCase
  ...
  context_ User::MoneyBalance, :vcr do
  context_ "balance_formatted" do
      before do
        @user = create(:user, unpaid_balance_cents: 1_00)
      end

  test "returns the user's balance" do
        expect(@user.unpaid_balance_cents).to eq 100
        expect(@user.balance_formatted).to eq "$1"
      end
    end

  context_ "#instantly_payable_unpaid_balances" do
  test "returns maximum unpaid balances whose sum is less than instantly payable amount available on Stripe" do
        user = create(:user)
