...
class UserPayoutInfoTest < ActiveSupport::TestCase
  self.described_class = User::PayoutInfo



  context_ User::PayoutInfo do
  ...
  context_ "when there is no active bank account" do
        before do
          bank_account.destroy!
        end

  test "returns nil for active_bank_account" do
          result = user.payout_info
  ...
  context_ "when there are no payouts_paused comments" do
        let!(:comment) { nil }

  test "returns nil for payouts_paused_for_reason" do
          result = user.payout_info
  ...
  context_ "when last payout is nil" do
  ...
  context_ "when user is payable via stripe from admin" do
          before do
  ...
  test "includes manual payout info with stripe information" do
            result = user.payout_info
  ...
  context_ "when user is payable via paypal from admin" do
          before do
            allow(Payouts).to receive(:is_user_payable)
              .with(user, manual_payout_end_date, processor_type: PayoutProcessorType::STRIPE, from_admin: true)
              .and_return(false)
  ...
  test "includes manual payout info with paypal information" do
            result = user.payout_info
  ...
  context_ "when user is not payable via stripe or paypal from admin" do
          before do
            allow(Payouts).to receive(:is_user_payable)
              .with(user, manual_payout_end_date, processor_type: PayoutProcessorType::STRIPE, from_admin: true)
              .and_return(false)
  ...
  test "does not include manual payout info" do
            result = user.payout_info
  ...
  context_ "when last payout exists" do
