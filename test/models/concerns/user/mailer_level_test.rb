...
class UserMailerLevelTest < ActiveSupport::TestCase
  self.described_class = User::MailerLevel



  context_ User::MailerLevel do
  context_ "#mailer_level" do
      before do
        @creator = create(:user)
      end

  context_ "when creator belongs to level_1" do
  test "returns :level_1" do
  ...
  context_ "when creator belongs to level_2" do
  test "returns :level_2" do
  ...
  test "returns :level_2 for very large sales_cents_total number" do
  ...
  test "doesn't query elasticsearch when level is available in redis" do
