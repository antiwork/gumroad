...
class TimestampStateFieldsTest < ActiveSupport::TestCase
  self.described_class = TimestampStateFields



  context_ TimestampStateFields do
  ...
  test "returns boolean value when using predicate methods" do
        expect(user.created?).to be(true)
        expect(user.not_created?).to be(false)
      end

  test "updates record via update methods" do
  ...
  test "reponds to state methods" do
        expect(user.created?).to be(true)
