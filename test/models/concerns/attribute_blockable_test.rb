...
class AttributeBlockableTest < ActiveSupport::TestCase
  self.described_class = AttributeBlockable



  context_ AttributeBlockable do
  ...
  context_ ".attr_blockable" do
  test "defines blocked? methods for the specified attribute" do
  ...
  test "defines blocked? methods for custom method names" do
  ...
  test "defines block and unblock methods" do
        expect(user_with_blocked_email).to respond_to(:block_by_email!)
        expect(user_with_blocked_email).to respond_to(:unblock_by_email!)
      end

  context_ "generated instance methods" do
  context_ "#blocked_by_email?" do
  test "returns true for blocked emails" do
            expect(user_with_blocked_email.blocked_by_email?).to be true
          end

  test "returns false for unblocked emails" do
  ...
  context_ "#blocked_by_email_object" do
  test "returns blocked object with blocked_at timestamp for blocked emails" do
  ...
  context_ "#blocked_by_form_email?" do
  test "uses the email attribute for blocking checks" do
  ...
  test "does not consider unblocked records" do
  ...
  test "finds active block after unblocking and reblocking" do
  ...
  context_ "#block_by_email!" do
  test "blocks the user by their email" do
  ...
  test "accepts expires_in parameter" do
            user = create(:user, email: "test@example.com")
  ...
  test "accepts by_user_id parameter" do
  ...
  test "accepts both by_user_id and expires_in parameters" do
  ...
  test "does nothing when email is blank" do
            user = create(:user)
            user.update_column(:email, "")
            expect { user.block_by_email! }.not_to change { PlatformBlock.count }
          end

  test "does nothing when email is nil" do
            user = create(:user)
            user.update_column(:email, nil)
            expect { user.block_by_email! }.not_to change { PlatformBlock.count }
          end

  test "updates cache with correct blockable_method key" do
  ...
  test "immediately reflects blocked status via cache" do
  ...
  context_ "#unblock_by_email!" do
  test "unblocks the user by their email" do
  ...
  test "clears cached blocked_by_attributes when unblocking" do
  ...
  context_ "cache management with custom method names" do
  context_ "#block_by_form_email!" do
  test "updates cache with blockable_method key, not object_type key" do
  ...
  test "correctly identifies blocked status after blocking" do
  ...
  test "maintains separate cache entries for email and form_email" do
  ...
  context_ "#unblock_by_form_email!" do
  test "clears cache using blockable_method key, not object_type key" do
  ...
  test "correctly updates blocked status after unblocking" do
  ...
  test "handles mixed cache state correctly" do
  ...
  context_ "cache key consistency" do
  test "uses consistent cache keys across all operations" do
  ...
  test "does not pollute cache with object_type keys" do
  ...
  context_ "cache management with email_domain methods" do
  context_ "#block_by_form_email_domain!" do
  test "updates cache with blockable_method key" do
  ...
  test "correctly identifies blocked status" do
  ...
  test "maintains independent cache from email methods" do
  ...
  context_ "#unblock_by_form_email_domain!" do
  test "clears cache using correct blockable_method key" do
  ...
  test "correctly updates blocked status" do
  ...
  context_ "blockable attribute introspection" do
  context_ ".blockable_attributes" do
  test "returns an array of attribute configurations" do
          expect(User.blockable_attributes).to be_an(Array)
          expect(User.blockable_attributes).not_to be_empty
        end

  test "includes configuration for each attr_blockable declaration" do
  ...
  test "tracks custom attribute mappings correctly" do
  ...
  test "returns unique entries per model class" do
  ...
  context_ ".blockable_method_names" do
  test "returns an array of blockable method names" do
          expect(User.blockable_method_names).to be_an(Array)
          expect(User.blockable_method_names).to all(be_a(Symbol))
        end

  test "includes all blockable method names defined on the model" do
  ...
  test "preloads all blockable attributes" do
  ...
  context_ ".with_blocked_attributes_for" do
      let!(:users) { 3.times.map { |i| create(:user, email: "unblocked#{i}@example.com") } }
      let!(:blocked_user) { create(:user, email: blocked_email) }
  ...
  context_ "bulk loading blocked attributes" do
  test "correctly identifies blocked and unblocked records" do
  ...
  test "populates blocked_by_attributes for all records" do
  ...
  test "handles mixed blocked and unblocked records" do
  ...
  test "works with custom method names that map to different object types" do
  ...
  test "uses the right object_type for each custom method when preloading" do
  ...
  context_ "performance" do
  test "issues one PlatformBlock query per preloaded attribute" do
          perf_users = []
          5.times do |i|
  ...
  test "handles records with nil values" do
  ...
  test "only preloads active blocks, ignoring unblocked records" do
  ...
  context_ "different blocked object types" do
  ...
  test "works with different PlatformBlock::TYPES" do
  ...
  context_ "edge cases and error handling" do
  test "raises an error for missing PlatformBlock::TYPES" do
  ...
  test "handles expired blocked objects" do
  ...
  test "handles blocked objects without expires_at" do
  ...
  context_ "integration with blocked_by_attributes" do
  test "initializes with empty hash" do
        user = User.new
        expect(user.blocked_by_attributes).to eq({})
      end

  test "persists cached blocked attributes" do
  ...
  context_ "#blocked_objects_for_values" do
      let(:user) { create(:user, email: "test@example.com") }

  test "returns blocked objects for given values" do
  ...
  test "returns empty collection when no values are blocked" do
        values = ["unblocked1@example.com", "unblocked2@example.com"]
        blocked_objects = user.blocked_objects_for_values(:email, values)
        expect(blocked_objects).to be_empty
      end

  test "works with different object types" do
  ...
  test "handles empty values array" do
