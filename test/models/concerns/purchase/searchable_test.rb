...
class PurchaseSearchableTest < ActiveSupport::TestCase
  self.described_class = Purchase::Searchable



  context_ Purchase::Searchable do
    before do
      travel_to(Date.new(2019, 1, 1))
    end

  context_ "#indexed_json" do
      before do
  ...
  test "supports only returning specific fields" do
  ...
  context_ "Subscription Callbacks" do
      before do
  ...
  test "updates related purchases when deactivated_at changes" do
  ...
  test "does not update related purchases when unrelated attributes changes" do
  ...
  context_ "RelatedPurchase Callbacks" do
      before do
        @seller = create(:user)
  ...
  context_ "when creating non-successful purchase" do
        before do
          product_2 = create(:product, user: @seller)
  ...
  test "does not queue update_by_query job" do
  ...
  context_ "when transitioning purchase to successful" do
        before do
          @product_2 = create(:product, user: @seller)
          @purchase_2 = create(:purchase_in_progress, seller: @seller, link: @product_2, email: @purchase.email)
  ...
  context_ "when updating an irrelevant attribute" do
        before do
  ...
  context_ "when creating a subscription purchase" do
  test "queues an update to related purchase's documents" do
          subscription = create(:subscription)
  ...
  context_ "with a subscription's new purchase" do
        before do
          subscription = create(:subscription)
          @purchase.subscription = subscription
          @purchase.is_original_subscription_purchase = true
  ...
  context_ "when updating purchase_state" do
          before do
            @purchase_2.update!(purchase_state: "successful")
          end

  test "queues an update to related purchases' documents" do
            expect(ElasticsearchIndexerWorker).to have_enqueued_sidekiq_job("update", "record_id" => @purchase.id, "class_name" => "Purchase", "fields" => ["latest_charge_date"])
          end
        end
  ...
  test "does not queue an update to related purchases' documents" do
  ...
  context_ "when updating an irrelevant attribute on a successful purchase" do
          before do
            @purchase_2.update!(purchase_state: "successful")
            ElasticsearchIndexerWorker.jobs.clear
            @purchase_2.update!(can_contact: false)
  ...
  context_ "VariantAttribute Callbacks" do
  context_ ".variants_changed" do
        before do
  ...
  test "queues udpate for purchase, and related purchases" do
  ...
  context_ "AffiliateCredit Callbacks" do
      before do
        @purchase = create(:purchase)
        ElasticsearchIndexerWorker.jobs.clear
      end

  context_ "when affiliate_credit is created" do
  test "updates purchase document" do
  ...
  context_ "when affiliate_credit is updated" do
        before do
  ...
  context_ "Product Callbacks" do
  test "enqueues update_by_query job when taxonomy is updated" do
        product = create(:product)
  ...
  test "does not enqueue update_by_query job when product has no sales" do
        product = create(:product)
        ElasticsearchIndexerWorker.jobs.clear
  ...
  test "updates the sales' Elasticsearch documents when the taxonomy is changed", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  ...
  context_ "purchaser_id reindexing" do
  test "enqueues ES update when purchaser_id changes" do
  ...
  test "updates purchaser_id in Elasticsearch document", :sidekiq_inline, :elasticsearch_wait_for_refresh do
        purchase = create(:purchase, purchaser: nil)
  ...
  context_ "updating a purchase", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  test "sets the subscription id" do
        subscription = create(:subscription)
        purchase = create(:purchase)
