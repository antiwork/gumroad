...
class PurchaseBlockableTest < ActiveSupport::TestCase
  self.described_class = Purchase::Blockable



  context_ Purchase::Blockable do
    let(:product) { create(:product) }
    let(:buyer) { create(:user) }
    let(:purchase) { create(:purchase, link: product, email: "gumbot@gumroad.com", purchaser: buyer) }

  context_ "#buyer_blocked?" do
  test "returns false when buyer is not blocked" do
        expect(purchase.buyer_blocked?).to eq(false)
      end

  context_ "when the purchase's browser is blocked" do
        before do
  ...
  context_ "when the purchase's email is blocked" do
        before do
  ...
  context_ "when the purchase's paypal email is blocked" do
  ...
  context_ "when the buyer's email address is blocked" do
        before do
  ...
  context_ "when the purchase's ip address is blocked" do
        before do
  ...
  context_ "when the purchase's payment method is blocked" do
        before do
  ...
  context_ "email blocking on fraud" do
  context_ "for a fraudulent transaction" do
  ...
  context_ "for a non-fraudulent transaction" do
  test "does not block the email" do
          purchase = build(:purchase_in_progress,
                           email: "foo@example.com",
  ...
  context_ "ip address blocking" do
  context_ "when purchase's ip address is not blocked" do
  test "returns false for blocked check" do
          expect(purchase.blocked_by_ip_address?).to be false
        end
      end

  context_ "when purchase's ip address is blocked" do
        before do
          PlatformBlock.add!(object_type: PlatformBlock::TYPES[:ip_address], object_value: purchase.ip_address, expires_in: 1.hour)
        end

  test "returns true for blocked check" do
  ...
  context_ "when the purchase is made through Stripe" do
  test "blocks buyer's email, browser_guid, ip_address and stripe_fingerprint" do
  ...
  context_ "when the purchase is made through PayPal" do
  ...
  test "blocks buyer's email, browser_guid, ip_address and card_visual" do
          purchase.block_buyer!
  ...
  context_ "when blocking user is provided" do
        let(:admin_user) { create(:admin_user) }

  test "blocks buyer and references the blocker" do
  ...
  test "sets `is_buyer_blocked_by_admin` to true" do
  ...
  context_ "comments" do
        let(:admin_user) { create(:admin_user) }

  context_ "when comment content is provided" do
  test "adds buyer blocked comments with the provided content" do
  ...
  context_ "when comment content is not provided" do
  context_ "when the blocking user is an admin" do
  test "adds buyer blocked comments with the default content" do
  ...
  context_ "when the blocking user is not an admin" do
  test "adds buyer blocked comments with the default content" do
              user = create(:user)
  ...
  context_ "when the blocking user is not provided" do
  test "adds buyer blocked comments with the default content and GUMROAD_ADMIN as author" do
  ...
  context_ "#unblock_buyer!" do
  context_ "when buyer is not blocked" do
  test "does not call #unblock! on any blocked objects" do
          expect_any_instance_of(PlatformBlock).not_to receive(:unblock!)
          purchase.unblock_buyer!
        end
      end

  context_ "when the purchase is made through Stripe" do
  test "unblocks the buyer's email, browser, IP address and stripe_fingerprint" do
  ...
  context_ "when the stripe_fingerprint is nil" do
  test "unblocks the buyer's stripe_fingerprint from a recent purchase" do
          purchase.block_buyer!
  ...
  test "unblocks the buyer's email, browser, IP address and card_visual" do
          # Block purchase first to create the blocked objects
          purchase.block_buyer!
  ...
  test "sets `is_buyer_blocked_by_admin` to false" do
        purchase.block_buyer!
  ...
  context_ "when the purchase fails due to a fraud related reason" do
  ...
  context_ "when the purchase fails due to a non-fraud related reason" do
  ...
  context_ "ban card testers" do
        before do
          @purchaser = create(:user, email: "purchaser@example.com")
          Feature.activate(:ban_card_testers)
        end

  context_ "when previous failed purchases exist with same email or browser_guid but with different cards" do
  context_ "when previous failed purchases were made within the week" do
            before do
              3.times do |n|
  ...
  context_ "when previous failed purchases weren't made within the week" do
            before do
              3.times do |n|
  ...
  context_ "when purchases with different cards fail from the same IP address" do
  context_ "when failures happen within a day" do
            before do
              3.times do |n|
  ...
  context_ "when the ip_address is not already blocked" do
  test "blocks the IP address" do
  ...
  context_ "when the ip_address is already blocked" do
  test "doesn't overwrite the previous ip_address block" do
                freeze_time do
  ...
  context_ "when failures doesn't happen in a day" do
            before do
              3.times do |n|
  ...
  context_ "block purchases on product" do
        before do
  ...
  context_ "when number of failed purchases exceeds the threshold" do
          before do
            9.times do |n|
  ...
  context_ "when price is not zero" do
  test "blocks purchases on product" do
              travel_to(Time.current) do
                expect do
                  @purchase.mark_failed!
                end.to change { PlatformBlock.count }.from(0).to(1)
  ...
  context_ "when the error code belongs to IGNORED_ERROR_CODES list" do
            before do
              @purchase = create(:purchase, link: @product, purchase_state: "in_progress")
            end
  ...
  context_ "when number of failed purchases doesn't exceed the threshold" do
          before do
            create(:failed_purchase, link: @product)
            @purchase = create(:purchase, link: @product, purchase_state: "in_progress")
          end
  ...
  context_ "when multiple purchases fail in a row" do
          before do
            $redis.set(RedisKey.card_testing_max_number_of_failed_purchases_in_a_row, 3)
          end

  context_ "when all recent purchases were failed" do
            before do
              2.times do |n|
  ...
  context_ "when recent purchases fail with an error code from IGNORED_ERROR_CODES list" do
            before do
              2.times do |n|
  ...
  context_ "when a successful purchase exists in the recent purchases" do
            before do
              create(:purchase, link: @product, purchase_state: "in_progress").mark_failed!
              create(:purchase, link: @product, purchase_state: "in_progress").mark_failed!
  ...
  context_ "when a not_charged purchase exists in the recent purchases" do
            before do
              create(:purchase, link: @product, purchase_state: "in_progress").mark_failed!
              create(:purchase, link: @product, purchase_state: "in_progress").mark_failed!
  ...
  context_ "pause payouts for seller internally based on recent failures" do
  ...
  context_ "when feature is inactive" do
          before { Feature.deactivate(:block_seller_based_on_recent_failures) }

  test "does not pause payouts for the seller" do
  ...
  context_ "when seller is verified" do
  ...
  context_ "when seller is not verified" do
  test "pauses payouts when threshold is exceeded" do
  ...
  context_ "when error code is ignored" do
  test "does not pause payouts for the seller" do
            create_list(:failed_purchase, 5, link: product, price_cents: 250)
  ...
  context_ "when seller account is older than 2 years" do
  ...
  test "does not pause payouts for the seller even with high failed amounts" do
  ...
  test "does not create a comment" do
            create_list(:failed_purchase, 10, link: old_product, price_cents: 500)
            old_purchase.mark_failed!
  ...
  context_ "when seller account is slightly newer than 2 years" do
  ...
  test "pauses payouts for the seller when threshold is exceeded" do
  ...
  context_ "when total failed amount is below threshold" do
  test "does not pause payouts for the seller" do
  ...
  context_ "when total failed amount is above threshold" do
  test "pauses payouts internally" do
            create_list(:failed_purchase, 5, link: product, price_cents: 250)
            purchase.mark_failed!
  ...
  test "creates a comment with the failed amount" do
            create_list(:failed_purchase, 5, link: product, price_cents: 250)
            purchase.mark_failed!
  ...
  context_ "when some purchases are outside the watch window" do
  test "does not pause payouts internally" do
              travel_to Time.current do
  ...
  context_ "when total failed amount is above default threshold" do
  test "pauses payouts internally" do
              # default max amount is $2000
  ...
  test "returns stripe fingerprint" do
  ...
  context_ "when number of free purchases of the same product from same IP address exceeds the threshold" do
  context_ "when the purchase happens within the configured time limit" do
  test "blocks the ip_address" do
            freeze_time do
              expect do
  ...
  context_ "when the purchase happens outside the configured time limit" do
  test "doesn't block the ip_address" do
  ...
  context_ "when the purchase is created for another product" do
  test "doesn't block the ip_address" do
          expect do
  ...
  context_ "when the purchase is created from another ip_address" do
  test "doesn't block the ip_address" do
          expect do
  ...
  context_ "when purchase is not free" do
  test "doesn't block the ip_address" do
          expect do
  ...
  context_ "#suspend_buyer_on_fraudulent_card_decline!" do
      before do
  ...
  context_ "when the error code is not CARD_DECLINED_FRAUDULENT" do
  test "doesn't suspend the buyer" do
  ...
  context_ "when the buyer account was created more than 6 hours ago" do
  test "doesn't suspend the buyer" do
  ...
  context_ "when the error code is CARD_DECLINED_FRAUDULENT" do
  context_ "when buyer account was created less than 6 hours ago" do
  test "suspends the buyer" do
            expect do
              @purchase.mark_failed!
  ...
  context_ "when the buyer is already flagged for fraud" do
        before do
          @buyer.flag_for_fraud!(author_name: "admin")
        end

  test "suspends the buyer without re-flagging" do
          expect { @purchase.mark_failed! }.to change { @buyer.reload.suspended_for_fraud? }.from(false).to(true)
        end
      end
    end

  context_ "#block_buyer_based_on_chargeback_count!" do
      let(:seller) { create(:user) }
      let(:product) { create(:product, user: seller) }
      let(:buyer) { create(:user) }
  ...
  test "creates a comment with the chargeback count" do
  ...
  context_ "when buyer is already blocked" do
        before do
          create_chargebacked_purchases_by_email(5, "repeat-offender@example.com")
          purchase.block_buyer!
        end

  test "does not re-block the buyer" do
          expect { purchase.block_buyer_based_on_chargeback_count! }.not_to change { PlatformBlock.count }
        end
      end

  context_ "when buyer has 5 chargebacks by purchaser_id with different email" do
        before do
  ...
  context_ "#pause_payouts_for_seller_based_on_chargeback_rate!" do
      let(:seller) { create(:user) }
      let(:product) { create(:product, user: seller) }
      let(:purchase) { create(:purchase, link: product) }

  context_ "when seller payouts are already paused internally" do
        before do
          seller.update!(payouts_paused_internally: true)
          allow(seller).to receive(:lost_chargebacks).and_return({ volume: "4.2%", count: "15.0%" })
        end

  test "does not change the payout pause source" do
  ...
  test "does not create additional comments" do
          expect do
            purchase.pause_payouts_for_seller_based_on_chargeback_rate!
  ...
  context_ "when chargeback volume is at 3.0%" do
        before do
  ...
  context_ "when chargeback volume is below 3.0%" do
        before do
  ...
  context_ "when chargeback volume exceeds 3.0%" do
        before do
          allow(seller).to receive(:lost_chargebacks).and_return({ volume: "4.2%", count: "15.0%" })
        end
  ...
  test "creates a comment with the chargeback rate" do
          purchase.pause_payouts_for_seller_based_on_chargeback_rate!
  ...
  context_ "when chargeback volume is significantly above 3.0%" do
        before do
  ...
  test "creates a comment with the correct chargeback rate" do
          purchase.pause_payouts_for_seller_based_on_chargeback_rate!
  ...
  context_ "edge case: when chargeback volume is just above 3.0%" do
        before do
