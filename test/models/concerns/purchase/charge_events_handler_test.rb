...
class PurchaseChargeEventsHandlerTest < ActiveSupport::TestCase
  self.described_class = Purchase::ChargeEventsHandler
  self.rspec_metadata = { vcr: true }



  context_ Purchase::ChargeEventsHandler, :vcr do
    include CurrencyHelper
    include ProductsHelper

  context_ "charge event looked up by external ID" do
  ...
  test "uses charge_reference which is expected to be the purchase external ID" do
  ...
  context_ "setting the purchase fee from a charge event" do
      before do
        @l = create(:product)
  ...
  test "set the fee on the purchase if the information charge event has the info" do
  ...
  context_ "informational charge event for purchase" do
      before do
        @initial_balance = 200
  ...
  test "does not affect seller balance" do
        Purchase.handle_charge_event(@e)
        @p.reload
        @u.reload
  ...
  context_ "charge succeeded event for purchase" do
  ...
  context_ "payment failed event for purchase" do
      before do
        @initial_balance = 200
        @u = create(:user, unpaid_balance_cents: @initial_balance)
        @l = create(:product, user: @u)
  ...
  test "finds the purchase using processor_payment_intent and marks it as failed" do
  ...
  test "handles stripe events correctly - type informational" do
  ...
  test "sets the purchase chargeback_date flag and email us" do
  ...
  test "enqueues UpdateSalesRelatedProductsInfosJob" do
  ...
  test "updates dispute evidence as seller contacted" do
        charge_event = build(:charge_event_dispute_formalized, charge_id: transaction_id)
        Purchase.handle_charge_event(charge_event)
  ...
  context_ "when the purchase is not eligible for a dispute evidence" do
        before do
          allow_any_instance_of(Purchase).to receive(:eligible_for_dispute_evidence?).and_return(false)
        end

  test "doesn't enqueue FightDisputeJob job" do
          charge_event = build(:charge_event_dispute_formalized, charge_id: transaction_id)
          Purchase.handle_charge_event(charge_event)
  ...
  context_ "when a chargeback is reversed" do
  test "enqueues UpdateSalesRelatedProductsInfosJob" do
  ...
  test "handles dispute formalized event properly for partially refunded purchase" do
  ...
  context_ "dispute formalized event for free trial subscriptions" do
  ...
  test "prevents the subscriber's review from being counted" do
          expect do
            Purchase.handle_charge_event(charge_event)
  ...
  context_ "chargeback events for unsuccessful purchases" do
        let!(:purchase) do
  ...
  context_ "chargeback notification received" do
  test "does not process the message" do
  ...
  context_ "chargeback reversed notification received" do
  test "does not process the message" do
  ...
  context_ "for a successful purchase" do
  test "marks the purchased as refunded, updates balances" do
            ...
            expect_any_instance_of(ErrorNotifier).not_to receive(:notify)
