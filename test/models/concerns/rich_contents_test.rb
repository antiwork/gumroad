...
class RichContentsTest < ActiveSupport::TestCase
  self.described_class = RichContents



  context_ RichContents do
    let(:description) { [{ "type" => "paragraph", "content" => [{ "text" => Faker::Lorem.unique.sentence, "type" => "text" }] }] }

  context_ "for a product" do
      let(:product) { create(:product) }

  context_ "#rich_content_json" do
  context_ "when product has rich content" do
  ...
  test "returns the product-level alive rich contents in order" do
  ...
  context_ "when `has_same_rich_content_for_all_variants` is true" do
            before do
              product.update!(has_same_rich_content_for_all_variants: true)
            end

  test "returns the product-level alive rich contents" do
              expect(product.rich_content_json).to eq([
                                                        { id: rich_content3.external_id, page_id: rich_content3.external_id, variant_id: nil, title: "Page 3", description: { type: "doc", content: rich_content3.description }, updated_at: rich_content3.updated_at },
                                                        { id: rich_content1.external_id, page_id: rich_content1.external_id, variant_id: nil, title: "Page 1", description: { type: "doc", content: rich_content1.description }, updated_at: rich_content1.updated_at }
                                                      ])
  ...
  context_ "when product does not have rich content" do
  ...
  context_ "#rich_content_folder_name" do
  test "returns the folder name when the corresponding folder exists in the rich content" do
  ...
  context_ "#rich_content_json" do
  context_ "when variant has rich content" do
  ...
  test "returns the variant-level alive rich contents" do
  ...
  context_ "when corresponding product's `has_same_rich_content_for_all_variants` is true" do
            before do
  ...
  context_ "when variant does not have rich content" do
  test "returns empty array" do
            expect(variant.rich_content_json).to eq([])
          end
        end
