...
class ProductAsJsonTest < ActiveSupport::TestCase
  self.described_class = Product::AsJson
  self.rspec_metadata = { vcr: true }



  context_ Product::AsJson, :vcr do
  context_ "as_json method" do
      before do
        @product = create(:product, name: "some link", require_shipping: true)
      end

  test "returns the correct has for default (public)" do
  ...
  test "includes pricing data for a tiered membership product" do
  ...
  test "returns thumbnail_url information" do
  ...
  test "returns the correct text for custom_summary" do
  ...
  test "returns the correct values for custom_fields" do
        json = @product.as_json(api_scopes: ["view_public"])
  ...
  test "returns the correct hash for api, :view_public,:edit_products" do
  ...
  test "returns the correct value for max_purchase_count if it is not set by the user" do
        expect(@product.as_json(api_scopes: %w[view_public edit_products])["max_purchase_count"]).to be(nil)
      end

  test "returns the correct value for max_purchase_count if it is set by the user" do
  ...
  test "returns the correct hash for api, :view_sales" do
  ...
  test "includes the preorder_link information for an unreleased link" do
  ...
  test "includes the preorder_link information for a released link" do
  ...
  test "returns deprecated `url` attribute" do
  ...
  context_ "when user has purchasing_power_parity_enabled" do
          before do
  ...
  test "includes PPP prices for every country" do
  ...
  context_ "when injecting a preloaded factors" do
  test "includes PPP prices for every country without calling PurchasingPowerParityService" do
  ...
  context_ "when user has purchasing_power_parity_enabled and product has purchasing_power_parity_disabled" do
          before do
            @product.user.update!(purchasing_power_parity_enabled: true)
  ...
  test "doesn't include PPP prices for every country" do
            result = @product.as_json(api_scopes: %w[view_sales])
            expect(result["purchasing_power_parity_prices"]).to be_nil
          end
        end

  context_ "when api_scopes includes 'view_sales'", :sidekiq_inline, :elasticsearch_wait_for_refresh do
  test "includes sales data" do
  ...
  context_ "as_json_for_mobile_api" do
  test "returns proper json for a product" do
  ...
  test "returns thumbnail information" do
          thumbnail = create(:thumbnail)
          product = thumbnail.product
  ...
  test "returns creator info for a product" do
  ...
  test "returns a blank preview_url if there is no preview for the product" do
  ...
  test "returns proper json for a product with a youtube preview" do
  ...
  test "returns proper json for a product with a soundcloud preview" do
  ...
  test "returns proper json for a product with a vimeo preview" do
  ...
  test "does not return preview heights less than 204 pixels" do
          link = create(:product, preview: fixture_file_upload("kFDzu.png", "image/png"))
  ...
  test "returns 'has_rich_content' true" do
          product = create(:product)
  ...
  context_ "as_json_variant_details_only" do
  context_ "for a product with variants" do
  test "includes a hash of variants data under 'categories'" do
  ...
  test "sets category title to \"Version\" if there is no title" do
            product = create(:product)
  ...
  test "includes a hash of tier data under 'categories'" do
            circle_integration = create(:circle_integration)
            discord_integration = create(:discord_integration)
  ...
  context_ "for a product with skus_enabled" do
  test "includes a hash of SKUs data under 'skus'" do
  ...
  context_ "for a product without variants" do
  test "returns empty objects" do
            product = create(:product)
  ...
  context_ "recommendable attribute" do
  test "returns true if the product is recommendable" do
  ...
  context_ "rated_as_adult attribute" do
  test "returns true if the product is rated_as_adult" do
