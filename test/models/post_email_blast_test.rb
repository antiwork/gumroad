...
class PostEmailBlastTest < ActiveSupport::TestCase
  self.described_class = PostEmailBlast



  context_ PostEmailBlast do
    let(:blast) { create(:post_email_blast) }

  context_ ".aggregated", :freeze_time do
      before do
  ...
  test "returns the aggregated data" do
  ...
  context_ "Latency metrics", :freeze_time do
  context_ "#start_latency" do
  test "returns the difference between requested_at and started_at" do
          expect(blast.start_latency).to eq(5.minutes)
        end

  test "returns nil if started_at is nil" do
          blast.update!(started_at: nil)
          expect(blast.start_latency).to be_nil
        end

  test "returns nil if requested_at is nil" do
          blast.update!(requested_at: nil)
          expect(blast.start_latency).to be_nil
        end
      end

  context_ "#first_email_delivery_latency" do
  test "returns the difference between requested_at and first_email_delivered_at" do
          expect(blast.first_email_delivery_latency).to eq(10.minutes)
        end

  test "returns nil if first_email_delivered_at is nil" do
  ...
  context_ "#last_email_delivery_latency" do
  test "returns the difference between requested_at and last_email_delivered_at" do
          expect(blast.last_email_delivery_latency).to eq(20.minutes)
        end

  test "returns nil if last_email_delivered_at is nil" do
  ...
  context_ "#deliveries_per_minute" do
  test "returns the deliveries per minute" do
  ...
  context_ ".acknowledge_email_delivery" do
      let(:blast) { create(:post_email_blast, :just_requested) }

  test "sets first_email_delivered_at and last_email_delivered_at to the current time and increment deliveries count", :freeze_time do
  ...
  test "called twice only updates last_email_delivered_at to the current time" do
  ...
  context_ ".format_datetime" do
  test "returns a string without the timezone" do
        expect(described_class.format_datetime(Time.zone.local(2001, 2, 3, 4, 5, 6))).to eq("2001-02-03 04:05:06")
      end

  test "returns nil if the datetime is nil" do
