...
class SkuTest < ActiveSupport::TestCase
  ...
  test "has the proper category name given 2 variants" do
        expect(@sku.sku_category_name).to eq "Size - Color"
      end

  test "has the proper category name given 1 variants" do
  ...
  test "includes custom_sku" do
        json = @sku.as_json(for_views: true)
        expect(json["custom_sku"]).to eq("customSKU")
      end

  test "does not include custom_sku if it does not exist" do
  ...
  context_ "#to_option_for_product" do
  ...
  context_ "updating price_difference_cents" do
      let(:product) { create(:product) }
      let!(:sku) { create(:sku, link: product, custom_sku: "customSKU", price_difference_cents: 20) }

  test "enqueues Elasticsearch update if a price_difference_cents has changed" do
