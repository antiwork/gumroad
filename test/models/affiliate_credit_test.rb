...
class AffiliateCreditTest < ActiveSupport::TestCase
  self.described_class = AffiliateCredit



  context_ AffiliateCredit do
  ...
  test "requires an affiliate or oauth application to be present" do
        expect(build(:affiliate_credit, oauth_application: nil, affiliate: nil)).not_to be_valid
  ...
  context_ "#amount_partially_refunded_cents" do
  test "returns the sum of amount_cents of affiliate_partial_refunds" do
  ...
  context_ "#fee_partially_refunded_cents" do
  test "returns the sum of fee_cents of affiliate_partial_refunds" do
        affiliate_credit = create(:affiliate_credit)
  ...
  context_ "when a product commission is set" do
  ...
  test "creates an affiliate credit with the correct amount and product commission" do
  ...
  context_ "when affiliate applies to all products" do
  ...
  test "creates an affiliate credit with the correct amount and affiliate commission" do
  ...
  context_ "when a product commission is not set" do
        let(:apply_to_all_products) { false }
  ...
  test "creates an affiliate credit with the correct amount and commission" do
          affiliate_credit = AffiliateCredit.create!(purchase:, affiliate:, affiliate_amount_cents: 10, affiliate_fee_cents: 5, affiliate_balance: create(:balance))
          expect(affiliate_credit.amount_cents).to eq(10)
          expect(affiliate_credit.fee_cents).to eq(5)
          expect(affiliate_credit.basis_points).to eq(affiliate.affiliate_basis_points)
