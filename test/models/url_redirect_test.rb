...
class UrlRedirectTest < ActiveSupport::TestCase
  self.described_class = UrlRedirect



  context_ UrlRedirect do
    before do
  ...
  test "has a token that is unique" do
      url_redirect = create(:url_redirect)
      expect(url_redirect.token).not_to be(nil)
      expect(url_redirect.token.size).to be > 10
    end

  context_ "product_files_hash" do
  ...
  test "only contains alive, non-stream only product files" do
        travel_to(Time.current) do
          ...
          expect(product_files_hash).not_to include("url" => purchase.url_redirect.signed_location_for_file(stream_only_video),
                                                    "filename" => stream_only_video.s3_filename)
          expect(product_files_hash).not_to include("url" => purchase.url_redirect.signed_location_for_file(deleted_file),
                                                    "filename" => deleted_file.s3_filename)
        end
      end
    end

  context_ "referenced_link" do
  context_ "test purchase" do
        before do
          @product = create(:product)
          @url_redirect = create(:url_redirect, link: @product, purchase: nil)
        end

  test "returns the correct link" do
          expect(@url_redirect.referenced_link).to eq @product
        end
      end

  context_ "imported_customer" do
        before do
          @product = create(:product)
  ...
  test "returns the correct link for an imported_customer without a url_redirect" do
          expect(@url_redirect.referenced_link).to eq @product
        end
      end
  ...
  context_ "redirect_or_s3_location" do
  context_ "imported_customer" do
        before do
          @product = create(:product)
          @imported_customer = create(:imported_customer, link: @product, importing_user: @product.user)
  ...
  test "returns the right url when there is no provided url redirect for and installment" do
          @url_redirect = create(:url_redirect, link: @product, imported_customer: @imported_customer, purchase: nil)
  ...
  test "returns the file url for an external link file" do
          @url_redirect = create(:url_redirect, link: @product, purchase: nil)
          @product.product_files << create(:product_file, url: "http://gumroad.com", filetype: "link")
          expect(@url_redirect.redirect_or_s3_location).to eq @url_redirect.signed_location_for_file(@product.alive_product_files.first)
        end

  test "returns the right url when there is no provided url redirect for an installment and there are multiple files" do
          @url_redirect = create(:url_redirect, link: @product, imported_customer: @imported_customer, purchase: nil)
          @installment = create(:installment, link: @product, installment_type: "product")
          3.times do
            @product.product_files << create(
  ...
  test "returns the right url when there is a provided url redirect for an installment" do
          @installment = create(:installment, link: @product, installment_type: "product")
  ...
  context_ "#is_file_downloadable" do
      let(:product) { create(:product) }
      let(:url_redirect) { create(:url_redirect, link: product) }

  test "returns false if product is a rental and file is streamable" do
  ...
  test "returns false if file is an external link" do
        product_file = create(:external_link, link: product)

        expect(url_redirect.is_file_downloadable?(product_file)).to eq(false)
      end

  test "returns false if file is stream-only" do
        product_file = create(:streamable_video, stream_only: true, link: product)

        expect(url_redirect.is_file_downloadable?(product_file)).to eq(false)
      end

  test "returns true if file can be downloaded" do
  ...
  test "returns the most recent archive" do
        expect(@url_redirect.entity_archive).to eq(@last_product_files_archive)
      end

  test "returns nil if product has stampable pdfs" do
  ...
  test "creates the right signed url for a file" do
        ...
        expect(signed_s3_url).not_to match(/episode2/)
        expect(signed_s3_url).not_to match(/manual/)
      end

  test "creates the correct signed URL for a stamped pdf" do
  ...
  context_ "product_file_json_data_for_mobile" do
  context_ "when there is an associated purchase" do
  test "contains the purchase information if there is an associated purchase" do
          product = create(:product)
          product.product_files << create(
  ...
  context_ "#video_files_playlist" do
      let(:product) { create(:product) }
  ...
  context_ "when the purchase is associated with the product" do
  ...
  test "returns the video files playlist with files ordered by how they appear in the product-level rich content" do
  ...
  context_ "when the purchase is associated with a product variant" do
  ...
  test "returns the video files playlist with files ordered by how they appear in the variant-level rich content" do
            video_playlist = url_redirect.video_files_playlist(file1)[:playlist]
            expect(video_playlist.size).to eq 4
            expect(video_playlist[0][:sources][1]).to include file1.s3_filename
            expect(video_playlist[1][:sources][1]).to include file2.s3_filename
  ...
  test "returns the video files playlist with files ordered by their position for an installment" do
  ...
  test "sets the `index_to_play` to 0 if there are no files to include" do
  ...
  test "sets the `index_to_play` to 0 if the initial_product_file is not a part of the products belonging to the UrlRedirect" do
        url_redirect = create(:url_redirect, link: product, purchase: build(:purchase, link: product))
  ...
  context_ "#html5_video_url_and_guid_for_product_file" do
      before do
  ...
  test "returns the video URL and GUID for the file" do
  ...
  test "updates purchase rental_expired if is_rental is set to false" do
  ...
  context_ "#with_product_files" do
  test "returns the correct object when associated with an installment" do
  ...
  test "returns the correct object when associated with a product" do
        product = create(:product)
        purchase = create(:purchase, link: product)
  ...
  context_ "when the variant has no associated product files" do
          before do
  ...
  test "returns the correct object" do
          expect(@url_redirect.with_product_files).to eq @variant
        end

  test "returns the correct object when associated with a variant that has been deleted" do
          @variant.mark_deleted!
          expect(@url_redirect.reload.with_product_files).to eq @variant
        end

  test "returns the object for the non-deleted variant if only some variants have been deleted" do
          @variant.mark_deleted!
  ...
  test "returns empty hash if there's no associated object" do
        url_redirect = create(:url_redirect)

        expect(url_redirect.rich_content_json).to eq([])
      end

  test "returns nil when associated with an installment" do
        product = create(:product_with_files)
  ...
  context_ "when associated with a variant" do
        before do
          @product = create(:product_with_files)
  ...
  context_ "when associated product's `has_same_rich_content_for_all_variants` is set to true" do
          before do
            @product.update!(has_same_rich_content_for_all_variants: true)
          end

  test "returns the product-level rich content" do
  ...
  context_ "when associated product's `has_same_rich_content_for_all_variants` is set to false" do
          before do
            @product.update!(has_same_rich_content_for_all_variants: false)
          end

  test "returns the variant-level rich content" do
  ...
  test "returns the rich content of the cheapest variant when the purchase is associated with a deleted variant" do
  ...
  test "returns the product-level rich content when the purchase is associated with a deleted variant and there are no other variants" do
  ...
  context_ "when associated with a product" do
        before do
          @product = create(:product_with_files)
  ...
  test "returns product-level rich content" do
  ...
  test "returns the rich content of the cheapest variant when the purchase is associated with the product but there are variants" do
          variant_category = create(:variant_category, link: @product)
          variant1 = create(:variant, variant_category:, price_difference_cents: 100, product_files: @product.product_files)
          variant2 = create(:variant, variant_category:, price_difference_cents: 50, product_files: @product.product_files)
          create(:rich_content, entity: variant1, title: "Variant 1 page title", description: [{ "type" => "paragraph", "content" => [{ "text" => "Lorem ipsum", "type" => "text" }] }])
  ...
  context_ "when associated with a completed commission", :vcr do
  ...
  test "includes commission files in the rich content json" do
  ...
  context_ "when the commission is not completed" do
          before { commission.update(status: Commission::STATUS_IN_PROGRESS) }

  test "does not include commission files" do
            rich_content_json = commission.deposit_purchase.url_redirect.rich_content_json
  ...
  context_ "#has_embedded_posts?" do
  context_ "when associated with a product" do
        let(:product) { create(:product) }
        let(:purchase) { create(:purchase, link: product) }
        let(:url_redirect) { create(:url_redirect, purchase:) }

  test "returns true if the product has rich content with embedded posts" do
          create(:rich_content, entity: product, description: [{ "type" => "posts" }])

          expect(url_redirect.has_embedded_posts?).to be(true)
        end

  test "returns false if the product has rich content without embedded posts" do
  ...
  test "returns false if the product has no rich content" do
          expect(url_redirect.has_embedded_posts?).to be(false)
        end
      end
  ...
  test "returns true if the variant has rich content with embedded posts" do
          create(:rich_content, entity: variant, description: [{ "type" => "posts" }])

          expect(url_redirect.has_embedded_posts?).to be(true)
        end

  test "returns false if the variant has rich content without embedded posts" do
          create(:rich_content, entity: variant, description: [{ "type" => "paragraph", "content" => [{ "type" => "text", "text" => "Hello" }] }])

          expect(url_redirect.has_embedded_posts?).to be(false)
        end

  test "returns false if the variant has no rich content" do
          expect(url_redirect.has_embedded_posts?).to be(false)
        end

  test "returns true if the corresponding product's `has_same_rich_content_for_all_variants` is set to true and it has rich content with embedded posts" do
          product.update!(has_same_rich_content_for_all_variants: true)
          create(:rich_content, entity: product, description: [{ "type" => "posts" }])
          create(:rich_content, entity: variant, description: [{ "type" => "paragraph", "content" => [{ "type" => "text", "text" => "Hello" }] }])
  ...
  test "returns the installment files if the installment has files" do
          @installment.product_files << @product.product_files.first

          expect(@url_redirect.reload.alive_product_files).to eq [@product.product_files.first]
        end

  test "returns all product files if the installment has no files" do
  ...
  context_ "when `has_same_rich_content_for_all_variants` flag is set to true" do
          before do
            @product.update!(has_same_rich_content_for_all_variants: true)
          end

  test "returns product-level product files in the order they appear in the product-level rich content" do
  ...
  context_ "when `has_same_rich_content_for_all_variants` flag is set to false" do
          before do
            @product.update!(has_same_rich_content_for_all_variants: false)
          end

  test "returns purchased variant-level product files in the order they appear in the variant-level rich content" do
            expect(@url_redirect.alive_product_files).to eq([@product_file1, @product_file2, @product_file4])
          end

  context_ "when the purchased variant is deleted" do
            before do
              @category_1_option_a.mark_deleted!
            end

  test "returns the product-level product files in the order they appear in the product-level rich content" do
              expect(@url_redirect.alive_product_files).to eq([@product_file1, @product_file2, @product_file3, @product_file4])
            end
          end
        end

  context_ "when the purchase does not have variants" do
          before do
  ...
  context_ "#enqueue_job_to_regenerate_deleted_stamped_pdfs" do
  test "enqueues StampPdfForPurchaseJob only for missing deleted stamped pdfs" do
        url_redirect = create(:url_redirect)
  ...
  context_ "#missing_stamped_pdf?" do
  test "returns true when no alive stamped pdf exists for the file" do
        url_redirect = create(:url_redirect)
        product_file = create(:pdf_product_file, link: url_redirect.link, pdf_stamp_enabled: true)
  ...
  test "returns false when an alive stamped pdf exists for the file" do
        url_redirect = create(:url_redirect)
        product_file = create(:pdf_product_file, link: url_redirect.link, pdf_stamp_enabled: true)
  ...
  context_ "#update_transcoded_videos_last_accessed_at", :freeze_time do
  test "sets last_accessed_at to now" do
  ...
  context_ "#enqueue_job_to_regenerate_deleted_transcoded_videos" do
  test "enqueues a job for the deleted transcoded videos" do
  ...
  test "doesn't enqueue a job when nothing is missing" do
        product_file = create(:streamable_video, :analyze)
