...
class UserTest < ActiveSupport::TestCase
  self.described_class = User
  self.rspec_metadata = { vcr: true }



  context_ User, :vcr do
  ...
  context_ "affiliate associations" do
  ...
  context_ "collaborator associations" do
        it { is_expected.to have_many(:collaborators).with_foreign_key(:seller_id) }

  context_ "#collaborating_products" do
  test "only contains those from collaborations the user have accepted" do
            user = create(:user)
  ...
  context_ "scopes" do
  context_ ".payment_reminder_risk_state" do
  test "selects creators that are not reviewed, flagged for TOS, or compliant" do
  ...
  context_ ".not_suspended" do
  test "selects creators that are not suspended" do
          not_reviewed = create(:user, user_risk_state: "not_reviewed")
  ...
  context_ ".holding_balance_more_than" do
        before do
          @sam = create(:user)
  ...
  test "returns users who have unpaid balances more than the specified amount" do
          expect(described_class.holding_balance_more_than(120)).to match_array([@sam, @jake])
        end
      end

  context_ ".holding_balance" do
        before do
          @sam = create(:user)
  ...
  test "returns users who have unpaid balances more than 0" do
          expect(described_class.holding_balance).to match_array([@sam, @jake])
        end
      end

  context_ ".holding_non_zero_balance" do
        before do
          @sam = create(:user)
          create(:balance, user: @sam, amount_cents: 10)
          create(:balance, user: @sam, amount_cents: 11, date: 1.day.ago)
  ...
  test "returns users who have non-zero unpaid balances" do
          expect(described_class.holding_non_zero_balance).to match_array([@sam, @jake])
        end
      end
    end

  context_ "has_cdn_url" do
      before do
  ...
  context_ "#subscribe_preview_url" do
        before do
  ...
  context_ "#resized_avatar_url" do
        before do
  ...
  context_ "#financial_annual_report_url_for" do
  context_ "when no annual reports attached" do
  ...
  context_ "when annual report does not exist for year param" do
  ...
  context_ "when annual report exists" do
          let(:user) { create(:user, :with_annual_report) }

  test "returns report URL for the current year by default" do
            expect(user.financial_annual_report_url_for).to match("https://public-files.gumroad.com/")
          end

  context_ "for a previous year" do
            let(:year) { 2019 }

  test "returns report URL for the selected year" do
  ...
  context_ "#display_name" do
  context_ "when name is present" do
        before do
  ...
  context_ "when name is blank" do
        before do
          @user = create(:user, name: "", username: nil)
        end

  context_ "when 'prefer_email_over_default_username' is set to true" do
          subject(:display_name) { @user.display_name(prefer_email_over_default_username: true) }

  context_ "when a custom username is not set" do
  ...
  context_ "when a custom username is set" do
            before do
              @user.update!(username: "johndoe")
            end

  test "returns custom username" do
              expect(display_name).to eq("johndoe")
            end
          end
        end

  context_ "when 'prefer_email_over_default_username' is set to false" do
  ...
  context_ "#support_or_form_email" do
      let(:support_email) { "support-email@example.com" }
      let(:email) { "seller-email@example.com" }

  context_ "when support_email is set" do
  test "returns the support_email value" do
  ...
  context_ "when support_email is absent" do
  test "returns the email value" do
  ...
  context_ "#product_level_support_emails" do
      let(:user) { create(:user) }
  ...
  test "returns the user's product support emails" do
  ...
  context_ "#update_product_level_support_emails!" do
      let(:user) { create(:user) }
  ...
  test "updates products support emails" do
  ...
  context_ "#has_valid_payout_info?" do
      let(:user) { create(:user) }

  test "returns true if the user has valid PayPal account info" do
        allow(PaypalPayoutProcessor).to receive(:has_valid_payout_info?).and_return(true)
        expect(user.has_valid_payout_info?).to eq true
      end

  test "returns true if the user has valid Stripe account info" do
        allow(StripePayoutProcessor).to receive(:has_valid_payout_info?).and_return(true)
        expect(user.has_valid_payout_info?).to eq true
      end

  test "returns false if the user has neither PayPal nor Stripe account info" do
        expect(user.has_valid_payout_info?).to eq false
      end
    end

  context_ "merchant_account" do
      let(:user) { create(:user) }

  context_ "user has no merchant accounts" do
  test "returns nil" do
          expect(user.merchant_account("charge-processor-id")).to eq(nil)
        end
      end

  context_ "user has one merchant account" do
  ...
  test "returns the merchant account if matching charge processor id" do
          expect(user.merchant_account("charge-processor-id-1")).to eq(merchant_account_1)
        end

  test "returns nil if stripe merchant account is for cross-border payouts only and can not accept charges" do
          create(:merchant_account, user:, country: "TH")
          expect(user.merchant_account("stripe")).to be nil
        end

  test "returns the merchant account if it can accept charges" do
          merchant_account = create(:merchant_account, user:, country: "HK")
          expect(user.merchant_account("stripe")).to eq(merchant_account)
        end

  test "returns nil if not matching charge processor id" do
          expect(user.merchant_account("charge-processor-id-2")).to eq(nil)
        end
      end

  context_ "user has multiple merchant accounts" do
        let(:merchant_account_1) { create(:merchant_account, user:, charge_processor_id: "charge-processor-id-1") }
  ...
  test "returns the Stripe Connect account if present and merchant migration is enabled" do
  ...
  test "returns the custom stripe merchant account if no Stripe Connect account present" do
  ...
  test "returns the custom stripe merchant account if merchant migration is not enabled" do
          Feature.deactivate_user(:merchant_migration, @creator)
          expect(@creator.merchant_account(StripeChargeProcessor.charge_processor_id)).to eq(@stripe_account)
        end

  test "returns nil if neither Stripe Connect nor custom stripe merchant account present" do
          @stripe_connect_account.mark_deleted!
  ...
  context_ "#subdomain_with_protocol" do
  test "returns subdomain_with_protocol of the user after converting underscores to hyphens" do
        @creator = create(:user)
  ...
  test "returns subdomain of the user after converting underscores to hyphens" do
        @creator = create(:user)
        # We don't support underscores in username now, but we need to generate subdomain_with_protocol for
        # old creators who have underscores in their username.
        @creator.update_column(:username, "test_user_1")
  ...
  test "returns nil if blank hostname provided" do
        expect(User.find_by_hostname("")).to eq(nil)
        expect(User.find_by_hostname(nil)).to eq(nil)
      end

  test "finds user by subdomain" do
        expect(User.find_by_hostname("john.#{@root_hostname}")).to eq(@creator_with_subdomain)
      end

  test "finds user by custom domain" do
        expect(User.find_by_hostname("example.com")).to eq(@creator_with_custom_domain)
      end
    end

  context_ "#two_factor_authentication_enabled" do
      before do
        @user = create(:user, skip_enabling_two_factor_authentication: false)
      end

  test "sets two_factor_authentication_enabled to true by default" do
        expect(@user.two_factor_authentication_enabled).to eq true
      end
    end

  context_ "#set_refund_fee_notice_shown" do
  test "sets refund_fee_notice_shown to true by default" do
        expect(create(:user).refund_fee_notice_shown?).to eq true
      end
    end

  context_ "#set_refund_policy_enabled" do
  test "sets refund_policy_enabled to true by default" do
  ...
  test "sets refund_policy_enabled to false" do
          user = create(:user)
  ...
  context_ "when seller_refund_policy_new_users_enabled feature flag is set to true" do
        before do
  ...
  context_ "#account_level_refund_policy_enabled?" do
      let(:user) { create(:user) }

      it { expect(user.account_level_refund_policy_enabled?).to be true }

  context_ "with account_level_refund_policy_delayed_for_sellers feature flag" do
        before { Feature.activate_user(:account_level_refund_policy_delayed_for_sellers, user) }

  context_ "with time" do
  test "returns false before LAST_ALLOWED_TIME_FOR_PRODUCT_LEVEL_REFUND_POLICY" do
  ...
  test "returns true after LAST_ALLOWED_TIME_FOR_PRODUCT_LEVEL_REFUND_POLICY" do
  ...
  context_ "with seller_refund_policy_disabled_for_all" do
  ...
  context_ "#paypal_payout_email" do
      let(:user) { create(:user, payment_address: "payme@example.com") }

  test "returns the payment_address if it is present" do
  ...
  test "returns the email associated with connected PayPal account if payment_address is not present" do
        expect(user.paypal_payout_email).to eq "payme@example.com"
  ...
  test "returns nil if neither the payment_address nor connected PayPal account are present" do
        user.update!(payment_address: "")
        expect(user.paypal_payout_email).to eq nil
      end

  test "returns nil if payment_address is blank and connected PayPal account details are not available" do
        user.update!(payment_address: "")
        create(:merchant_account_paypal, user:, charge_processor_merchant_id: "B66YJBBNCRW6L")
  ...
  context_ "#build_user_compliance_info" do
      before do
        @user = build(:user)
      end

  test "sets json_data of user_compliance_info to an empty json" do
  ...
  context_ "when user can be deactivated" do
  ...
  test "invalidates all the active sessions" do
  ...
  context_ "when user has a saved credit card" do
          before do
            @credit_card = create(:credit_card, users: [@user])
          end

  test "clears the saved credit card information" do
            expect do
              @user.deactivate!
  ...
  context_ "when user has a custom domain" do
          before do
            @custom_domain = create(:custom_domain, user: @user)
          end

  test "marks the custom domain as deleted" do
            expect do
              @user.deactivate!
            end.to change { @custom_domain.reload.deleted_at }.from(nil).to(be_present)
          end

  test "handles deactivation when custom domain is already deleted" do
            @custom_domain.mark_deleted!
            expect { @user.deactivate! }.not_to raise_error
          end
        end

  context_ "when the user has active subscriptions" do
  ...
  test "cancels the active subscriptions" do
            expect do
              @user.deactivate!
  ...
  context_ "when user cannot be deactivated" do
  ...
  test "does not deactivate the user" do
            return_value = nil
            ...
            expect(@user.read_attribute(:username)).not_to be_nil
  ...
  test "does not clear the saved credit card information" do
              expect do
                @user.deactivate!
  ...
  context_ "when user has unpaid balances" do
          before do
  ...
  context_ "when user has negative balances and the feature delete_account_forfeit_balance is active" do
          before do
  ...
  context_ "google_analytics_id" do
        [
          { id: nil, valid: true },
  ...
  test "expects #{data[:id]} to be #{data[:valid] ? 'valid' : 'invalid'}" do
  ...
  context_ "name" do
  test "is valid if it's blank" do
          @user.name = nil
          expect(@user).to be_valid
        end

  test "is valid if it's normal length" do
          @user.name = sample_string_of_length(25)
          expect(@user).to be_valid
        end

  test "is invalid if too long" do
          @user.name = sample_string_of_length(256)
          expect(@user).to be_invalid
        end

  context_ "format" do
  test "is invalid if it contains colon character on create" do
  ...
  test "is invalid if it contains colon character when name is updated" do
  ...
  test "is valid if colon character exists but name is not changed" do
  ...
  context_ "username" do
  test "is valid if it's nil" do
          @user.username = nil
          expect(@user).to be_valid
        end

  test "is gets nilified if it's empty" do
          @user.username = ""
  ...
  context_ "length" do
  test "is valid if it's 3 or more characters but less than 21 characters" do
            @user.username = sample_string_of_length(rand(3..19))
            expect(@user).to be_valid
          end

  test "is invalid if greater than 20 characters" do
            @user.username = sample_string_of_length(21)
            expect(@user).to be_invalid
          end

  test "is invalid ok if it's less than 3 characters" do
  ...
  context_ "format" do
  test "doesn't allow underscore in usernames" do
            @user.username = "a_aa"
            expect(@user).to be_invalid
          end

  test "doesn't allow hyphens in usernames" do
            @user.username = "a-a"
            expect(@user).to be_invalid
          end

  test "doesn't allow usernames with only numbers" do
            @user.username = "1234"
            expect(@user).to be_invalid
          end

  test "is valid with numbers" do
            @user.username = "abc123"
            expect(@user).to be_valid
          end

  test "is invalid if japanese" do
            @user.username = "日本の"
            expect(@user).to be_invalid
          end

  test "is invalid if caps" do
            @user.username = "LOUDNOISES"
            expect(@user).to be_invalid
          end

  test "is invalid with only numbers" do
            @user.username = "12345"
            expect(@user).to be_invalid
          end

  test "is invalid if it has spaces" do
            @user.username = "a a"
            expect(@user).to be_invalid
          end
        end

  test "is invalid if it's blacklisted" do
          @user.username = DENYLIST.sample
          expect(@user).to be_invalid
        end

  context_ "validation condition" do
          before do
            @user2 = create(:user)
  ...
  context_ "when username is of old style and it isn't changed" do
  test "allows saving the user object" do
  ...
  context_ "when username is changed" do
  test "enforces new username format" do
  ...
  context_ "email" do
  test "is invalid if email is required and not present" do
          @user.email = nil
          allow(@user).to receive(:email_required?).and_return(true)
          expect(@user).to be_invalid
        end

  test "is invalid if email is not in email format" do
          @user.email = "invalid"
          expect(@user).to be_invalid
        end

  test "is invalid if email address starts with a ." do
          @user.email = ".blah@blah.com"
          expect(@user).to be_invalid
        end

  test "is invalid if email address has whitespace in it" do
          @user.email = "bla\th@blah.com"
          expect(@user).to be_invalid
        end

  test "is valid if email address is correct" do
          @user.email = "blah@blah.com"
          expect(@user).to be_valid
        end

  test "is valid if email address has a dash" do
          @user.email = "blah-blah@blah.com"
          expect(@user).to be_valid
        end

  test "is valid if email address has an underscore" do
          @user.email = "blah_blah@blah.com"
          expect(@user).to be_valid
        end

  test "is valid if email address domain is IP" do
          @user.email = "blah@[192.0.0.1]"
          expect(@user).to be_valid
        end

  test "is valid if email has 255 characters" do
          @user.email = "a" * 249 + "@b.com"
          expect(@user).to be_valid
        end

  test "is invalid if email has more than 255 characters" do
          @user.email = "a" * 250 + "@b.com"
          expect(@user).to be_invalid
        end
      end

  context_ "kindle_email" do
  test "is valid if kindle_email is blank" do
  ...
  test "is invalid if kindle_email is not an email address" do
          @user.kindle_email = "invalid"
          expect(@user).to be_invalid
        end

  test "is invalid if kindle_email address starts with a ." do
          @user.kindle_email = ".blah@kindle.com"
          expect(@user).to be_invalid
        end

  test "is invalid if kindle_email address has whitespace in it" do
          @user.kindle_email = "bla\th@kindle.com"
          expect(@user).to be_invalid
        end

  test "is valid if kindle_email is correct email address but does not end in @kindle.com" do
          @user.kindle_email = "blah@blah.com"
          expect(@user).to be_invalid
        end

  test "is valid if kindle_email is correct" do
          @user.kindle_email = "blah@kindle.com"
          expect(@user).to be_valid
        end

  test "is valid if kindle_email has a dash" do
          @user.kindle_email = "blah-blah@kindle.com"
          expect(@user).to be_valid
        end

  test "is valid if kindle_email has an underscore" do
          @user.kindle_email = "blah_blah@kindle.com"
          expect(@user).to be_valid
        end

  test "is valid if kindle_email uses different cases" do
          @user.kindle_email = "ExAmple123@KINDLE.com"
          expect(@user).to be_valid
        end

  test "is valid if kindle_email does not have more than 255 characters" do
          @user.kindle_email = "a" * 244 + "@kindle.com"
          expect(@user).to be_valid
        end

  test "is invalid if kindle_email has more than 255 characters" do
          @user.kindle_email = "a" * 245 + "@kindle.com"
          expect(@user).to be_invalid
        end
      end

  context_ "password" do
  context_ "presence" do
  test "is invalid if it is not present and password is required" do
            @user.password = nil
            allow(@user).to receive(:password_required?).and_return(true)
            expect(@user).to be_invalid
          end

  test "is valid if it is not present and password is not required" do
  ...
  context_ "confirmation" do
          before :each do
            @user.password = "password"
          end

  test "is invalid if confirmation does not match and password is required" do
            @user.password_confirmation = @user.password + "typo"
            allow(@user).to receive(:password_required?).and_return(true)
            expect(@user).to be_invalid
          end

  test "is valid if confirmation does not match and password is not required" do
            @user.password_confirmation = @user.password + "typo"
            allow(@user).to receive(:password_required?).and_return(false)
            expect(@user).to be_valid
          end
  ...
  context_ "length" do
  test "is valid if it's 6 or more characters but less than 128 characters" do
            @user.password = sample_string_of_length(rand(6..127))
            expect(@user).to be_valid
          end

  test "is invalid if greater than 128 characters" do
            @user.password = sample_string_of_length(129)
            expect(@user).to be_invalid
          end

  test "is invalid if it's less than 4 characters" do
  ...
  context_ "locale" do
  test "is valid if it's null" do
          @user.locale = nil
          expect(@user).to be_valid
        end

  test "is valid if it's available" do
          @user.locale = "en"
          expect(@user).to be_valid
        end
      end

  context_ "currency type" do
  test "is valid if currency type is in valid currencies" do
          @user.currency_type = "usd"
          expect(@user).to be_valid
        end

  test "is invalid if currency type is not in valid currencies" do
  ...
  context_ "#subscribe_preview_url" do
  context_ "when user doesn't have a subscribe preview" do
  test "returns nil" do
            expect(@user.subscribe_preview_url).to eq(nil)
          end
        end

  context_ "when user has a subscribe preview" do
          before do
            @user_with_preview = create(:user, :with_subscribe_preview)
          end

  test "returns URL to user's subscribe preview" do
            key = @user_with_preview.subscribe_preview.key
  ...
  context_ "when avatar is attached but file is missing from storage" do
  test "returns URL to default avatar" do
  ...
  context_ "when temp file is deleted during variant processing" do
  test "returns URL to default avatar" do
            allow(@user.avatar).to receive(:attached?).and_return(true)
  ...
  context_ "when avatar blob is deleted but attachment record still exists" do
  test "returns URL to default avatar" do
            allow(@user.avatar).to receive(:attached?).and_return(true)
  ...
  context_ "avatar_url" do
  test "allows avatar to be nil" do
  ...
  test "validates the uploaded avatar file's extension" do
          user = create(:named_user, :with_avatar)

          expect(user).to be_valid
        end

  test "fails the validation when uploaded profile picture is too heavy" do
  ...
  test "fails the validation when uploaded profile picture is of unsupported filetype" do
  ...
  test "fails validation when the uploaded profile picture is smaller than 200x200px" do
          blob = ActiveStorage::Blob.create_and_upload!(io: File.open("#{Rails.root}/test/support/fixtures/test-small.png"), filename: "test-small.png")
          @user.avatar.attach(blob)
  ...
  test "doesn't fail validation when an existing profile picture is smaller than 200x200px" do
          @user.avatar.attach(io: File.open("#{Rails.root}/test/support/fixtures/test-small.png"), filename: "test-small.png")

          expect(@user.validate).to eq(true)
          expect(@user.errors[:base]).to be_empty
        end

  test "returns original file url for files with conversion issues" do
  ...
  context_ "account_created_email_domain_is_not_blocked validation" do
  context_ "when the email domain is blocked" do
          before do
            PlatformBlock.add!(object_type: PlatformBlock::TYPES[:email_domain], object_value: "example.com")
          end

  test "fails the validation" do
  ...
  context_ "when the email domain is not blocked" do
  test "validates the user successfully" do
  ...
  context_ "when the email is not valid" do
  test "returns is invalid error" do
  ...
  context_ "account_created_ip" do
  context_ "when the IP is blocked" do
          before do
  ...
  context_ "when the IP is not blocked" do
  test "validates the user successfully" do
            @user.account_created_ip = "127.0.0.1"
  ...
  context_ "when the IP is not set" do
  test "validates the user successfully" do
  ...
  context_ "facebook_meta_tag" do
  test "allows facebook_meta_tag to be nil" do
          @user.facebook_meta_tag = nil
          expect(@user).to be_valid
        end

  test "validates facebook_meta_tag length is less than or equal to 100" do
  ...
  test "validates the facebook_meta_tag has the correct format" do
          @user.facebook_meta_tag = '<meta name="facebook-domain-verification" content="y5fgkbh7x91y5tnt6yt3sttk" />'
          expect(@user).to be_valid
        end

  test "fails the validation when facebook_meta_tag is invalid" do
  ...
  context_ "reserved domain validation for support_email" do
  test "allows support_email to be nil" do
          @user.support_email = nil
          expect(@user).to be_valid
        end

  test "fails the validation when domain is reserved" do
  ...
  context_ "custom_fee_per_thousand" do
  test "allows nil and an integer between 0 and 1000" do
  ...
  context_ "user roles" do
  test "is considered an affiliate if it is present in the affiliates table" do
        @user = create(:user)
  ...
  test "is not considered an affiliate if it is not present in the affiliates table" do
        @user = create(:user)
  ...
  context_ "#account_active?" do
  test "returns true for a live user" do
        user = build(:user)
  ...
  test "returns false for a suspended user" do
        user = create(:user)
  ...
  context_ "email" do
  test "does not allow same email address" do
        user = create(:user)
        expect(build(:user, email: user.email).save).to be(false)
      end

  test "does not have unconfirmed_email if all emails have been confirmed" do
        user = create(:user)
  ...
  context_ "append http" do
  test "appends an http to notification_endpoint" do
        user = create(:user)
  ...
  context_ "valid_password?" do
  test "can check a user's password" do
        user = build(:user, password: "password")
        expect(user).to be_valid_password("password")
        expect(user).not_to be_valid_password("INVALD")
      end
    end

  context_ "#clear_products_cache" do
      before do
        @user = create(:user)
  ...
  context_ "callback triggering behavior" do
  test "is called automatically after update if any of `User::LINK_PROPERTIES` are changed" do
  ...
  test "isn't called if an attribute not included in `User::LINK_PROPERTIES` is changed" do
  ...
  test "queues product caching invalidation worker for all user products" do
        expect(InvalidateProductCacheWorker).to receive(:perform_bulk).with([[@product_1.id], [@product_2.id]])
        @user.clear_products_cache
      end
    end

  context_ "#credit_card_info" do
      let(:user) { create(:user) }
      let(:link) { create(:product) }
      let(:credit_card) { create(:credit_card) }

  test "appends zero to date if expiry month is smaller than 10" do
  ...
  test "does not suspend the user if the user is verified" do
        @user.update_attribute(:verified, true)
        expect(@user.suspend_for_fraud(author_id: @admin_user.id)).to be(false)
      end

  test "does not flag the user if the user is verified" do
        @user.update_attribute(:verified, true)
  ...
  test "suspends the user if the user is not verified, and was previously flagged for fraud" do
        @user.flag_for_fraud!(author_id: @admin_user.id)
        expect(@user.suspend_for_fraud!(author_id: @admin_user.id)).to be(true)
      end

  test "suspends the user directly from not_reviewed state" do
        expect(@user.user_risk_state).to eq("not_reviewed")
        expect(@user.suspend_for_fraud!(author_id: @admin_user.id)).to be(true)
        expect(@user.reload.suspended_for_fraud?).to be(true)
      end

  test "suspends the user directly from compliant state" do
  ...
  test "suspends for fraud from flagged_for_tos_violation state" do
        @user.flag_for_tos_violation!(author_id: @admin_user.id, product_id: @product_1.id)
        expect(@user.suspend_for_fraud!(author_id: @admin_user.id)).to be(true)
        expect(@user.reload.suspended_for_fraud?).to be(true)
      end

  test "suspends for tos_violation from flagged_for_fraud state" do
        @user.flag_for_fraud!(author_id: @admin_user.id)
        expect(@user.suspend_for_tos_violation!(author_id: @admin_user.id)).to be(true)
        expect(@user.reload.suspended_for_tos_violation?).to be(true)
      end

  test "is expected to call invalidate_active_sessions! if user is suspended_for_fraud" do
  ...
  test "is expected to call invalidate_active_sessions! if user is suspended_for_tos_violation" do
        expect(@user).to receive(:invalidate_active_sessions!)
  ...
  test "marks custom domain as deleted when suspended for fraud" do
          @user.flag_for_fraud!(author_id: @admin_user.id)
  ...
  test "marks custom domain as deleted when suspended for TOS violation" do
          @user.flag_for_tos_violation!(author_id: @admin_user.id, product_id: @product_1.id)
  ...
  test "handles suspension when custom domain is already deleted" do
          @custom_domain.mark_deleted!
          @user.flag_for_fraud!(author_id: @admin_user.id)
  ...
  test "handles suspension when user has no custom domain" do
  ...
  test "adds a comment when flagging for TOS violation" do
        expect do
          @user.flag_for_tos_violation!(author_id: @admin_user.id, product_id: @product_1.id)
  ...
  test "does not add a comment for bulk flagging for TOS violation" do
        expect do
          @user.flag_for_tos_violation!(author_id: @admin_user.id, bulk: true)
        end.not_to change { @product_1.comments.reload.count }
      end



  context_ "seller with multiple accounts" do
        before do
  ...
  test "disables all the sellers links if the user is suspended" do
          ...
          expect(@product_3.reload.banned_at).not_to be(nil)
          expect(@product_4.reload.banned_at).not_to be(nil)

          @user.flag_for_fraud(author_id: @admin_user.id)
          @user.suspend_for_fraud(author_id: @admin_user.id)
          expect(@product_1.reload.banned_at).not_to be(nil)
          expect(@product_2.reload.banned_at).not_to be(nil)
        end

  test "reenables old links if the user is moved to probation" do
          @user_2.mark_compliant(author_id: @admin_user.id)
          @user_2.flag_for_tos_violation(author_id: @admin_user.id, product_id: @product_3.id)
          @user_2.suspend_for_tos_violation(author_id: @admin_user.id)
          expect(@product_3.reload.banned_at).not_to be(nil)
  ...
  test "suspends all the others sellers accounts if suspended for fraud", :sidekiq_inline do
  ...
  test "does not suspend all the others sellers accounts if suspended for tos violation" do
          @user.flag_for_tos_violation(author_id: @admin_user.id, product_id: @product_1.id)
          @user.suspend_for_tos_violation(author_id: @admin_user.id)
          expect(@user_2.reload.suspended?).to be(false)
        end

  test "re-enables all the sellers related accounts if the seller is marked compliant" do
  ...
  test "re-enables all the sellers links if the seller is marked compliant" do
          @user.flag_for_fraud!(author_id: @admin_user.id)
          @user.suspend_for_fraud(author_id: @admin_user.id)
          expect(@product_1.reload.banned_at).not_to be(nil)
          expect(@product_2.reload.banned_at).not_to be(nil)
  ...
  context_ "when suspended user's risk_state is updated" do
        before do
          @user.flag_for_fraud(author_id: @admin_user.id)
          @user.suspend_for_fraud(author_id: @admin_user.id)
  ...
  test "adds user's subdomain to stripe_apple_pay_domains when marked compliant" do
          @user.mark_compliant(author_id: @admin_user.id)
          expect(CreateStripeApplePayDomainWorker).to have_enqueued_sidekiq_job(@user.id)
        end
      end

  context_ "#flagged_for_explicit_nsfw?" do
  test "returns true when user is flagged for TOS violation with explicit NSFW reason" do
  ...
  test "returns false when user is flagged for TOS violation with different reason" do
  ...
  test "returns false when user is not flagged for TOS violation" do
          expect(@user.flagged_for_explicit_nsfw?).to be(false)
        end
      end

  context_ "#flag_for_explicit_nsfw_tos_violation!" do
  test "flags user for TOS violation with explicit NSFW reason" do
          expect do
  ...
  test "unpublishes all alive products" do
  ...
  context_ "purchasing_power_parity_limit" do
      let (:user) { create(:user) }

  context_ "attempting to set limit between 1-100" do
  test "does not throw" do
          expect { user.update!(purchasing_power_parity_limit: 40) }.not_to raise_error
        end
      end

  context_ "attempting to set limit below 1" do
  test "throws an error" do
          expect { user.update!(purchasing_power_parity_limit: 0) }.to raise_error(ActiveRecord::RecordInvalid)
        end
      end

  context_ "attempting to set limit above 100" do
  test "throws an error" do
  ...
  context_ "min_ppp_factor" do
  context_ "no limit set" do
        let(:user) { create(:user) }
  test "returns 0" do
  ...
  context_ "limit set" do
        let(:user) { create(:user, purchasing_power_parity_limit: 40) }
  test "returns the inverse limit as a decimal percentage" do
          expect(user.min_ppp_factor).to eq(0.6)
        end
      end
    end

  context_ "max_product_price" do
  context_ "not verified" do
        let(:user) { create(:user) }
  test "returns the default maximum product price" do
  ...
  test "returns nil, as the max product price is unlimited" do
  ...
  test "is valid with an empty hash" do
        @user.json_data = {}
        expect(@user).to be_valid
      end

  test "understands nil to be an empty hash" do
        @user.json_data = nil
        expect(@user).to be_valid
      end

  test "accepts a key, value pair" do
        @user.json_data[:fizz] = "buzz"
        expect(@user).to be_valid
      end

  test "is still valid when a key is set to an empty value" do
        @user.json_data[:some_key] = nil
        expect(@user).to be_valid
      end

  test "is never anything but a hash" do
  ...
  context_ "Australia tax period" do
      before do
        @user = create(:user)
      end
  ...
  context_ "#save_gumroad_day_timezone" do
      let!(:seller) { create(:user) }

  test "does nothing and returns if waive_gumroad_fee_on_new_sales? is false" do
  ...
  test "saves the seller's current timezone as gumroad_day_timezone" do
  ...
  test "does not overwrite the gumroad_day_timezone if it's already set" do
        Feature.activate_user(:waive_gumroad_fee_on_new_sales, seller)
        expect(seller.waive_gumroad_fee_on_new_sales?).to be true
  ...
  context_ "#gumroad_day_saved_fee_cents" do
  test "returns 0 if no new paid sales made by seller on Gumroad Day" do
  ...
  test "returns 10% of the new sales volume made by seller on Gumroad Day" do
  ...
  context_ "#gumroad_day_saved_fee_amount" do
  test "returns nil if gumroad_day_saved_fee_cents is 0" do
        seller_with_no_sales = create(:user)
        expect(seller_with_no_sales.gumroad_day_saved_fee_cents).to eq(0)
  ...
  test "returns formatted amount for gumroad_day_saved_fee_cents" do
  ...
  context_ "name_or_username" do
      let(:user) { create(:user, name: "Katsuya Noguchi", username: "katsuya") }

  test "returns name if name and username is present" do
        expect(user.name_or_username).to eq user.name
      end

  test "returns name if name is present but username is not" do
        user.username = nil
        expect(user.name_or_username).to eq user.name
      end

  test "returns username if username is present but name is not" do
        user.name = nil
        expect(user.name_or_username).to eq user.username
      end
    end

  context_ "has_workflows" do
      before do
        @user = create(:user)
        @product = create(:product, user: @user)
      end

  test "returns true if user has seller workflows" do
        create(:workflow, seller: @user, published_at: 1.day.ago)
        expect(@user.has_workflows?).to be(true)
      end

  test "returns true if user has product workflows" do
        create(:workflow, seller: @user, link: @product, published_at: 1.day.ago)
        expect(@user.has_workflows?).to be(true)
      end

  test "returns false if user has no workflows" do
        expect(@user.has_workflows?).to be(false)
      end
    end

  context_ "#pay_with_paypal_enabled?" do
      let(:user) { create(:user) }

  context_ "when a merchant account is connected" do
        before do
  ...
  test "returns false if disabled via feature flag" do
  ...
  context_ "when a merchant account is not connected" do
        before do
          Feature.deactivate(:disable_braintree_sales)
        end

  test "returns true for non-complaint user" do
          expect(user.alive_user_compliance_info).to be_nil
          expect(user.pay_with_paypal_enabled?).to be(true)
        end

  test "returns true for user in un-supported Paypal Connect country" do
          create(:user_compliance_info, user:, country: "India")
          expect(user.pay_with_paypal_enabled?).to be(true)
        end

  test "returns false for user in supported country" do
          create(:user_compliance_info, user:)
          expect(user.pay_with_paypal_enabled?).to be(false)
        end
  ...
  context_ "#pay_with_card_enabled?" do
      let(:user) { create(:user) }
  ...
  context_ "when an active merchant account record exists" do
  test "returns true" do
            expect(user.pay_with_card_enabled?).to be(true)
          end
        end

  context_ "when no active merchant account record exists" do
          before do
  ...
  context_ "#requires_credit_card?" do
      let(:user) { create(:user) }
  ...
  test "returns true if user has an active preorder authorization" do
  ...
  test "returns false if user has no active subscription or preorder authorization" do
        expect(user.requires_credit_card?).to be(false)
      end

  test "returns false if the user only has free subscriptions" do
  ...
  test "returns false if the user only has test subscriptions" do
        subscription = create(:subscription, user:)
  ...
  context_ "#alive_product_files_excluding_product" do
      before do
  ...
  test "only returns alive product files from alive products" do
  ...
  test "only returns alive product files from alive products excluding product files with the passed product id" do
  ...
  test "doesn't return duplicate product files with same url" do
  ...
  context_ "#alive_product_files_preferred_for_product" do
      let(:user) { create(:user) }
  ...
  test "returns alive product files that are unique by `url` even if there are no product files associated with the specified product" do
  ...
  test "returns alive product files associated with the specified product even if it not published" do
  ...
  test "does not include duplicate product files and prefers those product files among the duplicates that belong to the specified product" do
        product_file = product.product_files.alive.first
        duplicate_file_url = "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachment/pencil.png"
  ...
  test "strips facebook_meta_tag" do
  ...
  test "strips google_analytics_id" do
  ...
  test "strips username and still allows it to be reset to nil" do
  ...
  context_ "#move_purchases_to_new_email" do
  test "runs after 'email' is updated and schedules an update job" do
        user = create(:user)
  ...
  context_ "#update_alive_cart_email" do
  test "syncs the cart email to the user's new email when user email is updated" do
  ...
  context_ "#make_affiliate_of_the_matching_approved_affiliate_requests" do
  ...
  context_ "when user signs up and confirms using the same email address which was provided while applying to be an affiliate of a creator" do
  context_ "when this is the first time we are processing the user's pre-signup approved affiliate request" do
  test "makes the user an affiliate of the matching approved affiliate requests" do
  ...
  context_ "when the user's pre-signup approved affiliate request has been already processed and the user has confirmed their account again after changing their email" do
          before do
            user.update!(pre_signup_affiliate_request_processed: true)
          end

  test "does not process the user's pre-signup approved affiliate request again" do
            expect_any_instance_of(AffiliateRequest).not_to receive(:make_requester_an_affiliate!)

            expect do
              user.confirm
            end.not_to change { user.pre_signup_affiliate_request_processed? }
          end
        end
      end

  context_ "when there are no approved affiliate requests matching the email of the confirmed user" do
  ...
  test "returns matching TZ database name" do
  ...
  context_ "#timezone_formatted_offset" do
  test "returns matching UTC offset" do
  ...
  context_ "#supports_card?" do
      before do
        @creator = create(:user)
      end

  context_ "creator supports native paypal payments" do
        before do
          create(:merchant_account_paypal, user: @creator, charge_processor_merchant_id: "CJS32DZ7NDN5L", currency: "gbp")
        end

  test "returns true if card processor is nil" do
  ...
  test "returns true if card processor is stripe" do
          stripe_card = create(:credit_card)
          expect(@creator.supports_card?(stripe_card.as_json)).to be true
        end

  test "returns false if card processor is braintree" do
  ...
  test "returns true if card processor is paypal" do
  ...
  context_ "creator doesn't support native paypal payments" do
  test "returns true if card processor is nil" do
          new_card = CreditCard.new_card_info
          expect(@creator.supports_card?(new_card)).to be true
  ...
  test "returns true if card processor is braintree" do
          braintree_paypal_card = create(:credit_card, chargeable: create(:paypal_chargeable))
          expect(@creator.supports_card?(braintree_paypal_card.as_json)).to be true
        end

  test "returns false if card processor is paypal" do
          native_paypal_card = create(:credit_card, chargeable: create(:native_paypal_chargeable))
  ...
  context_ "#merchant_account_currency" do
  test "returns the currency of merchant account for given charge processor" do
        creator = create(:user)
  ...
  context_ "#paypal_disconnect_allowed?" do
  test "returns true if creator has no pending preorders or active subscribers using paypal direct payments, else false" do
        creator = create(:user)
  ...
  context_ "#stripe_account" do
  test "returns the custom stripe connect account which is managed by gumroad if present" do
        creator = create(:user)
  ...
  context_ "#stripe_connect_account" do
  test "returns creator's own standard stripe connect account if present" do
        creator = create(:user)
  ...
  context_ "#invalidate_active_sessions!" do
      let(:user) { create(:user) }
  ...
  test "invalidates all active sessions of the user and revokes active access tokens that are used to authorize the user in the mobile application" do
        travel_to(DateTime.current) do
          expect do
  ...
  context_ "#init_default_notification_settings" do
  test "sets default notification settings for new users" do
        user = User.new
  ...
  context_ "#enable_tipping" do
  test "sets tipping_enabled to true for new users" do
        user = User.new
  ...
  context_ "#enable_discover_boost" do
  test "sets discover_boost_enabled to true for new users" do
        user = User.new
  ...
  context_ "after_commit callback to enqueue GenerateSubscribePreviewJob" do
      before do
  ...
  test "does not schedule GenerateSubscribePreviewJob when email changes" do
        @user.update!(email: "new.email@example.com")
        expect(GenerateSubscribePreviewJob.jobs.size).to eq(0)
      end

  test "schedules GenerateSubscribePreviewJob when a new user is created" do
  ...
  test "schedules GenerateSubscribePreviewJob when the username changes" do
        @user.update!(username: "newusername")
        expect(GenerateSubscribePreviewJob).to have_enqueued_sidekiq_job(@user.id)
      end

  test "schedules GenerateSubscribePreviewJob when the name changes" do
        @user.update!(name: "Marty McFly")
        expect(GenerateSubscribePreviewJob).to have_enqueued_sidekiq_job(@user.id)
      end

  test "schedules GenerateSubscribePreviewJob when the seller highlight_color changes" do
        @user.seller_profile.update!(highlight_color: "#133337")
        @user.save!
        expect(GenerateSubscribePreviewJob).to have_enqueued_sidekiq_job(@user.id)
      end

  test "schedules GenerateSubscribePreviewJob when the seller background_color changes" do
        @user.seller_profile.update!(background_color: "#133337")
        @user.save!
        expect(GenerateSubscribePreviewJob).to have_enqueued_sidekiq_job(@user.id)
      end

  test "schedules GenerateSubscribePreviewJob when the seller font changes" do
        @user.seller_profile.update!(font: "Inter")
        @user.save!
        expect(GenerateSubscribePreviewJob).to have_enqueued_sidekiq_job(@user.id)
      end

  test "schedules GenerateSubscribePreviewJob when avatar changes" do
  ...
  context_ "after_save callback to enqueue StripeApplePayDomain jobs" do
      before :each do
        @user = create(:user, username: "usernamesample")
      end

  test "schedules StripeApplePayDomain jobs when username is changed" do
        CreateStripeApplePayDomainWorker.jobs.clear
  ...
  test "schedules StripeApplePayDomain jobs when new user is created" do
        expect(CreateStripeApplePayDomainWorker).to have_enqueued_sidekiq_job(@user.id)
        expect(DeleteStripeApplePayDomainWorker.jobs.size).to eq(0)
      end

  test "does not schedule StripeApplePayDomain jobs when username is unchanged" do
        CreateStripeApplePayDomainWorker.jobs.clear
        @user.name = "newname"
        @user.save!
  ...
  context_ "after_create #create_global_affiliate!" do
  test "creates a global affiliate record after user creation" do
        user = build(:user)
        expect do
          user.save
  ...
  context_ "after_create #create_refund_policy!" do
  test "creates a refund policy after user creation" do
        user = build(:user)
        expect { user.save }.to change(SellerRefundPolicy, :count).by(1)
      end
    end

  context_ "#generate_username" do
  test "enqueues a job to generate a unique username" do
  ...
  context_ "when the seller has $100K revenue" do
        before do
  ...
  context_ "when the seller less than $100K revenue" do
        before do
  ...
  context_ "#admin_page_url" do
  test "returns the admin users page url" do
        user = create(:user)
        expect(user.admin_page_url).to eq("#{PROTOCOL}://#{DOMAIN}/admin/users/#{user.id}")
      end
    end

  context_ "#compliance_info_resettable?" do
  test "returns true if the user doesn't have an active Stripe account" do
        user = create(:user_with_compliance_info)
        expect(user.compliance_info_resettable?).to eq true
      end

  test "returns true if the user doesn't have any purchases or balances associated with an active Stripe account" do
        user = create(:user_with_compliance_info)
        create(:merchant_account, user:)
        expect(user.compliance_info_resettable?).to eq true
      end

  test "returns false if the user has a balance associated with an active Stripe account" do
        user = create(:user_with_compliance_info)
        merchant_account = create(:merchant_account, user:)
  ...
  test "returns false if the user has a purchase associated with an active Stripe account" do
        user = create(:user_with_compliance_info)
  ...
  context_ "#has_unconfirmed_email?" do
  test "returns true if the user has an unconfirmed email" do
        user = create(:user, unconfirmed_email: Faker::Internet.email)
        expect(user.has_unconfirmed_email?).to eq true
      end

  test "returns true if the user.confirmed? is false" do
        user = create(:user, confirmed_at: nil)
        expect(user.has_unconfirmed_email?).to eq true
      end

  test "return false if the user has a confirmed email" do
        user = create(:user)
        expect(user.has_unconfirmed_email?).to eq false
      end
    end

  context_ "#collaborator_for?" do
      let(:user) { create(:user) }
      let(:product) { create(:product) }

  test "returns true if the user is currently a collaborator on another seller's products, false otherwise" do
  ...
  test "returns false if the user is no longer a collaborator" do
  ...
  test "returns false if the user is not a collaborator" do
        create(:collaborator, affiliate_user: user)
        expect(user.collaborator_for?(product)).to eq false
      end
    end

  context_ "#alive_cart" do
      let(:user) { create(:user) }

  test "returns an alive cart" do
        cart = create(:cart, user:)
        expect(user.alive_cart).to eq cart
      end

  test "does not return a deleted cart" do
        create(:cart, user:, deleted_at: Time.current)
        expect(user.alive_cart).to be_nil
      end
    end

  context_ "#update_audience_members_affiliates" do
  test "changing email updates members records" do
  ...
  context_ "#purchasing_power_parity_excluded_product_external_ids" do
      before do
        @user = create(:user)
        @product = create(:product, user: @user)
  ...
  test "returns the excluded product external_ids" do
  ...
  context_ "#update_purchasing_power_parity_excluded_products!" do
      before do
        @user = create(:user)
        @product_1 = create(:product, user: @user)
  ...
  test "sets purchasing_power_parity_disabled to true for the passed products and false for excluded products" do
  ...
  test "updates PPP flag on a published bundle with no bundle products" do
  ...
  context_ "#eligible_for_service_products?" do
      let(:user) { create(:user) }

  context_ "user is at least 30 days old" do
        before do
  ...
  context_ "user is less than 30 days old" do
  test "returns false" do
  ...
  context_ "#eligible_for_instant_payouts?" do
  ...
  test "returns false when user is not compliant" do
        [:not_reviewed,
         :on_probation,
         :flagged_for_fraud,
  ...
  test "returns false when payouts are paused" do
        allow(user).to receive(:payouts_paused?).and_return(true)
        expect(user.eligible_for_instant_payouts?).to eq(false)
      end

  test "returns false when user does not have 4 completed payments" do
        user.payments.last.destroy
        expect(user.eligible_for_instant_payouts?).to eq(false)
      end

  test "returns false when user is not from the US" do
  ...
  context_ "#instant_payouts_supported?" do
      let(:user) { create(:user) }
  ...
  test "returns false when user has no active bank account" do
        allow(user).to receive(:active_bank_account).and_return(nil)
        expect(user.instant_payouts_supported?).to eq(false)
      end

  test "returns false when bank account does not support instant payouts" do
        allow(bank_account).to receive(:supports_instant_payouts?).and_return(false)
        expect(user.instant_payouts_supported?).to eq(false)
      end

  test "returns false when user is not eligible for instant payouts" do
        allow(user).to receive(:eligible_for_instant_payouts?).and_return(false)
        expect(user.instant_payouts_supported?).to eq(false)
      end

  test "returns true if user is eligible for instant payouts and their bank account supports them" do
        expect(user.instant_payouts_supported?).to eq(true)
      end
    end

  context_ "#payouts_paused?" do
      let(:user) { create(:user) }

  test "returns true when payouts are paused internally" do
        user.payouts_paused_internally = true
        user.payouts_paused_by_user = false
        expect(user.payouts_paused?).to eq(true)
      end

  test "returns true when payouts are paused by user" do
        user.payouts_paused_internally = false
        user.payouts_paused_by_user = true
        expect(user.payouts_paused?).to eq(true)
      end

  test "returns true when payouts are paused both internally and by user" do
        user.payouts_paused_internally = true
        user.payouts_paused_by_user = true
        expect(user.payouts_paused?).to eq(true)
      end

  test "returns false when payouts are not paused" do
        user.payouts_paused_internally = false
        user.payouts_paused_by_user = false
  ...
  context_ "#payouts_paused_by_source" do
      let!(:seller) { create(:user) }

  test "returns source as admin if payouts are paused internally by an admin" do
  ...
  test "returns source as stripe if payouts are paused internally by stripe" do
  ...
  test "returns source as system if payouts are automatically paused internally" do
  ...
  test "returns source as user if payouts are paused by seller" do
  ...
  test "returns source as admin if payouts are paused by seller as well as admin" do
        seller.update!(payouts_paused_internally: true, payouts_paused_by_user: true, payouts_paused_by: User.last.id)
        expect(seller.payouts_paused_by_source).to eq(User::PAYOUT_PAUSE_SOURCE_ADMIN)
      end

  test "returns source as stripe if payouts are paused by seller as well as stripe" do
  ...
  test "returns source as system if payouts are paused by seller as well as system" do
  ...
  context_ "#payouts_paused_for_reason" do
      let!(:seller) { create(:user) }

  test "returns nil if payouts are not paused internally" do
        expect(seller.payouts_paused_for_reason).to be nil
      end

  test "returns nil if payouts are paused by admin but there are no corresponding comments" do
  ...
  test "returns the content of the last comment of payouts_paused type if payouts are paused by admin" do
        seller.update!(payouts_paused_internally: true, payouts_paused_by: User.last.id)
  ...
  test "returns nil if payouts are not paused by admin" do
        seller.comments.create!(
          author_id: User.last.id,
          content: "Chargeback rate too high.",
          comment_type: Comment::COMMENT_TYPE_PAYOUTS_PAUSED
  ...
  context_ "#minimum_payout_amount_cents" do
      let(:user) { create(:user) }

  test "returns the payout threshold" do
        user.payout_threshold_cents = 20_000
        expect(user.minimum_payout_amount_cents).to eq(20_000)
      end

  context_ "when the user is in a cross-border payout country" do
        let(:user) { create(:user) }
        let!(:compliance_info) { create(:user_compliance_info_korea, user:) }

  test "returns the higher of payout threshold and country minimum" do
  ...
  context_ "#made_a_successful_sale_with_a_stripe_connect_or_paypal_connect_account?" do
      let(:user) { create(:user) }
      let!(:stripe_connect_account) { create(:merchant_account_stripe_connect, user:) }

  context_ "when the user has made a successful sale with a Stripe Connect account" do
        before do
          create(:purchase, seller: user, link: create(:product, user:), merchant_account: stripe_connect_account)
        end

  context_ "when the Stripe Connect account is alive" do
  test "returns true" do
            expect(user.made_a_successful_sale_with_a_stripe_connect_or_paypal_connect_account?).to eq(true)
          end
        end

  context_ "when the Stripe Connect account has been deleted" do
          before do
            stripe_connect_account.mark_deleted!
          end
  ...
  context_ "when the user has not made a successful sale with a Stripe Connect account" do
        before do
  ...
  context_ "when the user has no Stripe Connect account" do
  test "returns false" do
  ...
  context_ "when the user has made a successful sale with a PayPal Connect account" do
  ...
  context_ "when the PayPal Connect account is alive" do
  test "returns true" do
            expect(user.made_a_successful_sale_with_a_stripe_connect_or_paypal_connect_account?).to eq(true)
          end
        end

  context_ "when the PayPal Connect account has been deleted" do
          before do
  ...
  context_ "when the user has not made a successful sale with a PayPal Connect account" do
        let!(:paypal_connect_account) { create(:merchant_account_paypal, user:) }
  ...
  context_ "when the user has no PayPal Connect account" do
        let!(:paypal_connect_account) { create(:merchant_account_paypal, user:) }
  ...
  context_ "#eligible_for_abandoned_cart_workflows?" do
      let(:user) { create(:user) }

  context_ "when user has a Stripe Connect account" do
        let!(:stripe_connect_account) { create(:merchant_account_stripe_connect, user:) }

  test "returns false if the Stripe Connect account has been deleted and there were no linked successful sales" do
          stripe_connect_account.mark_deleted!

          expect(user.eligible_for_abandoned_cart_workflows?).to eq(false)
        end

  test "returns true if the Stripe Connect account has been deleted and there was at least one successful sale with that Stripe Connect account" do
          create(:purchase, seller: user, link: create(:product, user:), merchant_account: stripe_connect_account)
          stripe_connect_account.mark_deleted!
  ...
  context_ "when user has a PayPal Connect account" do
        let!(:paypal_connect_account) { create(:merchant_account_paypal, user:) }

  test "returns false if the PayPal Connect account has been deleted and there were no linked successful sales" do
          paypal_connect_account.mark_deleted!

          expect(user.eligible_for_abandoned_cart_workflows?).to eq(false)
        end

  test "returns true if the PayPal Connect account has been deleted and there was at least one successful sale with that PayPal Connect account" do
          create(:purchase, seller: user, link: create(:product, user:), merchant_account: paypal_connect_account)
          paypal_connect_account.mark_deleted!
  ...
  test "returns true when user has completed payments" do
        create(:payment_completed, user:)
        expect(user.eligible_for_abandoned_cart_workflows?).to eq(true)
      end

  test "returns false when user has no Stripe Connect account and no completed payments" do
        expect(user.eligible_for_abandoned_cart_workflows?).to eq(false)
      end
    end

  context_ "#eligible_to_send_emails?" do
      let(:user) { create(:user) }
  ...
  context_ "when user is not a team member" do
  test "returns true when user has a completed payment and has made minimum required sales" do
          create(:payment_completed, user:)
  ...
  test "returns false when user has no completed payment" do
          allow(user).to receive(:sales_cents_total).and_return(Installment::MINIMUM_SALES_CENTS_VALUE)
          expect(user.eligible_to_send_emails?).to eq(false)
        end

  test "returns false when user has not made minimum required sales" do
          create(:payment_completed, user:)
  ...
  context_ "#has_all_eligible_refund_policies_as_no_refunds?" do
      let(:seller) { create(:named_seller) }
  ...
  context_ "when all refund policies are no-refunds" do
        before do
  ...
  context_ "when some refund policies are not no-refunds" do
  test "returns false" do
          expect(seller.has_all_eligible_refund_policies_as_no_refunds?).to be false
        end
      end

  context_ "when user has no refund policies" do
        before do
  ...
  context_ "#accessible_communities_ids" do
      let!(:user) { create(:user) }
      let!(:product) { create(:product, user:) }
      let!(:other_product) { create(:product) }

  context_ "when user is a seller" do
        let!(:community) { create(:community, seller: user, resource: product) }

  test "includes communities owned by the seller" do
  ...
  test "excludes communities where the resource is deleted" do
          Feature.activate_user(:communities, user)
          product.update!(community_chat_enabled: true)
  ...
  test "excludes communities when feature flag is disabled" do
          Feature.deactivate_user(:communities, user)
          product.update!(community_chat_enabled: true)
          expect(user.accessible_communities_ids).to eq([])
        end

  test "excludes communities when community chat is disabled" do
          Feature.activate_user(:communities, user)
  ...
  context_ "when user is a buyer" do
  ...
  test "includes communities of purchased products" do
  ...
  context_ "when purchase is made with email" do
  ...
  context_ "when user is both seller and buyer" do
        let!(:community) { create(:community, seller: user, resource: product) }
        let!(:other_community) { create(:community, seller: other_product.user, resource: other_product) }
        let!(:purchase) { create(:purchase, purchaser: user, link: other_product) }

  test "includes both seller and buyer communities" do
          Feature.activate_user(:communities, user)
          Feature.activate_user(:communities, other_product.user)
          product.update!(community_chat_enabled: true)
          other_product.update!(community_chat_enabled: true)
  ...
  test "excludes communities where feature flag is disabled" do
          Feature.deactivate_user(:communities, user)
          Feature.deactivate_user(:communities, other_product.user)
          product.update!(community_chat_enabled: true)
          other_product.update!(community_chat_enabled: true)
  ...
  test "excludes communities where community chat is disabled" do
          Feature.activate_user(:communities, user)
          Feature.activate_user(:communities, other_product.user)
          product.update!(community_chat_enabled: false)
          other_product.update!(community_chat_enabled: false)
  ...
  context_ "#purchased_small_bets?" do
      let(:user) { create(:user) }
  ...
  test "returns true if the user has purchased the small bets product" do
  ...
  context_ "#eligible_for_ai_product_generation?" do
      let(:user) { create(:user) }
  ...
  test "returns true when user made successful sale with Stripe Connect account" do
        stripe_connect_account = create(:merchant_account_stripe_connect, user:)
        create(:purchase, seller: user, link: create(:product, user:), merchant_account: stripe_connect_account)
        expect(user.eligible_for_ai_product_generation?).to eq(true)
      end

  test "returns true when user made successful sale with PayPal Connect account" do
        paypal_connect_account = create(:merchant_account_paypal, user:)
        create(:purchase, seller: user, link: create(:product, user:), merchant_account: paypal_connect_account)
        expect(user.eligible_for_ai_product_generation?).to eq(true)
      end

  test "returns false when user has no completed payments or successful sales" do
        expect(user.eligible_for_ai_product_generation?).to eq(false)
      end

  test "returns false when feature flag is inactive" do
  ...
  test "returns false when user has insufficient sales" do
        allow(user).to receive(:sales_cents_total).and_return(5_000)
        create(:payment_completed, user:)
        expect(user.eligible_for_ai_product_generation?).to eq(false)
      end

  test "returns true regardless of other conditions in development environment" do
  ...
  context_ ".id?" do
  context_ "with valid numeric values" do
  test "returns true for integer 1" do
  ...
  test "returns true for large numeric string" do
          expect(User.id?("7269625173515")).to be(true)
        end

  test "returns true for large integer" do
          expect(User.id?(7269625173515)).to be(true)
        end
      end

  context_ "with invalid non-numeric values" do
  test "returns false for string '1gum'" do
          expect(User.id?("1gum")).to be(false)
        end

  test "returns false for string with special characters" do
          expect(User.id?("1test@gumroad.com")).to be(false)
        end
      end

  context_ "with blank values" do
