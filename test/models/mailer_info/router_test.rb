...
class MailerInfoRouterTest < ActiveSupport::TestCase
  self.described_class = MailerInfo::Router



  context_ MailerInfo::Router do
    let(:domain) { :gumroad }
    let(:date) { Date.current }

  context_ ".determine_email_provider" do
  test "raises error for invalid domain" do
        expect do
          described_class.determine_email_provider(:invalid)
        end.to raise_error(ArgumentError, "Invalid domain: invalid")
      end

  test "returns SendGrid in test environment" do
        expect(described_class.determine_email_provider(domain)).to eq(MailerInfo::EMAIL_PROVIDER_SENDGRID)
      end

  context_ "when resend feature is active" do
        before do
          allow(Rails.env).to receive(:test?).and_return(false)
          Feature.activate(:resend)
        end

  context_ "without counts" do
  test "returns SendGrid" do
            expect(described_class.determine_email_provider(domain)).to eq(MailerInfo::EMAIL_PROVIDER_SENDGRID)
          end
        end

  test "returns SendGrid when max count is reached" do
  ...
  test "returns Resend based on probability" do
  ...
  test "returns SendGrid based on probability" do
  ...
  test "increments counter when choosing Resend" do
          described_class.set_probability(domain, date, 1.0)
          described_class.set_max_count(domain, date, 100)
          $redis.set(described_class.send(:current_count_key, domain, date:), 0)
          allow(Kernel).to receive(:rand).and_return(0.5)
  ...
  test "sets probability in Redis" do
  ...
  test "sets max count in Redis" do
        described_class.set_max_count(domain, date, 100)
  ...
  test "returns stats for domain" do
        described_class.set_probability(domain, date, 0.5)
        described_class.set_max_count(domain, date, 100)
  ...
  context_ ".stats" do
  test "returns stats for all domains" do
