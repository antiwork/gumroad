...
class MailerInfoHeaderBuilderTest < ActiveSupport::TestCase
  self.described_class = MailerInfo::HeaderBuilder



  context_ MailerInfo::HeaderBuilder do
    let(:mailer_class) { "CustomerMailer" }
    let(:mailer_method) { "test_email" }
    let(:mailer_args) { ["test@example.com"] }

  context_ ".perform" do
  test "delegates to instance" do
  ...
  context_ "#build_for_sendgrid" do
      let(:builder) do
        described_class.new(
          mailer_class:,
          mailer_method:,
  ...
  context_ "with preorder receipt email" do
  ...
  test "includes authorization purchase id" do
          headers = builder.build_for_sendgrid
          smtpapi = JSON.parse(headers[MailerInfo::SENDGRID_X_SMTPAPI_HEADER])
  ...
  context_ "with abandoned cart email" do
  ...
  test "raises error with unexpected args" do
          expect do
            described_class.new(
              mailer_class:,
              mailer_method:,
  ...
  context_ "#truncated_mailer_args" do
      let(:builder) do
        described_class.new(
          mailer_class:,
          mailer_method:,
  ...
  test "truncates string arguments to 20 chars" do
