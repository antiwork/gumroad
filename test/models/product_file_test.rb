...
class ProductFileTest < ActiveSupport::TestCase
  self.described_class = ProductFile



  context_ ProductFile do
  context_ ".archivable" do
  test "only includes archivable files" do
  ...
  context_ "#has_alive_duplicate_files?" do
  ...
  test "returns true if there's an alive record with the same url" do
        file_1.mark_deleted
        expect(file_1.has_alive_duplicate_files?).to eq(true)
        expect(file_2.has_alive_duplicate_files?).to eq(true)
      end

  test "returns false if there's no other alive record with the same url" do
        file_1.mark_deleted
        file_2.mark_deleted
  ...
  context_ "#can_send_to_kindle?" do
  context_ "when file size is above the limit" do
  ...
  context_ "when the file size is nil" do
  test "returns false" do
  ...
  context_ "when the file format is not supported by kindle" do
  test "returns false" do
  ...
  context_ "when the file format is supported by kindle" do
  ...
  test "returns true for epub files" do
  ...
  context_ "#must_be_pdf_stamped?" do
  test "returns false for a non-pdf file" do
        product_file = create(:non_readable_document)

        expect(product_file.must_be_pdf_stamped?).to eq(false)
      end

  test "returns false for a pdf file with pdf stamping disabled" do
        product_file = create(:readable_document, pdf_stamp_enabled: false)

        expect(product_file.must_be_pdf_stamped?).to eq(false)
      end

  test "returns false for a pdf file with pdf stamping enabled" do
  ...
  context_ "#archivable?" do
  test "returns true only for archivable files" do
        archivable = [
          create(:readable_document),
  ...
  context_ "#stream_only?" do
  test "returns false for non-video file" do
        product_file = create(:readable_document)

        expect(product_file.stream_only?).to eq(false)
      end

  test "returns false for non-streamable video file" do
        product_file = create(:non_streamable_video)

        expect(product_file.stream_only?).to eq(false)
      end

  test "returns false for streamable video file not marked as stream_only" do
        product_file = create(:streamable_video)

        expect(product_file.stream_only?).to eq(false)
      end

  test "returns false for non-video file marked as stream_only" do
        product_file = create(:readable_document, stream_only: true)

        expect(product_file.stream_only?).to eq(false)
      end

  test "returns true for streamable video file marked as stream_only" do
  ...
  context_ "file group and analyze" do
  test "enqueues analyze after file creation" do
        product_file = create(:readable_document)
  ...
  test "preserves s3 key for files containing percent and ampersand in filename" do
  ...
  test "saves subtitle files correctly" do
  ...
  test "deletes all subtitle files correctly" do
        product_file = create(:product_file)
  ...
  test "handles edits to subtitle files correctly" do
        product_file = create(:product_file)
        subtitle_file = create(:subtitle_file, product_file:)
        product_file.subtitle_files.append(subtitle_file)
  ...
  context_ "invalid file urls" do
  test "does not create files with invalid urls" do
          invalid_product_file = build(:product_file, url: "undefined")
          expect(invalid_product_file).not_to be_valid
          invalid_product_file.save
          expect(invalid_product_file.errors.full_messages.first).to eq "Please provide a valid file URL."
        end

  test "does not create files with invalid external links" do
  ...
  context_ "renaming" do
        before do
          @product_file = create(:readable_document)
        end

  test "schedules renaming of S3 file if display_name updated" do
          @product_file.update!(description: "chapter one of the book")
          expect(RenameProductFileWorker).not_to have_enqueued_sidekiq_job(@product_file.id)
  ...
  test "does not schedule rename in S3 if display_name updated for an external link file" do
          product_file = create(:product_file, filetype: "link", url: "https://gumroad.com")
          product_file.update!(display_name: "trillion-dollar-company")
          expect(RenameProductFileWorker).not_to have_enqueued_sidekiq_job(product_file.id)
        end

  test "renames files and preserves the Content-Type", :sidekiq_inline do
  ...
  test "renames a file to a long name", :sidekiq_inline do
          new_name = "A" * 250
  ...
  test "creates the product file with filetype set to lowercase" do
  ...
  context_ "product file for installments" do
  test "schedules analyze and not schedule PdfUnstampableNotifierJob if the file belongs to an installment" do
  ...
  context_ "#transcodable?" do
      let(:product_file) do
        create(:streamable_video, width: 2550, height: 1750, bitrate: 869_480)
      end

  test "returns `false` if the width is not set" do
        product_file.update!(width: nil)

        expect(product_file.transcodable?).to be(false)
      end

  test "returns `false` if the height is not set" do
        product_file.update!(height: nil)

        expect(product_file.transcodable?).to be(false)
      end

  test "returns `false` if the file is not a video file" do
        expect(product_file).to receive(:streamable?).and_return(false)

        expect(product_file.transcodable?).to be(false)
      end

  test "returns `true` if the dimensions are set for a video file" do
        expect(product_file.transcodable?).to be(true)
      end
    end

  context_ "#transcoding_in_progress?" do
  test "returns true if there's a transcoded video in progress" do
  ...
  context_ "#transcoding_failed" do
  ...
  context_ "#attempt_to_transcode?" do
      let(:product_file) do
        create(:streamable_video, width: 2550, height: 1750, bitrate: 869_480)
      end

  test "returns `false` if there are transcoding jobs in the 'processing' state" do
  ...
  test "with allowed_when_processing=true returns `true` if there are transcoding jobs in the 'processing' state" do
        create(:transcoded_video, streamable: product_file, original_video_key: product_file.s3_key, state: "processing")

        expect(product_file.attempt_to_transcode?(allowed_when_processing: true)).to be(true)
      end

  test "returns `false` if there are transcoding jobs in the 'completed' state" do
        create(:transcoded_video, streamable: product_file, original_video_key: product_file.s3_key, state: "completed")

        expect(product_file.attempt_to_transcode?).to be(false)
      end

  test "returns `true` if the there are no 'processing' or 'completed' transcoding jobs" do
        expect(product_file.attempt_to_transcode?).to be(true)
      end
    end

  context_ "s3 properties" do
      before do
  ...
  test "handles / in the filename properly" do
  ...
  test "works as expected for files without an extension" do
  ...
  context_ "#delete!" do
  test "deletes all associated subtitle files for an mp4 file" do
  ...
  test "escapes the user-provided filename even if the user has changed the filename since the video was transcoded" do
  ...
  test "escapes the filename in legacy S3 attachments" do
  ...
  test "returns nil when the S3 playlist object does not exist" do
        s3_object = double("s3_object")
  ...
  test "escapes the newlines in the filename" do
  ...
  context_ "#subtitle_files_for_mobile" do
      let(:product_file) { create(:product_file) }

  context_ "when there are no alive subtitle files associated" do
  test "returns an empty array" do
          expect(product_file.subtitle_files_for_mobile).to eq([])
        end
      end

  context_ "when associated alive subtitle files exist" do
  ...
  test "returns url and language for all the files" do
          expected_result = [
  ...
  test "returns a truthy value when the CDN URL is in a specific format" do
        product_file = create(:product_file)

        expect(product_file.has_cdn_url?).to be_truthy
      end

  test "returns a falsey value when the CDN URL is not in the regular Gumroad format" do
  ...
  context_ "#has_valid_external_link?" do
  test "returns truthy value when url is valid" do
  ...
  test "returns falsey value when url is invalid" do
  ...
  context_ "#external_link?" do
  test "returns true for files which are external links" do
        product_file = create(:product_file, filetype: "link", url: "http://gumroad.com")
        expect(product_file.external_link?).to eq(true)
      end

  test "returns false for non-external link files" do
        product_file = create(:readable_document)
        expect(product_file.external_link?).to eq(false)
      end
    end

  context_ "#display_extension" do
  test "returns URL for files which are external links" do
        product_file = create(:product_file, filetype: "link", url: "http://gumroad.com")
        expect(product_file.display_extension).to eq("URL")
      end

  test "returns extension for s3 files with an extension" do
        product_file = create(:product_file, url: "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/files/43a5363194e74e9ee75b6203eaea6705/original/black/white.mp4")
        expect(product_file.display_extension).to eq("MP4")
      end

  test "returns empty string for s3 files without an extension" do
  ...
  context_ "#queue_for_transcoding" do
  test "returns `false` when both `streamable?` and `analyze_completed?` are `false`" do
        product_file = create(:product_file)
  ...
  test "returns `false` when `streamable?` is `false` and `analyze_completed?` is `true`" do
  ...
  test "returns `true` when both `streamable?` and `analyze_completed?` are `true`" do
        product_file = create(:product_file, analyze_completed: true)
  ...
  test "returns `false` when `streamable?` is `true` and `analyze_completed?` is `false`" do
        product_file = create(:product_file)
        product_file.update_columns(filegroup: "video") # Need to bypass callbacks otherwise the value is overwritten
  ...
  test "returns original file as tempfile" do
        product_file = create(:readable_document)
        yielded = false
  ...
  context_ "latest_media_location for different file types" do
  test "returns latest media location for readable" do
  ...
  test "returns latest media location for streamable" do
  ...
  test "returns latest media location for listenable" do
  ...
  test "returns the location with latest timestamp if multiple media_locations exist" do
        consumption_timestamp = Time.current.change(usec: 0)
  ...
  test "marks invalid if the attached thumbnail is not an image" do
          product_file = create(:streamable_video)
  ...
  test "marks invalid if the attached thumbnail is too large" do
          product_file = create(:streamable_video)
  ...
  test "is valid if the attached thumbnail is an image with size smaller than 5 MB" do
          product_file = create(:streamable_video)
  ...
  test "returns nil if no thumbnail exists" do
          product_file = create(:readable_document)
  ...
  test "returns the CDN URL if thumbnail exists" do
  ...
  test "returns the original thumbnail URL when the thumbnail variant processing fails" do
          product_file = create(:streamable_video)
          product_file.thumbnail.attach(io: File.open(Rails.root.join("spec", "support", "fixtures", "smilie.png")), filename: "smilie.png")
  ...
  context_ "#thumbnail_variant" do
  test "returns the resized thumbnail variant" do
          product_file = create(:streamable_video)
          product_file.thumbnail.attach(io: File.open(Rails.root.join("spec", "support", "fixtures", "smilie.png")), filename: "smilie.png")
  ...
  context_ "isbn" do
  context_ "normalization" do
  test "converts em and en dashes to ASCII dash" do
          file = create(:pdf_product_file, isbn: "978–3—16−1484100")
          expect(file.isbn).to eq("978-3-16-1484100")
        end

  test "converts whitespaces to ASCII dash" do
          file = create(:pdf_product_file, isbn: "978 3 16 1484100")
          expect(file.isbn).to eq("978-3-16-1484100")
        end

  test "upcases the string" do
          file = create(:pdf_product_file, isbn: "978316148410x")
          expect(file.isbn).to eq("978316148410X")
        end

  test "strips whitespace from the string" do
          file = create(:pdf_product_file, isbn: "  978316148410X  ")
          expect(file.isbn).to eq("978316148410X")
        end

  test "nullifies isbn if blank or only whitespace" do
  ...
  context_ "when pdf" do
  test "is valid with a valid isbn-13" do
            product_file = build(:pdf_product_file, isbn: Faker::Code.isbn(base: 13))
            expect(product_file).to be_valid
          end

  test "is valid with a valid isbn-10" do
            product_file = build(:pdf_product_file, isbn: Faker::Code.isbn)
            expect(product_file).to be_valid
          end

  test "is invalid with an invalid isbn" do
  ...
  test "is valid with a nil isbn" do
            product_file = build(:pdf_product_file, isbn: nil)
            expect(product_file).to be_valid
          end
        end

  context_ "when epub" do
  test "is valid with a valid isbn-13" do
  ...
  context_ "when unrelated file type" do
  test "doesn't allow isbn" do
  ...
  context_ "scopes" do
  context_ ".in_order" do
  test "returns files in ascending order of positions" do
  ...
  test "returns files in order of ids" do
  ...
  context_ ".pdf" do
  test "filters out non-pdf files" do
          create(:external_link)
          create(:listenable_audio)
          create(:streamable_video)
          create(:non_readable_document)
  ...
  context_ ".not_external_link" do
  test "filters out external link files" do
          create(:product_file, filetype: "link", url: "https://www.gumroad.com")
          create(:product_file, filetype: "link", url: "https://www.twitter.com")
  ...
  context_ "#cannot_be_stamped?" do
      let(:product_file) { create(:pdf_product_file) }

  context_ "when stampable_pdf is nil" do
  test "returns false" do
          expect(product_file.cannot_be_stamped?).to be(false)
        end
      end

  context_ "when stampable_pdf is false" do
  ...
  context_ "when stampable_pdf is true" do
  ...
  context_ "#stamp_existing_pdfs_if_needed" do
  ...
  context_ "when PDF stamping is newly enabled" do
  test "enqueues a job to stamp existing PDFs if needed" do
            file.update!(pdf_stamp_enabled: true)
            expect(StampPdfForPurchaseJob).to have_enqueued_sidekiq_job(purchase.id)
          end
        end

  context_ "when PDF stamping is newly disabled" do
          let(:file) { create(:pdf_product_file, pdf_stamp_enabled: true) }

  test "does not enqueue a job to stamp existing PDFs" do
  ...
  context_ "#signed_url" do
      let(:product_file) { create(:product_file) }

  test "returns nil when the S3 object does not exist" do
  ...
  test "returns the url for external links" do
  ...
  context_ "subtitle file S3 NotFound handling" do
      let(:product_file) { create(:product_file) }
      let(:english_srt_url) { "#{AWS_S3_ENDPOINT}/#{S3_BUCKET}/attachment/english.srt" }
  ...
  context_ "#as_json" do
  test "excludes subtitle files whose S3 object does not exist" do
  ...
  context_ "#subtitle_files_urls" do
  test "excludes subtitle files whose S3 object does not exist" do
