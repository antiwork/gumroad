...
class InstallmentTest < ActiveSupport::TestCase
  self.described_class = Installment
  ...
  context_ "scopes" do
  context_ ".visible_on_profile" do
  test "returns only non-workflow audience type published posts that are shown on profile" do
  ...
  test "returns false if post has no files" do
        expect(@installment.is_downloadable?).to eq(false)
      end

  test "returns false if post has only stream-only files" do
        @installment.product_files << create(:streamable_video, stream_only: true)

        expect(@installment.is_downloadable?).to eq(false)
      end

  test "returns true if post has files that are not stream-only" do
  ...
  context_ "#send_installment_from_workflow_for_member_cancellation" do
      before do
  ...
  test "sends a subscription_cancellation_installment email for the cancellation" do
  ...
  test "does not send an email for non member cancellation installments" do
  ...
  test "does not send an email for alive subscriptions" do
  ...
  test "does not send an email if sale's can_contact is set to false" do
  ...
  test "does not send an email if sale is chargebacked" do
  ...
  test "does not send an email if the customer has already received a cancellation email for this installment from the creator" do
  ...
  test "does not send an email if the workflow does not apply to the purchase" do
        creator = create(:user)
  ...
  test "returns message with inline syntax highlighting the code snippets" do
  ...
  test "renders both regular and discounted upsell cards" do
  ...
  context_ "with media embeds" do
  test "replaces media embed iframe with a link to the media thumbnail" do
  ...
  test "replaces media embed iframe with a link to the media's title if thumbnail is missing" do
  ...
  context_ "#message_with_inline_abandoned_cart_products" do
  ...
  context_ "when products are missing" do
  test "returns the message as it is" do
  ...
  test "returns the message with the products" do
          ...
          expect(message).not_to include("Product 4")
          expect(message).not_to include("<product-list-placeholder />")
  ...
  context_ "when a custom checkout_url is provided" do
  test "returns the message with the custom checkout_url" do
  ...
  context_ "#generate_url_redirect_for_subscription" do
      before do
        @subscription = create(:subscription, link: @installment.link)
      end

  test "creates a new url_redirect" do
  ...
  context_ "#url_redirect" do
      before do
        @subscription = create(:subscription, link: @installment.link)
        @installment.generate_url_redirect_for_subscription(@subscription)
      end

  test "returns correct url_redirect" do
  ...
  context_ "#follower_or_audience_url_redirect" do
  test "returns a url_redirect without an associated purchase object" do
        post = create(:post)
  ...
  test "returns the purchase url redirect if it cannot find a url redirect using the subscription" do
        expect(@installment.download_url(@subscription, @purchase)).to eq @purchase_url_redirect.url.sub("/r/", "/d/")
      end

  test "creates a new url redirect if none exists for installment with files" do
        user = create(:user)
  ...
  test "creates a new url redirect if none exists for installment with files even for installments with send_emails=false" do
  ...
  test "does not return a download url if the installment is going to a follower and has no files" do
  ...
  test "invalidates the cache and read the updated value" do
  ...
  context_ "#displayed_name" do
  test "returns the installment name" do
        @installment.update_attribute(:name, "welcome")
        expect(@installment.displayed_name).to eq("welcome")
      end

  test "returns the message as the name without html tags" do
  ...
  context_ "#eligible_purchase_for_user" do
  test "returns the user's purchase that passes filters for the post and hence is eligible to view the attached content" do
        creator = create(:user)
        buyer = create(:user)
  ...
  context_ "#targeted_at_purchased_item?" do
      let(:variant) { create(:variant) }
      let(:product) { variant.link }
      let(:purchase) { create(:purchase, link: product, variant_attributes: [variant]) }

  context_ "product installment" do
  test "returns true if it is targeted at the purchased product" do
          post = build(:installment, link: product)
          expect(post.targeted_at_purchased_item?(purchase)).to eq true
        end

  test "returns false if it is not targeted at the purchased product" do
          post = build(:installment)
          expect(post.targeted_at_purchased_item?(purchase)).to eq false
        end
      end

  context_ "variant installment" do
  test "returns true if it is targeted at the purchased variant" do
          post = build(:variant_installment, base_variant: variant)
          expect(post.targeted_at_purchased_item?(purchase)).to eq true
        end

  test "returns false if it is not targeted at the purchased variant" do
          post = build(:variant_installment)
          expect(post.targeted_at_purchased_item?(purchase)).to eq false
        end
      end

  context_ "other installment" do
  test "returns true if bought_products includes the purchased product" do
  ...
  test "returns true if bought_variants includes the purchased variant" do
  ...
  test "returns false if neither bought_products nor bought_variants are present" do
  ...
  test "returns false if bought_products does not include the purchased product" do
  ...
  test "returns false if bought_variants does not include the purchased variant" do
  ...
  context_ "#passes_member_cancellation_checks?" do
      before do
  ...
  context_ "when purchase_id is present" do
  test "returns the subdomain URL of the post" do
  ...
  context_ "when purchase_id is not present" do
  test "returns the subdomain URL of the post" do
          target_url = Rails.application.routes.url_helpers.custom_domain_view_post_url(
            host: @post.user.subdomain_with_protocol,
            slug: @post.slug
  ...
  context_ "#eligible_purchase?" do
  context_ "when purchase is nil" do
  ...
  context_ "when installment does not need purchase to access content" do
  ...
  context_ "when purchased product is a post's product" do
  ...
  context_ "when purchased product is not a post's product" do
  ...
  context_ "when installment is a variant post" do
        let(:product) { create(:product) }
  ...
  context_ "when purchased product's creator is a post's creator" do
          let(:purchase) { create(:purchase, link: product, created_at: 1.second.ago) }
  ...
  context_ "when purchased product's creator is not a post's creator" do
  ...
  context_ "when installment is a follower post" do
        let(:creator) { create(:user) }
        let(:product) { create(:product, user: creator) }
  ...
  context_ "when installment is an affiliate post" do
        let(:creator) { create(:user) }
  ...
  context_ "#publish!" do
  test "publishes the installment" do
        expect do
          @installment.publish!
        end.to change { @installment.published_at }.from(nil).to(be_within(1.second).of(Time.current))
      end

  test "sets the 'published_at' to the optionally provided 'published_at' argument" do
  ...
  test "sets 'workflow_installment_published_once_already' to true if it's a workflow installment" do
        expect do
          @installment.publish!
  ...
  test "raises an Installment::InstallmentInvalid error" do
          expect do
            @installment.publish!
  ...
  context_ "content moderation" do
  ...
  context_ "when editing an already-published post" do
          before do
            allow(ContentModeration::ModerateRecordService).to receive(:check).with(@installment, :post).and_return(
              ContentModeration::ModerateRecordService::CheckResult.new(passed: true, reasons: [])
            )
  ...
  test "re-checks moderation when the message changes" do
            expect(ContentModeration::ModerateRecordService).to receive(:check).with(@installment, :post).and_return(
  ...
  context_ "when editing a draft post" do
  test "does not run moderation on name/message edits" do
            expect(ContentModeration::ModerateRecordService).not_to receive(:check)
  ...
  context_ "#is_affiliate_product_post?" do
      before do
        @affiliate_installment = create(:affiliate_installment)
      end

  test "returns false when it is not an affiliate installment" do
        expect(@installment.is_affiliate_product_post?).to eq(false)
      end

  test "returns false when it does not have exactly one affiliate product" do
        expect(@affiliate_installment.is_affiliate_product_post?).to eq(false)
        @affiliate_installment.update!(affiliate_products: ["p1", "p2"])
        expect(@affiliate_installment.is_affiliate_product_post?).to eq(false)
      end

  test "returns true when it is an affiliate installment that has exactly one affiliate product" do
        @affiliate_installment.update!(affiliate_products: ["p"])
        expect(@affiliate_installment.is_affiliate_product_post?).to eq(true)
      end
    end

  context_ "#affiliate_product_name" do
  test "returns nil when it is not an affiliate installment" do
        expect(@installment.affiliate_product_name).to eq(nil)
      end

  test "returns the associated affiliate product's name" do
  ...
  context_ "#audience_members_filter_params" do
      # the actual filtering is tested in audience_member_spec.rb
  test "converts post filters into AudienceMember filters" do
  ...
  context_ "#audience_members_count" do
  test "returns the number of audience members" do
  ...
  context_ "#send_preview_email" do
      let(:recipient) { create(:user) }

  test "raises an error if the recipient has not confirmed their email address" do
  ...
  test "sends an email to the recipient" do
        expect(PostSendgridApi).to receive(:process).with(
          post: @post,
  ...
  test "creates a UrlRedirect and sends an email to the recipient when the post has files" do
  ...
  context_ "when post has 'abandoned_cart' type" do
        before { @post.update!(installment_type: "abandoned_cart") }

  test "sends abandoned cart preview email" do
          expect do
            @post.send_preview_email(recipient)
  ...
  context_ "#can_be_blasted?" do
  test "returns true when send_emails = true and no blasts already exist" do
  ...
  context_ ".receivable_by_customers_of_product" do
  test "returns posts that the customers of a given product would receive" do
        product = create(:product)
  ...
  context_ "#featured_image_url" do
      let(:installment) { create(:installment) }

  test "returns nil when message is blank" do
        installment.message = ""
  ...
  test "only returns the first element's image src if it's a figure" do
  ...
  test "returns empty array when message is blank" do
        installment.message = ""
  ...
  test "only returns tags from the last element if it's a paragraph" do
        installment.message = <<~HTML
          <p>First paragraph</p>
          <p>#tag1 #tag2 #tag3</p>
        HTML
  ...
  test "returns tags when all words in the last paragraph start with #" do
        installment.message = <<~HTML
  ...
  context_ "#message_snippet" do
      let(:installment) { create(:installment) }

  test "returns empty string when message is blank" do
        installment.message = ""
  ...
  test "strips HTML tags from message" do
  ...
  context_ "when installment is not a workflow installment" do
  ...
  context_ "when subscription has not been resubscribed" do
  test "returns true" do
          expect(installment.delivery_due?(purchase)).to be true
        end
      end

  context_ "when subscription has been resubscribed" do
  ...
  context_ "when delivery time has passed" do
          before do
  ...
  context_ "when delivery time has not passed" do
          before do
  ...
  context_ "when purchase has no subscription" do
