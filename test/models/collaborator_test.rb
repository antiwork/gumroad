...
class CollaboratorTest < ActiveSupport::TestCase
  self.described_class = Collaborator
  ...
  context_ "affiliate_basis_points presence" do
  test "requires affiliate_basis_points to be present if apply_to_all_products is set" do
  ...
  test "does not require affiliate_basis_points to be present if apply_to_all_products is not set" do
          collaborator = build(:collaborator, apply_to_all_products: false, affiliate_basis_points: nil)
          expect(collaborator).to be_valid
        end
      end

  context_ "collaborator_does_not_require_approval" do
  context_ "when affiliate_user has changed" do
          let(:collaborator) { build(:collaborator, affiliate_user: create(:user, require_collab_request_approval: true)) }

  test "requires the affiliate user to allow collaborator requests without approval" do
  ...
  context_ "when affiliate_user has not changed" do
          let(:collaborator) { create(:collaborator) }
          before { collaborator.affiliate_user.update!(require_collab_request_approval: true) }

  test "does not require the affiliate user to allow collaborator requests without approval" do
  ...
  context_ "scopes" do
  context_ ".invitation_accepted and .invitation_pending" do
  test "returns collaborators without invitations" do
  ...
  context_ "#invitation_accepted?" do
  test "returns true when the collaborator has no invitation" do
  ...
  context_ "#as_json" do
  test "returns a hash of attributes" do
  ...
  test "includes the invitation_accepted status" do
        collaborator = create(:collaborator)
  ...
  test "disables the is_collab flag on all associated products" do
  ...
  test "returns the collaborator's basis points" do
          expect(collaborator.basis_points).to eq 10_00
        end
      end

  context_ "when product_id is provided" do
  context_ "and the product affiliate commission is set" do
  ...
  context_ "#show_as_co_creator_for_product?" do
  ...
  test "returns true if dont_show_as_co_creator is false, false otherwise" do
  ...
  test "returns true if the product_affiliate's dont_show_as_co_creator is false, false otherwise" do
          expect(collaborator.show_as_co_creator_for_product?(product)).to eq true
