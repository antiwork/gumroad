...
class UpsellTest < ActiveSupport::TestCase
  self.described_class = Upsell



  context_ Upsell do
  context_ "validations" do
  context_ "when any products don't belong to the seller" do
        before do
  ...
  context_ "when all products belong to the seller" do
        before do
          @seller = create(:user)
  ...
  context_ "when the offered product doesn't belong to the seller" do
        before do
  ...
  context_ "when the offered product belongs to the seller" do
        before do
          @seller = create(:user)
  ...
  context_ "when the offered product is a call" do
        before do
  ...
  context_ "when the offered variant doesn't belong to the the offered product" do
        before do
  ...
  context_ "when the offered variant belongs to the offered product" do
        before do
          @seller = create(:user)
  ...
  context_ "when the offer code doesn't belong to the seller and the offered product" do
        before do
  ...
  context_ "when the offer code belongs to the seller and the offered product" do
        before do
          @seller = create(:user)
          @product = create(:product, user: @seller)
  ...
  context_ "when there is more than one upsell variant per selected variant" do
        before do
          @seller = create(:user)
          @product = create(:product, user: @seller)
  ...
  context_ "when there is only one upsell variant per selected variant" do
        before do
          @seller = create(:user)
  ...
  context_ "when there is already an upsell for the product" do
        before do
          @seller = create(:user)
          @product = create(:product, user: @seller)
  ...
  context_ "when `deleted_at` is set for the upsell" do
          before do
  ...
  context_ "when there isn't an upsell for the product" do
        before do
          @seller = create(:user)
          @product = create(:product, user: @seller)
  ...
  test "returns the upsell encoded in an object" do
  ...
  test "includes configured discounts for existing-customer offer codes" do
