...
class SellerProfileTest < ActiveSupport::TestCase
  self.described_class = SellerProfile



  context_ SellerProfile do
  context_ "#custom_styles" do
      subject { create(:seller_profile, highlight_color: "#009a49", font: "Roboto Mono", background_color: "#000000") }

  test "has CSS for background color, accent color, and font" do
  ...
  test "rebuilds CSS when custom style attribute is saved" do
  ...
  context_ "#font_family" do
      subject { create(:seller_profile) }

  test "returns the active font, then ABC Favorit and a generic fallback" do
        expect(subject.font_family).to eq(%("ABC Favorit", "ABC Favorit", sans-serif))
      end

  test "returns a serif fallback for a serif font" do
        subject.update!(font: "Domine")
        expect(subject.font_family).to eq(%("Domine", "ABC Favorit", serif))
      end

  test "returns a monospace fallback for a monospace font" do
