...
class CreditTest < ActiveSupport::TestCase
  self.described_class = Credit



  context_ Credit do
  context_ "create_for_credit!" do
      let(:user) { create(:user) }
      let(:merchant_account) { create(:merchant_account, user:) }

  test "assigns to the Gumroad Stripe merchant account" do
  ...
  test "updates the unpaid_balance_cents for users after creation" do
  ...
  test "creates a balance record after credit" do
        credit = Credit.create_for_credit!(user:, amount_cents: 1000, crediting_user: User.first)
        expect(credit.balance).to eq(Balance.last)
      end

  test "creates a comment after credit" do
  ...
  test "applies the credit to the oldest unpaid balance" do
  ...
  test "applies the credit to an unpaid balance for the Gumroad Stripe merchant account" do
        credit = Credit.create_for_credit!(user:, amount_cents: 1000, crediting_user: User.first)
        expect(credit.balance.merchant_account).to eq(MerchantAccount.gumroad(StripeChargeProcessor.charge_processor_id))
      end
    end

  context_ "create_for_dispute_won!" do
      let(:user) { create(:user) }
      let(:merchant_account) { create(:merchant_account, user:) }
  ...
  test "assigns the purchase of the dispute as the chargebacked purchase" do
  ...
  test "applies the credit to an unpaid balance for the creator's merchant account" do
        credit = Credit.create_for_dispute_won!(user:, merchant_account:, dispute:,
                                                chargedback_purchase: purchase,
                                                balance_transaction_issued_amount:,
                                                balance_transaction_holding_amount:)
  ...
  context_ "create_for_returned_payment_difference!" do
      let(:user) { create(:user) }
      let(:merchant_account) { create(:merchant_account, user:) }
  ...
  context_ "when the difference is positive" do
        let(:difference_amount_cents) { 1_00 }

  test "is for a zero amount" do
          expect(credit.amount_cents).to eq(0)
        end

  test "assigns the returned payment" do
          expect(credit.returned_payment).to eq(returned_payment)
        end

  test "creates a balance transaction with a zero issued amount" do
  ...
  test "creates a balance transaction with the difference as the holding amount" do
  ...
  test "does not update the balances amount_cents" do
  ...
  test "updates the balances holding_amount_cents" do
          balance = create(:balance, user:, merchant_account:, date: 10.days.ago)
          expect { credit }.to change { balance.reload.holding_amount_cents }
          expect(credit.balance).to eq(balance)
        end

  test "does not update the unpaid_balance_cents for users after creation" do
  ...
  test "creates a command after credit with the returned payment id" do
          credit
          user.reload
  ...
  test "applies the credit to the unpaid balance for the creator's merchant account" do
  ...
  context_ "when the difference is negative" do
        let(:difference_amount_cents) { -1_00 }
  ...
  context_ "create_for_refund_fee_retention!", :vcr do
  ...
  test "assigns the refund as fee_retention_refund" do
  ...
  test "updates the unpaid_balance_cents for the seller" do
        expect(Stripe::Transfer).to receive(:create).and_call_original
  ...
  test "applies the credit to the oldest unpaid balance and not the unpaid balance from purchase date" do
  ...
  context_ "Gumroad-controlled non-US Stripe account sales" do
  test "applies the credit to the oldest unpaid balance and not the unpaid balance from purchase date" do
  ...
  context_ "when the Gumroad tax and affiliate credit are present" do
  test "returns a positive credit for the affiliate commission and taxes" do
  ...
  context_ "when the Gumroad tax and affiliate credit are not present" do
  ...
  context_ "Stripe Connect sales" do
        let(:purchase) do
  ...
  test "does not create a comment after credit" do
        expect do
  ...
  context_ "create_for_balance_change_on_stripe_account!" do
      let!(:creator) { create(:user) }
