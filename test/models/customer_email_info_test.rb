...
class CustomerEmailInfoTest < ActiveSupport::TestCase
  self.described_class = CustomerEmailInfo



  context_ CustomerEmailInfo do
  context_ ".find_or_initialize_for_charge" do
      let(:purchase) { create(:purchase) }
      let(:charge) { create(:charge, purchases: [purchase]) }

  context_ "when the record doesn't exist" do
  test "initializes a new record" do
  ...
  test "finds the existing record" do
          email_info = CustomerEmailInfo.find_or_initialize_for_charge(
            charge_id: charge.id,
            email_name: SendgridEventInfo::RECEIPT_MAILER_METHOD
          )
  ...
  context_ ".find_or_initialize_for_purchase" do
      let(:purchase) { create(:purchase) }
  ...
  context_ "#mark_bounced!" do
  test "attempts to unsubscribe the buyer of the purchase" do
        email_info = create(:customer_email_info_delivered)
