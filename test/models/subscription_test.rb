...
class SubscriptionTest < ActiveSupport::TestCase
  self.described_class = Subscription
  self.rspec_metadata = { vcr: true }



  context_ Subscription, :vcr do
  ...
  context_ "#latest_plan_change" do
  test "returns the most recent, live plan change" do
  ...
  context_ "#latest_applicable_plan_change" do
  test "returns the most recent, live plan change that is applicable" do
  ...
  context_ "create_interruption_event", :freeze_time do
  test "records a deactivated event if deactivated_at is set and was previously blank" do
  ...
  test "records a restarted event if deactivated_at is cleared" do
          @subscription.update!(deactivated_at: Time.current)
          expect do
  ...
  test "does nothing if deactivated_at has not changed" do
          expect do
  ...
  context_ "send_ended_notification_webhook" do
  test "sends a 'subscription_ended' notification if the subscription has just been deactivated" do
          @subscription.update!(deactivated_at: Time.current)
  ...
  test "does not send a 'subscription_ended' notification if the subscription was already deactivated" do
          @subscription.update!(deactivated_at: Time.current)
  ...
  test "does not send a 'subscription_ended' notification if the subscription is not deactivated" do
  ...
  context_ "scopes" do
  context_ ".active_without_pending_cancel" do
  ...
  context_ "when subscription is a test" do
          before do
  ...
  context_ "when subscription has failed" do
          before do
  ...
  context_ "when subscription was cancelled" do
          before do
  ...
  context_ "#as_json" do
  test "returns the expected JSON representation" do
        expected = {
  ...
  test "excludes 'not_charged' plan change purchases" do
  ...
  test "excludes failed purchases" do
  ...
  test "includes license_key for membership products with licensing enabled" do
  ...
  context_ "#subscription_mobile_json_data" do
      before do
  ...
  test "returns nothing if the subscription is no longer alive" do
        @subscription.cancel_effective_immediately!
        expect(@subscription.subscription_mobile_json_data).to eq nil
      end

  test "returns the correct json format for the mobile api" do
  ...
  test "includes the first installment for new subscribers if the creator set should_include_last_post to true" do
  ...
  context_ "#credit_card_to_charge" do
  context_ "when test subscription" do
  test "returns nil" do
          user = create(:user, credit_card: create(:credit_card))
  ...
  context_ "when guest subscription purchase" do
  test "returns the credit card used with the original purchase" do
          user = create(:user)
          product = create(:subscription_product, user:)
  ...
  context_ "when user has a card saved on file and doesn't have a card in the purchase" do
  test "returns the card saved on file, not the card used during purchase" do
  ...
  context_ "when user has a card associated to the subscription" do
  test "returns the subscription's card" do
          buyers_card = create(:credit_card)
  ...
  test "returns the installments made after subscription created, plus the last one made before the subscription if link option is set" do
  ...
  test "returns the installments made after subscription created, plus the last one made before the subscription if link option is set, ordered with published_at date" do
        @product.update_attribute(:should_include_last_post, true)
  ...
  test "returns the installments made after subscription created without the last one made before the subscription if link option is not set" do
        expect(@subscription.installments).to eq [@new_installment]
      end

  test "does not include unpublished installments" do
        expect(@subscription.installments).not_to include @unpublished_installment
      end

  test "does not include any installment older than the last installment before the creation of the subscription" do
        expect(@subscription.installments).not_to include @very_old_installment
      end

  context_ "cancelled subscriptions" do
        before do
  ...
  test "returns installment created while subscription active, plus the last installment before the subscription was created" do
          expect(@subscription.installments).to eq [@correct_installment]
        end

  test "does not include any installment older than the last installment before the creation of the subscription if link option is set" do
          @product.update_attribute(:should_include_last_post, true)
          expect(@subscription.installments).not_to include @very_old_installment
          expect(@subscription.installments).to include @old_installment
        end

  test "does not include any past installments if link option is not set" do
          expect(@subscription.installments).not_to include @old_installment
        end

  test "does not return installments created after subscription cancelled" do
          expect(@subscription.installments).not_to include @current_installment
        end
      end

  context_ "failed subscriptions" do
        before do
          @product = create(:subscription_product, user: create(:user), is_recurring_billing: true)
          @user = create(:user, credit_card: create(:credit_card))
  ...
  test "returns installment created while subscription active, plus the last installment before the subscription was created if link option is set" do
          @product.update_attribute(:should_include_last_post, true)
          expect(@subscription.installments).to eq [@old_installment, @correct_installment]
        end

  test "returns only the installment created while subscription active if link option is not set" do
          expect(@subscription.installments).to eq [@correct_installment]
        end
  ...
  test "does not return installment created in the month that subscription failed" do
          expect(@subscription.installments).not_to include @end_of_month_cancelled
        end

  test "does not return installments created after subscription failed" do
          expect(@subscription.installments).not_to include @current_installment
        end
      end

  context_ "workflow installments" do
        before do
          @product = create(:subscription_product, user: create(:user), is_recurring_billing: true)
          @user = create(:user, credit_card: create(:credit_card))
  ...
  test "does not include any workflow installment" do
  ...
  context_ "#charge!" do
  test "uses the authenticated buyer when resolving charge discounts" do
  ...
  test "creates a new purchase row", :vcr do
        expect do
          @subscription.charge!
        end.to change { Purchase.count }.by(1)
      end

  test "gives new purchase right attributes", :vcr do
  ...
  test "charges stripe", :vcr do
        @subscription.charge!
      end

  test "creates a purchase event", :vcr do
  ...
  test "uses the previously saved payment instrument to charge an unregistered user's subscription" do
  ...
  test "uses the previously saved payment instrument to charge a registered user's subscription", :vcr do
        user = create(:user)
        discover_cc = CreditCard.create(build(:chargeable, card: StripePaymentMethodHelper.success_discover))
  ...
  test "uses the payment instrument attached to the subscription in case the purchaser account does not have a saved payment instrument", :vcr do
        user = create(:user)
  ...
  context_ "with an Indian credit card", :vcr do
  ...
  context_ "with a successful mandate" do
  ...
  test "uses the mandate associated with the saved credit card to successfully charge" do
            expect { subscription.charge! }.to change { Purchase.count }.by(1)
  ...
  context_ "with a cancelled mandate" do
  ...
  test "uses the mandate associated with the saved credit card and fails" do
            expect { subscription.charge! }.to change { Purchase.count }.by(1)
  ...
  test "uses the payment instrument attached to the subscription in case the purchaser account's saved payment instrument is not supported by this creator", :vcr do
        user = create(:user)
  ...
  test "transfers VAT ID and elected tax country from the original purchase to recurring charge" do
  ...
  test "transfers VAT ID from the original purchase's tax refund to recurring charge" do
        create(:zip_tax_rate, country: "IT", zip_code: nil, state: nil, combined_rate: 0.22, is_seller_responsible: false)
  ...
  test "transfers VAT ID from subscription's stored business_vat_id to recurring charge" do
        create(:zip_tax_rate, country: "IT", zip_code: nil, state: nil, combined_rate: 0.22, is_seller_responsible: false)
  ...
  test "transfers VAT ID from a recurring charge's VAT refund to subsequent recurring charges" do
        create(:zip_tax_rate, country: "IT", zip_code: nil, state: nil, combined_rate: 0.22, is_seller_responsible: false)
  ...
  context_ "handling of unexpected errors", :vcr do
  ...
  context_ "when a generic Stripeerror occurs" do
  test "does not leave the purchase in in_progress state" do
            expect do
  ...
  context_ "physical subscription" do
        before do
  ...
  test "charges the price of the subscription and shipping" do
          expect { @subscription.charge! }.to change { Purchase.count }.by(1)
  ...
  test "copies shipping information over to new purchase" do
          expect { @subscription.charge! }.to change { Purchase.count }.by(1)
          purchase = Purchase.last
          expect(purchase.purchase_state).to eq "successful"
          expect(purchase.subscription).to eq @subscription
  ...
  context_ "limited quantites" do
          before do
            @physical_link.update(max_purchase_count: 5)
          end

  test "does not reduce the number available" do
            expect { @subscription.charge! }.not_to change { @physical_link.reload.remaining_for_sale_count }
          end

  context_ "multi quantity purchase" do
            before do
  ...
  test "charges the correct amounts" do
  ...
  test "does not reduce the number available" do
              expect { @double_subscription.charge! }.not_to change { @physical_link.reload.remaining_for_sale_count }
            end
          end
        end
      end

  context_ "limited quantities" do
  context_ "limited quantity" do
          before do
  ...
  test "does not reduce the number available", :vcr do
            expect { @subscription.charge! }.not_to change { @product.reload.remaining_for_sale_count }
          end
        end

  context_ "changing variants" do
          before do
            @product = create(:subscription_product, user: create(:user))
  ...
  test "allows the recurring charge to go through regardless of variant changes", :vcr do
  ...
  context_ "limited variant quantity" do
          before do
            @product = create(:subscription_product, user: create(:user))
            @variant_category = create(:variant_category, link: @product, title: "colors")
  ...
  test "does not reduce the amount available", :vcr do
            expect { @subscription.charge! }.not_to change { @variant.reload.quantity_left }
          end

  context_ "no variants left" do
            before do
  ...
  context_ "new purchase" do
  test "does not allow extra purchases to go through" do
                @purchase = create(:purchase, link: @product, price_cents: @product.price_cents, is_original_subscription_purchase: true,
  ...
  test "allows recurring charges to go through and create new purchase row", :vcr do
              expect { @subscription.charge! }.to change {
                Purchase.count
              }.by(1)
            end

  test "makes the new purchase row successful", :vcr do
              @subscription.charge!
              expect(Purchase.last.purchase_state).to eq "successful"
            end
          end
  ...
  context_ "variable priced products" do
          before do
  ...
  test "sets the price of the purchase row correctly", :vcr do
            purchase = @subscription.charge!
            expect(purchase.subscription).to eq @subscription
  ...
  context_ "limited offer code quantity" do
  context_ "offer codes still available" do
            before do
              @product = create(:subscription_product, user: create(:user))
  ...
  test "does not reduce the amount available", :vcr do
              expect { @subscription.charge! }.not_to change { @offer_code.reload.is_valid_for_purchase? }
            end
          end

  context_ "last offer code available" do
            before do
              @product = create(:membership_product, user: create(:user))
  ...
  context_ "discount with duration" do
  context_ "tiered membership" do
  ...
  context_ "when the discount is no longer valid" do
  test "charges the full price" do
              subscription.charge!
  ...
  context_ "legacy subscription" do
          let(:user) { create(:user) }
  ...
  test "charges user at the same amount that they originally subscribed in", :vcr do
          # change the currency
          ...
          travel_to(1.month.from_now) do
            ...
            expect(purchase.price_cents).not_to eq @purchase.price_cents
          end
        end
      end

  context_ "price changes" do
        before do
  ...
  test "charges the user the original amount", :vcr do
  ...
  context_ "with foreign currency" do
          before do
            @product = create(:subscription_product, user: create(:user), price_currency_type: "jpy", price_cents: 400)
            @subscription = create(:subscription, user: create(:user, credit_card: create(:credit_card)), link: @product)
            @purchase = create(:purchase, link: @product, email: @subscription.user.email, price_cents: get_usd_cents("jpy", @product.price_cents),
  ...
  test "charges the user the original amount in the foreign currency", :vcr do
            @product.update(price_cents: 500)
  ...
  test "does not requeue 1 hour job", :freeze_time, :vcr do
            allow(ChargeProcessor).to receive(:create_payment_intent_or_charge!).and_raise(ChargeProcessorUnavailableError.new)
            @subscription.charge!(from_failed_charge_email: true)

            expect(RecurringChargeWorker).not_to have_enqueued_sidekiq_job(@subscription.id)
          end
        end

  context_ "user removed credit card" do
          before do
            @subscription.user.update!(credit_card_id: nil)
          end

  test "sends charge declined emails" do
            mail_double = double
  ...
  context_ "when there are no successful, non-refunded or reversed purchases" do
  test "schedules the UnsubscribeAndFail job" do
  ...
  context_ "card error" do
  context_ "card_declined" do
          before do
            allow(ChargeProcessor).to receive(:create_payment_intent_or_charge!).and_raise(ChargeProcessorCardError.new("card_declined"))
          end

  test "sends the correct email", :vcr do
            mail_double = double
            allow(mail_double).to receive(:deliver_later)
            expect(CustomerLowPriorityMailer).to receive(:subscription_card_declined).and_return(mail_double)
            @subscription.charge!
  ...
  test "schedules ChargeDeclinedReminderWorker", :freeze_time, :vcr do
            @subscription.charge!
  ...
  test "requeues UnsubscribeAndFail", :freeze_time, :vcr do
              @subscription.charge!
  ...
  context_ "card_declined_insufficient_funds" do
          before do
  ...
  test "schedules a ChargeDeclinedReminderWorker" do
            @subscription.charge!

            expect(ChargeDeclinedReminderWorker).to have_enqueued_sidekiq_job(@subscription.id)
          end

  test "requeues RecurringCharge", :freeze_time do
            @subscription.charge!
  ...
  test "associates the recurring charges to the same affiliate", :vcr do
  ...
  test "does not associate the recurring charge to the affiliate if affiliate is using a Brazilian Stripe Connect account", :vcr do
          @product.update!(price_cents: 10_00)
          affiliate_user = create(:affiliate_user)
          direct_affiliate = create(:direct_affiliate, affiliate_user:, seller: @product.user, affiliate_basis_points: 1000, products: [@product])
          subscription = create(:subscription, user: create(:user, credit_card: create(:credit_card)), link: @product)
  ...
  context_ "recommended" do
        before do
          allow_any_instance_of(Link).to receive(:recommendable?).and_return(true)
        end

  test "shows the recurring charges as recommended, charge the extra fee, and create a new recommended_purchase_info", :vcr do
          @product.update!(price_cents: 10_00)
          subscription = create(:subscription, user: create(:user, credit_card: create(:credit_card)), link: @product)
          purchase = create(:purchase_in_progress, link: @product, email: subscription.user.email, full_name: "squiddy",
                                                   price_cents: @product.price_cents, is_original_subscription_purchase: true,
  ...
  context_ "discover fee" do
  test "charges the discover fee percentage from the original purchase instead of the current product discover fee" do
  ...
  context_ "free trials" do
  test "charges the discover fee percentage from the original free trial purchase instead of the current product discover fee" do
  ...
  context_ "free trial ratings" do
  test "allows free trial subscriptions' ratings to be counted on successful charge" do
          purchase = create(:free_trial_membership_purchase)
  ...
  context_ "#schedule_charge", :freeze_time do
      before do
        @scheduled_time = Time.current + 1.day
      end

  test "schedules RecurringCharge at the specified time" do
  ...
  test "logs the scheduling operation" do
  ...
  context_ "#unsubscribe_and_fail!" do
  ...
  test "does not set cancelled_by_buyer" do
        expect(@subscription.cancelled_by_buyer).to be(false)
        @subscription.unsubscribe_and_fail!
        expect(@subscription.cancelled_by_buyer).to be(false)
      end

  context_ "when creator has payment notifications ON" do
  ...
  context_ "when creator has payment notifications OFF" do
  ...
  context_ "when subscription has previously failed" do
  context_ "within the last 7 days" do
  test "sends email to customer but not creator on repeated recent failure" do
            create(:purchase,
  ...
  context_ "more than 7 days ago" do
  test "sends email to customer and creator on new failure" do
            create(:purchase,
                   link: @subscription.link,
                   subscription: @subscription,
                   email: @subscription.user.email,
  ...
  test "emails the customer" do
        expect do
          @subscription.unsubscribe_and_fail!
        end.to have_enqueued_mail(CustomerLowPriorityMailer, :subscription_autocancelled).with(@subscription.id)
      end

  test "enqueues the ping job to notify seller of subscription cancellation" do
        @subscription.unsubscribe_and_fail!
  ...
  test "enqueues the ping job to notify seller of subscription ending" do
        @subscription.unsubscribe_and_fail!
  ...
  context_ "#end_subscription!" do
  test "ends the subscription" do
  ...
  context_ "#cancel!" do
  context_ "by_seller=false" do
  test "sets cancelled_at and user_requested_cancellation", :freeze_time do
  ...
  context_ "by_admin=true" do
  test "sets the cancelled_by_admin correctly" do
  ...
  test "enqueues deactivate integrations worker" do
        @subscription.deactivate!
        expect(DeactivateIntegrationsWorker).to have_enqueued_sidekiq_job(@subscription.original_purchase.id)
      end

  test "creates a subscription_event of type deactivated" do
        @subscription.deactivate!
        expect(@subscription.subscription_events.last.event_type).to eq("deactivated")
      end

  context_ "when creator has member cancellation workflow jobs" do
  context_ "and the membership has been cancelled" do
  test "schedules a member cancellation installment for a creator's seller workflow" do
  ...
  test "schedules a member cancellation installment for a creator's product workflow" do
  ...
  test "does not schedule a member cancellation installment for workflows that are not product or seller workflows, even if their trigger is member cancellation" do
  ...
  test "does not schedule a member cancellation installment if the seller workflow isn't for member cancellation" do
  ...
  test "does not schedule a member cancellation installment if the product workflow isn't for member cancellation" do
  ...
  test "does not schedule a member cancellation installment if the workflow doesn't apply to the purchase" do
  ...
  test "does not schedule a member cancellation installment if the installment rule is nil" do
            workflow = create(:workflow, seller: @creator, link: @product, workflow_trigger: "member_cancellation")
  ...
  context_ "and the membership has ended for other reasons" do
  ...
  context_ "such as payment failures" do
  test "does not schedule member cancellation workflow jobs" do
  ...
  context_ "such as reaching the end of its fixed-length duration" do
  test "does not schedule member cancellation workflow jobs" do
  ...
  context_ "#update_current_plan!" do
  test "archives the existing original purchase" do
        setup_subscription
  ...
  test "creates a new original purchase with the updated tier, price, and quantity" do
        setup_subscription
  ...
  test "copies correct attributes from the original purchase" do
  ...
  test "creates a purchase event for the new original purchase" do
        setup_subscription
  ...
  test "does not charge the user" do
        setup_subscription
  ...
  test "caches the buyer-specific amount when applying a tiered existing-customer discount",
  ...
  test "does not use the subscription owner to auto-discover discounts for unauthenticated updates",
         vcr: { cassette_name: "Subscription/_update_current_plan_/creates_a_new_original_purchase_with_the_updated_tier_price_and_quantity" } do
        setup_subscription
  ...
  test "does not copy an exhausted original discount onto a different auto-discovered tiered discount",
         vcr: { cassette_name: "Subscription/_update_current_plan_/creates_a_new_original_purchase_with_the_updated_tier_price_and_quantity" } do
        setup_subscription
  ...
  test "keeps a re-resolved tiered discount when clear_discount is true",
         vcr: { cassette_name: "Subscription/_update_current_plan_/creates_a_new_original_purchase_with_the_updated_tier_price_and_quantity" } do
        setup_subscription
  ...
  test "keeps a re-resolved non-tiered discount when clear_discount is true",
         vcr: { cassette_name: "Subscription/_update_current_plan_/creates_a_new_original_purchase_with_the_updated_tier_price_and_quantity" } do
        setup_subscription
        original_offer_code = create(:offer_code,
                                     code: "singlecycle",
  ...
  test "does not use the subscription owner to qualify unauthenticated discount updates",
         vcr: { cassette_name: "Subscription/_update_current_plan_/creates_a_new_original_purchase_with_the_updated_tier_price_and_quantity" } do
        setup_subscription
        ownership_product = create(:product, user: @product.user)
  ...
  test "does not update the creator's balance" do
        setup_subscription
  ...
  context_ "updating to a PWYW tier" do
        before :each do
          setup_subscription
          @new_tier.update!(customizable_price: true)
        end

  test "calculates displayed_price_cents correctly" do
  ...
  context_ "when skip_preparing_for_charge is true" do
  test "does not call Stripe or perform any chargeable-related operations" do
          setup_subscription
  ...
  context_ "when applying a quantity change" do
        before do
          setup_subscription
        end

  test "updates the purchase quantity and price" do
  ...
  context_ "when applying a plan change" do
        before :each do
          setup_subscription
        end

  test "uses that price as the new price even if product price is higher" do
  ...
  test "uses that price as the new price even if product price is lower" do
  ...
  context_ "and free trial is enabled" do
  test "does not require the new 'original purchase' to be marked a free trial purchase" do
  ...
  context_ "but product is no longer for sale" do
  test "still allows the plan to be changed" do
  ...
  context_ "when purchase has a license" do
  test "associates the license with the new original_purchase" do
          setup_subscription
  ...
  context_ "when purchase was recommended" do
  test "charges the discover fee percentage from the original purchase instead of the current product discover fee" do
          setup_subscription(was_product_recommended: true, discover_fee_per_thousand: 300)
          @product.update!(discover_fee_per_thousand: 400)
          allow_any_instance_of(Subscription).to receive(:mor_fee_applicable?).and_return(false)
  ...
  context_ "when purchase has sent emails" do
  test "associates the emails with the new original_purchase" do
          setup_subscription
  ...
  context_ "when comments are associated with the purchase" do
  test "updates the comments with the new original_purchase" do
          setup_subscription
  ...
  context_ "when purchase has a URL redirect" do
  test "creates a URL redirect for the new original_purchase" do
  ...
  context_ "for test subscription" do
  test "marks the new original purchase 'test_successful'" do
          setup_subscription
  ...
  context_ "when the original purchase has an offer code discount with duration_in_months" do
        before do
          setup_subscription
  ...
  test "copies duration_in_months to the new original purchase's discount" do
  ...
  test "preserves nil duration_in_months for unlimited discounts" do
          @original_purchase.create_purchase_offer_code_discount!(
            offer_code: @offer_code,
            offer_code_amount: 25,
            offer_code_is_percent: true,
  ...
  test "uses current offer code values when offer_code is provided" do
  ...
  test "sets a new offer code on the new purchase when offer_code is a different code" do
          @original_purchase.create_purchase_offer_code_discount!(
            offer_code: @offer_code,
            offer_code_amount: 25,
            offer_code_is_percent: true,
  ...
  test "builds a discount for a new offer code when original had no discount" do
  ...
  test "clears the offer code and discount when clear_discount is true" do
          @original_purchase.create_purchase_offer_code_discount!(
            offer_code: @offer_code,
            offer_code_amount: 25,
            offer_code_is_percent: true,
  ...
  test "keeps a deleted offer code without applying its discount when clear_deleted_discount is true",
  ...
  test "keeps a deleted tiered offer code discount when clear_deleted_discount is true",
           vcr: { cassette_name: "Subscription/_update_current_plan_/when_the_original_purchase_has_an_offer_code_discount_with_duration_in_months/clears_the_offer_code_and_discount_when_clear_discount_is_true" } do
  ...
  test "does not clear an auto-discovered replacement discount when clear_deleted_discount is true",
           vcr: { cassette_name: "Subscription/_update_current_plan_/when_the_original_purchase_has_an_offer_code_discount_with_duration_in_months/clears_the_offer_code_and_discount_when_clear_discount_is_true" } do
  ...
  context_ "for a subscription with fixed length" do
  test "raises an error" do
          setup_subscription
  ...
  context_ "for installment plans" do
        let(:installment_plan_purchase) { create(:installment_plan_purchase) }
        let(:subscription) { installment_plan_purchase.subscription }
  ...
  context_ "last purchase state" do
  context_ "failed" do
        before do
  ...
  test "within time frame is true" do
          travel_to(Date.today + 4) do
  ...
  context_ "#cancel_effective_immediately!" do
  test "sends email and sets cancelled attributes", :freeze_time do
  ...
  test "does not send email but sets cancelled attributes if from chargeback", :freeze_time do
        expect(CustomerLowPriorityMailer).not_to receive(:subscription_product_deleted)
  ...
  context_ "#cancel_immediately_if_pending_cancellation!" do
  test "enqueues the ping job to notify seller of subscription ending" do
  ...
  test "returns true if the subscription is currently for that tier" do
        expect(subscription.for_tier?(tier_1))
      end

  test "returns true if the subscription is pending a change to that tier" do
  ...
  test "returns false if the subscription is not for that tier or pending a change to that tier" do
        expect(subscription.for_tier?(tier_2)).to eq false
      end
    end

  context_ "#pending_cancellation?" do
  test "returns true if the subscription is pending cancellation" do
        @subscription.cancel!
        expect(@subscription.pending_cancellation?).to be(true)
      end

  test "returns false if the subscription has already been cancelled" do
        @subscription.cancel_effective_immediately!
        expect(@subscription.pending_cancellation?).to be(false)
      end

  test "returns false if the subscription was deactivated for some other reason" do
        @subscription.unsubscribe_and_fail!
        expect(@subscription.pending_cancellation?).to be(false)
      end

  test "returns false for a live subscription" do
        expect(@subscription.pending_cancellation?).to be(false)
      end
    end

  context_ "#cancelled?" do
  test "returns true if the subscription has been cancelled" do
        @subscription.cancel_effective_immediately!
        expect(@subscription.cancelled?).to eq true
      end

  test "returns false if the subscription is pending cancellation" do
        @subscription.cancel!
        expect(@subscription.cancelled?).to eq false
      end

  test "returns true if the subscription is pending cancellation but flag to treat as cancelled is set" do
        @subscription.cancel!
  ...
  context_ "#deactivated?" do
  test "returns true if the subscription has been deactivated" do
        @subscription.deactivated_at = 1.day.ago
        expect(@subscription.deactivated?).to eq true
      end

  test "returns false if the subscription has not been deactivated" do
        @subscription.deactivated_at = nil
        expect(@subscription.deactivated?).to eq false
      end
    end

  context_ "#cancelled_by_seller?" do
  test "returns true for a subscription that was cancelled by the seller" do
  ...
  test "returns false for a subscription that was cancelled by the buyer" do
  ...
  test "returns false for a live subscription that is not pending cancellation" do
        subscription = build(:subscription)
        expect(subscription.cancelled_by_seller?).to eq false
      end

  test "returns false for a live subscription that is pending cancellation by buyer" do
        subscription = build(:subscription, cancelled_at: 1.week.from_now, cancelled_by_buyer: true)
        expect(subscription.cancelled_by_seller?).to eq false
      end

  test "returns true for a live subscription that is pending cancellation by seller" do
        subscription = build(:subscription, cancelled_at: 1.week.from_now, cancelled_by_buyer: false)
        expect(subscription.cancelled_by_seller?).to eq true
      end

  test "returns false for a failed subscription" do
        subscription = build(:subscription, failed_at: 1.day.ago)
        expect(subscription.cancelled_by_seller?).to eq false
      end

  test "returns false for an ended subscription" do
        subscription = build(:subscription, ended_at: 1.day.ago)
        expect(subscription.cancelled_by_seller?).to eq false
      end
    end

  context_ "#pending_failure?" do
  test "returns false for a subscription in free trial" do
        purchase = create(:free_trial_membership_purchase)
  ...
  test "returns true for a live subscription in grace period" do
  ...
  context_ "#status" do
  test "returns 'alive' for a live subscription" do
        subscription = build(:subscription)
        expect(subscription.status).to eq "alive"
      end

  test "returns 'pending_failure' for a subscription in a grace period" do
        subscription = create(:subscription)
        create(:purchase, subscription:, is_original_subscription_purchase: true, purchase_state: "successful")
        travel_to 1.month.from_now
        create(:purchase, subscription:, purchase_state: "failed")
  ...
  test "returns 'pending_cancellation' for a subscription pending cancellation" do
        subscription = create(:subscription, cancelled_at: 1.month.from_now)
        expect(subscription.status).to eq "pending_cancellation"
      end

  test "returns termination reason for a terminated subscription" do
  ...
  context_ "#end_time_of_subscription" do
  test "is in 1 month", :freeze_time do
  ...
  test "is in 3 months", :freeze_time do
  ...
  test "is in 6 months", :freeze_time do
  ...
  test "is in 1 year", :freeze_time do
  ...
  test "is in 2 years", :freeze_time do
  ...
  test "is the most recent ended time for test subscription", :freeze_time do
        product = create(:membership_product, subscription_duration: :quarterly)
  ...
  test "is Time.current for test subscription without succeeded_at set", :freeze_time do
        product = create(:membership_product, subscription_duration: :quarterly)
        subscription = create(:subscription, user: create(:user, credit_card: create(:credit_card)), link: product, is_test_subscription: true)
  ...
  test "is Time.current when there are no successful purchases", :freeze_time do
        product = create(:membership_product, subscription_duration: :quarterly)
        subscription = create(:subscription, user: create(:user, credit_card: create(:credit_card)), link: product)
  ...
  context_ "when the subscription has a free trial" do
        before do
          purchase = create(:free_trial_membership_purchase)
          @subscription = purchase.subscription
        end

  context_ "during the free trial" do
  test "returns the time the free trial ends" do
            expect(@subscription.end_time_of_subscription).to eq @subscription.free_trial_ends_at
          end
        end

  context_ "after the free trial" do
  test "returns the time the free trial ends" do
  ...
  context_ "refunds and chargebacks" do
  context_ "when the last purchase was refunded or chargedback" do
          let(:purchase) { create(:membership_purchase) }
          let(:subscription) { purchase.subscription }

  test "returns the current time if it is the only purchase", :freeze_time do
  ...
  test "returns the current time if the last paid period has lapsed" do
  ...
  context_ "but a prior purchase covers the current time" do
  test "returns the end time based on that prior purchase" do
              end_time = purchase.succeeded_at + subscription.period
              later_purchase = create(:purchase, subscription:, link: subscription.link, stripe_refunded: true)
              expect(subscription.end_time_of_subscription).to eq end_time
  ...
  context_ "#send_renewal_reminders?" do
  test "returns false when feature `membership_renewal_reminders` is disabled" do
        setup_subscription(recurrence: BasePrice::Recurrence::QUARTERLY)

        expect(@subscription.send_renewal_reminders?).to be(false)
      end

  test "returns true when feature `membership_renewal_reminders` is enabled" do
        setup_subscription(recurrence: BasePrice::Recurrence::QUARTERLY)
  ...
  context_ "#send_renewal_reminder_at" do
  context_ "when the subscription is monthly" do
  test "returns one day prior" do
  ...
  context_ "when the subscription is quarterly" do
  test "returns seven days prior" do
          setup_subscription(recurrence: BasePrice::Recurrence::QUARTERLY)
          travel_to(Time.current) do
            @original_purchase.update!(succeeded_at: Time.current)
  ...
  context_ "when the subscription is yearly" do
  test "returns seven days prior" do
  ...
  context_ "when the subscription is every two years" do
  test "returns seven days prior" do
  ...
  context_ "#end_time_of_last_paid_period" do
      before do
  ...
  test "returns the paid-through time of the most recent successful, not charged back, fully refunded, or deleted purchase" do
  ...
  test "returns the paid-through time of a partially refunded purchase if that is the most recent successful purchase" do
  ...
  test "returns the paid-through time of a chargedback purchase if that is the most recent successful purchase and the chargeback was reversed" do
  ...
  test "returns the free trial termination time if there are no successful charges" do
  ...
  context_ "#last_successful_charge_at" do
  context_ "when there are successful purchases" do
  test "returns the succeeded_at time of the most recent successful purchase" do
          subscription = create(:subscription)
  ...
  context_ "when there are successful test purchases" do
  test "returns the succeeded_at time of the most recent successful purchase" do
  ...
  context_ "#overdue_for_charge?" do
      before :each do
        @purchase = create(:membership_purchase)
        @subscription = @purchase.subscription
      end

  context_ "before the end of the subscription period" do
  test "returns false" do
          expect(@subscription.overdue_for_charge?).to eq false
        end
      end

  context_ "after the end of the subscription period" do
  ...
  context_ "for a subscription with a free trial" do
        before do
  ...
  context_ "#seconds_overdue_for_charge" do
      before :each do
        @purchase = create(:membership_purchase, succeeded_at: 1.hour.ago)
        @subscription = @purchase.subscription
      end

  test "returns 0 for currently active subscriptions" do
        expect(@subscription.seconds_overdue_for_charge).to eq 0
      end

  test "returns 0 for a subscription with no successful purchases" do
        @purchase.update!(purchase_state: "failed")
        expect(@subscription.seconds_overdue_for_charge).to eq 0
      end

  test "returns the seconds overdue for charge for a subscription overdue for a charge" do
  ...
  context_ "#has_a_charge_in_progress?" do
  test "returns true if there's an associated purchase in progress otherwise returns false" do
        purchase = create(:membership_purchase, succeeded_at: 1.hour.ago)
        subscription = purchase.subscription
  ...
  context_ "#prorated_discount_price_cents" do
      before :each do
  ...
  context_ "before the start of the subscription period" do
  test "returns the full price" do
          expect(
  ...
  context_ "halfway through the subscription period" do
  test "returns half the price" do
  ...
  context_ "when the month has less than 30 days" do
        before do
  ...
  context_ "after the end of the month" do
  test "returns 0" do
            expect(
  ...
  context_ "when a renewal cycle's price diverges from the signup cycle" do
  test "prorates against the most recent successful charge, not the signup-cycle price" do
  ...
  context_ "#current_billing_period_seconds" do
      let(:seconds_per_day) { 24 * 60 * 60 }

  context_ "for a monthly subscription" do
  test "returns the correct number of seconds in a 28 day month" do
  ...
  test "returns the correct number of seconds in a 29 day month" do
  ...
  test "returns the correct number of seconds in a 30 day month" do
  ...
  test "returns the correct number of seconds in a 31 day month" do
  ...
  context_ "for a quarterly subscription" do
        let(:product) do
  ...
  test "returns the correct number of seconds for a subscription starting in January" do
  ...
  test "returns the correct number of seconds for a subscription starting in June" do
  ...
  context_ "with no successful charges" do
  test "returns 0" do
          subscription = create(:subscription)
  ...
  context_ "during a free trial" do
  test "returns the duration of the free trial" do
  ...
  context_ "#termination_reason" do
      let(:terminated_at) { Date.new(2020, 1, 1) }

  test "returns the correct reason if subscription ended due to fixed period ending" do
  ...
  test "returns the correct reason if subscription was cancelled" do
  ...
  test "returns the correct reason if subscription was cancelled due to failed payments" do
  ...
  test "returns nil if the subscription does not have a termination time set" do
        subscription = build(:subscription)
        expect(subscription.termination_reason).to be_nil
      end
    end

  context_ "payment options" do
      before do
        @user = create(:user, credit_card: create(:credit_card))
      end

  context_ "for a non-tiered membership subscription" do
  test "has the proper payment option" do
  ...
  context_ "for a tiered membership subscription" do
  test "has the proper payment option" do
          product = create(:membership_product)
          subscription = create(:subscription, link: product, user: @user, created_at: 3.days.ago)
          create(:purchase, is_original_subscription_purchase: true, link: product, subscription:, purchaser: @user)
  ...
  context_ "#expected_completion_time", :freeze_time do
      let(:t0) { Time.utc(2023, 1, 1, 12, 0, 0) }

  test "returns nil for non fixed-length subscriptions" do
        purchase = create(:membership_purchase)
        subscription = purchase.subscription
  ...
  context_ "for fixed-length monthly memberships" do
  test "calculates end_of_last_paid_period + period * remaining when some charges completed" do
          travel_to(t0)
  ...
  test "considers free_trial_ends_at during the free trial" do
          travel_to(t0)
  ...
  test "equals end_of_last_paid_period when all required charges are completed" do
          travel_to(t0)
  ...
  context_ "#has_fixed_length?" do
  test "returns true if charge_occurrence_count is set" do
        subscription = build(:subscription, charge_occurrence_count: 1)
        expect(subscription.has_fixed_length?).to eq true
      end

  test "returns false if charge_occurrence_count is not set" do
        subscription = build(:subscription)
        expect(subscription.has_fixed_length?).to eq false
      end
    end

  context_ "#charges_completed?" do
      before do
        product = create(:membership_product)
        @subscription = create(:subscription, link: product)
  ...
  test "returns `true` when the required number of charges are processed" do
  ...
  test "returns `false` when the required number of charges are not processed" do
        @subscription.update_columns(charge_occurrence_count: 2)

        expect(@subscription.charges_completed?).to be(false)
      end

  test "returns `false` when the subscription has no set number of charges" do
        expect(@subscription.charges_completed?).to be(false)
      end
    end

  context_ "#price" do
  test "uses the last_payment_option_id column if it's not nil" do
  ...
  test "uses the 'payment_options.alive.last' query if last_payment_option is nil" do
        subscription = create(:subscription)
  ...
  context_ "#current_subscription_price_cents" do
  context_ "when the subscription doesn't have an offer code with a limited duration" do
  test "returns the original purchase displayed price" do
  ...
  context_ "when the subscription has an offer code with a limited duration" do
  ...
  context_ "when the discount's duration has elapsed" do
  test "returns the original purchase's displayed price before discount" do
            expect(@subscription.current_subscription_price_cents).to eq(1000)
          end
        end

  context_ "when the discount's duration has not elapsed" do
          before do
            @purchase.purchase_offer_code_discount.update!(duration_in_billing_cycles: 2)
          end

  test "returns the original purchase's displayed price" do
  ...
  context_ "with a discount" do
          let!(:offer_code) { create(:offer_code, products: [product], amount_cents: 100, duration_in_billing_cycles: 1) }
          let!(:purchase) { create(:installment_plan_purchase, subscription:, link: product, offer_code:) }

  test "applies to all installments even if it's only for one membership cycle" do
            expect(purchase.price_cents).to eq(300)
  ...
  context_ "#current_plan_displayed_price_cents" do
  context_ "non-tiered memberships" do
  test "returns the original purchase displayed price" do
          @subscription.original_purchase.update!(displayed_price_cents: 1234)
  ...
  test "returns the tier minimum price if it is lower than the current subscription price" do
            @tier_price.update!(price_cents: @new_price)
            expect(@subscription.current_subscription_price_cents).to eq @original_price
          end

  test "returns the current subscription price if it is lower than the tier price" do
  ...
  context_ "when the purchase has cached offer code details" do
  test "returns the cached pre-discount price" do
  ...
  context_ "when the purchase does not have cached offer code details" do
  test "uses the existing offer code to calculate the pre-discount cost" do
              expect(@subscription.current_plan_displayed_price_cents).to eq 400 # $3 paid price + $1 discount
            end

  context_ "and the offer code is 100% off" do
  test "falls back to the purchase displayed price" do
  ...
  context_ "#resubscribe!" do
  test "restarts subscription if it is pending cancellation" do
        @subscription.cancel!
        expect(@subscription.pending_cancellation?).to be(true)
  ...
  test "restarts subscription if it is cancelled" do
        @subscription.cancel_effective_immediately!
  ...
  test "creates a subscription restarted event when resubscribing", :freeze_time do
        @subscription.cancel_effective_immediately!
  ...
  test "restarts subscription if it has failed" do
        @subscription.unsubscribe_and_fail!
  ...
  test "does not restart subscription if has ended" do
        @subscription.end_subscription!
  ...
  test "returns true if new charge is not needed" do
        @subscription.cancel!
        expect(@subscription.pending_cancellation?).to be(true)
  ...
  test "returns false if new charge is needed" do
        @subscription.unsubscribe_and_fail!
  ...
  test "enqueues activate integrations worker if subscription had been deactivated" do
        @subscription.cancel_effective_immediately!
        @subscription.resubscribe!
        expect(ActivateIntegrationsWorker).to have_enqueued_sidekiq_job(@subscription.original_purchase.id)
      end

  test "does not enqueue activate integrations worker if subscription had not been deactivated" do
        @subscription.cancel!
        @subscription.resubscribe!
        expect(ActivateIntegrationsWorker.jobs.size).to eq(0)
      end

  test "creates a subscription_event of type restarted" do
        @subscription.cancel_effective_immediately!
        expect do
          @subscription.resubscribe!
  ...
  context_ "workflows" do
  context_ "when the subscription has lapsed" do
  test "schedules any workflow installments that were missed during the lapsed period" do
            freeze_time do
              @subscription.cancel_effective_immediately!
  ...
  context_ "when the subscription is pending cancellation" do
  test "does not schedule any workflow installments" do
            @subscription.cancel!
  ...
  context_ "#send_restart_notifications!" do
  test "notifies the creator if the subscription had been terminated" do
        @subscription.cancel!
  ...
  test "notifies the customer if the subscription had been terminated" do
        @subscription.cancel!
  ...
  test "sends a subscription_restarted notification if the subscription had been terminated" do
        @subscription.cancel!
  ...
  context_ "#last_resubscribed_at", :freeze_time do
      before do
        @subscription.subscription_events.create!(event_type: :deactivated, occurred_at: 1.week.ago)
      end

  test "returns the last restart time if the subscription has been restarted" do
  ...
  test "returns nil if the subscription has not been restarted" do
        expect(@subscription.last_resubscribed_at).to be_nil
      end
    end

  context_ "#last_deactivated_at", :freeze_time do
      before do
        @subscription.subscription_events.create!(event_type: :restarted, occurred_at: 1.week.ago)
      end

  test "returns the last deactivation time if the subscription has been deactivated" do
  ...
  test "returns nil if the subscription has not been deactivated" do
        expect(@subscription.last_deactivated_at).to be_nil
      end
    end

  context_ "#resubscribed?" do
      subject { @subscription.resubscribed? }

  context_ "when the subscription has not had an interruption" do
        it { is_expected.to eq(false) }
      end

  context_ "when the subscription has had an interruption" do
        before do
          @subscription.subscription_events.create!(event_type: :deactivated, occurred_at: 1.week.ago)
  ...
  context_ "#custom_fields" do
  test "returns the custom fields on the original purchase" do
  ...
  context_ "#has_free_trial?" do
  test "returns true if free_trial_ends_at is set" do
        subscription = build(:subscription, free_trial_ends_at: 1.day.ago)
        expect(subscription.has_free_trial?).to eq true
      end

  test "returns false if free_trial_ends_at is not set" do
        subscription = build(:subscription, free_trial_ends_at: nil)
        expect(subscription.has_free_trial?).to eq false
      end
    end

  context_ "#should_exclude_product_review_on_charge_reversal?" do
  test "returns false if the subscription does not have a free trial" do
  ...
  context_ "for a free trial subscription" do
  ...
  test "returns true if the initial successful charge does not allow a review" do
  ...
  test "returns false if the initial successful charge does not allow a review but the original purchase already excludes reviews" do
  ...
  test "returns false if the initial successful charge does allow a review" do
          create(:purchase, subscription:)
          expect(subscription.should_exclude_product_review_on_charge_reversal?).to eq false
        end

  test "returns true if there is not yet a successful charge" do
          expect(subscription.should_exclude_product_review_on_charge_reversal?).to eq true
        end
      end
    end

  context_ "#alive_or_restartable??" do
  test "returns true if ended_at is not set and the subscription is not cancelled by the seller" do
        subscription = create(:subscription)

        expect(subscription.alive_or_restartable?).to eq(true)
      end

  test "returns true if ended_at is not set and the subscription is cancelled by the buyer" do
        subscription = create(:subscription, cancelled_at: 1.day.ago, cancelled_by_buyer: true)

        expect(subscription.alive_or_restartable?).to eq(true)
      end

  test "returns false if ended_at is set" do
        subscription = create(:subscription, ended_at: 1.day.ago)

        expect(subscription.alive_or_restartable?).to eq(false)
      end

  test "returns false if the subscription is cancelled by the seller" do
  ...
  context_ "no subscription events have been created" do
  context_ "subscription is not deactivated" do
  test "returns true if the time is after created_at" do
            expect(subscription.alive_at?(purchase_date + 1.day)).to eq true
            expect(subscription.alive_at?(purchase_date - 1.day)).to eq false
          end
        end

  context_ "subscription is deactivated" do
  test "returns true if the time is between created_at..deactivated_at" do
  ...
  context_ "subscription has been deactivated and resubscribed" do
  test "returns true if the time is between subscribe and deactivate events" do
  ...
  context_ "#discount_applies_to_next_charge?" do
      let(:user) { create(:user) }
      let(:product) { create(:membership_product_with_preset_tiered_pricing, user:) }
      let(:offer_code) { create(:offer_code, products: [product]) }
  ...
  context_ "when the subscription has an offer code" do
        before do
  ...
  test "recomputes after reload" do
            expect(subscription.discount_applies_to_next_charge?).to eq(false)
  ...
  context_ "when the offer code is not expired" do
          before do
            subscription.original_purchase.purchase_offer_code_discount.update!(duration_in_billing_cycles: 2)
          end
  ...
  test "returns true even if the offer code is only for one membership cycle" do
          expect(subscription.discount_applies_to_next_charge?).to eq(true)
        end
      end
    end

  context_ "#auto_renewal_offer_code" do
      let(:seller) { create(:user) }
  ...
  test "discovers the best tiered renewal discount for the subscriber" do
        auto = subscription.auto_renewal_offer_code
        expect(auto.offer_code).to eq(tiered_code)
        expect(auto.resolved_percent).to eq(50)
      end

  test "discovers a standalone tiered renewal discount without existing_customers_only" do
        tiered_code.mark_deleted!
  ...
  test "applies a standalone tiered renewal discount to a guest subscription with no user" do
        tiered_code.mark_deleted!
        standalone_code = create(:offer_code,
                                 user: seller,
                                 products: [product],
  ...
  test "discovers universal existing-customer renewal discounts" do
        tiered_code.mark_deleted!
  ...
  test "discovers fixed-amount existing-customer renewal discounts" do
        tiered_code.mark_deleted!
  ...
  test "ignores inactive renewal discounts" do
        create(:offer_code,
               user: seller,
               code: "expiredrenewal",
               products: [product],
  ...
  test "ignores renewal discounts capped to billing cycles" do
        tiered_code.mark_deleted!
        create(:offer_code,
               user: seller,
               products: [product],
  ...
  test "ignores sold-out renewal discounts" do
  ...
  test "ignores renewal discounts with unmet minimum quantity" do
        create(:offer_code,
               user: seller,
  ...
  test "ignores renewal discounts with unmet minimum amount" do
        create(:offer_code,
               user: seller,
  ...
  test "uses the selected PWYW renewal price for minimum amount checks" do
  ...
  test "keeps the selected PWYW renewal price when the cached tier is zero percent" do
        original_purchase = subscription.original_purchase
  ...
  test "handles a cached 100 percent PWYW tier" do
        tiered_code.update!(
          amount_percentage: 100,
          ownership_duration_tiers: [
  ...
  test "keeps the selected PWYW renewal price when the cached tier is non-zero percent" do
        tiered_code.update!(
          amount_percentage: 25,
          ownership_duration_tiers: [
  ...
  test "applies advanced tiers to the selected PWYW renewal price after a non-zero cached tier" do
        tiered_code.update!(
  ...
  test "applies the advanced tier to the selected PWYW renewal price" do
        original_purchase = subscription.original_purchase
  ...
  test "memoizes missing renewal discounts until reload" do
        tiered_code.mark_deleted!
  ...
  test "does not reuse subscriber-memoized discounts for explicit guest checks" do
        expect(subscription.auto_renewal_offer_code.offer_code).to eq(tiered_code)

        expect(subscription.auto_renewal_offer_code(authenticated_offer_code_buyer: nil)).to be_nil
      end

  test "returns nil when the original purchase already carries a still-applicable discount" do
        offer_code = create(:offer_code,
                            user: seller,
                            code: "stillapplies",
                            products: [product],
  ...
  test "ignores deleted zeroed original discounts when discovering renewal discounts" do
        tiered_code.mark_deleted!
  ...
  test "re-evaluates tiered discounts attached to the original purchase" do
  ...
  test "re-evaluates capped tiered discounts attached to the original purchase" do
        original_purchase = subscription.original_purchase
  ...
  test "re-evaluates minimum-quantity tiered discounts attached to the original purchase" do
        original_purchase = subscription.original_purchase
        original_purchase.update!(offer_code: tiered_code, quantity: 1)
        original_purchase.create_purchase_offer_code_discount!(
          offer_code: tiered_code,
  ...
  test "re-evaluates minimum-amount tiered discounts attached to the original purchase" do
        original_purchase = subscription.original_purchase
  ...
  test "re-evaluates inactive tiered discounts attached to the original purchase" do
        original_purchase = subscription.original_purchase
        original_purchase.update!(offer_code: tiered_code)
        original_purchase.create_purchase_offer_code_discount!(
          offer_code: tiered_code,
  ...
  test "re-evaluates soft-deleted tiered discounts attached to the original purchase" do
        original_purchase = subscription.original_purchase
        original_purchase.update!(offer_code: tiered_code)
        original_purchase.create_purchase_offer_code_discount!(
          offer_code: tiered_code,
  ...
  test "records the auto-discovered discount on the renewal purchase" do
        renewal_purchase = subscription.build_purchase
        expect(renewal_purchase.offer_code).to eq(tiered_code)
  ...
  test "records the auto-discovered discount's pre-discount price per unit" do
  ...
  test "returns the cookie key" do
        expect(@subscription.cookie_key).to eq("subscription_#{@subscription.external_id_numeric}")
      end
    end

  context_ "#emails" do
  context_ "when the subscription has a user" do
  test "returns a hash of relevant emails" do
  ...
  context_ "when the subscription doesn't have a user" do
  test "returns a hash of relevant emails" do
  ...
  context_ "when the subscription is a gift" do
  ...
  test "returns giftee email as purchase and subscription emails" do
          expect(subscription.emails).to eq({
  ...
  test "returns user's form_email" do
          expect(subscription.email).to eq(subscription.user.form_email)
        end
      end

  context_ "when user is not present" do
        before do
          subscription.update!(user: nil)
        end

  test "returns purchase email" do
  ...
  context_ "#refresh_token" do
      let(:subscription) { create(:subscription, token: nil, token_expires_at: nil) }

  test "sets a new token and expiration date" do
  ...
  test "returns the newly set token" do
  ...
  context_ "when the original purchase is a gift sender purchase" do
        before do
  ...
  context_ "when the original purchase does not have a gift" do
  test "returns false" do
  ...
  context_ "#grant_access_to_product?" do
  context_ "installment plans" do
        let(:purchase) { create(:installment_plan_purchase) }
        let(:subscription) { purchase.subscription }

  test "returns false if the subscription has failed" do
          subscription.unsubscribe_and_fail!
  ...
  test "returns true even when the subscription has ended" do
  ...
  context_ "memberships" do
        let(:purchase) { create(:membership_purchase) }
        let(:subscription) { purchase.subscription }

  test "blocks access when cancelled if configured" do
  ...
  test "blocks access when failed if configured" do
          subscription.link.update!(block_access_after_membership_cancellation: false)
  ...
  test "blocks access when ended if configured" do
          subscription.link.update!(block_access_after_membership_cancellation: false)
  ...
  context_ "offer code persistence for subsequent charges" do
  context_ "installment plans" do
        let(:seller) { create(:user) }
  ...
  context_ "when offer code is deleted after initial purchase" do
  test "preserves the discount for subsequent installments" do
  ...
  context_ "when offer code expires after initial purchase" do
  test "preserves the discount for subsequent installments" do
  ...
  context_ "when offer code reaches max usage after initial purchase" do
  test "preserves the discount for subsequent installments" do
  ...
  context_ "when offer code amount changes after initial purchase" do
  test "uses the original snapshotted amount" do
            purchase = create(:installment_plan_purchase, link: product, offer_code: offer_code, purchaser: buyer)
            subscription = purchase.subscription
  ...
  context_ "when offer code is deleted after initial purchase" do
  test "does not count subsequent installments towards max purchases" do
            offer_code.update!(max_purchase_count: 1)
            purchase = create(:installment_plan_purchase, link: product, offer_code: offer_code, purchaser: buyer)
            subscription = purchase.subscription
  ...
  test "passes offer code validation for subsequent installments" do
            offer_code.update!(max_purchase_count: 1)
            purchase = create(:installment_plan_purchase, link: product, offer_code: offer_code, purchaser: buyer)
            subscription = purchase.subscription
  ...
  test "preserves percentage discount when offer code is deleted" do
  ...
  context_ "memberships with duration" do
        let(:seller) { create(:user) }
        let(:product) { create(:membership_product_with_preset_tiered_pricing, user: seller) }
        let(:offer_code) { create(:offer_code, products: [product], amount_cents: 100, duration_in_months: 3) }
        let(:buyer) { create(:user) }

  context_ "when offer code is deleted within duration" do
  test "preserves the discount for subsequent charges" do
  ...
  context_ "when offer code expires within duration" do
  test "preserves the discount for subsequent charges" do
            offer_code.update!(valid_at: 1.week.ago, expires_at: 1.day.from_now)
            purchase = create(:membership_purchase, link: product, offer_code: offer_code, purchaser: buyer, variant_attributes: [product.alive_variants.first])
            subscription = purchase.subscription
  ...
  context_ "when duration has elapsed" do
  test "does not apply the discount" do
            purchase = create(:membership_purchase, link: product, offer_code: offer_code, purchaser: buyer, variant_attributes: [product.alive_variants.first])
            subscription = purchase.subscription
            subscription.original_purchase.create_purchase_offer_code_discount!(
              offer_code: offer_code,
  ...
  context_ "backwards compatibility" do
        let(:seller) { create(:user) }
        let(:product) { create(:membership_product_with_preset_tiered_pricing, user: seller) }
        let(:offer_code) { create(:offer_code, products: [product], amount_cents: 100) }
        let(:buyer) { create(:user) }

  context_ "when original purchase has no cached discount (legacy)" do
  test "falls back to live offer code" do
            purchase = create(:membership_purchase, link: product, offer_code: offer_code, purchaser: buyer, variant_attributes: [product.alive_variants.first])
            subscription = purchase.subscription
  ...
  context_ "when purchase has no offer code" do
  test "does not create a discount" do
  ...
  context_ "#update_business_vat_id!" do
      let(:seller) { create(:user) }
      let(:product) { create(:subscription_product, user: seller) }
      let(:subscription) { create(:subscription, link: product, business_vat_id: nil) }

  test "updates subscription's business_vat_id when not already set" do
        subscription.update_business_vat_id!("IE6388047V")

        expect(subscription.reload.business_vat_id).to eq "IE6388047V"
      end

  test "does not update subscription's business_vat_id when already set" do
  ...
  test "does not update subscription's business_vat_id when nil is provided" do
        subscription.update_business_vat_id!(nil)

        expect(subscription.reload.business_vat_id).to be_nil
      end

  test "does not update subscription's business_vat_id when empty string is provided" do
