...
class CollaboratorInvitationTest < ActiveSupport::TestCase
  self.described_class = CollaboratorInvitation



  context_ CollaboratorInvitation, type: :model do
  context_ "#accept!" do
  test "destroys the invitation" do
  ...
  test "sends a notification email" do
        invitation = create(:collaborator_invitation)
  ...
  test "marks the collaborator as deleted" do
        expect { invitation.decline! }.to change { collaborator.reload.deleted? }.from(false).to(true)
      end

  test "disables the is_collab flag on associated products" do
  ...
  test "sends an email to the collaborator" do
        expect { invitation.decline! }
