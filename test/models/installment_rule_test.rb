...
class InstallmentRuleTest < ActiveSupport::TestCase
  self.described_class = InstallmentRule



  context_ InstallmentRule do
  context_ "version" do
      before do
  ...
  test "does not increment the version if period is changed" do
        expect do
          ...
          @post_rule.save
        end.not_to change { @post_rule.reload.version }
      end
  ...
  context_ "displayable_time_duration" do
      before do
        @product = create(:product)
        @post = create(:installment, link: @product, installment_type: "product")
  ...
  test "returns the correct duration based on the time period" do
  ...
  context_ "to_be_published_at_cannot_be_in_the_past" do
        before do
  ...
  test "allows to_be_published_at to be nil" do
          expect(@post_rule.to_be_published_at).to be_nil
          expect(@post_rule).to be_valid
        end

  test "disallows to_be_published_at to be in the past" do
  ...
  context_ "when about to be marked as deleted" do
  test "allows to_be_published_at to be in the past" do
            @post_rule.to_be_published_at = Time.current
  ...
  context_ "to_be_published_at_must_exist_for_non_workflow_posts" do
  test "disallows to_be_published_at to be nil" do
