...
class OauthApplicationTest < ActiveSupport::TestCase
  self.described_class = OauthApplication



  context_ OauthApplication do
  context_ "validity" do
      before do
        @user = create(:user)
      end

  test "does not validate name uniqueness" do
        create(:oauth_application, owner: @user, name: :foo)
        expect(build(:oauth_application, owner: @user, name: :foo)).to be_valid
      end

  test "allows 2 applications to be created with the same name" do
        create(:oauth_application, owner: @user, name: :foo)
        create(:oauth_application, owner: @user, name: :foo)
        expect(OauthApplication.count).to eq 2
      end

  test "allows applications to be created with affiliate_basis_points in an acceptable range" do
  ...
  test "does not allow applications to be created with invalid affiliate_basis_points" do
  ...
  test "saves with no icons attached" do
  ...
  test "saves with a valid icon type attached" do
        oauth_application = create(:oauth_application)
  ...
  test "errors with invalid icon type attached" do
        oauth_application = create(:oauth_application)
  ...
  test "defines private scopes as neither public nor default" do
        expect(private_scopes).to match_array(doorkeeper_scopes - public_scopes - default_scopes)
      end

  test "includes public scopes in user-created applications" do
        public_scopes.each do |scope|
          expect(@oauth_application.scopes).to include(scope.to_s)
        end
      end

  test "does not include default scopes in user-created applications" do
        default_scopes.each do |scope|
          expect(@oauth_application.scopes).not_to include(scope.to_s)
        end
      end

  test "does not include private scopes in user-created applications" do
        private_scopes.each do |scope|
          expect(@oauth_application.scopes).not_to include(scope.to_s)
        end
      end
  ...
  context_ "#get_or_generate_access_token" do
      before do
        @oauth_application = create(:oauth_application)
      end

  test "generates new access token if none exist" do
        expect do
  ...
  test "generates new access token if existing ones are revoked" do
  ...
  test "returns an existing non-revoked access token if one exists" do
        @oauth_application.get_or_generate_access_token
  ...
  context_ "when no access grant exists" do
  test "creates an access grant automatically" do
  ...
  context_ "when access grant already exists" do
        before do
          @oauth_application.get_or_generate_access_token # this will generate the access grant
        end

  test "does not create another access grant" do
  ...
  test "marks connected resource subscription as deleted" do
        expect do
  ...
  test "marks multiple resource subscriptions as deleted in bulk" do
  ...
  test "removes resource subscriptions for the user" do
        expect do
          @oauth_application.revoke_access_for(@subscriber_1)
        end.to change { ResourceSubscription.alive.count }.by(-1)
