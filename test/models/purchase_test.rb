...
class PurchaseTest < ActiveSupport::TestCase
  self.described_class = Purchase
  self.rspec_metadata = { vcr: true }



  context_ Purchase, :vcr do
  ...
  test "returns in_progress purchases" do
          expect(Purchase.in_progress).to include @in_progress_purchase
        end

  test "does not return failed purchases" do
          expect(Purchase.in_progress).not_to include @successful_purchase
        end
      end
  ...
  test "returns successful purchases" do
          expect(Purchase.successful).to include @successful_purchase
        end

  test "does not return failed purchases" do
          expect(Purchase.successful).not_to include @failed_purchase
        end
      end

  context_ ".not_successful" do
        before do
          @successful_purchase = create(:purchase, purchase_state: "successful")
          @failed_purchase = create(:purchase, purchase_state: "failed")
        end

  test "returns only unsuccessful purchases" do
          expect(described_class.not_successful).to include @failed_purchase
          expect(described_class.not_successful).not_to include @successful_purchase
        end
      end
  ...
  test "does not returns successful purchases" do
          expect(Purchase.failed).not_to include @successful_purchase
        end

  test "does return failed purchases" do
          expect(Purchase.failed).to include @failed_purchase
        end
      end

  context_ "stripe_failed" do
        before do
  ...
  test "returns failed purchases with non-blank stripe fingerprint" do
          expect(Purchase.stripe_failed).to include @failed_purchase_with_stripe_fingerprint
        end

  test "does not return successful purchases" do
          expect(Purchase.stripe_failed).not_to include @successful_purchase_with_stripe_fingerprint
        end

  test "does not return failed purchases with blank stripe fingerprint" do
          expect(Purchase.stripe_failed).not_to include @failed_purchase_no_stripe_fingerprint
        end
      end

  context_ "non_free" do
        before do
          @non_zero_fee = create(:purchase)
          @zero_fee = create(:free_purchase)
        end

  test "returns purchases with fee > 0" do
          expect(Purchase.non_free).to include @non_zero_fee
        end

  test "does not return purchases with 0 fee" do
          expect(Purchase.non_free).not_to include @zero_fee
        end
      end
  ...
  test "returns non-refunded non-free purchases" do
          expect(Purchase.paid).to include @non_refunded_purchase
        end

  test "does not return refunded purchases" do
          expect(Purchase.paid).not_to include @refunded_purchase
        end

  test "does not return free purchases" do
          expect(Purchase.paid).not_to include @free_purchase
        end

  test "has charge processor id set" do
  ...
  test "does not return chargebacked purchase" do
          expect(Purchase.not_chargedback).not_to include @chargebacked_purchase
          expect(Purchase.not_chargedback).not_to include @reversed_chargebacked_purchase
        end

  test "returns non-chargebacked purchase" do
          expect(Purchase.not_chargedback).to include @non_chargebacked_purchase
          expect(Purchase.not_chargedback).not_to include @reversed_chargebacked_purchase
        end
      end

  context_ "not_chargedback_or_chargedback_reversed" do
        before do
          @chargebacked_purchase = create(:purchase, chargeback_date: Date.yesterday)
          @reversed_chargebacked_purchase = create(:purchase, chargeback_date: Date.yesterday, chargeback_reversed: true)
          @non_chargebacked_purchase = create(:purchase)
  ...
  test "does not return chargebacked purchase" do
          expect(Purchase.not_chargedback_or_chargedback_reversed).not_to include @chargebacked_purchase
        end
  ...
  test "returns chargebacked reversed purchase" do
          expect(Purchase.not_chargedback_or_chargedback_reversed).to include @reversed_chargebacked_purchase
        end
      end

  context_ "additional contribution and max purchase quantity" do
        before do
          @product = create(:product, max_purchase_count: 1)
          @purchase = create(:purchase, link: @product, is_additional_contribution: true)
        end

  test "does not count the additional contribution towards the max quantity" do
          expect(@product.remaining_for_sale_count).to eq 1
        end
      end

  context_ "not_additional_contribution" do
        before do
          @additional_contribution = create(:purchase, is_additional_contribution: true)
          @not_additional_contribution = create(:purchase)
        end

  test "returns puchases that are not additional contributions" do
          expect(Purchase.not_additional_contribution).to include @not_additional_contribution
        end

  test "does not return purchases that are additional contribution" do
          expect(Purchase.not_additional_contribution).not_to include @additional_contribution
        end
      end

  context_ "not_recurring_charge" do
        before do
  ...
  test "does not return purchases that are subscriptions and not original_subscription_purchase" do
          expect(Purchase.not_recurring_charge).not_to include @recurring_purchase
        end

  test "returns purchases that are original_subscription_purchase" do
          expect(Purchase.not_recurring_charge).to include @original_subscription_purchase
        end

  test "returns normal purchases" do
          expect(Purchase.not_recurring_charge).to include @normal_purchase
        end
      end

  context_ "recurring_charge" do
        before do
          @normal_purchase = create(:purchase)
          subscription = create(:subscription)
          @original_subscription_purchase = create(:purchase, subscription:, is_original_subscription_purchase: true)
  ...
  test "does not return purchases that are original_subscription_purchase" do
          expect(Purchase.recurring_charge).not_to include @original_subscription_purchase
        end
  ...
  test "does not return normal purchases" do
          expect(Purchase.recurring_charge).not_to include @normal_purchase
        end
      end

  context_ ".paypal_orders" do
        before do
          @paypal_order_purchase = create(:purchase, paypal_order_id: "SamplePaypalOrderID")
          @non_paypal_order_purchase = create(:purchase)
        end

  test "returns only paypal order purchases" do
          expect(described_class.paypal_orders).to include @paypal_order_purchase
          expect(described_class.paypal_orders).not_to include @non_paypal_order_purchase
        end
      end

  context_ ".unsuccessful_paypal_orders" do
        before do
  ...
  test "returns only unsuccessful paypal order purchases created in specified time" do
          expect(@unsuccessful_paypal_order_purchases).to include @unsuccessful_paypal_order_purchase
          expect(@unsuccessful_paypal_order_purchases).not_to include @obsolete_unsuccessful_paypal_order_purchase
          expect(@unsuccessful_paypal_order_purchases).not_to include @recent_unsuccessful_paypal_order_purchase
          expect(@unsuccessful_paypal_order_purchases).not_to include @successful_paypal_order_purchase
          expect(@unsuccessful_paypal_order_purchases).not_to include @successful_non_paypal_order_purchase
          expect(@unsuccessful_paypal_order_purchases).not_to include @unsuccessful_non_paypal_order_purchase
        end
      end

  context_ ".with_credit_card_id" do
  test "returns the records with a credit_card_id value present" do
          ...
          expect(result).not_to include purchase2
          expect(result).to include purchase3
        end
      end

  context_ ".not_rental_expired" do
  test "returns purchases where rental_expired field is nil or false" do
  ...
  context_ ".for_library" do
  test "excludes archived original subscription purchases" do
  ...
  test "includes updated original subscription purchases with not_charged state" do
          purchase = create(:purchase, purchase_state: "not_charged")

          expect(Purchase.for_library).to include(purchase)
        end

  test "excludes purchase with access revoked" do
  ...
  context_ ".for_mobile_listing" do
  test "returns successful purchases" do
  ...
  test "excludes failed, refunded or chargedback, gift sender, recurring charge, buyer deleted, and expired rental purchases" do
  ...
  context_ ".for_sales_api" do
  test "includes successful purchases" do
          purchase = create(:purchase, purchase_state: "successful")
          expect(Purchase.for_sales_api).to match_array [purchase]
        end

  test "includes free trial not_charged purchases" do
          purchase = create(:free_trial_membership_purchase)
          expect(Purchase.for_sales_api).to match_array [purchase]
        end

  test "does not include other purchases" do
          %w(
            failed
  ...
  context_ ".for_visible_posts" do
  test "returns only eligible purchases for viewing posts" do
          buyer = create(:user)
  ...
  context_ ".exclude_not_charged_except_free_trial" do
  test "excludes purchases that are 'not_charged' but are not free trial purchases" do
  ...
  context_ ".no_or_active_subscription" do
  test "returns non-subscription purchases and purchases with an active subscription" do
          normal_purchase = create(:purchase)
          subscription = create(:subscription)
  ...
  test "does not include purchases with inactive subscription" do
          normal_purchase = create(:purchase)
  ...
  context_ ".inactive_subscription" do
  test "returns subscription purchases which have been deactivated" do
          create(:purchase)
  ...
  context_ ".can_access_content" do
  test "includes non-subscription purchases" do
          purchase = create(:purchase)
          expect(Purchase.can_access_content).to match_array [purchase]
        end

  context_ "subscription purchases" do
          let(:purchase) { create(:membership_purchase) }
          let(:subscription) { purchase.subscription }

  test "includes active subscription purchases" do
            expect(Purchase.can_access_content).to match_array [purchase]
          end

  test "includes inactive subscription purchases where subscribers are allowed to access product content after the subscription has lapsed" do
            subscription.update!(deactivated_at: 1.minute.ago)
            expect(Purchase.can_access_content).to match_array [purchase]
          end

  test "excludes inactive subscription purchases if subscribers should lose access when subscription lapses" do
            subscription.update!(deactivated_at: 1.minute.ago)
  ...
  context_ "check perceived_price_cents_matches_price_cents" do
        let(:product) { create(:product, price_cents: 10_00) }
        let(:purchase) { build(:purchase, link: product, perceived_price_cents: 5_00) }

  test "returns false if the perceived price is different from the link price" do
          purchase.save
  ...
  test "returns true if the purchase is_upgrade_purchase" do
  ...
  context_ "not_for_sale" do
  test "doesn't allow purchases of unpublished products" do
  ...
  test "allows purchases when is_commission_completion_purchase is true even if product is unpublished" do
        link = create(:product, purchase_disabled_at: Time.current)
  ...
  context_ "temporarily blocked product" do
      before do
  ...
  context_ "when the price is zero" do
        before { @product.price_cents = 0 }

  test "allows the purchase of temporarily blocked products" do
  ...
  context_ "when the price is not zero" do
  test "doesn't allow purchases of temporarily blocked products" do
  ...
  context_ "sold_out" do
  test "doesn't allow purchase once sold out" do
  ...
  test "doesn't count failed purchases towards the sold-out count" do
        link = create(:product, max_purchase_count: 1)
  ...
  test "allows saving a purchase when sold out" do
        link = create(:product, max_purchase_count: 1)
        purchase = create(:purchase, link:, seller: link.user)
  ...
  test "doesn't count additional contributions toward max_purchase_count" do
        link = create(:product, max_purchase_count: 1)
        create(:purchase, link:, seller: link.user)
  ...
  test "does not count recurring charges towards the max_purchase_count" do
  ...
  test "does count original_subscription_purchase towards max_purchase_count" do
  ...
  context_ "variants_available" do
      before :each do
        @product = create(:product)
  ...
  test "succeeds when all variants are available for the given quantities" do
        purchase = create(:purchase, link: @product, variant_attributes: [@variant1, @variant2])
        expect(purchase.errors).to be_blank
      end

  test "fails if at least one variant is not available for the given quantity" do
  ...
  test "fails if at least one variant is unavailable because it is deleted" do
        @variant2.mark_deleted!
        purchase = create(:purchase, link: @product, variant_attributes: [@variant1, @variant2])
        expect(purchase.errors.full_messages).to include "Sold out, please go back and pick another option."
      end

  test "fails if at least one variant is unavailable because it is sold out" do
        create(:purchase, link: @product, variant_attributes: [@variant1], quantity: 2)
        purchase = create(:purchase, link: @product, variant_attributes: [@variant1, @variant2])
        expect(purchase.errors.full_messages).to include "Sold out, please go back and pick another option."
      end

  context_ "when original_variant_attributes is set" do
  test "succeeds even when an original variant is sold out or marked deleted" do
  ...
  test "fails when at least one new variant is sold out" do
  ...
  test "returns paypal_refund_expired as true for unrefundable PayPal purchases and false for others" do
  ...
  test "uses the cached resolved discount amount for offer code display" do
  ...
  test "has the purchaser_id if one exists" do
  ...
  test "has the right daystamp" do
        day = 1.day.ago
  ...
  test "has the right iso2 code for the country" do
        @purchase.update_attribute(:country, "United States")
        expect(@purchase.as_json[:country_iso2]).to eq "US"
      end

  test "performs a safe country code lookup for a GeoIp2 country that isn't found in IsoCountryCodes" do
  ...
  test "returns country and state as is if they are set" do
  ...
  test "returns country and state based on ip_address if they don't exist" do
  ...
  test "does not have sku id if not sku exists but product is sku enabled" do
        @purchase.link.update_attribute(:skus_enabled, true)
        expect(@purchase.as_json[:sku_id]).to be_nil
      end

  test "contains receipt_url only when include_receipt_url is set" do
  ...
  test "contains `can_ping` only when `include_ping` is set" do
        expect(@purchase.as_json.key?(:can_ping)).to eq(false)
      end

  test "returns the correct value for `can_ping` when the user has notification endpoint set" do
        seller = @purchase.link.user
  ...
  test "returns the correct value for `can_ping` when the user has an oauth app" do
        seller = @purchase.link.user
  ...
  test "returns the provided override for `can_ping` if provided" do
        seller = @purchase.link.user
        seller.update!(notification_endpoint: "http://test/")
  ...
  test "returns the correct value for `recurring_charge`" do
        # A regular purchase
  ...
  test "returns information about the product" do
  ...
  test "doesn't set the card expiry month and year fields" do
  ...
  test "returns the dispute information" do
  ...
  test "includes relevant flags" do
  ...
  test "falls back to the purchaser's name if full_name is blank" do
        @purchase.update! full_name: "", purchaser: create(:user, name: "Mr Gumroadson")

        expect(@purchase.as_json[:full_name]).to eq("Mr Gumroadson")
      end

  context_ "when the product is of type subscription" do
  context_ "but not a tiered membership" do
          before do
  ...
  test "returns the correct value for `subscription_duration` when the default subscription period is opted for" do
            # The first purchase of a subscription product
  ...
  test "returns the correct value for `subscription_duration` when a non-default subscription period is opted for" do
            # The first purchase of a subscription product
  ...
  context_ "and is a tiered membership" do
          before do
            user = @purchase.link.user
  ...
  context_ "and has a free trial" do
            before do
  ...
  test "does not contain `subscription_duration` in the return value for a product which is not of type subscription" do
        expect(@purchase.as_json).not_to have_key(:subscription_duration)
      end

  context_ "with include_variant_details: true" do
  test "includes variant details regardless of skus_enabled" do
  ...
  test "includes SKU details regardless of skus_enabled" do
          product = @purchase.link
  ...
  test "includes empty hashes if no variants" do
          variants_json = @purchase.as_json(include_variant_details: true)[:variants]
  ...
  context_ "with creator_app_api: true" do
  test "includes the product's thumbnail url, if present" do
  ...
  test "includes price & formatted_total_price" do
  ...
  test "includes the refund state" do
  ...
  test "includes the chargeback state" do
          purchase = create(:purchase)
          json = purchase.as_json(creator_app_api: true)
  ...
  context_ "with query" do
  test "returns paypal_email when it matches query" do
  ...
  test "does not return paypal_email when it matches query but not an email" do
          query = "test_card"
  ...
  test "does not return paypal_email when it does not match a query" do
          paypal_email = "jane@paypal.com"
          @purchase.update!(card_visual: paypal_email)
  ...
  test "contains `can_revoke_access` and `can_undo_revoke_access`" do
  ...
  context_ "upsells" do
  context_ "when there isn't an upsell purchase" do
  test "doesn't include upsell information" do
            expect(@purchase.as_json[:upsell]).to be_nil
          end
        end

  context_ "when there is an upsell purchase" do
  test "includes upsell information" do
  ...
  context_ "when the purchase was recommended by more like this" do
  test "returns true for is_more_like_this_recommended" do
  ...
  context_ "with custom fields" do
  test "returns the correct format for version 1" do
  ...
  test "returns the correct format for version 2" do
          @purchase.purchase_custom_fields << build(:purchase_custom_field, name: "custom", value: "value")
          @purchase.purchase_custom_fields << build(:purchase_custom_field, name: "boolean", value: "true", type: CustomField::TYPE_CHECKBOX)
  ...
  context_ "max purchase limit reached" do
        before { purchase1.save! }

  context_ "purchase limit is then raised" do
  ...
  context_ "affiliate_merchant_account" do
  context_ "purchase is on a Gumroad merchant account" do
        let(:purchase) { create(:purchase) }

  test "returns a Gumroad merchant account" do
          expect(purchase.affiliate_merchant_account.user_id).to eq(nil)
        end

  test "returns a merchant account that matches the charge processor of the purchase" do
          expect(purchase.affiliate_merchant_account.charge_processor_id).to eq(purchase.charge_processor_id)
        end
      end

  context_ "purchase is on a creator's merchant account" do
  ...
  context_ "when include_tax_rate is true" do
  test "returns GST with percentage for Australia" do
            expect(purchase_au.tax_label).to eq("GST (10%)")
          end

  test "returns GST with percentage for Singapore" do
            expect(purchase_sg.tax_label).to eq("GST (7%)")
          end
        end

  context_ "when include_tax_rate is false" do
  test "returns GST without percentage" do
  ...
  context_ "when include_tax_rate is true" do
  test "returns VAT with percentage for EU countries" do
            expect(eu_purchase.tax_label).to eq("VAT (19%)")
          end

  test "returns VAT with percentage for Norway" do
            expect(norway_purchase.tax_label).to eq("VAT (25%)")
          end

  test "returns VAT with percentage for countries that collect tax on all products" do
            expect(japan_purchase.tax_label).to eq("VAT (10%)")
          end

  test "returns VAT with percentage for countries that collect tax on digital products" do
            expect(digital_purchase.tax_label).to eq("VAT (20%)")
          end

  test "rounds down the percentage" do
  ...
  context_ "when include_tax_rate is false" do
  test "returns VAT without percentage" do
  ...
  context_ "when include_tax_rate is true" do
  context_ "when was_tax_excluded_from_price is true" do
  test "returns 'Sales tax' for US" do
  ...
  test "returns 'Sales tax' for Canada" do
  ...
  test "returns 'Sales tax' for other countries" do
  ...
  context_ "when was_tax_excluded_from_price is false" do
  test "returns 'Sales tax (included)' for US" do
  ...
  test "returns 'Sales tax (included)' for Canada" do
  ...
  test "returns 'Sales tax (included)' for other countries" do
  ...
  context_ "when include_tax_rate is false" do
  test "returns 'Sales tax' regardless of was_tax_excluded_from_price" do
  ...
  context_ "edge cases" do
  test "returns nil when has_tax_label? is false" do
  ...
  test "defaults to 'Sales tax (included)' when zip_tax_rate is nil" do
          purchase = create(:purchase, price_cents: 100, total_transaction_cents: 120, gumroad_tax_cents: 20, zip_tax_rate: nil)
          expect(purchase.tax_label).to eq("Sales tax (included)")
        end

  test "uses tax_cents when gumroad_tax_cents is 0" do
  ...
  context_ "tax_label_with_creator_tax_info" do
      let(:purchase) { create(:purchase) }

  context_ "purchase without any tax rate attached to it" do
  test "defers to #tax_label" do
  ...
  context_ "purchase with zip tax rate" do
  ...
  context_ "without an user association" do
  ...
  test "returns #tax_label's result" do
            expect(purchase.tax_label_with_creator_tax_info).to eq(purchase.tax_label)
          end
        end

  context_ "with a user association but no invoice_sales_tax_id" do
  ...
  context_ "with a user association and invoice_sales_tax_id" do
          let(:purchase) { create(:purchase, gumroad_tax_cents: 100) }
  ...
  test "appends the tax ID to the tax_label result" do
  ...
  context_ "#sync_status_with_charge_processor" do
  test "calls Purchase::SyncStatusWithChargeProcessorService for the purchase" do
        purchase = create(:purchase)
  ...
  context_ "#find_enabled_integration" do
      let(:discord_integration) { create(:discord_integration) }
      let(:circle_integration) { create(:circle_integration) }

  test "returns the enabled integration for a standalone product purchase" do
  ...
  test "returns the enabled integration for a variant purchase" do
  ...
  test "returns nil if the purchased product has an enabled integration but the variant does not" do
        product = create(:product_with_digital_versions, active_integrations: [discord_integration, circle_integration])
  ...
  context_ "#perceived_price_cents nil" do
        before { @subject.perceived_price_cents = nil }

  test "can be valid because it wasn't used - mainly used in the view" do
          expect(@subject).to be_valid
        end
      end

  context_ "#perceived_price_cents does not match but user set his own price for a customizable link" do
        before do
  ...
  context_ "#variant_extra_cost" do
  context_ "for a purchase with no variants" do
  test "returns 0" do
  ...
  context_ "for a purchase with variants" do
  context_ "for a non-tiered membership product" do
  test "sums the variants' price_difference_cents" do
            product = create(:product)
  ...
  context_ "for a tiered membership product" do
          before :each do
            recurrence_price_values = [
  ...
  test "sums the price cents for the variant prices with the given recurrence" do
            expect(@purchase.variant_extra_cost).to eq 50_00
          end

  context_ "where a variant doesn't have prices" do
  test "returns 0, regardless of the variant's price_difference_cents" do
  ...
  context_ "where a variant has only rental prices" do
  test "returns 0" do
  ...
  context_ "when existing price has been deleted" do
            before :each do
              @yearly_price.mark_deleted!
              @tier.prices.find_by(recurrence: BasePrice::Recurrence::YEARLY).mark_deleted!
            end

  test "returns 0 if original_price is not set" do
              expect(@purchase.variant_extra_cost).to eq 0
            end

  test "counts the deleted price if original_price is set" do
  ...
  test "doesn't reset the rate when it gets saved again " do
        purchase = create(:purchase, price_cents: 1_00)
  ...
  test "is 0 if merchant account is a Brazilian Stripe Connect account" do
  ...
  context_ "processor_fee_cents" do
  test "gets calculated correctly" do
        purchase = create(:purchase)
  ...
  context_ "payment" do
  test "is the difference between price and fee" do
        purchase = create(:purchase, price_cents: 1_00)
        expect(purchase.payment_cents).to eq 7 # calculated fee is 93c -- 10c (10%) + 50c + 3c (2.9% cc fee) + 30c (fixed cc fee)
      end
    end

  context_ "save_with_payment" do
      before do
        @user = create(:user)
        @product = create(:product, user: @user)
      end

  test "doesn't hit stripe if invalid" do
  ...
  test "does not hit stripe if #{k}" do
          v.call(@user, @product)
          @product.save!
  ...
  context_ "#charged_using_paypal_connect_account?" do
  test "returns true if merchant account is a paypal connect account otherwise false" do
  ...
  context_ "new flat fee" do
      before do
        @creator = create(:user)
        @product = create(:product, user: @creator, price_cents: 10_00)
      end

  context_ "charge on gumroad stripe account" do
  test "uses flat fee if applicable to the creator otherwise uses tier fee" do
  ...
  context_ "charge on a custom stripe connect account" do
  test "uses flat fee if applicable to the creator otherwise uses tier fee" do
  ...
  context_ "charge on a paypal connect account" do
  test "uses flat fee if applicable to the creator otherwise uses tier fee" do
  ...
  context_ "charge on gumroad paypal account via braintree" do
  test "uses flat fee if applicable to the creator otherwise uses tier fee" do
  ...
  test "charges discover fee of 30%" do
  ...
  test "does not charge 10% Gumroad fee for regular product sale" do
  ...
  test "does not charge 10% Gumroad fee for new membership product sale" do
  ...
  test "does not charge 10% Gumroad fee for recommended regular product sale" do
          purchase = create(:purchase, link: create(:product, user: seller), price_cents: 10_00,
  ...
  test "does not charge 10% Gumroad fee for recommended new membership product sale" do
          purchase = create(:membership_purchase, link: create(:membership_product, user: seller), price_cents: 10_00,
                                                  was_product_recommended: true,
                                                  recommended_by: RecommendationType::GUMROAD_SEARCH_RECOMMENDATION)
  ...
  test "charges the boost fee minus the 10% Gumroad fee for recommended new membership product sale" do
  ...
  test "charges 10% Gumroad fee for recurring charge on existing membership" do
  ...
  test "charges the boost fee including 10% Gumroad fee for recurring charge on recommended membership sale" do
  ...
  test "charges 10% Gumroad fee for charge on existing preorder" do
  ...
  context_ "with waive_gumroad_fee_on_new_sales feature flag set" do
        let!(:seller) { create(:named_seller) }
  ...
  context_ "purchase requires either purchaser or email" do
  test "works with a purchaser" do
        user = create(:user)
        purchase = build(:purchase, purchaser: user)
        expect(purchase).to be_valid
      end

  test "works with an email" do
        purchase = build(:purchase, email: "test@example.com")
        expect(purchase).to be_valid
      end

  test "is not valid without either" do
        purchase = build(:purchase, email: nil)
        expect(purchase).not_to be_valid
      end
    end

  context_ "create_url_redirect!" do
      before do
        @purchase = create(:purchase)
        @purchase.perceived_price_cents = 100
  ...
  test "doesn't create it multiple times, even if called a bunch" do
        expect do
  ...
  context_ "commission completion purchase" do
        let(:purchase) { create(:purchase, is_commission_completion_purchase: true) }

  test "does not create a url redirect" do
          purchase.create_url_redirect!
  ...
  context_ "has past installments" do
          before do
  ...
  test "sends the last installment as an email to the new subscriber" do
  ...
  test "does not send the last installment to the subscriber on recurring charges" do
  ...
  context_ "subscription should not send last update" do
            before do
              @product.update_attribute(:should_include_last_post, false)
            end

  test "does not send the last installment to the new subscriber" do
              @subscription.purchases << @purchase
              @purchase.update_balance_and_mark_successful!
  ...
  context_ "non-webhook product" do
        before do
          @product = create(:product)
        end

  test "creates a url_redirect" do
          purchase = create(:purchase, link: @product)
  ...
  context_ "financial_transaction_valid?" do
  test "has charge processor details if amount not 0" do
  ...
  test "does not update with charge processor details if amount = 0" do
  ...
  context_ "when the creator does not have their own merchant account" do
  test "is charged using a Gumroad merchant account for suppliers" do
          purchase.process!
  ...
  context_ "when the creator has their own merchant account" do
  ...
  context_ "when the link is digital" do
  test "is charged using the creators merchant account" do
            purchase.process!
            expect(purchase.errors).to be_empty
            expect(purchase.merchant_account).to eq(merchant_account)
          end
  ...
  context_ "when the link is physical" do
          let(:physical) { true }
  ...
  context_ "when the purchase has sales tax that gumroad is collecting and will pay as the merchant" do
          let(:purchase) do
            create(:purchase, seller: user, link:, price_cents: link.price_cents, fee_cents: 30, purchase_state: "in_progress", merchant_account: nil, chargeable:,
  ...
  context_ "not_double_charged" do
      before do
        @product = create(:product)
        @ip_address = generate(:ip)
      end

  test "allows double charges with bundle product purchases" do
  ...
  test "disallows double-charges to the same email and IP address" do
        ...
        expect(purchase1.id).not_to eq purchase2.id
        expect(purchase2).not_to be_valid
      end

  test "allows double-charges to different IP addresses" do
        create(:purchase, link: @product, seller: @product.user, email: "bob@gumroad.com", ip_address: @ip_address, created_at: Time.current)
        purchase2 = build(:purchase, link: @product, email: "bob2@gumroad.com", created_at: Time.current)
        expect(purchase2).to be_valid
      end

  test "disallows double-charges if the first purchase is in progress" do
  ...
  test "allows double-charge if purchase is from the profile page and of a quantity-enabled product" do
  ...
  context_ "as first product purchase" do
  test "allows the gift-purchase" do
  ...
  context_ "after purchasing it as a non-gift" do
          before do
            create(:purchase, link: product)
          end
  ...
  context_ "after gifting to someone else" do
          before do
  ...
  test "allows purchase as a non-gift to original purchaser" do
  ...
  context_ "when gifting a subscription" do
  ...
  context_ "purchasing physical products" do
        let(:product) { create(:physical_product) }

  test "prohibits double-charges within 10 seconds" do
  ...
  test "allows double-charges after 10 seconds" do
  ...
  context_ "purchasing licensed products" do
  ...
  context_ "when upgrading a subscription" do
  test "prohibits double-charges within 10 seconds" do
  ...
  context_ "purchaser_email_or_email" do
  test "provides email if no purchaser" do
  ...
  test "provides purchaser email if it exists" do
  ...
  test "provides email if purchase email blank" do
  ...
  test "provides email if both are present" do
        buyer = create(:user, email: "margaret@example.com")
  ...
  context_ "additional information passed to charge processor" do
  context_ "reference" do
        let(:purchase) { create(:purchase_with_balance, chargeable: build(:chargeable)) }

  test "is sent with the default if the statement description isn't customized" do
  ...
  context_ "soft descriptor with creator" do
        before do
  ...
  test "is sent with creator name" do
  ...
  context_ "total_transaction_amount_for_gumroad_cents" do
      let(:seller) { create(:named_seller) }
  ...
  test "is the sum of the fee cents and tax that gumroad collected" do
        expect(purchase.total_transaction_amount_for_gumroad_cents).to eq(182) # 132c fee (10% + 50c + 2.9% + 30c) + 50c gumroad tax
      end

  context_ "use when charging" do
  test "is sent to the charge processor" do
          expect(ChargeProcessor).to receive(:create_payment_intent_or_charge!).with(
            anything,
            chargeable,
  ...
  context_ "calculate_price_range_cents" do
  ...
  test "handles euro-style entries" do
  ...
  test "returns correct purchase info" do
  ...
  test "returns purchase info with account_by_this_email_exists set to true if purchaser_id is set for the purchase" do
  ...
  test "returns nil for content_url and content_token if url_redirect is not present" do
  ...
  test "shows test purchase notice if purchase is a test" do
  ...
  test "returns sales tax amount and indicates it has sales tax to show if exclusive and present" do
  ...
  test "returns sales tax amount and indicates it has sales tax to show if inclusive and present" do
        purchase.tax_cents = 25
  ...
  test "returns quantity and show_quantity for physical product purchase" do
  ...
  test "returns shipping and has_shipping_to_show for physical product purchase with shipping" do
        link.update_attribute(:is_physical, true)
  ...
  test "returns the tracking url for physical product purchase where order has been shipped with tracking" do
        link.update_attribute(:is_physical, true)
  ...
  test "returns membership-specific information for a purchase" do
        purchase = create(:membership_purchase)
        product = purchase.link
  ...
  test "returns license_key if it exists" do
  ...
  test "returns should_show_receipt as true if purchase is a received gift" do
  ...
  test "includes bundle products" do
  ...
  test "returns unique permalink even if product has a custom permalink" do
        expect(Purchase.purchase_response(url_redirect, link, purchase)[:permalink]).to eq("unique")
      end

  test "returns purchase response with purchase info and payload_for_ping_notification merged" do
  ...
  test "returns emailed preorder notice if link is in preorder state" do
  ...
  test "returns emailed physical preorder notice if link is physical and in preorder state" do
        allow(link).to receive(:is_in_preorder_state).and_return(true)
  ...
  test "returns subscription notice if link is subscription" do
  ...
  test "returns physcial subscription notice if link is a physcial subscription" do
        allow(link).to receive(:is_recurring_billing).and_return(true)
        allow(link).to receive(:is_physical).and_return(true)
        expect(Purchase.purchase_response(url_redirect, link, purchase)[:extra_purchase_notice])
  ...
  context_ "#notify_seller!" do
  context_ "purchase is a bundle product purchase" do
        let(:purchase) { create(:purchase, is_bundle_product_purchase: true) }

  test "doesn't notify the seller" do
          expect { purchase.notify_seller! }.not_to have_enqueued_mail(ContactingCreatorMailer, :notify)
        end
      end

  context_ "purchase is a commission completion purchase" do
        let(:purchase) { create(:purchase, is_commission_completion_purchase: true) }
  ...
  context_ "#create_artifacts_and_send_receipt!" do
  context_ "purchase is a bundle purchase" do
        let(:seller) { create(:named_seller) }
  ...
  test "creates bundle product purchase artifacts" do
          purchase.create_artifacts_and_send_receipt!
  ...
  test "doesn't send the receipt" do
          expect { purchase.notify_seller! }.not_to have_enqueued_mail(CustomerMailer, :receipt)
        end
      end
    end

  context_ "#mark_product_purchases_as_chargedback!" do
      let(:purchase) { create(:purchase, link: create(:product, :bundle)) }
  ...
  test "marks all bundle purchases as charged back" do
        ...
        expect(purchase.product_purchases.first.chargeback_date).not_to be_nil
        expect(purchase.product_purchases.second.chargeback_date).not_to be_nil
      end
    end

  context_ "#mark_product_purchases_as_chargeback_reversed!" do
      let(:purchase) { create(:purchase, link: create(:product, :bundle)) }
  ...
  test "marks all bundle purchases as chargeback reversed" do
  ...
  context_ "#mark_product_purchases_as_refunded!" do
      let(:purchase) { create(:purchase, link: create(:product, :bundle)) }
  ...
  test "marks all bundle product purchases as fully refunded" do
  ...
  test "marks all bundle product purchases as partially refunded" do
  ...
  context_ "#has_content?" do
      before :each do
        allow(purchase).to receive(:webhook_failed).and_return false
      end

  context_ "when the purchased product has product files" do
        let(:product) { create(:product_with_files) }
        let(:purchase) { create(:purchase, link: product) }
        let!(:url_redirect) { create(:url_redirect, purchase:, link: product) }

  test "returns true if webhook did not fail, pdf stamp is disabled and url redirect is present" do
          expect(purchase.has_content?).to be(true)
        end

  test "returns false if webhook has falied" do
          allow(purchase).to receive(:webhook_failed).and_return true
          expect(purchase.has_content?).to be(false)
        end

  test "returns false if product has stampable files but the stamping hasn't finished" do
          product.product_files << create(:readable_document, pdf_stamp_enabled: true)

          expect(purchase.has_content?).to be(false)
        end

  test "returns true if product has stampable files and the stamping has finished" do
          product.product_files << create(:readable_document, pdf_stamp_enabled: true)
  ...
  test "returns false if url redirect is nil" do
          allow(purchase).to receive(:url_redirect).and_return nil
          expect(purchase.has_content?).to be(false)
        end
      end

  context_ "when the purchased product does not have product files" do
        let(:product) { create(:product) }
        let(:purchase) { create(:purchase, link: product) }
        let!(:url_redirect) { create(:url_redirect, purchase:, link: product) }
  ...
  context_ "#successful_and_valid?" do
  test "returns true if is is successful, not charged back, not refunded and not additional contribution" do
  ...
  test "returns false if it is not successful" do
  ...
  context_ "subscription purchase" do
        before do
  ...
  test "returns true if it has not been cancelled nor failed" do
          expect(@subscription_purchase.successful_and_valid?).to be(true)
        end

  test "returns true if the subscription has been upgraded" do
  ...
  test "returns false if it has been cancelled" do
  ...
  test "returns false if it has been failed" do
  ...
  context_ "#successful_and_not_reversed?" do
  context_ "when include_gift is false" do
  test "returns true for a successful purchase" do
  ...
  test "returns false for a successful chargedback purchase" do
  ...
  test "returns false for a successful refunded purchase" do
          purchase = build(:purchase, purchase_state: "successful", stripe_refunded: true)
          expect(purchase.successful_and_not_reversed?).to eq false
        end

  test "returns false for a received gift purchase" do
          purchase = build(:purchase, :gift_receiver)
          expect(purchase.successful_and_not_reversed?).to eq false
        end
      end

  context_ "when include_gift is true" do
  test "returns true for a successful purchase" do
          ["preorder_authorization_successful", "successful", "not_charged"].map do |successful_state|
            purchase = build(:purchase, purchase_state: successful_state)
  ...
  context_ "when the user specifies their own zip (e.g. via shipping)" do
  ...
  test "correctly parses the zip from different formats" do
  ...
  context_ "when a product's sales number changes" do
      before do
        @product = create(:product)
        @purchase = create(:purchase, link: @product, purchase_state: "in_progress")
      end

  test "schedules a sidekiq job to invalidate the product's cache in 1 minute" do
  ...
  context_ "successful purchase" do
      before do
        @user = create(:user)
  ...
  test "schedules a sidekiq job to invalidate the product's cache on inventory change in 1 minute" do
  ...
  test "sets updated_at on the sku" do
  ...
  test "does not put the seller on probation for expensive sales" do
  ...
  test "does not create a license for the gifter" do
        @giftee_purchase.mark_gift_receiver_purchase_successful
        expect(@gifter_purchase.reload.license).to be(nil)
        expect(@giftee_purchase.reload.license).not_to be(nil)
      end

  test "has the same license key for all subsequent purchases of a subscription as the original purchase" do
        user = create(:user)
        ...
        expect(original_subscription_purchase.license.serial).not_to be_nil
  ...
  context_ "#license_json" do
  test "returns the license information" do
          purchase = create(:purchase, link: @product)
          license = create(:license, purchase:)
  ...
  context_ "when multiseat is disabled" do
  test "returns `is_multiseat_license` as false" do
            purchase = create(:purchase, link: @product)
            license = create(:license, purchase:)
  ...
  context_ "when multiseat is enabled" do
  test "returns `is_multiseat_license` as true" do
  ...
  context_ "variant_names_hash" do
      before do
  ...
  test "returns the selected SKU regardless of skus_enabled" do
        @purchase.process!
        @purchase.link.update!(skus_enabled: false)
        expect(@purchase.variant_names_hash).to eq("Size - Color" => "Small - Red")
      end

  test "returns the selected variants regardless of skus_enabled" do
  ...
  context_ "referrer" do
  test "truncates the referrer and then save it" do
        purchase = create(:purchase, referrer: "a" * 1000)
        expect(purchase.referrer).to eq("a" * 191)
      end
    end

  context_ "#schedule_subscription_jobs" do
      before do
  ...
  test "schedules the job to end the subscription if the set number of charges have completed" do
  ...
  context_ "rental expiration reminder emails" do
      before do
  ...
  test "has scheduled all 3 reminder email jobs" do
  ...
  test "standalone rate applied when shipped to a region that has a shipping rate configured - qty of 1" do
  ...
  test "combined rate applied when shipped to a region that has a shipping rate configured - qty of 5" do
        @purchase.country = "United States"
        @purchase.zip_code = 94_107
        @purchase.state = "CA"
  ...
  context_ "validate shipping" do
        before do
          user = create(:user)
          @phys_link = create(:product, price_cents: 100_00, user:, is_physical: true, require_shipping: true)
        end

  test "does not allow shipping to a region where there is no shipping rate configured" do
  ...
  test "does not allow shipping to a region that is not compliant" do
  ...
  test "allows to ship to any destinate if ELSEWHERE is a configured shipping region" do
  ...
  test "converts the shipping charges to USD before charging" do
        @purchase.country = "Germany"
  ...
  test "returns shipping added to price_cents if the purchase is a test purchase" do
        @purchase.country = "Germany"
        ...
        expect(ChargeProcessor).not_to receive(:charge!)
  ...
  context_ "validate quantity" do
      before do
        @product = create(:product, price_cents: 100_00)
      end

  test "renders purchases valid if the quantity is 1 or more" do
  ...
  test "renders purchases invalid if the quantity is 0 or less" do
  ...
  context_ "validate is_free_trial_purchase" do
  context_ "when product has free trial enabled" do
        let(:product) do
          create(:membership_product, :with_free_trial_enabled)
        end
        let(:email) { "subscriber@example.com" }

  test "requires is_free_trial_purchase to be set for initial purchase of product" do
  ...
  test "does not require is_free_trial_purchase to be set when changing plan" do
          purchase = build(:membership_purchase, link: product, is_updated_original_subscription_purchase: true)
          expect(purchase).to be_valid
        end

  context_ "when the user has already subscribed" do
          let!(:existing_subscription) do
            purchase = create(:free_trial_membership_purchase, link: product, email:)
            purchase.subscription
          end

  test "allows re-purchasing if the existing subscription(s) have paid charges" do
  ...
  test "does not allow re-purchasing if the existing subscription(s) do not have paid charges" do
  ...
  context_ "recurring charges" do
          let(:original_purchase) { create(:membership_purchase, link: product, is_free_trial_purchase: true) }

  test "does not allow is_free_trial_purchase to be set for recurring charges" do
  ...
  test "does not error if is_free_trial_purchase is not set" do
  ...
  context_ "when product does not have free trial enabled" do
  test "does not allow is_free_trial_purchase to be set" do
  ...
  test "allows is_free_trial_purchase to be set for a pre-existing purchase" do
          purchase = create(:membership_purchase)
          purchase.is_free_trial_purchase = true
  ...
  test "allows is_free_trial_purchase to be set when changing a subscription plan" do
          purchase = create(:membership_purchase)
          purchase.is_free_trial_purchase = true
  ...
  context_ "purchase sales tax info" do
  test "creates a purchase sales tax info entry if it does not have one" do
        purchase = create(:purchase, price_cents: 100_00, chargeable: create(:chargeable))
        ...
        expect(actual_purchase_sales_tax_info).not_to be(nil)
  ...
  test "does not create a purchase sales tax info entry if it already has one" do
  ...
  test "stores VAT ID on subscription when present in sales tax info" do
  ...
  test "handles invalid countries from GEOIP lookup for IP address" do
        purchase = create(:purchase, price_cents: 100_00, chargeable: create(:chargeable))
        purchase.sales_tax_country_code_election = Compliance::Countries::DEU.alpha2
        purchase.country = Compliance::Countries::USA.common_name
        purchase.zip_code = "94117"
  ...
  context_ "sku_custom_name_or_external_id" do
      before do
        @product = create(:product)
        @purchase = create(:purchase, link: @product)
      end

  context_ "with sku" do
        before do
          @product.skus_enabled = true
        end

  test "returns the sku external id" do
          @purchase.variant_attributes << create(:sku)
          expect(@purchase.sku_custom_name_or_external_id).to eq(Sku.last.external_id.to_s)
        end

  test "returns the custom sku" do
  ...
  context_ "without sku" do
  context_ "when product is physical and has a variant" do
  ...
  test "returns the variant external id" do
            expect(@purchase.sku_custom_name_or_external_id).to eq(variant.external_id)
          end
        end

  test "returns the link external id" do
  ...
  test "does not enqueue the job to schedule workflow emails if recurring payment" do
        @recurring_purchase.mark_successful!
  ...
  context_ "#email_digest" do
  test "returns a HMAC digest of id and email" do
  ...
  test "returns nil when email is blank" do
        purchase = build(:purchase, email: nil)
        expect(purchase.email_digest).to be_nil
      end
    end

  context_ "#receipt_url" do
      let(:purchase) { create(:purchase) }

  test "returns the correct receipt URL" do
  ...
  context_ "email" do
  context_ "on create" do
  context_ "valid email" do
          let(:purchase) do
  ...
  context_ ".counts_towards_inventory" do
  test "only includes purchases that could become successful" do
        product = create(:product)
  ...
  test "excludes recurring charges" do
        product = create(:product)
        subscription = create(:subscription, link: product)
  ...
  test "excludes additional contributions" do
  ...
  context_ "memberships with tiers" do
  test "only counts active memberships + non-subscription sales" do
  ...
  context_ ".counts_towards_offer_code_uses" do
  test "includes successful purchases" do
        purchase = create(:purchase, purchase_state: "successful")
        expect(Purchase.counts_towards_offer_code_uses).to match_array [purchase]
      end

  test "includes preorder authorization purchases" do
        purchase = create(:preorder_authorization_purchase)
        expect(Purchase.counts_towards_offer_code_uses).to match_array [purchase]
      end

  test "includes original (non-archived) membership purchases" do
        purchase = create(:membership_purchase)
        expect(Purchase.counts_towards_offer_code_uses).to match_array [purchase]
      end

  test "includes free trial membership purchases" do
        purchase = create(:free_trial_membership_purchase)
        expect(Purchase.counts_towards_offer_code_uses).to match_array [purchase]
      end
  ...
  context_ ".counts_towards_volume" do
  test "includes successful purchases" do
        purchase = create(:purchase, purchase_state: "successful")
  ...
  test "returns false if the purchase is recommended by library or more like this" do
        expect(@purchase.charge_discover_fee?).to eq(false)
  ...
  context_ "paypal purchase failure" do
      before do
        @purchase = build(:purchase_with_balance, save_card: true, chargeable: build(:paypal_chargeable))
        @purchase.process!
      end

  test "emails the buyer saying the purchase failed" do
        mail_double = double
  ...
  test "emails the buyer saying the purchase failed for native paypal too" do
  ...
  test "doesn't email the buyer if not a paypal purchase" do
        mail_double = double
        allow(mail_double).to receive(:deliver_later)
  ...
  test "unsubcribes the buyer of the purchase from all sales made by the seller" do
        expect do
  ...
  context_ "when purchase record is invalid" do
        before do
  ...
  test "unsubscribes the buyer without running validations" do
  ...
  context_ "#toggle_off_can_contact_if_buyer_has_unsubscribed" do
  ...
  context_ "when customer has previously unsubscribed" do
        before do
  ...
  test "sets can_contact to false on new purchases automatically" do
          new_purchase = create(:purchase, link: product_2, email: buyer_email, seller:)

          expect(new_purchase.can_contact).to be(false)
        end

  test "does not add the customer to AudienceMember for the new purchase" do
  ...
  test "prevents the customer from appearing in email blast audience" do
  ...
  context_ "when customer has not previously unsubscribed" do
  test "allows new purchases to have can_contact: true" do
          new_purchase = create(:purchase, link: product_2, email: buyer_email, seller:)
          expect(new_purchase.can_contact).to be(true)
          expect(AudienceMember.find_by(email: buyer_email, seller:)).to be_present
        end

  test "allows the customer to appear in email blast audience" do
          installment = create(:installment, seller:, installment_type: "audience")
          create(:purchase, link: product_2, email: buyer_email, seller:)
  ...
  context_ "#attach_credit_card_to_purchaser" do
  test "the method is not called when the purchaser_id is not updated" do
        user = create(:user)
  ...
  test "the method is not called when the purchaser_id is set to `nil`" do
        user = create(:user)
        subscription = create(:subscription, user:)
        purchase = create(:purchase, purchaser: user, subscription:,
                                     is_original_subscription_purchase: true)
  ...
  test "the method is not called for non-subscription purchases" do
  ...
  context_ "when changing the purchaser id" do
        let(:user) { create(:user) }
  ...
  context_ "when :attach_credit_card_to_purchaser feature is disabled" do
  test "does not call the method" do
            expect(purchase).not_to receive(:attach_credit_card_to_purchaser)
  ...
  context_ "when :attach_credit_card_to_purchaser feature is enabled" do
          before { Feature.activate(:attach_credit_card_to_purchaser) }

  test "calls the method" do
  ...
  test "attaches the credit card of the latest successful purchase to the purchaser" do
  ...
  test "does not attempt to attach a credit card to the purchaser if one already exists" do
            user = create(:user, credit_card: create(:credit_card))
            purchase = create(:purchase, subscription:, credit_card: create(:credit_card),
                                         is_original_subscription_purchase: true)
  ...
  context_ "#update_rental_expired" do
  test "updates rental_expired field if is_rental is set to false" do
  ...
  context_ ".formatted_error_code" do
  test "falls back to purchase.stripe_error_code" do
        purchase = create(:purchase, stripe_error_code: "error_code")
        expect(purchase.formatted_error_code).to eq("Error Code")
      end

  test "falls back to purchase.error_code if purchase.stripe_error_code is empty" do
  ...
  test "displays corresponding stripe message for purchase.stripe_error_code" do
        purchase = create(:purchase,
  ...
  test "displays corresponding paypal message for purchase.stripe_error_code" do
        purchase = create(:purchase,
  ...
  test "displays corresponding paypal message for purchase.stripe_error_code in case of paypal connect" do
        purchase = create(:purchase,
  ...
  context_ "#has_payment_error?" do
  test "returns true if stripe_error_code is present" do
        purchase = build(:purchase, stripe_error_code: "foo")
        expect(purchase.has_payment_error?).to eq true
      end

  test "returns true if error_code is a payment error code" do
  ...
  test "returns true if the purchase has failed" do
        purchase = build(:purchase, purchase_state: "failed")
        expect(purchase.has_payment_error?).to eq true
      end

  test "returns false if error_code is a non-payment error code" do
        purchase = build(:purchase, error_code: "foo")
        expect(purchase.has_payment_error?).to eq false
      end

  test "returns false if error_code and stripe_error_code are not present" do
        purchase = build(:purchase)
        expect(purchase.has_payment_error?).to eq false
      end
    end

  context_ "#has_payment_network_error?" do
  test "returns true if error_code is a STRIPE_UNAVAILABLE error" do
  ...
  test "returns true if error_code is a PAYPAL_UNAVAILABLE error" do
        purchase = build(:purchase, error_code: PurchaseErrorCode::PAYPAL_UNAVAILABLE)
        expect(purchase.has_payment_network_error?).to eq true
      end

  test "returns true if stripe_error_code is a PROCESSING_ERROR error" do
        purchase = build(:purchase, stripe_error_code: PurchaseErrorCode::PROCESSING_ERROR)
        expect(purchase.has_payment_network_error?).to eq true
      end

  test "returns false if error_code or stripe_error_code are other errors" do
        purchase = build(:purchase, error_code: "foo")
  ...
  test "returns false if error_code and stripe_error_code are absent" do
        purchase = build(:purchase)
        expect(purchase.has_payment_network_error?).to eq false
      end
    end

  context_ "#has_retryable_payment_error?" do
  test "returns true if stripe_error_code is STRIPE_INSUFFICIENT_FUNDS error" do
  ...
  test "returns false if stripe_error_code is a different error" do
        purchase = build(:purchase, stripe_error_code: PurchaseErrorCode::PROCESSING_ERROR)
        expect(purchase.has_retryable_payment_error?).to eq false
      end

  test "returns false is stripe_error_code is nil" do
        purchase = build(:purchase)
        expect(purchase.has_retryable_payment_error?).to eq false
      end
    end
  ...
  context_ "#tiers" do
  context_ "for a non-tiered membership purchase" do
  ...
  context_ "that is associated with a tier" do
  test "returns an array containing the tier" do
            purchase = create(:purchase, link: @product, variant_attributes: [@second_tier])
            expect(purchase.tiers).to eq [@second_tier]
          end
        end

  context_ "that is not associated with a tier" do
  test "returns an array containing the default tier" do
            purchase = create(:purchase, link: @product)
  ...
  context_ "#show_view_content_button_on_product_page?" do
  test "returns true for a tiered membership product with a url redirect" do
        purchase = create(:membership_purchase)
        purchase.create_url_redirect!
  ...
  test "returns true if product has attached files" do
  ...
  test "returns true even if product has no attached files" do
  ...
  context_ "#downcase_email" do
  test "downcases the email when validating" do
  ...
  context_ "charge_card!" do
  test "adds proper error code to purchase if creator's paypal account is restricted and cannot accept payment" do
  ...
  context_ "#original_purchase" do
  test "returns the original subscription purchase" do
        original_purchase = create(:membership_purchase, is_archived_original_subscription_purchase: true)
  ...
  test "returns itself when not a subscription" do
        purchase = create(:purchase)
        expect(purchase.original_purchase).to eq(purchase)
      end

  test "returns itself when it's the original purchase" do
        purchase = create(:membership_purchase)
        expect(purchase.original_purchase).to eq(purchase)
      end
    end

  context_ "#true_original_purchase" do
  test "returns the (true) original subscription purchase" do
        original_purchase = create(:membership_purchase, is_archived_original_subscription_purchase: true)
        purchase = create(:purchase, link: original_purchase.link, subscription: original_purchase.subscription, is_original_subscription_purchase: true)
  ...
  context_ "double charge check for the same product on the same IP address with same email" do
      before do
        @product = create(:product)
        @params = { link: @product, ip_address: "1.1.1.1", email: "gumroad@example.com" }
      end

  context_ "when product doesn't have variants" do
        before do
          create(:purchase, @params)
        end

  test "doesn't create duplicate purchase" do
          expect do
            create(:purchase, @params)
  ...
  context_ "after 3 minutes from previous purchase" do
  test "allows to create duplicate purchase" do
  ...
  context_ "when product has variants" do
        before do
          variant_category = create(:variant_category, link: @product)
  ...
  test "allows to create purchases when the previously bought product is of a different variant" do
          expect do
  ...
  test "doesn't create duplicate purchase when the previously bought product is of same variant" do
          expect do
            create(:purchase, @params.merge(variant_attributes: [@variant1]))
          end.to raise_error(ActiveRecord::RecordInvalid).with_message(/You have already paid for this product. It has been emailed to you./)
        end
  ...
  context_ "after 3 minutes from previous purchase" do
  test "allows to create duplicate purchase of same variant" do
            travel_to(3.minutes.from_now) do
              expect do
                create(:purchase, @params.merge(variant_attributes: [@variant1]))
              end.to change { Purchase.successful.count }.by(1)
  ...
  context_ "#not_charged_and_not_free_trial?" do
  context_ "with not_charged purchase state" do
  test "returns true for a non-free trial purchase" do
          purchase = build(:purchase, purchase_state: "not_charged")
          expect(purchase.not_charged_and_not_free_trial?).to eq true
        end

  test "returns false for a free trial purchase" do
  ...
  context_ "with other purchase states" do
  ...
  context_ "when amount is in the thousands" do
  ...
  context_ "for JPY (single-unit currency)" do
  ...
  test "converts JPY partial refund amount to correct USD cents" do
          expect(purchase.refunding_amount_cents("5764")).to eq((5764 / 153.3446 * 100).round)
        end

  test "converts JPY full refund amount to match price_cents" do
  ...
  context_ "#original_offer_code" do
  ...
  context_ "when the offer code was deleted and include_deleted is true" do
  test "uses the cached offer code details" do
  ...
  test "returns offer_code if the offer_code is not deleted" do
        expect(purchase_with_valid_offer_code.original_offer_code).to eq offer_code
      end

  test "uses the cached offer code details if present" do
        purchase_with_valid_offer_code.create_purchase_offer_code_discount(offer_code:, offer_code_amount: 50, offer_code_is_percent: true, pre_discount_minimum_price_cents: 1800)
  ...
  test "uses the offer code if the purchase is missing cached offer code details" do
        expect(purchase_with_valid_offer_code.displayed_price_cents_before_offer_code).to eq 1300
      end
    end

  context_ "#displayed_price_cents_before_offer_code" do
      let(:product) { create(:product, price_cents: 500) }

  test "returns the displayed_price_cents for a purchase with no offer code" do
        purchase = build(:purchase, link: product)
        expect(purchase.displayed_price_cents_before_offer_code).to eq 500
      end

  context_ "with an offer code" do
  ...
  context_ "for a 100% off offer code" do
          before do
  ...
  test "returns nil if the purchase is missing cached offer code details" do
  ...
  context_ "#set_price_and_rate" do
      let(:seller) { create(:user) }
  ...
  test "caches the buyer-specific tiered discount amount" do
  ...
  test "rejects an existing-customer discount when the buyer does not qualify" do
        purchase = build(:purchase, purchaser: buyer, link: product, seller:, offer_code:)
  ...
  test "rejects a tiered discount when no tier matches the purchase" do
  ...
  test "keeps the existing-customer discount error when the purchase is saved" do
        purchase = build(:purchase, purchaser: buyer, link: product, seller:, offer_code:)
  ...
  context_ "has many `purchase_integrations`" do
  test "returns alive and deleted purchase_integrations" do
          expect do
  ...
  context_ "has many `live_purchase_integrations`" do
  test "does not return deleted purchase_integrations" do
          expect do
  ...
  context_ "#transcode_product_videos" do
      before do
  ...
  context_ "when product.transcode_videos_on_purchase is disabled" do
        before do
          @product.transcode_videos_on_purchase = false
          @product.save!
        end

  test "doesn't transcode videos on purchase" do
          @purchase.mark_successful!
  ...
  context_ "when product.transcode_videos_on_purchase is enabled" do
        before do
          @product.enable_transcode_videos_on_purchase!
        end

  test "transcodes videos and sets product.transcode_videos_on_purchase to false" do
          @purchase.mark_successful!
  ...
  context_ "#formatted_affiliate_credit_amount" do
      before { allow_any_instance_of(Purchase).to receive(:get_rate).and_return("0.8") }

  test "returns the formatted affiliate credit amount in USD" do
  ...
  context_ "#format_price_in_currency" do
      before { allow_any_instance_of(Purchase).to receive(:rate_converted_to_usd).and_return(0.5) }

  test "formats the amount in the purchase's currency" do
        purchase = create(:purchase)
  ...
  test "formats the amount for a subscription purchase in the purchase's currency" do
        purchase = create(:membership_purchase)
  ...
  context_ "#enqueue_update_sales_related_products_infos_job" do
      let(:product) { create(:product) }

  context_ "when the product stats has been backfilled" do
  test "enqueues UpdateSalesRelatedProductsInfosJob upon purchase success" do
  ...
  context_ "when there is a shipping fee" do
  ...
  context_ "when there is a price" do
        let(:purchase) { create(:purchase) }
  ...
  test "adds member when successful" do
  ...
  test "removes member when uncontactable" do
        purchase = create(:purchase)
  ...
  test "removes member when uncontactable with no other audience types" do
        purchase = create(:purchase)
        expect do
          purchase.update!(can_contact: false)
        end.to change(AudienceMember, :count).by(-1)
  ...
  test "removes member when subscription is deactivated" do
        purchase = create(:membership_purchase)
  ...
  test "recreates member when changing email" do
        purchase = create(:purchase)
  ...
  context_ "purchasing power parity validations" do
  context_ "when the card country doesn't match the IP country" do
  ...
  context_ "when the seller has payment method verification disabled" do
          before do
  ...
  context_ "#prepare_merchant_account" do
  test "adds an error if merchant account is a Brazilian Stripe Connect account and purchase has an affiliate" do
  ...
  context_ "#giftee_name_or_email" do
      let(:purchase) { create(:purchase) }

  context_ "for a non-gift purchase" do
  test "returns nil" do
          expect(purchase.giftee_name_or_email).to be_nil
        end
      end

  context_ "when the gift email is not hidden" do
  ...
  test "returns the gift email" do
          expect(purchase.giftee_name_or_email).to eq "giftee@example.com"
        end
      end

  context_ "when the gift email is hidden" do
  context_ "for a gifter purchase" do
  ...
  test "returns the giftee's name" do
            expect(purchase.giftee_name_or_email).to eq "Gift User"
          end
        end

  context_ "for a giftee purchase" do
  ...
  context_ "#json_data_for_mobile" do
      before do
  ...
  test "returns purchase information" do
  ...
  context_ "price validation" do
  context_ "purchase is a bundle product purchase" do
        let(:purchase) { create(:purchase, is_bundle_product_purchase: true, price_cents: 0) }

  test "doesn't add an error when the price is 0 for a non-free product" do
          expect(purchase.errors).to be_empty
        end
      end
    end

  context_ "#display_referrer" do
      let!(:purchase) { create(:purchase) }

  context_ "library purchase" do
        before { purchase.update!(recommended_by: RecommendationType::GUMROAD_LIBRARY_RECOMMENDATION) }

  test "returns the correct referrer" do
          expect(purchase.display_referrer).to eq("Gumroad Library")
        end
      end

  context_ "discover purchase" do
  ...
  context_ "common referrer purchase" do
  ...
  context_ "normal referrer purchase" do
  ...
  context_ "receipt recommendation" do
        before do
          purchase.update!(
            was_product_recommended: true,
  ...
  test "returns 'Gumroad receipt'" do
          expect(purchase.display_referrer).to eq("Gumroad Receipt")
        end
      end

  context_ "product page recommendation" do
        before do
          purchase.update!(
            was_product_recommended: true,
  ...
  test "returns 'Gumroad product page'" do
          expect(purchase.display_referrer).to eq("Gumroad Product Page")
        end
      end

  context_ "wishlist recommendation" do
        before do
          purchase.update!(
            was_product_recommended: true,
  ...
  test "returns 'Gumroad wishlist'" do
          expect(purchase.display_referrer).to eq("Gumroad Wishlist")
        end
      end

  context_ "unknown recommendation type" do
        before do
          purchase.update!(
            was_product_recommended: true,
  ...
  context_ "more like this recommendation" do
        before do
          purchase.update!(
            was_product_recommended: true,
  ...
  context_ "non-PPP-discounted purchase" do
  test "returns nil" do
  ...
  context_ "#linked_license" do
  test "returns the linked license" do
  ...
  context_ "#build_flow_of_funds_from_combined_charge", :vcr do
      before do
  ...
  test "returns a flow of funds object for the purchase with proper amounts based on purchase's portion in the charge" do
  ...
  test "returns a proper amounts based on purchase's portion in the charge when combined flow of funds has negative amounts" do
        combined_flow_of_funds = FlowOfFunds.new(
          issued_amount: FlowOfFunds::Amount.new(currency: Currency::USD, cents: -100_00),
          settled_amount: FlowOfFunds::Amount.new(currency: Currency::CAD, cents: -125_00),
          gumroad_amount: FlowOfFunds::Amount.new(currency: Currency::USD, cents: -10_00),
  ...
  test "returns proper amounts based on purchase's portion in the charge when some purchases have affiliate fees" do
        combined_flow_of_funds = FlowOfFunds.new(
            issued_amount: FlowOfFunds::Amount.new(currency: Currency::USD, cents: -100_00),
            settled_amount: FlowOfFunds::Amount.new(currency: Currency::CAD, cents: -125_00),
  ...
  context_ "#mandate_options_for_stripe" do
  test "returns nil for PayPal and Braintree purchases" do
  ...
  test "returns nil for Stripe purchases if card country is not India" do
        product = create(:membership_product)
        subscription = create(:subscription, link: product)
  ...
  test "returns nil for a multi buy purchase" do
  ...
  test "returns nil for purchases that do not require future off-session charges" do
        allow_any_instance_of(StripeChargeablePaymentMethod).to receive(:country).and_return("IN")
  ...
  test "returns correct parameter to create a mandate on Stripe for purchases that require off-session charges" do
        allow_any_instance_of(StripeChargeablePaymentMethod).to receive(:country).and_return("IN")
  ...
  context_ "#is_an_off_session_charge_on_indian_card?" do
  context_ "when card country is not India" do
  test "returns false if it is a regular purchase" do
          expect(create(:purchase_in_progress).is_an_off_session_charge_on_indian_card?).to be false
        end

  test "returns false if it is a membership purchase" do
  ...
  test "returns false if it is a recurring charge and charge processor is not Stripe" do
          product = create(:subscription_product)
          subscription = create(:subscription, link: product)
          create(:purchase, subscription:, is_original_subscription_purchase: true)
  ...
  test "returns false if it is a preorder charge and charge processor is not Stripe" do
  ...
  test "returns false if it is a recurring charge and charge processor is Stripe" do
          product = create(:subscription_product)
          subscription = create(:subscription, link: product)
          create(:purchase, subscription:, is_original_subscription_purchase: true)
  ...
  test "returns false if it is a preorder charge and charge processor is Stripe" do
          product = create(:product, is_in_preorder_state: true)
          preorder_link = create(:preorder_link, link: product)
          authorization_purchase = create(:preorder_authorization_purchase, link: product)
          preorder = preorder_link.build_preorder(authorization_purchase)
  ...
  context_ "when card country is India" do
  test "returns false if it is a regular purchase" do
  ...
  test "returns false if it is a preorder charge but charge processor is not Stripe" do
          product = create(:product, is_in_preorder_state: true)
          preorder_link = create(:preorder_link, link: product)
          authorization_purchase = create(:preorder_authorization_purchase, link: product)
          preorder = preorder_link.build_preorder(authorization_purchase)
  ...
  test "returns true if it is a recurring charge" do
          product = create(:subscription_product)
          subscription = create(:subscription, link: product)
          create(:purchase, subscription:, is_original_subscription_purchase: true)
  ...
  test "returns true if it is a preorder charge" do
          product = create(:product, is_in_preorder_state: true)
          preorder_link = create(:preorder_link, link: product)
          authorization_purchase = create(:preorder_authorization_purchase, link: product)
          preorder = preorder_link.build_preorder(authorization_purchase)
  ...
  context_ "#can_force_update?" do
  test "returns true if purchase is in progress and is not an off session charge on Indian card" do
        expect(create(:purchase_in_progress).can_force_update?).to be true
      end

  test "returns false if purchase is not in progress" do
  ...
  test "returns true if an off session charge on Indian card is in progress and was not created in the last 26 hours" do
  ...
  test "returns false if an off session charge on Indian card is in progress and was created in the last 26 hours" do
        allow_any_instance_of(Purchase).to receive(:is_an_off_session_charge_on_indian_card?).and_return true

        expect(create(:purchase_in_progress, created_at: 10.hours.ago).can_force_update?).to be false
      end

  test "returns false if an off session charge on Indian card is not in progress" do
        allow_any_instance_of(Purchase).to receive(:is_an_off_session_charge_on_indian_card?).and_return true
  ...
  context_ "#save_charge_data" do
  test "saves all charge related info from the given charge on the purchase" do
  ...
  test "calls update_charge_details_from_processor! on the assoiated charge" do
        stripe_charge = ChargeProcessor.get_charge(StripeChargeProcessor.charge_processor_id, "ch_2OTlIf9e1RjUNIyY1adIdtGp")
  ...
  test "returns false when stripe_refunded is nil or false" do
        purchase = create(:purchase, stripe_refunded: nil)
  ...
  context_ "#amount_refundable_cents_in_currency" do
      let(:purchase) { create(:purchase, link: create(:product, price_currency_type: Currency::EUR), price_cents: 200) }
  ...
  test "returns the refundable amount in the purchase's currency" do
        expect(purchase.amount_refundable_cents_in_currency).to eq(160)
      end

  context_ "when the product has been deleted" do
        before { purchase.link.destroy! }

  test "uses displayed_price_currency_type from the purchase record" do
          expect(purchase.reload.amount_refundable_cents_in_currency).to eq(160)
        end
      end
    end

  context_ "#shipping_information" do
  ...
  test "returns the shipping information" do
  ...
  context_ "require_shipping is false for the product" do
        before do
          purchase.link.update!(require_shipping: false)
        end

  test "returns an empty object" do
          expect(purchase.shipping_information).to eq({})
        end
      end

  context_ "when a value is nil" do
        before do
  ...
  context_ "#name_or_email" do
      let(:purchase) { create(:purchase) }

  context_ "full name is nil" do
  test "returns the email" do
          expect(purchase.name_or_email).to eq(purchase.email)
        end
      end

  context_ "full name is empty" do
  ...
  context_ "full name is present" do
  ...
  context_ "#prepare_for_charge!" do
  context_ "in a country with taxes" do
  ...
  test "calculates taxes" do
          purchase.prepare_for_charge!
          expect(purchase.gumroad_tax_cents).to eq 20
        end

  test "does not apply taxes if merchant account is a Brazilian Stripe Connect account" do
  ...
  test "returns the commission for the deposit and completion purchases" do
  ...
  context_ "when the purchase has no associated commission" do
        let(:purchase) { create(:purchase) }
  ...
  context_ "#eligible_for_review_reminder?" do
  ...
  context_ "when purchaser has opted out of review reminders" do
  ...
  context_ "when purchase is a bundle purchase" do
  ...
  context_ "when product review exists" do
  ...
  context_ "when purchase is not successful" do
  ...
  context_ "when purchaser is nil" do
  ...
  context_ "when the purchase is a gifted subscription" do
  ...
  test "returns the license of the gifted purchase" do
          gifted_license = create(:license, purchase: gifted_purchase)
          expect(gifted_purchase.license).to eq(gifted_license)
        end
      end

  context_ "when the purchase is not a gifted subscription" do
        let(:purchase) { create(:purchase, subscription:) }

  test "returns the license of the original purchase" do
  ...
  context_ "#call" do
  context_ "when purchasing a call" do
        let(:subject) { build(:call_purchase) }

        it { is_expected.to validate_presence_of(:call) }

  test "marks the purchase as invalid if the call is not valid" do
  ...
  context_ "when not purchasing a call" do
  ...
  context_ "#determine_affiliate_fee_cents" do
      let(:product) { create(:product, price_cents: 10_00) }
  ...
  test "returns affiliate's share of the fee" do
  ...
  test "returns 0 if seller bears affiliate fees" do
  ...
  context_ "#gift_purchases_cannot_be_on_installment_plans" do
  test "does not allow gift purchases to be on installment plans" do
  ...
  context_ "within_refund_policy_timeframe?" do
      let(:purchase) { create(:purchase) }
  ...
  context_ "when purchase is not successful or gift receiver purchase was not successful or not in not_charged state" do
  test "returns false" do
  ...
  context_ "when purchase is refunded or chargedback" do
  test "returns false" do
  ...
  context_ "when there is no refund policy" do
  test "returns false" do
          allow(purchase).to receive(:successful?).and_return(true)
          allow(purchase).to receive(:refunded?).and_return(false)
  ...
  context_ "when refund policy max_refund_period_in_days is nil or <= 0" do
  test "returns false" do
          allow(purchase).to receive(:successful?).and_return(true)
          allow(purchase).to receive(:refunded?).and_return(false)
          allow(purchase).to receive(:chargedback?).and_return(false)
  ...
  context_ "when the purchase is within the refund policy timeframe" do
  test "returns true" do
          allow(purchase).to receive(:successful?).and_return(true)
          allow(purchase).to receive(:refunded?).and_return(false)
          allow(purchase).to receive(:chargedback?).and_return(false)
  ...
  context_ "when the purchase is outside the refund policy timeframe" do
  test "returns false" do
          allow(purchase).to receive(:successful?).and_return(true)
          allow(purchase).to receive(:refunded?).and_return(false)
          allow(purchase).to receive(:chargedback?).and_return(false)
  ...
  context_ "#calculate_custom_fee_per_thousand" do
  test "does nothing and returns if custom fee is already set" do
  ...
  test "does nothing and returns if discover fee is being charged" do
        seller = create(:user)
  ...
  context_ "for a recurring charge" do
        let!(:subscription) { create(:subscription) }
        let!(:original_subscription_purchase) { create(:purchase, subscription:, is_original_subscription_purchase: true) }

  context_ "when original subscription purchase had a custom fee" do
  test "sets custom fee same as original subscription purchase's custom fee" do
  ...
  context_ "when original subscription purchase did not have a custom fee but the seller has one set" do
  test "falls back to the seller's custom fee" do
            recurring_purchase = create(:purchase, subscription:, is_original_subscription_purchase: false)
  ...
  context_ "when neither the original subscription purchase nor the seller has a custom fee" do
  test "does not set a custom fee" do
            expect(original_subscription_purchase.custom_fee_per_thousand).to be_nil
  ...
  context_ "for a preorder charge" do
        let!(:seller) { create(:user) }
  ...
  context_ "when preorder authorization purchase had a custom fee" do
  test "sets custom fee same as preorder authorization purchase's custom fee" do
  ...
  context_ "when preorder authorization purchase did not have a custom fee" do
  test "does not set a custom fee" do
            preorder.authorize!
            preorder.mark_authorization_successful
            product.update!(is_in_preorder_state: false)
  ...
  context_ "for a regular purchase" do
        let!(:seller) { create(:user) }
        let!(:product) { create(:product, user: seller) }

  context_ "when seller has a custom fee set" do
  test "sets custom fee same as seller's custom fee" do
            seller.update!(custom_fee_per_thousand: 50)
            purchase = create(:purchase, link: product)
            expect(purchase.custom_fee_per_thousand).to eq(50)
  ...
  context_ "when seller does not have a custom fee set" do
  test "does not set a custom fee" do
            purchase = create(:purchase, link: product)
  ...
  context_ "for a new subscription purchase" do
        let!(:seller) { create(:user) }
  ...
  context_ ".validate_offer_code_usage_across_line_items" do
      let(:seller) { create(:user) }
  ...
  test "fails every line item when the cart collectively exceeds max_purchase_count" do
  ...
  test "leaves line items alone when the cart fits within max_purchase_count" do
  ...
  test "skips single-line carts (already handled by per-purchase validation)" do
  ...
  context_ "#auto_delete_single_use_offer_code" do
      before do
        @user = create(:user)
        @product = create(:product, user: @user, price_cents: 1000)
      end

  test "auto-deletes a single-use offer code after a successful purchase" do
  ...
  test "does not auto-delete offer codes for non-successful purchases" do
        offer_code = create(:offer_code, products: [@product], max_purchase_count: 1)
  ...
  test "does not auto-delete when the purchase has no offer code" do
        purchase = create(:purchase, link: @product, seller: @user, price_cents: @product.price_cents)

        expect(purchase).to be_persisted
      end

  test "does not auto-delete multi-use offer codes" do
  ...
  test "does not break the purchase flow if auto-delete raises an error" do
        offer_code = create(:offer_code, products: [@product], max_purchase_count: 1)
