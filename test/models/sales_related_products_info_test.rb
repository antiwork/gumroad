...
class SalesRelatedProductsInfoTest < ActiveSupport::TestCase
  self.described_class = SalesRelatedProductsInfo



  context_ SalesRelatedProductsInfo do
  context_ ".find_or_create_info" do
      let(:sales_related_products_info) { create(:sales_related_products_info) }

  context_ "when the info exists" do
  test "returns the info" do
  ...
  context_ "when the info does not exist" do
        let(:product1) { create(:product) }
        let(:product2) { create(:product) }

  test "creates the info" do
          expect do
  ...
  context_ ".update_sales_counts" do
  test "upserts and increments/decrements the sales counts" do
  ...
  context_ ".related_products" do
  test "returns related products sorted in descending order by sales count" do
  ...
  test "validates the arguments" do
        expect do
  ...
  context_ ".related_product_ids_and_sales_counts" do
  test "validates the arguments" do
        expect do
  ...
  test "returns a hash of related products and sales counts" do
  ...
  context_ "when smaller_product_id is greater than larger_product_id" do
  ...
  context_ "when smaller_product_id is equal to larger_product_id" do
  test "adds an error" do
          expect(build(:sales_related_products_info, smaller_product:, larger_product: smaller_product)).not_to be_valid
        end
      end

  context_ "when smaller_product_id is less than larger_product_id" do
  ...
  context_ "scopes" do
  context_ ".for_product_id" do
  test "returns matching records for a product id" do
