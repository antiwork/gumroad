...
class RichContentTest < ActiveSupport::TestCase
  self.described_class = RichContent



  context_ RichContent do
  context_ "validations" do
  context_ "description" do
  context_ "invalid descriptions" do
  ...
  context_ "valid descriptions" do
  ...
  context_ "#embedded_product_file_ids_in_order" do
      let(:product) { create(:product) }
      let(:rich_content) { create(:product_rich_content, entity: product) }

  test "returns the ids of the embedded product files in order" do
  ...
  context_ "#has_license_key?" do
      let(:product) { create(:product) }

  test "returns false if it does not contain license key" do
  ...
  test "returns true if it contains license key" do
  ...
  test "returns true if it contains license key nested inside a list item" do
  ...
  test "returns true if it contains license key nested inside a blockquote" do
  ...
  context_ "#has_posts?" do
      let(:product) { create(:product) }

  test "returns false if it does not contain posts" do
        rich_content = create(:rich_content, entity: product, description: [{ "type" => "paragraph", "content" => [{ "type" => "text", "text" => "Hello" }] }])
        expect(rich_content.has_posts?).to be(false)
      end

  test "returns true if it contains posts" do
        rich_content = create(:rich_content, entity: product, description: [{ "type" => "posts" }])
        expect(rich_content.has_posts?).to be(true)
      end

  test "returns true if it contains posts nested inside a list item" do
  ...
  test "returns true if it contains posts nested inside a blockquote" do
  ...
  context_ "#custom_field_nodes" do
      let(:product) { create(:product) }
  ...
  test "parses out deeply nested custom field nodes in order" do
        description = [
          {
            "type" => "orderedList",
            "attrs" => { "start" => 1 },
