...
class CommissionTest < ActiveSupport::TestCase
  self.described_class = Commission
  self.rspec_metadata = { vcr: true }


  context_ Commission, :vcr do
  ...
  test "validates presence of deposit_purchase" do
  ...
  test "validates that deposit_purchase and completion_purchase are different" do
  ...
  test "validates that deposit_purchase and completion_purchase belong to the same commission" do
  ...
  test "validates that the purchased product is a commission" do
  ...
  context_ "#create_completion_purchase!" do
  context_ "when status is already completed" do
        let!(:commission) { create(:commission, status: Commission::STATUS_COMPLETED) }

  test "does not create a completion purchase" do
          expect { commission.create_completion_purchase! }.not_to change { Purchase.count }
        end
      end

  context_ "when status is not completed" do
  ...
  test "creates a completion purchase with correct attributes, processes it, and updates status" do
  ...
  context_ "when the completion purchase fails" do
  test "marks the purchase as failed" do
  ...
  context_ "when the product price changes after the deposit purchase" do
  test "creates a completion purchase with the original price" do
  ...
  context_ "when the product adds a new variant after the deposit purchase" do
  ...
  test "creates a completion purchase without any variant attributes" do
  ...
  context_ "when the purchased variant has changed since the deposit purchase" do
        let!(:product) { create(:commission_product, price_cents: 1000) }
  ...
  context_ "variant price changed" do
  test "creates a completion purchase with the original price" do
  ...
  context_ "variant soft deleted" do
  test "creates a completion purchase with the original price" do
            expect(deposit_purchase.price_cents).to eq(1000)
  ...
  context_ "when the deposit purchase used a discount code" do
  ...
  context_ "offer code has been soft deleted" do
  test "creates a completion purchase with the original price" do
            expect(deposit_purchase.price_cents).to eq(500)
  ...
  context_ "offer code is single-use" do
  test "creates a completion purchase with the original price" do
            expect(deposit_purchase.price_cents).to eq(500)
  ...
  context_ "when the deposit purchase has PPP discount applied" do
        before do
  ...
  test "creates a completion purchase with PPP discount applied" do
  ...
  context_ "#completion_price_cents" do
  ...
  test "returns the correct completion price" do
        expect(commission.completion_price_cents).to eq(5000)
      end
    end

  context_ "statuses" do
      let(:commission) { build(:commission) }

  context_ "#is_in_progress?" do
  test "returns true if the status is in_progress" do
          commission.status = Commission::STATUS_IN_PROGRESS
          expect(commission.is_in_progress?).to be true
        end

  test "returns false if the status is completed or cancelled" do
  ...
  context_ "#is_completed?" do
  test "returns true if the status is completed" do
          commission.status = Commission::STATUS_COMPLETED
          expect(commission.is_completed?).to be true
        end

  test "returns false if the status is in_progress or cancelled" do
          commission.status = Commission::STATUS_IN_PROGRESS
  ...
  context_ "#is_cancelled?" do
  test "returns true if the status is cancelled" do
          commission.status = Commission::STATUS_CANCELLED
          expect(commission.is_cancelled?).to be true
        end

  test "returns false if the status is in_progress or completed" do
          commission.status = Commission::STATUS_IN_PROGRESS
  ...
  context_ "#completion_display_price_cents" do
