...
class WorkflowTest < ActiveSupport::TestCase
  self.described_class = Workflow



  context_ Workflow do
    before do
  ...
  test "returns only published workflows" do
  ...
  context_ "#applies_to_purchase?" do
  context_ "for a product workflow" do
        let(:workflow) { create(:workflow) }

  test "returns true for a purchase of the workflow product" do
          purchase = create(:purchase, link: workflow.link)
          expect(workflow.applies_to_purchase?(purchase)).to eq true
        end

  test "returns false for a purchase of the workflow product that fails filters" do
          purchase = create(:purchase, link: workflow.link)
  ...
  test "returns false for a purchase of a different product" do
          purchase = create(:purchase)
          expect(workflow.applies_to_purchase?(purchase)).to eq false
        end
      end

  context_ "for a variant workflow" do
        let(:workflow) { create(:variant_workflow) }

  test "returns true for a purchase of the workflow variant" do
          purchase = create(:purchase, variant_attributes: [workflow.base_variant])
          expect(workflow.applies_to_purchase?(purchase)).to eq true
        end

  test "returns false for a purchase of the workflow variant that fails filters" do
          purchase = create(:purchase, variant_attributes: [workflow.base_variant])
          allow(workflow).to receive(:purchase_passes_filters).with(purchase).and_return(false)
          expect(workflow.applies_to_purchase?(purchase)).to eq false
        end

  test "returns false for a purchase of a different variant" do
          purchase = create(:purchase, variant_attributes: [create(:variant)])
          expect(workflow.applies_to_purchase?(purchase)).to eq false
        end
      end

  context_ "for a seller workflow" do
        let(:workflow) { create(:seller_workflow) }
        let(:purchase) { create(:purchase) }

  test "returns true for a purchase that passes the workflow filters" do
          allow(workflow).to receive(:purchase_passes_filters).with(purchase).and_return(true)
          expect(workflow.applies_to_purchase?(purchase)).to eq true
        end

  test "returns false for apurchase that fails the workflow filters" do
          allow(workflow).to receive(:purchase_passes_filters).with(purchase).and_return(false)
          expect(workflow.applies_to_purchase?(purchase)).to eq false
        end
      end
  ...
  context_ "#targets_variant?" do
  context_ "for a variant workflow" do
        let(:workflow) { create(:variant_workflow) }

  test "returns true if it targets the given variant" do
          expect(workflow.targets_variant?(workflow.base_variant)).to eq true
        end

  test "returns false if it targets a different variant" do
          expect(workflow.targets_variant?(create(:variant))).to eq false
        end
      end

  context_ "for a workflow with bought_variants set" do
  ...
  test "returns false if it does not target the given variant" do
          expect(workflow.targets_variant?(create(:variant))).to eq false
        end
      end

  context_ "for a non-variant workflow without bought_variants set" do
  ...
  test "marks workflow and installments as deleted" do
        seller = create(:user)
  ...
  context_ "#schedule_installment", :freeze_time do
  test "does nothing when the workflow is not published" do
        @workflow.mark_deleted!
        @workflow.schedule_installment(@post)
        expect(SendWorkflowPostEmailsJob.jobs).to be_empty
      end

  test "does nothing when the post is not published" do
        @post.unpublish!
        @workflow.schedule_installment(@post)
        expect(SendWorkflowPostEmailsJob.jobs).to be_empty
      end

  test "does nothing when workflow has a member_cancellation trigger and send_to_past_customers is false" do
  ...
  context_ "when workflow has a member_cancellation trigger and send_to_past_customers is true" do
        before do
          @workflow.update!(workflow_trigger: Workflow::MEMBER_CANCELLATION_WORKFLOW_TRIGGER, send_to_past_customers: true)
        end

  test "enqueues SendWorkflowEmailsToPastCanceledMembersJob on first publish" do
          @workflow.schedule_installment(@post)
          expect(SendWorkflowEmailsToPastCanceledMembersJob).to have_enqueued_sidekiq_job(@post.id)
          expect(SendWorkflowPostEmailsJob.jobs).to be_empty
        end

  test "re-enqueues SendWorkflowEmailsToPastCanceledMembersJob when called from an installment rule edit so version-stale workers are replaced" do
  ...
  test "does nothing if the post is only for new recipients and it hasn't been scheduled before" do
        @post.update!(is_for_new_customers_of_workflow: true)
        @workflow.schedule_installment(@post)
        expect(SendWorkflowPostEmailsJob.jobs).to be_empty
      end

  test "does nothing for an abandoned cart installment" do
  ...
  context_ "workflow wasn't previously scheduled" do
  test "enqueues job with earliest_valid_time = nil" do
          @workflow.schedule_installment(@post)
          expect(SendWorkflowPostEmailsJob).to have_enqueued_sidekiq_job(@post.id, nil)
        end
      end

  context_ "workflow was previously scheduled (old_delayed_delivery_time != nil)" do
        let(:old_delayed_delivery_time) { 6.hours.to_i }

  context_ "post is for new recipients" do
          before { @post.update!(is_for_new_customers_of_workflow: true) }

  test "enqueues job with earliest_valid_time = post.published_at if the post was published after the old delay" do
  ...
  test "enqueues job with earliest_valid_time = old_delayed_delivery_time.seconds.ago if the post was published before the old delay" do
  ...
  context_ "post is not for new recipients" do
          before { @post.update!(is_for_new_customers_of_workflow: false) }

  test "enqueues job with earliest_valid_time = old_delayed_delivery_time.seconds.ago" do
            @workflow.schedule_installment(@post, old_delayed_delivery_time:)
            expect(SendWorkflowPostEmailsJob).to have_enqueued_sidekiq_job(@post.id, old_delayed_delivery_time.seconds.ago.iso8601)
          end
        end
  ...
  context_ "#add_and_validate_filters" do
  ...
  test "does nothing if the workflow is already published" do
  ...
  test "sets 'published_at' and 'first_published_at'" do
        expect do
          @workflow.publish!
  ...
  test "does not update 'first_published_at' if it is already set" do
  ...
  test "publishes and schedules all alive installments" do
  ...
  test "raises an error if the seller is not eligible to send emails" do
  ...
  test "does nothing if the workflow is already unpublished" do
        expect { @workflow.unpublish! }.not_to change { @workflow.reload }
      end

  test "sets 'published_at' to nil and does not change 'first_published_at'" do
        @workflow.update!(published_at: 1.day.ago, first_published_at: 2.days.ago)
  ...
  test "unpublishes all alive installments" do
  ...
  context_ "#has_never_been_published?" do
  test "returns true if the workflow has never been published" do
        expect(@workflow.has_never_been_published?).to be(true)
      end

  test "returns false if the workflow has been published before" do
