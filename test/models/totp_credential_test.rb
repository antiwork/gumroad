...
class TotpCredentialTest < ActiveSupport::TestCase
  self.described_class = TotpCredential



  context_ TotpCredential do
  ...
  context_ "#totp_provisioning_uri" do
  ...
  context_ "#generate_recovery_codes" do
      let(:credential) { create(:totp_credential, :confirmed, user:) }

  test "returns 10 plaintext codes" do
  ...
  test "stores bcrypt hashes in recovery_codes" do
  ...
  test "sets recovery_codes_generated_at" do
        freeze_time do
          credential.generate_recovery_codes
  ...
  context_ "#redeem_recovery_code" do
      let(:credential) { create(:totp_credential, :confirmed, user:) }
      let!(:codes) { credential.generate_recovery_codes }

  test "returns true and removes a valid recovery code" do
  ...
  test "accepts codes with a dash" do
        code = codes.second
