...
class ScheduledPayoutTest < ActiveSupport::TestCase
  self.described_class = ScheduledPayout



  context_ ScheduledPayout do
  ...
  test "requires action to be one of refund, payout, hold" do
  ...
  test "requires status to be one of pending, executed, cancelled, flagged, held" do
  ...
  test "requires delay_days to be a non-negative integer" do
  ...
  test "requires payout_amount_cents to be a non-negative integer" do
  ...
  context_ "no_in_progress_scheduled_payout_for_user" do
        let(:user) { create(:user) }

        %w[pending flagged held].each do |status|
  test "is invalid when the user already has a #{status} scheduled payout" do
  ...
  test "is valid when the user's only existing scheduled payout is #{status}" do
            create(:scheduled_payout, user: user, status: status)
            scheduled_payout = build(:scheduled_payout, user: user)
  ...
  context_ "#set_scheduled_at" do
  test "sets scheduled_at from delay_days on create when not provided" do
        freeze_time do
  ...
  test "does not override scheduled_at if already set" do
  ...
  context_ "when action is refund" do
  ...
  test "enqueues RefundUnpaidPurchasesWorker and marks as executed" do
  ...
  test "raises if user is not suspended" do
  ...
  context_ "when action is payout" do
        let(:scheduled_payout) { create(:scheduled_payout, user: user, action: "payout", scheduled_at: 1.day.ago) }

  test "creates a payment via Payouts.create_payment and marks as executed" do
  ...
  test "raises if payout fails" do
  ...
  test "raises if no payment is created" do
          allow(Payouts).to receive(:create_payment)
            .with(Date.yesterday.to_s, user.current_payout_processor, user)
            .and_return([nil, nil])
  ...
  test "raises with payment errors when create_payment returns errors" do
          allow(Payouts).to receive(:create_payment)
            .with(Date.yesterday.to_s, user.current_payout_processor, user)
  ...
  test "raises if already executed" do
  ...
  context_ "#cancel!" do
  test "cancels a pending payout" do
  ...
  test "cancels a flagged payout" do
  ...
  context_ "#user_has_active_chargebacks?" do
      let(:user) { create(:user) }
      let(:product) { create(:product, user: user) }
      let(:scheduled_payout) { create(:scheduled_payout, user: user) }

  test "returns false when user has no chargebacks" do
        expect(scheduled_payout.user_has_active_chargebacks?).to be false
      end

  test "returns true when user has unreversed chargebacks" do
  ...
  context_ "#execute! with chargebacks" do
      let(:user) { create(:user) }
      let(:product) { create(:product, user: user) }

  test "flags for review and sends email when user has active chargebacks" do
