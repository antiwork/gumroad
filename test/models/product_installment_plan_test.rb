...
class ProductInstallmentPlanTest < ActiveSupport::TestCase
  self.described_class = ProductInstallmentPlan



  context_ ProductInstallmentPlan do
  ...
  context_ "with a tiered membership product" do
  ...
  context_ "with a preorder product" do
  ...
  context_ "with a physical product" do
  ...
  context_ "with a bundle product" do
  ...
  context_ "when all products in the bundle are eligible" do
          before do
  ...
  context_ "when some products in the bundle are not eligible" do
          before do
            bundle.bundle_products << build(:bundle_product, bundle: bundle, product: course_product)
  ...
  context_ "when the bundle has no products" do
  test "is valid" do
            expect(installment_plan.valid?).to be true
          end
        end
  ...
  context_ "with eligible product types" do
  ...
  context_ "installment payment price" do
  ...
  test "must be at least the minimum price for each installment" do
  ...
  context_ "when the base price is zero but variants have paid price differences" do
  ...
  test "is valid when variant prices satisfy the minimum installment price" do
          installment_plan.number_of_installments = 2
          expect(installment_plan).to be_valid
        end

  test "is not valid when variant prices are too low for the number of installments" do
  ...
  test "is valid when PWYW is on but paid variants exist" do
  ...
  context_ "#calculate_installment_payment_price_cents" do
      let(:installment_plan) { build(:product_installment_plan, number_of_installments: 3) }

  test "splits price evenly when divisible by number of installments" do
        result = installment_plan.calculate_installment_payment_price_cents(3000)
        expect(result).to eq([1000, 1000, 1000])
      end

  test "adds remainder to first installment when price not evenly divisible" do
        result = installment_plan.calculate_installment_payment_price_cents(3002)
        expect(result).to eq([1002, 1000, 1000])
      end
    end

  context_ "#destroy_if_no_payment_options!" do
      let(:product) { create(:product, price_cents: 99 * 2, price_currency_type: "usd") }
      let(:installment_plan) { build(:product_installment_plan, link: product, number_of_installments: 2) }

  context_ "with no payment options" do
  test "destroys the installment plan" do
  ...
  test "soft deletes the installment plan even if product is no longer eligible for installment plans" do
