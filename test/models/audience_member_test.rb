...
class AudienceMemberTest < ActiveSupport::TestCase
  self.described_class = AudienceMember
  self.rspec_metadata = { freeze_time: true }



  context_ AudienceMember, :freeze_time do
  context_ "validations" do
  test "validates json schema" do
  ...
  test "saving assigns derived columns" do
  ...
  test "filters by purchased and not-purchased products and variants" do
  ...
  test "deduplicates rows joined by json_table" do
  ...
  test "filters by creation dates" do
  ...
  test "filters by affiliate products" do
  ...
  context_ "with_ids" do
  test "returns the members, including the last record id matching the filters" do
  ...
  context_ ".refresh_all! and #refresh!" do
      let(:seller) { create(:user) }

  test "creates / updates / deletes members" do
