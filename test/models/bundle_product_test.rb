...
class BundleProductTest < ActiveSupport::TestCase
  self.described_class = BundleProduct



  context_ BundleProduct do
  context_ "#standalone_price_cents" do
      let(:bundle_product) { create(:bundle_product, quantity: 2) }

  test "returns the correct price" do
        expect(bundle_product.standalone_price_cents).to eq(200)
      end

  context_ "when the product has a variant" do
        before do
  ...
  test "returns the bundle products in order" do
  ...
  context_ "when the product doesn't belong to the bundle's user" do
        before do
  ...
  context_ "when the product is versioned but no variant is set" do
        before do
          bundle_product.product = create(:product_with_digital_versions, user: bundle_product.bundle.user)
        end
  ...
  context_ "when the product is versioned and a variant is set" do
        before do
          bundle_product.product = create(:product_with_digital_versions, user: bundle_product.bundle.user)
          bundle_product.variant = bundle_product.product.alive_variants.first
        end
  ...
  context_ "when the product has a single alive variant and no variant is set" do
        before do
          bundle_product.product = create(:product_with_digital_versions, user: bundle_product.bundle.user)
  ...
  context_ "when the product is a call" do
        before do
  ...
  context_ "when the bundle is a bundle" do
        before do
  ...
  context_ "when the bundle has an installment plan" do
          let!(:installment_plan) { create(:product_installment_plan, link: bundle) }

  context_ "when the product is eligible for installment plans" do
  test "doesn't add an error" do
  ...
  context_ "when the product is not eligible for installment plans" do
  test "adds an error" do
  ...
  context_ "when the bundle has no installment plan" do
  test "doesn't validate product eligibility for installment plans" do
            bundle_product = build(:bundle_product, bundle: bundle, product: ineligible_product)
            expect(bundle_product).to be_valid
          end
        end
