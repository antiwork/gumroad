...
class GuyanaBankAccountTest < ActiveSupport::TestCase
  self.described_class = GuyanaBankAccount


  context_ GuyanaBankAccount do
  ...
  test "joins the bank code and branch code with a dash" do
  ...
  test "requires exactly 11 alphanumeric characters" do
  ...
  test "requires exactly 8 digits" do
  ...
  context_ "validations on legacy records" do
  test "skips bank_code and branch_code checks when neither has changed" do
        ba = create(:guyana_bank_account)
  ...
  test "allows records that match the required account number regex" do
        ...
        expect(gy_bank_account).not_to be_valid
