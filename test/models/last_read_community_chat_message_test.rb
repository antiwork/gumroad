...
class LastReadCommunityChatMessageTest < ActiveSupport::TestCase
  self.described_class = LastReadCommunityChatMessage



  context_ LastReadCommunityChatMessage do
  ...
  context_ "when no record exists" do
  ...
  context_ "when record already exists" do
        before do
          create(
  ...
  context_ "when given message is newer than existing message" do
  ...
  context_ "when given message is older than existing message" do
          let(:message2) { create(:community_chat_message, community: community, created_at: 2.hours.ago) }

  test "does not update the record" do
            expect do
              described_class.set!(
                user_id: user.id,
                community_id: community.id,
  ...
  context_ "when last read record exists" do
        before do
          create(
            :last_read_community_chat_message,
            user:,
  ...
  test "returns count of messages newer than the last read message" do
  ...
  test "returns count using provided message if specified" do
          count = described_class.unread_count_for(
            user_id: user.id,
            community_id: community.id,
            community_chat_message_id: message2.id
  ...
  context_ "when no last read record exists" do
  test "returns count of all messages in the community" do
          count = described_class.unread_count_for(
            user_id: user.id,
            community_id: community.id
          )
  ...
  test "only counts alive messages" do
          count = described_class.unread_count_for(
            user_id: user.id,
            community_id: community.id
          )
