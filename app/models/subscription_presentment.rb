...
# The buyer-local currency and amount a subscription is billed in, fixed when the amount is
# agreed and then honoured until it is deliberately changed.
#
...
#
# Rows are IMMUTABLE and effective-dated, one per fixing rather than one per subscription. A
# subscription's price does legitimately change mid-life — Subscription#update_current_plan!
# handles upgrades, downgrades and quantity changes, and #current_subscription_price_cents
# moves when a fixed-duration discount expires — and each of those re-fixes the buyer-currency
# amount at that day's rate. Updating a single row in place would overwrite the rate the
# previous amount was fixed at, which is the baseline the absorbed drift is measured against,
# so a new row is written instead and the newest one is what a charge reads.
class SubscriptionPresentment < ApplicationRecord
  ...
  private
    ...
    # A row the charge path could never use is worse than no row: the subscription would look
    # like it had a buyer-currency price while every renewal silently charged canonical dollars.
    # So this mirrors the currency gates the charge path itself applies, rather than only
    # checking that the currency exists.
