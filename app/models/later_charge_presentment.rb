...
class LaterChargePresentment < ApplicationRecord
  ...
  private
    ...
    def presentment_currency_is_chargeable
      ...
      unless StripeChargeProcessor.quoted_currency_supported?(currency)
        errors.add(:presentment_currency, "cannot be charged in minor units by Stripe")
      end
      if StripeChargeProcessor.whole_unit_presentment_currency?(currency) && presentment_price_cents.to_i % 100 != 0
        errors.add(:presentment_price_cents, "must be a whole currency unit")
      end
    end
end
