...
class ChapterFile < ApplicationRecord
  include S3Retrievable, ExternalId, JsonData, Deletable, CdnDeletable

  VALID_FILE_TYPE_REGEX = /\A.+\.vtt\z/
  ...
  def schedule_calculate_size
    ChapterFileSizeWorker.perform_in(5.seconds, id)
  end

  def has_alive_duplicate_files?
    ChapterFile.alive.where(url:).exists?
  end

  private
    def ensure_valid_file_type
      return if url.match?(VALID_FILE_TYPE_REGEX)
      errors.add(:base, "Chapter file type not supported. Please upload only WebVTT files with extension .vtt.")
    end
end
