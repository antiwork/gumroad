...
class Api::V2::RefundPoliciesController < Api::V2::BaseController
  REFUND_PERIOD_ALLOWED_VALUES = %w[7 14 30 183].freeze
  ...
  NO_REFUNDS_NOT_ALLOWED_MESSAGE = "refund_period \"none\" is only allowed for physical products."
  ...
  def update
    ...
    if params[:refund_period].to_s == "none"
      return render_response(false, message: "Refund period must be one of: #{REFUND_PERIOD_ALLOWED_VALUES.join(', ')}.")
    end
  ...
  private
    ...
    def serialized_refund_period(refund_policy)
      days = refund_policy.effective_max_refund_period_in_days
      days.zero? ? "none" : days.to_s
    end
end
