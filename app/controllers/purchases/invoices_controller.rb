...
class Purchases::InvoicesController < ApplicationController
  ...
  def create
    if params["vat_id"].present? && !@purchase.successful?
      return render json: { success: false, message: "Your purchase has not been completed by PayPal yet. Please try again soon." }
    end
    ...
    business_vat_id = validate_vat_id(raw_vat_id, chargeable)
    ...
    begin
      ...
      if business_vat_id
        notice = tax_refund_notice(chargeable)
        message << " " << notice
      end
