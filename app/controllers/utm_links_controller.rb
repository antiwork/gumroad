...
class UtmLinksController < Sellers::BaseController
  ...
  def new
    ...
    props = UtmLinkPresenter.new(seller: current_seller).new_page_react_props(copy_from: params[:copy_from])
    props[:form_dynamic] = {
      new_permalink: InertiaRails.optional { UtmLink.generate_permalink },
    }

    render inertia: "Analytics/UtmLinks/New", props: props
  end
  ...
  private
    ...
    def save_utm_link
      ...
      notice_message = @utm_link ? "Link updated!" : "Link created!"
      redirect_to utm_links_dashboard_path, notice: notice_message, status: :see_other
    rescue ActiveRecord::RecordInvalid => e
      error = e.record.errors.first
      error_key = error.attribute.to_s
      if @utm_link
        redirect_to edit_dashboard_utm_link_path(@utm_link.external_id), inertia: { errors: { error_key => [error.message] } }, alert: error.message
      else
        redirect_to new_dashboard_utm_link_path(copy_from: params[:copy_from]), inertia: { errors: { error_key => [error.message] } }, alert: error.message
      end
    end
