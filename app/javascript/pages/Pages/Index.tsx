  ...
  return (
    <>
      ...
      <section className="grid gap-4 p-4 md:p-8">
        {pages.length === 0 ? (
          // With no pages beyond the profile there's no tree to manage yet, so
          // show the standard empty state instead of a one-row list.
          <Placeholder>
            <h2>No pages yet</h2>
            ...
            {pages.map((page) => (
              <Row key={page.slug} role="listitem" className="sm:pl-8">
                ...
                <RowActions>
                  <NavigationButton
                    size="icon"
                    href={Routes.edit_page_path(page.slug)}
