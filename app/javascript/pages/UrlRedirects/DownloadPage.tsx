...
type DownloadPageProps = LayoutProps & {
  content: {
...
const DownloadPage = () => {
  ...
  const { setData: setAudioDurationsPollingData } = useForm<{ audio_duration_file_ids: string[] }>({
    audio_duration_file_ids: [],
  });
  ...
  const unprocessedAudioIdsRef = React.useRef<string[]>([]);
  React.useEffect(() => {
    unprocessedAudioIdsRef.current = unprocessedAudioIds;
  }, [unprocessedAudioIds]);
  ...
  React.useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      ...
      const idsToFetch = unprocessedAudioIdsRef.current.slice(0, MAX_AUDIO_IDS_PER_FETCH);
      if (idsToFetch.length === 0) return;

      setAudioDurationsPollingData("audio_duration_file_ids", idsToFetch);
      isFetchingAudioDurationsRef.current = true;
      router.reload({
        only: ["audio_durations"],
        data: { audio_duration_file_ids: idsToFetch },
        preserveState: true,
        preserveScroll: true,
        replace: true,
        onFinish: () => {
          isFetchingAudioDurationsRef.current = false;
        },
      });
