...
export type OfferCode = {
    ...
    required_product_ownership_months: number | null;
    fallback_discount: { type: "cents" | "percent"; value: number } | null;
};
                                ...
                                {offerCodes.map((offerCode) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td>{`${validAt ? `${formatDate(validAt)} - ` : ""}${expiresAt ? formatDate(expiresAt) : "No end date"
                                                }`}</td>
            ...
            save={asyncVoid(async (offerCode) => {
                ...
                try {
                    ...
                    const { offer_codes: offerCodes, pagination } = await updateDiscount(selectedOfferCode.id, {
                        ...
                        requiredProductOwnershipMonths: offerCode.required_product_ownership_months,
                        fallbackDiscount: offerCode.fallback_discount,
                    });
                    resetQueryState();
...
const Form = ({
    ...
    const [hasOwnershipDuration, setHasOwnershipDuration] = React.useState(!!offerCode?.required_product_ownership_months);
    const [ownershipMonths, setOwnershipMonths] = React.useState<{ value: number | null; error?: boolean }>({
        value: offerCode?.required_product_ownership_months ?? null,
    });
    const [fallbackDiscount, setFallbackDiscount] = React.useState<InputtedDiscount>(
        offerCode?.fallback_discount ?? {
            type: "percent",
            value: 0,
        },
    );
    ...
    const handleSubmit = () => {
        ...
        const isRequiredProductInvalid = hasRequiredProduct && requiredProductId.value === null;
        const isOwnershipMonthsInvalid = hasOwnershipDuration && ownershipMonths.value === null;
        const isFallbackDiscountInvalid = hasOwnershipDuration && fallbackDiscount.value === null;
        ...
        const hasValidationErrors =
            ...
            isRequiredProductInvalid ||
            isOwnershipMonthsInvalid ||
            isFallbackDiscountInvalid;
        ...
        if (hasValidationErrors) {
            ...
            setRequiredProductId((prev) => ({ ...prev, error: isRequiredProductInvalid }));
            setOwnershipMonths((prev) => ({ ...prev, error: isOwnershipMonthsInvalid }));
            setFallbackDiscount((prev) => ({ ...prev, error: isFallbackDiscountInvalid }));
        ...
        save({
            ...
            required_product_ownership_months: hasOwnershipDuration ? ownershipMonths.value : null,
            fallback_discount: hasOwnershipDuration ? { type: fallbackDiscount.type, value: fallbackDiscount.value ?? 0 } : null,
        });
    };
                            ...
                            summary={
                                <label>
                                    <input
                                        ...
                                        onChange={(evt) => setHasRequiredProduct(evt.target.checked)}
                                    />
                                    Require previous product purchase (for upgrades)
                                </label>
                            }
                        >
                            <div className="dropdown">
                                <fieldset className={cx({ danger: requiredProductId.error })}>
                                    <legend>
                                        <label htmlFor={`${uid}requiredProduct`}>Which product must they already own?</label>
                                    </legend>
                                    <Select
                                        ...
                                        value={
                                            requiredProductId.value
                                                ? {
                                                    id: requiredProductId.value,
                                                    label:
                                                        products.find((p) => p.id === requiredProductId.value)?.name ?? "Unknown product",
                                                }
                                                : null
                                        }
                                        isClearable
                                        placeholder="Select previous version or product"
                                        onChange={(selected) => {
                                ...
                                <Details
                                    ...
                                    open={hasOwnershipDuration}
                                    summary={
                                        <label>
                                            <input
                                                type="checkbox"
                                                role="switch"
                                                checked={hasOwnershipDuration}
                                                onChange={(evt) => setHasOwnershipDuration(evt.target.checked)}
                                            />
                                            Offer different discounts based on purchase date
                                        </label>
                                    }
                                >
                                    <div className="dropdown">
                                        <fieldset className={cx({ danger: ownershipMonths.error })}>
                                            <legend>
                                                <label htmlFor={`${uid}ownershipMonths`}>
                                                    Ownership duration threshold (months)
                                                </label>
                                            </legend>
                                            <NumberInput
                                                value={ownershipMonths.value}
                                                onChange={(value) => {
                                                    if (value === null || value >= 0) setOwnershipMonths({ value });
                                                }}
                                            >
                                                {(props) => (
                                                    <input
                                                        id={`${uid}ownershipMonths`}
                                                        placeholder="6"
                                                        aria-invalid={ownershipMonths.error}
                                                        {...props}
                                                    />
                                                )}
                                            </NumberInput>
                                            <p className="help-text">
                                                Recent buyers (owned for less than this duration) receive the main discount above. Long-time owners get the discount below.
                                            </p>
                                        </fieldset>
                                        <fieldset>
                                            <legend>Discount for long-time owners</legend>
                                            <DiscountInput
                                                discount={fallbackDiscount}
                                                setDiscount={setFallbackDiscount}
                                                currencyCode={currencyCode}
                                            />
                                        </fieldset>
                                    </div>
