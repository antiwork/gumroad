...
const CollaboratorForm = ({
  ...
  const submitForm = (acknowledgement?: typeof WITH_CONFIRMED_ACKNOWLEDGEMENT) => {
    ...
    if (
      ...
      commissionErrorMessage = `Hoa hồng cộng tác viên phải từ ${pageMetadata.max_percent_commission}% trở xuống`;
    ...
    if (!isEditPage) {
      const emailError =
        form.data.email?.length === 0
          ? "Phải cung cấp email cộng tác viên"
    ...
    if (enabledProducts.length === 0) {
      showAlert("Phải chọn ít nhất một sản phẩm", "error");
      return;
    }
  ...
  return (
    <Layout
      title={isEditPage ? pageMetadata.title : "Cộng tác viên mới"}
      headerActions={
        <>
          ...
          <WithTooltip position="bottom" tip={collaboratorsDisabledReason}>
            ...
            >
              {form.processing ? "Đang lưu..." : isEditPage ? "Lưu thay đổi" : "Thêm cộng tác viên"}
            </Button>
          </WithTooltip>
        </>
      }
    >
      <form>
        <section className="p-8!">
          <header>
            {isEditPage ? <h2>Sản phẩm</h2> : null}
            <div>Cộng tác viên sẽ nhận được một phần doanh thu từ các sản phẩm đã chọn.</div>
            ...
            <fieldset className={cx({ danger: form.errors.email })}>
              ...
              <div className="input">
                <input
                  ...
                  placeholder="Email tài khoản Gumroad của cộng tác viên"
                  onChange={(evt) => {
            ...
            <Table>
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell>
                    <label htmlFor="all-products-cut">Tất cả sản phẩm</label>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <label>
                      <input
                        ...
                        disabled={!form.data.apply_to_all_products}
                      />
                      Hiển thị là người đồng sáng tạo
                    </label>
                  </TableCell>
                </TableRow>
                        ...
                        {product.has_another_collaborator || product.has_affiliates ? (
                          <small>
                            {product.has_another_collaborator
                              ? "Đã có cộng tác viên"
                              : "Chọn sản phẩm này sẽ xóa tất cả các đơn vị liên kết của nó."}
                          </small>
                        ) : null}
                      </TableCell>
                      <TableCell>
          ...
          <label>
            ...
            />
            Hiển thị sản phẩm chưa xuất bản và không đủ điều kiện
          </label>
        </section>
        <Modal
          open={isConfirmationModalOpen}
          title="Xóa đơn vị liên kết?"
          onClose={() => setIsConfirmationModalOpen(false)}
        >
          <h4 className="mb-3">
            Các đơn vị liên kết sẽ bị xóa khỏi các sản phẩm sau:
            <ul>
            ...
            {listedProductsWithAffiliatesCount < productsWithAffiliates.length ? (
              <span>{`và ${productsWithAffiliates.length - listedProductsWithAffiliatesCount} người khác.`}</span>
            ) : null}
          </h4>
