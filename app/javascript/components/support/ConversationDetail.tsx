...
function MessageListItem({ message, isLastMessage }: { message: Message; isLastMessage: boolean }) {
  ...
  return (
    <Row role="listitem" className="items-stretch gap-0 p-0">
      ...
      >
        <img className={classNames("user-avatar w-9!", { "border-none": image === pinkIcon })} src={image} />
        <div className={classNames("font-bold", { "flex-1": isExpanded })}>
          {message.role === "user" ? (currentSeller?.name ?? "You") : message.staffName || startCase(message.role)}
        </div>
        <div className={classNames("ml-2 line-clamp-1 min-w-0 flex-1", { hidden: isExpanded })}>
      ...
      <RowActions
        className="peer cursor-pointer p-4 pl-0 peer-hover:bg-active-bg hover:bg-active-bg"
        onClick={() => setIsExpanded((v) => !v)}
      >
          ...
          {attachments.length > 0 ? (
            <Rows role="list" className="mt-4 w-full max-w-[500px]">
              {attachments.map((attachment) => (
                <Row
                  role="listitem"
                  className={classNames({ "p-0": attachment.contentType?.startsWith("image/") })}
                  key={attachment.url}
                >
