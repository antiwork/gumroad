...
// `currencySelector` defaults off for the same reason it does on PoweredByFooter: this footer also
// runs the blog and the API docs, where no price is on screen. Discover opts in.
export const HomeFooter = ({ currencySelector = false }: { currencySelector?: boolean | undefined } = {}) => {
    ...
    <div className="flex flex-col justify-between gap-16 bg-black px-8 py-16 leading-relaxed text-white lg:flex-row lg:px-[4vw] lg:py-24">
      ...
      <div className="flex w-full max-w-3xl flex-col gap-16">
        ...
        <div className="flex flex-col gap-6">
          ...
          {currencySelector ? <FooterCurrencySelector detectedCurrency={detectedCurrency ?? null} /> : null}
        </div>
      </div>
    </div>
  );
