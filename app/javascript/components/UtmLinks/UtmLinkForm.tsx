...
export const UtmLinkForm = () => {
  const { form_static, utm_link } = cast<UtmLinkNewPageProps | UtmLinkEditPageProps>(usePage().props);
  const isEditing = utm_link?.id !== undefined;
  const isDuplicating = utm_link !== null && utm_link.id === undefined;
  const uid = React.useId();

  const initialTitle = isDuplicating ? duplicatedTitle(utm_link.title) : (utm_link?.title ?? "");
  const initialDestination = utm_link?.destination_option?.id
    ? (form_static.destination_options.find((o) => o.id === assertDefined(utm_link.destination_option).id) ?? null)
    : null;

  const { protocol: shortUrlProtocol, host, pathname } = new URL(utm_link?.short_url ?? form_static.short_url);
  ...
  const generateNewPermalink = () => {
    router.reload({
      ...
      onSuccess: (data) => {
        ...
        form.setData("permalink", assertDefined(props.form_dynamic?.new_permalink));
      },
        ...
        <section className="p-4! md:p-8!">
          ...
          >
            ...
            <Select
              ...
              options={form_static.destination_options}
              value={destination}
              isMulti={false}
              isDisabled={isEditing}
              onChange={(option) => {
                setDestination(
                  option ? (form_static.destination_options.find((o) => o.id === option.id) ?? null) : null,
                );
                form.clearErrors();
              }}
            />
          ...
          >
            <fieldset className={cx({ danger: getFieldError("utm_source") })}>
              ...
              <UtmFieldSelect
                ...
                baseOptionValues={form_static.utm_fields_values.sources}
            ...
            <fieldset className={cx({ danger: getFieldError("utm_medium") })}>
              ...
              <UtmFieldSelect
                id={`${uid}-medium`}
                placeholder="email"
                baseOptionValues={form_static.utm_fields_values.mediums}
          ...
          <fieldset className={cx({ danger: getFieldError("utm_campaign") })}>
            ...
            <UtmFieldSelect
              ...
              baseOptionValues={form_static.utm_fields_values.campaigns}
          ...
          <fieldset className={cx({ danger: getFieldError("utm_term") })}>
            ...
            <UtmFieldSelect
              ...
              baseOptionValues={form_static.utm_fields_values.terms}
          ...
          <fieldset className={cx({ danger: getFieldError("utm_content") })}>
            ...
            <UtmFieldSelect
              ...
              baseOptionValues={form_static.utm_fields_values.contents}
