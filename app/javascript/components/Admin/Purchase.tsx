...
export type Purchase = PurchaseStatesInfo & {
  ...
  refund_policy:
    ...
    | null;
  can_contact: boolean;
  gift: { is_sender_purchase: boolean; other_purchase_id: number; other_email: string; note: string } | null;
  successful: boolean;
...
const AdminPurchase = ({ purchase }: { purchase: Purchase }) => (
  <div className="grid gap-4 rounded border border-border bg-background p-4">
      ...
      <dl>
        ...
        {purchase.card ? (
          <>
            ...
            <dt>IP Address</dt>
            <dd>
              <Link href={Routes.admin_search_purchases_path({ query: purchase.ip_address })}>
                {purchase.ip_address}
              </Link>
            </dd>
    ...
    {purchase.gift ? (
      <>
        <hr />
        {purchase.gift.is_sender_purchase ? (
          <>
            <details>
              <summary>
              ...
              <dl>
                <dt>For</dt>
                <dd>{purchase.gift.other_email}</dd>

                <dt>Note</dt>
                <dd>{purchase.gift.note}</dd>

                <dt>Receiver purchase id</dt>
                <dd>
                  <Link href={Routes.admin_purchase_path(purchase.gift.other_purchase_id)}>
                    {purchase.gift.other_purchase_id}
                  </Link>
                </dd>
              </dl>
            </details>
            ...
            <details>
              ...
              <Form
                url={Routes.update_giftee_email_admin_purchase_path(purchase.id)}
                method="POST"
