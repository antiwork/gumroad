        ...
        <div>
          ...
          !((state.results?.total ?? 0) < (state.offset ?? 1) + (state.results?.products.length ?? 0)) ? (
            <div className="mt-8 w-full text-center">
              <button className="inline-flex items-center justify-center gap-2 cursor-pointer no-underline text-base leading-[1.4] px-4 py-3 rounded border [border-color:rgb(var(--color)/var(--border-alpha))] bg-transparent text-current transition-transform ease-out duration-150 hover:-translate-x-1 hover:-translate-y-1 hover:shadow-[0.25rem_0.25rem_0_currentColor]"
              onClick={() => dispatchAction({ type: "load-more" })}>
                Load more
              </button>
            </div>
          ) : null}
