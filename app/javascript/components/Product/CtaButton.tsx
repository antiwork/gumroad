...
export const CtaButton = React.forwardRef<HTMLAnchorElement, Props>(
  ({ product, purchase, discountCode, selection, label, onClick, showInstallmentPlanNotes = false }, ref) => {
    ...
    return (
      <>
        <NavigationButton ref={ref} href={url.toString()} color="accent" className="whitespace-nowrap" {...buttonCommonProps}>
          {label ??
            (purchase
              ? "Purchase again"
              : product.recurrences
        ...
        {product.installment_plan && product.installment_plan.number_of_installments > 1 ? (
          <>
            <NavigationButton color="black" href={urlWithInstallments.toString()} className="whitespace-nowrap" {...buttonCommonProps}>
