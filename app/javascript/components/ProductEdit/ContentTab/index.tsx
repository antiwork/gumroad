...
export const ContentTabContent = ({ selectedVariantId }: { selectedVariantId: string | null }) => {
  ...
  // Create uploader early so it's available for uploadFiles function
  ...
  const uploader = evaporateUploader;
          ...
          <S3UploadConfigProvider value={s3UploadConfig}>
            <>
                            ...
                            <Card>
                              {product.native_type === "membership" ? (
                                <CardContent asChild details>
                                  <details>
                                    <summary
                                      className="grow grid-flow-col grid-cols-[1fr_auto] before:col-start-2"
                                      inert
                                    >
                                      Membership
                                    </summary>
              ...
              {confirmingDeletePage !== null ? (
                ...
                >
                  Are you sure you want to delete the page "{titleWithFallback(confirmingDeletePage.title)}"? Existing
                  customers will lose access to this content. This action cannot be undone.
                </Modal>
              ) : null}
              {editor ? (
                <>
                  <Modal
                    open={showInsertPostModal}
                    onClose={() => setShowInsertPostModal(false)}
                    title="Insert Twitter post"
                  >
                    <EmbedMediaForm
                      type="twitter"
                      onClose={() => setShowInsertPostModal(false)}
