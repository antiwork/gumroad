...
module NotifyOfSaleHeaders
  ...
  def set_notify_of_sale_headers(is_preorder:)
    ...
    if @product.is_recurring_billing
      @mail_heading = @subject = if @purchase.try(:is_upgrade_purchase)
                        "A subscriber has upgraded their subscription for #{@product.name} and was charged #{formatted_price}"
                      elsif @purchase.try(:is_recurring_subscription_charge)
                        "New recurring charge for #{@product.name} of #{formatted_price}"
                      else
                        "You have a new subscriber for #{@product.name} of #{formatted_price}"
                      end
    elsif is_preorder
      ...
      @subject = if formatted_price.present?
        "New sale of #{@product.name} for #{formatted_price}"
      else
        "New sale of #{@product.name}"
      end
    end
