...
class PaginatedInstallmentsPresenter
  ...
  def props
    if query.blank?
      installments = Installment.includes(:installment_rule, :alive_product_files, :blasts).all
      ...
      installments = es_search.records.includes(:alive_product_files, :blasts).load
