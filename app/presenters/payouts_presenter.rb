  ...
  def props
    {
      ...
      instant_payout: seller.instant_payouts_supported? ? {
        payable_amount_cents: StripePayoutProcessor.instantly_payable_balance_amount_cents(seller),
        payable_balances: StripePayoutProcessor.instantly_payable_balances(seller).sort_by(&:date).reverse.map do |balance|
          {
