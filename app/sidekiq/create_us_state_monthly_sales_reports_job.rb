...
class CreateUsStateMonthlySalesReportsJob
  ...
  def perform(subdivision_code, month, year)
    ...
    begin
      ...
      NotificationWorker.perform_async("payments", "US Sales Tax Reporting", "#{subdivision.name} reports for #{year}-#{month} are ready:\nGumroad format: #{s3_signed_url}", { "color" => "green" })
    ensure
      temp_file.close
    end
  end
