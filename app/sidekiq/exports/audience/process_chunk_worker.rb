...
class Exports::Audience::ProcessChunkWorker
  ...
  private
    def process_chunk
      members = AudienceMember.where(id: @chunk.audience_member_ids).select(:id, :email, :min_created_at)
      # Convert TimeWithZone to string for YAML serialization
      audience_data = members.map { |member| [member.email, member.min_created_at.to_s] }
