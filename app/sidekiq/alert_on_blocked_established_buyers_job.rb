...
class AlertOnBlockedEstablishedBuyersJob
  ...
  # at, and has to be wide enough to still find them weeks after they stopped trying.
  FAILURE_LOOKBACK = 7.days
  ...
  private
      ...
      candidates.each_slice(HISTORY_COUNT_BATCH) do |batch|
        ...
        failures.each do |purchase|
          ...
          stranded << {
            ...
            settled_purchases: settled[purchase.email],
            blocked_at: block.blocked_at,
            failed_at: purchase.created_at,
            attempts: attempts[purchase.email],
          }
        end
      end
    ...
    # no fingerprint to key on — measured 0 of 3,414 such failures over 7 days carried one. Email is
    # what the buyer actually presented, and it is what the seller and support search by.
    ...
    #
    # Keyed on the address as stored, which Purchase#downcase_email guarantees is already lowercase
    # — so every hash in this job can key on purchase.email directly.
    ...
    # Drops buyers carrying a chargeback or a full refund on ANY purchase, which the counts above
    # only excluded from the total. #buyer_has_clean_payment_history? gets this for free by counting
    # per card; counting per email means a buyer with three clean purchases and a fourth charged
    # back would otherwise read as established. A disputed buyer is exactly who a block is for.
    def reject_disputed(settled)
      disputed = Purchase.where(email: settled.keys)
                         .where("purchases.stripe_refunded = 1 OR purchases.chargeback_date IS NOT NULL")
                         .distinct
                         .pluck(:email)
                         .to_set
    ...
    # Drops buyers whose newest settled purchase postdates their newest blocked failure: eligibility
    # is "stranded now", and a failure row alone cannot say that. A buyer who got through afterwards
    # either was never fully blocked or already found a way round it.
    #
    ...
    def reject_recovered(failures)
      ...
      failures.reject do |purchase|
        bought_at = newest_success[purchase.email]
    ...
    def message_for(scan)
      ...
      [
        ...
        *lines,
        ...
        "",
        "Platform blocks do not expire, so these can predate the checkout by years. Review with `Onetime::ClearMistakenBuyerBlocks` or unblock individually; see gumroad-private#1640.",
      ].compact.join("\n")
    end
    ...
    def line_for(entry)
      ...
      "• #{new_marker}#{entry[:email]} — #{entry[:settled_purchases]} settled purchases, " \
        "blocked since #{entry[:blocked_at].to_date}, last tried #{entry[:failed_at].to_date}#{attempts}"
    end
