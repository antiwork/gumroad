...
class UpdateTaxRatesJob
  ...
  def perform
    ...
    summary_rates.select { |summary_rate|  Compliance::Countries::EU_VAT_APPLICABLE_COUNTRY_CODES.include?(summary_rate.country_code) || tax_jar_country_codes_map[summary_rate.country_code].present? }.each do |summary_rate|
      ...
      if zip_tax_rate
        if zip_tax_rate.combined_rate != summary_rate.average_rate.rate
          NotificationService.send_notification("payments", "VAT Rate Updater", "VAT rate has changed for #{country_code} from #{zip_tax_rate.combined_rate} to #{summary_rate.average_rate.rate}", "green")
          zip_tax_rate.combined_rate = summary_rate.average_rate.rate
          zip_tax_rate.save!
        end
      else
        NotificationService.send_notification("payments", "VAT Rate Updater", "Creating missing tax rate for #{country_code} with rate of #{summary_rate.average_rate.rate}", "green")
    ...
    summary_rates.select { |summary_rate| summary_rate.country_code == Compliance::Countries::USA.alpha2 }.each do |summary_rate|
      ...
      if zip_tax_rate
        if zip_tax_rate.combined_rate != summary_rate.average_rate.rate || is_seller_responsible != zip_tax_rate.is_seller_responsible
          NotificationService.send_notification("payments", "VAT Rate Updater", "US Sales Tax rate for state #{state} has changed. Rate was #{zip_tax_rate.combined_rate}, now it's #{summary_rate.average_rate.rate}. is_seller_responsible was #{zip_tax_rate.is_seller_responsible}, now it's #{is_seller_responsible}", "green")
          zip_tax_rate.combined_rate = summary_rate.average_rate.rate
          zip_tax_rate.is_seller_responsible = is_seller_responsible
          zip_tax_rate.save!
        end
      else
        NotificationService.send_notification("payments", "VAT Rate Updater", "Creating US Sales Tax rate for state #{state} with rate of #{summary_rate.average_rate.rate} and is_seller_responsible #{is_seller_responsible}", "green")
        ZipTaxRate.create!(
          country: "US",
          state:,
          zip_code: nil,
    ...
    summary_rates.select { |summary_rate| summary_rate.country_code == Compliance::Countries::CAN.alpha2 }.each do |summary_rate|
      ...
      if zip_tax_rate
        if zip_tax_rate.combined_rate != summary_rate.average_rate.rate
          NotificationService.send_notification("payments", "VAT Rate Updater", "Canada Sales Tax rate for province #{province} has changed. Rate was #{zip_tax_rate.combined_rate}, now it's #{summary_rate.average_rate.rate}.", "green")
          zip_tax_rate.combined_rate = summary_rate.average_rate.rate
          zip_tax_rate.save!
        end
      else
        NotificationService.send_notification("payments", "VAT Rate Updater", "Creating Canada Sales Tax rate for province #{province} with rate of #{summary_rate.average_rate.rate}", "green")
        ZipTaxRate.create!(
          country: "CA",
          state: province,
          zip_code: nil,
