...
class PerformPayoutsUpToDelayDaysAgoWorker
  ...
  # This job now only reads seller ids and hands slices of them to
  # PerformPayoutsForUserSliceWorker, so it no longer needs a budget big enough for the
  # whole cohort's eligibility checks — just for the id reads, which are batched in
  # Payouts.holding_balance_user_ids and take minutes, not hours. Keeping the cap tight
  # means a pathologically slow read fails and retries instead of running until the
  # worker is recycled.
  ...
  def perform(payout_processor_type, bank_account_types = nil)
    ...
    # Count this job as an in-flight payout batch so the deploy pipeline holds production
    # deploys while payouts are running (see HealthcheckController#payouts and
    ...
    with_payout_batch_in_flight do
      ...
      # retries exhaust and every seller in the bucket goes unpaid for the week (4 incidents:
      # #434, #870, #955, and the 2026-07-08 UK batch). Payouts are a weekly batch job, not a
      # user-facing request — a long-running query here is expected.
