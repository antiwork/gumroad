...
module StripeMerchantAccountManager
  ...
  def self.create_account(user, passphrase:, from_admin: false, notify: true)
    ...
    if merchant_account.present? && merchant_account.charge_processor_alive_at.nil?
      ...
      # tax-ID rejections (placeholder values like 123456789 that Stripe disallows) are
      # expected seller-input errors: the seller sees Stripe's message inline on the payments
      # settings page (a payout note is also recorded below for bank rejections), and the
      # sync path (update_bank_account) already treats bank rejections as expected without
      # alerting. Don't page Sentry for them — only unexpected failures should alert.
      ErrorNotifier.notify(e) unless bank_account_invalid_error?(e) || tax_id_invalid_error?(e)
    end
