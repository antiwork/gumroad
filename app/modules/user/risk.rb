  ...
  def enable_sellers_other_accounts(transition)
    ...
    if payment_address.present?
      User.where(payment_address:).where.not(id:).find_each do |user|
        ...
        Rails.logger.error("enable_sellers_other_accounts: Failed to mark user #{user.id} compliant (payment_address): #{e.message}")
      end
    end
    ...
    if fingerprints.present?
      User.joins(:bank_accounts)
        ...
        Rails.logger.error("enable_sellers_other_accounts: Failed to mark user #{user.id} compliant (fingerprint): #{e.message}")
      end
    end
  end
