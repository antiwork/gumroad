...
class Purchase
  ...
  def refund_purchase!(flow_of_funds, refunding_user_id, stripe_refund = nil, is_for_fraud = false,
    ...
    # `funds_refunded` is canonical US dollar cents in both branches, which is what makes the
    # comparison against `total_transaction_cents` below a canonical-to-canonical one.
    #
    # On an ordinary (non-buyer-currency) purchase, `flow_of_funds.issued_amount` is already
    # canonical US dollars — that is the leg Gumroad issues the charge in — so the fallback is
    # safe to use directly.
    #
    # On a buyer-currency purchase, `flow_of_funds.issued_amount` is denominated in the buyer's
    # presentment currency, NOT in US dollars, so it must never be booked as canonical cents.
    # The block above guarantees that: either the caller passed the canonical figure, or
    # `derive_presentment_refund_from_flow_of_funds` converts the presentment amount into a
    # separate canonical US dollar value (and refuses, rather than guessing, when the
    # flow-of-funds currency does not match the presentment currency). Either way
    # `canonical_gross_refund_cents` is set by the time we get here, so the fallback below is
    # unreachable for a presentment refund.
