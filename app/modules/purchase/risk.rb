...
module Purchase::Risk
  ...
  private
    ...
    def check_for_past_blocked_guids
      ...
      # Do not suggest another browser or connection here. This block follows the person, not the
      # device: we block the browser together with the email address, so switching browser, network
      # or payment method changes nothing, and the old wording sent blocked buyers around that loop
      # until they gave up (gumroad-private#1480). Only support can look at the block and lift it.
