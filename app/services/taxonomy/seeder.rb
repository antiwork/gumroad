...
# Applies the taxonomy tree to whatever environment it runs in, by loading the seed file rather
# than restating the tree, so there is no second copy to drift.
#
...
#
# Blast radius is wider than the name suggests, and this runs on every deploy: the seed file's last
# line applies TaxonomyAttributeDefinitions, which updates TaxonomyAttribute rows in place and
# DEACTIVATES any active row missing from that registry. An attribute hand-edited in a console is
# reverted on the next deploy — the registry is the source of truth.
class Taxonomy::Seeder
  SEED_PATH = "db/seeds/010_development_staging_test/taxonomy_create.rb"

  # Returns the number of taxonomies created. Safe to run on every deploy: every call in the seed
  # file is an idempotent find_or_create_by!, so a no-op run costs a few hundred SELECTs.
  #
  # The count is a before/after difference, so a concurrent delete would skew it. It is only ever
  # read as a deploy-log signal, and the migration lock keeps concurrent deploys out.
  def perform
