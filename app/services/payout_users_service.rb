  ...
  def create_payments
    ...
    user_ids.each do |user_id|
      ...
      if payout_type == Payouts::PAYOUT_TYPE_INSTANT
        ...
        up_to_date = instantly_payable_balances.sort_by(&:created_at).last.date.to_s
      end

      payment, payment_errors = Payouts.create_payment(up_to_date, processor_type, user, payout_type:)
