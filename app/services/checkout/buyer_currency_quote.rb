...
class Checkout::BuyerCurrencyQuote
  ...
  ChargeQuote = Struct.new(:seller,
                           ...
                           :rounding_delta_cents,
                           :fx_rate,
                           # Stripe's own settlement rate for this charge's currency pair
                           # (see StripeFxQuote::Quote#base_rate). Carried per charge because
                           # every charge mints its own quote against its own Stripe account,
                           # so the two rates are not shared across a multi-seller cart.
                           :base_rate,
                           :stripe_fx_quote_id,
                           :stripe_fx_quote_expires_at,
  ...
  def self.verify!(token:, seller:, merchant_account:, currency:, canonical_total_cents:, canonical_line_items:)
    ...
    Result.new(
      ...
      base_rate: charge_payload["base_rate"].present? ? BigDecimal(charge_payload["base_rate"]) : nil,
      ...
      charges = charge_quotes.map do |charge_quote|
        {
          ...
          base_rate: charge_quote.base_rate&.to_s("F"),
        }
      end
