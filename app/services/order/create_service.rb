        ...
        purchase_params = build_purchase_params(
          ...
          common_params
            .except(
              ...
              # Client-confirm payment-surface hints consumed by
              # Order::PreparePaymentIntentService (which receives the order params directly from
              # the controller); they are not Purchase attributes, so letting them through raises
              # ActiveModel::UnknownAttributeError in Purchase::CreateService#build_purchase.
              :payment_element_mount_currency,
              :payment_element_mounted_payment_method_types
            )
