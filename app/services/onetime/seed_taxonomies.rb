...
#
# Blast radius is wider than the name suggests, and this now runs on every deploy: the seed file's
# last line applies TaxonomyAttributeDefinitions, which updates TaxonomyAttribute rows in place and
# DEACTIVATES any active row missing from that registry. An attribute hand-edited in a console will
# be reverted on the next deploy — the registry is the source of truth. Those rows log their own
# CREATE/UPDATE/DEACTIVATE lines to the deploy output.
module Onetime
  class SeedTaxonomies
    ...
    # read as a deploy-log signal, and the migration lock keeps concurrent deploys out; a console
    # session deleting taxonomies mid-deploy would make it undercount.
    def process
