...
module Ai::StoreAgentApiCatalog
  ...
  ENDPOINTS = [
    ...
    ep("update_user_custom_html", :patch, "/user/custom_html", "Do not call this to author a new profile page in one turn — a whole page does not fit in one tool call. Use stage_profile_section, one section per turn, then propose_profile_page. This call replaces the creator's ENTIRE profile custom HTML with a new page. Destructive: anything not included in custom_html is lost, and the page becomes the whole storefront — so it must show everything the store shows: render all products (with working links) dynamically from the gumroad-data JSON injected into every served page — and when products_total exceeds the products array's length, or posts_total exceeds the posts array's length, that list is capped, so the page must show the count for that section rather than silently omitting the rest — plus the creator's name and bio via data-gumroad-field elements the server fills at render time (they are NOT in the JSON). propose_profile_page is what calls this after the draft is complete. To change part of an existing page, use edit_user_custom_html.", scope: "edit_profile", params: %w[custom_html], requires_read: "get_user_custom_html"),
